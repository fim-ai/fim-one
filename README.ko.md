...
*AI 기반 커넥터 허브 — 한 시스템에 Copilot으로 임베드하거나 모든 시스템을 허브로 연결하세요.*
...
- **에이전트 관리** — 바인딩된 모델, 도구, 지침으로 에이전트를 생성, 구성, 게시. 에이전트별 실행 모드(Standard/Planner) 및 온도 제어. 선택적 `discoverable` 플래그는 CallAgentTool을 통한 LLM 자동 발견을 활성화.
- **에이전트 스킬 시스템** — 에이전트를 위한 점진적 지침 로딩. 스킬(SOP, 스크립트, 도메인 지식)은 시스템 프롬프트에서 경량 스텁(~15 토큰)으로 참조되며, 에이전트는 `read_skill(name)`을 호출하여 필요 시 전체 콘텐츠를 로드하므로 대화당 지침 토큰 비용을 ~80% 감소. 에이전트별 `compact_instructions` 필드로 사용자 정의 ContextGuard 압축 전략 지원.
- **마켓플레이스(Shadow Market Org)** — 내장 Market org는 리소스 공유를 위한 보이지 않는 백엔드 엔티티로 작동. 리소스는 마켓플레이스 브라우징을 통해 발견되고 명시적으로 구독(풀 모델) — 자동 가입 멤버십 없음. 마켓플레이스에 게시하려면 항상 검토 필요.
- **리소스 구독** — 사용자가 마켓플레이스에서 공유 리소스를 브라우징하고 구독. UI 또는 API를 통해 구독/구독 취소. 모든 리소스 유형(에이전트, 커넥터, 지식 기반, MCP 서버, 스킬, 워크플로우)이 마켓플레이스 게시 및 구독 관리 지원.
- **관리자 패널** — 시스템 통계 대시보드(사용자, 대화, 토큰, 모델 사용 차트, 에이전트별 토큰 분석), 커넥터 호출 메트릭(성공률, 지연시간, 호출 수), 검색/페이지네이션이 포함된 사용자 관리, 역할 전환, 비밀번호 재설정, 계정 활성화/비활성화, 도구별 활성화/비활성화 제어.
- **첫 실행 설정 마법사** — 첫 실행 시 포털은 관리자 계정(사용자명, 비밀번호, 이메일) 생성을 안내. 이 일회성 설정이 로그인 자격증명이 되며 설정 파일 불필요.
- **개인 센터** — 사용자별 전역 시스템 지침, 모든 대화에 적용.
- **언어 선호도** — 사용자별 언어 설정(auto/en/zh)으로 모든 LLM 응답을 선택한 언어로 지정.
