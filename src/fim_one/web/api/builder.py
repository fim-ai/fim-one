...
async def create_builder_session(
    ...
    if body.target_type == "connector":
        ...
        instructions = (
            ...
            f"Safety rules (the user's existing work is precious):\n"
            f"- NEVER call connector_delete_action more than twice in a row without the user \n"
            f"  EXPLICITLY asking to wipe/rebuild/reset. If the user seems to want a rebuild, \n"
            f"  first list what you plan to delete and ask them to confirm in the chat.\n"
            f"- When using connector_import_openapi, ALWAYS call it with dry_run=true first, \n"
            f"  show the user the preview (count + sample of actions that would be created), \n"
            f"  and wait for explicit confirmation before calling again with dry_run=false.\n"
            f"- Prefer connector_update_action over delete+create when modifying an existing action.\n\n"
