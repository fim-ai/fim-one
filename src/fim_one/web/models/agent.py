...
class Agent(UUIDPKMixin, TimestampMixin, Base):
    ...
    # If True, after the assistant answer the chat stream emits
    # ``post_processing`` + ``suggestions`` SSE events with 3 follow-up
    # questions generated by the fast LLM.  Default OFF to keep latency
    # low for task-style agents and to avoid an extra LLM round-trip
    # unless the agent owner explicitly opts in.
    suggest_followups: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
        server_default=sa.text("FALSE"),
