...
class AgentResult:
    ...
    # True when the loop ended on a ``finish`` signal tool call instead of
    # an inline answer.  ``answer`` is empty in that case; the real answer
    # is generated by ``ReActAgent.stream_answer()`` continuing ``messages``
    # as a natively streamed turn.
    finish_signaled: bool = False
