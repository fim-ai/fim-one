"""ORM models for FIM One.

These live under ``fim_one.db`` (next to ``Base``/engine) rather than
``fim_one.web`` so that ``db/engine.py`` and ``migrations/env.py`` can populate
``Base.metadata`` without importing the FastAPI web layer. ``fim_one.web.models``
remains a thin back-compat shim re-exporting this package.
"""
