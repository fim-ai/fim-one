...
> 深入了解：[设计理念](https://docs.fim.ai/philosophy) | [执行模式](https://docs.fim.ai/execution-modes) | [竞争格局](https://docs.fim.ai/competitive-landscape)
...
> 完整说明：[设计理念](https://docs.fim.ai/philosophy)
...
┌──────────────────────────────────────────────────────────────┐
│                     应用与交互层                               │
│   Portal (Web UI)    API (无头模式)    iframe (嵌入)          │
│   飞书 / Slack 机器人   Webhook       企微 / 钉钉             │
└──────────────────────────┬───────────────────────────────────┘
                           │
┌──────────────────────────▼───────────────────────────────────┐
│                    FIM Agent 中间件                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────┐  ┌──────┐ │
│  │    连接器     │  │  编排引擎    │  │  RAG /   │  │认证  │ │
│  │  + MCP Hub   │  │ ReAct / DAG  │  │  知识库  │  │管理  │ │
│  └──────────────┘  └──────────────┘  └──────────┘  └──────┘ │
└──────────────────────────┬───────────────────────────────────┘
                           │
┌──────────────────────────▼───────────────────────────────────┐
│                     业务系统与数据层                            │
│   ERP · CRM · OA · 财务 · 数据库 · 自定义 API                │
│   飞书 · 钉钉 · 企微 · Slack · 邮件 · Webhook               │
└──────────────────────────────────────────────────────────────┘
```
...
```
                        ...
                        │     FIM Agent Hub          │
                        │                            │
 ERP (SAP/Kingdee) ────│►  Agent A: 财务审计         │───► 飞书 / Slack
 CRM (Salesforce)  ────│►  Agent B: 合同审查         │───► 邮件 / 企微
 OA (Seeyon/Weaver) ───│►  Agent C: 审批助手         │───► Teams / Webhook
 Custom DB (PG/MySQL) ──│►  Agent D: 数据报表         │───► 任意 API
                        │                            │
                        └───────────────────────────┘
                              Portal / API / iframe
```

每个连接器都是一个标准化桥梁 — Agent 不需要知道也不关心对方是 SAP 还是自定义的 PostgreSQL 数据库。详见 [连接器架构](https://docs.fim.ai/connector-architecture)。
...
User Query
    ...
    v
+--------------+
|  DAG Planner |  LLM 将目标分解为步骤 + 依赖边
+--------------+
    |
    v
+--------------+
| DAG Executor |  并发启动独立步骤（asyncio）
|              |  每个步骤由 ReAct Agent 处理
+--------------+
    |                         +-------+
    ...
    v
+---------------+
| Plan Analyzer |  LLM 评估结果；目标未达成则重新规划
+---------------+
    |
    v
 Final Answer
