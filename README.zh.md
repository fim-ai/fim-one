...
- **自动路由** — 自动查询分类将每个请求路由到最优执行模式（ReAct 或 DAG）。前端支持三态切换（自动/标准/规划器）。通过 `AUTO_ROUTING` 配置。
...
- **Agent 管理** — 创建、配置和发布 Agent，可绑定模型、工具和指令。支持逐 Agent 设置执行模式（标准/规划器）和温度控制。可选的 `discoverable` 标志启用通过 CallAgentTool 的 LLM 自动发现。
- **平台组织** — 内置 `platform` 组织自动加入所有用户，取代了遗留的"全局"可见性概念。为跨组织共享资源（智能体、连接器、知识库、MCP 服务器）提供中央枢纽。
- **资源订阅与市场** — 用户从组织市场浏览和订阅共享资源。通过 UI 或 API 进行订阅/取消订阅。所有资源类型均支持组织级发布和订阅管理。
...
FIM One 提供两种执行模式，并支持自动路由：

| 模式     | 适用场景             | 工作原理                             |
| -------- | -------------------- | ------------------------------------ |
| 自动     | 所有查询（默认）     | Fast LLM 分类查询并路由到 ReAct 或 DAG |
