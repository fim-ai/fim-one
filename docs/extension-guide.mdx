...
description: "How to plug custom LLM providers, tools, memory backends, embeddings, rerankers, web backends, and RAG components into FIM Agent."
---

## Overview

FIM Agent is built around a set of thin abstract base classes — one per swappable component. Every component has a single responsibility and a minimal interface. You implement the abstract methods, wire the instance into the appropriate registry or injector, and the rest of the system uses your implementation automatically.
...
Drop your file in `src/fim_agent/core/tool/builtin/`. The `discover_builtin_tools()` scanner will find any concrete (non-abstract) `BaseTool` subclass automatically — no manual registration needed.
