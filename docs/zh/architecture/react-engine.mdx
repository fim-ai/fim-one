...
关于此循环如何与 DAG 调度、工作流蓝图和行业**图工程**并行运作，请参阅[规划景观](/architecture/planning-landscape)。

## 架构

ReAct引擎实现了一个两阶段执行模型。第一阶段是迭代式工具使用循环：智能体反复向LLM请求操作，执行请求的工具，追加观察结果，并持续执行，直到LLM发出“完成”信号。第二阶段是答案合成：通过单独的流式LLM调用读取完整的执行轨迹，并生成面向用户的响应。

这种拆分是经过刻意设计的。工具迭代以速度为优化目标——循环中的每次LLM调用都使用非流式的`chat()`，因为用户无需看到部分JSON操作或中间推理token。答案生成则以用户体验为优化目标——使用流式的`stream_chat()`，让用户实时看到token逐步出现。这样就兼顾了两者的优势：快速执行工具，并及时交付答案。
...
工具循环会生成一个包含完整对话历史的`AgentResult`——系统提示词、用户查询、每条智能体消息以及每个工具结果。`stream_answer()`方法会将这条轨迹提炼为简洁、连贯的答案。在合成上下文中，每个工具结果最多截取2,000个字符，即使经过复杂的多工具工作流，也能保持提示词精简。

**模型绑定。** LLM会注入`ReActAgent.__init__()`并存储为`self._llm`。单次`run()`调用中的每次调用——包括工具循环的所有迭代和最终答案合成——都使用同一个实例。模型不会在迭代之间发生变化。若要使用其他模型，必须构造新的`ReActAgent`。在DAG模式下，`DAGExecutor._resolve_agent()`利用了这一模式：它会在某个步骤的ReAct循环开始前立即为该步骤创建一个新的智能体，并根据`step.model_hint`从`ModelRegistry`中选择模型。详情请参阅[DAG引擎——按步骤覆盖](/architecture/dag-engine#multi-llm-architecture)。
...
当智能体已使用至少`_COMPLETION_CHECK_MIN_TOOLS`个工具（默认值：**3**，可通过`REACT_COMPLETION_CHECK_MIN_TOOLS`配置）并决定生成最终答案时，会在接受答案前注入一次性验证提示：

1. 答案是否完全解决了原始问题？
2. 关键事实是否从工具结果中得到验证？
3. 收集的信息中是否存在矛盾？

清单是内部交互。回答它的轮次是验证，而不是答案——逐字接受它会导致"让我验证：1. ... ✓"泄露到聊天中。因此该轮次在内部被消耗，智能体被再次要求提供面向用户的答案，这正是`stream_answer()`流式传输的内容。如果智能体反而决定某些内容缺失，它会恢复工具调用循环，其后续答案将被原样采用。

清单每次运行最多触发一次，不会对简单对话响应（未使用工具）触发，并且当剩余迭代预算无法同时覆盖验证轮次和答案轮次时会被跳过。

该阈值防止了简单任务上的不必要延迟——单个工具调用（例如，运行快速计算）不需要验证，而多工具调查受益于最终的合理性检查。设置`REACT_COMPLETION_CHECK_MIN_TOOLS=1`以启用始终验证。

该功能默认启用（`ReActAgent`上的`completion_check=True`），可按智能体实例禁用。DAG步骤继承默认值——每个步骤都获得自己的一次性清单。
