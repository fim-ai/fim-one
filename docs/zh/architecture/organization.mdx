...
组织是FIM One的团队协作单位。它们让用户组能够在受信任的范围内共享资源——智能体、连接器、MCP服务器、工作流和技能。知识库是唯一的例外：它们永远不可共享，只能作为共享智能体的内部依赖项到达其他成员（参见[共享和凭证](/architecture/sharing-and-credentials)）。

FIM One中的每个资源最初都是**个人的**（仅对其创建者可见）。当你将资源发布到组织时，它会变成**可发现的**，其他组织成员可以通过市场的组织范围发现它。成员浏览组织的共享资源，并订阅他们需要的资源。
...
组织和全局市场共享相同的基于订阅的访问模型。关键区别在于信任：组织代表一个团队或公司，其中成员彼此了解和信任，因此审查是可选的，凭证共享也很直接。
...
| 资源 | 用途 | 配置者 | 使用者 |
|---|---|---|---|
| **通道** | 出站桥接到即时通讯平台（目前支持Feishu；计划支持Slack / WeCom / Teams） | 组织所有者/管理员 | 每个智能体的[钩子系统](/architecture/hook-system)和主动通知工具 |
| **OAuth提供商** | 社交登录凭证（GitHub、Google、Feishu、Discord） | 系统管理员（环境变量） | 登录页面 |
| **API密钥** | 无头客户端的程序化访问令牌 | 每个用户，组织范围 | 外部集成 |
...
关键区别：**可发布资源**（智能体、连接器、技能等）是需要显式成员订阅的按用户工件；**基础设施资源**（通道、OAuth、API密钥）是所有智能体隐式共享的平台级接线。通道特别是整个组织的即时通讯热线——一个维护良好的Feishu通道支持每个智能体的审批闸门、每个定时报告、每个升级事件。

参见[通道概览](/configuration/channels/overview)了解通道生命周期和支持的平台。
