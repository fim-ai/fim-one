...
| `REACT_MAX_TURN_TOKENS` | 否 | `0` | 紧急断路器：单个 ReAct 轮次的最大累积令牌数（所有迭代的提示词 + 完成）。默认 `0` = 无限制。**这不是用于日常令牌控制的** —— 请使用按用户的 `token_quota`。这是针对极端情况（如智能体陷入无限工具调用循环）的最后手段安全阀。达到此限制会中止任务执行，浪费所有已消耗的令牌并返回不完整的结果。除非您有特定的失控智能体问题需要控制，否则保持 `0` |
...
| `REACT_TOOL_RESULT_BUDGET` | 否 | `40000` | 单个会话中所有工具结果的聚合令牌预算。当工具结果令牌总数超过此上限时，新结果会被截断并显示通知。防止大型 API 响应（例如 5 个连接器调用各返回 8K）导致的上下文膨胀。设置为 `0` 以禁用上限 |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | 否 | `800` | 当智能体的最终答案超过此字符数时，跳过答案后的完成检查 LLM 调用。长详细答案不需要"我是否遗漏了什么？"验证往返。设置更低的值以更积极地跳过；设置为非常大的值以始终运行检查 |
| `REACT_CYCLE_DETECTION_THRESHOLD` | 否 | `2` | 当同一工具连续以相同参数调用此次数时，会注入一个确定性警告，告诉智能体尝试不同的方法。与依赖 LLM 注意到循环的自我反思不同，这是一个基于哈希的检查，无法绕过。也适用于 DAG 步骤 |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | 否 | `3` | 完成检查清单启动前的最小工具调用数。简单任务（1-2 次工具调用）会跳过验证以避免不必要的延迟。设置为 `1` 以启用始终开启的验证。也适用于 DAG 步骤 |
| `REACT_TURN_PROFILE_ENABLED` | 否 | `true` | 发出按轮次的阶段级别计时日志（`memory_load`、`compact`、`tool_schema_build`、`llm_first_token`、`llm_total`、`tool_exec`）。每轮一行结构化日志。设置为 `false` 以完全禁用分析（零开销） |
| `REACT_PLAN_TOOL_ENABLED` | 否 | `true` | 注册 `update_plan` 待办工具，以便智能体在多步骤任务期间写下并维护计划检查清单。对于 DAG 步骤智能体和没有工具的智能体会自动跳过 |
| `REACT_PLAN_REMINDER_INTERVAL` | 否 | `3` | 在重新注入陈旧计划提醒前，没有 `update_plan` 调用的工具轮次数（嵌入完整检查清单），以便计划在上下文压缩中保留 |
| `REACT_MAX_CONTINUATIONS` | 否 | `3` | 当模型的答案被提供商的输出令牌限制截断（`finish_reason=length`）时的最大续接轮次。截断的片段会被拼接成一个无缝答案，包括在智能体循环和流式合成中 |
...
| `REACT_BG_WAIT_TIMEOUT` | 否 | `300` | 当智能体想要完成答案时，等待仍在运行的后台工具的最大秒数。窗口内未完成的任务会被取消并显示显式超时通知 |
| `LLM_RATE_LIMIT_PER_USER` | 否 | `true` | 使用按用户键控的速率限制桶而不是单个进程全局桶。防止一个嘈杂的用户饿死同一工作进程上的所有其他用户。底层速率硬编码为每个桶 60 请求/分钟和 100K 令牌/分钟 —— 此设置仅控制桶是共享的（全局）还是分区的（按用户）。设置为 `false` 以恢复到旧版全局桶（不推荐） |
