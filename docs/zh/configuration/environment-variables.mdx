...
| 变量 | 必需 | 默认值 | 描述 |
|---|---|---|---|
| `REACT_MAX_ITERATIONS` | 否 | `20` | 每个 ReAct 请求的最大工具调用迭代次数。值越高越彻底但速度越慢且成本越高 |
...
| `REACT_TOOL_OBS_TRUNCATION` | 否 | `8000` | 综合最终答案时每个工具观察的最大字符数。较高的值可保留更多结构化数据（JSON、表格），但会消耗更多令牌 |
...
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | 否 | `800` | 当智能体的最终答案超过此字符数时，跳过答案后的完成检查 LLM 调用。长详细答案不需要"我是否遗漏了什么？"验证往返。设置较低值以更激进地跳过；设置为非常大的值以始终运行检查 |
...
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | 否 | `3` | 完成检查清单启动前的最少工具调用次数。简单任务（1-2 次工具调用）会跳过验证以避免不必要的延迟。设置为 `1` 以始终启用验证。也适用于 DAG 步骤 |
| `REACT_TURN_PROFILE_ENABLED` | 否 | `true` | 发出每轮阶段级别的计时日志（`memory_load`、`compact`、`tool_schema_build`、`llm_first_token`、`llm_total`、`tool_exec`）。每轮一行结构化日志。设置为 `false` 禁用分析（零开销） |
| `REACT_PLAN_TOOL_ENABLED` | 否 | `true` | 注册 `update_plan` 待办工具，以便智能体在多步骤任务中写下并维护计划检查清单。对于 DAG 步骤智能体和无工具的智能体自动跳过 |
| `REACT_PLAN_REMINDER_INTERVAL` | 否 | `3` | 在重新注入陈旧计划提醒前，没有 `update_plan` 调用的工具轮次（嵌入完整检查清单），以便计划在上下文压缩中保留 |
| `LLM_RATE_LIMIT_PER_USER` | 否 | `true` | 使用按用户键控的速率限制桶而不是单个进程全局桶。防止一个嘈杂用户饿死同一工作进程上的所有其他用户。底层速率硬编码为每个桶 60 请求/分钟和 100K 令牌/分钟 —— 此设置仅控制桶是共享（全局）还是分区（按用户）。设置为 `false` 恢复到旧版全局桶（不推荐） |
