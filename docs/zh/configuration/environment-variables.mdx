...
设置 `LLM_REASONING_EFFORT` 后，FIM Agent 会启用模型的扩展思考能力，并将内部思维链展示在 UI 的"思考中"步骤中。FIM Agent 使用 [LiteLLM](https://github.com/BerriAI/litellm) 自动将推理强度参数转换为各提供商的原生格式。
...
| 变量 | 必填 | 默认值 | 说明 |
|---|---|---|---|
| `DATABASE_URL` | 否 | `sqlite+aiosqlite:///./data/fim_agent.db` | 数据库连接字符串。**SQLite**（零配置）：`sqlite+aiosqlite:///./data/fim_agent.db`。**PostgreSQL**（生产环境推荐）：`postgresql+asyncpg://user:pass@localhost:5432/fim_agent`。Docker Compose 自动配置 PostgreSQL。 |
