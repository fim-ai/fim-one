...
| `REACT_MAX_TURN_TOKENS` | 否 | `0` | 紧急断路器：单个 ReAct 轮次的最大累积令牌数（所有迭代的提示词 + 完成）。默认 `0` = 无限制。**这不是用于日常令牌控制的** ——请使用按用户的 `token_quota`。这是针对极端情况（如智能体陷入无限工具调用循环）的最后手段安全阀。达到此限制会中止任务执行，浪费已消耗的所有令牌并返回不完整结果。除非您有特定的失控智能体问题需要解决，否则保持为 `0` |
...
| `REACT_SELF_REFLECTION_INTERVAL` | 否 | `6` | 每 N 次工具调用后注入一次自我反思提示词，帮助智能体纠正路线并避免循环 |
| `REACT_TOOL_OBS_TRUNCATION` | 否 | `8000` | 合成最终答案时每个工具观察的最大字符数。较高的值会保留更多结构化数据（JSON、表格），但会消耗更多令牌 |
| `REACT_TOOL_RESULT_BUDGET` | 否 | `40000` | 单个会话中所有工具结果的聚合令牌预算。当工具结果令牌总数超过此上限时，新结果会被截断并显示通知。防止大型 API 响应（例如 5 个连接器调用各返回 8K）导致的上下文膨胀。设置为 `0` 禁用此上限 |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | 否 | `800` | 当智能体的最终答案超过此字符数时，跳过答案后的完成检查 LLM 调用。冗长详细的答案不需要"我是否遗漏了什么？"验证往返。设置较低值可更激进地跳过；设置为非常大的值可始终运行检查 |
| `REACT_CYCLE_DETECTION_THRESHOLD` | 否 | `2` | 当同一工具连续以相同参数调用此次数时，会注入确定性警告，告诉智能体尝试不同的方法。与依赖 LLM 注意到循环的自我反思不同，这是基于哈希的检查，无法绕过。也适用于 DAG 步骤 |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | 否 | `3` | 完成检查清单触发前的最少工具调用次数。简单任务（1-2 次工具调用）会跳过验证以避免不必要的延迟。设置为 `1` 可始终进行验证。也适用于 DAG 步骤 |
...
| `REACT_PLAN_TOOL_ENABLED` | 否 | `true` | 注册 `update_plan` 待办工具，以便智能体在多步骤任务期间记录和维护计划检查清单。对于 DAG 步骤智能体和无工具的智能体自动跳过 |
| `REACT_PLAN_REMINDER_INTERVAL` | 否 | `3` | 在重新注入陈旧计划提醒前，没有 `update_plan` 调用的工具轮次数（嵌入完整检查清单），以便计划在上下文压缩中保留 |
| `REACT_MAX_CONTINUATIONS` | 否 | `3` | 当模型答案被提供商的输出令牌限制截断（`finish_reason=length`）时的最大续接轮次。截断的段落会被拼接成一个无缝答案，既在智能体循环中也在流式合成中 |
| `LLM_RATE_LIMIT_PER_USER` | 否 | `true` | 使用按用户键控的速率限制桶而不是单个进程全局桶。防止一个嘈杂的用户饿死同一工作进程上的所有其他用户。底层速率硬编码为每个桶每分钟 60 个请求和每分钟 100K 个令牌——此设置仅控制桶是共享（全局）还是分区（按用户）。设置为 `false` 恢复到旧版全局桶（不推荐） |
