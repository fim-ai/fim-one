...
|---|---|---|---|---|---|
| **OpenAI** | `gpt-5.4` | `gpt-5.4-mini` / `gpt-5.4-nano` | ✅ `reasoning_effort` | ✅ 全部 | 原生工具调用最佳；GPT-5.4 是最新旗舰版（2026年3月） |
| **Anthropic** | `claude-sonnet-4-6` | `claude-haiku-4-5` | ✅ 通过 LiteLLM | ✅ 全部 | 原生 API 路由；完整 `reasoning_content` 支持；1M 上下文 GA |
| **Google Gemini** | `gemini-2.5-pro` / `gemini-3.1-pro-preview` | `gemini-2.5-flash` / `gemini-3-flash-preview` | ✅ `reasoning_effort` | ✅ 全部 | 2.5 为稳定 GA；3.x 为预览版；`gemini-3-pro-preview` 于3月9日停用 |
| **DeepSeek** | `deepseek-chat`（V3.2） | `deepseek-chat` | ✅ `deepseek-reasoner` | ❌ | 仅文本；V4（2026年4月）将添加视觉 |
...
<Tip>
**视觉**表示模型是否接受图像输入。这是[智能文档处理（IDP）](/features/idp)所必需的——如果您的模型不支持视觉，IDP 将回退到仅文本提取。标记为 ⚠️ 的提供商在某些模型上支持视觉，但在其他模型上不支持；请检查您正在使用的特定模型。

聊天附件遵循相同的标志：对于仅文本模型，附加的图像不会被发送，模型仅接收其文件名。聊天编辑器会在您发送前根据该轮实际使用的模型（智能体设置，然后是活跃模型组，然后是系统默认值）进行提示。文档文本不受影响——PDF 或 DOCX 仍然会提取其内容并注入。
</Tip>

此表列出了我们推荐的组合，而非 FIM One 支持的完整提供商集合。xAI（Grok）、ByteDance 豆包、Mistral 和任何 OpenAI 兼容中继都可以使用；请参阅[提供商能力矩阵](/architecture/llm-provider-guide#provider-capability-matrix)了解完整列表以及每个提供商的路由方式。
