...
最经过实战检验的选项。OpenAI模型拥有最好的原生函数调用（工具调用）支持，这直接影响智能体的可靠性。GPT-5系列（2025年8月+）相比GPT-4是一个重大的代际飞跃。

**推荐模型：**
- 主力：`gpt-5.4`（最新旗舰，2026年3月——100万+上下文，计算机使用）或`o3`（最佳推理准确度）
...
**推理：** 设置`LLM_REASONING_EFFORT=medium`——与o系列和GPT-5.x模型原生兼容。GPT-5.4支持`reasoning_effort`，级别为`none`、`low`、`medium`、`high`、`xhigh`。o系列需要`max_completion_tokens`而非`max_tokens`，LiteLLM会自动处理。注意：`/v1/chat/completions`拒绝将工具与推理结合的GPT-5.x请求——FIM One先通过Responses API路由OpenAI模型（两者在那里协同工作），只有没有`/v1/responses`路由的端点才会回退到聊天完成，在智能体工具使用步骤中显式设置`reasoning_effort: "none"`。GPT-5.4需要`temperature=1`——FIM One通过LiteLLM的参数过滤（`drop_params`）自动处理。
