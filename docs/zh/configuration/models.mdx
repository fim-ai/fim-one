...
|---|---|---|---|---|---|
| **OpenAI** | `gpt-5.4` | `gpt-5.4-mini` / `gpt-5.4-nano` | ✅ `reasoning_effort` | ✅ 全部 | 原生工具调用能力最佳；GPT-5.4是最新旗舰模型（2026年3月） |
| **Anthropic** | `claude-sonnet-4-6` | `claude-haiku-4-5` | ✅ 通过LiteLLM | ✅ 全部 | 原生API路由；完整支持`reasoning_content`；1M上下文已正式发布 |
| **Google Gemini** | `gemini-2.5-pro` / `gemini-3.1-pro-preview` | `gemini-2.5-flash` / `gemini-3-flash-preview` | ✅ `reasoning_effort` | ✅ 全部 | 2.5是稳定正式版；3.x是预览版；`gemini-3-pro-preview`已于3月9日停止服务 |
| **DeepSeek** | `deepseek-chat`（V3.2） | `deepseek-chat` | ✅ `deepseek-reasoner` | ❌ | 仅支持文本；V4（2026年4月）将增加视觉能力 |
| **Qwen (Alibaba)** | `qwen3.5-plus` / `qwen3-max` | `qwen3.5-flash` / `qwen-turbo` | ✅ 在`qwen3-max`上启用`enable_thinking` | ⚠️ 仅qwen3.5 | 中文能力最强；qwq/推理仅支持文本 |
| **ChatGLM (Zhipu)** | `glm-4.7` | `glm-4.7-flash` | `glm-5` | ⚠️ GLM-4.6V | 不支持强制FC；视觉功能需要单独的VLM模型 |
| **MiniMax** | `MiniMax-M2.7` | `MiniMax-M2.5` | ❌ | ❌ | 仅支持文本；M2.7是最新版本（2026年3月）；SWE-Bench得分为80.2% |
| **Kimi (Moonshot)** | `kimi-k2.5` | `kimi-k2` | ✅ `kimi-k2-thinking` | ⚠️ 仅K2.5 | K2-thinking仅支持文本；启用thinking时不支持强制FC |
| **Ollama (local)** | `qwen3.5` / `llama4` | `qwen3.5:9b` | ❌ | 不同模型有所不同 | 完全离线运行，无需API密钥；Llama 4支持视觉能力 |

<Tip>
**视觉**表示模型是否接受图像输入。对于[智能文档处理（IDP）](/concepts/document-processing)，这是一项必要能力——如果你的模型不支持视觉，IDP将回退到仅文本提取。标记为⚠️的提供商仅部分模型支持视觉；请确认你使用的具体模型。

聊天附件遵循相同的开关：使用仅支持文本的模型时，不会发送附加图像，模型只会收到其文件名。聊天编辑器会在发送前根据该轮实际使用的模型告知你这一点（依次为智能体设置、当前激活的模型组，然后是系统默认模型）。文档文本不受影响——PDF或DOCX仍会提取其内容并注入。
</Tip>

此表列出的是我们推荐的组合，并非FIM One支持的全部提供商。xAI（Grok）、字节跳动Doubao、Mistral以及任何兼容OpenAI的中继服务均可使用；完整列表及每种提供商的路由方式，请参阅[提供商能力矩阵](/architecture/llm-provider-guide#provider-capability-matrix)。
