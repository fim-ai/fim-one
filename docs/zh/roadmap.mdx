...
- [ ] Gemini Context Cache Adapter（独立 REST 缓存生命周期 vs Anthropic 内联标记）
- [ ] 提示词注册表扩展至规划器 / 验证器 / 域分类器 / 紧凑型
- [ ] 单个智能体 `cache_ttl`（临时 5 分钟 vs 扩展 1 小时）
- [ ] DAG 步骤级检查点表，用于 DAG 中途恢复
- [ ] 专用 `tool_call_id` Message 列（大规模孤立查询索引查找）
- [ ] 中途思维 token 重建（恢复粒度细于完整 SSE 事件）
