...
不要在没有触发器的情况下启动这些功能（参见重新规划 §3）：MCP 网关等待 ≥2 个主动的"在我的智能体中挂载你的工具"请求；通道化等待实现者提出许可问题；IdP/OrgSync 等待客户拉取；其余的等待需要它们的已交付参与。
...
- [ ] 智能体工作区剩余部分——交接笔记、工作区文件浏览器 UI、跨会话对话回忆 {/* dev: dev/agent-workspace.md */}
...
- [ ] 热中流 DAG 恢复——SSE 重新连接重新附加到正在运行的轮次（冷重试-恢复已发布） {/* dev: dev/incremental-dag.md */}
