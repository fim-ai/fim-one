...
- [x] 审批闸门在委托中保持——`call_agent` 和工作流 `AGENT` 节点运行智能体自己的钩子而不是无

#### 答案渲染

- [x] 答案渲染Mermaid图表、SVG图形和卡片式对比表，支持答案、代码块和表格的复制/导出功能
- [x] 渲染的markdown经过清理，防止模型输出和上传文件中的原始HTML注入
- [x] 图表和代码块可下载为文件；推理过程在实时和历史对话中默认折叠为单行预览

#### 工作台UX

- [x] 侧边栏围绕聊天集群重新组织——对话直接显示在"新建聊天/搜索"下方，模块导航在紧凑的底部停靠栏中
- [x] `/clear` 斜杠命令从输入框启动新的对话
...
- [ ] 连接器授权第 2 层（要求按用户凭证、密钥绑定健康检查）+ 第 3 层（登录票据交换）{/* dev: dev/connector-rbac/00-overview.md */}
- [ ] 公开 API 第 2 阶段——按密钥速率限制/配额、版本控制、SDK、开发者门户 {/* dev: dev/public-api-phase2.md */}
...
- [ ] 热中流 DAG 恢复——SSE 重连重新附加到运行中的轮次（冷重试-恢复已发布）{/* dev: dev/incremental-dag.md */}
