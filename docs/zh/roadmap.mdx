...
#### 智能体循环强化（Claude Code模式）

- [x] ~~计划板：`update_plan` todo工具+陈旧计划重新注入提醒保持长ReAct运行目标一致并在压缩后存活~~
- [x] ~~LLM调用弹性：输出截断续接+529/504重试与Retry-After（过载模型回退已作为`FallbackLLM`发布）~~
- [x] ~~预算截断的工具结果在切割前持久化到工作区；有损压缩前的完整记录快照~~
- [x] ~~ReAct中的后台工具执行——白名单慢工具返回任务id并在完成时通知~~
- [x] ~~增量DAG重新规划：已完成步骤跨重新规划轮次保留；步骤级检查点在重试时恢复中断的运行~~ {/* dev: dev/incremental-dag.md */}
- [ ] 热中流DAG恢复（SSE重连重新附加到运行中的轮次）——冷重试恢复已通过检查点发布 {/* dev: dev/incremental-dag.md */}
