...
- [x] ~~LLM 调用弹性：输出截断继续 + 529/504 重试与 Retry-After（过载模型回退已作为 `FallbackLLM` 发布）~~
- [x] ~~预算截断的工具结果在切割前持久化到工作区；有损压缩前的完整记录快照~~
- [ ] ReAct 中的后台工具执行——慢工具返回占位符并在完成时通知
- [ ] 增量 DAG 重新规划：跨重新规划轮次保持已完成步骤；为中 DAG 恢复持久化步骤状态 {/* dev: dev/incremental-dag.md */}
