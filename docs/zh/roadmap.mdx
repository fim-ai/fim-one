...
- [x] ~~所有者凭证回退是可选的：连接器/MCP 服务器默认 `allow_fallback` 关闭（现有行已翻转）；缺少凭证的无回退资源从工具集中隐藏，而不是在调用时失败~~
...
- [ ] **MCP 连接池** — 使用按用户环境隔离的 STDIO 连接池；为 SSE/HTTP 共享 `httpx.AsyncClient`。目标 ≤100 ms 热启动，每个服务器 O(1) HTTP 连接
...
- [ ] 每个钩子的配置传递（`{"name": ..., "config": {...}}` 架构）
- [ ] DAG `tools_used` 准确性（通过步骤完成回调从每步 ReAct 获取的真实工具名称）
- [ ] `CallAgentTool` + 工作流 `AGENT` 节点的钩子继承（安全默认值 vs 灵活性默认值）
- [ ] 内置钩子：`ConnectorCallLog` 自动记录、只读模式阻止、数据库结果截断、每个连接器的速率限制
- [ ] `SessionStart` 钩子点 + 用户定义的 YAML 钩子
- [x] ~~PreToolUse 强制钩子在错误时失败关闭——崩溃的审批闸门不再默默允许调用；非强制钩子通过 `fail_open` 保持失败开放~~ *（在 v0.8.8 中发布）*
...
- [ ] 按键使用配额（月度令牌/请求预算）
- [ ] 高QPS键的使用统计写入批处理（进程本地关闭时刷新、精确计数、无新依赖）——延迟至测得的热点 {/* dev: dev/public-api-phase2.md */}
- [ ] 按端点的作用域强制
- [ ] API 版本控制（`/v1/...`）+ 弃用头部
- [ ] Webhook 回调（按键）
...
#### 提示词缓存 + 推理后续 {/* dev: dev/prompt-cache-followups.md */}

- [x] ~~推理细节端到端保留——循环流式传输其真实最终答案（无损耗重新合成）；推理在压缩、上下文重建和子智能体步骤中保留~~ *(v0.8.8中已发布)*
- [ ] Gemini上下文缓存适配器（独立REST缓存生命周期vs Anthropic内联标记）
- [ ] 提示词注册表扩展到规划器/验证器/域分类器/紧凑
- [ ] 按智能体`cache_ttl`（临时5分钟vs扩展1小时）
- [ ] DAG步骤级检查点表用于DAG中途恢复
- [ ] 专用`tool_call_id`消息列（大规模索引孤立查询查找）
- [ ] 中流思维令牌重建（恢复粒度比完整SSE事件更细）
- [ ] API中继缓存诚实性探针（管理员触发：检测中继剥离`cache_control`）

#### 可靠性后续工作（智能体核心优先级矩阵）

- [ ] 内容替换状态持久化（流式不变量 #2："一旦看到，命运冻结"）
- [ ] 附件上下文路由器（去重、聚合预算、活跃性检查；与工作区卸载配对）
- [ ] 侧查询工作线程（用于召回/分类/摘要的专用池，以便它们不会争用主速率限制预算）

#### 生态系统 + 扩展

- [ ] **定时任务 + 事件触发的智能体** — `scheduled_jobs` + `job_runs` + APScheduler；cron + webhook-inbound。Hub 模式下的异步子智能体用例
- [ ] **工作流触发身份可观测性** — `ExecutionContext.trigger_source`（`webhook | cron | manual | batch | sub`）在全部 5 个 WorkflowEngine 站点中填充；在运行面板和连接器日志中展示
