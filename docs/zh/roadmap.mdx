...
#### DB 连接器围栏——第一层，三个 PR {/* dev: dev/connector-rbac/00-overview.md */}

- [ ] PII 列脱敏（`ConnectorScopeGuard` PreToolUse 钩子）
- [ ] 模式可见性——表/列允许拒绝 + 动词阻断（只读强制执行）
- [ ] 围栏可审计性——`ConnectorCallLog` 中的 `caller_user_id`、`effective_credential_source`、`scope_rules_applied`
- [ ] 每钩子配置透传（`{"name", "config"}` 模式）——ScopeGuard 规则的载体 {/* dev: dev/hook-system.md */}
