...
- [x] OAuth自动链接要求双方都有已验证的地址，因此从未验证的提供商电子邮件创建的账户永远不会成为绑定目标
...
- [x] 管理员模型列表支持复选框多选，支持Shift+点击范围选择和一次请求批量删除
- [x] 运行中的智能体步骤在单个折叠标题中显示生成的单行标题，保留在对话历史中
- [x] 新发送的消息上升到记录的顶部，答案在其下方的空间中展开
- [x] 列表页面在首次加载时错开卡片，所有动画都遵守系统的减少动作偏好设置
- [x] 智能体在运行中提出澄清性的多选问题（ask_user_question）——ReAct转向在聊天卡片上暂停，并使用答案恢复
- [x] 编辑器在附加的图像到达纯文本模型时发出警告，从该转向实际使用的模型中解析
- [x] 未发送的编辑器文本、剪辑和附件按对话保留（以及新建聊天），在刷新、对话切换和会话过期后保持
...
- [x] GPT-5.x 采用 Responses-API-first（工具+推理一起；404 回退到聊天完成）；其他系列按设计保持在完成上
- [x] GPT-5.x 在工具轮次中保持其推理——原生 `/v1/responses` 带加密推理重放，`FIM_GPT5_RESPONSES_MODE` 用于回滚
- [x] 输出限制切割会丢弃整个工具调用批次并请求较小的重试，因此没有回复仅运行其计划的一部分
- [ ] 在下一次 LiteLLM 升级时验证 Responses-bridge 流式使用数字（怀疑上游映射错误）
- [ ] 一旦原生 GPT-5.x 路径完成整个版本发布，就停用 LiteLLM 聊天→responses 桥接
