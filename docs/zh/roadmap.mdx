...
重新规划于2026-09-06。核心已构建完成：ReAct引擎、连接器、凭据、审批闸门、审计、多租户组织。接下来将完成代码库中已有的内容并确保其正确性，而不是开启新的功能领域。下方未列出的任何内容均不在计划中；请参阅结尾说明。

### v0.9 —— 安全共享

**目标**：数据库连接器可以安全地交给同事，收回连接器时确实能够将其收回，并且代码树中已经做到一半的路径也得到收尾。

#### 数据库连接器闸门 {/* dev: dev/connector-rbac/00-overview.md */}

- [x] PII列标记——已标记的列会保留在架构中，其值在每个查询结果中均以掩码形式返回
- [x] 架构可见性——表／列允许或拒绝规则，以及动词阻止（只读强制执行）
- [x] 闸门可审计性——`ConnectorCallLog`中的`scope_rules_applied`记录每次调用所使用的闸门

#### 共享撤销

- [x] 取消发布正常工作——订阅仅在资源仍处于共享状态时授予访问权限，并会在使用时重新检查
- [x] 管理员在Market中下架资源时，会将资源恢复为私有状态，而不只是将其从列表中隐藏
- [x] 取消订阅解决方案不再移除用户自行设置的连接器或凭据
- [x] 删除组织后，其已发布的资源会返回给所有者，并撤销组织范围的订阅

#### 完成已有工作

- [ ] 在主聊天路径中实现分块压缩输入和模型感知的预算，确保任何模型组合都不会超出上下文窗口
- [ ] 在下一次LiteLLM升级时验证Responses桥接的流式使用量数据——怀疑上游存在错误映射
- [ ] 原生GPT-5.x路径完成一次完整发布后，移除LiteLLM的聊天→Responses桥接
- [ ] Stripe计费正式上线——每晚与Stripe对账以检查遗漏的webhook，完成全栈回归测试，使用正式价格ID
...
如果某项能力未在上文列出，就表示目前没有计划支持它。这是经过慎重考虑的选择，而非疏漏：项目目前处于维护阶段，工作优先级取决于实际需求，而不是为了完善功能矩阵。

如果你需要的功能尚未提供，请提交 issue，描述你遇到的问题。只有具体且受阻的使用场景，才会推动某项内容被列入本页面。
