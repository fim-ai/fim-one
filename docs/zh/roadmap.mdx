...
- 组织系统：完整的 CRUD 操作，具有基于角色的成员资格（所有者/管理员/成员），管理员管理 UI
...
- **评估中心**：定量智能体质量基准测试——测试数据集 CRUD（提示词 + 预期行为 + 断言），评估运行（并行执行 + LLM 评分器 + 单个案例通过/失败/延迟/令牌结果），结果查看器，带自动轮询；迁移 `r8t0v2x4z567`
- 三个模型角色（通用/快速/推理），每层环境配置隔离；快速模型不再继承主模型设置
...
- [x] ~~**影子市场组织 + 资源订阅**~~：内置市场组织（影子，无自动加入）替代平台组织；资源通过市场浏览发现并显式订阅（拉取模型）；用于订阅共享资源的市场 API；发布到市场始终需要审查；资源订阅表；基于组织的资源共享替代全局可见性
...
- [x] Stripe 计费 MVP——免费 + 专业版；结账、客户门户、webhook 生命周期；`/settings?tab=billing`；管理员套餐/订阅 CRUD；配额强制执行遵守每个用户的套餐
...
- [x] 翻译词汇表作为单一信息源——`scripts/translation-glossary.md` 整合各语言环境规则；pre-commit 无条件拒绝对生成的语言环境文件的手动编辑
- [x] 许可证 + 管辖法律迁移至 FIM Labs Pte. Ltd.（新加坡）；SIAC 仲裁使用英文；新的顶级 `NOTICE` 文件
- [x] 演练场后续建议已恢复，每个智能体可选择启用
- [x] 稳定性修复——严格交替提供商历史、并行工具调用边界检测、无界智能体确认流、通道角色门控、重试去重、拒绝后无释义
...
- [x] **JWT 令牌类型限制** — auth/SSE 端点仅接受真正的访问令牌（加上用于流式传输的 SSE 票证），关闭了一个 2FA 绕过漏洞，其中任何同签名的令牌都可以进行身份验证
- [x] **OAuth 账户接管与会话修复** — 电子邮件自动链接需要提供商验证的电子邮件；OAuth 刷新令牌以哈希方式存储，以便会话轮换正常工作
- [x] **PostgreSQL 时区感知时间戳** — 发布审查批准不再在 PG 上返回 500 错误；CI mypy 门禁 + 时区感知列规则防止重复发生
...
- [ ] 按密钥速率限制（`X-RateLimit-*` 头、429）
- [ ] 按密钥使用配额（月度令牌/请求预算）
- [ ] 高QPS密钥的使用统计写入批处理（进程本地关闭时刷新、精确计数、无新依赖）——延迟到测量到热点为止 {/* dev: dev/public-api-phase2.md */}
- [ ] 按端点的作用域强制执行
- [ ] API 版本控制（`/v1/...`）+ 弃用头
- [ ] Webhook 回调（按密钥）
