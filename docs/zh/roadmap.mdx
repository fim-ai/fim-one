...
- [x] ~~JWT 令牌类型限制（2FA 绕过）+ OAuth 账户接管与会话修复~~ *(在 v0.8.7 中发布)*
- [x] ~~PostgreSQL 时区感知时间戳~~ *(在 v0.8.6 中发布)*
...
#### 内容保护栏 {/* dev: dev/openai-agents-insights.md */}

- [x] ~~内容保护栏v0（输入/输出三脚架、越狱检测器、环境变量配置、`guardrail_tripwired` SSE事件）~~ *(在v0.8.7中发布)*
- [ ] 离题过滤器（分类器支持的输入保护栏）——如果正则表达式仍然足够，推迟到v0.5
- [ ] PII编辑器输出保护栏（正则表达式+分类器混合）
- [ ] 每个智能体保护栏配置UI（管理员面板）
...
#### 通道→集成晋升 {/* dev: dev/channel-integration-sso.md */}

- [ ] `ThirdPartyIntegration` 模型——将通道晋升为交付+登录+组织图同步子功能
- [ ] 飞书SSO（"使用飞书登录"生成FIM会话+上游令牌，移除Tier-2的逐用户API密钥摩擦）
- [ ] 组织图同步（飞书部门→FIM组织树）；WeCom+钉钉下一步
...
- [ ] **连接器渐进式披露（第 5 阶段）**：**语义引导工具选择**（从查询进行实体抽取 → 本体注册表查找 → 连接器集合缩减；对于 50+ 连接器部署可实现 90%+ 的令牌减少）；批量/ETL 连接器的扩展模式；CLI 风格的通用 `connector <name> <action> <params>` 接口
- [ ] **跨连接器实体对齐（本体注册表）**：定义共享实体类型（客户、订单、资产），并跨连接器进行字段映射；DAGPlanner 自动解析跨系统 JOIN 键；启用跨连接器查询（例如"在 Salesforce 中订购了 Shopify 产品的客户"），无需硬编码字段名称
- [ ] **热插拔连接器**：上传 OpenAPI 规范，AI 生成配置，5 分钟内上线（无需重启）
- [x] ~~**市场重新设计阶段 1 — 解决方案 + 组件**~~：两层市场模型（解决方案：智能体/技能/工作流；组件：连接器/MCP 服务器）；范围选择器（全局市场/组织）；统一订阅模型（删除组织自动出现）；市场范围中移除知识库；数据迁移为现有组织成员的订阅进行回填
- [ ] **市场包系统**：Marketplace 的可分发资源束——用统一的打包层替代每种类型的"市场"。`fim-package.yaml` 清单声明：元数据（名称、版本、描述、作者、许可证、标签、`min_fim_version`）、入口点（主要技能或智能体）、资源列表（智能体、技能、连接器、知识库、MCP 服务器、工作流）带配置引用、包间依赖项（semver 范围）、所需凭证（映射到连接器引用以供安装时收集）和带默认值的用户可配置变量。**两种消费模式**：（1）**安装**——批量创建所有资源 + 通过 ID 替代自动连接内部引用；安装链接到源以获取版本更新通知；`POST /api/market/packages/{id}/install`；（2）**分叉**——克隆为用户拥有的可编辑副本，无更新链接（这就是模板模式）；`POST /api/market/packages/{id}/fork`。额外端点：发布（`POST /api/market/packages` 带审查工作流）、卸载（`DELETE /packages/{id}/uninstall` 带依赖检查 + 修改资源确认）、版本历史（`GET /packages/{id}/versions`）、升级（`POST /packages/{id}/upgrade` 带逐资源差异预览）。嵌套包需求的依赖解析器，带冲突检测。`PackageInstallation` 表按用户跟踪已安装的包，带资源 ID 映射以供卸载/升级。**与单个资源发布并存**——包是组合层，不是替代品；单个连接器仍可独立发布。示例依赖树：`Package: contract-review` → `Skill: contract-review`（入口点）→ `Agent: contract-analyst` + `Agent: risk-scorer` → `KB: legal-clauses` + `Connector: docusign-api` + `MCP: pdf-extractor` + `Workflow: contract-approval-flow`
- [ ] **创作者计划**：Marketplace 货币化层——创作者资料及作品集页面、逐包分析（安装、分叉、活跃用户、评分/评价）、包推动新订阅时的联盟佣金跟踪。付费包层级，带定价、购买流程和审批工作流。创作者仪表板，带安装趋势、收益报告和用户反馈。用于程序化包发布的公共创作者 API（包作者的 CI/CD）。社区功能：包评论、问答、每个版本的更新日志
- [ ] **可嵌入小部件**：`<script src="fim-one.js">` 注入到主机页面
- [ ] **页面上下文注入**：小部件读取主机页面上下文（当前 ID、URL、DOM 选择器）
- [ ] **高级触发器**：Webhook 入站事件；计划作业增强（多时区、日历感知）
- [ ] **批处理执行**：通过 DAG 处理 1000+ 项目
- [ ] **企业安全**：IP 白名单、静态加密、SSO
- [ ] **知识库高级编辑器**：为管理大型知识库的高级用户提供的 Builder 模式智能体——批量 URL 摄入、重复检测、间隙分析、文档生命周期管理；使用 ReAct 工具循环扩展现有 KB AI 聊天
- [ ] **Stripe 计费（v1 MVP — Pro 订阅）**：Free + Pro 两层订阅，具有每月令牌配额。Stripe Checkout（托管）+ 客户门户（自助服务）+ webhook 驱动的生命周期（`checkout.session.completed` / `customer.subscription.updated|deleted` / `invoice.payment_succeeded|failed`）。在配额耗尽时软上限（HTTP 402 + 升级提示）——v1 中无超额费用。仅按用户计费；组织/团队订阅推迟至 v3。前置条件：
  - [x] ~~**数据模型 + SDK 基础工作**（P1）—— `billing_plans` / `subscriptions` / `stripe_webhook_events` 表、ORM 模型、Stripe SDK 单例、Free + Pro 种子~~*（在 v0.8.6 中发布）*
  - [x] ~~**后端 API + webhook 处理程序**（P2）—— `/api/billing/*` + `/api/webhooks/stripe` 带签名验证 + 幂等性；计划感知配额；每小时生命周期扫描~~*（在 v0.8.6 中发布）*
  - [x] ~~**前端计费选项卡 + 402 升级对话框**（P3）—— `/settings?tab=billing` 配额显示、升级 CTA、`past_due` 横幅、流中 402 对话框~~*（在 v0.8.6 中发布）*
  - [x] ~~**管理员计划管理**（P4）—— `admin/billing/{plans,subscriptions}` CRUD~~*（在 v0.8.6 中发布）*
  - [x] ~~**管理员控制的计费功能标志**（P5）—— `system_settings.billing_enabled` 控制 Stripe 管道；幂等激活为 Free+Pro 取种、设置默认计划指针、回填用户；切换开/关是激活后的纯标志翻转~~*（在 v0.8.6 中发布）*
  - [ ] **协调 + e2e + 上线**（P6）—— 每晚 `subscriptions` ↔ `stripe.Subscription.list()` 协调脚本用于错过的 webhook 恢复；全栈快乐路径/中期取消/逾期回归测试；从测试模式 `stripe_price_id` 切换到实时 `price_id`；使用真实卡在暂存环境进行烟雾测试。

- [ ] **团队计划（Stripe 席位）** — 通过 `stripe.Subscription.quantity` 的按席位定价，与组织成员集成。让公司订阅一个具有 N 个席位的团队范围计划；配额和功能标志通过席位组而非个人用户进行解析。以 v1.0 Stripe MVP 和现有组织模型为基础。
- [ ] **非计费部署的组级令牌配额** — 无 Stripe 的企业/私有部署配置组织级令牌预算。配额链扩展为 `override > group > plan > default`；组解析使用 `max(user_quota, group_quota)` 以便个别 VIP 不会受到团队上限的约束。与团队计划同时推出，以便相同的基础设施为计费和自托管拓扑服务。

**影响**：企业在数天内将 FIM One 从零部署到多系统编排。包系统创建创作者生态——解决方案作者发布复合包（技能 + 智能体 + 连接器 + 知识库 + 工作流），企业一键安装，创作者从采用中获利。安装/分叉二元性涵盖"按原样使用"和"从模板自定义"两种用例。
