...
**影响**：计划自动化 + 可观测性构成完整的生产级后台自动化闭环，FIM Agent 可自信地大规模运行。
...
* **连接器渐进式披露 (Phase 5)**：~~启发式预加载~~ → **语义引导工具选择**（从 query 中抽取业务实体 → 查询 Ontology Registry → 缩减连接器候选集；50+ 连接器部署下 token 降低 90%+）；Scale 模式支持批量/ETL 连接器；CLI 化统一接口 `connector <name> <action> <params>`
