...
- [x] ~~计划板：`update_plan` todo 工具 + 陈旧计划重新注入提醒保持长 ReAct 运行目标一致并在压缩后存活~~
- [x] ~~LLM 调用弹性：输出截断续接 + 529/504 重试与 Retry-After（过载模型回退已作为 `FallbackLLM` 发布）~~
- [x] ~~预算截断的工具结果在切割前持久化到工作区；有损压缩前的完整记录快照~~
- [x] ~~ReAct 中的后台工具执行——白名单慢速工具返回任务 id 并在完成时通知~~
- [ ] 增量 DAG 重新规划：跨重新规划轮次保持已完成步骤；为 DAG 中途恢复持久化步骤状态 {/* dev: dev/incremental-dag.md */}
