...
- [x] 最终答案通过 `finish` 移交原生流式输出；流式 Markdown 按块渲染，稳定无闪烁
- [x] 答案支持渲染 Mermaid 图表、SVG 图形和卡片式对比表格，并提供答案、代码块和表格的复制/导出及文件下载功能
- [x] 渲染的 Markdown 已做安全过滤（关闭原始 HTML 注入）；对话导出针对 CJK 排版优化（PDF 内嵌真实字体，DOCX 声明东亚字体）
- [x] 推理折叠为单行预览；正在运行的智能体步骤显示自动生成的单行标题，并保留在对话历史中
- [x] 侧边栏围绕聊天集群重新整理；新增 `/clear` 命令；管理员模型列表支持复选框多选，可配合 Shift 键选择范围并批量删除
- [x] 新发送的消息置顶显示于对话记录；列表页面卡片交错入场；所有动画遵循减少动效设置
- [x] 智能体可在运行中途提出多选澄清问题（ask_user_question）——对话在聊天卡片处暂停，待用户回答后继续
- [x] 编辑器为每个对话保留未发送的草稿，当图片将发送至纯文本模型时发出警告，并在发送前等待上传中的文件完成
- [x] GPT-5.x 优先采用 Responses API，并在工具调用轮次间支持加密推理回放（可通过 `FIM_GPT5_RESPONSES_MODE` 回滚）
- [x] 输出超限时丢弃整批工具调用，并要求以更小的请求重试
- [x] 类型化 DAG 步骤——纯转换/合成步骤作为单次直接 LLM 调用执行；先询问的目标在一轮内完成
- [x] 上下文预算设置在模型硬限制的 8% 以下；计划板规范（重复/无计划提醒，完成前验证）
- [x] 审批闸门在委托链中保持有效——`call_agent` 和工作流 AGENT 节点执行智能体自身的钩子，而非跳过
- [x] OAuth 自动关联要求双方均有已验证的邮箱地址；刷新令牌轮换携带唯一 `jti`，旧令牌立即失效
- [x] Feishu 回调 URL 通过验证——未签名的推送通过 Encrypt Key 信封 + Verification Token 进行身份验证
- [x] 实例访问模型用于计费——支持无订阅 / 含包含额度+付费 / 仅付费等策略，含包含额度的默认套餐不可删除

### v0.8.11 (2026-09-03) — Floating Composer
- [x] 聊天输入框现为悬浮在消息流上方的胶囊组件——附件、模式/智能体选择器与发送按钮集于一体；消息在其下方滚动

### v0.8.12（2026-09-04）——共享编辑器 + ENV 同步
- [x] KB、智能体和连接器 AI 助手使用与聊天相同的浮动编辑器；输入法组合期间按 Enter 不再发送
- [x] `scripts/env_from_active_group.py` 使 `.env` 中的模型回退设置与管理面板中的活动组保持同步
- [x] 图像生成支持 OpenAI 的 `gpt-image-*` 模型
...
#### DB 连接器隔离边界 {/* dev: dev/connector-rbac/00-overview.md */}

- [x] PII 列标记——已标记的列会保留在架构中，其值在每个查询结果中都会返回掩码
- [x] 架构可见性——表／列允许或拒绝规则＋动词阻断（只读强制）
- [x] 隔离边界可审计性——`ConnectorCallLog` 中的 `scope_rules_applied` 会记录每次调用所运行于其下的隔离边界
- [ ] 按钩子传递配置（`{"name", "config"}` 架构）——用于传递 ScopeGuard 规则 {/* dev: dev/hook-system.md */}

#### 上下文健壮性

- [ ] 在主聊天路径上实现分块压缩输入和模型感知预算，使任意模型组合都能保持在上下文窗口内

#### 模型层

- [ ] 在下次升级 LiteLLM 时验证 Responses-bridge 流式传输的使用数据（疑似上游映射错误）
- [ ] 一旦原生 GPT-5.x 路径完整运行一个发布周期，即退役 LiteLLM chat→responses 桥接
...
**目标**：两个最具销售价值的交付界面，均运行在同一内核与组装层之上。

- [ ] JS 气泡/iframe 嵌入——将一段代码片段嵌入宿主系统；匿名访客身份识别与计费归因在构建前确定
- [ ] Feishu 入站 @提及——智能体在群组中运行：查询数据、提交审批、推进流程
- [ ] 出站模式：故障告警、预算预警、定时摘要、升级通知、审计回执
- [ ] 企业微信/钉钉通道跟进 Feishu
