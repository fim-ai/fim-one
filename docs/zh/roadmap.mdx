...
- [x] Force-logout 时间戳比较将时区感知/朴素值规范化为 UTC 转换——撤销的令牌在 SQLite 和 PostgreSQL 上都能可靠地被拒绝
- [x] Docker Compose PostgreSQL 凭证可通过 `POSTGRES_*` 环境变量覆盖（单一来源进入连接字符串）——不再意外泄露暴露部署上的 `fim:fim` 默认值
...
- [x] 共享LLM HTTP池在LiteLLM客户端缓存驱逐关闭后自我修复——调用透明地重新创建池，而不是在重启前无限期失败
...
- [ ] 单个钩子配置传递（`{"name": ..., "config": {...}}`架构）
- [ ] DAG `tools_used`准确性（通过步骤完成回调从单步ReAct获取真实工具名称）
- [ ] `CallAgentTool` + Workflow `AGENT`节点的钩子继承（安全默认值与灵活性默认值）
- [ ] 内置钩子：`ConnectorCallLog`自动记录、只读模式阻止、数据库结果截断、单个连接器速率限制
- [ ] `SessionStart`钩子点 + 用户定义的YAML钩子
- [x] ~~骨架 + FeishuGateHook + 审批演练场 + ReAct/DAG运行时~~ *（在v0.8.5中发布）*
...
- [x] 推理细节端到端保留——循环流式传输其真实最终答案（无损失重新合成）；推理在压缩、上下文重建和子智能体步骤中保留
- [ ] Gemini 上下文缓存适配器（独立 REST 缓存生命周期 vs Anthropic 内联标记）
- [ ] 提示词注册表扩展到规划器 / 验证器 / 领域分类器 / 紧凑
- [ ] 每个智能体 `cache_ttl`（临时 5 分钟 vs 扩展 1 小时）
- [ ] DAG 步骤级检查点表用于中途 DAG 恢复
- [ ] 专用 `tool_call_id` 消息列（大规模索引孤立查询查找）
- [ ] 中流思维令牌重建（恢复粒度比完整 SSE 事件更细）
