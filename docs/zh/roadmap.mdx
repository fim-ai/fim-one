...
- [x] ~~Docker Compose PostgreSQL凭证可通过`POSTGRES_*`环境变量覆盖（单一来源进入连接字符串）——不再意外在暴露的部署上发布`fim:fim`默认值~~ *(在v0.8.8中发布)*
- [x] ~~SSRF阻止列表关闭IPv4映射IPv6绕过（`::ffff:`实例元数据）；MCP SSE/Streamable-HTTP服务器URL在创建+连接时进行SSRF验证~~ *(在v0.8.8中发布)*
- [x] ~~所有者凭证回退是可选的：连接器/MCP服务器默认`allow_fallback`关闭（现有行已翻转）；缺少凭证的无回退资源从工具集中隐藏，而不是在调用时失败~~
- [x] ~~资源访问统一于可见性（自有+已订阅）：已订阅的连接器/知识库/MCP服务器可绑定到智能体（不再出现"未拥有"403），工作流连接器步骤强制执行运行者的访问权限而不是按id运行任何连接器~~
