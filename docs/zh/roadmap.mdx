...
- [x] ~~计划板：`update_plan` todo 工具 + 过期计划重新注入提醒保持长 ReAct 运行目标一致并在压缩后存活~~
- [x] ~~LLM 调用弹性：输出截断续接 + 529/504 重试与 Retry-After（过载模型回退已作为 `FallbackLLM` 发布）~~
- [ ] 大型工具结果持久化到工作区文件而非头部截断；LLM 压缩前完整记录快照
- [ ] ReAct 中的后台工具执行——慢速工具返回占位符并在完成时通知
- [ ] 增量 DAG 重新规划：跨重新规划轮次保持已完成步骤；持久化步骤状态以实现 DAG 中途恢复 {/* dev: dev/incremental-dag.md */}
