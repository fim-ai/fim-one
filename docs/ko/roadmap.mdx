...
- [x] 렌더링된 마크다운은 새니타이징되어 모델 출력 및 업로드된 파일의 원본 HTML 주입을 차단
- [x] 다이어그램 및 코드 블록은 파일로 다운로드 가능하며, 추론은 라이브 및 과거 대화에서 기본적으로 한 줄 미리보기로 접힘
- [x] 대화 내보내기는 CJK에 맞게 조판됨 — PDF는 실제 글꼴을 포함하고(올바른 간격, 글머리 기호 및 굵은 글씨), DOCX는 동아시아 글꼴을 선언하며, 둘 다 동일한 크기 척도로 표시
