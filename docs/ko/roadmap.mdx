...
- [ ] 콘텐츠 교체 상태 지속성 (스트리밍 불변식 #2: "한 번 본 것은 운명이 정해짐")
- [ ] 첨부 파일 컨텍스트 라우터 (중복 제거, 예산 집계, 생존성 확인; Workspace 오프로딩과 쌍을 이룸)
- [ ] 사이드 쿼리 워커 (회수 / 분류 / 요약용 전용 풀로 메인 속도 제한 예산과 경합하지 않음)
- [x] 관리자 LLM 제공자 편집이 프로세스 재시작 없이 적용됨 (LiteLLM 클라이언트 캐시를 무효화 시 플러시 + 60초 레지스트리 TTL 안전망)
