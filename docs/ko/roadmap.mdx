...
- [x] ~~강제 로그아웃 타임스탬프 비교는 타임존 인식/순진한 값을 UTC로 정규화합니다 — 취소된 토큰이 SQLite 및 PostgreSQL 모두에서 안정적으로 거부됩니다~~ *(v0.8.8에서 출시)*
- [x] ~~Docker Compose PostgreSQL 자격증명은 `POSTGRES_*` 환경 변수를 통해 재정의 가능합니다 (연결 문자열로 단일 소싱됨) — 노출된 배포에서 실수로 배송된 `fim:fim` 기본값이 더 이상 없습니다~~ *(v0.8.8에서 출시)*
- [x] ~~SSRF 차단 목록은 IPv4 매핑 IPv6 우회(`::ffff:` 인스턴스 메타데이터)를 닫습니다; MCP SSE/Streamable-HTTP 서버 URL은 생성 + 연결 시 SSRF 검증됩니다~~ *(v0.8.8에서 출시)*
- [x] ~~소유자 자격증명 폴백은 선택 사항입니다: 커넥터/MCP 서버는 기본적으로 `allow_fallback` 해제 상태입니다 (기존 행 전환됨); 자격증명이 없는 폴백 없는 리소스는 호출 시 실패하는 대신 도구 집합에서 숨겨집니다~~
