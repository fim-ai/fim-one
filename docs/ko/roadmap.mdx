...
재현 가능하지만 아직 수정되지 않은 프로덕션 버그입니다. 각 항목은 증상, 의심되는 영역, 해결 방법(있는 경우)을 명시합니다. 수정이 범위가 정해지고 일정이 잡히면 항목이 버전 섹션으로 이동합니다.

- **Agent 편집기가 편집 없이 저장되지 않은 변경 경고를 표시합니다.** `/agents/[id]`를 통해 기존 agent를 열고 즉시 뒤로 가기를 클릭하면 필드를 건드리지 않았는데도 "Unsaved changes" 대화상자가 나타납니다. Dirty check는 20개 이상의 필드를 로드된 agent 페이로드와 비교하므로, 상태 초기화와 dirty 비교 간의 한 가지 비대칭 기본값만으로도 가짜 불일치를 유발할 수 있습니다. 현재 의심 대상은 중첩된 `model_config_json` / notification / approval-routing 필드 중 하나이며, `undefined` vs `null` vs `""` 정규화에서 비롯되었을 가능성이 있습니다. 특히 org 범위 agent에서 재현됩니다. 해결 방법: 대화상자를 닫으세요(`Discard and leave`) — 실제로 변경된 것이 없으므로 데이터 손실이 없습니다. 시도된 수정(`cb40c86a`)은 리소스 피커에서 관련 orphan-badge 깜박임을 제거했지만 이 문제는 해결하지 못했습니다.

- **Agent 편집 저장이 `Input should be 'initiator', 'agent_owner' or 'org_members'` 오류로 실패할 수 있습니다.** Pydantic은 `/api/agents/{id}` PUT 경계에서 `confirmation_approver_scope` 필드를 거부합니다. 데이터베이스에 저장된 모든 값이 세 가지 유효한 리터럴 중 하나임에도 불구하고 말입니다. 의심 사항: 프론트엔드 `as "initiator" | "agent_owner" | "org_members"` 캐스트는 컴파일 타임 전용 약속이므로, 레거시 또는 예상치 못한 런타임 문자열(템플릿, 가져오기 또는 이전 마이그레이션에서 비롯되었을 수 있음)이 `setConfirmationApproverScope`를 통과하여 그대로 다시 전송될 수 있습니다. 해결 방법: 저장하기 전에 Approval → Approver Scope 드롭다운에서 값을 명시적으로 다시 선택하세요.

- **Playground 중지 및 재시도가 페이지 새로 고침으로 항상 지워지는 일시적 시각 아티팩트를 표시합니다.** 세 가지 동시 렌더 소스 — `activeConversation.messages`(DB 스냅샷), SSE `messages` 스트림, 낙관적 `pendingQuery` 플레이스홀더 — 가 단일 파생 상태로 축소되지 않으므로, "Retry"를 클릭한 후 쌍을 이루는 assistant 응답이 도착하기 전 사이에 UI는 (a) 사전 스트림 창에서 동일한 쿼리를 잠시 두 번 렌더링하거나, (b) `hasLiveMessages`가 true이고 스냅샷이 다시 로드되기 전에 재시도 기록에서 이전 orphan 사용자 버블을 삭제하거나, (c) SSE "done" 이벤트와 다음 `selectConversation` 새로 고침 사이의 좁은 창에서 깜박일 수 있습니다. **데이터는 절대 손실되지 않습니다** — 모든 사용자 메시지(중단된 재시도 포함)는 `conversation.messages`에 유지되고, `normalize_alternating_messages`를 통해 다음 LLM 호출로 전달되며, `48ba08c6` 렌더 수정에서 도입된 `HistoryTurn.orphanUserContents`를 통해 새로 고침 후 올바르게 렌더링됩니다. 참고로 Claude의 자체 웹 UI는 유사한 클래스의 버그를 나타냅니다 — 응답 중간에 중지하고 즉시 후속 쿼리를 보내면 때때로 후속 쿼리가 첫 번째 쿼리의 형제 편집 분기로 포크되어 새로운 턴으로 추가되지 않습니다 — 따라서 이는 낙관적 UI + SSE + 유지된 기록 설계에서 알려진 어려운 문제이며, FIM One 특정 결함이 아닙니다. 적절한 수정을 위해서는 세 가지 렌더 소스를 단일 파생 상태로 축소해야 합니다. 더 광범위한 Playground 상태 머신 리팩토링까지 연기되었습니다.
...
- [x] ~~JWT token-type confinement (2FA bypass) + OAuth account-takeover & session fixes~~ *(v0.8.7에서 출시)*
- [x] ~~PostgreSQL timezone-aware timestamps~~ *(v0.8.6에서 출시)*
- [x] ~~Force-logout timestamp comparison normalizes tz-aware/naive values to UTC by conversion — revoked tokens reliably rejected on both SQLite and PostgreSQL~~ *(v0.8.8에서 출시)*
- [x] ~~Docker Compose PostgreSQL credentials overridable via `POSTGRES_*` env (single-sourced into the connection string) — no more accidentally-shipped `fim:fim` default on exposed deployments~~ *(v0.8.8에서 출시)*
- [x] ~~SSRF blocklist closes the IPv4-mapped IPv6 bypass (`::ffff:` instance-metadata); MCP SSE/Streamable-HTTP server URLs SSRF-validated on create + connect~~ *(v0.8.8에서 출시)*
...
- [ ] **연결기 점진적 공개 단계 3-4** — 통합 `ConnectorExecutor` (API/DB/MCP); `jsonschema` 작업 검증; 프로토콜 무관 발견/실행
- [ ] **YAML/JSON 연결기 구성** — 플랫폼 자동 생성 MCP 서버
- [ ] **데이터베이스 연결기 단계 4** — Oracle (`oracledb`), SQL Server (`aioodbc`), GBase (`aioodbc` + GBase ODBC). DM8 / KingbaseES / HighGo v0.8.5에서 출시
...
#### 훅 시스템 {/* dev: dev/hook-system.md */}

- [ ] 훅별 설정 통과 (`{"name": ..., "config": {...}}` 스키마)
- [ ] DAG `tools_used` 정확도 (단계 완료 콜백을 통한 ReAct의 실제 도구 이름)
- [ ] `CallAgentTool` + Workflow `AGENT` 노드에 대한 훅 상속 (보안 기본값 vs 유연성 기본값)
- [ ] 기본 제공 훅: `ConnectorCallLog` 자동 기록, 읽기 전용 모드 차단, DB 결과 잘림, 커넥터별 속도 제한
- [ ] `SessionStart` 훅 포인트 + 사용자 정의 YAML 훅
- [x] ~~PreToolUse 강제 훅이 오류 시 실패 종료 — 충돌하는 승인 게이트가 더 이상 호출을 자동으로 허용하지 않음; 비강제 훅은 `fail_open`을 통해 실패 개방 유지~~ *(v0.8.8에서 출시)*
...
- [ ] 고QPS 키에 대한 사용량 통계 쓰기 배칭 (프로세스 로컬 플러시온셧다운, 정확한 카운트, 새로운 의존성 없음) — 측정된 핫스팟까지 연기됨 {/* dev: dev/public-api-phase2.md */}
- [ ] 엔드포인트별 범위 적용
- [ ] API 버전 관리 (`/v1/...`) + 지원 중단 헤더
- [ ] Webhook 콜백 (키별)
- [ ] SDK 생성 (Python + TypeScript)
- [ ] 개발자 포털 (시도해보기 패널 + 키별 분석)
- [ ] API 키 로테이션 (24시간 유예)
...
- [x] ~~추론 세부 정보가 종단 간 보존됨 — 루프가 진정한 최종 답변을 스트리밍함(손실 없는 재합성 없음); 추론이 압축, 컨텍스트 재구축 및 하위 에이전트 단계에서 유지됨~~ *(v0.8.8에서 배포됨)*
- [ ] Gemini Context Cache Adapter (Anthropic 인라인 마커 대 별도 REST 캐시 수명 주기)
- [ ] 프롬프트 레지스트리 확장 — 플래너 / 검증자 / 도메인 분류기 / 컴팩트
- [ ] 에이전트별 `cache_ttl` (임시 5분 vs 연장 1시간)
- [ ] DAG 단계 수준 체크포인트 테이블 (중간 DAG 재개용)
- [ ] 전용 `tool_call_id` Message 열 (대규모 고아 쿼리 조회 인덱싱)
- [ ] 중간 스트림 사고 토큰 재구성 (완전한 SSE 이벤트보다 세밀한 재개 세분성)
- [ ] API 릴레이 캐시 정직성 프로브 (관리자 트리거: 릴레이가 `cache_control`을 제거하는지 감지)

#### 신뢰성 후속 조치 (Agent Core 우선순위 매트릭스)

- [ ] 콘텐츠 교체 상태 지속성 (스트리밍 불변성 #2: "한 번 본 것은 운명이 정해짐")
- [ ] 첨부 파일 컨텍스트 라우터 (중복 제거, 예산 집계, 생존 여부 확인; 워크스페이스 오프로딩과 쌍을 이룸)
- [ ] 사이드 쿼리 워커 (회상 / 분류 / 요약을 위한 전용 풀로 메인 속도 제한 예산과 경합하지 않음)

#### 에코시스템 + 확장

- [ ] **예약된 작업 + 이벤트 트리거 에이전트** — `scheduled_jobs` + `job_runs` + APScheduler; cron + webhook-inbound. Hub 모드의 비동기 서브-에이전트 사용 사례
- [ ] **워크플로우 트리거 ID 관찰성** — `ExecutionContext.trigger_source` (`webhook | cron | manual | batch | sub`)가 모든 5개 WorkflowEngine 사이트에서 채워짐; 실행 패널 및 커넥터 로그에 표시
...
- [ ] 샌드박스 강화 (v2 코드 실행 격리)
- [ ] 성능 테스트 (동시 로드 벤치마크)
- [ ] 내부 하네스 벤치마크 (Eval Center를 통한 하네스 매개변수 변경 정량화)
