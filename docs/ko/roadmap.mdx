...
- [x] 답변이 Mermaid 다이어그램, SVG 그림 및 카드 스타일 비교 표를 렌더링하며, 답변, 코드 블록 및 표에 복사/내보내기 기능 제공
- [x] 렌더링된 마크다운이 새니타이즈됨 — 모델 출력 및 업로드된 파일의 원본 HTML 주입 차단
- [x] 다이어그램 및 코드 블록을 파일로 다운로드 가능; 추론은 라이브 및 과거 대화에서 기본적으로 한 줄 미리보기로 축소됨
- [x] 대화 내보내기는 CJK에 맞게 조판됨 — PDF는 실제 글꼴을 포함하고(올바른 간격, 글머리 기호 및 굵게), DOCX는 동아시아 글꼴을 선언하며, 둘 다 동일한 크기 척도로 표시됨
