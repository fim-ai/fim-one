...
- [x] 스킬 및 워크플로우는 관리자 모듈 플래그 뒤에 소프트 셀프(기본값 꺼짐) — 코어 전용 부팅; 삭제된 항목 없음, 관리자 → 설정 → 모듈에서 되돌릴 수 있음
- [x] 공유 수렴 — KB 공유 제거(KB는 공유된 에이전트를 통해서만 다른 사용자에게 도달), DB 커넥터는 공유 불가 + 원본 SQL은 소유자 전용, 워크플로우 빌더는 9개 참조 전용 노드로 축소
- [x] Feishu 승인 강화 — 카드 클릭은 승인자 신원 강제, 콜백 서명은 폐쇄 실패 + 암호화된 봉투 복호화, 승인은 의도하지 않은 채팅으로 라우팅되지 않음
- [x] 사용 시간 액세스 재확인 — 공유된 MCP 서버 및 바인딩된 KB는 실행당 재검증; 조직 탈퇴 시 구독 및 저장된 자격 증명 즉시 취소
- [x] 에이전트 루프 강화 — 계획 보드, 백그라운드 도구, 증분 DAG 재계획 + 체크포인트 재개, 압축은 도구 쌍 유지, 절단 계속, 529/504 재시도
- [x] `run_workflow` 에이전트 도구 + 워크플로우 정확성 — 에이전트 노드는 전체 에이전트 실행, 확인 게이트는 폐쇄 실패, 커넥터 호출은 액세스 확인 및 감사 로깅
- [x] 계정 삭제 통합 — 관리자 및 셀프 서비스는 모든 레코드 및 디스크 상 파일을 포함하는 하나의 제거 루틴을 통해 진행; 조직 소유자는 먼저 소유권 이전 필요
- [x] 소유자 자격 증명 폴백은 이제 옵트인(주요 변경) — 커넥터/MCP 서버는 기본값 `allow_fallback` 꺼짐, 기존 행 전환; 자격 증명이 없는 폴백 불가 리소스는 도구 세트에서 숨겨짐
- [x] Webhook/cron 워크플로우 실행은 소유자의 토큰 할당량으로 측정 — 측정되지 않은 무료 LLM 트리거 경로는 폐쇄됨
- [x] 리소스 바인딩은 가시성에서 통합 — 구독된 커넥터/KB/MCP 서버는 에이전트에 바인딩 가능; 워크플로우 커넥터 단계는 실행자의 액세스 강제
- [x] 대화 워크스페이스는 채팅에 연결 — `workspace://` 초과 크기 도구 결과 오프로드, 예산 절단 구조, 사전 압축 트랜스크립트 스냅샷
...
**목표**: 사후 처리 자산이 완전한 데이터 Q&A 제품으로 조립됨 — 읽기 전용 DB 연결기 + 펜스 + 승인 게이트 + IM 진입점. Tier-1 펜스가 보안 부채를 제품 기능으로 전환함.
...
- [x] 승인 게이트는 위임 전체에서 유지 — `call_agent` 및 워크플로우 `AGENT` 노드는 에이전트 자체 훅을 실행하며 아무것도 실행하지 않음

#### 답변 렌더링

- [x] 답변은 Mermaid 다이어그램, SVG 그림 및 카드 스타일 비교 테이블을 렌더링하며, 답변, 코드 블록 및 테이블에서 복사/내보내기 지원
- [x] 렌더링된 마크다운은 새니타이제이션되어 모델 출력 및 업로드된 파일의 원본 HTML 주입을 차단
...
- [ ] 전달된 각 참여당 하나의 시나리오 템플릿 추출 — 경쟁력은 시나리오 자산 × 전달 속도

### v0.10 — Two Mouths: JS Embed + IM Inbound {/* dev: dev/im-channels.md */}

**Goal**: 두 가지 가장 판매 가능한 전달 표면, 모두 동일한 커널 및 어셈블리 레이어 위에 구현.

- [ ] JS bubble / iframe embed — 호스트 시스템에 하나의 스니펫 삽입; 익명 방문자 신원 + 빌링 속성은 빌드 전에 결정
- [ ] Feishu inbound @mention — 에이전트가 그룹에 상주: 데이터 쿼리, 파일 승인, 흐름 추적
- [ ] Outbound patterns: 실패 알림, 예산 경고, 예약된 다이제스트, 에스컬레이션, 감사 영수증
- [ ] WeCom / DingTalk 채널이 Feishu를 따름

### 보류 중 — 신호 게이트

이 항목들을 시작하지 마세요. 트리거가 필요합니다 (재계획 §3 참조): MCP 게이트웨이는 ≥2건의 미청청 "내 에이전트에 도구를 마운트하세요" 요청을 기다리고 있습니다. 채널화는 라이선싱에 대해 질문하는 구현자를 기다리고 있습니다. IdP/OrgSync는 고객 풀을 기다리고 있습니다. 나머지는 이들이 필요한 전달된 참여를 기다리고 있습니다.

- [ ] MCP 게이트웨이 출력 — 커넥터 발견/실행을 다운스트림 에이전트용 MCP 도구로 역노출
- [ ] 채널화 / 화이트라벨 활성화 — 상용 라이선스 경로 이미 준비됨
- [ ] Identity Provider 모듈 + Channel 슬림다운 — Feishu SSO, org 그래프 동기화 {/* dev: dev/connector-rbac/07-identity-providers.md */}
...
- [ ] Agent Workspace 나머지 — 인수 노트, 파일 브라우저 UI, 크로스 세션 회상, 압축 세그먼트 (에이전트가 다시 읽는 grep 가능한 온디스크 요약) {/* dev: dev/agent-workspace.md */}
- [ ] Guardrails v1 — 주제 벗어남 필터, PII 편집기 출력 가드레일, 에이전트별 가드레일 구성 UI
- [ ] Hook System 추가 기능 — 내장 훅, `SessionStart` + 사용자 YAML 훅 {/* dev: dev/hook-system.md */}
- [ ] 커넥터 플랫폼 심화 — Progressive Disclosure Phase 3-4, YAML/JSON 커넥터 구성, DB 커넥터 Phase 4 (Oracle / SQL Server / GBase), MCP 연결 풀링
- [ ] Prompt 캐시 후속 조치 — Gemini 컨텍스트 캐시 어댑터, 에이전트별 `cache_ttl` {/* dev: dev/prompt-cache-followups.md */}
- [ ] Hot 중간 스트림 DAG 재개 — SSE 재연결이 실행 중인 턴에 다시 연결됨 (콜드 재시도-재개는 이미 배포됨) {/* dev: dev/incremental-dag.md */}
- [ ] Ecosystem — 예약된/이벤트 트리거 에이전트, 워크플로우 트리거 ID 관찰성, 워크플로우별 `credential_policy`, DB Schema Advanced Builder, 샌드박스 강화 v2

### v0.9 사전 계획에서 배포됨

- [x] ~~인증 및 보안: JWT 토큰 타입 제한 + OAuth 수정 (v0.8.7); PG 시간대 인식 타임스탬프 (v0.8.6); 강제 로그아웃 UTC + `POSTGRES_*` 오버라이드 + SSRF IPv6 매핑 수정 (v0.8.8); 소유자 폴백 옵트인 + 가시성 통합 바인딩 + 웹훅/크론 미터링 (v0.8.9)~~
- [x] ~~공급자 호환성: Anthropic 적응형 추론 + 공유 LLM 풀 자가 치유 (v0.8.8)~~
- [x] ~~콘텐츠 가드레일 v0: 트립와이어 레이어 + 탈옥 감지기 (v0.8.7)~~ {/* dev: dev/archive/openai-agents-insights.md */}
- [x] ~~훅 시스템: 스켈레톤 + FeishuGateHook + 승인 연습장 + ReAct/DAG 런타임 (v0.8.5); PreToolUse 강제 실패 폐쇄 (v0.8.8)~~
- [x] ~~Feishu 통로 1단계 + 작업 완료 알림 (v0.8.5)~~
...
- [x] ~~워크플로우 연결 종속성 자동 구독, 워크플로우 실제 실행기~~ *(v0.8.1)*
