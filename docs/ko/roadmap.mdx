...
- [x] 사이드바가 채팅 클러스터 중심으로 재구성됨 — 대화가 새 채팅/검색 바로 아래에 표시되고, 모듈 네비게이션은 컴팩트한 하단 도크에 위치
- [x] `/clear` 슬래시 명령어로 입력 상자에서 새 대화 시작
- [x] 관리자 모델 목록이 Shift-클릭 범위 선택 및 한 번의 요청으로 일괄 삭제를 지원하는 체크박스 다중 선택 지원
- [x] 실행 중인 에이전트 단계가 생성된 한 줄 제목을 단일 폴드된 헤더에 표시하고, 대화 기록에 유지
