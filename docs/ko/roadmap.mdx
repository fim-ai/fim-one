...
#### DB 커넥터 펜스 {/* dev: dev/connector-rbac/00-overview.md */}

- [x] PII 열 표시 — 표시된 열은 스키마에 유지되며, 모든 쿼리 결과에서 값이 마스킹되어 반환됩니다
- [x] 스키마 가시성 — 테이블/열 허용-거부 및 동사 차단(읽기 전용 적용)
- [x] 펜스 감사 가능성 — `ConnectorCallLog`의 `scope_rules_applied`에 각 호출이 실행된 펜스가 기록됩니다
- [ ] 훅별 구성 전달(`{"name", "config"}` 스키마) — ScopeGuard 규칙의 전달 매개체 {/* dev: dev/hook-system.md */}
