...
- [ ] 훅별 구성 전달 (`{"name", "config"}` 스키마) — ScopeGuard 규칙의 캐리어 {/* dev: dev/hook-system.md */}
...
다음 트리거 없이는 시작하지 마세요 (재계획 §3 참조): MCP 게이트웨이는 ≥2건의 미청청 "내 에이전트에 도구를 마운트하세요" 요청을 기다리고 있습니다. 채널화는 라이선싱에 대해 질문하는 구현자를 기다리고 있습니다. IdP/OrgSync는 고객 풀을 기다리고 있습니다. 나머지는 이들이 필요한 전달된 참여를 기다리고 있습니다.

- [ ] MCP 게이트웨이 출력 — 커넥터 발견/실행을 다운스트림 에이전트용 MCP 도구로 역노출
- [ ] 채널화 / 화이트라벨 활성화 — 상용 라이선스 경로 이미 준비됨
- [ ] Identity Provider 모듈 + Channel 슬림다운 — Feishu SSO, 조직 그래프 동기화 {/* dev: dev/connector-rbac/07-identity-providers.md */}
- [ ] 커넥터 권한 부여 Tier 2 (사용자별 자격증명 필요, 키 바인딩 상태) + Tier 3 (로그인 티켓 교환) {/* dev: dev/connector-rbac/00-overview.md */}
...
- [ ] Agent Workspace 나머지 — 인계 노트, 워크스페이스 파일 브라우저 UI, 세션 간 대화 회상 {/* dev: dev/agent-workspace.md */}
- [ ] Guardrails v1 — 주제 벗어남 필터, PII 편집기 출력 가드레일, 에이전트별 가드레일 구성 UI
- [ ] Hook System 추가 기능 — `CallAgentTool` / 워크플로우 `AGENT` 노드용 훅 상속, 기본 제공 훅, `SessionStart` + 사용자 YAML 훅 {/* dev: dev/hook-system.md */}
