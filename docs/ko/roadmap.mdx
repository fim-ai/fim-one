...
- [x] ~~JWT 토큰 타입 제한 (2FA 우회) + OAuth 계정 탈취 및 세션 수정~~ *(v0.8.7에서 출시)*
- [x] ~~PostgreSQL 타임존 인식 타임스탬프~~ *(v0.8.6에서 출시)*
- [x] ~~강제 로그아웃 타임스탬프 비교는 타임존 인식/순진한 값을 UTC로 정규화 — 취소된 토큰은 SQLite 및 PostgreSQL 모두에서 안정적으로 거부됨~~ *(v0.8.8에서 출시)*
- [x] ~~Docker Compose PostgreSQL 자격증명은 `POSTGRES_*` 환경 변수를 통해 재정의 가능 (연결 문자열에 단일 소싱) — 노출된 배포에서 실수로 배포된 `fim:fim` 기본값 없음~~ *(v0.8.8에서 출시)*
- [x] ~~SSRF 차단 목록은 IPv4 매핑 IPv6 우회(`::ffff:` 인스턴스 메타데이터)를 닫음; MCP SSE/Streamable-HTTP 서버 URL은 생성 + 연결 시 SSRF 검증됨~~ *(v0.8.8에서 출시)*
- [x] ~~소유자 자격증명 폴백은 선택 사항: 커넥터/MCP 서버는 기본적으로 `allow_fallback` 비활성화 (기존 행 전환); 자격증명이 없는 폴백 없는 리소스는 호출 시 실패하는 대신 도구 집합에서 숨겨짐~~
- [x] ~~리소스 접근은 가시성에서 통합됨 (소유 + 구독): 구독된 커넥터/KB/MCP 서버는 에이전트에 바인딩 가능 (더 이상 "소유하지 않음" 403 없음), 워크플로우 커넥터 단계는 실행자의 접근을 적용하며 ID로 커넥터를 실행하지 않음~~
- [x] ~~웹훅/크론 워크플로우 실행은 소유자의 토큰 할당량으로 측정됨 — 무인 LLM/에이전트 단계는 사전에 제한되고 사용량은 채팅과 동일한 할당량으로 계산되어 무계량 무료 LLM 경로를 닫음~~
...
- [ ] **Connector Progressive Disclosure Phase 3-4** — 통합 `ConnectorExecutor` (API/DB/MCP); `jsonschema` 액션 검증; 프로토콜 무관 발견/실행
- [ ] **YAML/JSON 커넥터 설정** — 플랫폼이 MCP 서버 자동 생성
- [ ] **데이터베이스 커넥터 Phase 4** — Oracle (`oracledb`), SQL Server (`aioodbc`), GBase (`aioodbc` + GBase ODBC). DM8 / KingbaseES / HighGo는 v0.8.5에서 출시됨
- [ ] **MCP Connection Pooling** — STDIO 풀링 및 사용자별 환경 격리; SSE/HTTP용 `httpx.AsyncClient` 공유. 목표 ≤100 ms 웜 스타트, 서버당 O(1) HTTP 연결
- [x] ~~스마트 에이전트가 `run_workflow`를 가져와 결정론적 워크플로우를 서브 스텝으로 실행 (정적 레시피 ↔ Skill의 동적 레시피); 인간 승인 워크플로우는 트리거 전용 유지, 실행은 감사됨, 깊이/재진입성 보호~~
