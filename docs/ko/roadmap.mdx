...
#### DB 커넥터 펜스 — Tier 1, PR 3개 {/* dev: dev/connector-rbac/00-overview.md */}

- [ ] PII 컬럼 리댁션 (`ConnectorScopeGuard` PreToolUse 훅)
- [ ] 스키마 가시성 — 테이블/컬럼 허용-거부 + 동사 차단 (읽기 전용 강제)
- [ ] 펜스 감사 가능성 — `ConnectorCallLog`의 `caller_user_id`, `effective_credential_source`, `scope_rules_applied`
- [ ] 훅별 설정 전달 (`{"name", "config"}` 스키마) — ScopeGuard 규칙의 캐리어 {/* dev: dev/hook-system.md */}

#### 컨텍스트 견고성

- [ ] 메인 채팅 경로에서 청크 압축 입력 및 모델 인식 예산을 적용하여 어떤 모델 조합도 컨텍스트 윈도우 내에 유지

#### 모델 레이어

- [ ] 다음 LiteLLM 업그레이드 시 Responses-bridge 스트리밍 사용량 수치 검증 (업스트림 잘못된 매핑 의심)
- [ ] 네이티브 GPT-5.x 경로가 전체 릴리스를 완료하면 LiteLLM chat→responses 브리지 폐기
...
**목표**: 동일한 커널 및 어셈블리 레이어 위에서 가장 판매 가능성이 높은 두 가지 전달 방식.

- [ ] JS 버블 / iframe 임베드 — 호스트 시스템에 코드 한 줄 삽입; 익명 방문자 신원 및 과금 귀속은 빌드 전에 결정
- [ ] Feishu 인바운드 @멘션 — 에이전트가 그룹 내에 상주: 데이터 조회, 승인 처리, 플로우 추적
- [ ] 아웃바운드 패턴: 장애 알림, 예산 경고, 예약 다이제스트, 에스컬레이션, 감사 영수증
- [ ] WeCom / DingTalk 통道, Feishu 이후 순차 지원
