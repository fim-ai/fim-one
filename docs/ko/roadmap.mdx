...
- [x] Force-logout timestamp comparison는 tz-aware/naive 값을 UTC로 정규화하여 변환합니다 — 취소된 토큰이 SQLite 및 PostgreSQL 모두에서 안정적으로 거부됩니다
- [x] Docker Compose PostgreSQL 자격증명은 `POSTGRES_*` env를 통해 재정의 가능합니다 (연결 문자열로 단일 소싱됨) — 노출된 배포에서 실수로 배포된 `fim:fim` 기본값이 더 이상 없습니다
...
- [ ] DAG `tools_used` 정확도 (단계 완료 콜백을 통한 단계별 ReAct의 실제 도구 이름)
...
- [x] ~~스켈레톤 + FeishuGateHook + 승인 Playground + ReAct/DAG 런타임~~ *(v0.8.5에서 출시)*
...
- [x] 추론 세부 정보가 종단 간 보존됨 — 루프가 진정한 최종 답변을 스트리밍함(손실 없는 재합성 없음); 추론이 압축, 컨텍스트 재구성, 하위 에이전트 단계를 거쳐 유지됨
- [ ] Gemini Context Cache Adapter (별도 REST 캐시 수명 주기 대 Anthropic 인라인 마커)
- [ ] 프롬프트 레지스트리 확장 — 플래너 / 검증자 / 도메인 분류기 / 컴팩트
- [ ] 에이전트별 `cache_ttl` (임시 5분 대 확장 1시간)
- [ ] DAG 단계 수준 체크포인트 테이블 — 중간 DAG 재개
- [ ] 전용 `tool_call_id` Message 열 (대규모 고아 쿼리 조회 인덱싱)
- [ ] 중간 스트림 사고 토큰 재구성 (완전한 SSE 이벤트보다 세분화된 재개 세분성)
