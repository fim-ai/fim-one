...
이들을 시작하지 마세요. 트리거가 필요합니다 (replan §3 참조): MCP 게이트웨이는 ≥2건의 자발적인 "내 에이전트에 도구를 마운트하세요" 요청을 기다립니다. 채널화는 라이선싱에 대해 질문하는 구현자를 기다립니다. IdP/OrgSync는 고객 풀을 기다립니다. 나머지는 이들이 필요한 전달된 engagement를 기다립니다.

- [ ] MCP gateway output — 커넥터 discover/execute를 다운스트림 에이전트용 MCP 도구로 역노출
- [ ] Channelization / white-label enablement — 상용 라이선스 경로가 이미 준비됨
- [ ] Identity Provider module + Channel slim-down — Feishu SSO, org graph sync {/* dev: dev/connector-rbac/07-identity-providers.md */}
- [ ] Connector authorization Tier 2 (사용자별 자격증명 필요, 키 바인딩 상태) + Tier 3 (로그인 티켓 교환) {/* dev: dev/connector-rbac/00-overview.md */}
- [ ] Public API Phase 2 — 키별 속도 제한/할당량, 버전 관리, SDK, 개발자 포털 {/* dev: dev/public-api-phase2.md */}
- [ ] Observability — Agent Trace Layer (Trace/Span 모델, 타임라인 뷰어, OTel 내보내기) + 메트릭 대시보드 {/* dev: dev/agent-trace-layer.md */}
- [ ] Agent Workspace remainder — 인수 노트, 워크스페이스 파일 브라우저 UI, 크로스 세션 대화 회상 {/* dev: dev/agent-workspace.md */}
- [ ] Guardrails v1 — 주제 벗어남 필터, PII 편집기 출력 가드레일, 에이전트별 가드레일 설정 UI
- [ ] Hook System extras — 내장 훅, `SessionStart` + 사용자 YAML 훅 {/* dev: dev/hook-system.md */}
- [ ] Connector platform depth — Progressive Disclosure Phase 3-4, YAML/JSON 커넥터 설정, DB 커넥터 Phase 4 (Oracle / SQL Server / GBase), MCP 연결 풀링
- [ ] Prompt cache follow-ups — Gemini context cache 어댑터, 에이전트별 `cache_ttl` {/* dev: dev/prompt-cache-followups.md */}
- [ ] Hot mid-stream DAG resume — SSE 재연결이 실행 중인 턴에 재연결 (콜드 재시도-재개는 이미 배포됨) {/* dev: dev/incremental-dag.md */}
- [ ] Ecosystem — 예약된/이벤트 트리거 에이전트, 워크플로우 트리거 ID 관찰성, 워크플로우별 `credential_policy`, DB Schema Advanced Builder, sandbox hardening v2
