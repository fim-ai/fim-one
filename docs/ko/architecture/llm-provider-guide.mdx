...
타사 게이트웨이는 직접 제공자와 다른 방식으로 실패하며, 대부분의 실패는 무음입니다. 아래의 각 행은 증상을 메커니즘과 FIM One이 이미 취하고 있는 조치와 연결합니다.

| 증상 | 메커니즘 | FIM One의 조치 |
|---|---|---|
| 추론이 구성되었지만 나타나지 않음 | 릴레이 경로에 `/claude` 힌트가 없어서 모델이 `openai/`로 해석됩니다. Chat Completions 스키마에는 추론 개념이 없으므로 요청이 프로세스를 떠나기 전에 매개변수가 삭제됩니다 | 구성 시 모델 이름과 해석된 접두사를 지정하는 경고를 기록하고, `provider`를 설정하거나 Anthropic 기본 URL을 사용하도록 지시합니다 (`OpenAICompatibleLLM.__init__`) |
| 매개변수가 수락되는 것으로 나타나지만 효과가 없음 | `litellm.drop_params=True`는 해석된 제공자가 선언하지 않은 모든 항목을 자동으로 제거합니다 | 의도적입니다. 이를 통해 하나의 요청 빌더가 모든 제공자에서 작동합니다. 대가는 "오류 없음"이 매개변수가 도착했다는 증거가 아니라는 것입니다 |
| OpenAI가 아닌 모델에서 첫 토큰이 몇 분 걸리거나 에이전트가 텍스트를 반환하고 도구 호출을 하지 않음 | 릴레이가 OpenAI가 아닌 모델에 대해 `/v1/responses`를 광고하고, 요청을 수락한 후, 전체 답변을 버퍼링한 후 재생합니다. Uniapi에서 Claude로 대략 4분의 첫 토큰 지연이 관찰되었으며, 두 번째 재현에서는 호출이 정상적으로 반환되었지만 에이전트가 도구 호출을 0개 만들었습니다. 오류가 없으므로 오류 트리거 폴백이 발생하지 않습니다 | 브리지는 Responses에서 기능 이득을 얻는 유일한 제품군인 GPT-5.x로 제한됩니다. 다른 모든 것은 chat completions로 직접 이동하고 프로브하지 않습니다 (`_dispatch_acompletion`, 커밋 `137ede4c`) |
| `ValidationException: This model does not support assistant message prefill` | `anthropic/` 라우팅된 Bedrock 릴레이의 json_mode. LiteLLM은 `response_format`을 여는 중괄호를 어시스턴트 메시지로 미리 채워서 시뮬레이션하며, 최신 Bedrock 버전은 어시스턴트 턴으로 끝나는 대화를 거부합니다 | 해당 모델에 대해 `json_mode_enabled=false`를 설정하거나, 미리 채우기가 삽입되지 않는 OpenAI 호환 게이트웨이를 통해 라우팅합니다 |
| Zhipu 엔드포인트에 대한 모든 호출에서 `404` | 클라이언트가 이미 `/v4`로 끝나는 기본 URL에 OpenAI 스타일 `/v1`을 추가합니다 | 제공자가 문서화한 대로 정확하게 기본 URL을 구성합니다. FIM One은 `api_base`를 변경하지 않고 전달합니다 |
| 조용한 기간 후 `APIConnectionError: Connection error` | 중개자가 FIN이나 RST를 보내지 않고 유휴 풀된 연결을 회수했으며, httpx가 다음 쓰기에서 반죽은 소켓을 반환했습니다 | Keep-alive 만료 기본값은 5초이므로 턴 간 유휴 연결이 재사용되지 않고 폐기됩니다. `LLM_HTTP_MAX_KEEPALIVE=0`을 설정하여 재사용을 완전히 비활성화합니다 (`_get_shared_http_client`) |
| `Cannot send a request, as the client has been closed` | LiteLLM이 유휴 TTL에서 캐시된 SDK 클라이언트를 제거했으며, OpenAI SDK가 해당 클라이언트가 보유하고 있던 공유 httpx 세션을 닫았습니다 | 풀은 모든 시도 전에 재검증되고 닫혀 있을 때 재구축되며, LiteLLM의 오래된 클라이언트 캐시가 함께 플러시됩니다 (`_get_shared_http_client`, `_flush_litellm_client_cache`) |
| 청구된 입력 토큰이 보고된 캐시 읽기와 일치하지 않음 | 릴레이가 전달하기 전에 `cache_control`을 제거하므로 응답이 여전히 캐시 카운터를 보고하는 동안 전체 가격을 지불합니다 | `TurnProfiler`는 턴당 `read_tokens`과 `create_tokens`을 기록하며, 이는 릴레이 정직성 프로브로도 작동합니다. 청구서와 비교합니다 |
| `Function tools with reasoning_effort are not supported ... Please use /v1/responses instead` | 릴레이가 `reasoning_effort` 필드의 값이 아닌 존재 여부에 따라 chat completions을 보호하므로, FIM One이 추론을 비활성화하기 위해 보내는 명시적 `none`이 가드도 트립합니다. Uniapi에서 `gpt-5.6-luna`로 관찰됨 | 아무것도 없으며, Responses 경로가 작동하는 동안 아무것도 필요하지 않습니다. 해당 모델은 Responses 요청이 이미 실패한 후에만 chat completions에 도달합니다. 이를 Responses를 원하는 릴레이의 신호로 읽으며, `FIM_GPT5_RESPONSES_MODE=off`를 설정하는 이유로 읽지 마세요 |
| GPT-5.x가 Responses를 지원하는 엔드포인트에서 chat completions에 유지됨 | `404`가 해당 엔드포인트 및 모델에 대한 부정적 판정으로 캐시되었습니다 | `404`만 캐시됩니다. 누락된 경로는 구조적이기 때문입니다. `400`은 해당 호출에 대해 폴백되고 의도적으로 캐시되지 않으므로, 단일 오래된 추론 항목이 엔드포인트를 영구적으로 블랙리스트할 수 없습니다 (`_remember_native_failure`). 캐시는 프로세스별이므로 재시작은 어느 쪽이든 다시 프로브합니다 |
| 추론 블록이 거부되거나 접두사 캐시가 히트하지 않음 | 릴레이가 기록을 다시 작성하거나 재정렬하므로 재생된 `signature`이 더 이상 일치하지 않습니다 | 재생은 `reasoning_replay_policy`에 의해 중앙에서 결정되며, Anthropic 계열 ID만 재생됩니다. 릴레이 뒤의 Claude 모델이 인식할 수 없는 ID를 가지고 있으면 정책 테이블에 해당 조각을 추가합니다 |
