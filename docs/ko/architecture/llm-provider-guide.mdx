...
| Anthropic + thinking | `_run_native` | native_fc (thinking 자동 비활성화) | 구조화된 출력의 경우에만 thinking 비활성화 |
| Bedrock relay (thinking 없음) | `_run_native` | native_fc | 완전 지원 |
| Bedrock relay + thinking | `_run_native` | native_fc (thinking 자동 비활성화) | 구조화된 출력의 경우에만 thinking 비활성화 |
| OpenAI 호환 엔드포인트를 통한 Bedrock | `_run_native` | native_fc | 완전 지원 — prefill 문제 없음; 위의 팁 참고 |
...
FIM One은 확장 사고/추론을 제어하기 위해 두 개의 환경 변수를 노출합니다:

| 변수 | 값 | 효과 |
|---|---|---|
| `LLM_REASONING_EFFORT` | `low`, `medium`, `high` | LiteLLM에 `reasoning_effort`로 전달됩니다. Anthropic: `thinking` 매개변수로 매핑됩니다. OpenAI o-series: 통과됩니다. 기타: 자동으로 삭제됩니다(`drop_params=True`). |
...
2. **도구를 사용하는 GPT-5.x**: FIM One은 먼저 OpenAI Responses API(LiteLLM의 채팅 완료 브리지를 통해)를 시도합니다. 여기서 도구와 추론이 함께 허용되고 추론 상태가 도구 호출 라운드 전체에서 유지됩니다. 엔드포인트에 사용 가능한 `/v1/responses` 경로가 없는 경우(일부 OpenAI 호환 프록시), FIM One은 자동으로 `/v1/chat/completions`로 폴백하고 엔드포인트별 판정을 기억합니다. 해당 폴백 경로에서 도구를 사용하는 GPT-5.x는 명시적 `reasoning_effort: "none"`을 전송합니다. 엔드포인트가 도구 + 추론 조합을 거부하고 필드를 생략하는 것이 동등하지 않기 때문입니다(서버 기본값은 `none`이 아닙니다).
...
Bedrock + json_mode. 두 가지 해결책: (1) `LLM_JSON_MODE_ENABLED=false`를 설정하거나 관리자 모델 설정에서 JSON Mode를 비활성화; 또는 (2) Bedrock 제공자가 OpenAI 호환 `/v1/chat/completions` 엔드포인트를 제공하는 경우, 해당 엔드포인트로 전환 — FIM One이 `openai/`로 해석하며 prefill 주입이 발생하지 않습니다.

**"Thinking may not be enabled when tool_choice forces tool use"** / **"tool_choice 'specified' is incompatible with thinking enabled"**
Anthropic 모델의 경우, `structured_llm_call`이 native_fc 호출에 대해 자동으로 thinking을 비활성화합니다. 항상 활성화된 thinking을 지원하는 다른 제공자(예: Kimi K2.5)의 경우, 모델의 고급 설정에서 "Native Function Calling"을 비활성화하거나 전역적으로 `LLM_TOOL_CHOICE_ENABLED=false`를 설정하세요. 성능 저하 체인은 native_fc를 건너뛰고 대신 json_mode 또는 plain_text를 통해 구조화된 출력을 추출합니다.

**"DAG pipeline failed: LLM 'steps' is not an array"**
LLM이 `steps` 필드를 문자열 또는 단일 객체로 반환했습니다. 이는 일반적으로 구조화된 출력이 json_mode로 폴백되었음을 의미합니다(스키마 강제가 없음). 로그에서 `structured_llm_call: level=xxx`를 확인하세요 — `native_fc` 대신 `json_mode`를 표시하면 native_fc가 자동으로 실패하고 있습니다. 사용자 정의 `BaseLLM` 서브클래스를 사용하는 경우, `reasoning_effort` kwarg를 수용하는지 확인하세요.

**ReAct가 예기치 않게 JSON mode로 폴백**
...
모델이 어떤 레벨에서도 파싱 가능한 JSON을 생성하지 못했습니다. 최신 모델에서는 드문 경우입니다. 확인 사항: (1) 스키마가 유효한 JSON Schema인지, (2) 모델이 전체 응답을 생성할 충분한 `max_tokens`를 가지고 있는지, (3) 시스템 프롬프트가 스키마 지침과 모순되지 않는지. DAG 플래너와 분석기 모두 `default_value` 폴백을 제공하므로, 이 오류는 명시적으로 기본값을 생략하는 호출 사이트에서만 전파됩니다.
