...
이 루프가 DAG 스케줄링, Workflow 블루프린트, 업계 **그래프 엔지니어링**과 어떻게 함께 작동하는지는 [Planning Landscape](/architecture/planning-landscape)를 참조하세요.

## 아키텍처

ReAct 엔진은 2단계 실행 모델을 구현합니다. 첫 번째 단계는 반복적인 도구 사용 루프입니다. 에이전트는 LLM에 작업을 요청하고, 요청된 도구를 실행하며, 관찰 결과를 추가한 다음 LLM이 "완료"라고 신호를 보낼 때까지 이 과정을 반복합니다. 두 번째 단계는 답변 합성입니다. 별도의 스트리밍 LLM 호출이 전체 실행 추적을 읽고 사용자에게 표시할 응답을 생성합니다.

이러한 분리는 의도적으로 설계되었습니다. 도구 반복은 속도에 최적화되어 있습니다. 루프 내 모든 LLM 호출은 비스트리밍 `chat()`을 사용합니다. 사용자는 부분적인 JSON 작업이나 중간 추론 토큰을 볼 필요가 없기 때문입니다. 답변 생성은 UX에 최적화되어 있습니다. `stream_chat()`을 사용해 스트리밍하므로 사용자는 토큰이 실시간으로 나타나는 것을 볼 수 있습니다. 그 결과 도구는 빠르게 실행하면서도 답변은 반응성 있게 전달하는 두 가지 장점을 모두 얻을 수 있습니다.
...
도구 루프는 전체 대화 기록을 포함하는 `AgentResult`를 생성합니다. 여기에는 시스템 프롬프트, 사용자 질의, 모든 어시스턴트 메시지, 모든 도구 결과가 포함됩니다. `stream_answer()` 메서드는 이 추적 기록을 간결하고 일관된 답변으로 정리합니다. 합성 컨텍스트에서 각 도구 결과는 2,000자로 잘리므로, 여러 도구를 사용하는 복잡한 워크플로를 거친 후에도 프롬프트를 간결하게 유지할 수 있습니다.

**모델 바인딩.** LLM은 `ReActAgent.__init__()`에 주입되고 `self._llm`으로 저장됩니다. 단일 `run()` 호출 내의 모든 호출, 즉 모든 도구 루프 반복과 최종 답변 합성은 동일한 인스턴스를 사용합니다. 반복 과정에서 모델은 변경되지 않습니다. 다른 모델을 사용하려면 새로운 `ReActAgent`를 생성해야 합니다. DAG 모드에서 `DAGExecutor._resolve_agent()`는 이 패턴을 활용합니다. 각 단계 직전에 새로운 에이전트를 생성하고, 해당 단계의 ReAct 루프가 시작되기 전에 `step.model_hint`를 기반으로 `ModelRegistry`에서 모델을 선택합니다. 자세한 내용은 [DAG 엔진 — 단계별 재정의](/architecture/dag-engine#multi-llm-architecture)를 참조하세요.
...
1. 답변이 원래 질문을 완전히 다루고 있는가?
2. 도구 결과에서 핵심 사실이 검증되었는가?
3. 수집된 정보에 모순이 있는가?

체크리스트는 내부 교환입니다. 이를 답변하는 턴은 검증이지 답변이 아니므로, 이를 그대로 수락하면 "검증해보겠습니다: 1. ... ✓"이 채팅에 유출됩니다. 따라서 해당 턴은 내부에서 소비되고 에이전트에게 사용자 대면 답변을 다시 요청하며, 이것이 `stream_answer()`가 스트리밍하는 것입니다. 에이전트가 대신 뭔가 누락되었다고 결정하면, 도구 호출 루프를 재개하고 나중의 답변이 그대로 사용됩니다.

체크리스트는 실행당 최대 한 번 발동하며, 단순 대화형 응답(도구 미사용)에는 트리거되지 않으며, 남은 반복 예산이 검증 턴과 답변 턴을 모두 커버할 수 없을 때는 건너뜁니다.

임계값은 단순 작업의 불필요한 지연을 방지합니다. 단일 도구 호출(예: 빠른 계산 실행)은 검증을 보증하지 않지만, 다중 도구 조사는 최종 건전성 검사의 이점을 얻습니다. 항상 켜진 검증을 위해 `REACT_COMPLETION_CHECK_MIN_TOOLS=1`을 설정하세요.

이 기능은 기본적으로 활성화되어 있으며(`ReActAgent`의 `completion_check=True`), 에이전트 인스턴스별로 비활성화할 수 있습니다. DAG 단계는 기본값을 상속합니다. 각 단계는 자체 일회성 체크리스트를 얻습니다.
