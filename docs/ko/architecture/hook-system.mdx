...
Shipped된 것과 앞으로 진행할 것의 스냅샷:

- ✅ `HookRegistry`, `HookContext`, `HookResult` primitives가 ReAct와 DAG 모두에 연결됨
...
- ✅ Feishu 채널 콜백 엔드포인트가 `card.action.trigger`를 디코딩하고 pending 행을 업데이트함
- ✅ Agent 레벨 hook 선언: `agent.model_config_json.hooks.class_hooks`가 모든 ReAct/DAG 세션에서 인스턴스화된 `HookRegistry`로 해석됨
- ✅ **Hooks가 모든 실행 표면에서 작동**: 메인 채팅 경로(Portal, API, DAG), 위임된 sub-agent(`CallAgentTool`), Workflow `AGENT` 노드. Eval Center는 의도적으로 **hooks를 우회**함 (자동화된 평가는 인간 승인에서 차단되면 안 됨).

  위임된 agent는 호출자의 registry를 상속하지 않음 — 자신의 `model_config_json`에서 자신의 것을 빌드함. 확인 闸门이 모든 agent에 자동으로 첨부되므로, 재구축은 상속의 상위 집합임: delegate는 闸门 *plus* 자신이 선언한 것을 얻고, 闸门은 호출자의 `require_confirmation_for_all`이 아닌 delegate의 것을 해석함. 두 표면 모두 **closed fail**: hooks를 빌드할 수 없는 agent는 실행되지 않음.
- ❌ `AuditLogHook`, `ReadOnlyGuard`, `ResultTruncateHook`, `ConnectorRateLimitHook` (v0.9)
- ❌ 사용자 정의 YAML hook 선언 (v0.9)

Hook System은 v0.9 프로덕션 강화를 위한 **하중을 지탱하는 기초**임. 첫 번째 사용자(`FeishuGateHook`)는 또한 자체적으로 프로덕션 기능이므로, 전체 hook 카탈로그를 기다리지 않고 2026-04-24 roadshow를 위해 스켈레톤이 조기에 shipped됨.
