...
| `DAG_REPLAN_STOP_CONFIDENCE` | No | `0.8` | 에이전트가 목표를 **도달 불가능**하다고 판단할 때만 적용됨(불가능한 요청, 누락된 기능, 사용 불가능한 리소스): 이 확실성 이상에서 재시도 중단. 미완성 또는 절반만 완성된 결과물은 항상 신뢰도와 관계없이 재계획되며, `DAG_MAX_REPLAN_ROUNDS`만 이러한 재시도를 제한함 |
| `DAG_VERIFY_TRUNCATION` | No | `2000` | 단계 검증 LLM에 전송되는 단계 출력의 최대 문자 수(품질 판단용) |
| `DAG_ANALYZER_TRUNCATION` | No | `10000` | 실행 후 분석기 형식 지정 시 단계 결과당 최대 문자 수 |
| `DAG_STEP_EVIDENCE_CHARS` | No | `16000` | 단계당 보존되는 원본 도구 출력(웹 페칭, 검색 결과, 파일 읽기)의 최대 문자 수로, 권위 있는 "소스 증거"로 사용됨. 이는 분석기 및 최종 합성에 단계의 요약과 함께 제공되므로 답변의 사실적 주장(합계, 열거, 심각도)을 요약이 자동으로 삭제하거나 잘못 표시했을 수 있는 항목 대신 소스에 대해 검증할 수 있음. 증거 캡처를 비활성화하려면 `0`으로 설정 |
...
| `DAG_STEP_VERIFICATION` | No | `false` | 각 DAG 단계 후 일반적인 LLM 기반 품질 검사. 실패 시 단계가 피드백과 함께 한 번 재시도됨. **기본값 해제** — 모든 단계에서 지연 시간을 추가하며 거의 필요하지 않음. 대부분의 단계 출력은 재검사 없이 허용됨. 단계 결과 품질이 낮은 경우가 자주 관찰될 때만 사용 |
| `DAG_CITATION_VERIFICATION` | No | `true` | 전문 분야 단계에 대한 인용 정확성 검사. **전제 조건**: 쿼리가 먼저 LLM 도메인 분류기에 의해 전문 분야로 분류되어야 함(`ESCALATION_DOMAINS` 참조). 도메인이 감지되고 이 플래그가 `true`일 때, 완료된 각 단계는 법률/의료/금융 인용에 대해 스캔되고 정확성이 검증됨 — 할루시네이션된 문서 번호, 위조된 사건 참조, 잘못된 규제 인용을 포착함. 도메인 분류가 `null`을 반환하면(일반 쿼리) 이 설정과 관계없이 인용 검증이 실행되지 않음 |
