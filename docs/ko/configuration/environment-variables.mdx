...
| `REACT_MAX_TURN_TOKENS` | 아니오 | `0` | 긴급 차단기: 단일 ReAct 턴당 최대 누적 토큰(모든 반복에 걸친 프롬프트 + 완료). 기본값 `0` = 무제한. **이것은 일일 토큰 제어용이 아님** — 그 목적으로는 사용자별 `token_quota`를 사용하세요. 이것은 에이전트가 무한 도구 호출 루프에 갇혀 있는 것과 같은 극단적인 시나리오를 위한 최후의 안전 장치입니다. 이 한계에 도달하면 작업이 중간에 중단되어 지금까지 소비한 모든 토큰이 낭비되고 불완전한 결과가 반환됩니다. 구체적인 폭주 에이전트 문제가 있을 때만 `0`이 아닌 값으로 유지하세요 |
| `REACT_TOOL_SELECTION_THRESHOLD` | 아니오 | `12` | 등록된 도구의 총 개수가 이 임계값을 초과할 때, 각 요청 전에 가장 관련성 높은 부분 집합을 선택하는 경량 LLM 호출 실행 |
| `REACT_TOOL_SELECTION_MAX` | 아니오 | `6` | 스마트 선택 후 유지할 최대 도구 개수(`REACT_TOOL_SELECTION_THRESHOLD`를 초과할 때만 적용) |
| `REACT_SELF_REFLECTION_INTERVAL` | 아니오 | `6` | N번의 도구 호출마다 자기 성찰 프롬프트를 삽입하여 에이전트가 방향을 수정하고 루프를 피하도록 도움 |
| `REACT_TOOL_OBS_TRUNCATION` | 아니오 | `8000` | 최종 답변을 합성할 때 도구 관찰당 최대 문자 수. 값이 높을수록 더 많은 구조화된 데이터(JSON, 표)를 보존하지만 더 많은 토큰 사용 |
| `REACT_TOOL_RESULT_BUDGET` | 아니오 | `40000` | 단일 세션의 모든 도구 결과에 대한 집계 토큰 예산. 도구 결과 토큰 합계가 이 한계를 초과하면 새 결과는 공지와 함께 잘림. 대규모 API 응답(예: 각각 8K를 반환하는 5개의 커넥터 호출)으로 인한 컨텍스트 팽창 방지. `0`으로 설정하면 한계 비활성화 |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | 아니오 | `800` | 에이전트의 최종 답변이 이 많은 문자를 초과할 때 사후 답변 완료 확인 LLM 호출 건너뛰기. 길고 상세한 답변은 "뭔가 놓친 게 있나?" 검증 왕복이 필요 없음. 더 적극적으로 건너뛰려면 낮게 설정하고, 항상 확인을 실행하려면 매우 큰 값으로 설정 |
| `REACT_CYCLE_DETECTION_THRESHOLD` | 아니오 | `2` | 동일한 인수로 같은 도구가 연속으로 이 많은 횟수 호출될 때, 에이전트에게 다른 접근 방식을 시도하도록 지시하는 결정론적 경고 삽입. 자기 성찰(LLM이 루프를 인식하는 데 의존)과 달리 이것은 우회할 수 없는 해시 기반 확인. DAG 단계에도 적용 |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | 아니오 | `3` | 완료 체크리스트가 실행되기 전의 최소 도구 호출 수. 간단한 작업(1-2개 도구 호출)은 불필요한 지연을 피하기 위해 검증 건너뜀. 항상 켜진 검증을 위해 `1`로 설정. DAG 단계에도 적용 |
| `REACT_TURN_PROFILE_ENABLED` | 아니오 | `true` | 턴당 단계 수준 타이밍 로그(`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`) 내보내기. 턴당 하나의 구조화된 로그 라인. 프로파일링을 완전히 비활성화하려면 `false`로 설정(오버헤드 없음) |
| `REACT_PLAN_TOOL_ENABLED` | 아니오 | `true` | `update_plan` 할일 도구를 등록하여 에이전트가 다단계 작업 중에 계획 체크리스트를 작성하고 유지하도록 함. DAG 단계 에이전트 및 도구가 없는 에이전트에 대해 자동으로 건너뜀 |
| `REACT_PLAN_REMINDER_INTERVAL` | 아니오 | `3` | `update_plan` 호출 없이 경과한 도구 라운드 수 후에 오래된 계획 미리 알림(전체 체크리스트 포함) 재삽입되어 컨텍스트 압축을 통해 계획이 유지됨 |
| `REACT_MAX_CONTINUATIONS` | 아니오 | `3` | 모델의 답변이 제공자의 출력 토큰 한계(`finish_reason=length`)로 잘릴 때 최대 연속 라운드 수. 잘린 세그먼트는 에이전트 루프와 스트리밍된 합성 모두에서 하나의 매끄러운 답변으로 연결됨 |
| `LLM_RATE_LIMIT_PER_USER` | 아니오 | `true` | 단일 프로세스 전역 버킷 대신 사용자별 키 지정 속도 제한 버킷 사용. 한 명의 시끄러운 사용자가 같은 워커의 다른 모든 사용자를 굶기는 것을 방지. 기본 속도는 버킷당 분당 60개 요청 및 분당 100K 토큰으로 하드코딩됨 — 이 설정은 버킷이 공유(전역)인지 분할(사용자별)인지만 제어. 레거시 전역 버킷으로 되돌리려면 `false`로 설정(권장하지 않음) |
