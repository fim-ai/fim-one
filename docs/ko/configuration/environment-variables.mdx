...
### 관리자 패널의 ENV fallback을 최신 상태로 유지하기

Admin → Models에서 모델 그룹이 활성화되면 런타임은 해당 그룹에서 모델을 가져오며, 위의 `LLM_*` / `FAST_LLM_*` / `REASONING_LLM_*` 값은 fallback으로만 사용됩니다. 데이터베이스를 열지 않는 경로에서는 여전히 해당 값을 직접 읽습니다. 여기에는 `scripts/translate.py`(i18n pre-commit hook), `evals/`, Jina/OpenAI reranker, 이미지 생성이 포함됩니다. 이러한 경로가 오래된 provider를 사용하지 않도록 활성 그룹을 env 파일에 복사하세요.
...
스크립트는 ORM을 통해 provider 키를 읽고 파일에 직접 기록하며, 어떤 값도 출력하지 않습니다. 이후 앱을 다시 시작하세요.
