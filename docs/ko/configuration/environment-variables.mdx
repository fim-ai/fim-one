...
| `REACT_MAX_TURN_TOKENS` | 아니오 | `0` | 긴급 차단기: 단일 ReAct 턴당 최대 누적 토큰(모든 반복에 걸친 프롬프트 + 완료). 기본값 `0` = 무제한. **이것은 일일 토큰 제어용이 아님** — 그 목적으로는 사용자별 `token_quota`를 사용하세요. 이것은 에이전트가 무한 도구 호출 루프에 갇혀 있는 것 같은 극단적인 시나리오에 대한 최후의 안전 장치입니다. 이 제한에 도달하면 작업이 중간에 중단되어 지금까지 소비한 모든 토큰이 낭비되고 불완전한 결과가 반환됩니다. 특정 폭주 에이전트 문제가 있을 때만 `0`이 아닌 값으로 설정하세요 |
| `REACT_TOOL_SELECTION_THRESHOLD` | 아니오 | `12` | 등록된 도구의 총 개수가 이 임계값을 초과할 때, 각 요청 전에 경량 LLM 호출이 가장 관련성 높은 부분 집합을 선택합니다 |
...
| `REACT_TOOL_OBS_TRUNCATION` | 아니오 | `8000` | 최종 답변을 종합할 때 도구 관찰당 최대 문자 수. 높은 값은 더 많은 구조화된 데이터(JSON, 표)를 보존하지만 더 많은 토큰을 소비합니다 |
| `REACT_TOOL_RESULT_BUDGET` | 아니오 | `40000` | 단일 세션의 모든 도구 결과에 대한 집계 토큰 예산. 총 도구 결과 토큰이 이 상한을 초과하면 새 결과는 알림과 함께 잘립니다. 대규모 API 응답(예: 각각 8K를 반환하는 5개의 연결기 호출)으로 인한 컨텍스트 팽창을 방지합니다. `0`으로 설정하면 상한을 비활성화합니다 |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | 아니오 | `800` | 에이전트의 최종 답변이 이 많은 문자를 초과할 때 사후 답변 완료 확인 LLM 호출을 건너뜁니다. 길고 상세한 답변은 "뭔가 놓친 게 있나?" 검증 왕복이 필요하지 않습니다. 더 공격적으로 건너뛰려면 낮게 설정하고, 항상 확인을 실행하려면 매우 큰 값으로 설정하세요 |
| `REACT_CYCLE_DETECTION_THRESHOLD` | 아니오 | `2` | 동일한 인수로 같은 도구가 연속으로 이 횟수만큼 호출될 때, 결정론적 경고가 삽입되어 에이전트에게 다른 접근 방식을 시도하도록 지시합니다. 자기 성찰(LLM이 루프를 인식하는 데 의존)과 달리 이것은 우회될 수 없는 해시 기반 확인입니다. DAG 단계에도 적용됩니다 |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | 아니오 | `3` | 완료 체크리스트가 작동하기 전의 최소 도구 호출 수. 간단한 작업(1-2개 도구 호출)은 불필요한 지연을 피하기 위해 검증을 건너뜁니다. 항상 검증하려면 `1`로 설정하세요. DAG 단계에도 적용됩니다 |
| `REACT_TURN_PROFILE_ENABLED` | 아니오 | `true` | 턴별 단계 수준 타이밍 로그(`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`)를 내보냅니다. 턴당 하나의 구조화된 로그 라인. 프로파일링을 완전히 비활성화하려면 `false`로 설정하세요(오버헤드 없음) |
| `REACT_PLAN_TOOL_ENABLED` | 아니오 | `true` | `update_plan` 할일 도구를 등록하여 에이전트가 다단계 작업 중에 계획 체크리스트를 작성하고 유지하도록 합니다. DAG 단계 에이전트 및 도구가 없는 에이전트에 대해 자동으로 건너뜁니다 |
| `REACT_PLAN_REMINDER_INTERVAL` | 아니오 | `3` | `update_plan` 호출 없이 오래된 계획 미리 알림(전체 체크리스트 포함)이 대화에 다시 삽입되기 전의 도구 라운드 수입니다. 따라서 계획이 컨텍스트 압축을 견딥니다 |
| `REACT_PLAN_REPEAT_THRESHOLD` | 아니오 | `4` | 같은 도구를 호출하는 연속 라운드(다른 인수 사용) 수입니다. 이후 미리 알림이 반복되는 무익한 호출 대신 접근 방식을 변경하도록 에이전트에게 지시합니다. 정확히 중복된 호출은 주기 감지에 의해 별도로 처리됩니다 |
| `REACT_PLAN_NUDGE_AFTER` | 아니오 | `5` | 계획이 기록되지 않은 도구 라운드 수입니다. 이후 일회성 제안이 계획을 작성하도록 제시합니다. 계획 도구가 활성화된 경우에만 적용됩니다 |
| `REACT_MAX_CONTINUATIONS` | 아니오 | `3` | 모델의 답변이 제공자의 출력 토큰 제한(`finish_reason=length`)에 의해 잘릴 때 최대 연속 라운드 수입니다. 잘린 세그먼트는 에이전트 루프와 스트리밍된 종합 모두에서 하나의 매끄러운 답변으로 연결됩니다 |
| `REACT_BACKGROUND_TOOLS_ENABLED` | 아니오 | `true` | 느린 도구(샌드박스된 python/shell/node 실행)에 `run_in_background` 옵션을 제공합니다. 에이전트는 즉시 작업 ID를 받고 계속 작업하며, 결과는 도구가 완료될 때 `<task_notification>` 메시지로 도착합니다 |
| `REACT_BG_WAIT_TIMEOUT` | 아니오 | `300` | 에이전트가 답변을 최종화하려고 할 때 여전히 실행 중인 백그라운드 도구를 기다리는 최대 초 수입니다. 창 내에서 완료되지 않은 작업은 명시적 타임아웃 알림과 함께 취소됩니다 |
| `DAG_CHECKPOINT_EVIDENCE_CHARS` | 아니오 | `4000` | DAG 충돌 복구 체크포인트 파일(`data/dag_checkpoints/`)의 단계별 증거 상한입니다. 단계 요약은 전체적으로 저장됩니다 |
| `DAG_CHECKPOINT_MAX_AGE_HOURS` | 아니오 | `24` | 이보다 오래된 DAG 체크포인트는 로드 시 무시되므로 오래된 충돌 잔여물이 새 실행으로 복구되지 않습니다 |
| `LLM_RATE_LIMIT_PER_USER` | 아니오 | `true` | 단일 프로세스 전역 버킷 대신 사용자별 키 지정 속도 제한 버킷을 사용합니다. 한 명의 시끄러운 사용자가 같은 워커의 다른 모든 사용자를 굶주리는 것을 방지합니다. 기본 속도는 버킷당 분당 60개 요청 및 분당 100K 토큰으로 하드코딩되어 있습니다 — 이 설정은 버킷이 공유(전역)인지 분할(사용자별)인지만 제어합니다. 레거시 전역 버킷으로 되돌리려면 `false`로 설정하세요(권장하지 않음) |
...
| Variable | Required | Default | Description |
|---|---|---|---|
| ~~`SYSTEM_PROMPT_RESERVE`~~ | — | — | **제거됨.** 이전에는 시스템 프롬프트를 위해 컨텍스트 예산에서 고정 4K를 차감했습니다. ContextGuard가 메시지 목록 토큰을 추정할 때 이미 시스템 프롬프트를 포함하므로 이중 계산이 발생했습니다. 이제 예산 공식은 단순히 `context_size - max_output_tokens`이며, 시스템 프롬프트의 실제 크기는 동적으로 계산됩니다 |
