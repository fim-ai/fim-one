...
| `REACT_MAX_TURN_TOKENS` | No | `0` | 긴급 차단기: 단일 ReAct 턴당 최대 누적 토큰(모든 반복에 걸친 프롬프트 + 완료). 기본값 `0` = 무제한. **이는 일일 토큰 제어용이 아닙니다** — 그 목적으로는 사용자별 `token_quota`를 사용하세요. 이는 무한 도구 호출 루프에 갇힌 에이전트 같은 극단적인 시나리오에 대한 최후의 안전 장치입니다. 이 한계에 도달하면 작업이 중간에 중단되어 지금까지 소비한 모든 토큰이 낭비되고 불완전한 결과가 반환됩니다. 특정 폭주 에이전트 문제를 포함해야 하는 경우가 아니면 `0`으로 유지하세요 |
| `REACT_TOOL_SELECTION_THRESHOLD` | No | `12` | 등록된 도구의 총 개수가 이 임계값을 초과하면, 각 요청 전에 가장 관련성 높은 부분 집합을 선택하기 위해 경량 LLM 호출이 수행됩니다 |
| `REACT_TOOL_SELECTION_MAX` | No | `6` | 스마트 선택 후 유지할 최대 도구 개수(`REACT_TOOL_SELECTION_THRESHOLD`를 초과할 때만 적용) |
| `REACT_SELF_REFLECTION_INTERVAL` | No | `6` | N개의 도구 호출마다 자기 성찰 프롬프트를 주입하여 에이전트가 방향을 수정하고 루프를 피하도록 도움 |
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | 최종 답변을 합성할 때 도구 관찰당 최대 문자 수입니다. 높은 값은 더 많은 구조화된 데이터(JSON, 테이블)를 보존하지만 더 많은 토큰을 소비합니다 |
| `REACT_TOOL_RESULT_BUDGET` | No | `40000` | 단일 세션의 모든 도구 결과에 대한 집계 토큰 예산입니다. 도구 결과 토큰의 총합이 이 상한을 초과하면 새 결과는 알림과 함께 잘립니다. 대규모 API 응답(예: 각각 8K를 반환하는 5개의 커넥터 호출)으로 인한 컨텍스트 팽창을 방지합니다. 상한을 비활성화하려면 `0`으로 설정하세요 |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | 에이전트의 최종 답변이 이 많은 문자를 초과할 때 사후 답변 완료 확인 LLM 호출을 건너뜁니다. 길고 상세한 답변은 "뭔가 놓친 게 있나?" 검증 왕복이 필요하지 않습니다. 더 공격적으로 건너뛰려면 낮게 설정하고, 항상 확인을 실행하려면 매우 큰 값으로 설정하세요 |
| `REACT_CYCLE_DETECTION_THRESHOLD` | No | `2` | 동일한 인수로 같은 도구가 연속으로 이 횟수만큼 호출될 때, 에이전트에게 다른 접근 방식을 시도하도록 지시하는 결정론적 경고가 주입됩니다. 자기 성찰(LLM이 루프를 인식하는 데 의존)과 달리 이는 우회할 수 없는 해시 기반 확인입니다. DAG 단계에도 적용됩니다 |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | 완료 체크리스트가 실행되기 전의 최소 도구 호출 수입니다. 간단한 작업(1-2개 도구 호출)은 불필요한 지연을 피하기 위해 검증을 건너뜁니다. 항상 켜진 검증을 위해 `1`로 설정하세요. DAG 단계에도 적용됩니다 |
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | 턴당 단계 수준의 타이밍 로그(`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`)를 내보냅니다. 턴당 하나의 구조화된 로그 라인입니다. 프로파일링을 완전히 비활성화하려면(오버헤드 없음) `false`로 설정하세요 |
| `REACT_PLAN_TOOL_ENABLED` | No | `true` | 에이전트가 다단계 작업 중에 계획 체크리스트를 작성하고 유지하도록 `update_plan` 할일 도구를 등록합니다. DAG 단계 에이전트 및 도구가 없는 에이전트의 경우 자동으로 건너뜁니다 |
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | `update_plan` 호출 없이 진행되는 도구 라운드 수로, 그 후 오래된 계획 미리 알림(전체 체크리스트 포함)이 대화에 다시 주입되어 계획이 컨텍스트 압축을 견디도록 합니다 |
| `REACT_MAX_CONTINUATIONS` | No | `3` | 모델의 답변이 제공자의 출력 토큰 한계(`finish_reason=length`)로 잘릴 때의 최대 연속 라운드입니다. 잘린 세그먼트는 에이전트 루프와 스트리밍된 합성 모두에서 하나의 seamless 답변으로 연결됩니다 |
| `REACT_BACKGROUND_TOOLS_ENABLED` | No | `true` | 느린 도구(샌드박스 python/shell/node 실행)에서 `run_in_background` 옵션을 제공합니다. 에이전트는 즉시 작업 ID를 받고 계속 작업하며, 도구가 완료되면 결과가 `<task_notification>` 메시지로 도착합니다 |
| `REACT_BG_WAIT_TIMEOUT` | No | `300` | 에이전트가 답변을 최종화하려고 할 때 여전히 실행 중인 백그라운드 도구를 기다릴 최대 초 단위 시간입니다. 창 내에서 완료되지 않은 작업은 명시적 타임아웃 알림과 함께 취소됩니다 |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | 단일 프로세스 전역 버킷 대신 사용자별 키가 지정된 속도 제한 버킷을 사용합니다. 한 명의 시끄러운 사용자가 같은 워커의 다른 모든 사용자를 굶기는 것을 방지합니다. 기본 속도는 버킷당 분당 60개 요청 및 분당 100K 토큰으로 하드코딩되어 있습니다 — 이 설정은 버킷이 공유(전역)인지 분할(사용자별)인지만 제어합니다. 레거시 전역 버킷으로 되돌리려면 `false`로 설정하세요(권장하지 않음) |
