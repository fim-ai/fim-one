...
|---|---|---|---|
| `REACT_MAX_ITERATIONS` | No | `20` | 단일 ReAct 요청당 최대 도구 호출 반복 횟수. 값이 높을수록 더 철저하지만 느리고 비용이 더 많이 듭니다 |
| `REACT_MAX_TURN_TOKENS` | No | `0` | 긴급 차단 장치: 단일 ReAct 턴당 최대 누적 토큰(모든 반복에 걸친 프롬프트 + 완료). 기본값 `0` = 무제한. **이것은 일일 토큰 제어용이 아닙니다** — 그 목적으로는 사용자별 `token_quota`를 사용하세요. 이것은 무한 도구 호출 루프에 갇힌 에이전트 같은 극단적인 상황을 위한 최후의 안전 장치입니다. 이 한계에 도달하면 작업이 중간에 중단되어 지금까지 소비된 모든 토큰이 낭비되고 불완전한 결과가 반환됩니다. 특정한 폭주 에이전트 문제가 있을 때만 `0`이 아닌 값으로 설정하세요 |
| `REACT_TOOL_SELECTION_THRESHOLD` | No | `12` | 등록된 도구의 총 개수가 이 임계값을 초과할 때, 각 요청 전에 가장 관련성 높은 부분 집합을 선택하기 위해 경량 LLM 호출을 수행합니다 |
| `REACT_TOOL_SELECTION_MAX` | No | `6` | 스마트 선택 후 유지할 최대 도구 개수(`REACT_TOOL_SELECTION_THRESHOLD`를 초과할 때만 적용됨) |
| `REACT_SELF_REFLECTION_INTERVAL` | No | `6` | N번의 도구 호출마다 자체 반성 프롬프트를 주입하여 에이전트가 방향을 수정하고 루프를 피하도록 도움 |
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | 최종 답변을 종합할 때 도구 관찰당 최대 문자 수. 값이 높을수록 더 많은 구조화된 데이터(JSON, 테이블)를 보존하지만 더 많은 토큰을 소비합니다 |
| `REACT_TOOL_RESULT_BUDGET` | No | `40000` | 단일 세션의 모든 도구 결과에 대한 집계 토큰 예산. 도구 결과 토큰의 합계가 이 한계를 초과하면 새 결과는 알림과 함께 잘립니다. 대규모 API 응답(예: 각각 8K를 반환하는 5개의 커넥터 호출)으로 인한 컨텍스트 팽창을 방지합니다. 한계를 비활성화하려면 `0`으로 설정하세요 |
...
| `REACT_CYCLE_DETECTION_THRESHOLD` | No | `2` | 동일한 인수로 같은 도구가 연속으로 이 횟수만큼 호출될 때, 에이전트에게 다른 접근 방식을 시도하도록 지시하는 결정론적 경고가 주입됩니다. 자체 반성(LLM이 루프를 인식하는 데 의존)과 달리 이것은 우회할 수 없는 해시 기반 확인입니다. DAG 단계에도 적용됩니다 |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | 완료 체크리스트가 실행되기 전의 최소 도구 호출 수. 단순한 작업(1-2개 도구 호출)은 불필요한 지연을 피하기 위해 검증을 건너뜁니다. 항상 켜진 검증을 원하면 `1`로 설정하세요. DAG 단계에도 적용됩니다 |
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | 턴별 단계 수준 타이밍 로그(`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`)를 내보냅니다. 턴당 하나의 구조화된 로그 라인. 프로파일링을 완전히 비활성화하려면 `false`로 설정하세요(오버헤드 없음) |
...
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | `update_plan` 호출 없이 계획이 오래되었다는 알림(전체 체크리스트 포함)이 대화에 다시 주입되기 전의 도구 라운드 수. 계획이 컨텍스트 압축을 견디도록 합니다 |
