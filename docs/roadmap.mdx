...
- [x] OAuth auto-link requires a verified address on both sides, so an account made from an unverified provider email is never a bind target

#### Answer Rendering

- [x] Final answers stream natively — the loop hands off via a `finish` signal and the answer is written as a live token-streamed turn
- [x] Streamed markdown renders per block — finished blocks stay stable and half-arrived inline syntax no longer flickers
- [x] Answers render Mermaid diagrams, SVG figures and card-style comparison tables, with copy/export on answers, code blocks and tables
- [x] Rendered markdown is sanitized, closing raw-HTML injection from model output and uploaded files
- [x] Diagrams and code blocks download as files; reasoning folds to one-line previews by default in live and past conversations
- [x] Conversation exports are typeset for CJK — PDF embeds a real font (correct spacing, bullets and bold), DOCX declares an East Asian font, both on one size scale
...
- [x] Context budgets sit 8% below the model hard limit; startup warns when the fast model's window can't hold the general budget
