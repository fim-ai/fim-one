...
- [x] ~~**Intelligent Document Processing (Vision-Aware)**: Adaptive document handling — PDF pages rendered as images via PyMuPDF for vision-capable models (GPT-4o, Claude 3/4, Gemini); text-only fallback via pdfplumber. Per-model `supports_vision` flag + auto-detection. Three modes (vision/text/auto) configurable via `DOCUMENT_PROCESSING_MODE` env var. Cached page rendering with `.pages/` sidecar. 53 tests.~~ *(shipped in v0.8.1)*
