...
- [ ] `identity_providers` + `user_identity_map` models — DB-level per-org SSO/OAuth config replacing ENV-level third-party login
- [ ] Feishu SSO via IdP ("Login with Feishu" yields FIM session + upstream token, removes per-user API-key friction for Tier-2)
- [ ] Org graph sync via IdP (Feishu departments → FIM org tree); WeCom + DingTalk next
- [ ] Channel slims to Delivery + Gate; identity capabilities (SSO / OrgSync) live only in IdP
...
- [x] ~~Reasoning detail preserved end-to-end — loop streams its genuine final answer (no lossy re-synthesis); reasoning survives compaction, context rebuilds, and sub-agent steps~~ *(shipped in v0.8.8)*
