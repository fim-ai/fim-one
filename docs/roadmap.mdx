...
- [ ] **Dashboard Enhancement**: Render existing but unused API data — (1) **KB Top 3** card (most active knowledge bases by document count), (2) **Connector Health** card (circuit breaker status, green/yellow/red per connector), (3) **Workflow stats** section (active workflows count, recent runs with success/fail indicators, overall success rate trend). All data sources already exist in backend; dashboard API aggregation + frontend cards only
