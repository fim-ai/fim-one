...
- [x] **Conversation Recovery (partial — MVP)**: synthetic `tool_result` rows from `DbMemory._repair_dangling_tool_calls` are now persisted as durable `role="tool"` DB rows (idempotent via dialect-aware JSON path lookup), and a new `POST /chat/resume` endpoint replays cached SSE events after a given monotonic cursor so disconnected clients can resume a dropped stream without re-running the agent. Frontend playground auto-reconnects via `useSseResume` hook with exponential backoff (300ms → 1s → 3s, max 3 attempts) and "Reconnecting…" badge. Full design (dedicated `tool_call_id` column, per-step DAG checkpoint table, mid-stream thinking reconstruction) is tracked below as explicit follow-ups.
- [x] **System prompt section registry — memoized sections + cache breakpoints (MVP: ReAct only)**: new `fim_one.core.prompt` module with `PromptRegistry`, `PromptSection`, `DYNAMIC_BOUNDARY`, and `is_cache_capable()` helper. ReAct JSON mode, native function-calling mode, and synthesis now split system prompts into a cacheable static prefix (identity + guidelines + tool descriptions, ~95% of the prompt) and a per-call dynamic suffix (datetime + language directive); cache-capable providers (Claude, Bedrock Anthropic, Vertex Claude) receive two system messages with `cache_control: {"type": "ephemeral"}` on the prefix, while every other provider sees a byte-identical single concatenated message. `ChatMessage.cache_control` extension passes the breakpoint through `to_openai_dict()` → LiteLLM → Anthropic. Measured 91.9%–95.9% cache ratio on Claude 4 turns (~8,640 input tokens saved per 10-iteration ReAct run).
- [x] **Thinking-block persistence with signature (correctness fix)**: assistant messages carrying extended-thinking content now persist `reasoning_content` + `signature` in `metadata_["thinking"]` and replay them on subsequent turns. `ChatMessage.signature` field added; LiteLLM's three signature shapes (flat attr, `thinking_signature` alias, nested `thinking_blocks`) are all duck-typed. Without this, Anthropic's Claude 4 models rejected multi-turn conversations with HTTP 400 on thinking block signature mismatch. `is_thinking_capable()` ability gate extended to cover Claude 4 family, Anthropic proxies, DeepSeek R1, Qwen QwQ, OpenAI o-series, GPT-5 reasoning, and Gemini 2.x thinking — any CoT-emitting model receives the stream subscription, but only Anthropic needs the signature replay.
- [x] **Provider-aware reasoning replay policy (modelless hardening)**: the blanket serialization of `reasoning_content` across all providers (introduced by the fix above) was silently breaking every non-Anthropic provider's prefix / KV cache by mutating history bytes, and violating DeepSeek/Qwen documentation ("do not send `reasoning_content` back"). A centralized `reasoning_replay_policy(model_id)` resolver in `core/prompt/reasoning.py` now returns one of `anthropic_thinking` / `informational_only` / `unsupported` and gates serialization in `ChatMessage.to_openai_dict(replay_policy=...)`. Enforcement happens at the single choke point `OpenAICompatibleLLM._build_request_kwargs()`. Cross-provider matrix: Claude replays; DeepSeek-R1 / Qwen-QwQ / Gemini-thinking / o-series drop the field on outbound; GPT-4o / Gemini 1.5 / Mistral pass through unchanged. 50 tests cover the matrix with reverse assertions (proving the field is NOT leaked).
- [x] **Prompt cache observability**: `UsageSummary` tracks `cache_read_input_tokens` and `cache_creation_input_tokens` through the model layer; `TurnProfiler` aggregates them with an `add_cache_hit()` call site in both ReAct JSON and native function-calling paths, emits a single structured `turn_cache` log line per turn (`model | read_tokens | create_tokens | saved_input_tokens ~90%`), and the `/chat/*` `done_payload` exposes a `cache: { read_tokens, creation_tokens }` field for the frontend. Deliberately does **not** return a dollar estimate — pricing lookup belongs above the LLM layer so upstream can price per tier / relay markup. Also functions as a probe for detecting whether API relay stations honor Anthropic's cache discount (compare billed input vs `read_tokens`).
...
#### Reliability Follow-ups (from CC Insights P1/P2 matrix)

Bulk of the CC Insights integration blueprint (Phase 0–3, I.1–I.16) shipped across v0.8.2 and v0.8.3. The items below are from the parallel Priority Matrix that still need attention.

- [ ] **Content replacement state persistence** (CC invariant #2): "once seen, fate frozen" — message content that was already emitted to the client must not be retroactively mutated across resume / reload. Requires a replacement ledger aligned with the SSE cursor from A1. Blocked on understanding actual user-visible glitches; no active complaint.
