...
- **Stop-and-retry in the Playground can leave the user query bubble visually duplicated, or briefly vanish across turn transitions.** When a streaming turn is aborted and then retried, the backend persists the orphan user message; the frontend then has to reconcile three live render sources — `activeConversation.messages` (DB snapshot), the SSE `messages` stream, and the optimistic `pendingQuery` placeholder — with no single source of truth. During the pre-stream window both sources can render the same text ("query appears twice"), and during turn transitions the display can momentarily drop back to an older turn. The underlying data is correct: all orphan messages are persisted, carried into the next LLM call as context (via the new `normalize_alternating_messages` pipeline), and visible after a page refresh; the artifact is purely transient render timing. For context, Claude's own web UI exhibits an analogous class of bug — stopping mid-response and immediately sending a follow-up query sometimes forks the follow-up as a sibling-edit branch of the first query rather than appending it as a new turn — so this is a known hard problem in optimistic-UI + SSE + persisted-history designs, not a FIM-One-specific defect. A proper fix requires collapsing the three render sources into one derived state; deferred until a broader Playground state-machine refactor.
