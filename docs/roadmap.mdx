...
- [x] **Workflow Phase 2 Nodes**: Iterator (list iteration with max_iterations safety) and VariableAggregator (merge branch outputs via list/concat/merge/first_non_empty modes) — full frontend + backend + 20 new tests (138 total). Stats panel with success rate bar, relative time. 6 built-in templates.
