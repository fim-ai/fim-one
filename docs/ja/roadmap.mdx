...
- [x] GPT-5.x は Responses-API-first に移行（ツール + 推論を一緒に処理；404 はチャット完了にフォールバック）；その他のファミリーは設計上、完了のままです
- [ ] 次の LiteLLM アップグレード時に Responses-bridge ストリーミング使用数を確認（アップストリーム マッピング誤りの疑い）
