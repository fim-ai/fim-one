...
- [x] Force-logout タイムスタンプ比較は、タイムゾーン対応/非対応の値を UTC に正規化します — 失効したトークンは SQLite と PostgreSQL の両方で確実に拒否されます
- [x] Docker Compose PostgreSQL 認証情報は `POSTGRES_*` env で上書き可能です（接続文字列に単一ソース化）— 公開されたデプロイメントで誤ってデフォルトの `fim:fim` がシップされることはありません
...
- [x] ~~コンテンツガードレール v0 (入力/出力トリップワイヤー、ジェイルブレイク検出器、環境変数設定、`guardrail_tripwired` SSE イベント)~~ *(v0.8.7 でリリース済み)*
- [ ] オフトピックフィルター (分類器ベースの入力ガードレール) — 正規表現で十分な場合は v0.5 に延期
- [ ] PII リダクター出力ガードレール (正規表現 + 分類器ハイブリッド)
- [ ] エージェントごとのガードレール設定 UI (管理パネル)
...
- [ ] `identity_providers` + `user_identity_map` モデル — ENV レベルのサードパーティログイン置き換える DB レベルの組織ごと SSO/OAuth 設定
- [ ] Feishu SSO via IdP ("Login with Feishu" は FIM セッション + アップストリームトークン生成、Tier-2 のユーザーごと API キー摩擦を削除)
...
- [ ] **Connector Progressive Disclosure (Phase 5)**: **Semantic-Guided Tool Selection** (クエリからのエンティティ抽出 → Ontology Registry ルックアップ → コネクタセット削減; 50+ コネクタデプロイメントで 90%+ トークン削減); バッチ/ETL コネクタ向けスケールモード; CLI スタイルの汎用 `connector <name> <action> <params>` インターフェース
- [ ] **Cross-Connector Entity Alignment (Ontology Registry)** — *2026-04-21 にダウングレード: オンデマンドカスタム配信、コア機能ではない*: 共有エンティティタイプ (Customer、Order、Asset) をコネクタ間のフィールドマッピングで定義; DAGPlanner がクロスシステム JOIN キーを自動解決; ハードコードされたフィールド名なしでクロスコネクタクエリを有効化 (例: "Salesforce の顧客で Shopify で注文した顧客")
- [ ] **ホットプラグコネクタ**: OpenAPI スペックをアップロード、AI が設定を生成、5 分でライブ (再起動なし)
- [x] ~~**Marketplace Redesign Phase 1 — Solutions + Components**~~: 2 層マーケットモデル (Solutions: Agent/Skill/Workflow; Components: Connector/MCP Server); スコープセレクタ (Global Market / org); 統一サブスクリプションモデル (org 自動表示削除); Market スコープから KB を削除; データマイグレーション既存 org メンバーのサブスクリプションをバックフィル
- [ ] **Market Package System**: Marketplace 向けの配布可能リソースバンドル — リソースタイプごとの「マーケットプレイス」を統一パッケージングレイヤーに置き換え。`fim-package.yaml` マニフェストは以下を宣言: メタデータ (名前、バージョン、説明、作成者、ライセンス、タグ、`min_fim_version`)、エントリポイント (プライマリ Skill または Agent)、リソースリスト (agents、skills、connectors、KBs、MCP servers、workflows) と設定参照、パッケージ間依存関係 (semver 範囲)、必須認証情報 (インストール時収集用コネクタ参照にマップ)、およびデフォルト値を持つユーザー設定可能変数。**2 つの消費モード**: (1) **install** — すべてのリソースをバッチ作成 + ID 置換経由で内部参照を自動配線; インストールはソースにリンク、バージョン更新通知用; `POST /api/market/packages/{id}/install`; (2) **fork** — ユーザー所有の編集可能コピーとしてクローン、更新リンクなし (これはテンプレートモード); `POST /api/market/packages/{id}/fork`。追加エンドポイント: 公開 (`POST /api/market/packages` レビューワークフロー付き)、アンインストール (`DELETE /packages/{id}/uninstall` 依存関係チェック + 変更リソース確認付き)、バージョン履歴 (`GET /packages/{id}/versions`)、アップグレード (`POST /packages/{id}/upgrade` リソースごとの差分プレビュー付き)。ネストされたパッケージ要件の依存関係リゾルバー、競合検出付き。`PackageInstallation` テーブルはユーザーごとのインストール済みパッケージを追跡、アンインストール/アップグレード用リソース ID マッピング付き。**個別リソース公開と共存** — Package は合成レイヤー、置き換えではない; 単一 Connector は依然スタンドアロンで公開可能。例依存関係ツリー: `Package: contract-review` → `Skill: contract-review` (エントリポイント) → `Agent: contract-analyst` + `Agent: risk-scorer` → `KB: legal-clauses` + `Connector: docusign-api` + `MCP: pdf-extractor` + `Workflow: contract-approval-flow`
- [ ] **Creator Program**: マーケットプレイス収益化レイヤー — ポートフォリオページ付きクリエータープロフィール、パッケージごとの分析 (インストール、フォーク、アクティブユーザー、評価/レビュー)、パッケージが新規サブスクリプションを促進する場合のアフィリエイト手数料追跡。価格設定付き有料パッケージティア、購入フロー、承認ワークフロー。インストール傾向、収益レポート、ユーザーフィードバック付きクリエータダッシュボード。CI/CD パッケージ作成者向けプログラマティックパッケージ公開用公開クリエータ API。コミュニティ機能: パッケージコメント、Q&A、バージョンごとのチェンジログ
...
- [ ] **KB Advanced Editor**: 大規模ナレッジベースを管理するパワーユーザー向けビルダーモードエージェント — 一括 URL 取り込み、重複検出、ギャップ分析、ドキュメントライフサイクル管理; 既存 KB AI チャットを ReAct ツールループで拡張
- [ ] **Stripe Billing (v1 MVP — Pro Subscription)**: Free + Pro 2 層サブスクリプション、月次トークンクォータ付き。Stripe Checkout (ホスト型) + Customer Portal (セルフサービス) + webhook 駆動ライフサイクル (`checkout.session.completed` / `customer.subscription.updated|deleted` / `invoice.payment_succeeded|failed`)。クォータ枯渇時のソフトキャップ (HTTP 402 + アップグレードプロンプト) — v1 では超過料金なし。ユーザーごとの請求のみ; Org/Team サブスクリプションは v3 に延期。前提条件:
  - [x] ~~**Data model + SDK groundwork** (P1) — `billing_plans` / `subscriptions` / `stripe_webhook_events` テーブル、ORM モデル、Stripe SDK シングルトン、Free + Pro シード~~ *(v0.8.6 で配信)*
  - [x] ~~**Backend API + webhook handler** (P2) — `/api/billing/*` + `/api/webhooks/stripe` 署名検証 + べき等性付き; プラン対応クォータ; 時間ごとのライフサイクルスイープ~~ *(v0.8.6 で配信)*
  - [x] ~~**Frontend billing tab + 402 upgrade dialog** (P3) — `/settings?tab=billing` クォータ表示、アップグレード CTA、`past_due` バナー、ストリーム中 402 ダイアログ~~ *(v0.8.6 で配信)*
  - [x] ~~**Admin plan management** (P4) — `admin/billing/{plans,subscriptions}` CRUD~~ *(v0.8.6 で配信)*
  - [x] ~~**Admin-controlled billing feature flag** (P5) — `system_settings.billing_enabled` は Stripe パイプラインをゲート; べき等的な有効化は Free+Pro をシード、デフォルトプランポインタを設定、ユーザーをバックフィル; トグルオン/オフは有効化後の純粋フラグフリップ~~ *(v0.8.6 で配信)*
  - [ ] **Reconciliation + e2e + go-live** (P6) — 夜間 `subscriptions` ↔ `stripe.Subscription.list()` 調整スクリプト、見落とし webhook 復旧用; フルスタック happy-path / キャンセル中期 / past-due リグレッションテスト; テストモード `stripe_price_id` からライブ `price_id` に切り替え; ステージングで実カード使用のスモークテスト。

- [ ] **Team plan (Stripe seats)** — `stripe.Subscription.quantity` 経由のシートごと価格設定、Organization メンバーシップと統合。企業が N シート付きの 1 つのチーム全体プランにサブスクライブ可能; クォータと機能フラグは個別ユーザーではなくシートグループ経由で解決。v1.0 Stripe MVP と既存 Organization モデルに基づいて構築。
- [ ] **Group-level token quota for non-billing deployments** — Stripe なしのエンタープライズ/プライベートデプロイメントは organization レベルのトークン予算を設定。クォータチェーン `override > group > plan > default` に拡張; グループ解決は `max(user_quota, group_quota)` を使用、個別 VIP はチームキャップに制約されない。Team plan と並行してランド、同じプリミティブが請求ありとセルフホストトポロジーの両方に対応。

**Impact**: エンタープライズは FIM One をゼロからマルチシステムオーケストレーションに数日でデプロイ。パッケージシステムはクリエータエコシステムを作成 — ソリューション作成者は複合バンドル (Skill + Agents + Connectors + KBs + Workflows) を公開、エンタープライズはワンクリックでインストール、クリエータは採用から収益を得る。Install/fork 二重性は「そのまま使用」と「テンプレートからカスタマイズ」の両方のユースケースを単一メカニズムでカバー。
