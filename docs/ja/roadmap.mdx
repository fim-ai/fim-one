...
- [x] ~~LLM呼び出しの耐性：出力切り詰め継続＋529/504リトライ（Retry-After付き）（オーバーロードモデルフォールバックは既に`FallbackLLM`として実装済み）~~
- [x] ~~予算切り詰めツール結果はワークスペースに永続化してから切り詰め；ロッシー圧縮前に完全なトランスクリプトスナップショット~~
- [x] ~~ReAct内のバックグラウンドツール実行——ホワイトリスト登録された遅いツールはタスクIDを返し、完了時に通知~~
- [x] ~~段階的DAG再計画：完了したステップは再計画ラウンド間で保持；ステップレベルのチェックポイントは中断された実行をリトライで再開~~ {/* dev: dev/incremental-dag.md */}
