...
- [x] Force-logout タイムスタンプ比較により、タイムゾーン対応/非対応の値を UTC に正規化——失効したトークンは SQLite と PostgreSQL の両方で確実に拒否される
- [x] Docker Compose PostgreSQL 認証情報は `POSTGRES_*` env で上書き可能（接続文字列に単一ソース化）——公開されたデプロイメントで誤ってデフォルトの `fim:fim` が配布されることはなくなった
...
- [ ] DAG `tools_used` 精度（ステップ完了コールバック経由の実際のツール名からの ReAct）
...
- [x] 推理詳細がエンドツーエンドで保持される — ループがその真正な最終回答をストリーミングする（ロッシーな再合成なし）；推理はコンパクション、コンテキスト再構築、サブエージェントステップを通じて保持される
- [ ] Gemini Context Cache Adapter（REST キャッシュライフサイクルと Anthropic インラインマーカーの分離）
- [ ] プロンプトレジストリ拡張からプランナー / 検証者 / ドメイン分類器 / コンパクトへ
- [ ] エージェントごとの `cache_ttl`（一時的な 5 分 vs 拡張 1 時間）
- [ ] DAG ステップレベルのチェックポイントテーブル（DAG 中盤での再開用）
- [ ] 専用 `tool_call_id` メッセージカラム（スケール時のインデックス付きオーファンクエリ検索）
- [ ] ミッドストリーム思考トークン再構築（完全な SSE イベントより細かい再開粒度）
- [ ] API リレーキャッシュ正直性プローブ（管理者トリガー：`cache_control` をストリップするリレーを検出）
