...
### v0.8.9 (2026-07-08) — モジュール最適化 + 共有の統合 + 承認の強化
- [x] スキルとワークフローは管理者モジュールフラグの背後にソフトシェルフ化（デフォルトオフ）— コアのみブート；何も削除されず、管理者 → 設定 → モジュールから復元可能
- [x] 共有を統合 — KB共有を削除（KBは共有エージェント経由でのみ他者に到達）、DBコネクタは共有不可 + 生SQL所有者のみ、ワークフロービルダーを参照のみの9ノードに削減
- [x] Feishu承認の強化 — カードクリックは承認者の身元を強制、コールバック署名は失敗時にクローズ + 暗号化エンベロープを復号化、承認は意図しないチャットにルーティングされない
- [x] 使用時アクセス再確認 — 共有MCP サーバーとバインドされたKBは実行ごとに再検証；組織を離れるとサブスクリプションと保存された認証情報は即座に取り消される
- [x] エージェントループの強化 — プランボード、バックグラウンドツール、段階的DAG再計画 + チェックポイント再開、圧縮はツールペアリングを保持、切り詰め継続、529/504リトライ
- [x] `run_workflow` エージェントツール + ワークフロー正確性 — エージェントノードは完全なエージェントを実行、確認ゲートは失敗時にクローズ、コネクタ呼び出しはアクセス確認と監査ログ
- [x] アカウント削除の統合 — 管理者とセルフサービスは1つのパージルーチンを通じて、すべてのレコードとディスク上のファイルをカバー；組織所有者は最初に所有権を譲渡する必要がある
...
- [x] ~~JWTトークンタイプの制限（2FAバイパス）+ OAuthアカウント乗っ取り＆セッション修正~~ *(v0.8.7でリリース)*
- [x] ~~PostgreSQLタイムゾーン対応タイムスタンプ~~ *(v0.8.6でリリース)*
- [x] ~~ログアウト強制タイムスタンプ比較がタイムゾーン対応/非対応値をUTCに正規化——失効トークンはSQLiteとPostgreSQL両方で確実に拒否~~ *(v0.8.8でリリース)*
- [x] ~~Docker Compose PostgreSQL認証情報が`POSTGRES_*`環境変数でオーバーライド可能（接続文字列に単一ソース化）——公開デプロイメントで誤ってデフォルトの`fim:fim`がシップされることはない~~ *(v0.8.8でリリース)*
- [x] ~~SSRFブロックリストがIPv4マップIPv6バイパス（`::ffff:`インスタンスメタデータ）を閉塞；MCP SSE/Streamable-HTTPサーバーURLはSSRF検証を作成時+接続時に実施~~ *(v0.8.8でリリース)*
- [x] ~~所有者認証情報フォールバックはオプトイン：コネクタ/MCPサーバーはデフォルト`allow_fallback`オフ（既存行は反転）；フォールバックなしリソースで認証情報がない場合は呼び出し時に失敗する代わりにツールセットから非表示~~ *(v0.8.9でリリース)*
- [x] ~~リソースアクセスが可視性で統一（所有+購読）：購読コネクタ/KB/MCPサーバーはエージェントにバインド可能（「所有していない」403なし）、ワークフローコネクタステップはランナーのアクセスを強制（IDで任意のコネクタを実行しない）~~ *(v0.8.9でリリース)*
- [x] ~~Webhook/cronワークフロー実行は所有者のトークンクォータに計測——無人LLM/エージェントステップは事前にゲート、使用量はチャットと同じクォータにカウント、無計測フリーLLMパスを閉塞~~ *(v0.8.9でリリース)*
...
- [ ] **YAML/JSON コネクタ設定** — プラットフォームが自動生成する MCP サーバー
- [ ] **データベースコネクタ Phase 4** — Oracle (`oracledb`)、SQL Server (`aioodbc`)、GBase (`aioodbc` + GBase ODBC)。DM8 / KingbaseES / HighGo は v0.8.5 で提供
- [ ] **MCP コネクションプーリング** — STDIO をプール化し、ユーザーごとの環境分離を実現; `httpx.AsyncClient` を SSE/HTTP で共有。ウォームスタート ≤100 ms、HTTP サーバーあたり O(1) 接続を目標
- [x] ~~エージェントが `run_workflow` をプルして、決定論的ワークフローをサブステップとして実行 (静的レシピ ↔ Skill の動的レシピ); 人間がゲートするワークフローはトリガーのみ、実行は監査対象、深さ/再入性はガード~~ *(v0.8.9 で提供)*
...
#### エージェントループの堅牢化（Claude Codeパターン）

- [x] ~~計画ボード：`update_plan` todoツール＋古い計画の再注入リマインダーにより、長いReAct実行が目標を維持し、圧縮後も継続~~ *(v0.8.9で実装)*
- [x] ~~LLM呼び出しの耐性：出力切り詰め継続＋529/504リトライ（Retry-After付き）、オーバーロードモデルフォールバック（`FallbackLLM`として既に実装）~~ *(v0.8.9で実装)*
- [x] ~~予算切り詰めツール結果はワークスペースに永続化してから削除；ロッシー圧縮前に完全なトランスクリプトスナップショットを保存~~ *(v0.8.9で実装)*
- [x] ~~ReAct内のバックグラウンドツール実行——ホワイトリスト登録された遅いツールはタスクIDを返し、完了時に通知~~ *(v0.8.9で実装)*
- [x] ~~段階的DAG再計画：完了したステップは再計画ラウンド間で保持；ステップレベルのチェックポイントは中断された実行をリトライで再開~~ *(v0.8.9で実装)* {/* dev: dev/incremental-dag.md */}
- [x] ~~圧縮はtool_use/tool_resultペアリングを保持——ツール多用の長い実行はコンパクト後のプロバイダー400で失敗しなくなり、バックグラウンドツール結果はステップストリーム＋DAG証拠に到達~~ *(v0.8.9で実装)*
- [ ] ホットミッドストリームDAG再開（SSE再接続は実行中のターンに再接続）——コールドリトライ再開はチェックポイント経由で実装済み {/* dev: dev/incremental-dag.md */}
