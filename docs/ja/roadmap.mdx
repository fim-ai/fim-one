...
- [x] OpenAIモデルはResponses-API-firstで対応 — GPT-5.xはツールと推論を一緒に実行し、`/v1/responses`がないエンドポイントはチャット補完にフォールバック
- [ ] 次のLiteLLMアップグレード時にResponses-bridgeストリーミング使用数を検証（アップストリームのマッピング誤りが疑われる）
