...
- [x] ~~強制ログアウト時刻比較により tz 対応/非対応値を UTC に正規化——失効トークンは SQLite と PostgreSQL の両方で確実に拒否~~ *(v0.8.8 でリリース)*
- [x] ~~Docker Compose PostgreSQL 認証情報は `POSTGRES_*` env で上書き可能（接続文字列に単一ソース化）——意図しない `fim:fim` デフォルトの公開デプロイメントへの搭載を防止~~ *(v0.8.8 でリリース)*
- [x] ~~SSRF ブロックリストが IPv4 マップ IPv6 バイパス（`::ffff:` インスタンスメタデータ）を閉鎖；MCP SSE/Streamable-HTTP サーバー URL は作成時＋接続時に SSRF 検証~~ *(v0.8.8 でリリース)*
- [x] ~~所有者認証情報フォールバックはオプトイン：コネクタ/MCP サーバーはデフォルト `allow_fallback` オフ（既存行は反転）；認証情報がないフォールバックなしリソースはツールセットから非表示（呼び出し時に失敗する代わりに）~~
...
- [ ] Per-hook config pass-through (`{"name": ..., "config": {...}}` スキーマ)
- [ ] DAG `tools_used` 精度（ステップ完了コールバック経由の実際のツール名から ReAct）
- [ ] `CallAgentTool` + Workflow `AGENT` ノードの Hook 継承（セキュリティデフォルト vs 柔軟性デフォルト）
- [ ] 組み込み Hook：`ConnectorCallLog` 自動記録、読み取り専用モードブロック、DB 結果切り詰め、コネクタごとのレート制限
- [ ] `SessionStart` Hook ポイント + ユーザー定義 YAML Hook
- [x] ~~PreToolUse 強制 Hook はエラー時にフェイルクローズ——クラッシュした確認ゲートはもはや呼び出しを暗黙的に許可しない；非強制 Hook は `fail_open` 経由でフェイルオープンを維持~~ *(v0.8.8 でリリース)*
- [x] ~~Skeleton + FeishuGateHook + Approval Playground + ReAct/DAG ランタイム~~ *(v0.8.5 でリリース)*
...
- [ ] 高QPS キーの使用統計書き込みバッチ処理（プロセスローカルフラッシュオンシャットダウン、正確なカウント、新しい依存関係なし）— 測定されたホットスポットまで延期 {/* dev: dev/public-api-phase2.md */}
- [ ] エンドポイントごとのスコープ強制
...
- [x] ~~推論詳細がエンドツーエンドで保持される — ループはその真の最終回答をストリーミング（非可逆的な再合成なし）；推論はコンパクション、コンテキスト再構築、およびサブエージェントステップを通じて保持される~~ *(v0.8.8でリリース)*
- [ ] Gemini Context Cache Adapter（Anthropicのインラインマーカーとは別のREST キャッシュライフサイクル）
- [ ] プロンプトレジストリの拡張からプランナー / 検証者 / ドメイン分類器 / コンパクトへ
- [ ] エージェントごとの `cache_ttl`（一時的な5分対拡張1時間）
- [ ] DAG ステップレベルのチェックポイントテーブル（DAG 中盤での再開用）
- [ ] 専用 `tool_call_id` メッセージカラム（スケール時のインデックス付きオーファンクエリルックアップ）
- [ ] ミッドストリーム思考トークン再構築（完全なSSE イベントより細かい再開粒度）
- [ ] API リレーキャッシュ正直性プローブ（管理者トリガー：リレーが `cache_control` をストリップしているかどうかを検出）

#### 信頼性フォローアップ（Agent Core優先度マトリックス）

- [ ] コンテンツ置換状態の永続化（ストリーミング不変性 #2: 「一度見たら、運命は確定」）
- [ ] アタッチメントコンテキストルーター（重複排除、予算集約、生存性チェック；Workspaceオフロードと連携）
- [ ] サイドクエリワーカー（メインレート制限予算と競合しないよう、リコール/分類/要約用の専用プール）

#### エコシステム + スケーリング

- [ ] **スケジュール済みジョブ + イベントトリガー型エージェント** — `scheduled_jobs` + `job_runs` + APScheduler; cron + webhook-inbound。Hub モード向けの非同期サブエージェント使用例
- [ ] **ワークフロートリガー ID 可観測性** — `ExecutionContext.trigger_source` (`webhook | cron | manual | batch | sub`) が 5 つの WorkflowEngine サイト全体で入力され、実行パネルとコネクタログに表示
