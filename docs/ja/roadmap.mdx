...
- [x] ~~Docker Compose PostgreSQL 認証情報は `POSTGRES_*` env で上書き可能（接続文字列に単一ソース化）——公開されたデプロイメントで誤って出荷された `fim:fim` デフォルトはもう発生しない~~ *(v0.8.8 でリリース)*
...
- [x] ~~所有者認証情報フォールバックはオプトイン：コネクタ/MCP サーバーはデフォルトで `allow_fallback` オフ（既存行は反転）；フォールバックなしのリソースで認証情報がない場合、呼び出し時に失敗するのではなく、ツールセットから非表示になる~~
- [x] ~~リソースアクセスは可視性で統一（所有 + 購読）：購読したコネクタ/KB/MCP サーバーはエージェントにバインド可能（「所有していない」403 はもう発生しない）、ワークフロー コネクタ ステップはランナーのアクセスを強制する（ID でコネクタを実行しない）~~
