...
- [x] 最終回答はネイティブにストリーミングされます — ループは `finish` シグナルを介してハンドオフされ、回答はライブトークンストリーミングターンとして書き込まれます
- [x] 回答はMermaidダイアグラム、SVG図、カードスタイルの比較表をレンダリングし、回答、コードブロック、テーブルにコピー/エクスポート機能があります
- [x] レンダリングされたマークダウンはサニタイズされ、モデル出力とアップロードされたファイルからの生のHTML注入を防ぎます
- [x] ダイアグラムとコードブロックはファイルとしてダウンロードでき、推論はライブおよび過去の会話ではデフォルトで1行プレビューに折りたたまれます
- [x] 会話エクスポートはCJK用に組版されます — PDFは実フォント（正しい間隔、箇条書き、太字）を埋め込み、DOCXは東アジアフォントを宣言し、両方とも1つのサイズスケールで表示されます
