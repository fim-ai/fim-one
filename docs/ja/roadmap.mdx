...
- [x] サイドバーがチャットクラスター中心に再編成——会話が「新規チャット/検索」の直下に表示され、モジュールナビゲーションはコンパクトな下部ドックに配置
- [x] `/clear` スラッシュコマンドで入力ボックスから新しい会話を開始
- [x] 管理者モデルリストが Shift クリック範囲とワンリクエスト一括削除に対応したチェックボックス複数選択をサポート
- [x] 実行中のエージェントステップが生成された1行タイトルを単一の折りたたみヘッダーに表示し、会話履歴に保持
- [x] 新しく送信されたメッセージがトランスクリプトの最上部に移動し、回答がその下のスペースに展開
