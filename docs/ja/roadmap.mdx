...
#### エージェントループの堅牢化（Claude Code パターン）

- [x] ~~計画ボード：`update_plan` todo ツール + 古い計画の再注入リマインダーにより、長時間の ReAct 実行を目標に保ち、圧縮後も継続~~
- [x] ~~LLM 呼び出しの耐性：出力切り詰め継続 + 529/504 リトライと Retry-After（オーバーロードモデルフォールバックは既に `FallbackLLM` として実装済み）~~
- [x] ~~予算切り詰めツール結果はワークスペースに永続化してから削除；損失のある圧縮前に完全なトランスクリプトスナップショット~~
- [x] ~~ReAct でのバックグラウンドツール実行——ホワイトリスト登録された遅いツールはタスク ID を返し、完了時に通知~~
- [x] ~~段階的 DAG 再計画：完了したステップは再計画ラウンド全体で保持；ステップレベルのチェックポイントは中断された実行をリトライで再開~~ {/* dev: dev/incremental-dag.md */}
- [x] ~~圧縮は tool_use/tool_result ペアリングを保持——ツール多用の長時間実行はコンパクト後のプロバイダー 400 で失敗しなくなり、バックグラウンドツール結果がステップストリーム + DAG エビデンスに到達~~
- [ ] ホットミッドストリーム DAG 再開（SSE 再接続は実行中のターンに再接続）——コールドリトライ再開はチェックポイント経由で実装済み {/* dev: dev/incremental-dag.md */}
