...
**目標**: データベースコネクタを同僚に安全に渡せるようにし、ツリー内にすでに存在する未完成の経路を完了させる。

#### DBコネクターフェンス {/* dev: dev/connector-rbac/00-overview.md */}

- [x] PII列のマーキング — マーキングされた列はスキーマに残り、すべてのクエリ結果で値がマスクされる
- [x] スキーマの可視性 — テーブルと列の許可・拒否、および動詞のブロック（読み取り専用の強制）
- [x] フェンスの監査可能性 — `ConnectorCallLog` の `scope_rules_applied` に、各呼び出しが適用したフェンスを記録する

#### 共有の取り消し

- [x] 公開解除が機能する — サブスクリプションによるアクセスは、リソースが引き続き共有されている間に限られ、使用時に再確認されます
- [x] Admin Market での取り下げにより、リソースは一覧から非表示になるだけでなく、非公開に戻されます
- [x] ソリューションのサブスクリプションを解除しても、ユーザーが自分で設定したコネクタや認証情報が削除されなくなりました
- [x] 組織を削除すると、公開済みのリソースは所有者に戻され、組織スコープのサブスクリプションが取り消されます

#### 既存の項目を仕上げる

- [ ] メインチャット経路でのチャンク分割コンパクション入力とモデル対応予算。これにより、どのようなモデル構成でもウィンドウ内に収まるようにする
- [ ] 次回のLiteLLMアップグレードでResponses-bridgeのストリーミング使用量を検証する（上流でのマッピングミスが疑われる）
- [ ] ネイティブなGPT-5.x経路が完全なリリースを終えたら、LiteLLMのchat→responsesブリッジを廃止する
- [ ] Stripeの課金を本番稼働 — 失われたwebhookを検出するためStripeとの夜間照合、フルスタック回帰テスト、ライブ価格ID
...
上記に記載されていない機能は、計画されていません。これは見落としではなく、意図的な選択です。プロジェクトはメンテナンス段階にあり、機能一覧を充実させることではなく、実際に誰かが必要としているものを優先して作業を進めています。

不足しているものがある場合は、行き詰まっている問題を説明する issue を作成してください。具体的にブロックされているユースケースこそが、項目をこのページに追加するきっかけになります。
