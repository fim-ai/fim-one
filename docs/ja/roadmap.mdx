...
- [x] ~~JWT トークンタイプ制限（2FA バイパス）+ OAuth アカウント乗っ取り＆セッション修正~~ *(v0.8.7 でリリース)*
- [x] ~~PostgreSQL タイムゾーン対応タイムスタンプ~~ *(v0.8.6 でリリース)*
- [x] Force-logout タイムスタンプ比較は、タイムゾーン対応/非対応値を UTC に正規化することで、SQLite と PostgreSQL の両方で失効トークンを確実に拒否します
