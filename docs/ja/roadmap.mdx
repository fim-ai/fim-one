...
- [x] サイドバーがチャットクラスタの周りに再編成されました — 会話は「新規チャット/検索」の直下に配置され、モジュールナビゲーションはコンパクトな下部ドックに配置されます
- [x] `/clear` スラッシュコマンドで入力ボックスから新しい会話を開始します
- [x] 管理者モデルリストは Shift クリック範囲とワンリクエスト一括削除をサポートするチェックボックス複数選択に対応しています
- [x] 実行中のエージェントステップは生成された1行のタイトルを単一の折りたたみヘッダーに表示し、会話履歴に保持されます
- [x] 新しく送信されたメッセージはトランスクリプトの上部に上昇し、回答がその下のスペースに拡大します
- [x] リストページは最初の読み込み時にカードを段階的に表示し、すべてのアニメーションはシステムの動きを減らす設定を尊重します
