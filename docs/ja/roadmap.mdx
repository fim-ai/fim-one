...
- [x] ~~ログアウト強制タイムスタンプ比較は tz 対応/非対応値を UTC に正規化 — 失効トークンは SQLite と PostgreSQL の両方で確実に拒否~~ *(v0.8.8 でリリース)*
- [x] ~~Docker Compose PostgreSQL 認証情報は `POSTGRES_*` env で上書き可能（接続文字列に単一ソース化）— 公開デプロイで誤ってリリースされた `fim:fim` デフォルトはもう発生しない~~ *(v0.8.8 でリリース)*
- [x] ~~SSRF ブロックリストは IPv4 マップ IPv6 バイパス（`::ffff:` インスタンスメタデータ）を閉じる；MCP SSE/Streamable-HTTP サーバー URL は作成 + 接続時に SSRF 検証~~ *(v0.8.8 でリリース)*
