...
- [x] ~~計画ボード：`update_plan` todo ツール + 古い計画の再注入リマインダーが長い ReAct 実行を目標に保ち、圧縮後も機能~~
- [x] ~~LLM 呼び出しの耐性：出力切り詰め継続 + 529/504 リトライと Retry-After（オーバーロードモデルフォールバックは既に `FallbackLLM` として実装済み）~~
- [ ] 大規模なツール結果をワークスペースファイルに永続化し、ヘッド切り詰めの代わりに使用；LLM 圧縮前の完全なトランスクリプトスナップショット
- [ ] ReAct でのバックグラウンドツール実行——遅いツールはプレースホルダーを返し、完了時に通知
- [ ] インクリメンタル DAG 再計画：再計画ラウンド全体で完了したステップを保持；DAG 中盤の再開用にステップ状態を永続化 {/* dev: dev/incremental-dag.md */}
