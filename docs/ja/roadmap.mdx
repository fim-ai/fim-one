...
- [ ] コンテンツ置換状態の永続化（ストリーミング不変式 #2：「一度見たら、運命は確定」）
- [ ] アタッチメントコンテキストルーター（重複排除、予算集約、生存確認；Workspaceオフロードと連携）
- [ ] サイドクエリワーカー（リコール/分類/要約用の専用プール。メインレート制限予算との競合を回避）
- [x] 管理者LLMプロバイダー編集がプロセス再起動なしで適用される（LiteLLMクライアントキャッシュをインバリデート時にフラッシュ + 60秒レジストリTTLセーフティネット）
