...
- ✅ エージェントレベルのフック宣言：`agent.model_config_json.hooks.class_hooks` が ReAct/DAG セッションごとにインスタンス化された `HookRegistry` に解決
- ✅ **すべての実行サーフェスでフックが発火**：メインチャットパス（Portal、API、DAG）、委譲されたサブエージェント（`CallAgentTool`）、Workflow `AGENT` ノード。Eval Center は意図的に**フックをバイパス**（自動評価は人間の承認でブロックされてはいけない）。

  委譲されたエージェントは呼び出し元のレジストリを継承しない——自身の `model_config_json` から**独自の**レジストリを構築する。確認ゲートはすべてのエージェントに自動的に付加されるため、再構築は継承のスーパーセット：委譲先はゲート*プラス*自身が宣言したもの を取得し、ゲートは呼び出し元の `require_confirmation_for_all` ではなく委譲先の `require_confirmation_for_all` を解決する。両方のサーフェスは**安全側で失敗**：フックを構築できないエージェントは実行されない。
- ❌ `AuditLogHook`、`ReadOnlyGuard`、`ResultTruncateHook`、`ConnectorRateLimitHook`（v0.9）
- ❌ ユーザー定義 YAML フック宣言（v0.9）

Hook System は v0.9 本番環境対応の**基盤となるシステム**。その最初のユーザー（`FeishuGateHook`）も本番機能そのもの。そのため、フック全体カタログを待つのではなく、2026-04-24 路演のために早期にスケルトンをリリースした。
