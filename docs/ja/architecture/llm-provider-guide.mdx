...
title: "LLM プロバイダー互換性"
icon: "microchip"
description: "FIM One が LLM 呼び出しをルーティングする方法、tool_choice アーキテクチャ、およびプロバイダー固有の落とし穴——特に Anthropic thinking と AWS Bedrock について。"
mode: "wide"
---
...
One-shot の構造化抽出（スキーマアノテーション、DAG 計画、計画分析）。モデルに特定の仮想関数の呼び出しを強制し、構造化された JSON 出力を保証します。これはプロバイダー固有のエラーを引き起こす呼び出し箇所です。

`structured_llm_call` は、3 段階の縮退チェーンを実装します。3 つの段階はコードにちなんで命名されており、[以下のセクション](#structured-output-the-four-guarantee-tiers)で説明する 4 つの保証レベルとは番号が一致しません。
...
| レベル | リクエストに含まれるもの | ペイロードの到着先 | 保証される内容 | 適用する仕組み |
|---|---|---|---|---|
| **T1 — ネイティブな function calling、`strict: true`** | 関数定義に `strict` を設定した `tools=[…]` | `tool_calls[0].function.arguments` 内の JSON 文字列 | スキーマレベル：すべての必須フィールドが存在し、未宣言のキーがなく、型と enum が正しい | デコーダーのステートマシン |
| **T2 — `response_format: {"type": "json_schema"}`** | `strict: true` を設定した、`response_format` 内のスキーマ自体 | `message.content` 内の JSON 文字列 | T1 と同じスキーマレベル。ただし、tool call ではなく assistant 自身の返信に対して適用 | デコーダーのステートマシン |
| **T3 — `response_format: {"type": "json_object"}`** | スキーマなしで、型のみ | `message.content` 内の JSON 文字列 | 構文上のみ：テキストが JSON として解析できること。どのキーが含まれるかは制約されない | デコーダー（構文） |
| **T4 — 自由形式のテキストと指示** | 何もなし。スキーマは prompt で説明する | `message.content`。説明文やコードフェンスで囲まれる場合がある | なし | prompt |
...
両者は異なるチャネルを通じて同じ保証を提供しますが、そのチャネルの違いは3つの点で重要です。

**ターンの形状が異なります。** T1はツール呼び出しターンを返します。つまり、モデルは*実行する*ことを決定しています。T2は通常のアシスタントメッセージを返します。つまり、モデルはある形で*回答しています*。呼び出し元が必要としているのが1回の抽出であり、エージェントループではない場合、T2はモデルが「呼び出す」ための仮想関数を作り出すことなく、その意図を直接示します。

**T1の強制は、思考と衝突します。** T1からスキーマで制約されたペイロードを取得するには、通常、`required` または名前付き関数のいずれかによって選択を強制する必要があります。拡張思考が有効な場合、強制されたツール選択を拒否するプロバイダーがいくつかあります。どのプロバイダーが該当するかは、下記の表Bに記載されています。T2にはこのような競合がありません。T2はツールの制約ではなくレスポンスの制約であるため、思考を有効にした状態でも利用できる唯一のスキーマ制約付きパスです。推論モデルを標準とするデプロイメントにとって、これはスタイル上の選択ではなく、T2を選ぶ実務的な理由になります。

**T1では、T2にはないスキーマ連携のラウンドが必要です。** 仮想関数には、名前、説明、そしてモデルがその関数の呼び出しを拒否できるようにするかどうかの判断が必要です。

### T2 のコスト

制約付きデコーディングは無料ではなく、すでに用意しているスキーマが、そのまま受け入れられるスキーマであるとは限りません。

- **スキーマのサブセット。** OpenAI の strict mode では、すべてのオブジェクトに `additionalProperties: false` を指定し、すべてのプロパティを `required` に列挙する必要があります。オプショナル性は `required` から省略するのではなく、`null` とのユニオンとして表現します。ルートはオブジェクトでなければなりません。プロパティの総数とネストの深さには上限があります。手書きのスキーマの多くは、利用可能にする前に編集が必要です。
...
すべてのティアは形式を制約しますが、真実性を制約するものはありません。T2 のレスポンスはスキーマに完全に準拠していても、事実としては誤っている可能性があります。また、列挙型で制約されたフィールドは、正しい値がどれも存在しない場合でも、許可された値のいずれかを返します。これは、デコーダーの役割が答えを知ることではなく、出力を文法の範囲内に収めることだからです。スキーマによるゲーティングによって、解析エラーやフィールド形式の不具合は解消されます。しかし、値の内容を確認する必要がなくなるわけではありません。

<Warning>
**2 つの番号体系があり、対応していません。** このページでは、上記の保証ティアに **T1–T4** を使用しています。一方、`structured_llm_call` の劣化チェーンにおける 3 つの段階には **Level 1/2/3** を使用しています。これらはコード内の名称（`native_fc`、`json_mode`、`plain_text`）にちなんだものです。FIM One の Level 2 は `json_object` であり、T2 ではなく **T3** です。このチェーンが 3 つの段階で構成されているのは、T2 を完全にスキップしているためであり、ティアが 3 つしかないためではありません。
</Warning>

### FIM Oneの位置付け

| `structured_llm_call` level | Tier | コードが実際に行うこと |
|---|---|---|
| Level 1 — `native_fc` | **`strict` なしの T1** | `_build_tool_def` は `name` / `description` / `parameters` だけを出力する。呼び出し箇所で `strict` を設定しているものはないため、`responses_adapter.py` の `convert_tools` ではデフォルトで `False` になる。スキーマは、生成すべき内容の説明としてモデルに渡されるのであって、生成可能な内容を制約するものではない |
| *(absent)* | **T2** | 未実装。`json_schema` はコードベースのどこにも登場せず、`abilities` にもモデルごとのサポートを表現できるフラグはない |
| Level 2 — `json_mode` | **T3** | `structured.py::_call_llm` と `react.py::_json_response_format` にハードコードされた `{"type": "json_object"}` |
| Level 3 — `plain_text` | **T4** | 自由形式のコンテンツに対する `extract_json()`。呼び出し箇所ごとに、オプションで正規表現のフォールバックを使用できる |

ここから、現在の設計における正直な制約として、5つの帰結が導かれます。

1. **チェーンには、スキーマによって制約されたフォールバック段階がない。** Level 1 が失敗した場合、次の段階は `json_object` であり、保証されるのはテキストをパースできることだけです。フィールドを引き続き強制する中間段階はありません。
2. **Level 1 の保証は、その名前から受ける印象よりも弱い。** `strict` なしのネイティブ function calling は、2024年以前の非 strict 形式です。モデルは通常スキーマに従いますが、必須フィールドの省略、存在しないキーの追加、数値として宣言された値の文字列での返却を防ぐことはできません。
3. **`anthropic/` ルートでは、Level 2 は実質的には T3 ではない。** Anthropic Messages API には `response_format` がないため、LiteLLM は assistant prefill を注入して JSON mode をエミュレートします。これはプロンプト層の仕組みなので、実効的な保証は T4 と T3 の間に位置し、T3 と同等ではありません。以下で説明する Bedrock の prefill trap は、このエミュレーションが明示的に失敗するケースです。より見えにくい問題は、パラメーター名が示唆するような保証が、そもそも存在していなかったことです。
4. **結果が後からスキーマに対して検証されることはない。** `jsonschema` は依存関係に含まれていません。行われる検証はすべてオプションの `parse_fn` 内、呼び出し箇所ごとのものであり、その厳密さは呼び出し箇所によって異なります。
5. **失敗は表面化せず、吸収される。** ほぼすべての呼び出し元が `default_value` を渡すため、チェーンを使い切っても例外を送出せず、一見もっともらしいオブジェクトを返します。`StructuredCallResult.level_used` にはどの段階で値が生成されたかが記録されますが、これを読み取る呼び出し箇所はありません。また、`default_value` の経路では、実際には何も生成されていないチェーンについても `plain_text` と報告されます。

**実際にどの段階にいるかを確認する方法。** `structured_llm_call` は、完了した各呼び出しについて `INFO` レベルで1行をログに記録します。このログはデータが `parse_fn` を通過した後にのみ出力されるため、実際に成功した段階が示されます。
...
完全な失敗は `WARNING` レベルで `level=none outcome=default_value` として記録されます。この2行を1日分のトラフィックに対して grep することだけが、デプロイ環境において T2 の欠落がコストを生んでいるかどうかを知る方法です。`default_value` の設計により、症状はエラーではなく、質の低い回答として現れるためです。

### 各ティアのプロバイダーサポート

<Note>
**この表が記載するのは、FIM One のコードパスではなくアップストリーム API です。** このページにある他の表では、すべて動作を実装する関数を示しています。この表でそれができないのは、FIM One がどのプロバイダーの T2 機能も使用せず、いずれにも `strict` を設定していないためです。ここでは、T2 を実装する判断を、実際に利用可能な機能に基づいて開始できるようにしています。ベンダーの機能は急速に変化しており、あるファミリー内でも一部のモデルに先行してサポートが追加されることがあります。行の内容を前提にする前に、ベンダーの公式ドキュメントを再確認してください。
</Note>

| プロバイダー | T1 `strict` | T2 `json_schema` | T3 `json_object` | 備考 |
|---|---|---|---|---|
| **OpenAI** | ✅ | ✅ | ✅ | 両方の strict ティアのリファレンス実装です。Strict function calling と `response_format: json_schema` は、同じ文法エンジンと同じスキーマサブセットを共有します。`json_object` は互換性のために維持されている旧来の JSON モードです |
| **Anthropic** | ⚠️ | ⚠️ | ❌ | Messages API に `response_format` はまったくありません。tool の `input_schema` は検証されますが、デコーダーによる強制は行われません。Strict tool use とスキーマ制約付き出力フォーマットは、最近の Claude モデルでベータヘッダー経由により利用できます。FIM One は LiteLLM 経由で Anthropic にアクセスしますが、LiteLLM は prefill によって JSON モードをエミュレートし、ベータ機能は使用しません。そのため、ここではこれらの機能はどれも有効ではありません |
| **Gemini** | ✅ | ✅ | ✅ | `responseSchema`（OpenAPI 3.0 のサブセット）と後発の `responseJsonSchema` はデコーダーによって強制されます。また、OpenAI 互換エンドポイントでは `response_format: json_schema` を受け付けます。強制呼び出しには、`allowed_function_names` を指定した `ANY` を使用します |
| **xAI (Grok)** | ✅ | ✅ | ✅ | strict structured outputs を含む OpenAI 互換のサーフェスです |
| **Mistral** | ✅ | ✅ | ✅ | ユーザーが指定した JSON Schema を受け付けるカスタム structured outputs を提供します |
| **DeepSeek** | ❌ | ❌ | ✅ | JSON 出力は `json_object` のみです。ドキュメント上の要件は、プロンプトに "json" という単語を含め、例を示すことです。これは T3 パラメーターに付加された T4 の仕組みです。さらに `deepseek-reasoner` は強制された tool choice を拒否するため、そのスキーマでゲートされたパスは両端で閉じられています |
| **Qwen** (DashScope) | ⚠️ | ⚠️ | ✅ | 互換性エンドポイントでは `json_object` を利用できます。スキーマ制約付きデコーディングは、API ではなくサービングスタックを通じてセルフホスト Qwen で安定して動作します。DashScope 自体の `json_schema` サポートはモデル世代によって異なります。モデルごとに確認してください |
| **GLM** (Zhipu / Z.AI) | ❌ | ⚠️ | ✅ | OpenAI 互換エンドポイントで `json_object` を利用できます。強制された tool choice は完全にサポートされていないため、Table B では `tool_choice_enabled=false` を推奨設定としています。プラットフォームが使用するモデルについて明示的にドキュメント化していない限り、`json_schema` は利用できないものとして扱ってください |
| **Kimi** (Moonshot) | ⚠️ | ⚠️ | ✅ | `json_object` がドキュメント化されています。Thinking バリアントでは `auto` tool choice のみを受け付けるため、`strict` のサポートにかかわらず、これらのモデルでは T1 パスを強制できません |
| **MiniMax** | ⚠️ | ⚠️ | ✅ | thinking が常に有効な場合でも、強制された choice による Function calling が動作します。そのため、常時 thinking を行うモデルの中では、最も利用しやすい T1 の経路です。スキーマレベルの強制はドキュメント化されていないため、T1 は non-strict と想定してください |
| **Doubao** (Volcengine Ark) | ⚠️ | ⚠️ | ✅ | Ark は Doubao の製品ラインの一部について structured output をドキュメント化しています。対応状況はモデルごとに異なるため、アカウントで使用するモデルに機能があると想定する前に確認してください |
| **Ollama / vLLM / llama.cpp** | ✅ | ✅ | ✅ | この一覧の中で最も強力なティアサポートを提供しますが、最も宣伝されていないものでもあります。文法制約付きデコーディング（GBNF、XGrammar、Outlines）はローカルで実行されるため、上記のホスト型 API のいくつかでは提供できない保証を、セルフホストの 14B モデルに与えることができます。サーバーに応じて、Ollama の `format`、vLLM の guided decoding パラメーター、または OpenAI 互換の `response_format` として公開されます |

単に表を言い換えるのではなく、その構成を説明する重要なパターンが 3 つあります。

**T3 は普遍的ですが、T2 はそうではありません。** FIM One がサポートするすべてのプロバイダーが `json_object` を提供していますが、スキーマティアを提供するプロバイダーはおよそ半数です。したがって、移植可能な structured-output パスは T3 以下を基盤に構築する必要があります。これが FIM One のチェーンが現在の形になっている理由です。T2 を追加するには、グローバルスイッチではなく、モデルごとの機能フラグを追加する必要があります。
...
モデルごとの `json_mode_enabled` フラグは、レベル 2（json_mode）を試行するかどうかを制御します。

- **DB で構成されたモデル**: Admin → Models → Advanced settings で切り替えます。このフラグは `ModelProviderModel.json_mode_enabled` に保存されます（デフォルトは `TRUE`）。
- **ENV で構成されたモデル**: 環境変数に `LLM_JSON_MODE_ENABLED=false` を設定します。
- **効果**: 無効にすると、`abilities["json_mode"]` は `False` を返すため、`response_format` が渡されることはなく、prefill も行われません。その結果、Bedrock が動作します。フォールバックチェーンは `native_fc → plain_text` となり、失敗する json_mode 呼び出しを完全にスキップします。
- **スキップによるコスト**: システムプロンプトが JSON を要求し、最新のモデルでは `extract_json()` が自由形式のコンテンツを確実に解析できるため、実際にはモデルは引き続き有効な JSON を返します。失われるのは出力そのものではなく保証です。チェーンは T4 で終了し、結果を制約するのはプロンプトだけになります。`anthropic/` ルートでは、この損失は見た目ほど大きくありません。エミュレートされた JSON モードも、デコーダーレベルでの保証ではなかったためです。
...
| 症状 | メカニズム | FIM Oneの対応 |
|---|---|---|
| Thinkingが設定されているが表示されない | リレーパスに `/claude` ヒントがないため、モデルが `openai/` として解決される。Chat Completions スキーマには thinking の概念がないため、リクエストがプロセスを離れる前にパラメータが削除される | 構築時に警告をログに記録し、モデルと解決されたプレフィックスを名前付けし、`provider` を設定するか Anthropic ベース URL を使用するよう指示する（`OpenAICompatibleLLM.__init__`） |
| パラメータが受け入れられているように見えるが効果がない | `litellm.drop_params=True` は、解決されたプロバイダが宣言していないすべてのものをサイレントに、パラメータごとに削除する | 意図的。1つのリクエストビルダーがすべてのプロバイダで機能するようにする。代償として、「エラーなし」はパラメータが到達したことの証拠ではない |
| 非OpenAIモデルで最初のトークンに数分かかるか、エージェントがテキストを返すがツール呼び出しを行わない | リレーが OpenAI 以外のモデルに対して `/v1/responses` をアドバタイズし、リクエストを受け入れてから、答え全体をバッファリングしてリプレイする。Uniapi で Claude を使用した場合、最初のトークンまでにおよそ4分かかることが観察されており、2番目の再現では呼び出しは正常に返されたがエージェントはその後ツール呼び出しを0回行った。何もエラーにならないため、エラーがトリガーされたフォールバックは発火しない | ブリッジは GPT-5.x にメリットゲートされており、これが Responses から機能を得る唯一のファミリーである。その他はすべてチャット補完に直接進み、プローブを行わない（`_dispatch_acompletion`、コミット `137ede4c`） |
| `ValidationException: This model does not support assistant message prefill` | `anthropic/` ルーティングされた Bedrock リレー上の json_mode。LiteLLM は開き括弧をアシスタントメッセージとしてプリフィルすることで `response_format` をシミュレートし、より新しい Bedrock バージョンはアシスタントターンで終わる会話を拒否する | そのモデルに対して `json_mode_enabled=false` を設定するか、プリフィルが挿入されない OpenAI 互換ゲートウェイを通じてルーティングする |
| Zhipu エンドポイントへのすべての呼び出しで `404` | クライアントが OpenAI スタイルの `/v1` を既に `/v4` で終わるベース URL に追加する | プロバイダが文書化したとおりにベース URL を正確に設定する。FIM One は `api_base` を変更せずに渡す |
| 静かな期間後の `APIConnectionError: Connection error` | 中間者が FIN または RST を送信せずにアイドル状態のプール接続を回収し、httpx が次の書き込みで半分死んだソケットを返す | キープアライブ有効期限はデフォルトで5秒なので、ターン間のアイドル接続は再利用されるのではなく破棄される。接続の再利用を完全に無効にするには `LLM_HTTP_MAX_KEEPALIVE=0` を設定する（`_get_shared_http_client`） |
| `Cannot send a request, as the client has been closed` | LiteLLM がアイドル TTL でキャッシュされた SDK クライアントを削除し、OpenAI SDK がそのクライアントが保持していた共有 httpx セッションを閉じた | プールは試行前に再検証され、閉じられたときに再構築され、LiteLLM の古いクライアントキャッシュは一緒にフラッシュされる（`_get_shared_http_client`、`_flush_litellm_client_cache`） |
| 請求入力トークンが報告されたキャッシュ読み取りと一致しない | リレーが転送前に `cache_control` を削除するため、レスポンスがキャッシュカウンターを報告している間、全額を支払う | `TurnProfiler` はターンごとに `read_tokens` と `create_tokens` をログに記録し、これはリレー誠実性プローブとしても機能する。請求書と比較する |
| `Function tools with reasoning_effort are not supported ... Please use /v1/responses instead` | リレーが `reasoning_effort` フィールドの値ではなく存在に基づいてチャット補完をガードするため、FIM One が推論を無効にするために送信する明示的な `none` がガードをトリップする。Uniapi で `gpt-5.6-luna` を使用した場合に観察された | 何もしない。Responses パスが機能している間は何も必要ない。そのモデルは Responses リクエストが既に失敗した後にのみチャット補完に到達する。リレーが Responses を望んでいるという兆候として読み、`FIM_GPT5_RESPONSES_MODE=off` を設定する理由ではない |
| GPT-5.x が Responses をサポートするエンドポイント上でチャット補完に留まる | `404` がそのエンドポイントとモデルの負の判定としてキャッシュされた | 欠落しているルートは構造的であるため、`404` のみがキャッシュされる。`400` はその1つの呼び出しに対してフォールバックし、単一の古い推論項目がエンドポイントを永続的にブラックリストに登録できないように意図的にキャッシュされない（`_remember_native_failure`）。キャッシュはプロセスごとなので、再起動はどちらの方法でも再プローブする |
| Thinking ブロックが拒否されるか、プレフィックスキャッシュがヒットしない | リレーが履歴を書き直すか並べ替えるため、リプレイされた `signature` が一致しなくなる | リプレイは `reasoning_replay_policy` によって中央で決定され、Anthropic ファミリー ID のみがすべてリプレイする。リレーの背後にある Claude モデルが認識不可能な ID を持つ場合、そのフラグメントをポリシーテーブルに追加する |
