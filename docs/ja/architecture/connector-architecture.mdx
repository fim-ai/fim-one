...
**コネクタガバナンスレイヤー**は、素のMCPにはないエンタープライズ向けのガバナンス機能を追加します。

| 懸念事項 | MCP | コネクタガバナンスレイヤー |
|---|---|---|
| 読み取り専用の強制 | なし | 操作に対する `read_only` フラグ。デフォルトで書き込みをブロック |
| 監査ログ | なし | すべてのツール呼び出しを記録（タイムスタンプ、ユーザー、ツール、パラメーター、結果） |
| 認証パススルー | なし | ホストシステムの認証をプロキシ。エージェントはログイン中のユーザーに代わって操作 |
| 確認ゲート | なし | 書き込み操作には人間による承認が必要（SSE `confirmation_required`） |
| サーキットブレーカー | なし | 接続障害が発生すると、グレースフルデグラデーションを実行 |
| 操作の分類 | なし | 操作を読み取り／書き込み／管理者としてタグ付けし、レベルごとのポリシーを適用 |
| 列レベルのマスキング | なし | 個人データとしてマークされたデータベース列は、値の代わりにマスクを返す |

### データベースカラムのマスキング

データベースコネクタは通常、1つの特権アカウントで設定されるため、すべての呼び出し元が同じテーブルにアクセスできます。コネクタのスキーママネージャーでカラムを **PII** としてマークすると、エージェントがそのカラムから参照できる情報を制限できます。

- カラムはスキーマにタグ付きで引き続き表示されるため、モデルはそのカラムの存在を認識でき、代替のカラムを勝手に生成しません。
- そのカラム名で返される値は、`SELECT *` を含め、結果がモデルに届く前に `***` に置き換えられます。
- 呼び出しログには、マスクされたカラムと、コネクタが読み取り専用モードだったかどうかが記録されます。

照合はカラム名で行われます。これは結果セットに含まれる唯一の識別子です。ここから意図的に設けられた2つの制限が生じます。データベースは引き続きそのカラムを*使用*するため、`WHERE salary > 5000` によるフィルタリングは通常どおり機能します。また、カラム名を変更するクエリ（`SELECT salary AS s`）では、マスクされていない値が返されます。クエリを実行するアカウントがカラムをまったく読み取れないようにする必要がある場合は、データベース側でその権限を付与し、アクセスを制限したアカウントをコネクタに指定してください。

所有者自身のクエリプレイグラウンドではマスキングされません。マスキングの対象はエージェントと会話のトランスクリプトに届く情報であり、接続を設定した本人が検査できる情報ではありません。
