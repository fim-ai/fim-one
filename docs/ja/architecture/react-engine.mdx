...
このループが DAG スケジューリング、ワークフロー ブループリント、および業界の**グラフ エンジニアリング**とどのように並行して機能するかについては、[Planning Landscape](/architecture/planning-landscape) を参照してください。

## アーキテクチャ

ReActエンジンは、2フェーズの実行モデルを実装しています。第1フェーズは反復的なツール使用ループです。エージェントはLLMにアクションを繰り返し要求し、要求されたツールを実行してその結果を追加し、LLMが「完了」を示すまで処理を続けます。第2フェーズは回答の合成です。独立したストリーミングLLM呼び出しが実行トレース全体を読み取り、ユーザーに表示するレスポンスを生成します。

この分割は意図的なものです。ツールの反復処理は速度を重視して最適化されています。ループ内のすべてのLLM呼び出しでは、ユーザーが部分的なJSONアクションや中間推論トークンを見る必要がないため、非ストリーミングの`chat()`を使用します。回答生成はUXを重視して最適化されており、ストリーミングの`stream_chat()`を使用することで、ユーザーはトークンがリアルタイムで表示されるのを確認できます。その結果、高速なツール実行と応答性の高い回答配信という、両方の利点を実現できます。
...
ツールループは、完全な会話履歴を含む`AgentResult`を生成します。そこには、システムプロンプト、ユーザーのクエリ、すべてのアシスタントメッセージ、すべてのツール結果が含まれます。`stream_answer()`メソッドは、このトレースを簡潔で一貫性のある回答にまとめます。合成コンテキスト内では、各ツール結果を2,000文字に切り詰めます。これにより、複雑な複数ツールのワークフローの後でもプロンプトを簡潔に保てます。

**モデルのバインディング。** LLMは`ReActAgent.__init__()`に注入され、`self._llm`として保存されます。単一の`run()`呼び出し内で行われるすべての呼び出し（ツールループのすべての反復と最終回答の合成）では、同じインスタンスが使用されます。反復の途中でモデルが変更されることはありません。別のモデルを使用するには、新しい`ReActAgent`を構築する必要があります。DAGモードでは、`DAGExecutor._resolve_agent()`がこのパターンを活用します。各ステップの直前、ReActループが始まる前に、`step.model_hint`に基づいて`ModelRegistry`からモデルを選択し、ステップごとに新しいエージェントを作成します。詳しくは、[DAG Engine — ステップごとのオーバーライド](/architecture/dag-engine#multi-llm-architecture)を参照してください。
...
エージェントが最低限の`_COMPLETION_CHECK_MIN_TOOLS`個のツール（デフォルト：**3**、`REACT_COMPLETION_CHECK_MIN_TOOLS`で設定可能）を使用し、最終的な回答を生成することを決定した場合、回答が受け入れられる前に、1回限りの検証プロンプトが挿入されます：

1. 回答は元の質問に完全に対応していますか？
2. ツール結果から重要な事実が検証されていますか？
3. 収集された情報に矛盾がありますか？

チェックリストは内部交換です。これに回答するターンは検証であり、回答ではありません。それを逐語的に受け入れることは、「検証させてください：1. ... ✓」がチャットに漏れることになります。そのため、そのターンは内部で消費され、エージェントはユーザー向けの回答を求められ、これが`stream_answer()`がストリーミングするものです。エージェントが代わりに何か不足していると判断した場合、ツール呼び出しループを再開し、その後の回答はそのまま採用されます。

チェックリストは実行ごとに最大1回発火し、ツールが使用されていない単純な会話応答ではトリガーされず、残りのイテレーション予算が検証ターンと回答ターンの両方をカバーできない場合はスキップされます。

このしきい値は単純なタスクの不要なレイテンシを防ぎます。単一のツール呼び出し（例：簡単な計算の実行）は検証を保証しませんが、複数ツールの調査は最終的なサニティチェックから利益を得ます。常時検証を行うには`REACT_COMPLETION_CHECK_MIN_TOOLS=1`を設定してください。

この機能はデフォルトで有効になっており（`ReActAgent`の`completion_check=True`）、エージェントインスタンスごとに無効にできます。DAGステップはデフォルトを継承します。各ステップは独自の1回限りのチェックリストを取得します。
