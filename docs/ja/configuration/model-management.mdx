...
**無効にする場合:**
- モデルが `"tool_choice 'specified' is incompatible with thinking enabled"` のようなエラーを返す場合。これはモデルの思考能力の特性ではなく、プロバイダーごとのルールです：`deepseek-reasoner` と思考機能付き Kimi モデルは強制的なツール選択を拒否しますが、MiniMax はすべての呼び出しで思考を行い、これを受け入れます。[プロバイダー機能マトリックス](/architecture/llm-provider-guide#provider-capability-matrix)の表 B に各プロバイダーの判定があります
- 構造化出力リクエストが呼び出しごとに約 10 秒のペナルティで一貫して遅く、その後 JSON Mode へのフォールバックが続く場合

**無効にした場合の効果:** FIM One はレベル 1（ネイティブ関数呼び出し）をスキップし、構造化出力のためにレベル 2（JSON Mode）から開始します。ReAct エージェントのツール呼び出しは完全に影響を受けません——この設定に関わらずすべてのモデルで機能する `tool_choice="auto"` を使用します。

<Note>
この設定は、構造化出力抽出に使用される**強制的な**ツール選択（DAG プランニング、スキーマアノテーション）にのみ影響します。ReAct エージェントには影響しません。ReAct エージェントは `tool_choice="auto"` を使用して自由にツール呼び出しのタイミングを決定します。
</Note>
...
ほとんどのモデルはデフォルト設定（両方のトグルがON）で正しく動作します。エラーが発生した場合や不要なレイテンシーが生じた場合のみ調整してください。

以下の表は一般的なプロバイダーとモデルをカバーしています。データはUniAPI機能タグから取得し、**2026-03-22**時点のランタイム動作に対して検証されています。モデルの機能は頻繁に変わるため、エラーが発生した場合はプロバイダーの最新ドキュメントを確認してください。

これはモデルごとのオペレータビューです。モデルを追加する際にどのトグルを設定するかを示しています。これが従うプロバイダーレベルのルールと、それを実装するコードアンカーは[プロバイダー機能マトリックス](/architecture/llm-provider-guide#provider-capability-matrix)にあります。この2つが矛盾する場合、マトリックスが正式なものであり、この表は更新が必要です。
