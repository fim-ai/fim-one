...
| `LLM_MODEL` | いいえ | `gpt-4o` | メインモデル——計画、分析、ReAct agentに使用 |
...
| `LLM_REASONING_EFFORT` | いいえ | *(無効)* | サポートされているモデル用の拡張思考レベル（OpenAI o-series、Gemini 2.5+、Claude）。値：`low`、`medium`、`high`。LiteLLMは各プロバイダーのネイティブ形式に自動的に変換します。モデルの思考プロセスはUI「thinking」ステップで表示されます。 |
| `LLM_REASONING_BUDGET_TOKENS` | いいえ | *(effortから自動)* | Anthropic思考の明示的なトークン予算（最小1024）。OpenAI/Geminiの場合、effortレベルが直接使用されます。`LLM_REASONING_EFFORT`が設定されている場合のみ有効です。 |
| `FIM_GPT5_RESPONSES_MODE` | いいえ | `native` | GPT-5.xモデルが使用するプロトコル。`native`はOpenAI Responses APIと直接通信し、各ターンの暗号化された推論を再生するため、モデルはツール呼び出し全体で思考を保持します。`bridge`はLiteLLMのチャット完了翻訳を使用し、機能しますが推論を毎ラウンド再導出します。`off`は通常のチャット完了を強制し、ツールが存在する場合は推論が無効になります。デバッグ中でない限り設定しないでください。`/v1/responses`ルートがないエンドポイントは独自にフォールバックします。GPT-5.xのみに適用されます。 |
| `LLM_JSON_MODE_ENABLED` | いいえ | `true` | `response_format=json_object`のグローバルトグル。プロバイダーがLiteLLMのアシスタント先読み注入を拒否する場合は`false`に設定してください（例：AWS Bedrockリレー→2番目以降のagent反復で`ValidationException`）。無効にすると、構造化呼び出しはJSONモードをスキップし、プレーンテキスト正規表現抽出にフォールバックします——品質低下なし。すべてのモデル（ENV設定およびAdmin設定）に適用されます。 |
| `LLM_TOOL_CHOICE_ENABLED` | いいえ | `true` | 構造化出力抽出での強制`tool_choice`のグローバルトグル（レベル1——ネイティブ関数呼び出し）。モデルが強制ツール選択でエラーを返す場合は`false`に設定してください（例：`tool_choice='specified'`を拒否する思考モードモデル）。無効にすると、構造化呼び出しはネイティブFCをスキップし、JSONモードから開始します。モデルごとのオーバーライドは設定→モデル→詳細で利用可能です。 |
...
| `REASONING_LLM_EFFORT` | いいえ | *(`LLM_REASONING_EFFORT`にフォールバック)* | 推論層の推論effortレベル。値：`low`、`medium`、`high` |
| `REASONING_LLM_BUDGET` | いいえ | *(`LLM_REASONING_BUDGET_TOKENS`にフォールバック)* | 推論のトークン予算（主にAnthropicの場合）。推論層の自動計算予算をオーバーライドします |
| `LLM_SUPPORTS_VISION` | いいえ | `true` *(楽観的)* | ENV モードドキュメント OCR（MarkItDown + `markitdown-ocr`経由）の試行を制御します。**Admin→Models でアクティブなモデルグループが設定されていない場合のみ適用**（純ENV モード）。デフォルトの`true`が有効な場合、`convert_to_markdown`と RAG 取り込みは`LLM_MODEL`がビジョンをサポートしていると想定し、画像 OCR のために呼び出します——これはすべての一般的な選択肢（`gpt-4o`、`claude-3-5-sonnet`、`gemini-1.5-pro/flash`）の正しい動作です。ENV設定の`LLM_MODEL`がビジョンをサポート**していない**場合は`false`に設定してください（例：`deepseek-v3`、`qwen-chat`、`llama-3.1`、`gpt-3.5-turbo`、`o1-mini`）。失敗するビジョン呼び出しをスキップし、テキストのみの抽出に直接進みます。Admin→Models パネルにアクティブなモデルグループが存在する場合、このフラグは無視され、グループの`supports_vision`フラグが優先されます——admin キュレーションの選択は常に DB モードの真実のソースです。 |

> **解決順序**：ユーザー設定→Admin モデル（DB）→ENV フォールバック。Admin→Models で「General」ロールのadminモデルが設定されている場合、これらのENV変数はフォールバックのみとして機能します。ヘルスチェックは両方のソースを考慮します。

### ENV フォールバックを管理パネルと同期させる

Admin → Models でモデルグループを有効にすると、ランタイムはそのグループからモデルを取得し、上記の `LLM_*` / `FAST_LLM_*` / `REASONING_LLM_*` の値はフォールバックとしてのみ使用されます。データベースを開かない補助経路では、これらの値が直接読み取られます。対象となるのは、`scripts/translate.py`（国際化の pre-commit フック）、`evals/`、Jina/OpenAI のリランカー、画像生成です。これらが古いプロバイダーを参照し続けるのを防ぐには、アクティブなグループを env ファイルにコピーします。
...
スクリプトは ORM を介してプロバイダーキーを読み取り、そのままファイルに書き込みます。内容が出力されることはありません。完了後、アプリを再起動してください。
...
どのプロバイダーが思考を受け入れるか、それぞれがどのように有効にされるか、どの`effort`値を取るか、推論テキストがどこに終わるかは、[プロバイダー機能マトリックスの表C](/architecture/llm-provider-guide#provider-capability-matrix)にコードアンカー付きで一度記録されます。その表が信頼できるリストです。このページは変数のみを文書化しています。

FIM Oneは`LLM_BASE_URL`からプロバイダーを解決し（1つが設定されている場合は明示的なプロバイダーフィールドを追加）、リクエストを正しいAPI形式にマップします。未知のURLはOpenAI互換として扱われます。
...
推理がアクティブな場合、一部のプロバイダーは`temperature`を制限します。**これらはすべて自動的に適用されます。`LLM_TEMPERATURE`は、ワークロードに適した値のままにしてください。**

- **Anthropic**: 拡張思考が有効な場合、`temperature=1`が必須です。リクエストビルダーはAnthropicルートで値をピン留めするため、設定された温度は拒否されるのではなく、これらの呼び出しでオーバーライドされます。
- **Anthropic、厳密なモデル**（Opus 4.7および4.8、Fable 5、Mythos 5）: 思考の有無に関わらず、`temperature`、`top_p`、`top_k`を完全に拒否します。FIM Oneはこれらのモデルのリクエストから`temperature`を削除します。
- **OpenAI GPT-5.x**: `temperature=1`のみをサポートします。LiteLLMの`drop_params`フィルタリングはサポートされていない値を削除します。

Anthropicを満たすために手動で`LLM_TEMPERATURE=1`を設定することは不要であり、思考以外のすべての呼び出しでより低い温度を実行する能力を失うコストがかかります。

#### `LLM_REASONING_BUDGET_TOKENS` の動作

この変数は**レガシーAnthropicシンキングパス上でのみ意味があります**（Claude 4.5以前、`anthropic/`としてルーティング）。ここでは自動計算されたバジェットをオーバーライドし、`thinking`パラメータ内の`budget_tokens`として送信されます。アダプティブシンキングモデル（Opus 4.6以降、Sonnet 4.6、Fable 5、Mythos 5）はバジェットの代わりにエフォートレベルを使用し、この変数は完全に無視されます。設定されていない場合、バジェットは`LLM_MAX_OUTPUT_TOKENS` x エフォート比率から導出されます：
...
OpenAIおよびGeminiの場合、プロバイダーは`reasoning_effort`レベルに基づいて内部的にトークン割り当てを処理します——`LLM_REASONING_BUDGET_TOKENS`は効果がありません。
...
|---|---|---|---|
| `REACT_MAX_ITERATIONS` | No | `20` | ReAct リクエストあたりの最大ツール呼び出し反復回数。値が大きいほど、より徹底的ですが、遅く、コストがかかります |
| `REACT_MAX_TURN_TOKENS` | No | `0` | 緊急サーキットブレーカー：単一の ReAct ターンあたりの最大累積トークン数（すべての反復にわたるプロンプト＋補完）。デフォルト `0` = 無制限。**これは日次トークン制御用ではありません** — 日次制御にはユーザーごとの `token_quota` を使用してください。これは、ツール呼び出しの無限ループに陥ったエージェントなど、極端なシナリオのための最後の手段の安全弁です。この制限に達すると、タスクが実行途中で中止され、消費されたすべてのトークンが無駄になり、不完全な結果が返されます。特定の暴走エージェント問題を抑制する必要がない限り、`0` のままにしてください |
| `REACT_TOOL_SELECTION_THRESHOLD` | No | `12` | 登録されたツールの総数がこのしきい値を超える場合、軽量な LLM 呼び出しが各リクエストの前に最も関連性の高いサブセットを選択します |
| `REACT_TOOL_SELECTION_MAX` | No | `6` | スマート選択後に保持する最大ツール数（ツール数が `REACT_TOOL_SELECTION_THRESHOLD` を超える場合にのみ有効） |
| `REACT_SELF_REFLECTION_INTERVAL` | No | `6` | N 回のツール呼び出しごとに自己反省プロンプトを挿入して、エージェントが方向を修正し、ループを回避するのを支援します |
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | 最終回答を合成する際のツール観測あたりの最大文字数。値が大きいほど、より多くの構造化データ（JSON、テーブル）が保持されますが、トークンが増加します |
| `REACT_TOOL_RESULT_BUDGET` | No | `40000` | 単一セッション内のすべてのツール結果の集計トークン予算。ツール結果トークンの合計がこの上限を超える場合、新しい結果は通知付きで切り詰められます。大規模な API レスポンス（例：各 8K を返す 5 つのコネクタ呼び出し）からのコンテキスト肥大化を防ぎます。上限を無効にするには `0` に設定します |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | エージェントの最終回答がこの多くの文字を超える場合、事後回答完了チェック LLM 呼び出しをスキップします。長い詳細な回答は「何か見落としていないか？」という検証ラウンドトリップを必要としません。より積極的にスキップするには低く設定し、常にチェックを実行するには非常に大きな値に設定します |
| `REACT_CYCLE_DETECTION_THRESHOLD` | No | `2` | 同じツールが同じ引数で連続してこの回数呼び出される場合、決定論的な警告が挿入され、エージェントに別のアプローチを試すよう指示します。自己反省（LLM がループに気付くことに依存）とは異なり、これはバイパスできないハッシュベースのチェックです。DAG ステップにも適用されます |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | 完了チェックリストが発火する前の最小ツール呼び出し数。シンプルなタスク（1～2 ツール呼び出し）は不要なレイテンシを回避するために検証をスキップします。常時検証の場合は `1` に設定します。DAG ステップにも適用されます |
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | ターンごとのフェーズレベルのタイミングログ（`memory_load`、`compact`、`tool_schema_build`、`llm_first_token`、`llm_total`、`tool_exec`）を出力します。ターンごとに 1 行の構造化ログ。プロファイリング全体を無効にするには `false` に設定します（ゼロオーバーヘッド） |
| `REACT_PLAN_TOOL_ENABLED` | No | `true` | `update_plan` todo ツールを登録して、エージェントが複数ステップのタスク中に計画チェックリストを書き込み、維持できるようにします。DAG ステップエージェントとツールのないエージェントでは自動的にスキップされます |
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | `update_plan` 呼び出しなしのツールラウンド数。古い計画リマインダー（完全なチェックリストを埋め込む）が会話に再挿入されるため、計画はコンテキスト圧縮を生き残ります |
| `REACT_PLAN_REPEAT_THRESHOLD` | No | `4` | 同じツールを呼び出す連続ラウンド数（異なる引数で）。リマインダーはエージェントに、実りのない呼び出しを繰り返す代わりにアプローチを変更するよう指示します。完全に同じ呼び出しはサイクル検出で個別に処理されます |
| `REACT_PLAN_NUDGE_AFTER` | No | `5` | 計画が記録されていないツールラウンド数。計画を書き込むことを提案するワンショットのナッジ。計画ツールが有効な場合にのみ適用されます |
| `REACT_FINISH_SIGNAL` | No | `true` | チャットパスでの FINAL ファースト回答：エージェントは `finish` シグナルでツールループを終了し、その後、本当にトークンストリーミングされたターンとして回答を書き込みます。インラインループ回答とバッファされた再生を復元するには `false` に設定します |
| `REACT_MAX_CONTINUATIONS` | No | `3` | モデルの回答がプロバイダーの出力トークン制限（`finish_reason=length`）によって切り詰められる場合の最大継続ラウンド数。切り詰められたセグメントは、エージェントループと流式合成の両方で 1 つのシームレスな回答に結合されます |
| `REACT_BACKGROUND_TOOLS_ENABLED` | No | `true` | 遅いツール（サンドボックス化された python/shell/node 実行）に `run_in_background` オプションを提供します。エージェントはすぐにタスク ID を取得して作業を続行し、ツールが完了するとタスク ID が `<task_notification>` メッセージとして到着します |
| `REACT_BG_WAIT_TIMEOUT` | No | `300` | エージェントが回答を最終化したい場合に、実行中のバックグラウンドツールを待つ最大秒数。ウィンドウ内に完了しないタスクは、明示的なタイムアウト通知でキャンセルされます |
| `DAG_CHECKPOINT_EVIDENCE_CHARS` | No | `4000` | DAG クラッシュ再開チェックポイントファイル（`data/dag_checkpoints/`）内のステップごとのエビデンス上限。ステップサマリーは完全に保存されます |
| `DAG_CHECKPOINT_MAX_AGE_HOURS` | No | `24` | このより古い DAG チェックポイントは読み込み時に無視されるため、古いクラッシュの残骸は新しい実行に再開されることはありません |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | 単一のプロセスグローバルバケットの代わりに、ユーザーごとのキー付きレート制限バケットを使用します。1 つのノイズの多いユーザーが同じワーカー上の他のすべてのユーザーを枯渇させるのを防ぎます。基礎となるレートはバケットあたり 60 リクエスト/分と 100K トークン/分にハードコードされています — この設定はバケットが共有（グローバル）か分割（ユーザーごと）かのみを制御します。レガシーグローバルバケット（推奨されません）に戻すには `false` に設定します |

### DAG Planner

| Variable | Required | Default | Description |
|---|---|---|---|
| `MAX_CONCURRENCY` | No | `5` | DAG エグゼキューターの最大並列ステップ数 |
| `DAG_STEP_MAX_ITERATIONS` | No | `15` | 各 DAG ステップ内の最大ツール呼び出し反復回数 |
| `DAG_STEP_TIMEOUT` | No | `600` | ステップ実行のタイムアウト（秒単位）。このタイムアウトを超えたステップは失敗とマークされ、依存するステップはカスケードスキップされます |
| `DAG_MAX_REPLAN_ROUNDS` | No | `3` | 目標が達成されない場合の最大自動再計画試行回数。ユーザーの割り込み（インジェクト）は無制限で、このバジェットにはカウントされません |
| `DAG_REPLAN_STOP_CONFIDENCE` | No | `0.8` | エージェントが目標が**到達不可能**（不可能なリクエスト、機能不足、リソース利用不可）と判断した場合にのみ適用されます。この確実性以上で再試行を停止します。不完全な成果物は確実性に関わらず常に再計画されます——これらの再試行は `DAG_MAX_REPLAN_ROUNDS` によってのみ制限されます |
| `DAG_VERIFY_TRUNCATION` | No | `2000` | ステップ検証 LLM に送信されるステップ出力の最大文字数（品質判定用） |
| `DAG_ANALYZER_TRUNCATION` | No | `10000` | 実行後アナライザーのフォーマット時のステップ結果あたりの最大文字数 |
| `DAG_STEP_EVIDENCE_CHARS` | No | `16000` | ステップあたりで保持される生のツール出力（ウェブ取得、検索結果、ファイル読み取り）の最大文字数。これは権威ある「ソース証拠」として、ステップの要約と共にアナライザーと最終合成に供給されるため、回答の事実主張（合計、列挙、重大度）は要約が静かに項目を削除または誤ってラベル付けした可能性があるのではなく、ソースに対して検証できます。`0` に設定して証拠キャプチャを無効にします |
| `DAG_REPLAN_RECENT_TRUNCATION` | No | `500` | 再計画コンテキスト構築時の最新ラウンドからのステップ結果あたりの最大文字数 |
| `DAG_REPLAN_OLDER_TRUNCATION` | No | `200` | 再計画コンテキスト構築時の古いラウンドからのステップ結果あたりの最大文字数。古いラウンドはコンテキストを節約するためにより積極的に切り詰められます |
| `DAG_TOOL_CACHE` | No | `true` | 単一の DAG 実行内で同一のツール呼び出しをキャッシュします。`cacheable` として明示的にマークされたツール（検索、知識検索などの読み取り専用ツール）のみがキャッシュされます。キャッシュを完全に無効にするには `false` に設定します |
| `DAG_STEP_VERIFICATION` | No | `false` | 各 DAG ステップ後の汎用 LLM ベースの品質チェック。失敗時、ステップはフィードバック付きで 1 回再試行されます。**デフォルトはオフ**——すべてのステップに遅延を追加し、ほとんど必要ありません。ステップ出力の品質が低い場合が頻繁に観察される場合にのみ使用してください |
| `DAG_CITATION_VERIFICATION` | No | `true` | 専門分野ステップの引用精度チェック。**前提条件**：クエリは最初に LLM ドメイン分類器によって専門分野として分類される必要があります（`ESCALATION_DOMAINS` を参照）。ドメインが検出され、このフラグが `true` の場合、完了した各ステップは法律/医療/金融引用についてスキャンされ、精度が検証されます——幻覚記事番号、捏造された事件参照、および不正な規制引用を検出します。ドメイン分類が `null`（一般クエリ）を返す場合、この設定に関わらず引用検証は実行されません |
...
| Variable | Required | Default | Description |
|---|---|---|---|
| ~~`SYSTEM_PROMPT_RESERVE`~~ | — | — | **削除されました。** 以前はコンテキスト予算からシステムプロンプト用に固定の4K予約を差し引いていました。ContextGuardがメッセージリストトークンを推定する際にシステムプロンプトを既に含めているため、二重計算が発生していました。予算計算式は現在`(context_size - max_output_tokens) × 0.92`です（マージンはトークン推定エラーを吸収します）。システムプロンプトの実際のサイズは動的に考慮されます |

## Web Tools (オプション)

| Variable | Required | Default | Description |
|---|---|---|---|
| `JINA_API_KEY` | No | — | Jina API キー。**デフォルト**検索バックエンドを提供し、サービス固有のキーが設定されていない場合は**fetch、embedding、reranker**の共有フォールバックとして機能します。[jina.ai](https://jina.ai/) で取得してください |
| `TAVILY_API_KEY` | No | — | Tavily Search API キー。`WEB_SEARCH_PROVIDER=tavily` の場合に必須です。プロバイダーが未設定の場合は自動検出でも使用されます |
| `BRAVE_API_KEY` | No | — | Brave Search API キー。`WEB_SEARCH_PROVIDER=brave` の場合に必須です。プロバイダーが未設定の場合は自動検出でも使用されます |
| `EXA_API_KEY` | No | — | Exa Search API キー。`WEB_SEARCH_PROVIDER=exa` の場合に必須です。プロバイダーが未設定の場合は自動検出でも使用されます。[Exa](/integrations/exa) を参照してください。[exa.ai](https://exa.ai/) で取得してください |
| `WEB_SEARCH_PROVIDER` | No | `jina` | 検索プロバイダーセレクター：`jina`（デフォルト）/ `tavily` / `brave` / `exa`。デフォルト以外のプロバイダーを使用する場合は、明示的に設定することをお勧めします |
| `WEB_FETCH_PROVIDER` | No | `jina`（キーが設定されている場合）/ `httpx`（それ以外） | Fetch プロバイダー：`jina`（Jina Reader API を使用）/ `httpx`（直接 HTTP リクエスト、API キー不要） |

> **クイックスタートのヒント**：`JINA_API_KEY` を設定するだけで、デフォルト Web 検索スタック、Web fetch、embedding、reranking が有効になります——1 つのキーで 4 つのサービスが利用できます。`WEB_SEARCH_PROVIDER` と対応する API キーで検索を Tavily、Brave、または Exa に切り替えることができます。
...
| Variable | Required | Default | Description |
|---|---|---|---|
| `DATABASE_URL` | No | `sqlite+aiosqlite:///./data/fim_one.db` | データベース接続文字列。**SQLite**（ゼロ設定）: `sqlite+aiosqlite:///./data/fim_one.db`。**PostgreSQL**（本番環境）: `postgresql+asyncpg://user:pass@localhost:5432/fim_one`。Docker Composeは自動的にPostgreSQLを設定します。 |
| `JWT_SECRET_KEY` | No | `CHANGE_ME` | JWTトークン署名用の秘密鍵。プレースホルダー値 `CHANGE_ME`（またはその他のレガシーデフォルト）は、初回起動時に安全な256ビットのランダムキーを自動生成し、`.env`に書き込みます。本番環境では明示的に設定して、再起動とレプリカ間でトークンの有効性を保つようにしてください。 |
| `FIM_BCRYPT_COST` | No | `12` | パスワードハッシング用のbcryptワークファクター（4～31に制限）。デフォルト12は最新のCPUで約200msのコストがかかります。弱いハードウェアでは低くし、セキュリティ強化デプロイメントでは高くしてください。 |
| `CORS_ORIGINS` | No | — | デフォルトのlocalhostエントリを超えて許可する追加のCORSオリジンのカンマ区切りリスト。フロントエンドが非localhostドメイン（例：`https://app.example.com`）で実行される場合に必須です。 |
| `UPLOADS_DIR` | No | `./uploads` | アップロードされたファイルのディレクトリ |
| `EXPORT_FONT_DIR` | No | *（自動検出）* | PDF エクスポート用の `NotoSansSC-Regular.ttf` / `NotoSansSC-Bold.ttf` を保持するディレクトリ。`python scripts/fetch_export_fonts.py` で取得します。Dockerイメージには既にバンドルされています。埋め込み可能なTrueType CJKフォントがない場合、PDFエクスポートは埋め込まれていないCIDフォントにフォールバックし、スペーシングが低下し、太字がありません。 |
| `MAX_UPLOAD_SIZE_MB` | No | `50` | 最大ファイルアップロードサイズ（メガバイト、バックエンド強制） |
| `NEXT_PUBLIC_MAX_UPLOAD_SIZE_MB` | No | `50` | フロントエンドUIに表示される最大ファイルアップロードサイズ。**ビルド時変数** — `MAX_UPLOAD_SIZE_MB` と一致する必要があります。 |
| `MCP_SERVERS` | No | — | MCPサーバー設定のJSONアレイ（`uv sync --extra mcp` が必要） |
| `ALLOW_STDIO_MCP` | No | `false` | stdio MCPサーバーを許可します。信頼できるローカルデプロイメントの場合のみ `true` に設定してください |
| `ALLOWED_STDIO_COMMANDS` | No | `npx,uvx,node,python,python3,deno,bun` | stdio MCPサーバーで許可する基本コマンドのカンマ区切りリスト。`ALLOW_STDIO_MCP=true` の場合のみ有効です |
| `LOG_LEVEL` | No | `INFO` | ログレベル: `DEBUG` / `INFO` / `WARNING` / `ERROR` / `CRITICAL` |
| `REDIS_URL` | No | — | クロスワーカー割り込みリレー用のRedis接続URL。**`WORKERS>1` の場合に必須** — これがないと、ストリーム中の割り込み/注入リクエストが別のワーカーにヒットして、サイレントに失敗する可能性があります。Docker Composeで自動設定されます。 |
| `WORKERS` | No | `1` | Uvicornワーカープロセス。`1` は安全で、外部サービスは不要です。本番環境のマルチワーカーの場合、PostgreSQL を使用してください（SQLiteはシングルライター）。SQLiteはローカル開発で軽い負荷の場合に機能します。認証、OAuth、ファイル操作は完全にマルチワーカー対応です（JWTベース）。Docker Composeは自動的にPostgreSQLとRedisの両方を設定します。 |

<Warning>
**マルチワーカーチェックリスト**（`WORKERS>1`）:
- **停止（ストリーム中止）** — 常に機能し、追加設定は不要です（シグナルは同じTCP接続で移動）。
- **注入（ストリーム中のフォローアップ）** — **`REDIS_URL` が必須**。Redisがない場合、注入リクエストが実行中の知識を持たない別のワーカーにランディングし、サイレントに失敗する可能性があります。
- **本番環境**: PostgreSQL（`DATABASE_URL`）を使用してください。SQLiteのシングルライターロックは、同時書き込み時に競合を引き起こす可能性があります。
- **ローカル開発**: SQLite + マルチワーカーは軽い使用には問題ありません。注入機能を使用する場合は、`REDIS_URL` を追加してください。
</Warning>
