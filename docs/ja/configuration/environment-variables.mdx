...
| 変数 | 必須 | デフォルト | 説明 |
|---|---|---|---|
| `REACT_MAX_ITERATIONS` | いいえ | `20` | ReAct リクエストあたりの最大ツール呼び出し反復回数。高いほどより徹底的ですが、遅く、コストが高くなります |
| `REACT_MAX_TURN_TOKENS` | いいえ | `0` | 緊急遮断機：単一の ReAct ターンあたりの最大累積トークン数（すべての反復にわたるプロンプト＋完了）。デフォルト `0` = 無制限。**これは日次トークン制御用ではありません** — その場合は `token_quota` を使用してください。これは、エージェントが無限ツール呼び出しループに陥るなどの極端なシナリオに対する最後の手段の安全弁です。この制限に達すると、タスクは実行途中で中止され、消費されたすべてのトークンが無駄になり、不完全な結果が返されます。暴走エージェント問題を特に抑える必要がない限り、`0` のままにしてください |
...
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | いいえ | `800` | エージェントの最終回答がこの文字数を超える場合、事後完了チェック LLM 呼び出しをスキップします。長い詳細な回答は「何か見落としましたか？」検証ラウンドトリップを必要としません。より積極的にスキップするには低く設定し、常にチェックを実行するには非常に大きな値に設定します |
| `REACT_CYCLE_DETECTION_THRESHOLD` | いいえ | `2` | 同じツールが同じ引数で連続してこの回数呼び出される場合、決定論的な警告が挿入され、エージェントに別のアプローチを試すよう指示します。自己反省（LLM がループに気付くことに依存）とは異なり、これはバイパスできないハッシュベースのチェックです。DAG ステップにも適用されます |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | いいえ | `3` | 完了チェックリストが発火する前の最小ツール呼び出し数。シンプルなタスク（1～2 ツール呼び出し）は不要なレイテンシを回避するため検証をスキップします。常時検証の場合は `1` に設定します。DAG ステップにも適用されます |
| `REACT_TURN_PROFILE_ENABLED` | いいえ | `true` | ターンごとのフェーズレベルのタイミングログ（`memory_load`、`compact`、`tool_schema_build`、`llm_first_token`、`llm_total`、`tool_exec`）を出力します。ターンあたり 1 行の構造化ログ。プロファイリング全体を無効にするには `false` に設定します（オーバーヘッドなし） |
| `REACT_PLAN_TOOL_ENABLED` | いいえ | `true` | `update_plan` todo ツールを登録して、エージェントが複数ステップのタスク中にプランチェックリストを書き込み、保持できるようにします。DAG ステップエージェントとツールのないエージェントでは自動的にスキップされます |
| `REACT_PLAN_REMINDER_INTERVAL` | いいえ | `3` | `update_plan` 呼び出しなしのツールラウンド数。古いプランリマインダー（完全なチェックリストを埋め込む）が会話に再挿入されるため、プランはコンテキスト圧縮を生き残ります |
| `REACT_PLAN_REPEAT_THRESHOLD` | いいえ | `4` | 同じツール（異なる引数で）を連続して呼び出すラウンド数。リマインダーがエージェントに繰り返しの代わりにアプローチを変更するよう指示します。完全に同じ呼び出しはサイクル検出で別途処理されます |
| `REACT_PLAN_NUDGE_AFTER` | いいえ | `5` | プランが記録されていないツールラウンド数。プランを書き込むことを提案する 1 回限りのナッジが表示されます。プランツールが有効な場合のみ適用されます |
| `REACT_MAX_CONTINUATIONS` | いいえ | `3` | モデルの回答がプロバイダーの出力トークン制限（`finish_reason=length`）で切り詰められた場合の最大継続ラウンド数。切り詰められたセグメントはシームレスな回答にステッチされ、エージェントループと流式合成の両方で行われます |
...
| `REACT_BG_WAIT_TIMEOUT` | いいえ | `300` | エージェントが回答を最終化したい場合に、実行中のバックグラウンドツールを待つ最大秒数。ウィンドウ内に完了しないタスクは明示的なタイムアウト通知でキャンセルされます |
| `DAG_CHECKPOINT_EVIDENCE_CHARS` | いいえ | `4000` | DAG クラッシュ再開チェックポイントファイル（`data/dag_checkpoints/`）内のステップあたりの証拠上限。ステップサマリーは完全に保存されます |
| `DAG_CHECKPOINT_MAX_AGE_HOURS` | いいえ | `24` | このより古い DAG チェックポイントは読み込み時に無視されるため、古いクラッシュ残骸は新しい実行に再開されることはありません |
| `LLM_RATE_LIMIT_PER_USER` | いいえ | `true` | 単一のプロセスグローバルバケットの代わりに、ユーザーごとのキー付きレート制限バケットを使用します。同じワーカー上の 1 つのノイズの多いユーザーが他のすべてのユーザーを飢えさせるのを防ぎます。基礎となるレートはバケットあたり 60 リクエスト/分および 100K トークン/分でハードコードされています — この設定はバケットが共有（グローバル）か分割（ユーザーごと）かのみを制御します。レガシーグローバルバケットに戻すには `false` に設定します（推奨されません） |
