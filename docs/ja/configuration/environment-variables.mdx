...
|---|---|---|---|
| `REACT_MAX_ITERATIONS` | No | `20` | ReAct リクエストあたりの最大ツール呼び出し反復回数。値が高いほどより徹底的ですが、遅くなり、コストが増加します |
| `REACT_MAX_TURN_TOKENS` | No | `0` | 緊急遮断弁：単一の ReAct ターンあたりの最大累積トークン数（すべての反復にわたるプロンプト＋完了）。デフォルト `0` = 無制限。**これは日次トークン制御用ではありません** — 日次制御にはユーザーごとの `token_quota` を使用してください。これは、エージェントが無限ツール呼び出しループに陥るなどの極端なシナリオに対する最後の手段の安全弁です。この制限に達すると、タスクが実行途中で中止され、消費されたすべてのトークンが無駄になり、不完全な結果が返されます。特定の暴走エージェント問題を抑制する必要がない限り、`0` のままにしてください |
| `REACT_TOOL_SELECTION_THRESHOLD` | No | `12` | 登録済みツールの総数がこのしきい値を超える場合、軽量な LLM 呼び出しが各リクエストの前に最も関連性の高いサブセットを選択します |
| `REACT_TOOL_SELECTION_MAX` | No | `6` | スマート選択後に保持する最大ツール数（ツール数が `REACT_TOOL_SELECTION_THRESHOLD` を超える場合のみ有効） |
| `REACT_SELF_REFLECTION_INTERVAL` | No | `6` | N 回のツール呼び出しごとに自己反省プロンプトを挿入して、エージェントが軌道修正し、ループを回避するのに役立ちます |
...
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | エージェントの最終回答がこの文字数を超える場合、事後完了チェック LLM 呼び出しをスキップします。長い詳細な回答は「何か見落としていないか？」という検証ラウンドトリップを必要としません。より積極的にスキップするには低く設定し、常にチェックを実行するには非常に大きな値に設定します |
...
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | 完了チェックリストが発火する前の最小ツール呼び出し数。シンプルなタスク（1～2 回のツール呼び出し）は不要なレイテンシを回避するため検証をスキップします。常時検証の場合は `1` に設定します。DAG ステップにも適用されます |
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | ターンごとのフェーズレベルのタイミングログ（`memory_load`、`compact`、`tool_schema_build`、`llm_first_token`、`llm_total`、`tool_exec`）を出力します。ターンあたり 1 行の構造化ログ。プロファイリング全体を無効にするには `false` に設定します（オーバーヘッドなし） |
| `REACT_PLAN_TOOL_ENABLED` | No | `true` | `update_plan` todo ツールを登録して、エージェントが複数ステップのタスク中にプラン チェックリストを書き込んで維持できるようにします。DAG ステップ エージェントおよびツールのないエージェントの場合は自動的にスキップされます |
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | `update_plan` 呼び出しなしのツール ラウンド数。この数に達すると、古いプラン リマインダー（完全なチェックリストを埋め込む）が会話に再挿入され、コンテキスト圧縮後もプランが保持されます |
| `REACT_MAX_CONTINUATIONS` | No | `3` | モデルの回答がプロバイダーの出力トークン制限（`finish_reason=length`）によって切り詰められた場合の最大継続ラウンド数。切り詰められたセグメントは、エージェント ループとストリーミング合成の両方で 1 つのシームレスな回答に結合されます |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | 単一のプロセスグローバル バケットの代わりに、ユーザーごとのキー付きレート制限バケットを使用します。1 つのノイズの多いユーザーが同じワーカー上の他のすべてのユーザーを飢えさせるのを防ぎます。基盤となるレートはバケットあたり 60 リクエスト/分および 100K トークン/分にハードコードされています — この設定は、バケットが共有（グローバル）か分割（ユーザーごと）かのみを制御します。レガシー グローバル バケットに戻すには `false` に設定します（推奨されません） |
