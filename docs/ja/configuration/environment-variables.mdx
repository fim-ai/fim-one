...
|---|---|---|---|
| `REACT_MAX_ITERATIONS` | No | `20` | ReAct リクエストあたりの最大ツール呼び出し反復回数。値が大きいほど、より徹底的ですが、速度が低下し、コストが増加します |
| `REACT_MAX_TURN_TOKENS` | No | `0` | 緊急サーキットブレーカー：単一の ReAct ターンあたりの最大累積トークン数（すべての反復にわたるプロンプト＋完了）。デフォルト `0` = 無制限。**これは日次トークン制御用ではありません** — 日次制御には `token_quota` を使用してください。これは、エージェントが無限ツール呼び出しループに陥った場合などの極端なシナリオに対する最後の手段の安全弁です。この制限に達すると、タスクが実行途中で中止され、消費されたすべてのトークンが無駄になり、不完全な結果が返されます。特定の暴走エージェント問題を抑制する必要がない限り、`0` のままにしてください |
...
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | 最終回答を合成する際のツール観測あたりの最大文字数。値が大きいほど、より多くの構造化データ（JSON、テーブル）が保持されますが、トークン数が増加します |
...
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | エージェントの最終回答がこの文字数を超える場合、事後完了チェック LLM 呼び出しをスキップします。長い詳細な回答は「何か見落としましたか？」という検証ラウンドトリップを必要としません。より積極的にスキップするには低く設定し、常にチェックを実行するには非常に大きな値に設定します |
| `REACT_CYCLE_DETECTION_THRESHOLD` | No | `2` | 同じツールが同じ引数で連続してこの回数呼び出される場合、エージェントに別のアプローチを試すよう指示する決定論的な警告が挿入されます。自己反省（LLM がループに気付くことに依存）とは異なり、これはバイパスできないハッシュベースのチェックです。DAG ステップにも適用されます |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | 完了チェックリストが実行される前の最小ツール呼び出し数。シンプルなタスク（1～2 ツール呼び出し）は不要なレイテンシを回避するため検証をスキップします。常時検証を行うには `1` に設定します。DAG ステップにも適用されます |
...
| `REACT_PLAN_TOOL_ENABLED` | No | `true` | `update_plan` todo ツールを登録して、エージェントが複数ステップのタスク中にプランチェックリストを記述および保持できるようにします。DAG ステップエージェントおよびツールのないエージェントの場合は自動的にスキップされます |
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | `update_plan` 呼び出しなしのツールラウンド数。古いプランリマインダー（完全なチェックリストを埋め込む）が会話に再挿入されるため、プランはコンテキスト圧縮を通じて保持されます |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | 単一のプロセスグローバルバケットの代わりに、ユーザーごとのキー付きレート制限バケットを使用します。同じワーカー上の 1 つのノイズの多いユーザーが他のすべてのユーザーを枯渇させるのを防ぎます。基礎となるレートはバケットあたり 60 リクエスト/分および 100K トークン/分でハードコードされています — この設定はバケットが共有（グローバル）か分割（ユーザーごと）かのみを制御します。レガシーグローバルバケットに戻すには `false` に設定します（推奨されません） |
