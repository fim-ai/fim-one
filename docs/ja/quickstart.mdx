...
ゼロオープンポートセットアップの場合、Nginxの代わりに[Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/)を使用してください。すべてのトラフィックはCloudflareのエッジを通過します。ポート80/443を公開したり、SSLサーティフィケートを管理したり、ファイアウォールルールを設定する必要はありません。

<Warning>
**中国本土ユーザー**: Cloudflare Free/Pro/Businessプランは**中国本土にPoP（Points-of-Presence）がありません**。中国本土からのトラフィックは海外のエッジ（通常は米国西部）にルーティングされ、頻繁な502エラーと高レイテンシが発生します。主なユーザーが中国本土にいる場合は、Cloudflare Tunnelを**使用しないでください**。中国本土への確実なアクセスにはCloudflare Enterprise with China Network（JD Cloudパートナーシップ）が必要です。
</Warning>
...
<Steps>
  ...
  <Step title="パブリックホスト名を設定する">
    ...
    その他のすべての設定（HTTP Host Header、Chunked Encoding、Timeouts、Access）はデフォルトのままにしてください。

    <Note>URLは`localhost`ではなくDockerサービス名`fim-one`を使用します。これは`cloudflared`が同じDockerネットワーク内の別のコンテナとして実行されるためです。</Note>
  </Step>
  <Step title="トンネルトークンをコピーする">
    トンネルの**Configure**ページで、インストールコマンドを見つけます。これには`eyJ...`で始まるトークンが含まれています。それをコピーしてください。
  </Step>
  <Step title="トークンを.envに追加する">
    ```bash
    # サーバーの.envファイルに追加
  ...
  <Step title="トンネルオーバーレイでデプロイする">
    ...
    `docker-compose.tunnel.yml`オーバーレイは`cloudflared`サイドカーコンテナを追加します。ベースの`docker-compose.yml`は変更されません。Cloudflareを使用していないコミュニティユーザーは引き続き`docker compose up -d`を使用できます。
  </Step>
  <Step title="古いDNSレコードを削除する">
    ドメインが以前にサーバーのIPを指すAレコードを持っていた場合、Cloudflare DNSで**削除してください**。トンネルは自動的にそのエッジエンドポイントを指すCNAMEレコードを作成します。
  </Step>
  <Step title="サーバーポートを閉じる">
    サーバーの`docker-compose.yml`から`ports`セクションを削除（またはコメントアウト）してください。トラフィックはトンネルを通じてのみ流れるようになります。インバウンドポートは不要です。
  </Step>
</Steps>

<Tip>
  Cloudflare Tunnelはすべてのプラン（Freeを含む）で無料です。帯域幅またはトラフィック制限はありません。
</Tip>
