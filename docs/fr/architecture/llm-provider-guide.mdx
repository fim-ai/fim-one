...
Extraction structurée en une seule étape (annotation de schéma, planification de DAG, analyse de plan). Force le modèle à appeler une fonction virtuelle spécifique, ce qui garantit une sortie JSON structurée. C’est le point d’appel qui déclenche les erreurs spécifiques aux fournisseurs.

`structured_llm_call` implémente une chaîne de dégradation à 3 niveaux. Les trois paliers sont nommés d’après le code, et leur numérotation ne correspond pas à celle des quatre niveaux de garantie décrits dans [la section ci-dessous](#structured-output-the-four-guarantee-tiers) :
...
| Niveau | Contenu de la requête | Emplacement de la charge utile | Garantie | Appliquée par |
|---|---|---|---|---|
| **T1 — appel de fonction natif, `strict: true`** | `tools=[…]` avec `strict` défini dans la définition de la fonction | Une chaîne JSON dans `tool_calls[0].function.arguments` | Au niveau du schéma : tous les champs requis sont présents, aucune clé non déclarée, types et énumérations corrects | Machine à états du décodeur |
| **T2 — `response_format: {"type": "json_schema"}`** | Le schéma lui-même, dans `response_format`, avec `strict: true` | Une chaîne JSON dans `message.content` | Au niveau du schéma, comme pour T1, mais dans la propre réponse de l’assistant plutôt que dans un appel d’outil | Machine à états du décodeur |
| **T3 — `response_format: {"type": "json_object"}`** | Le type uniquement, sans schéma | Une chaîne JSON dans `message.content` | Syntaxique uniquement : le texte est analysable comme du JSON. Les clés présentes ne sont soumises à aucune contrainte | Décodeur (syntaxe) |
| **T4 — texte libre accompagné d’instructions** | Rien. Le schéma est décrit dans le prompt | `message.content`, éventuellement entouré de texte ou de blocs de code | Aucune | Le prompt |
...
Ils offrent la même garantie par des canaux différents, et le canal compte à trois égards.

**La structure du tour diffère.** T1 renvoie un tour d’appel d’outil ; le modèle a décidé d’*agir*. T2 renvoie un message d’assistant ordinaire ; le modèle a *répondu* dans un format donné. Lorsque l’appelant veut une seule extraction et non une boucle d’agent, T2 l’exprime directement au lieu d’inventer une fonction virtuelle que le modèle devrait « appeler ».

**Forcer T1 entre en conflit avec la réflexion.** Obtenir de T1 une charge utile contrainte par un schéma implique généralement de forcer le choix, soit avec `required`, soit avec une fonction nommée. Plusieurs fournisseurs refusent un choix d’outil forcé lorsque la réflexion étendue est active. Le tableau B ci-dessous indique lesquels. T2 ne présente pas ce conflit : il s’agit d’une contrainte de réponse, et non d’une contrainte d’outil ; c’est donc le seul chemin contraint par un schéma qui reste compatible avec la réflexion activée. Pour un déploiement standardisé sur des modèles de raisonnement, c’est l’argument pratique en faveur de T2, plutôt qu’une préférence stylistique.

**T1 entraîne une étape de configuration du schéma que T2 évite.** Une fonction virtuelle nécessite un nom, une description et une décision quant à la possibilité pour le modèle de refuser de l’appeler.
...
| Niveau de `structured_llm_call` | Niveau | Ce que fait réellement le code |
|---|---|---|
| Niveau 1 — `native_fc` | **T1 sans `strict`** | `_build_tool_def` émet `name` / `description` / `parameters`, et rien d’autre. Aucun site d’appel ne définit `strict`, donc `convert_tools` dans `responses_adapter.py` lui attribue par défaut la valeur `False`. Le schéma parvient au modèle comme une description de ce qu’il doit produire, et non comme une contrainte sur ce qu’il peut produire |
| *(absent)* | **T2** | Non implémenté. `json_schema` n’apparaît nulle part dans la base de code, et `abilities` ne possède aucun indicateur permettant d’exprimer la prise en charge de cette fonctionnalité modèle par modèle |
| Niveau 2 — `json_mode` | **T3** | `{"type": "json_object"}`, défini en dur dans `structured.py::_call_llm` et `react.py::_json_response_format` |
| Niveau 3 — `plain_text` | **T4** | `extract_json()` appliqué à un contenu libre, avec un remplacement facultatif par expression régulière propre à chaque site d’appel |
...
| Fournisseur | T1 `strict` | T2 `json_schema` | T3 `json_object` | Notes |
|---|---|---|---|---|
| **OpenAI** | ✅ | ✅ | ✅ | L'implémentation de référence des deux niveaux stricts. L'appel de fonctions strict et `response_format: json_schema` partagent un même moteur de grammaire et un même sous-ensemble de schémas. `json_object` est l'ancien mode JSON, conservé pour la compatibilité |
| **Anthropic** | ⚠️ | ⚠️ | ❌ | L'API Messages ne propose aucun `response_format` ; le `input_schema` des outils est validé plutôt qu'imposé par le décodeur. L'utilisation stricte des outils et un format de sortie contraint par schéma existent derrière un en-tête bêta sur les modèles Claude récents. FIM One accède à Anthropic via LiteLLM, qui émule le mode JSON par préremplissage et n'utilise pas la version bêta ; rien de tout cela n'est donc actif ici |
| **Gemini** | ✅ | ✅ | ✅ | `responseSchema` (un sous-ensemble d'OpenAPI 3.0) et le plus récent `responseJsonSchema` sont imposés par le décodeur, et le point de terminaison compatible OpenAI accepte `response_format: json_schema`. L'appel forcé utilise `ANY` avec `allowed_function_names` |
| **xAI (Grok)** | ✅ | ✅ | ✅ | Surface compatible OpenAI, y compris les sorties structurées strictes |
| **Mistral** | ✅ | ✅ | ✅ | Les sorties structurées personnalisées acceptent un JSON Schema fourni par l'utilisateur |
| **DeepSeek** | ❌ | ❌ | ✅ | La sortie JSON est limitée à `json_object`. La condition documentée est que le prompt contienne le mot « json » et présente un exemple, ce qui constitue un dispositif T4 associé à un paramètre T3. `deepseek-reasoner` refuse en outre un choix d'outil forcé ; son chemin contrôlé par schéma est donc fermé aux deux extrémités |
| **Qwen** (DashScope) | ⚠️ | ⚠️ | ✅ | `json_object` est disponible sur le point de terminaison de compatibilité. Le décodage contraint par schéma est fiable avec Qwen auto-hébergé via la pile de service plutôt que via l'API ; la prise en charge de `json_schema` par DashScope varie selon la génération du modèle. Vérifiez pour chaque modèle |
| **GLM** (Zhipu / Z.AI) | ❌ | ⚠️ | ✅ | `json_object` est disponible sur le point de terminaison compatible OpenAI. Le choix d'outil forcé n'est pas pris en charge, ce qui explique pourquoi `tool_choice_enabled=false` est le paramètre recommandé dans le tableau B. Considérez `json_schema` comme indisponible, sauf si la plateforme le documente pour votre modèle |
| **Kimi** (Moonshot) | ⚠️ | ⚠️ | ✅ | `json_object` est documenté. Les variantes de raisonnement n'acceptent que le choix d'outil `auto` ; pour celles-ci, le chemin T1 ne peut donc pas être forcé, quelle que soit la prise en charge de `strict` |
| **MiniMax** | ⚠️ | ⚠️ | ✅ | L'appel de fonctions fonctionne avec un choix forcé, même lorsque le raisonnement est toujours activé, ce qui en fait le chemin T1 le plus exploitable parmi les modèles à raisonnement permanent. L'imposition au niveau du schéma n'est pas documentée ; partez du principe que T1 n'est pas strict |
| **Doubao** (Volcengine Ark) | ⚠️ | ⚠️ | ✅ | Ark documente les sorties structurées pour certaines parties de la gamme Doubao. La couverture dépend du modèle ; vérifiez donc avant de supposer que le modèle de votre compte en dispose |
| **Ollama / vLLM / llama.cpp** | ✅ | ✅ | ✅ | La prise en charge des niveaux la plus complète de cette liste, et la moins mise en avant. Le décodage contraint par grammaire (GBNF, XGrammar, Outlines) s'exécute localement ; un modèle auto-hébergé de 14B peut donc recevoir une garantie que plusieurs API hébergées ci-dessus ne peuvent pas fournir. La fonctionnalité est exposée via le `format` d'Ollama, les paramètres de décodage guidé de vLLM ou un `response_format` compatible OpenAI, selon le serveur |

Trois tendances méritent d'être nommées, car elles expliquent la forme du tableau plutôt que de se contenter de le répéter.
...
**Le raisonnement toujours activé tend à fermer la porte de T1.** GLM, les modèles de raisonnement de Kimi et `deepseek-reasoner` limitent ou refusent tous un choix d'outil forcé, et Anthropic le refuse lorsque le raisonnement est actif. MiniMax est l'exception. Lorsque la porte est fermée, T2 est la seule option restante contrôlée par schéma, ce qui explique pourquoi le niveau manquant est plus important sur un déploiement de modèles chinois que sur un déploiement OpenAI.

**L'auto-hébergement inverse l'ordre habituel.** Le décodage contraint est une propriété de la pile de service ; il est donc disponible sur un checkpoint local dont le respect des instructions est bien plus faible que celui d'un modèle de pointe. Les modèles qui ont le plus besoin d'un contrôle par schéma sont ceux qui sont le plus susceptibles de l'obtenir.
