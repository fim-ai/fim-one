...
Les organisations et le Market global partagent le même modèle d'accès basé sur l'abonnement. La différence clé est la confiance : les organisations représentent une équipe ou une entreprise où les membres se connaissent et se font confiance mutuellement, donc l'examen est optionnel et le partage d'identifiants est direct.
...
|---|---|---|---|
| **Channels** | Pont sortant vers une plateforme IM (Feishu aujourd'hui ; Slack / WeCom / Teams prévus) | Org Owner / Admin | Le [Hook System](/architecture/hook-system) de chaque agent et les outils de notification proactive |
| **OAuth Providers** | Identifiants de connexion sociale (GitHub, Google, Feishu, Discord) | System admin (env vars) | Page de connexion |
| **API Keys** | Jetons d'accès programmatique pour les clients sans interface | Chaque utilisateur, au niveau de l'org | Intégrations externes |
...
Voir [Channels overview](/configuration/channels/overview) pour le cycle de vie des canaux et les plateformes supportées.
