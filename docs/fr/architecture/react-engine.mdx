...
Pour comprendre comment cette boucle s'articule avec la planification DAG, les blueprints de flux de travail et l'**ingénierie graphique** industrielle, consultez [Planning Landscape](/architecture/planning-landscape).

## L’architecture

Le moteur ReAct implémente un modèle d’exécution en deux phases. La première phase est une boucle itérative d’utilisation d’outils : l’agent demande à plusieurs reprises au LLM une action, exécute l’outil demandé, ajoute l’observation, puis continue jusqu’à ce que le LLM indique « terminé ». La deuxième phase est la synthèse de la réponse : un appel LLM distinct en streaming lit l’ensemble de la trace d’exécution et produit la réponse destinée à l’utilisateur.

Cette séparation est délibérée. Les itérations d’outils sont optimisées pour la vitesse — chaque appel LLM dans la boucle utilise `chat()` sans streaming, car l’utilisateur n’a pas besoin de voir les actions JSON partielles ni les tokens de raisonnement intermédiaires. La génération de la réponse est optimisée pour l’expérience utilisateur — elle utilise `stream_chat()` en streaming afin que l’utilisateur voie les tokens apparaître en temps réel. Le résultat combine ainsi le meilleur des deux mondes : une exécution rapide des outils et une diffusion réactive de la réponse.
...
La boucle d’outils produit un `AgentResult` contenant l’historique complet de la conversation — le prompt système, la requête utilisateur, chaque message de l’assistant et chaque résultat d’outil. La méthode `stream_answer()` transforme cette trace en une réponse concise et cohérente. Dans le contexte de synthèse, les résultats des outils sont tronqués à 2 000 caractères chacun, ce qui permet de conserver un prompt léger, même après des workflows complexes utilisant plusieurs outils.

**Liaison du modèle.** Le LLM est injecté dans `ReActAgent.__init__()` et stocké dans `self._llm`. Chaque appel effectué au sein d’une même invocation de `run()` — toutes les itérations de la boucle d’outils ainsi que la synthèse finale de la réponse — utilise cette même instance. Le modèle ne change pas d’une itération à l’autre. Pour utiliser un autre modèle, il faut construire un nouvel objet `ReActAgent`. En mode DAG, `DAGExecutor._resolve_agent()` exploite ce fonctionnement : il crée un nouvel agent pour chaque étape — en sélectionnant le modèle dans `ModelRegistry` selon `step.model_hint` — immédiatement avant le début de la boucle ReAct de cette étape. Consultez [Moteur DAG — Remplacement par étape](/architecture/dag-engine#multi-llm-architecture) pour plus de détails.
...
Lorsque l'agent a utilisé au moins `_COMPLETION_CHECK_MIN_TOOLS` outils (par défaut : **3**, configurable via `REACT_COMPLETION_CHECK_MIN_TOOLS`) et décide de produire une réponse finale, une invite de vérification unique est injectée avant que la réponse soit acceptée :

1. La réponse répond-elle pleinement à la question initiale ?
...
La liste de vérification est un échange interne. Le tour qui y répond est une vérification, pas la réponse — l'accepter tel quel est ce qui laisserait fuir « Laissez-moi vérifier : 1. ... ✓ » dans le chat. Ce tour est donc consommé en interne et l'agent est interrogé une fois de plus pour la réponse destinée à l'utilisateur, qui est ce que `stream_answer()` diffuse. Si l'agent décide plutôt que quelque chose manque, il reprend la boucle d'appel d'outils et sa réponse ultérieure est prise telle quelle.

La liste de vérification se déclenche au maximum une fois par exécution, ne se déclenche pas pour les réponses conversationnelles simples (aucun outil utilisé), et est ignorée lorsque le budget d'itération restant ne peut pas couvrir à la fois le tour de vérification et le tour de réponse.

Le seuil prévient une latence inutile sur les tâches simples — un seul appel d'outil (par exemple, effectuer un calcul rapide) ne justifie pas une vérification, tandis qu'une investigation multi-outils bénéficie d'une vérification de cohérence finale. Définissez `REACT_COMPLETION_CHECK_MIN_TOOLS=1` pour une vérification toujours active.

La fonctionnalité est activée par défaut (`completion_check=True` sur `ReActAgent`) et peut être désactivée par instance d'agent. Les étapes DAG héritent de la valeur par défaut — chaque étape obtient sa propre liste de vérification unique.
