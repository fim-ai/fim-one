...
ReAct ne disparaîtra pas -- elle s'intégrera dans le modèle. Considérez l'analogie : vous n'écrivez pas manuellement les poignées de main TCP, mais TCP n'a pas disparu ; elle a été absorbée par le système d'exploitation. Quand les modèles sont suffisamment puissants, la boucle penser-agir-observer devient un comportement implicite à l'intérieur du modèle, et non un code de framework explicite. C'est déjà en cours : Claude Code est essentiellement un agent ReAct où la boucle est pilotée par le modèle lui-même, et non par un harnais externe.

La valeur durable de la planification DAG n'est pas « aider les modèles peu intelligents à décomposer les tâches » -- c'est l'**ordonnancement concurrent**. Même avec des modèles infiniment capables, la physique impose une latence : une chaîne série de 10 appels LLM prend 10 fois plus longtemps que 3 vagues parallèles. DAG est un problème d'ingénierie (comment exécuter les choses rapidement et de manière fiable), pas un problème d'intelligence (comment décider quoi exécuter). La logique de retry, le contrôle des coûts, la gestion des timeouts, l'observabilité -- ces éléments ne disparaissent pas quand les modèles deviennent plus intelligents.
...
C'est la phrase dont le reste de FIM One dépend. Une fois que le modèle possède le « quoi », la valeur durable se déplace vers la gouvernance du « comment » -- et le « comment » de plus haute valeur dans toute entreprise est la *portée* dans les systèmes qui refusent de communiquer entre eux. Le hub de connecteurs est à une étape inférentielle de là, et l'historique le montre : le projet a commencé comme un framework d'orchestration DAG (`fim-agent`) et est devenu une plateforme de connecteurs en une seule semaine -- non pas par une réécriture, mais parce que ce pari l'exigeait déjà.
...
3. **Décalage d'investissement.** Un éditeur de flux de travail visuel de qualité production (canevas, types de nœuds, passage de variables, débogage/relecture) représente des mois d'effort dédié. Le résultat serait une copie de faible fidélité de ce que la communauté de 120K stars de Dify maintient déjà. Cet effort est mieux dépensé sur l'architecture des connecteurs -- une capacité qu'aucun concurrent n'offre.

Le pari stratégique : **ne pas concurrencer sur la visualisation de flux de travail ; concurrencer en étant la plateforme d'agents où les systèmes rencontrent l'IA**. Laissez Dify posséder « construire des flux de travail IA visuellement ». FIM One possède « la plateforme d'agents tout-en-un pour les entreprises Global × China ».
