...
### Garder le fallback ENV synchronisé avec le panneau d’administration

Une fois qu’un groupe de modèles est activé dans Administration → Modèles, l’environnement d’exécution récupère ses modèles dans ce groupe, et les valeurs `LLM_*` / `FAST_LLM_*` / `REASONING_LLM_*` ci-dessus servent uniquement de valeurs de secours. Les chemins secondaires qui n’ouvrent jamais la base de données les lisent directement : `scripts/translate.py` (le hook i18n de pré-commit), `evals/`, le reranker Jina/OpenAI et la génération d’images. Pour éviter qu’ils n’utilisent un fournisseur obsolète, copiez le groupe actif dans le fichier d’environnement :
...
Le script lit les clés du fournisseur via l’ORM et les écrit directement dans le fichier ; rien n’est affiché. Redémarrez ensuite l’application.
