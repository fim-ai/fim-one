...
| `REACT_MAX_TURN_TOKENS` | Non | `0` | Disjoncteur d'urgence : nombre maximal de jetons cumulés (prompt + complément sur toutes les itérations) par tour ReAct unique. Par défaut `0` = illimité. **Ceci n'est PAS pour le contrôle des jetons quotidiens** — utilisez `token_quota` par utilisateur pour cela. C'est une soupape de sécurité de dernier recours pour les scénarios extrêmes comme un agent bloqué dans une boucle d'appels d'outils infinie. Atteindre cette limite interrompt la tâche en cours d'exécution, gaspillant tous les jetons consommés jusqu'à présent et renvoyant un résultat incomplet. Conservez à `0` sauf si vous avez un problème spécifique d'agent incontrôlable à contenir |
...
| `REACT_TOOL_OBS_TRUNCATION` | Non | `8000` | Nombre maximal de caractères par observation d'outil lors de la synthèse de la réponse finale. Les valeurs plus élevées conservent plus de données structurées (JSON, tableaux) au détriment de plus de jetons |
| `REACT_TOOL_RESULT_BUDGET` | Non | `40000` | Budget de jetons agrégé pour tous les résultats d'outils dans une session unique. Lorsque le total des jetons de résultats d'outils dépasse ce plafond, les nouveaux résultats sont tronqués avec un avis. Prévient l'encombrement du contexte provenant de grandes réponses API (par exemple, 5 appels de connecteur renvoyant 8K chacun). Définissez à `0` pour désactiver le plafond |
...
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | Non | `3` | Nombre minimum d'appels d'outils avant le déclenchement de la liste de vérification d'achèvement. Les tâches simples (1-2 appels d'outils) ignorent la vérification pour éviter une latence inutile. Définissez à `1` pour une vérification toujours active. S'applique également aux étapes DAG |
| `REACT_TURN_PROFILE_ENABLED` | Non | `true` | Émettre des journaux de synchronisation au niveau de la phase par tour (`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`). Une ligne de journal structuré par tour. Définissez à `false` pour désactiver complètement le profilage (surcharge zéro) |
| `REACT_PLAN_TOOL_ENABLED` | Non | `true` | Enregistrer l'outil `update_plan` todo pour que l'agent écrive et maintienne une liste de contrôle de plan pendant les tâches multi-étapes. Automatiquement ignoré pour les agents d'étapes DAG et les agents sans outils |
...
| `REACT_MAX_CONTINUATIONS` | Non | `3` | Nombre maximal de tours de continuation lorsque la réponse du modèle est coupée par la limite de jetons de sortie du fournisseur (`finish_reason=length`). Les segments tronqués sont cousus en une réponse transparente, à la fois dans la boucle d'agent et dans la synthèse en continu |
| `REACT_BACKGROUND_TOOLS_ENABLED` | Non | `true` | Offrir une option `run_in_background` sur les outils lents (exécution Python/shell/node en bac à sable). L'agent obtient immédiatement un identifiant de tâche et continue de travailler ; le résultat arrive sous forme de message `<task_notification>` lorsque l'outil se termine |
| `REACT_BG_WAIT_TIMEOUT` | Non | `300` | Nombre maximal de secondes à attendre pour les outils en arrière-plan encore en cours d'exécution lorsque l'agent souhaite finaliser sa réponse. Les tâches non terminées dans la fenêtre sont annulées avec une notification de délai d'attente explicite |
| `LLM_RATE_LIMIT_PER_USER` | Non | `true` | Utiliser des buckets de limite de débit à clé par utilisateur au lieu d'un bucket global unique au processus. Empêche un utilisateur bruyant de priver tous les autres sur le même worker. Le débit sous-jacent est codé en dur à 60 requêtes/min et 100K jetons/min par bucket — ce paramètre contrôle uniquement si le bucket est partagé (global) ou partitionné (par utilisateur). Définissez à `false` pour revenir au bucket global hérité (non recommandé) |
