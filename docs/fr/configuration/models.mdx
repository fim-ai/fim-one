...
|---|---|---|---|---|---|
| **OpenAI** | `gpt-5.4` | `gpt-5.4-mini` / `gpt-5.4-nano` | ✅ `reasoning_effort` | ✅ Tous | Meilleure prise en charge native des appels d’outils ; GPT-5.4 est le dernier modèle phare (mars 2026) |
| **Anthropic** | `claude-sonnet-4-6` | `claude-haiku-4-5` | ✅ via LiteLLM | ✅ Tous | Routage via l’API native ; prise en charge complète de `reasoning_content` ; contexte de 1M désormais disponible |
| **Google Gemini** | `gemini-2.5-pro` / `gemini-3.1-pro-preview` | `gemini-2.5-flash` / `gemini-3-flash-preview` | ✅ `reasoning_effort` | ✅ Tous | La version 2.5 est stable et généralement disponible ; la version 3.x est en préversion ; `gemini-3-pro-preview` a été arrêté le 9 mars |
| **DeepSeek** | `deepseek-chat` (V3.2) | `deepseek-chat` | ✅ `deepseek-reasoner` | ❌ | Texte uniquement ; V4 (avril 2026) ajoutera la vision |
| **Qwen (Alibaba)** | `qwen3.5-plus` / `qwen3-max` | `qwen3.5-flash` / `qwen-turbo` | ✅ `enable_thinking` sur `qwen3-max` | ⚠️ qwen3.5 uniquement | Le plus performant en chinois ; qwq/reasoning sont uniquement textuels |
| **ChatGLM (Zhipu)** | `glm-4.7` | `glm-4.7-flash` | `glm-5` | ⚠️ GLM-4.6V | Le FC forcé n’est pas pris en charge ; la vision nécessite un modèle VLM distinct |
| **MiniMax** | `MiniMax-M2.7` | `MiniMax-M2.5` | ❌ | ❌ | Texte uniquement ; M2.7 est le dernier modèle (mars 2026) ; 80,2 % sur SWE-Bench |
| **Kimi (Moonshot)** | `kimi-k2.5` | `kimi-k2` | ✅ `kimi-k2-thinking` | ⚠️ K2.5 uniquement | K2-thinking est uniquement textuel ; le FC forcé n’est pas pris en charge avec le mode thinking |
| **Ollama (local)** | `qwen3.5` / `llama4` | `qwen3.5:9b` | ❌ | Variable | Entièrement hors ligne, sans clé API ; Llama 4 prend en charge la vision |

<Tip>
**Vision** indique si le modèle accepte les entrées image. Cette fonctionnalité est requise pour le [traitement intelligent des documents (IDP)](/concepts/document-processing) : si votre modèle ne prend pas en charge la vision, IDP utilisera une extraction limitée au texte. Les fournisseurs marqués ⚠️ prennent en charge la vision pour certains modèles, mais pas pour d’autres ; vérifiez le modèle spécifique que vous utilisez.

Les pièces jointes des conversations suivent le même indicateur : avec un modèle uniquement textuel, une image jointe n’est pas envoyée et le modèle reçoit uniquement son nom de fichier. Le compositeur de conversation vous en informe avant l’envoi, en fonction du modèle qui serait effectivement utilisé pour le tour (paramètre de l’agent, puis groupe de modèles actif, puis modèle système par défaut). Le texte des documents n’est pas affecté : le contenu d’un PDF ou d’un DOCX est toujours extrait et injecté.
</Tip>

Ce tableau présente les combinaisons que nous recommandons, et non l’ensemble des fournisseurs pris en charge par FIM One. xAI (Grok), ByteDance Doubao, Mistral et tout relais compatible avec OpenAI fonctionnent également ; consultez la [matrice des capacités des fournisseurs](/architecture/llm-provider-guide#provider-capability-matrix) pour obtenir la liste complète et découvrir le mode de routage de chacun.
