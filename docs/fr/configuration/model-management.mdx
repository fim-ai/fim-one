...
Pour la configuration basée sur les variables d'environnement (sans interface d'administration), consultez [Variables d'environnement](/configuration/environment-variables). Pour les recommandations de sélection de modèles, consultez [Modèles recommandés](/configuration/models).
...
Contrôle si FIM One utilise un `tool_choice` forcé pour l'extraction de sortie structurée. Il s'agit du niveau 1 dans la chaîne de dégradation de sortie structurée — la méthode la plus fiable lorsque le modèle la supporte.

**Quand désactiver :**
- Votre modèle retourne des erreurs comme `"tool_choice 'specified' is incompatible with thinking enabled"`. Il s'agit d'une règle par fournisseur plutôt qu'une propriété des modèles de réflexion : `deepseek-reasoner` et les modèles Kimi avec réflexion rejettent un choix d'outil forcé, tandis que MiniMax réfléchit à chaque appel et l'accepte. Le tableau B de la [Matrice de capacités des fournisseurs](/architecture/llm-provider-guide#provider-capability-matrix) indique le verdict pour chaque fournisseur
- Les demandes de sortie structurée sont systématiquement lentes avec une pénalité d'environ 10 secondes par appel, suivie d'un basculement vers JSON Mode

**Effet lors de la désactivation :** FIM One ignore le niveau 1 (appel de fonction natif) et commence au niveau 2 (JSON Mode) pour la sortie structurée. L'appel d'outil de l'agent ReAct n'est pas du tout affecté — il utilise `tool_choice="auto"`, qui fonctionne avec tous les modèles indépendamment de ce paramètre.
...
Pour plus de détails techniques, voir [Compatibilité des fournisseurs LLM — tool_choice_enabled](/architecture/llm-provider-guide#the-fix-tool_choice_enabled).
...
Ceci est la vue par modèle de l'opérateur : quels bascules définir lorsque vous ajoutez un modèle. Les règles au niveau du fournisseur qu'il suit, avec les ancres de code qui les implémentent, se trouvent dans la [Matrice de capacité des fournisseurs](/architecture/llm-provider-guide#provider-capability-matrix). Si les deux divergent, la matrice est l'autorité et ce tableau doit être mis à jour.
