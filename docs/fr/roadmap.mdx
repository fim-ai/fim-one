...
Bugs tracés qui sont reproductibles en production mais pas encore corrigés. Chaque entrée nomme le symptôme, la zone de surface suspectée et la solution de contournement (le cas échéant). Les éléments passent à une section de version une fois qu'un correctif est défini et planifié.

- **L'éditeur d'agent affiche un avertissement de modifications non enregistrées sans aucune modification.** L'ouverture d'un agent existant via `/agents/[id]` et le clic immédiat sur le bouton retour déclenche la boîte de dialogue « Modifications non enregistrées » même si aucun champ n'a été modifié. La vérification de modification compare 20+ champs par rapport à la charge utile de l'agent chargée, donc une asymétrie de valeur par défaut entre l'initialisation d'état et la comparaison de modification suffit à causer une discordance fantôme — la suspicion actuelle porte sur l'un des champs imbriqués `model_config_json` / notification / approbation-routage, possiblement provenant de la normalisation `undefined` vs `null` vs `""`. Se reproduit particulièrement sur les agents à portée organisationnelle. Solution de contournement : rejeter la boîte de dialogue (`Discard and leave`) — aucune perte de données puisque rien n'a réellement changé. Une tentative de correctif (`cb40c86a`) a supprimé un scintillement de badge orphelin associé sur les sélecteurs de ressources mais n'a pas résolu ce problème.

- **L'enregistrement d'une modification d'agent peut échouer avec `Input should be 'initiator', 'agent_owner' or 'org_members'`.** Pydantic rejette le champ `confirmation_approver_scope` à la limite PUT `/api/agents/{id}` même si chaque valeur stockée dans la base de données est l'un des trois littéraux valides. Suspicion : le cast frontend `as "initiator" | "agent_owner" | "org_members"` est une promesse de compilation uniquement, donc une chaîne héritée ou inattendue au moment de l'exécution (possiblement provenant d'un modèle, d'une importation ou d'une migration plus ancienne) peut passer par `setConfirmationApproverScope` et être renvoyée verbatim. Solution de contournement : resélectionner explicitement une valeur dans la liste déroulante Approbation → Portée de l'approbateur avant d'enregistrer.

- **L'arrêt et la nouvelle tentative du playground affichent des artefacts visuels transitoires qu'une actualisation de page efface toujours.** Trois sources de rendu concurrentes — `activeConversation.messages` (snapshot BD), le flux SSE `messages` et l'espace réservé optimiste `pendingQuery` — ne sont pas regroupées dans un état dérivé unique, donc entre le clic sur « Retry » et l'arrivée de la réponse de l'assistant appariée, l'interface utilisateur peut (a) brièvement afficher la même requête deux fois dans la fenêtre pré-flux, (b) supprimer les bulles utilisateur orphelines antérieures de l'historique de nouvelle tentative tandis que `hasLiveMessages` est vrai et avant le rechargement de l'instantané, et (c) scintiller dans la fenêtre étroite entre l'événement SSE « done » et le prochain rafraîchissement `selectConversation`. **Les données ne sont jamais perdues** — chaque message utilisateur (y compris les nouvelles tentatives abandonnées) est conservé dans `conversation.messages`, porté dans l'appel LLM suivant via `normalize_alternating_messages` et rendu correctement après actualisation via `HistoryTurn.orphanUserContents` introduit dans le correctif de rendu `48ba08c6`. Pour le contexte, l'interface web propre de Claude présente une classe de bug analogue — l'arrêt au milieu d'une réponse et l'envoi immédiat d'une requête de suivi bifurquent parfois la requête de suivi en tant que branche d'édition sœur de la première requête plutôt que de l'ajouter en tant que nouveau tour — c'est donc un problème connu difficile dans les conceptions optimiste-UI + SSE + historique-persistant, et non un défaut spécifique à FIM One. Un correctif approprié nécessite de regrouper les trois sources de rendu dans un état dérivé unique ; reporté jusqu'à une refonte plus large de la machine d'état du Playground.
...
#### Durcissement de l'authentification et de la sécurité

- [x] ~~Confinement du type de jeton JWT (contournement 2FA) + correctifs de prise de compte OAuth et de session~~ *(livré dans v0.8.7)*
- [x] ~~Horodatages PostgreSQL sensibles au fuseau horaire~~ *(livré dans v0.8.6)*
- [x] ~~La comparaison d'horodatage de déconnexion forcée normalise les valeurs sensibles/non sensibles au fuseau horaire en UTC par conversion — les jetons révoqués sont fiablement rejetés sur SQLite et PostgreSQL~~ *(livré dans v0.8.8)*
- [x] ~~Les identifiants PostgreSQL Docker Compose sont remplaçables via les variables d'environnement `POSTGRES_*` (source unique dans la chaîne de connexion) — plus de défaut `fim:fim` accidentellement expédié sur les déploiements exposés~~ *(livré dans v0.8.8)*
- [x] ~~La liste de blocage SSRF ferme le contournement IPv6 mappé IPv4 (`::ffff:` métadonnées d'instance) ; les URL du serveur MCP SSE/Streamable-HTTP sont validées SSRF à la création et à la connexion~~ *(livré dans v0.8.8)*
- [x] ~~Le secours aux identifiants du propriétaire est optionnel : les connecteurs/serveurs MCP désactivent par défaut `allow_fallback` (les lignes existantes sont inversées) ; une ressource sans secours pour laquelle vous n'avez pas d'identifiants est masquée de l'ensemble d'outils au lieu d'échouer au moment de l'appel~~
- [x] ~~L'accès aux ressources unifié sur la visibilité (propre + abonné) : les connecteurs/KB/serveurs MCP abonnés sont liables aux agents (plus de 403 « non possédé »), et les étapes du connecteur de flux de travail appliquent l'accès du runner au lieu d'exécuter n'importe quel connecteur par id~~
- [x] ~~Les exécutions de flux de travail webhook/cron sont mesurées par rapport au quota de jetons du propriétaire — les étapes LLM/agent sans surveillance sont contrôlées à l'avance et leur utilisation compte vers le même quota que le chat, fermant le chemin LLM gratuit non mesuré~~

#### Compatibilité des modèles et fournisseurs

- [x] ~~Protocole de réflexion adaptative Anthropic pour Opus 4.6+/Sonnet 4.6/Fable 5 — la réflexion étendue fonctionne sur les modèles qui ont rejeté l'ancien paramètre de budget fixe (400 sur 4.7/4.8) ; avertit en cas de mauvais routage du proxy OpenAI~~ *(livré dans v0.8.8)*
- [x] ~~Le pool HTTP LLM partagé s'auto-répare après une éviction du cache client LiteLLM qui le ferme — les appels recréent le pool de manière transparente au lieu d'échouer indéfiniment jusqu'au redémarrage~~ *(livré dans v0.8.8)*

#### Connecteur + outillage

- [ ] **Phase 3-4 de divulgation progressive du connecteur** — `ConnectorExecutor` unifié (API/DB/MCP) ; validation d'action `jsonschema` ; découverte/exécution agnostiques au protocole
...
- [ ] **Mise en pool des connexions MCP** — pool STDIO avec isolation d'environnement par utilisateur ; partage de `httpx.AsyncClient` pour SSE/HTTP. Cible ≤100 ms de démarrage à chaud, connexions HTTP O(1) par serveur
- [x] ~~Les agents peuvent extraire `run_workflow` pour exécuter un workflow déterministe en tant qu'étape secondaire (recette statique ↔ recette dynamique de Skill) ; les workflows contrôlés par l'homme restent déclenchement uniquement, les exécutions sont auditées, profondeur/réentrance gardée~~

#### Garde-fous de contenu {/* dev: dev/archive/openai-agents-insights.md */}

- [x] ~~Garde-fous de contenu v0 (détecteur de tripwire entrée/sortie, détecteur de jailbreak, config env-var, événement SSE `guardrail_tripwired`)~~ *(livré en v0.8.7)*
...
- [ ] Interface de configuration des garde-fous par agent (panneau d'administration)
...
- [x] ~~PreToolUse enforcement hooks fail closed on error — a crashing approval gate no longer silently allows the call; non-enforcement hooks keep fail-open via `fail_open`~~ *(shipped in v0.8.8)*
...
- [ ] Modèles sortants : alertes d'échec, avertissements budgétaires, digests programmés, escalade, reçus d'audit, escalade d'approbation
- [ ] Phase 2 — Déclencheur entrant (@mention agent dans le groupe IM)
- [x] ~~Canal Feishu (sous-ensemble Phase 1) + Notification de fin de tâche~~ *(livré en v0.8.5)*

#### Couches d'autorisation des connecteurs {/* dev: dev/connector-rbac/00-overview.md */}

- [ ] Tier 1 — Mode DB (`ConnectorScopeGuard` hook PreToolUse : blocage des verbes, liste d'autorisation/refus de tables, masquage de colonnes, injection de prédicat de portée)
- [ ] Tier 2 — Mode Open API (interface d'administration pour exiger des identifiants par utilisateur ; tableau de bord de santé de la liaison de clés)
...
- [ ] Synchronisation du graphique organisationnel via IdP (départements Feishu → arborescence organisationnelle FIM) ; WeCom + DingTalk ensuite
- [ ] Channel se réduit à Delivery + Gate ; les capacités d'identité (SSO / OrgSync) vivent uniquement dans IdP
...
- [ ] Versioning API (`/v1/...`) + headers de dépréciation
- [ ] Callbacks webhook (par clé)
- [ ] Génération de SDK (Python + TypeScript)
- [ ] Developer Portal (panneaux Try-it + analytics par clé)
...
- [ ] Déchargement de la sortie d'outils — URIs `workspace://` pour les réponses >8K + outils `read/list/write_workspace_file`
...
- [x] ~~Détail de raisonnement préservé de bout en bout — la boucle diffuse sa véritable réponse finale (pas de re-synthèse avec perte); le raisonnement survit à la compaction, aux reconstructions de contexte et aux étapes de sous-agent~~ *(livré dans v0.8.8)*
- [ ] Adaptateur Gemini Context Cache (cycle de vie du cache REST séparé vs marqueur en ligne Anthropic)
- [ ] `cache_ttl` par agent (5min éphémère vs 1h étendu)

#### Durcissement de la boucle d'agent (modèles Claude Code)

- [x] ~~Tableau de plan : outil `update_plan` todo + réinjection de rappel de plan obsolète maintient les exécutions ReAct longues sur objectif et survit à la compaction~~
- [x] ~~Résilience des appels LLM : continuation de troncature de sortie + nouvelle tentative 529/504 avec Retry-After (secours de modèle de fallback déjà livré en tant que `FallbackLLM`)~~
- [x] ~~Les résultats d'outils tronqués par budget persistent dans l'espace de travail avant la coupure ; snapshot de transcript complet avant compaction avec perte~~
- [x] ~~Exécution d'outils en arrière-plan dans ReAct — les outils lents autorisés retournent un identifiant de tâche et notifient à la fin~~
- [x] ~~Replan DAG incrémental : les étapes complétées sont conservées entre les cycles de replan ; checkpoint au niveau des étapes reprend les exécutions interrompues en cas de nouvelle tentative~~ {/* dev: dev/incremental-dag.md */}
- [x] ~~La compaction préserve l'appairage tool_use/tool_result — les exécutions longues chargées d'outils ne meurent plus sur erreur 400 du fournisseur après compaction ; les résultats d'outils en arrière-plan atteignent le flux d'étapes + preuve DAG~~
- [ ] Reprise DAG en direct en milieu de flux (reconnexion SSE se rattache à un tour en cours) — reprise froide par checkpoint livrée {/* dev: dev/incremental-dag.md */}
...
- [ ] **Générateur de schéma DB avancé** — annotation pilotée par IA pour les déploiements de 1K-7K+ tables (sélectivité + raisonnement contextuel métier)
- [ ] Durcissement du bac à sable (isolation d'exécution de code v2)
