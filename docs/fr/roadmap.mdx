...
#### Barrières des connecteurs DB {/* dev: dev/connector-rbac/00-overview.md */}
...
- [x] Visibilité du schéma — autorisation et refus au niveau des tables/colonnes, ainsi que blocage des verbes (application de la lecture seule)
- [x] Auditabilité des barrières — `scope_rules_applied` dans `ConnectorCallLog` enregistre les barrières sous lesquelles chaque appel a été exécuté
- [ ] Transmission de la configuration par hook (`{"name", "config"}` schema) — le support des règles ScopeGuard {/* dev: dev/hook-system.md */}

#### Robustesse du contexte

- [ ] Compaction par blocs en entrée et budgets adaptés au modèle sur le chemin de chat principal, afin que toute combinaison de modèles reste dans la fenêtre de contexte

#### Couche Modèle

- [ ] Vérifier les chiffres d'utilisation du streaming du bridge Responses sur la prochaine mise à jour LiteLLM (un mauvais mappage en amont est suspecté)
- [ ] Retirer le bridge LiteLLM chat→responses une fois que le chemin natif GPT-5.x aura effectué une version complète
...
**Objectif** : Les deux surfaces de déploiement les plus vendables, toutes deux sur le même noyau et la même couche d'assemblage.

- [ ] Intégration JS bubble / iframe — un seul snippet dans un système hôte ; identité des visiteurs anonymes + attribution de facturation décidées avant la construction
- [ ] Mention @mention entrante Feishu — les agents vivent dans le groupe : interroger des données, soumettre des approbations, relancer des flux
- [ ] Modèles sortants : alertes d'échec, avertissements de budget, résumés planifiés, escalade, accusés d'audit
- [ ] Canaux WeCom / DingTalk à la suite de Feishu
