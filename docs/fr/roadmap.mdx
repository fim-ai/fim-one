...
- [x] ~~L'authentification par défaut du propriétaire est optionnelle : les connecteurs/serveurs MCP ont par défaut `allow_fallback` désactivé (lignes existantes inversées); une ressource sans authentification par défaut que vous ne possédez pas est masquée de l'ensemble d'outils au lieu d'échouer au moment de l'appel~~
- [x] ~~L'accès aux ressources unifié sur la visibilité (propre + abonné) : les connecteurs/KB/serveurs MCP abonnés sont liables aux agents (plus de 403 « non possédé »), et les étapes du connecteur de flux de travail appliquent l'accès du lanceur au lieu d'exécuter n'importe quel connecteur par id~~
