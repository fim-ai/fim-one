...
- [x] ~~Confinement du type de jeton JWT (contournement 2FA) + corrections de prise de compte OAuth et de session~~ *(livré en v0.8.7)*
- [x] ~~Horodatages PostgreSQL sensibles au fuseau horaire~~ *(livré en v0.8.6)*
- [x] ~~La comparaison d'horodatage de déconnexion forcée normalise les valeurs sensibles/naïves au fuseau horaire en UTC par conversion — les jetons révoqués sont fiablement rejetés sur SQLite et PostgreSQL~~ *(livré en v0.8.8)*
- [x] ~~Les identifiants PostgreSQL Docker Compose sont remplaçables via les variables d'environnement `POSTGRES_*` (source unique dans la chaîne de connexion) — plus de défaut `fim:fim` accidentellement livré sur les déploiements exposés~~ *(livré en v0.8.8)*
- [x] ~~La liste de blocage SSRF ferme le contournement IPv6 mappé IPv4 (`::ffff:` métadonnées d'instance) ; les URL du serveur MCP SSE/Streamable-HTTP sont validées SSRF à la création et à la connexion~~ *(livré en v0.8.8)*
- [x] ~~Le recours aux identifiants du propriétaire est optionnel : les connecteurs/serveurs MCP ont par défaut `allow_fallback` désactivé (les lignes existantes sont inversées) ; une ressource sans recours pour laquelle vous n'avez pas d'identifiants est masquée de l'ensemble d'outils au lieu d'échouer au moment de l'appel~~ *(livré en v0.8.9)*
- [x] ~~L'accès aux ressources est unifié sur la visibilité (propre + abonné) : les connecteurs/KB/serveurs MCP abonnés sont liables aux agents (plus de 403 « non possédé »), et les étapes du connecteur de flux de travail appliquent l'accès du responsable de l'exécution au lieu d'exécuter n'importe quel connecteur par id~~ *(livré en v0.8.9)*
- [x] ~~Les exécutions de flux de travail webhook/cron sont mesurées par rapport au quota de jetons du propriétaire — les étapes LLM/agent sans surveillance sont fermées à l'avance et leur utilisation compte pour le même quota que le chat, fermant le chemin LLM gratuit non mesuré~~ *(livré en v0.8.9)*
...
- [x] ~~Agents can pull `run_workflow` to run a deterministic workflow as a sub-step (static recipe ↔ Skill's dynamic recipe); human-gated workflows stay trigger-only, runs are audited, depth/re-entrancy guarded~~ *(shipped in v0.8.9)*
...
#### Renforcement de la boucle d'agent (modèles Claude Code)

- [x] ~~Tableau de plan : outil `update_plan` todo + réinjection de rappel de plan obsolète maintient les exécutions ReAct longues sur objectif et survit à la compaction~~ *(livré en v0.8.9)*
- [x] ~~Résilience des appels LLM : continuation de troncature de sortie + nouvelle tentative 529/504 avec Retry-After (secours de modèle de fallback déjà livré en tant que `FallbackLLM`)~~ *(livré en v0.8.9)*
- [x] ~~Les résultats d'outils tronqués par budget persistent dans l'espace de travail avant la coupure ; snapshot de transcript complet avant compaction avec perte~~ *(livré en v0.8.9)*
- [x] ~~Exécution d'outils en arrière-plan dans ReAct — les outils lents en liste blanche retournent un identifiant de tâche et notifient à la fin~~ *(livré en v0.8.9)*
- [x] ~~Replan DAG incrémental : les étapes complétées sont conservées entre les rounds de replan ; checkpoint au niveau des étapes reprend les exécutions interrompues en cas de nouvelle tentative~~ *(livré en v0.8.9)* {/* dev: dev/incremental-dag.md */}
- [x] ~~La compaction préserve l'appairage tool_use/tool_result — les exécutions longues avec beaucoup d'outils ne meurent plus sur erreur 400 du fournisseur après compaction ; les résultats d'outils en arrière-plan atteignent le flux d'étapes + preuve DAG~~ *(livré en v0.8.9)*
- [ ] Reprise DAG à chaud en mid-stream (reconnexion SSE se réattache à un tour en cours) — reprise à froid par retry-resume livrée via checkpoint {/* dev: dev/incremental-dag.md */}
