...
- [x] ~~Résilience des appels LLM : continuation de troncature de sortie + nouvelle tentative 529/504 avec Retry-After (basculement de modèle de surcharge déjà livré en tant que `FallbackLLM`)~~
- [x] ~~Les résultats d'outils tronqués par budget persistent dans l'espace de travail avant la coupure ; snapshot de transcription complète avant compaction avec perte~~
- [x] ~~Exécution d'outils en arrière-plan dans ReAct — les outils lents sur liste blanche retournent un identifiant de tâche et notifient à la fin~~
- [ ] Replanification DAG incrémentale : conserver les étapes complétées entre les cycles de replanification ; persister l'état des étapes pour la reprise en milieu de DAG {/* dev: dev/incremental-dag.md */}
