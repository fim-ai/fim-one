...
- Expansion AI Builder : 7 nouveaux outils de construction (GetSettings, TestConnection, ImportOpenAPI pour les connecteurs ; ListConnectors, AddConnector, RemoveConnector, SetModel pour les agents), drapeau `is_builder` sur les agents, actualisation automatique du prompt de construction, garde SSRF
- Frontend SSE v2 : curseur avec impulsion pointillée en streaming, snapshots de re-plan DAG sous forme de cartes réductibles, mise en page DAG découplée des états d'étape
...
- **Centre d'évaluation** : évaluation quantitative de la qualité des agents — CRUD de jeu de données de test (prompt + comportement attendu + assertions), exécutions d'éval (exécution parallèle + évaluateur LLM + résultats par cas réussi/échoué/latence/token), visionneuse de résultats avec interrogation automatique ; migration `r8t0v2x4z567`
...
- Cache d'outils pour l'exécution DAG — appels d'outils identiques mis en cache par exécution avec verrou asynchrone pour la prévention de la ruée vers le stampede (`DAG_TOOL_CACHE`)
...
- [x] ~~**Organisation du marché fantôme + Abonnements aux ressources**~~ : Organisation du marché intégrée (fantôme, pas d'adhésion automatique) remplace l'organisation de plateforme ; ressources découvertes via navigation sur le marché et explicitement abonnées (modèle pull) ; API Marché pour s'abonner aux ressources partagées ; publication sur le marché nécessite toujours un examen ; tableau d'abonnements aux ressources ; partage de ressources basé sur l'organisation remplaçant la visibilité globale
...
- [x] ~~**Identifiants de serveur MCP + Remplacement par utilisateur**~~ : tableau `mcp_server_credentials` ; point de terminaison `PUT /api/mcp-servers/{id}/my-credentials` ; drapeau `allow_fallback` pour le comportement de secours des identifiants
...
- [x] Corrections de stabilité — historique du fournisseur à alternance stricte, détection de limite d'appel d'outil parallèle, flux de confirmation d'agent non lié, gating de rôle de canal, suppression de doublons de nouvelle tentative, pas de paraphrase post-rejet
...
- [x] **JWT token-type confinement** — auth/SSE endpoints accept only genuine access tokens (plus SSE tickets for streaming), closing a 2FA bypass where any same-signed token authenticated
- [x] **OAuth account-takeover & session fixes** — email auto-link requires a provider-verified email; OAuth refresh tokens stored hashed so session rotation works
- [x] **PostgreSQL timezone-aware timestamps** — publish-review approval no longer 500s on PG; CI mypy gate + a tz-aware-column rule guard against recurrence
...
- [ ] Traitement par lot des statistiques d'utilisation pour les clés à haut QPS (flush local au processus, comptages exacts, pas de nouvelle dépendance) — reporté jusqu'à identification d'un goulot d'étranglement mesuré {/* dev: dev/public-api-phase2.md */}
