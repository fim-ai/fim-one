...
- [x] Protocole adaptive-thinking d'Anthropic pour Opus 4.6+/Sonnet 4.6/Fable 5 — la réflexion étendue fonctionne sur les modèles qui ont rejeté l'ancien paramètre de budget fixe (400 sur 4.7/4.8) ; avertit en cas de mauvais routage via proxy OpenAI
- [x] Le pool HTTP LLM partagé s'auto-répare après qu'une éviction du cache client LiteLLM le ferme — les appels recréent le pool de manière transparente au lieu d'échouer indéfiniment jusqu'au redémarrage
...
- [x] Détail du raisonnement préservé de bout en bout — la boucle diffuse sa réponse finale authentique (pas de re-synthèse avec perte) ; le raisonnement survit à la compaction, aux reconstructions de contexte et aux étapes de sous-agent
- [ ] Adaptateur Google Gemini Context Cache (cycle de vie de cache REST séparé vs marqueur inline Anthropic)
- [ ] Expansion du registre de prompts vers planificateur / vérificateur / classificateur de domaine / compact
- [ ] `cache_ttl` par agent (5 min éphémère vs 1 h étendu)
...
- [ ] Reconstruction de tokens de réflexion en milieu de flux (granularité de reprise plus fine que l'événement SSE complet)
