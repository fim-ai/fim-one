...
- **Canal Feishu (sous-ensemble Phase 1)** — Ressource `Channel` à portée organisationnelle avec identifiants chiffrés par Fernet ; `FeishuChannel` supporte l'envoi de cartes interactives + callback (vérification de signature + défi URL) ; interface de gestion Paramètres → Canaux (liste, créer/modifier avec protection d'état modifié, détails avec URL de callback copiable, envoi de test) ; API CRUD (`/api/channels`) et endpoint de callback d'événement (`/api/channels/{id}/callback`). Livré en avant-première pour la roadshow du 2026-04-24
...
- **Notifications de fin de tâche par agent** — Les agents ReAct ou DAG longue durée peuvent envoyer une carte récapitulative au canal de l'org quand une tâche se termine. Premier consommateur du modèle de notification sortante générique
- **Playground d'approbation de hooks** — La feuille de détails des canaux a une action « Tester le flux d'approbation » qui exerce le chemin de production complet (ligne `ConfirmationRequest` authentique, vrai callback Feishu, transitions d'état) — le même chemin de code qu'un hook de production utilise
- **Fallback CI i18n convivial pour contributeurs** — `.github/workflows/i18n-sync.yml` traduit EN → ZH/JA/KO/DE/FR sur master après fusion de PR et auto-commit avec `[skip ci]` ; les contributeurs n'ont plus besoin de `LLM_API_KEY` localement. Garde de pré-commit refuse les modifications manuelles aux fichiers de locale générés (`ALLOW_LOCALE_EDIT=1` override pour les corrections de traduction légitimes). Vérification de bout en bout via push de test de fumée
...
- **Renforcement** — Les clics de callback Feishu en doublon produisent une carte de remplacement au lieu de double-décision ; les clics de callback concurrents résolus via vérification de nombre de lignes `UPDATE ... WHERE status='pending'` conditionnel ; les approbations en attente expirent automatiquement après `CHANNEL_CONFIRMATION_TTL_MINUTES` (24h par défaut) via balayage en arrière-plan ; Paramètres → Canaux respecte le rôle org (les membres voient l'interface en lecture seule) ; l'agrégateur d'appels d'outils parallèles gère les fournisseurs qui réutilisent `index=0` pour chaque delta ; la redirection d'expiration de session préserve la chaîne de requête

## Versions Planifiées

### v0.8.6 — Polissage des canaux et des crochets

**Objectif** : Fermer les points en suspens du déploiement des canaux et crochets v0.8.5 avant l'arrivée de la vague de durcissement de production v0.9. La portée est intentionnellement étroite — polissage, pas nouvelle capacité.

- [ ] **Passage de configuration par crochet** — Les entrées `class_hooks` d'aujourd'hui sont des chaînes nues ; pour remplacer `FeishuGateHook.timeout_seconds`, `poll_interval_seconds` ou `callback_base_url` par agent, le schéma doit accepter des objets `{"name": "feishu_gate", "config": {...}}` qui sont transmis en tant que kwargs à la fabrique de crochets. Suivi à faible risque ; les valeurs par défaut actuelles (délai d'expiration 120s / sondage 1,5s / URL de rappel var env) sont acceptables pour le moment.
- [ ] **Précision de `tools_used` du DAG** — La carte de notification d'achèvement dérive actuellement `tools_used` de `plan.steps[*].tool_hint` (la suggestion du planificateur), et non des noms d'outils réels que les boucles ReAct par étape ont choisis. Canalisez les noms d'outils réellement choisis hors du rappel d'achèvement d'étape de l'exécuteur DAG afin que les cartes de notification reflètent ce qui a été réellement exécuté.
- [ ] **Politique d'héritage des crochets pour les sous-agents délégués et les nœuds AGENT du flux de travail** — Aujourd'hui, les enfants `CallAgentTool` et les nœuds `AGENT` du flux de travail créent des ReActAgents frais qui n'héritent pas du registre de crochets du parent, donc un appel d'outil sensible atteint via délégation contourne silencieusement `feishu_gate`. Décidez et documentez : les agents enfants héritent-ils (sécurisé par défaut, empêche le contournement de la porte) ou s'isolent-ils (permet aux équipes de déléguer du travail non soumis à l'approbation à un agent enfant) ? Les exécutions du centre d'évaluation restent désactivées par conception (l'automatisation ne peut pas être bloquée par une approbation humaine).

Tout ici est additif et derrière les abstractions existantes — aucune modification de schéma, aucune modification d'API de rupture, sûr à déployer par étapes entre v0.8.5 et v0.9.
