...
- [x] ~~Confinement du type de jeton JWT (contournement 2FA) + corrections de prise de compte OAuth et de session~~ *(livré dans v0.8.7)*
- [x] ~~Horodatages PostgreSQL sensibles au fuseau horaire~~ *(livré dans v0.8.6)*
- [x] ~~La comparaison d'horodatage de déconnexion forcée normalise les valeurs sensibles/naïves au fuseau horaire en UTC par conversion — les jetons révoqués sont fiablement rejetés sur SQLite et PostgreSQL~~ *(livré dans v0.8.8)*
...
- [x] ~~Le recours aux identifiants du propriétaire est optionnel : les connecteurs/serveurs MCP ont par défaut `allow_fallback` désactivé (les lignes existantes sont inversées) ; une ressource sans recours que vous n'avez pas les identifiants pour est masquée de l'ensemble d'outils au lieu d'échouer au moment de l'appel~~
...
#### Système de hooks {/* dev: dev/hook-system.md */}

- [ ] Passage de configuration par hook (`{"name": ..., "config": {...}}` schéma)
- [ ] Précision du DAG `tools_used` (noms réels des outils à partir de ReAct par étape via callback de fin d'étape)
- [ ] Héritage des hooks pour `CallAgentTool` + nœuds Workflow `AGENT` (défaut de sécurité vs défaut de flexibilité)
- [ ] Hooks intégrés : enregistrement automatique `ConnectorCallLog`, blocage du mode lecture seule, troncature des résultats DB, limite de débit par connecteur
- [ ] Point de hook `SessionStart` + hooks YAML définis par l'utilisateur
- [x] ~~Les hooks d'application PreToolUse échouent fermés en cas d'erreur — une porte d'approbation qui s'écrase ne permet plus silencieusement l'appel ; les hooks sans application conservent fail-open via `fail_open`~~ *(livré en v0.8.8)*
- [x] ~~Squelette + FeishuGateHook + Playground d'approbation + runtime ReAct/DAG~~ *(livré en v0.8.5)*
...
- [ ] Rotation de clé API (délai de grâce de 24h)
...
- [x] ~~Détail de raisonnement préservé de bout en bout — la boucle diffuse sa réponse finale authentique (pas de re-synthèse avec perte); le raisonnement survit à la compaction, aux reconstructions de contexte et aux étapes de sous-agent~~ *(livré en v0.8.8)*
- [ ] Adaptateur Gemini Context Cache (cycle de vie de cache REST séparé vs marqueur inline Anthropic)
- [ ] Expansion du registre de prompt vers planificateur / vérificateur / classificateur de domaine / compact
- [ ] `cache_ttl` par agent (éphémère 5min vs étendu 1h)
- [ ] Table de point de contrôle au niveau des étapes DAG pour reprise en milieu de DAG
- [ ] Colonne Message `tool_call_id` dédiée (recherche d'orphelin indexée à grande échelle)
- [ ] Reconstruction de token de réflexion en milieu de flux (granularité de reprise plus fine que l'événement SSE complet)
- [ ] Sonde d'honnêteté de cache de relais API (déclenchée par admin : détecte les relais supprimant `cache_control`)

#### Suivi de la fiabilité (Matrice de priorités Agent Core)

- [ ] Persistance de l'état de remplacement de contenu (invariant de streaming #2 : « une fois vu, sort figé »)
- [ ] Routeur de contexte des pièces jointes (déduplication, agrégation du budget, vérifications de disponibilité ; s'associe au déchargement Workspace)
- [ ] Workers de requête latérale (pools dédiés pour rappel / classification / résumé afin qu'ils ne rivalisent pas pour le budget de limite de débit principal)
...
- [ ] **Remplacement `credential_policy` par flux de travail** (`owner` / `caller` / `auto`) — remplace le mappage par défaut `trigger_source → identity`
