...
- [x] ~~Tableau de planification : outil `update_plan` todo + réinjection de rappel de plan obsolète maintient les exécutions ReAct longues sur objectif et survit à la compaction~~
- [x] ~~Résilience des appels LLM : continuation de troncature de sortie + nouvelle tentative 529/504 avec Retry-After (secours de modèle de surcharge déjà livré en tant que `FallbackLLM`)~~
- [x] ~~Les résultats d'outils tronqués par budget persistent dans l'espace de travail avant coupure ; snapshot de transcript complet avant compaction avec perte~~ 
...
- [x] ~~Replanification DAG incrémentale : les étapes terminées sont conservées entre les cycles de replanification ; checkpoint au niveau des étapes reprend les exécutions interrompues en cas de nouvelle tentative~~ {/* dev: dev/incremental-dag.md */}
- [ ] Reprise DAG en direct en milieu de flux (reconnexion SSE se rattache à un tour en cours) — reprise froide par checkpoint livrée via checkpoint {/* dev: dev/incremental-dag.md */}
