...
- [x] ~~Résilience des appels LLM : continuation de troncature de sortie + nouvelle tentative 529/504 avec Retry-After (secours de modèle de basculement déjà livré en tant que `FallbackLLM`)~~
- [ ] Les résultats d'outils volumineux persistent dans les fichiers d'espace de travail au lieu de troncature de tête ; snapshot de transcript complet avant compaction LLM
- [ ] Exécution d'outils en arrière-plan dans ReAct — les outils lents retournent un espace réservé et notifient à la fin
- [ ] Replanification DAG incrémentale : conserver les étapes complétées entre les cycles de replanification ; persister l'état des étapes pour la reprise mid-DAG {/* dev: dev/incremental-dag.md */}
