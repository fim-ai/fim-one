...
- [x] ~~Les identifiants PostgreSQL Docker Compose peuvent être remplacés via les variables d'environnement `POSTGRES_*` (source unique dans la chaîne de connexion) — plus de défaut `fim:fim` accidentellement expédié sur les déploiements exposés~~ *(livré dans v0.8.8)*
