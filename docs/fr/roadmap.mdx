...
- [x] Les réponses finales se diffusent nativement — la boucle se transmet via un signal `finish` et la réponse est écrite comme un tour en streaming de tokens en direct
- [x] Les réponses affichent des diagrammes Mermaid, des figures SVG et des tableaux de comparaison de style carte, avec copie/export sur les réponses, les blocs de code et les tableaux
- [x] Le markdown rendu est assaini, fermant l'injection HTML brut à partir de la sortie du modèle et des fichiers téléchargés
...
- [x] Les exports de conversation sont composés pour CJK — le PDF intègre une vraie police (espacement correct, puces et gras), DOCX déclare une police d'Asie de l'Est, tous deux à une échelle de taille
