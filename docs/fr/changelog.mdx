...
- **Quitter (ou être retiré d')une organisation révoque désormais son accès partagé.** Les ressources auxquelles vous ne pouviez accéder que via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour elles sont supprimés — vous ne pouvez donc plus utiliser ces ressources ni revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via le Marché ne sont pas affectées.
- **La suppression d'un compte supprime désormais tout, de manière cohérente.** Les chemins d'accès admin « supprimer l'utilisateur » et libre-service « supprimer mon compte » divergeaient autrefois — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin admin laissait derrière lui des clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent désormais par une seule routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
...
- **Les actions de connecteur nécessitant une confirmation ne peuvent plus s'exécuter sans surveillance dans un flux de travail.** Une action de connecteur marquée comme nécessitant une confirmation s'exécutait autrefois à l'intérieur d'un flux de travail avec la porte d'approbation silencieusement contournée. L'étape échoue désormais en fermé avec un message pour placer un nœud d'intervention humaine avant (la porte d'approbation native du flux de travail) ou utiliser une action qui ne nécessite pas de confirmation.
- **Les étapes du flux de travail n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un flux de travail avait le secours aux identifiants du propriétaire désactivé, un non-propriétaire exécutant ce flux de travail revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue désormais avec un message clair pour configurer vos propres identifiants, correspondant au chemin de chat.
...
- **Les étapes du flux de travail ne peuvent appeler que les connecteurs auxquels vous avez réellement accès.** Une étape de connecteur de flux de travail s'exécutait autrefois sur n'importe quel connecteur dont elle référençait l'id, tant que ce connecteur existait ; elle applique désormais la visibilité de l'exécutant (possédé ou abonné), donc un flux de travail partagé ne peut pas atteindre un connecteur auquel vous n'avez pas accès simplement en référençant son id.
...
- **Les réponses multi-étapes (DAG) ne déforment plus leurs sources.** Quand l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé progressait — donc une source qui listait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la gravité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation des objectifs ne pouvait pas le détecter car elle ne voyait jamais la source originale. Chaque étape conserve désormais sa sortie d'outil brute comme preuve de source faisant autorité, que la synthèse finale et l'évaluateur vérifient par rapport à la réponse, donc les totaux, énumérations et attributs correspondent à ce que les sources ont réellement dit (et la confiance baisse avec une discordance nommée quand ils ne correspondent pas).
- **Le nœud LLM du flux de travail n'affiche plus les contrôles de modèle qui ne signifiaient pas ce qu'ils impliquaient.** Il avait une liste déroulante de modèle codée en dur dont la sélection était ignorée au moment de l'exécution, plus un bouton bascule Fast/Main exposant un concept interne (dégradation automatique). Les deux sont supprimés — le nœud exécute désormais toujours votre modèle configuré par le système (Model Provider, revenant à `.env`), de la même manière que le playground le résout, avec une note d'une ligne pointant vers l'endroit où le modèle est configuré.
