...
- **La barre latérale place la navigation en premier.** La navigation des modules se situe désormais au-dessus de la liste de conversations avec des sections Build / Resources / Operate étiquetées, au lieu d'être cachée dans un dock en bas de la barre latérale.

- **Les réponses en streaming s'affichent régulièrement sans scintillement.** Une réponse en cours de construction s'affiche bloc par bloc : les blocs terminés restent stables au lieu de se réafficher à chaque image, et le formatage incomplet (gras, liens, mathématiques) est complété ou conservé jusqu'à ce qu'il finisse de s'afficher.

- **Les réponses finales s'affichent au fur et à mesure qu'elles sont écrites.** L'agent signale maintenant quand il a terminé la collecte et écrit la réponse comme un tour en streaming en direct, de sorte que les réponses longues arrivent jeton par jeton avec tous les détails au lieu d'être rejouées après une attente. `REACT_FINISH_SIGNAL=false` restaure le comportement précédent.

- **Les docs d'architecture mappent l'ingénierie graphique sur FIM One.** [Planning Landscape](/architecture/planning-landscape) explique l'ingénierie en boucle par rapport à celle en graphe, comment cela diffère des flux de travail visuels classiques, comment cela s'applique à Workflow / DAG / ReAct, et ce qu'il faut emprunter pour chaque moteur. Il indique également le fait actuel que chaque étape DAG est déjà un agent ReAct complet (et non un simple appel LLM). Les pages des moteurs ReAct et DAG y font référence.

- **Les chats simples offrent toujours des suggestions de suivi.** Les conversations sans agent lié reçoivent des questions suggérées après chaque réponse ; les chats liés à un agent nécessitent toujours le bouton bascule Suggestions de suivi par agent (désactivé par défaut). Les tours de fermeture tels que les salutations ou les remerciements peuvent ne retourner aucune puce.
...
- **Les blocs de raisonnement restent repliés pendant l'exécution d'un tour.** Le raisonnement dans la sortie ReAct et DAG affiche maintenant un aperçu d'une ligne par défaut au lieu de s'étendre au fur et à mesure du streaming, gardant le résultat visible ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement typographiés pour le chinois.** L'export PDF intègre désormais une véritable police CJK au lieu de la référencer, ce qui corrige l'espacement réduit dans le texte anglais et les puces qui s'affichaient comme des caractères chinois non liés, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est de sorte que le texte gras mixte chinois/anglais ne sort plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les en-têtes d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent sur la bonne voie lors de tâches longues.** L'agent est maintenant rappelé de changer d'approche quand il répète le même outil sans succès, incité à écrire un plan quand il travaille sans en avoir, et ne peut plus fournir une réponse finale alors que des éléments du plan sont toujours ouverts sans une étape de vérification explicite.
...
- **Les actions de bloc restent discrètes jusqu'à ce que vous les utilisiez.** Les boutons Copier et Exporter sur les réponses, les blocs de code, les diagrammes et les tableaux s'affichent au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale garde maintenant la conversation ensemble.** La liste de conversations se situe directement sous Nouveau chat et Recherche, et la navigation des modules s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et à la barre latérale une hiérarchie claire de haut en bas.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de s'afficher en streaming pendant qu'il est généré, puis se replie quand l'étape se termine.

- **Les pages de liste affichent progressivement leurs cartes.** Les agents, connecteurs, bases de connaissances, compétences, flux de travail, outils, conversations, artefacts et le marché échelonnent l'affichage de leurs cartes au premier chargement au lieu d'apparaître tous à la fois. Le filtrage et la pagination restent instantanés, de sorte que taper dans une zone de recherche ne rejoue jamais l'entrée.
