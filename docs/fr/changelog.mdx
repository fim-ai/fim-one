...
- **Les fichiers PDF et Word exportés sont correctement mis en page pour le chinois.** L'export PDF intègre désormais une véritable police CJK au lieu de la référencer, corrigeant l'espacement réduit dans le texte anglais et les puces qui s'affichaient sous forme de caractères chinois non liés, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est afin que le texte gras mixte chinois/anglais ne soit plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les titres subordonnés à la structure propre de la transcription.

- **Les agents restent sur le plan pendant les tâches longues.** L'agent est maintenant rappelé à changer d'approche lorsqu'il répète le même outil sans succès, incité à rédiger un plan lorsqu'il travaille sans plan, et ne peut plus fournir une réponse finale tant que les éléments du plan sont toujours ouverts sans une étape de vérification explicite.

- **Les réponses en streaming s'estompent progressivement au lieu d'apparaître soudainement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du streaming, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en évidence la ligne sous le curseur, et les puces de liste se situent derrière le texte au lieu de rivaliser avec lui.

- **Les actions de bloc restent discrètes jusqu'à ce que vous les utilisiez.** Les boutons de copie et d'export sur les réponses, les blocs de code, les diagrammes et les tableaux sont révélés au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale garde maintenant la conversation ensemble.** La liste de conversations se trouve directement sous Nouvelle conversation et Recherche, et la navigation des modules s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et une hiérarchie claire de haut en bas à la barre latérale.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de se diffuser en direct pendant sa génération, puis se replie à la fin de l'étape.

- **Les pages de liste font entrer progressivement leurs cartes en place.** Les agents, connecteurs, bases de connaissances, compétences, workflows, outils, conversations, artefacts et le marché échelonnent leurs cartes à l'entrée au premier chargement au lieu d'apparaître tous à la fois. Le filtrage et la pagination restent instantanés, donc taper dans une zone de recherche ne rejoue jamais l'entrée.
...
- **L'animation suit désormais le paramètre système « réduire les animations ». ** Le dégradé flottant de l'arrière-plan de connexion, l'entrée du héros, le texte du message injecté respirant et le curseur de streaming continuaient à bouger indépendamment de la préférence du système d'exploitation. Ils restent maintenant immobiles, tandis que les indicateurs signalant un travail en cours restent visibles pour que l'interface ne semble jamais figée.
...
- **Le streaming du raisonnement fonctionne à nouveau sur les modèles plus récents. ** Le raisonnement en direct était câblé à partir d'une liste codée en dur de noms de modèles, donc tout ce qui a été publié après l'écriture de cette liste affichait un spinner puis déversait le bloc de raisonnement entier à la fois. Tout modèle qui émet du raisonnement le diffuse maintenant en streaming.

- **La vue de chat suit la sortie rapide et arrive à la réponse. ** Elle ne s'arrête plus au milieu du raisonnement et ne signale plus faussement du nouveau contenu ci-dessous, elle suit le contenu qui se développe sans nouveaux événements, et quand un tour se termine, elle s'installe au début du résultat au lieu de la fin. S'éloigner en faisant défiler rend toujours le contrôle, et revenir au bas reprend le suivi.
...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus au milieu d'un tour. ** Pendant qu'un tour s'exécute, l'icône de l'onglet conserve la marque de l'application et la pulse entre la couleur complète et le gris, se maintenant pendant tout le tour et restaurant de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application. Avec les animations réduites activées, elle conserve une marque grise statique au lieu de s'animer.

- **Les agents ne récupètent plus un point de terminaison des dizaines de fois après un résultat tronqué. ** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à frapper le même point de terminaison avec des paramètres variables est averti puis arrêté brutalement.

- **Une exécution plantée signale maintenant une erreur au lieu d'un « terminé » vert. ** Quand l'agent s'arrêtait en raison d'une défaillance interne, le texte d'exception brut était livré comme réponse sous une coche de succès ; les défaillances passent maintenant par le canal d'erreur et apparaissent comme une notification d'erreur.
...
- **Les événements en streaming divisés entre les chunks réseau ne sont plus perdus. ** Un flux repris pouvait perdre un événement dont la trame arrivait en deux morceaux ; les deux chemins de flux partagent maintenant un analyseur qui gère les limites de chunks arbitraires.
...
- **Un outil suspendu ne peut plus bloquer une exécution indéfiniment. ** Chaque appel d'outil a maintenant une limite de temps réelle (180s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de bloquer le tour.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après la compaction. ** Le résumé qui condense les tours plus anciens était silencieusement supprimé à la prochaine exécution ; il est maintenant reporté, donc l'agent conserve ce qui a été discuté avant.

- **Les longs résultats d'outils conservent leur fin ainsi que leur début. ** Quand un long résultat est tronqué pour tenir dans la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande compte généralement le plus), la troncature tient correctement compte du texte non-latin, et les résultats ultérieurs sont garantis une fenêtre minimale au lieu d'être complètement effacés.
