...
- **Les réponses de chat affichent désormais les diagrammes Mermaid et les figures SVG.** Un bloc de code `mermaid` devient un diagramme thématisé et un bloc `svg` devient une image avec un bouton bascule de source et un téléchargement ; les deux reviennent au code brut si la source est incomplète ou ne s'analyse pas, de sorte qu'un diagramme partiellement diffusé n'affiche jamais d'erreur.

- **Les réponses, les blocs de code et les tableaux peuvent être copiés.** Chaque réponse dispose d'une action de copie au format Markdown, les blocs de code portent un en-tête indiquant leur langage, et les tableaux offrent une copie au format Markdown plus un téléchargement CSV (UTF-8, s'ouvre correctement dans Excel).

- **Les citations indiquent le document dont elles proviennent.** Un marqueur de citation s'affiche désormais sous forme de pastille portant le nom du document source, avec les marqueurs adjacents regroupés en une seule pastille et un compteur ; le popover liste chaque source derrière.
...
- **Les agents restent concentrés sur le plan pendant les tâches longues.** L'agent est maintenant rappelé à changer d'approche lorsqu'il répète le même outil sans succès, incité à rédiger un plan lorsqu'il travaille sans, et ne peut plus fournir de réponse finale tant que des éléments du plan restent ouverts sans une étape de vérification explicite.

- **Les réponses en flux s'affichent progressivement au lieu d'apparaître brusquement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du flux, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en surbrillance la ligne sous le curseur, et les puces de liste se situent derrière le texte au lieu de rivaliser avec lui.
...
- **Les citations nomment à nouveau le document d'où elles proviennent.** En mode de récupération simple, chaque source était étiquetée « Knowledge Base » car l'outil de récupération supprimait le nom de fichier d'origine ; il rapporte maintenant la source, ce qui correspond à ce que le pipeline de grounding faisait déjà.
...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en cours de tour.** La favicon affiche maintenant un glyphe d'activité stable pendant toute l'exécution et restaure de manière fiable l'icône réelle quand le tour se termine, même après une navigation dans l'application.

- **Les agents ne récupèrent plus un point de terminaison des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à frapper le même point de terminaison avec des paramètres variables reçoit un avertissement puis est arrêté brutalement.

- **Une exécution plantée rapporte maintenant une erreur au lieu d'un « terminé » vert.** Quand l'agent s'est arrêté en raison d'une défaillance interne, le texte d'exception brut était livré comme réponse sous une coche de succès ; les défaillances passent maintenant par le canal d'erreur et s'affichent comme une notification d'erreur.

- **La reconnexion à un flux ne rejoue plus la sortie que vous aviez déjà vue.** Le flux en direct porte maintenant le même curseur de position que le point de terminaison de reprise, donc après une connexion perdue seuls les événements manqués sont rejoués au lieu de tout le tour.

- **Les événements en flux divisés entre les chunks réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux morceaux ; les deux chemins de flux partagent maintenant un seul analyseur qui gère les limites de chunk arbitraires.

- **Appeler un autre agent retourne sa réponse réelle.** La réponse du sous-agent arrivait autrefois enfouie dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; il reçoit maintenant juste la réponse.

- **Un outil bloqué ne peut plus immobiliser une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps réel (180s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de bloquer le tour.

- **Un agent coincé à répéter le même appel d'outil est maintenant arrêté tôt.** Les appels répétés identiques sont refusés après quelques tentatives au lieu de consommer tout le budget d'itération.
