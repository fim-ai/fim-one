...
- **Les portées des clés API sont désormais transmises par requête au lieu d'être attachées à l'objet utilisateur.** Cela ferme un chemin de confusion de portée latent où un utilisateur mis en cache ou partagé via une carte d'identité pourrait hériter de l'ensemble de portées d'un autre contexte d'authentification.
- **Un fichier `.env` généré automatiquement contenant le secret JWT généré est désormais écrit avec des permissions réservées au propriétaire (0600).** Les fichiers `.env` préexistants conservent leurs permissions.
...
- **Les connecteurs de base de données ne peuvent plus être partagés, et leur outil SQL brut est réservé au propriétaire.** La publication d'un connecteur de base de données vers une organisation ou globalement est désormais rejetée, et l'outil de base de données SQL arbitraire n'est offert qu'au propriétaire du connecteur — jamais à un abonné, indépendamment du paramètre de secours des identifiants du propriétaire. Auparavant, un connecteur de base de données partagé exposait du SQL sans restriction sur le compte de base de données du propriétaire (généralement hautement privilégié) à quiconque s'abonnait, sans scoping par appelant. L'accès à la base de données limité par utilisateur reviendra via une future couche d'actions déclaratives.
- **Les workflows déclenchés par webhook et cron comptent désormais contre le quota de jetons du propriétaire.** Une exécution de workflow sans surveillance — déclenchée via sa clé API publique ou selon un calendrier — s'exécutait auparavant sans vérifier ni enregistrer l'utilisation des jetons pour les étapes LLM et agent, un chemin LLM gratuit non mesuré qui contournait entièrement les quotas. Ces exécutions sont désormais limitées au quota du propriétaire du workflow en amont (un déclencheur public est refusé, et une exécution programmée est ignorée, lorsque le propriétaire a déjà dépassé le quota) et leur utilisation de jetons est enregistrée par exécution et comptabilisée dans le même quota mensuel que le chat.

### Corrections

- **La connexion OAuth échoue maintenant rapidement avec un message clair "le fournisseur a pris trop de temps" lorsque GitHub/Google/Discord/Feishu sont lents**, au lieu de rester bloquée sur les délais d'attente par défaut de la bibliothèque et d'afficher un échec générique.
- **L'agent ne génère plus d'image parasite lorsqu'on lui demande de remodeler un livrable HTML, et ne prétend jamais qu'un fichier a été créé sans l'avoir réellement écrit dans l'espace de travail.** La génération d'images est maintenant explicitement limitée aux images raster, et les livrables de fichiers doivent être écrits via un appel d'outil — coller du contenu dans le chat ne compte plus comme "générer un fichier".
- **Les pages HTML générées utilisent maintenant par défaut une palette chaleureuse inspirée de Claude au lieu de l'apparence générique dégradée en violet.** Lorsque vous ne spécifiez pas de style, l'agent utilise une base minimale chaleureuse ; votre propre style gagne toujours lorsqu'il est spécifié.
- **Les longues conversations chargées d'outils ne se cassent plus lorsque l'historique est compacté.** La compaction de contexte pouvait couper la fenêtre conservée entre un appel d'outil et ses résultats, produisant une séquence de messages que le fournisseur LLM rejette carrément (HTTP 400) — exactement sur les longs passages qui ont le plus besoin de compaction. Le compacteur conserve maintenant chaque appel d'outil avec ses résultats du côté conservé.
- **Les résultats des outils en arrière-plan apparaissent maintenant dans le flux d'étapes et dans les preuves DAG.** Une exécution d'outil en arrière-plan livrait autrefois sa sortie réelle uniquement dans le message de notification interne de l'agent — la chronologie des étapes de l'interface utilisateur et les preuves de vérification de réponse DAG ne voyaient que l'espace réservé de dispatch. Le résultat vidé est maintenant émis comme un événement de complétion d'outil normal sous le vrai nom d'outil.
- **Une sortie de planificateur rare avec des identifiants d'étape en double ne casse plus tout le tour DAG.** Les identifiants en double étaient autrefois mal diagnostiqués comme une erreur de "dépendance circulaire" ; ils sont maintenant renommés de manière déterministe et le plan continue.
- **Plusieurs corrections de justesse de la boucle ReAct.** Un outil retournant une sortie vide conservait sa note "complété sans sortie" au lieu d'un résultat vide lorsque l'espace de travail est activé ; les étapes DAG bloquées par une dépendance insatisfaisable émettent maintenant un événement terminal approprié au lieu de laisser l'interface utilisateur en suspens ; et pour les modèles sans appel d'outil natif, la synthèse de réponse de secours voit maintenant les résultats d'outil, pas seulement les appels d'outil.
- **L'espace de travail de conversation est maintenant réellement connecté au chat — les résultats d'outils surdimensionnés sont récupérables au lieu d'être silencieusement perdus.** Les chemins de récupération d'espace de travail de la couche agent (déchargement de sortie volumineuse avec un pointeur `workspace://`, sauvetage par copie complète des résultats d'outil tronqués par budget, snapshots de transcription pré-compaction) existaient mais les points de terminaison de chat ne créaient jamais d'espace de travail, donc aucun d'eux ne s'exécutait : une fois le budget de résultat d'outil épuisé, le modèle recevait des observations vides sans moyen de relire les données, et répondait par conjecture. Les agents ReAct et DAG-step obtiennent maintenant un espace de travail par conversation, et la suppression d'une conversation nettoie ses fichiers d'espace de travail et tout point de contrôle de reprise après sinistre restant.
- **Les exécutions DAG reprises ne perdent plus la fin des longs résultats d'étapes.** Les preuves d'étapes stockées dans le point de contrôle de reprise après sinistre étaient coupées en tête uniquement au plafond de taille, supprimant silencieusement tout ce qui vivait à la fin de la sortie — c'est exactement là où les conclusions, les sommes de contrôle et les dernières lignes ont tendance à être. Les preuves plafonnées conservent maintenant à la fois la tête et la queue avec un marqueur d'omission explicite entre les deux.
- **Les agents ne "travaillent" plus en imprimant leurs propres notes via Python.** Les conseils d'efficacité qui encourageaient à écrire un script complet par appel d'outil sont maintenant limités aux tâches qui ont réellement besoin d'exécution de code — les questions de connaissance, les comparaisons et l'analyse que le modèle peut produire directement sont répondues directement au lieu d'être acheminées via des scripts sandbox inutiles.
- **Une étape d'agent à l'intérieur d'un flux de travail exécute maintenant l'agent complet, pas seulement son invite.** Le nœud Agent d'un flux de travail s'exécutait autrefois avec seulement les instructions de l'agent sur un modèle générique et un ensemble d'outils par défaut — supprimant silencieusement les connecteurs, les bases de connaissances, les serveurs MCP et le modèle configuré de l'agent. Il exécute maintenant l'agent exactement comme il le ferait dans le chat, avec toutes ses ressources liées et son modèle, et seulement si cet agent est visible au propriétaire du flux de travail.
- **Quitter (ou être retiré de) une organisation révoque maintenant son accès partagé.** Les ressources auxquelles vous ne pouviez accéder que par cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous avez enregistrés pour eux sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après le départ. Les ressources auxquelles vous vous êtes abonné via le Marché ne sont pas affectées.
- **La suppression d'un compte supprime maintenant tout, de manière cohérente.** Les chemins "supprimer l'utilisateur" admin et "supprimer mon compte" en libre-service divergeaient autrefois — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin admin laissait derrière lui les clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux exécutent maintenant une routine qui supprime chaque enregistrement et fichier, et la suppression ne échoue plus sur les bases de données qui appliquent les clés étrangères.
...
- **La suppression d'un agent ne casse plus les conversations passées.** Les conversations qui référençaient l'agent sont conservées comme des enregistrements historiques et ont simplement le lien d'agent effacé, plutôt que de bloquer la suppression.
- **Les actions de connecteur nécessitant une confirmation ne peuvent plus s'exécuter sans surveillance dans un flux de travail.** Une action de connecteur marquée comme nécessitant une confirmation s'exécutait autrefois à l'intérieur d'un flux de travail avec la porte d'approbation silencieusement contournée. L'étape échoue maintenant fermée avec un message pour placer un nœud d'intervention humaine avant (la porte d'approbation native du flux de travail) ou utiliser une action qui ne nécessite pas de confirmation.
- **Les étapes du flux de travail n'empruntent plus les identifiants MCP du propriétaire.** Lorsque le serveur MCP d'un flux de travail avait le repli d'identifiant du propriétaire désactivé, un non-propriétaire exécutant ce flux de travail revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue maintenant avec un message clair pour configurer vos propres identifiants, correspondant au chemin du chat.
...
- **Vous pouvez maintenant attacher un connecteur abonné, une base de connaissances ou un serveur MCP à un agent.** La liaison exigeait autrefois que vous *possédiez* la ressource, donc un connecteur auquel vous vous étiez légitimement abonné (partagé par l'organisation ou installé depuis le Marché) était rejeté comme "non possédé". La liaison suit maintenant la même règle de visibilité que le reste de l'application — tout ce que vous possédez ou auquel vous vous êtes abonné est bindable.
- **Les étapes du flux de travail ne peuvent appeler que les connecteurs auxquels vous avez réellement accès.** Une étape de connecteur de flux de travail s'exécutait autrefois n'importe quel connecteur dont elle référençait l'id, tant que ce connecteur existait ; elle applique maintenant la visibilité du coureur (possédé ou abonné), donc un flux de travail partagé ne peut pas atteindre un connecteur auquel vous n'avez pas accès simplement en référençant son id.
- **La recherche de base de connaissances fonctionne maintenant sur les bases de connaissances auxquelles vous vous êtes abonné, pas seulement celles que vous possédez.** Les données d'une base de connaissances vivent sous le stockage de son propriétaire, mais une étape de base de connaissances de flux de travail (et le chat de base de connaissances sans agent) cherchait autrefois sous le stockage du coureur — donc la recherche d'une base de connaissances partagée avec vous retournait silencieusement rien. Le propriétaire est maintenant résolu à partir de la base de connaissances elle-même et la recherche est vérifiée d'accès (propriétaire ou abonné), donc les bases de connaissances partagées retournent des résultats et les bases de connaissances que vous ne pouvez pas voir retournent rien au lieu d'un coup déroutant vide.
- **Les tâches en arrière-plan ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhook, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches fire-and-forget que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont maintenant conservées jusqu'à complétion (et leurs échecs enregistrés) donc elles se terminent de manière fiable.
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres demandes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les demandes simultanées pendant qu'un gros fichier était analysé. L'extraction est maintenant déchargée sur un thread de travail.
- **Les réponses multi-étapes (DAG) ne déforment plus leurs sources.** Lorsque l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé s'écoulait — donc une source qui énumérait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la gravité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation d'objectif ne pouvait pas l'attraper car elle ne voyait jamais la source originale. Chaque étape conserve maintenant sa sortie d'outil brute comme preuve de source faisant autorité, que la synthèse finale et l'évaluateur vérifient la réponse contre, donc les totaux, les énumérations et les attributs correspondent à ce que les sources ont réellement dit (et la confiance baisse avec une divergence nommée lorsqu'ils ne correspondent pas).
- **Le nœud LLM du flux de travail n'affiche plus les contrôles de modèle qui ne signifiaient pas ce qu'ils impliquaient.** Il avait une liste déroulante de modèle codée en dur dont la sélection était ignorée au moment de l'exécution, plus un bouton bascule de niveau Rapide/Principal exposant un concept interne (auto-dégradant). Les deux sont supprimés — le nœud exécute maintenant toujours votre modèle configuré par le système (Fournisseur de modèle, revenant à `.env`), de la même manière que le playground le résout, avec une note d'une ligne pointant vers où le modèle est configuré.
