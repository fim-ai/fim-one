...
- **Les agents ne "fonctionnent" plus en imprimant leurs propres notes via Python.** Les conseils d'efficacité qui encourageaient à écrire un script complet par appel d'outil sont maintenant limités aux tâches qui nécessitent réellement une exécution de code — les questions de connaissance, les comparaisons et l'analyse que le modèle peut produire directement sont répondues directement au lieu d'être acheminées via des scripts sandbox inutiles.
- **Une étape d'agent à l'intérieur d'un workflow exécute maintenant l'agent complet, pas seulement son prompt.** Le nœud Agent d'un workflow s'exécutait autrefois avec seulement les instructions de l'agent sur un modèle générique et un ensemble d'outils par défaut — supprimant silencieusement les connecteurs, les bases de connaissances, les serveurs MCP et le modèle configuré de l'agent. Il exécute maintenant l'agent exactement comme il le ferait en chat, avec toutes ses ressources liées et son modèle, et seulement si cet agent est visible pour le propriétaire du workflow.
- **Quitter (ou être retiré d') une organisation révoque maintenant son accès partagé.** Les ressources auxquelles vous ne pouviez accéder que via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour elles sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via la Place de marché ne sont pas affectées.
...
- **Vous pouvez maintenant attacher un connecteur abonné, une base de connaissances ou un serveur MCP à un agent.** La liaison exigeait autrefois que vous *possédiez* la ressource, donc un connecteur auquel vous vous étiez légitimement abonné (partagé par l'organisation ou installé depuis la Place de marché) était rejeté comme "non possédé". La liaison suit maintenant la même règle de visibilité que le reste de l'application — tout ce que vous possédez ou auquel vous vous êtes abonné est bindable.
- **Les étapes du workflow ne peuvent appeler que les connecteurs auxquels vous avez réellement accès.** L'étape de connecteur d'un workflow s'exécutait autrefois n'importe quel connecteur dont elle référençait l'id, tant que ce connecteur existait ; elle applique maintenant la visibilité du runner (possédé ou abonné), donc un workflow partagé ne peut pas accéder à un connecteur auquel vous n'avez pas accès en le référençant simplement par son id.
- **La recherche dans la base de connaissances fonctionne maintenant sur les KB auxquelles vous vous êtes abonné, pas seulement celles que vous possédez.** Les données d'une KB vivent sous le stockage de son propriétaire, mais une étape de base de connaissances du workflow (et le chat de base de connaissances sans agent) cherchait autrefois sous votre propre stockage — donc chercher une KB partagée avec vous retournait silencieusement rien. Le propriétaire est maintenant résolu à partir de la KB elle-même et la recherche est vérifiée pour l'accès (propriétaire ou abonné), donc les KB partagées retournent des résultats et les KB que vous ne pouvez pas voir retournent rien au lieu d'un hit vide confus.
- **Les tâches de fond ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches fire-and-forget que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont maintenant conservées jusqu'à la fin (et leurs échecs enregistrés) donc elles se terminent de manière fiable.
