...
- **Les tâches de fond ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches fire-and-forget que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont désormais conservées jusqu'à leur achèvement (et leurs échecs enregistrés) pour qu'elles se terminent de manière fiable.
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres demandes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les demandes concurrentes pendant qu'un gros fichier était analysé. L'extraction est désormais déportée vers un thread de travail.
