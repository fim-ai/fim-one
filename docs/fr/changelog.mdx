...
- **Quitter (ou être retiré d') une organisation révoque désormais son accès partagé.** Les ressources auxquelles vous ne pouviez accéder que par cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour elles sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via le Marché ne sont pas affectées.
- **Supprimer un compte supprime désormais tout, de manière cohérente.** Les chemins d'accès admin « supprimer l'utilisateur » et libre-service « supprimer mon compte » avaient l'habitude de diverger — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin admin laissait derrière lui des clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent maintenant par une seule routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
...
- **Supprimer un agent ne casse plus les conversations passées.** Les conversations qui référençaient l'agent sont conservées comme des enregistrements historiques et ont simplement le lien de l'agent effacé, plutôt que de bloquer la suppression.
- **Les actions de connecteur nécessitant une confirmation ne peuvent plus s'exécuter sans surveillance dans un flux de travail.** Une action de connecteur marquée comme nécessitant une confirmation s'exécutait autrefois à l'intérieur d'un flux de travail avec la porte d'approbation silencieusement contournée. L'étape échoue maintenant en fermé avec un message pour placer un nœud d'intervention humaine avant elle (la porte d'approbation native du flux de travail) ou utiliser une action qui ne nécessite pas de confirmation.
- **Les étapes du flux de travail n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un flux de travail avait le secours des identifiants du propriétaire désactivé, un non-propriétaire exécutant ce flux de travail revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue maintenant avec un message clair pour configurer vos propres identifiants, correspondant au chemin de chat.
- **Les appels de connecteur effectués à partir de flux de travail sont maintenant enregistrés dans l'audit.** Une action de connecteur exécutée à l'intérieur d'un flux de travail ne laissait aucune entrée dans le journal d'appels de connecteur (seuls les appels initiés par chat étaient enregistrés) ; les appels de flux de travail écrivent maintenant la même piste d'audit.
...
- **Les étapes du flux de travail ne peuvent appeler que les connecteurs auxquels vous avez réellement accès.** Une étape de connecteur de flux de travail s'exécutait autrefois n'importe quel connecteur dont elle référençait l'id, tant que ce connecteur existait ; elle applique maintenant la visibilité du lanceur (possédé ou abonné), donc un flux de travail partagé ne peut pas atteindre un connecteur auquel vous n'avez pas accès en le référençant simplement par son id.
- **La recherche dans la base de connaissances fonctionne maintenant sur les bases auxquelles vous vous êtes abonné, pas seulement celles que vous possédez.** Les données d'une base de connaissances vivent sous le stockage de son propriétaire, mais une étape de base de connaissances de flux de travail (et un chat de base de connaissances sans agent) cherchait autrefois sous votre propre stockage — donc chercher une base partagée avec vous retournait silencieusement rien. Le propriétaire est maintenant résolu à partir de la base elle-même et la recherche est vérifiée pour l'accès (propriétaire ou abonné), donc les bases partagées retournent des résultats et les bases que vous ne pouvez pas voir retournent rien au lieu d'un résultat vide déroutant.
- **Les tâches de fond ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches « fire-and-forget » que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont maintenant conservées jusqu'à l'achèvement (et leurs défaillances enregistrées) pour qu'elles se terminent de manière fiable.
...
- **Les réponses multi-étapes (DAG) ne déforment plus leurs sources.** Quand l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé s'écoulait vers l'avant — donc une source qui listait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la sévérité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation des objectifs ne pouvait pas l'attraper parce qu'elle ne voyait jamais la source originale. Chaque étape conserve maintenant sa sortie d'outil brute comme preuve de source faisant autorité, que la synthèse finale et l'évaluateur vérifient par rapport à la réponse, donc les totaux, énumérations et attributs correspondent à ce que les sources ont réellement dit (et la confiance baisse avec une divergence nommée quand ils ne correspondent pas).
- **Le nœud LLM du flux de travail n'affiche plus les contrôles de modèle qui ne signifiaient pas ce qu'ils impliquaient.** Il avait une liste déroulante de modèle codée en dur dont la sélection était ignorée au moment de l'exécution, plus un bouton bascule Fast/Main exposant un concept interne (auto-dégradant). Les deux sont supprimés — le nœud exécute maintenant toujours votre modèle configuré par le système (Model Provider, en revenant à `.env`), de la même manière que le playground le résout, avec une note d'une ligne pointant vers l'endroit où le modèle est configuré.
