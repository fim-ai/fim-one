...
- **L'animation suit maintenant le paramètre système « réduire les animations ». ** Le dégradé flottant de l'arrière-plan de connexion, l'entrée du héros, le texte du message injecté respirant et le curseur de streaming continuaient à se déplacer indépendamment de la préférence du système d'exploitation. Ils restent maintenant immobiles, tandis que les indicateurs signalant un travail en cours restent visibles pour que l'interface ne paraisse jamais figée.

- **Le sélecteur de mode d'exécution reste fermé pendant qu'un tour s'exécute.** Le chip était grisé mais ouvrait toujours son menu au clic, contrairement au sélecteur d'agent à côté de lui ; les deux sont maintenant inactifs jusqu'à la fin de l'exécution.

- **Le raisonnement en direct fonctionne à nouveau sur les modèles plus récents.** Le raisonnement en direct était câblé à partir d'une liste codée en dur de noms de modèles, donc tout ce qui a été publié après la rédaction de cette liste affichait un spinner puis déversait le bloc de raisonnement entier à la fois. Tout modèle qui émet un raisonnement le diffuse maintenant en continu.

- **La vue de chat suit la sortie rapide et arrive à la réponse.** Elle ne s'arrête plus au milieu du raisonnement et ne signale plus faussement du nouveau contenu en dessous, elle suit le contenu qui s'agrandit sans nouveaux événements, et quand un tour se termine, elle se stabilise en haut du résultat au lieu du bas. S'éloigner en faisant défiler rend toujours le contrôle, et revenir au bas reprend le suivi.

- **La connexion applique immédiatement la langue préférée du compte.** Le cookie de langue est maintenant synchronisé lors de la connexion et du rappel OAuth, et la page d'accueil est rechargée à neuf quand la langue du compte diffère de celle dans laquelle la page de connexion a été rendue, de sorte que la destination n'apparaît plus dans la langue précédente jusqu'au prochain rechargement.
...
- **La reconnexion à un flux ne rejoue plus la sortie que vous avez déjà vue.** Le flux en direct porte maintenant le même curseur de position que le endpoint de reprise, donc après une connexion interrompue seuls les événements manqués sont rejoués au lieu de tout le tour.

- **Les événements diffusés divisés entre les chunks réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux morceaux ; les deux chemins de flux partagent maintenant un analyseur qui gère les limites de chunks arbitraires.

- **L'appel d'un autre agent retourne sa réponse réelle.** La réponse du sous-agent arrivait autrefois enterrée dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; elle reçoit maintenant juste la réponse.

- **Un outil suspendu ne peut plus bloquer une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps réel (180 s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de bloquer le tour.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après la compaction.** Le résumé qui condense les tours antérieurs était silencieusement supprimé à la prochaine exécution ; il est maintenant porté en avant, de sorte que l'agent conserve ce qui a été discuté avant.

- **Les résultats d'outils volumineux conservent leur fin ainsi que leur début.** Quand un long résultat est coupé pour tenir dans la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande importe généralement le plus), la coupure tient correctement compte du texte non-latin, et les résultats ultérieurs se voient garantir une fenêtre minimale au lieu d'être complètement effacés.
...
- **Un objectif que l'agent est sûr d'avoir manqué est maintenant réessayé au lieu de terminer l'exécution.** La re-planification s'arrêtait chaque fois que l'analyseur était confiant, ce qui lisait un « le livrable est manquant » confiant comme une raison d'abandonner — exactement le cas pour lequel la re-planification existe. Les exécutions s'arrêtent maintenant tôt seulement quand l'objectif est jugé véritablement inatteignable, et `DAG_MAX_REPLAN_ROUNDS` limite le reste.
