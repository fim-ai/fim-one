...
- **Les outils lents peuvent désormais s'exécuter en arrière-plan sans bloquer l'agent.** L'exécution de code en sandbox (Python/Shell/Node) offre une option `run_in_background` : l'agent reçoit immédiatement un identifiant de tâche, continue à travailler sur d'autres étapes, et reçoit le résultat sous forme de notification quand l'outil se termine. Si l'agent essaie de finaliser alors que des tâches sont encore en cours, il les attend (avec une limite) et intègre leurs résultats avant de répondre.
- **L'historique des conversations est capturé avant la compaction, et les résultats d'outils tronqués par le budget de tokens sont sauvegardés dans l'espace de travail.** Avant que la compaction de longue conversation remplace l'historique par un résumé, la transcription complète est écrite dans l'espace de travail de la conversation sous forme de fichier JSONL ; de même, un résultat d'outil coupé par le budget de tokens agrégé est sauvegardé en intégralité dans l'espace de travail avec un pointeur laissé en place — dans les deux cas, l'agent (et vous) pouvez relire l'original au lieu de le perdre.
- **Les réponses ne s'arrêtent plus au milieu d'une phrase quand le modèle atteint sa limite de sortie.** Quand une réponse est coupée par le plafond de tokens de sortie du fournisseur, l'agent demande automatiquement au modèle de reprendre où il s'est arrêté (jusqu'à 3 fois) et assemble les segments en une réponse fluide — à la fois dans la boucle de raisonnement et dans la réponse finale en streaming.
- **Les appels LLM réessaient désormais en cas de surcharge du fournisseur (HTTP 529) et de délai d'attente de la passerelle (504), et respectent `Retry-After`.** Auparavant, un 529 d'Anthropic échouait immédiatement à moins qu'un modèle de secours soit configuré ; maintenant il recule et réessaie comme 429/5xx, en attendant exactement aussi longtemps que le fournisseur le demande quand un en-tête `Retry-After` est présent.
