...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en cours de tour.** La favicon affiche maintenant un glyphe d'activité stable pendant toute l'exécution et restaure de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application.

- **Les agents ne récupèrent plus un endpoint des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à frapper le même endpoint avec des paramètres variables est averti puis arrêté définitivement.

- **Une exécution plantée rapporte maintenant une erreur au lieu d'un « terminé » vert.** Quand l'agent s'est arrêté à cause d'une défaillance interne, le texte d'exception brut était livré comme réponse sous une coche de succès ; les défaillances passent maintenant par le canal d'erreur et s'affichent comme une notification d'erreur.

- **La reconnexion à un flux ne rejoue plus la sortie que vous avez déjà vue.** Le flux en direct porte maintenant le même curseur de position que l'endpoint de reprise, donc après une connexion perdue seuls les événements manqués sont rejoués au lieu de tout le tour.

- **Les événements en flux divisés entre les chunks réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux parties ; les deux chemins de flux partagent maintenant un analyseur qui gère les limites de chunk arbitraires.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après compaction.** Le résumé qui condense les tours plus anciens était silencieusement supprimé à la prochaine exécution ; il est maintenant reporté, donc l'agent garde ce qui a été discuté avant.

- **Les résultats d'outils volumineux conservent leur fin ainsi que leur début.** Quand un long résultat est tronqué pour tenir dans la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande importe généralement le plus), la troncature tient correctement compte du texte non-latin, et les résultats ultérieurs se voient garantir une fenêtre minimale au lieu d'être complètement effacés.
...
- **Un appel d'outil coupé par la limite de sortie du modèle ne passe plus comme travail terminé.** Quand une charge utile longue (écrire une page HTML complète, par exemple) épuisait le budget de sortie à mi-appel, l'appel à moitié écrit était silencieusement supprimé et le texte d'introduction de l'agent — « maintenant je vais enregistrer le fichier » — devenait le résultat de l'étape, donc une étape qui ne produisait rien était rapportée comme complétée. La troncature est maintenant détectée, et l'agent est invité à réessayer avec le travail divisé en appels plus petits.
