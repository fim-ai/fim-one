...
- **Quitter (ou être retiré d'une) organisation révoque désormais l'accès partagé.** Les ressources auxquelles vous pouviez accéder uniquement via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour elles sont supprimés — vous ne pouvez donc plus utiliser ces ressources ni revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via la Marketplace ne sont pas affectées.
- **Supprimer un compte supprime désormais tout, de manière cohérente.** Les chemins d'accès admin « supprimer l'utilisateur » et libre-service « supprimer mon compte » divergeaient auparavant — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, chacun en manquait donc silencieusement certains), et le chemin admin laissait derrière lui des clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent désormais par une seule routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
...
- **Les étapes de workflow n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un workflow avait le repli sur identifiants du propriétaire désactivé, un non-propriétaire exécutant ce workflow revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue désormais avec un message clair pour configurer vos propres identifiants, correspondant au chemin de chat.
- **Les appels de connecteur effectués à partir de workflows sont désormais enregistrés dans l'audit.** Une action de connecteur exécutée à l'intérieur d'un workflow ne laissait aucune entrée dans le journal d'appels de connecteur (seuls les appels initiés par chat étaient enregistrés) ; les appels de workflow écrivent désormais la même piste d'audit.
...
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres demandes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les demandes concurrentes pendant qu'un gros fichier était analysé. L'extraction est désormais déchargée sur un thread de travail.
- **Les réponses multi-étapes (DAG) ne déforment plus leurs sources.** Quand l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé progressait — donc une source qui énumérait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la gravité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation d'objectif ne pouvait pas l'attraper car elle ne voyait jamais la source originale. Chaque étape conserve désormais sa sortie d'outil brute comme preuve de source faisant autorité, que la synthèse finale et l'évaluateur vérifient par rapport à la réponse, donc les totaux, énumérations et attributs correspondent à ce que les sources ont réellement dit (et la confiance baisse avec une discordance nommée quand ils ne correspondent pas).
