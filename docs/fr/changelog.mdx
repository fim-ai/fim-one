...
- **L'agent ne génère plus d'image parasite lorsqu'on lui demande de remodeler un livrable HTML, et ne prétend jamais qu'un fichier a été créé sans l'avoir réellement écrit dans l'espace de travail.** La génération d'images est désormais explicitement limitée aux images raster, et les livrables de fichiers doivent être écrits via un appel d'outil — coller du contenu dans le chat ne compte plus comme « générer un fichier ».
- **Les pages HTML générées utilisent désormais par défaut une palette chaleureuse inspirée de Claude au lieu de l'apparence générique en dégradé violet.** Quand vous ne spécifiez pas de style, l'agent utilise une ligne de base minimaliste et chaleureuse ; votre propre style prime toujours quand il est spécifié.
- **Les conversations longues et gourmandes en outils ne se cassent plus quand l'historique est compacté.** La compaction de contexte pouvait couper la fenêtre conservée entre un appel d'outil et ses résultats, produisant une séquence de messages que le fournisseur LLM rejette carrément (HTTP 400) — exactement sur les longs passages qui ont le plus besoin de compaction. Le compacteur conserve désormais chaque appel d'outil avec ses résultats du côté conservé.
- **Les résultats d'outils en arrière-plan apparaissent désormais dans le flux d'étapes et dans les preuves DAG.** Une exécution d'outil en arrière-plan livrait autrefois sa sortie réelle uniquement dans le message de notification interne de l'agent — la chronologie des étapes de l'interface utilisateur et les preuves de vérification de réponse DAG ne voyaient que l'espace réservé de dispatch. Le résultat drainé est désormais émis comme un événement normal d'achèvement d'outil sous le vrai nom d'outil.
- **Un rare résultat de planificateur avec des identifiants d'étape en double ne casse plus tout le tour DAG.** Les identifiants en double étaient autrefois mal diagnostiqués comme une erreur de « dépendance circulaire » ; ils sont désormais renommés de manière déterministe et le plan procède.
- **Plusieurs corrections de justesse de la boucle ReAct.** Un outil retournant une sortie vide conservait sa note « complété sans sortie » au lieu d'un résultat vide quand l'espace de travail est activé ; les étapes DAG bloquées par une dépendance insatisfaisable émettent désormais un événement terminal approprié au lieu de laisser l'interface utilisateur suspendue ; et pour les modèles sans appel d'outil natif, la synthèse de réponse de secours voit désormais les résultats d'outil, pas seulement les appels d'outil.
- **L'espace de travail de conversation est désormais réellement câblé dans le chat — les résultats d'outils surdimensionnés sont récupérables au lieu d'être silencieusement perdus.** Les chemins de récupération d'espace de travail de la couche agent (déchargement de sortie volumineuse avec un pointeur `workspace://`, sauvetage en copie complète des résultats d'outils tronqués par budget, instantanés de transcription pré-compaction) existaient mais les points de terminaison de chat ne créaient jamais d'espace de travail, donc aucun d'eux ne s'exécutait : une fois le budget de résultat d'outil épuisé, le modèle recevait des observations vides sans aucun moyen de relire les données, et répondait par conjecture. Les agents ReAct et DAG-step obtiennent désormais un espace de travail par conversation, et la suppression d'une conversation nettoie ses fichiers d'espace de travail et tout point de contrôle de reprise d'incident restant.
- **Les exécutions DAG reprises ne perdent plus la fin des longues sorties d'étapes.** Les preuves d'étapes stockées dans le point de contrôle de reprise d'incident étaient coupées en tête uniquement au plafond de taille, supprimant silencieusement tout ce qui vivait à la fin de la sortie — c'est exactement où les conclusions, les sommes de contrôle et les lignes finales ont tendance à être. Les preuves plafonnées conservent désormais à la fois la tête et la queue avec un marqueur d'omission explicite entre les deux.
- **Les agents ne « travaillent » plus en imprimant leurs propres notes via Python.** Les conseils d'efficacité qui encourageaient l'écriture d'un script complet par appel d'outil sont désormais limités aux tâches qui ont réellement besoin d'exécution de code — les questions de connaissance, les comparaisons et l'analyse que le modèle peut produire directement sont répondues directement au lieu d'être acheminées via des scripts sandbox inutiles.
- **Une étape d'agent à l'intérieur d'un flux de travail exécute désormais l'agent complet, pas seulement son invite.** Le nœud Agent d'un flux de travail s'exécutait autrefois avec seulement les instructions de l'agent sur un modèle générique et un ensemble d'outils par défaut — supprimant silencieusement les connecteurs, les bases de connaissances, les serveurs MCP et le modèle configuré de l'agent. Il exécute désormais l'agent exactement comme il le ferait dans le chat, avec toutes ses ressources liées et son modèle, et seulement si cet agent est visible pour le propriétaire du flux de travail.
- **Quitter (ou être retiré de) une organisation révoque désormais son accès partagé.** Les ressources que vous pouviez atteindre uniquement via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous avez enregistrés pour eux sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via le Marché ne sont pas affectées.
- **La suppression d'un compte supprime désormais tout, de manière cohérente.** Les chemins « supprimer l'utilisateur » admin et « supprimer mon compte » en libre-service divergeaient autrefois — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichier, donc chacun en manquait silencieusement certains), et le chemin admin laissait derrière lui les clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent désormais par une routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
- **Un utilisateur qui possède toujours une organisation ne peut plus être supprimé.** La demande est désormais rejetée avec un message clair pour transférer la propriété (ou supprimer l'organisation) d'abord, au lieu d'échouer de manière obscure ou de laisser l'organisation orpheline.
...
- **Les actions de connecteur nécessitant une confirmation ne peuvent plus s'exécuter sans surveillance dans un flux de travail.** Une action de connecteur marquée comme nécessitant une confirmation s'exécutait autrefois à l'intérieur d'un flux de travail avec la porte d'approbation silencieusement contournée. L'étape échoue désormais de manière fermée avec un message pour placer un nœud d'intervention humaine avant celui-ci (la porte d'approbation native du flux de travail) ou utiliser une action qui ne nécessite pas de confirmation.
- **Les étapes de flux de travail n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un flux de travail avait le repli d'identifiant du propriétaire désactivé, un non-propriétaire exécutant ce flux de travail revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue désormais avec un message clair pour configurer vos propres identifiants, correspondant au chemin du chat.
- **Les appels de connecteur effectués à partir de flux de travail sont désormais enregistrés dans l'audit.** Une action de connecteur exécutée à l'intérieur d'un flux de travail ne laissait aucune entrée dans le journal des appels de connecteur (seuls les appels initiés par le chat étaient enregistrés) ; les appels de flux de travail écrivent désormais la même piste d'audit.
- **Vous pouvez désormais attacher un connecteur abonné, une base de connaissances ou un serveur MCP à un agent.** La liaison exigeait autrefois que vous *possédiez* la ressource, donc un connecteur auquel vous vous étiez légitimement abonné (partagé par l'organisation ou installé via le Marché) était rejeté comme « non possédé ». La liaison suit désormais la même règle de visibilité que le reste de l'application — tout ce que vous possédez ou auquel vous vous êtes abonné est bindable.
- **Les étapes de flux de travail ne peuvent appeler que les connecteurs auxquels vous avez réellement accès.** Une étape de connecteur de flux de travail s'exécutait autrefois n'importe quel connecteur dont elle référençait l'id, tant que ce connecteur existait ; elle applique désormais la visibilité du lanceur (possédé ou abonné), donc un flux de travail partagé ne peut pas atteindre un connecteur auquel vous n'avez pas accès en le référençant simplement par son id.
- **La recherche de base de connaissances fonctionne désormais sur les bases de connaissances auxquelles vous vous êtes abonné, pas seulement celles que vous possédez.** Les données d'une base de connaissances vivent sous le stockage de son propriétaire, mais une étape de base de connaissances de flux de travail (et une base de connaissances de chat sans agent) cherchait autrefois sous votre propre stockage — donc la recherche d'une base de connaissances partagée avec vous retournait silencieusement rien. Le propriétaire est désormais résolu à partir de la base de connaissances elle-même et la recherche est vérifiée pour l'accès (propriétaire ou abonné), donc les bases de connaissances partagées retournent des résultats et les bases de connaissances que vous ne pouvez pas voir retournent rien au lieu d'un résultat vide confus.
- **Les travaux en arrière-plan ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches fire-and-forget que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont désormais conservées jusqu'à achèvement (et leurs défaillances enregistrées) donc elles se terminent de manière fiable.
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres demandes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les demandes concurrentes pendant qu'un gros fichier était analysé. L'extraction est désormais déchargée vers un thread de travail.
- **Les réponses multi-étapes (DAG) ne misreprésentent plus leurs sources.** Quand l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé s'écoulait — donc une source qui listait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la sévérité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation d'objectif ne pouvait pas le détecter car elle ne voyait jamais la source originale. Chaque étape conserve désormais sa sortie d'outil brute comme preuve de source faisant autorité, que la synthèse finale et l'évaluateur vérifient par rapport à la réponse, donc les totaux, les énumérations et les attributs correspondent à ce que les sources ont réellement dit (et la confiance baisse avec une discordance nommée quand ils ne correspondent pas).
- **Le nœud LLM du flux de travail n'affiche plus les contrôles de modèle qui ne signifiaient pas ce qu'ils impliquaient.** Il avait une liste déroulante de modèle codée en dur dont la sélection était ignorée au moment de l'exécution, plus un curseur de niveau Rapide/Principal exposant un concept interne (auto-dégradant). Les deux sont supprimés — le nœud exécute désormais toujours votre modèle configuré par le système (Fournisseur de modèle, revenant à `.env`), de la même manière que le playground le résout, avec une note d'une ligne pointant vers où le modèle est configuré.
