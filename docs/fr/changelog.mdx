...
- **Les tâches de fond ne disparaissent plus aléatoirement.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches fire-and-forget que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont maintenant conservées jusqu'à leur achèvement (et leurs défaillances enregistrées) pour qu'elles se terminent de manière fiable.
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres requêtes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les requêtes concurrentes pendant qu'un gros fichier était analysé. L'extraction est maintenant déléguée à un thread worker.
- **Les réponses multi-étapes (DAG) ne déforment plus leurs sources.** Quand l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé progressait — donc une source qui listait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la sévérité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation d'objectif ne pouvait pas le détecter car elle ne voyait jamais la source originale. Chaque étape conserve maintenant sa sortie d'outil brute comme preuve de source faisant autorité, que la synthèse finale et l'évaluateur vérifient par rapport à la réponse, donc les totaux, énumérations et attributs correspondent à ce que les sources ont réellement dit (et la confiance diminue avec une discordance nommée quand ce n'est pas le cas).
