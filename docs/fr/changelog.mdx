...
- **Les agents GPT-5.x conservent leur raisonnement entre les appels d'outils.** FIM One communique désormais directement avec l'API Responses d'OpenAI pour les modèles GPT-5.x et transporte le raisonnement chiffré de chaque tour au suivant, de sorte qu'un agent multi-étapes s'appuie sur ce qu'il a déjà élaboré au lieu de le redériver à chaque tour d'outils. Les décomptes de jetons de raisonnement apparaissent dans l'utilisation, et le panneau de réflexion affiche le résumé généré par le modèle. Définissez `FIM_GPT5_RESPONSES_MODE=bridge` (ou `off`) pour revenir au comportement précédent ; les points de terminaison sans route `/v1/responses` basculent automatiquement vers les complétions de chat.

- **Les étapes de transformation pure dans les plans DAG s'exécutent comme un seul appel LLM direct.** Le planificateur étiquète les étapes de fusion/réécriture/résumé qui ne consomment que les sorties en amont comme des étapes directes, contournant la boucle d'outils qu'elles n'utiliseraient jamais, ce qui réduit le coût et la latence ; chaque résultat d'étape enregistre désormais aussi comment il s'est exécuté (appel direct ou boucle d'agent, itérations, outils utilisés).

- **Les agents peuvent poser des questions de clarification à choix multiples en cours d'exécution.** Quand une tâche pourrait prendre des directions matériellement différentes, l'agent fait une pause et présente jusqu'à quatre questions structurées dans le chat, chacune avec 2-4 options plus un champ texte libre « Autre » ; répondre reprend l'exécution sur place, et ignorer ou un délai d'expiration lui permet de continuer selon son propre jugement.

- **Les étapes d'agent portent des titres d'une ligne en direct.** Pendant qu'un agent travaille, chaque itération reçoit une étiquette générée courte (« Vérification des données de tarification… »), affichée dans un seul en-tête replié qui bascule vers l'étape la plus récente ; le développer révèle la chronologie complète des étapes, et les étiquettes persistent dans l'historique de conversation.

- **Les modèles d'un fournisseur peuvent être supprimés en masse.** Chaque ligne de modèle dans Admin → Models porte une case à cocher, l'en-tête sélectionne toute la liste, et maintenir Maj enfoncé tout en cliquant étend la sélection sur une plage de lignes. Supprimer la sélection supprime chaque modèle choisi en une seule requête, après une confirmation indiquant le nombre.

- **Les petits tableaux de comparaison dans les réponses de chat s'affichent désormais sous forme de mises en page de cartes.** Un tableau de 2-3 colonnes dont la première colonne contient des étiquettes d'attributs affiche chaque attribut à l'intérieur de sa cellule de valeur (correspondant au style de carte de comparaison popularisé par les interfaces de chat IA) ; les tableaux plus grands conservent la grille classique.
...
- **Les citations indiquent le document dont elles proviennent.** Un marqueur de citation s'affiche désormais sous forme de pastille portant le nom du document source, avec les marqueurs adjacents réduits en une pastille et un décompte ; le survol liste chaque source derrière.
...
- **Les diagrammes et blocs de code peuvent être téléchargés, pas seulement copiés.** Les diagrammes Mermaid exportent le SVG rendu, et chaque bloc de code se télécharge avec un nom de fichier correspondant à son langage ; un bloc `csv` se télécharge sous forme de fichier UTF-8 qui s'ouvre correctement dans Excel.

### Corrections

- **Les puces de suggestion de suivi restent dans la voix de l'utilisateur.** Les questions suggérées suivantes sont formulées comme quelque chose que l'utilisateur enverrait (demandes ou questions à l'assistant), et non comme des sondages de préférence inversés du type « lequel préférez-vous… ? ».

- **La documentation Exa n'appelle plus Exa le fournisseur de recherche par défaut.** Les pages d'intégration et de variables d'environnement décrivent désormais Exa comme un backend optionnel que vous activez avec `WEB_SEARCH_PROVIDER=exa` ; le chemin de recherche par défaut reste Jina.
