...
- **L'envoi d'un message dans un chat diffuse maintenant instantanément au lieu de bloquer.** Chaque envoi bloquait auparavant sur un rechargement complet de l'historique de la conversation avant que la réponse puisse commencer, ce qui rendait les chats plus longs figés pendant un moment et rechargait la transcription entière à chaque tour. Un tour terminé est maintenant intégré à l'historique en arrière-plan, de sorte que les envois diffusent immédiatement et ignorent le rechargement redondant.
- **Le raisonnement étendu fonctionne maintenant sur les modèles Anthropic plus récents (Opus 4.6+, Sonnet 4.6, Fable 5).** Ces modèles ont supprimé l'ancien paramètre de raisonnement à budget fixe — Opus 4.7/4.8 et Fable 5 le rejettent avec un 400 — de sorte que la configuration de l'effort de raisonnement générait une erreur ou ne faisait rien silencieusement. Nous envoyons maintenant le protocole de raisonnement adaptatif pour ces modèles (et supprimons les paramètres d'échantillonnage qu'ils n'acceptent plus), de sorte que l'effort de raisonnement prend effet correctement. Un modèle qui utilise le raisonnement adaptatif mais est connecté via un proxy OpenAI-compatible générique (où le raisonnement ne peut pas être activé) enregistre maintenant un avertissement clair au lieu d'échouer silencieusement — configurez-le en tant que fournisseur Anthropic pour activer le raisonnement.
- **La déconnexion forcée compare maintenant les horodatages correctement sur PostgreSQL.** La vérification d'invalidation de session supposait que l'heure d'invalidation stockée était UTC naïf et l'a forcée comme telle, ce qui a silencieusement corrompu les comparaisons par rapport aux valeurs non-UTC conscientes du fuseau horaire que PostgreSQL peut retourner. Les horodatages sont maintenant normalisés en UTC par conversion, non par remplacement, de sorte que les jetons révoqués sont fiablement rejetés sur chaque backend.
- **Les hooks d'application échouent maintenant en position fermée lorsqu'ils s'arrêtent.** Un hook `PRE_TOOL_USE` (par exemple une porte d'approbation) qui levait une exception retournait auparavant `allow=True`, de sorte que l'appel d'outil bloqué s'exécutait quand même — une porte qui s'arrête silencieusement devenait un contournement. Les hooks PRE nient maintenant par défaut en cas d'erreur ; les hooks non-application (limiteur de débit, enregistreurs) conservent l'ancien comportement d'échec ouvert via un nouveau drapeau `fail_open`.
