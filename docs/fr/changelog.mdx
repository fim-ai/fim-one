...
- **L'historique de conversation est capturé avant la compaction, et les résultats d'outils tronqués par le budget sont sauvegardés dans l'espace de travail.** Avant que la compaction de longue conversation remplace l'historique par un résumé, la transcription complète est écrite dans l'espace de travail de la conversation en tant que fichier JSONL ; de même, un résultat d'outil coupé par le budget de jetons agrégé est sauvegardé en intégralité dans l'espace de travail avec un pointeur laissé en place — dans les deux cas l'agent (et vous) peut relire l'original au lieu de le perdre.
- **Les réponses ne s'arrêtent plus au milieu d'une phrase quand le modèle atteint sa limite de sortie.** Quand une réponse est coupée par le plafond de jetons de sortie du fournisseur, l'agent demande automatiquement au modèle de reprendre où il s'est arrêté (jusqu'à 3 tours) et assemble les segments en une réponse transparente — à la fois dans la boucle de raisonnement et dans la réponse finale en flux continu.
- **Les appels LLM réessaient maintenant en cas de surcharge du fournisseur (HTTP 529) et délai d'expiration de la passerelle (504), et respectent `Retry-After`.** Auparavant, un 529 d'Anthropic échouait immédiatement à moins qu'un modèle de secours ne soit configuré ; maintenant il recule et réessaye comme 429/5xx, en attendant exactement aussi longtemps que le fournisseur le demande quand un en-tête `Retry-After` est présent.
- **Les agents conservent maintenant un plan visible lors de tâches multi-étapes.** Les agents ReAct obtiennent un outil `update_plan` pour noter leur plan sous forme de liste de contrôle et le mettre à jour au fur et à mesure de leur progression ; si le plan devient obsolète après plusieurs appels d'outils, la liste de contrôle complète est réinjectée comme rappel — ainsi les tâches longues restent sur la bonne voie même après la compaction du contexte. Activé par défaut (`REACT_PLAN_TOOL_ENABLED`) ; les sous-étapes DAG et les agents sans outils l'ignorent.

- **Les agents peuvent maintenant exécuter un workflow au milieu d'une conversation.** Les workflows actifs sans approbation humaine que vous possédez ou auxquels vous êtes abonné sont proposés à l'agent en tant qu'outil `run_workflow` — dans le chat normal et avec un agent sélectionné — ainsi une tâche peut confier un sous-processus déterministe et reproductible à un workflow et récupérer ses résultats. Chaque exécution est enregistrée dans l'historique des exécutions du workflow et son utilisation de jetons compte par rapport au quota du propriétaire du workflow (l'exécution est refusée si le propriétaire a déjà dépassé le quota). Les workflows avec une étape d'intervention humaine restent déclenchement uniquement (depuis la page Workflows ou leur clé API), car ils peuvent bloquer longtemps.
- **Deux nouveaux modèles de démarrage de workflow : Connector Action et MCP Tool Call.** La galerie « Créer à partir d'un modèle » présente maintenant les nœuds de connecteur et MCP — appelez un système externe ou un outil MCP, puis résumez le résultat avec un LLM. Choisissez le connecteur/serveur spécifique après la création.
