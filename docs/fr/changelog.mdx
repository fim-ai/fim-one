...
- **Le graphique Star History du README s'affiche à nouveau.** L'intégration en direct `api.star-history.com` expire après que GitHub ait restreint l'API stargazers (juil. 2026) ; le graphique est maintenant un snapshot SVG statique dans le dépôt (actualisez avec `python3 scripts/gen_star_history.py`).
