...
- **Les réponses finales se diffusent au fur et à mesure qu'elles sont écrites.** L'agent signale maintenant quand il a terminé la collecte et écrit la réponse sous forme de tour diffusé en direct, de sorte que les réponses longues arrivent jeton par jeton avec tous les détails au lieu d'être rejouées après une attente. `REACT_FINISH_SIGNAL=false` restaure le comportement précédent.

- **Les docs d'architecture mappent l'ingénierie de graphe sur FIM One.** [Planning Landscape](/architecture/planning-landscape) explique l'ingénierie en boucle vs graphe, comment cela diffère des workflows visuels classiques, comment cela se mappe à Workflow / DAG / ReAct, et ce qu'il faut emprunter pour chaque moteur. Elle indique également le fait actuel que chaque étape DAG est déjà un agent ReAct complet (pas un appel LLM unique). Les pages des moteurs ReAct et DAG y renvoient.
...
- **Un message nouvellement envoyé démarre en haut de l'écran.** La transcription conserve juste assez d'espace sous le tour actuel pour que sa première ligne atteigne le haut, de sorte que la question reste visible et la réponse se développe dans l'espace ci-dessous au lieu que le tour entier se colle au bord inférieur. L'espace disparaît une fois que le tour dépasse un écran.

- **Les blocs de raisonnement restent repliés pendant qu'un tour s'exécute.** Le raisonnement dans la sortie ReAct et DAG affiche maintenant un aperçu d'une ligne par défaut au lieu de se développer au fur et à mesure du streaming, gardant le résultat en vue ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement composés pour le chinois.** L'export PDF intègre maintenant une véritable police CJK au lieu d'en référencer une, corrigeant l'espacement effondré dans le texte anglais et les puces qui s'affichaient comme des caractères chinois non liés, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est de sorte que le texte gras mixte chinois/anglais ne sort plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les en-têtes à l'intérieur d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent sur le plan lors de tâches longues.** L'agent est maintenant rappelé de changer d'approche quand il répète le même outil sans succès, incité à écrire un plan quand il travaille sans, et ne peut plus fournir une réponse finale tant que les éléments du plan sont toujours ouverts sans une passe de vérification explicite.

- **Les réponses diffusées s'atténuent au lieu d'apparaître soudainement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du streaming, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en évidence la ligne sous le curseur, et les puces de liste se placent derrière le texte au lieu de rivaliser avec lui.

- **Les actions de bloc restent hors de vue jusqu'à ce que vous les atteigniez.** Les boutons de copie et d'export sur les réponses, les blocs de code, les diagrammes et les tableaux sont révélés au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale garde maintenant la conversation ensemble.** La liste de conversations se trouve directement sous Nouveau chat et Recherche, et la navigation du module s'est déplacée dans un dock compact teinté au-dessus de la zone utilisateur, donnant plus d'espace à la liste et à la barre latérale une hiérarchie claire de haut en bas.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de se diffuser en direct pendant qu'il est généré, puis se replie quand l'étape se termine.

- **Les pages de liste atténuent progressivement leurs cartes en place.** Les agents, connecteurs, bases de connaissances, compétences, workflows, outils, conversations, artefacts et le marché échelonnent leurs cartes au premier chargement au lieu d'apparaître tous à la fois. Le filtrage et la pagination restent instantanés, de sorte que la saisie dans une zone de recherche ne rejamais l'entrée.
...
- **Les demandes « Ask-first » se terminent en un tour en mode Planner.** Un objectif qui demande explicitement à l'agent de clarifier les exigences avec des questions à choix multiples avant de livrer (« 问清楚我再出方案 ») produit maintenant un questionnaire propre comme résultat du tour, au lieu d'étapes de planification qui attendent des réponses, boucles à travers des re-plans et se terminent par « objectif non atteint » plus des listes de questions répétées. Le mode Auto achemine ces demandes vers le mode Standard.

- **Les puces de suivi ne tirent plus la transcription au-delà de la réponse.** Après qu'un tour se gare sur le résultat, la croissance ultérieure de la mise en page (suggestions, titre) reste en place, et plier le tour en direct dans l'historique se re-gare à la tête de la réponse pour la même conversation au lieu de sauter au bas.

- **L'ouverture d'une conversation existante atterrit sur les derniers messages.** L'actualisation d'une URL de chat fait maintenant défiler vers le bas comme le changement depuis la barre latérale, au lieu de commencer en haut du fil.

- **Les réponses rapides ne semblent plus bloquées sur « Traitement en cours ».** Un tour terminé sans titre d'étape généré affiche l'étiquette du processus de réflexion au lieu du texte de shimmer de traitement en direct.

- **L'animation suit maintenant le paramètre « réduire le mouvement » du système.** Le dégradé dérivant du fond de connexion, l'entrée du héros, le texte du message injecté respirant et le curseur de diffusion en continu continuaient à se déplacer quel que soit le paramètre de préférence du système d'exploitation. Ils restent maintenant immobiles, tandis que les indicateurs qui signalent le travail en cours restent visibles pour que l'interface ne semble jamais bloquée.
...
- **Les flux de raisonnement sont à nouveau en direct sur les modèles plus récents.** Le raisonnement en direct était câblé à partir d'une liste codée en dur de noms de modèles, donc tout ce qui a été publié après l'écriture de cette liste affichait un spinner puis vidait tout le bloc de raisonnement à la fois. Tout modèle qui émet un raisonnement le diffuse maintenant en continu.

- **La vue de chat suit la sortie rapide et atterrit sur la réponse.** Elle ne s'arrête plus au milieu du raisonnement et ne signale pas faussement le nouveau contenu ci-dessous, elle suit le contenu qui se développe sans nouveaux événements, et quand un tour se termine, elle se stabilise en haut du résultat au lieu du bas. S'éloigner en faisant défiler rend toujours le contrôle, et revenir au bas reprend le suivi.

- **Une réponse courte reste où vous la lisiez après la fin du tour.** Une fois qu'un tour s'est terminé, la transcription a glissé vers le bas pour que la question et sa réponse serrent la boîte d'entrée ; la réponse conserve maintenant sa position en haut de la vue jusqu'à l'envoi du message suivant.

- **La connexion applique immédiatement la langue préférée du compte.** Le cookie de locale est maintenant synchronisé lors de la connexion et du rappel OAuth, et la page d'accueil est chargée à nouveau quand la langue du compte diffère de celle sur laquelle la page de connexion a été rendue, donc la destination n'apparaît plus dans la langue précédente jusqu'au rechargement suivant.

- **Cmd+K ouvre la recherche même quand la barre latérale est réduite.** L'écouteur de raccourci restait actif mais sa boîte de dialogue n'a jamais été montée dans l'état réduit, donc le raccourci ne faisait silencieusement rien.

- **Les citations nomment à nouveau le document dont elles proviennent.** En mode de récupération simple, chaque source était étiquetée « Knowledge Base » car l'outil de récupération supprimait le nom de fichier d'origine ; il signale maintenant la source, correspondant à ce que le pipeline de mise à la terre faisait déjà.

- **Les longues conversations sont moins susceptibles de rencontrer des erreurs de débordement de contexte difficile.** La compaction se déclenche maintenant avec une marge de sécurité en dessous de la limite d'entrée du modèle au lieu d'exactement sur elle, et le démarrage enregistre un avertissement quand la fenêtre du modèle rapide est trop petite pour contenir le budget de contexte du modèle général.

- **Les figures SVG restent lisibles en mode sombre.** Le SVG rédigé par le modèle suppose un canevas clair, donc les figures sont dessinées sur une surface claire fixe au lieu de devenir sombre sur sombre.

- **L'indicateur d'occupation de l'onglet du navigateur ne disparaît plus en cours de tour.** Pendant qu'un tour s'exécute, l'icône de l'onglet conserve la marque de l'application et la pulse entre la couleur complète et le gris, se maintenant pendant tout le tour et restaurant de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application. Avec le mouvement réduit activé, il maintient une marque grise statique au lieu de s'animer.

- **Les agents ne re-récupèrent plus un point de terminaison des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à frapper le même point de terminaison avec des paramètres variables est averti puis arrêté brutalement.

- **Une exécution plantée signale maintenant une erreur au lieu d'un « terminé » vert.** Quand l'agent s'est arrêté en raison d'une défaillance interne, le texte d'exception brut a été livré comme réponse sous une coche de succès ; les défaillances passent maintenant par le canal d'erreur et apparaissent comme une notification d'erreur.

- **La reconnexion à un flux ne rejoue plus la sortie que vous aviez déjà vue.** Le flux en direct porte maintenant le même curseur de position que le point de terminaison de reprise, donc après une connexion interrompue seuls les événements manqués sont rejoués au lieu de tout le tour.
...
- **L'appel d'un autre agent retourne sa réponse réelle.** La réponse du sous-agent arrivait autrefois enterrée dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; il reçoit maintenant juste la réponse.

- **Un outil bloqué ne peut plus bloquer une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps horloge murale (180 s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de coincer le tour.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après la compaction.** Le résumé qui condense les tours antérieurs était silencieusement supprimé à la prochaine exécution ; il est maintenant reporté, donc l'agent conserve ce qui a été discuté avant.

- **Les résultats d'outils volumineux conservent leur fin ainsi que leur début.** Quand un long résultat est réduit pour s'adapter à la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande importe généralement le plus), la réduction tient compte correctement du texte non-latin, et les résultats ultérieurs sont garantis une fenêtre minimale au lieu d'être complètement blancs.

- **Le même fichier généré n'apparaît plus comme deux téléchargements.** Une sortie HTML rendue enregistrée à nouveau sous un nom de fichier réel produisait autrefois deux cartes livrables avec un contenu identique ; les doublons sont maintenant détectés par contenu et réduits au fichier spécifiquement nommé.
...
- **Chaque outil dans une conversation fonctionne maintenant sur un système de fichiers partagé unique.** Les sorties volumineuses déchargées atterrissaient autrefois dans un répertoire que seuls les outils de lecture d'espace de travail pouvaient voir, tandis que `file_ops`, `shell_exec` et `python_exec` opéraient ailleurs — l'agent listerait un fichier, puis serait informé qu'il n'existe pas, et brûlerait son budget d'itération à la recherche. Les fichiers déchargés vivent maintenant dans le bac à sable de conversation que chaque outil (y compris les conteneurs exec) monte ; les fichiers des conversations antérieures restent lisibles.

- **La lecture d'un fichier volumineux déchargé ne génère plus une chaîne sans fin de fichiers pointeurs.** La sortie d'une lecture d'espace de travail pourrait elle-même dépasser le seuil de déchargement et être enregistrée comme un nouveau fichier pointant vers le contenu qui était déjà sur le disque, envoyant l'agent en cercles. Les lectures d'espace de travail sont maintenant limitées avec une ligne de reprise exacte au lieu d'être re-déchargées.

- **Le texte d'appel d'outil improvisé de DeepSeek ne fuit plus dans les réponses.** Quand un modèle DeepSeek écrivait une invocation d'outil en tant que texte `<｜｜DSML｜｜Tool call: ...` simple au lieu d'utiliser le canal d'appel de fonction, le balisage brut atteignait le chat verbatim. Il est maintenant supprimé comme d'autres pseudo-protocoles d'appel d'outil.

- **Un appel d'outil coupé par la limite de sortie du modèle ne passe plus comme travail terminé.** Quand une charge utile longue (écrire une page HTML complète, par exemple) épuisait le budget de sortie à mi-appel, l'appel semi-écrit était silencieusement supprimé et le texte d'introduction de l'agent — « maintenant je vais enregistrer le fichier » — devenait le résultat de l'étape, donc une étape qui ne produisait rien était signalée comme terminée. La troncature est maintenant détectée, et l'agent est invité à réessayer avec le travail divisé en appels plus petits.

- **Un objectif que l'agent est sûr d'avoir manqué est maintenant réessayé au lieu de terminer l'exécution.** La re-planification s'arrêtait chaque fois que l'analyseur était confiant, ce qui lisait un « le livrable est manquant » confiant comme une raison d'abandonner — le cas exact pour lequel la re-planification existe. Les exécutions s'arrêtent maintenant tôt seulement quand l'objectif est jugé véritablement inaccessible, et `DAG_MAX_REPLAN_ROUNDS` limite le reste.
