...
- **Le sélecteur de mode d'exécution reste fermé pendant qu'un tour s'exécute.** Le chip était grisé mais ouvrait quand même son menu au clic, contrairement au sélecteur d'agent à côté ; les deux sont maintenant inertes jusqu'à la fin de l'exécution.

- **Les flux de raisonnement fonctionnent à nouveau sur les modèles plus récents.** Le raisonnement en direct était câblé à partir d'une liste codée en dur de noms de modèles, donc tout ce qui a été publié après l'écriture de cette liste affichait un spinner puis déversait tout le bloc de raisonnement à la fois. Tout modèle qui émet du raisonnement le diffuse maintenant en continu.

- **La vue de chat suit la sortie rapide et arrive à la réponse.** Elle ne stagne plus au milieu du raisonnement et ne signale plus faussement du nouveau contenu en dessous, elle suit le contenu qui se développe sans nouveaux événements, et quand un tour se termine, elle s'installe en haut du résultat au lieu du bas. S'éloigner en scrollant rend toujours le contrôle, et revenir au bas reprend le suivi.
...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en milieu de tour.** Pendant qu'un tour s'exécute, l'icône de l'onglet conserve la marque de l'application et la pulse entre la couleur complète et le gris, se maintenant pendant tout le tour et restaurant de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application. Avec la réduction du mouvement activée, elle maintient une marque grise statique au lieu de s'animer.

- **Les agents ne récupèrent plus un endpoint des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à marteler le même endpoint avec des paramètres variables est averti puis arrêté fermement.
...
- **Un outil suspendu ne peut plus bloquer une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps réel (180s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de coincer le tour.

- **Un agent coincé à répéter le même appel d'outil est maintenant arrêté tôt.** Les appels identiques répétés sont refusés après quelques tentatives au lieu de brûler tout le budget d'itération.
