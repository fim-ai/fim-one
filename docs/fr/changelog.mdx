...
- **Les petits tableaux de comparaison dans les réponses de chat s'affichent désormais en mise en page de style carte.** Un tableau à 2-3 colonnes dont la première colonne contient des étiquettes d'attributs affiche chaque attribut dans sa cellule de valeur (correspondant au style de carte de comparaison popularisé par les interfaces de chat IA) ; les tableaux plus grands conservent la grille classique.
...
- **Les réponses, blocs de code et tableaux peuvent être copiés.** Chaque réponse dispose d'une action de copie au format Markdown, les blocs de code portent un en-tête nommant leur langage, et les tableaux offrent une copie au format Markdown plus un téléchargement CSV (UTF-8, s'ouvre correctement dans Excel).

- **Les citations indiquent le document dont elles proviennent.** Un marqueur de citation s'affiche désormais sous forme de pastille portant le nom du document source, les marqueurs adjacents étant regroupés dans une seule pastille avec un compteur ; le popover liste chaque source derrière.
...
- **Les réponses en flux s'affichent progressivement au lieu d'apparaître brutalement.** Les blocs nouvellement arrivés s'estompent brièvement lors du flux, en respectant la préférence de mouvement réduit de l'utilisateur. Les longs tableaux mettent également en surbrillance la ligne sous le curseur, et les puces de liste se placent derrière le texte au lieu de rivaliser avec lui.

- **Les actions de bloc restent discrètes jusqu'à ce que vous les utilisiez.** Les boutons de copie et d'export sur les réponses, les blocs de code, les diagrammes et les tableaux s'affichent au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale conserve désormais la conversation ensemble.** La liste de conversation se trouve directement sous Nouvelle conversation et Recherche, et la navigation du module s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et une hiérarchie claire de haut en bas à la barre latérale.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue à s'afficher en flux pendant sa génération, puis se replie à la fin de l'étape.
...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en cours de tour.** La favicon affiche maintenant un glyphe d'activité constant pendant toute l'exécution et restaure de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application.

- **Les agents ne récupètent plus un endpoint des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à frapper le même endpoint avec des paramètres variables est averti puis arrêté définitivement.
...
- **La reconnexion à un flux ne rejoue plus la sortie que vous avez déjà vue.** Le flux en direct porte maintenant le même curseur de position que l'endpoint de reprise, donc après une connexion interrompue seuls les événements manqués sont rejoués au lieu de tout le tour.

- **Les événements en flux divisés sur plusieurs chunks réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux morceaux ; les deux chemins de flux partagent maintenant un seul analyseur qui gère les limites de chunks arbitraires.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après compaction.** Le résumé qui condense les tours plus anciens était silencieusement supprimé à la prochaine exécution ; il est maintenant conservé, donc l'agent garde ce qui a été discuté avant.
...
- **Un objectif que l'agent est sûr d'avoir manqué est maintenant réessayé au lieu de terminer l'exécution.** La replanification s'arrêtait chaque fois que l'analyseur était confiant, ce qui lisait un « le livrable est manquant » confiant comme une raison d'abandonner — exactement le cas pour lequel la replanification existe. Les exécutions s'arrêtent maintenant tôt uniquement quand l'objectif est jugé véritablement inatteignable, et `DAG_MAX_REPLAN_ROUNDS` limite le reste.

- **L'auto-vérification de l'agent n'apparaît plus comme la réponse.** Après avoir utilisé plusieurs outils, l'agent exécute une vérification interne unique avant de répondre ; cette vérification était diffusée à vous verbatim, donc les réponses arrivaient comme « Laissez-moi vérifier : 1. Entièrement traité ? ✅ … » au lieu de la réponse elle-même. La vérification est maintenant interne, et la réponse que vous voyez est écrite comme sa propre étape.

- **La rotation du token de rafraîchissement invalide maintenant réellement le token précédent.** JWT `exp` n'a que la précision à la seconde, donc deux tokens de rafraîchissement émis pour le même utilisateur à la même seconde codaient autrefois de manière identique — la rotation réécrivait alors le hash stocké avec la même valeur, et un token de rafraîchissement volé pouvait être renouvelé indéfiniment. Chaque token de rafraîchissement porte maintenant un `jti` unique, donc le token pivoté diffère et l'ancien cesse de fonctionner immédiatement.
