...
- **La barre latérale place la navigation en premier.** La navigation des modules s'affiche au-dessus de la liste de conversations sous forme de liste continue compacte (sans étiquettes Build / Resources / Operate), avec un espacement plus serré sur les éléments de navigation, Nouveau chat / Recherche et les lignes de conversation, au lieu d'être cachée dans un dock en bas de la barre latérale.

- **Les réponses en streaming s'affichent régulièrement sans scintillement.** Une réponse en cours de construction s'affiche bloc par bloc : les blocs terminés restent stables au lieu de se réafficher à chaque image, et la mise en forme partiellement arrivée (gras, liens, mathématiques) est complétée ou conservée jusqu'à ce qu'elle se termine.

- **Les réponses finales se transmettent au fur et à mesure qu'elles sont écrites.** L'agent signale maintenant quand il a terminé la collecte et écrit la réponse sous forme de tour en streaming en direct, de sorte que les réponses longues arrivent token par token avec tous les détails au lieu d'être rejouées après une attente. `REACT_FINISH_SIGNAL=false` restaure le comportement précédent.

- **Les documents d'architecture mappent l'ingénierie graphique sur FIM One.** [Planning Landscape](/architecture/planning-landscape) explique l'ingénierie en boucle par rapport à celle en graphe, comment cela diffère des flux de travail visuels classiques, comment cela se mappe à Workflow / DAG / ReAct, et ce qu'il faut emprunter pour chaque moteur. Il indique également le fait actuel que chaque étape DAG est déjà un agent ReAct complet (et non un appel LLM unique). Les pages des moteurs ReAct et DAG y renvoient.

- **Les chats simples offrent toujours des suggestions de suivi.** Les conversations sans agent lié reçoivent des questions suggérées après chaque réponse ; les chats liés à un agent nécessitent toujours le bouton bascule Suggestions de suivi par agent (désactivé par défaut). Les tours de clôture tels que les salutations ou les remerciements peuvent ne retourner aucune puce.
...
- **Les blocs de raisonnement restent repliés pendant l'exécution d'un tour.** Le raisonnement dans la sortie ReAct et DAG affiche maintenant un aperçu d'une ligne par défaut au lieu de se développer au fur et à mesure du streaming, gardant le résultat en vue ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement mis en forme pour le chinois.** L'export PDF intègre maintenant une véritable police CJK au lieu d'en référencer une, corrigeant l'espacement effondré dans le texte anglais et les puces qui s'affichaient comme des caractères chinois sans rapport, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est afin que le texte gras mixte chinois/anglais ne sorte plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les titres d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent sur le plan pendant les tâches longues.** L'agent est maintenant rappelé à changer d'approche quand il répète le même outil sans succès, incité à écrire un plan quand il travaille sans un, et ne peut plus fournir une réponse finale tant que les éléments du plan sont toujours ouverts sans une passe de vérification explicite.

- **Les réponses en streaming s'affichent progressivement au lieu d'apparaître brusquement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du streaming, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en évidence la ligne sous le curseur, et les puces de liste se situent derrière le texte au lieu de concurrencer celui-ci.

- **Les actions de bloc restent discrètes jusqu'à ce que vous les utilisiez.** Copier et télécharger sur les réponses, les blocs de code, les diagrammes et les tableaux sont des icônes uniquement avec des info-bulles (télécharger toujours à droite), révélées au survol, et restent accessibles au clavier.

- **La barre latérale garde maintenant les chats ensemble.** La liste de conversations s'affiche directement sous Nouveau chat et Recherche, et la navigation des modules s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et à la barre latérale une hiérarchie claire de haut en bas.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de se diffuser en direct pendant sa génération, puis se replie quand l'étape se termine.

- **Les pages de liste affichent progressivement leurs cartes.** Les agents, connecteurs, bases de connaissances, compétences, flux de travail, outils, conversations, artefacts et le marché échelonnent leurs cartes au chargement initial au lieu d'apparaître tous à la fois. Le filtrage et la pagination restent instantanés, de sorte que taper dans une boîte de recherche ne rejamais l'entrée.
