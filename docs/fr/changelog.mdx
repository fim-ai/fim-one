...
- **Quitter (ou être retiré d') une organisation révoque désormais son accès partagé.** Les ressources auxquelles vous ne pouviez accéder que par cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour celles-ci sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via la Place de marché ne sont pas affectées.
- **Supprimer un compte supprime désormais tout, de manière cohérente.** Les chemins « supprimer l'utilisateur » (admin) et « supprimer mon compte » (libre-service) divergeaient autrefois — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin admin laissait derrière lui des clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent désormais par une routine unique qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
