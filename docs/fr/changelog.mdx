...
- **Quitter (ou être retiré d'une) organisation révoque maintenant l'accès partagé.** Les ressources auxquelles vous ne pouviez accéder que via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour elles sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via le Market ne sont pas affectées.
- **Supprimer un compte supprime maintenant tout, de manière cohérente.** Les chemins d'accès admin « supprimer l'utilisateur » et libre-service « supprimer mon compte » divergeaient autrefois — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin admin laissait derrière lui des clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent maintenant par une seule routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
...
- **Les actions de connecteur nécessitant une confirmation ne peuvent plus s'exécuter sans surveillance dans un workflow.** Une action de connecteur marquée comme nécessitant une confirmation s'exécutait autrefois à l'intérieur d'un workflow avec la porte d'approbation silencieusement contournée. L'étape échoue maintenant en position fermée avec un message pour placer un nœud Intervention humaine avant elle (la porte d'approbation native du workflow) ou utiliser une action qui ne nécessite pas de confirmation.
- **Les étapes du workflow n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un workflow avait le repli sur identifiants du propriétaire désactivé, un non-propriétaire exécutant ce workflow revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue maintenant avec un message clair pour configurer vos propres identifiants, correspondant au chemin de chat.
...
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres demandes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les demandes simultanées pendant qu'un gros fichier était analysé. L'extraction est maintenant déportée vers un thread de travail.
