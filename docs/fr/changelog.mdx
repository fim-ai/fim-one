...
- **Les portes d'approbation se bloquent désormais quand un agent est atteint via un autre agent ou un workflow.** Un appel de connecteur sensible qui demandait une confirmation en chat s'exécutait précédemment sans vérification une fois délégué via `call_agent` ou enveloppé dans un nœud workflow `AGENT` — ces chemins démarraient l'agent sans aucun hook. Les deux exécutent maintenant les portes propres de l'agent, en utilisant les paramètres de confirmation de cet agent, et refusent de l'exécuter si ses hooks ne peuvent pas être chargés.

- **Une faute de frappe dans `SMTP_SSL` n'envoie plus silencieusement votre mot de passe SMTP en clair.** Précédemment, toute valeur autre que `ssl` ou `tls` (par exemple `SMTP_SSL=true`) basculait vers une connexion non chiffrée sans avertissement, dans les e-mails système et les notifications par e-mail. Les valeurs non reconnues sont désormais rejetées catégoriquement, et la livraison non chiffrée doit être demandée explicitement comme `SMTP_SSL=none`, ce qui enregistre un avertissement à chaque envoi.
