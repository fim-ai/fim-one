...
- **Les réponses ne s'arrêtent plus en pleine phrase quand le modèle atteint sa limite de tokens de sortie.** Quand une réponse est coupée par le plafond de tokens de sortie du fournisseur, l'agent demande automatiquement au modèle de reprendre là où il s'est arrêté (jusqu'à 3 tours) et assemble les segments en une réponse fluide — à la fois dans la boucle de raisonnement et dans la réponse finale en streaming.
- **Les appels LLM réessaient maintenant en cas de surcharge du fournisseur (HTTP 529) et timeout de passerelle (504), et respectent `Retry-After`.** Auparavant, un 529 d'Anthropic échouait immédiatement sauf si un modèle de secours était configuré ; maintenant il attend et réessaye comme pour 429/5xx, en attendant exactement aussi longtemps que le fournisseur le demande quand un en-tête `Retry-After` est présent.
- **Les agents conservent maintenant un plan visible pendant les tâches multi-étapes.** Les agents ReAct obtiennent un outil `update_plan` pour noter leur plan sous forme de liste de tâches et le mettre à jour au fur et à mesure ; si le plan devient obsolète après plusieurs appels d'outils, la liste complète est réinjectée comme rappel — les tâches longues restent alignées même après compaction du contexte. Activé par défaut (`REACT_PLAN_TOOL_ENABLED`) ; les sous-étapes DAG et les agents sans outils l'ignorent.

- **Les agents peuvent maintenant exécuter un workflow en pleine conversation.** Les workflows actifs sans approbation humaine que vous possédez ou auxquels vous êtes abonné sont proposés à l'agent comme un outil `run_workflow` — en chat normal et avec un agent sélectionné — pour qu'une tâche puisse déléguer un sous-processus déterministe et répétable à un workflow et récupérer ses résultats. Chaque exécution est enregistrée dans l'historique des exécutions du workflow et son utilisation de tokens compte dans le quota du propriétaire du workflow (l'exécution est refusée si le propriétaire a déjà dépassé son quota). Les workflows avec une étape d'intervention humaine restent déclenchables uniquement (depuis la page Workflows ou leur clé API), car ils peuvent bloquer longtemps.
- **Deux nouveaux modèles de démarrage de workflow : Connector Action et MCP Tool Call.** La galerie « Créer à partir d'un modèle » présente maintenant les nœuds connecteur et MCP — appelez un système externe ou un outil MCP, puis résumez le résultat avec un LLM. Choisissez le connecteur/serveur spécifique après la création.
