...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en cours de tour.** La favicon affiche maintenant un glyphe d'activité stable pendant toute l'exécution et restaure de manière fiable l'icône réelle à la fin du tour, même après la navigation dans l'application.
...
- **Une exécution échouée rapporte maintenant une erreur au lieu d'un « terminé » vert.** Quand l'agent s'arrêtait en raison d'une défaillance interne, le texte d'exception brut était livré comme réponse sous une coche de succès ; les défaillances passent maintenant par le canal d'erreur et s'affichent comme une notification d'erreur.

- **La reconnexion à un flux ne rejoue plus la sortie que vous aviez déjà vue.** Le flux en direct porte maintenant le même curseur de position que l'endpoint de reprise, donc après une connexion perdue seuls les événements manqués sont rejoués au lieu de tout le tour.

- **Les événements en flux divisés entre les chunks réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux parties ; les deux chemins de flux partagent maintenant un seul analyseur qui gère les limites de chunk arbitraires.
