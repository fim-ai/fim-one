...
- **Les blocs de raisonnement restent repliés pendant l'exécution d'un tour.** Le raisonnement dans les sorties ReAct et DAG affiche désormais un aperçu d'une ligne par défaut au lieu de se développer au fur et à mesure du flux, ce qui maintient le résultat en vue ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement mis en forme pour le chinois.** L'export PDF intègre désormais une véritable police CJK au lieu de la référencer, ce qui corrige l'espacement réduit dans le texte anglais et les puces qui s'affichaient sous forme de caractères chinois sans rapport, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est afin que le texte gras mixte chinois/anglais ne soit plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui maintient les en-têtes d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent sur le plan pendant les tâches longues.** L'agent est maintenant rappelé de changer d'approche lorsqu'il répète le même outil sans succès, incité à rédiger un plan lorsqu'il travaille sans en avoir un, et ne peut plus fournir une réponse finale tant que les éléments du plan sont toujours ouverts sans une étape de vérification explicite.

- **Les réponses en flux s'estompent au lieu d'apparaître soudainement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du flux, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en évidence la ligne sous le curseur, et les puces de liste se situent derrière le texte au lieu de rivaliser avec lui.
...
- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de s'afficher en flux ouvert pendant sa génération, puis se replie à la fin de l'étape.
