...
- **L'espace de travail de conversation est maintenant réellement intégré au chat — les résultats d'outils surdimensionnés sont récupérables au lieu d'être silencieusement perdus.** Les chemins de récupération d'espace de travail de la couche agent (déchargement de sortie volumineuse avec un pointeur `workspace://`, sauvetage en copie complète des résultats d'outils tronqués par budget, snapshots de transcription pré-compaction) existaient mais les points de terminaison de chat ne créaient jamais d'espace de travail, donc aucun d'eux ne s'exécutait : une fois le budget de résultat d'outil épuisé, le modèle recevait des observations vides sans moyen de relire les données, et répondait par conjecture. Les agents ReAct et DAG-step obtiennent maintenant un espace de travail par conversation, et la suppression d'une conversation nettoie ses fichiers d'espace de travail et tout point de contrôle de reprise après sinistre restant.
- **Les exécutions DAG reprises ne perdent plus la fin des longs résultats d'étape.** Les preuves d'étape stockées dans le point de contrôle de reprise après sinistre étaient coupées au début uniquement à la limite de taille, supprimant silencieusement tout ce qui se trouvait à la fin de la sortie — c'est exactement là que les conclusions, les sommes de contrôle et les dernières lignes ont tendance à se trouver. Les preuves plafonnées conservent maintenant à la fois le début et la fin avec un marqueur d'omission explicite entre les deux.
- **Les agents ne « fonctionnent » plus en imprimant leurs propres notes via Python.** Les conseils d'efficacité qui encourageaient l'écriture d'un script complet par appel d'outil sont maintenant limités aux tâches qui nécessitent réellement l'exécution de code — les questions de connaissance, les comparaisons et l'analyse que le modèle peut produire directement sont répondues directement au lieu d'être acheminées via des scripts sandbox inutiles.
- **Une étape d'agent à l'intérieur d'un workflow exécute maintenant l'agent complet, pas seulement son invite.** Le nœud Agent d'un workflow s'exécutait autrefois avec seulement les instructions de l'agent sur un modèle générique et un ensemble d'outils par défaut — supprimant silencieusement les connecteurs, les bases de connaissances, les serveurs MCP et le modèle configuré de l'agent. Il exécute maintenant l'agent exactement comme il le ferait en chat, avec toutes ses ressources liées et son modèle, et seulement si cet agent est visible au propriétaire du workflow.
- **Quitter (ou être retiré d') une organisation révoque maintenant son accès partagé.** Les ressources que vous pouviez atteindre uniquement via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour eux sont supprimés — vous ne pouvez donc plus utiliser ces ressources ou revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via le Market ne sont pas affectées.
- **La suppression d'un compte supprime maintenant tout, de manière cohérente.** Les chemins « supprimer l'utilisateur » administrateur et « supprimer mon compte » en libre-service avaient l'habitude de diverger — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin administrateur laissait derrière lui des clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent maintenant par une routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
