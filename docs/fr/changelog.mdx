...
- **Les réponses, les blocs de code et les tableaux peuvent être copiés.** Chaque réponse dispose d'une action copier-en-Markdown, les blocs de code portent un en-tête nommant leur langage, et les tableaux offrent copier-en-Markdown plus un téléchargement CSV (UTF-8, s'ouvre correctement dans Excel).

- **Les citations indiquent le document d'où elles proviennent.** Un marqueur de citation s'affiche désormais sous forme de pastille portant le nom du document source, les marqueurs adjacents étant regroupés en une seule pastille avec un compteur ; le popover liste chaque source derrière.
...
- **Les diagrammes et les blocs de code peuvent être téléchargés, pas seulement copiés.** Les diagrammes Mermaid exportent le SVG rendu, et chaque bloc de code se télécharge avec un nom de fichier correspondant à son langage ; un bloc `csv` se télécharge en tant que fichier UTF-8 qui s'ouvre correctement dans Excel.

### Modifications

- **Les PDF et fichiers Word exportés sont correctement mis en forme pour le chinois.** L'export PDF intègre désormais une véritable police CJK au lieu de la référencer, ce qui corrige l'espacement effondré dans le texte anglais et les puces qui s'affichaient comme des caractères chinois sans rapport, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est pour que le texte gras mixte chinois/anglais ne soit plus à moitié en gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui maintient les titres subordonnés à la structure propre de la transcription.

- **Les agents restent concentrés sur le plan lors de tâches longues.** L'agent est maintenant rappelé à changer d'approche quand il répète le même outil sans succès, encouragé à rédiger un plan quand il travaille sans en avoir, et ne peut plus fournir une réponse finale tant que des éléments du plan restent ouverts sans une passe de vérification explicite.

- **Les réponses en flux s'estompent progressivement au lieu d'apparaître brutalement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du flux, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en surbrillance la ligne sous le curseur, et les puces de liste se situent derrière le texte au lieu de rivaliser avec lui.

- **Les actions de bloc restent discrètes jusqu'à ce que vous les utilisiez.** Les boutons Copier et Exporter sur les réponses, blocs de code, diagrammes et tableaux s'affichent au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale garde maintenant la conversation ensemble.** La liste de conversations se trouve directement sous Nouvelle conversation et Recherche, et la navigation du module s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et une hiérarchie claire de haut en bas à la barre latérale.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de s'afficher en flux ouvert pendant qu'il est généré, puis se replie quand l'étape se termine.
