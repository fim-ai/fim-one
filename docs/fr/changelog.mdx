...
- **La réflexion étendue fonctionne désormais sur les modèles Anthropic plus récents (Opus 4.6+, Sonnet 4.6, Fable 5).** Ces modèles ont supprimé l'ancien paramètre de réflexion à budget fixe — Opus 4.7/4.8 et Fable 5 le rejettent avec une erreur 400 — donc la configuration de l'effort de raisonnement générait une erreur ou ne faisait rien silencieusement. Nous envoyons désormais le protocole de réflexion adaptative pour ces modèles (et supprimons les paramètres d'échantillonnage qu'ils n'acceptent plus), de sorte que l'effort de raisonnement prend effet correctement. Un modèle qui utilise la réflexion adaptative mais est connecté via un proxy générique compatible OpenAI (où la réflexion ne peut pas être activée) enregistre désormais un avertissement clair au lieu d'échouer silencieusement — configurez-le en tant que fournisseur Anthropic pour activer la réflexion.
- **La déconnexion forcée compare désormais correctement les horodatages sur PostgreSQL.** La vérification d'invalidation de session supposait que l'heure d'invalidation stockée était un UTC naïf et la forçait comme tel, ce qui corrompait silencieusement les comparaisons par rapport aux valeurs tz-aware non-UTC que PostgreSQL peut retourner. Les horodatages sont désormais normalisés en UTC par conversion, non par remplacement, de sorte que les jetons révoqués sont fiablement rejetés sur chaque backend.
- **Les hooks d'application échouent maintenant en mode fermé en cas de crash.** Un hook `PRE_TOOL_USE` (par exemple une porte d'approbation) qui levait une exception retournait précédemment `allow=True`, de sorte que l'appel d'outil bloqué s'exécutait quand même — une porte qui s'écrase devenait silencieusement un contournement. Les hooks PRE refusent désormais par défaut en cas d'erreur ; les hooks non-application (limiteur de débit, enregistreurs) conservent l'ancien comportement fail-open via un nouveau drapeau `fail_open`.
