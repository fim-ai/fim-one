...
- **Les étapes de transformation pure dans les plans DAG s'exécutent en un seul appel LLM direct.** Le planificateur marque les étapes de fusion/réécriture/résumé qui ne consomment que les sorties en amont comme étapes directes, en contournant la boucle d'outils qu'elles n'utiliseraient jamais, ce qui réduit le coût et la latence ; chaque résultat d'étape enregistre désormais aussi comment il s'est exécuté (appel direct ou boucle d'agent, itérations, outils utilisés).

- **Les agents peuvent poser des questions de clarification à choix multiples en cours d'exécution.** Lorsqu'une tâche pourrait prendre des directions matériellement différentes, l'agent s'arrête et présente jusqu'à quatre questions structurées dans le chat, chacune avec 2-4 options plus un champ texte libre « Autre » ; répondre reprend l'exécution sur place, et ignorer ou un délai d'expiration lui permet de continuer selon son propre jugement.

- **Les étapes d'agent portent des titres d'une ligne en direct.** Pendant qu'un agent travaille, chaque itération reçoit un libellé généré court (« Vérification des données de tarification… »), affiché dans un seul en-tête replié qui bascule vers l'étape la plus récente ; le développer révèle la chronologie complète des étapes, et les libellés persistent dans l'historique de conversation.
...
- **Les petits tableaux de comparaison dans les réponses du chat s'affichent désormais sous forme de mises en page de style carte.** Un tableau à 2-3 colonnes dont la première colonne contient des libellés d'attributs affiche chaque attribut à l'intérieur de sa cellule de valeur (correspondant au style de carte de comparaison popularisé par les interfaces de chat IA) ; les tableaux plus grands conservent la grille classique.

- **Les réponses du chat affichent désormais les diagrammes Mermaid et les figures SVG.** Un bloc de code `mermaid` devient un diagramme thématisé et un bloc `svg` devient une image avec un bouton bascule de source et un téléchargement ; les deux reviennent au code brut si la source est incomplète ou ne s'analyse pas, donc un diagramme à moitié diffusé ne montre jamais d'erreur.
...
- **Les citations indiquent le document dont elles proviennent.** Un marqueur de citation s'affiche désormais sous forme de pastille portant le nom du document source, avec les marqueurs adjacents regroupés en une pastille et un nombre ; le survol liste chaque source derrière.

- **`/clear` fonctionne dans l'entrée du chat.** Taper `/clear` démarre une nouvelle conversation, identique à cliquer sur Nouveau chat ; un tour en cours est d'abord arrêté.
...
- **Les agents GPT-5.x exécutent les outils avec le raisonnement activé au lieu de générer une erreur.** Les modèles OpenAI passent maintenant d'abord par l'API Responses, où les outils de fonction et le raisonnement fonctionnent ensemble ; les points de terminaison sans route `/v1/responses` basculent automatiquement vers les complétions de chat, où les appels d'outils GPT-5.x désactivent explicitement le raisonnement car cette API l'exige.

- **Les demandes ask-first se complètent en un seul tour en mode Planner.** Un objectif qui demande explicitement à l'agent de clarifier les exigences avec des questions à choix multiples avant de livrer ("问清楚我再出方案") produit maintenant un questionnaire propre comme résultat du tour, au lieu d'étapes de planification qui attendent des réponses, boucles à travers des re-plans et se terminent par "objectif non atteint" plus des listes de questions répétées. Le mode Auto achemine ces demandes vers le mode Standard.

- **Les puces de suivi ne tirent plus la transcription au-delà de la réponse.** Après qu'un tour se gare sur le résultat, la croissance ultérieure de la mise en page (suggestions, titre) reste en place, et le repliage du tour en direct dans l'historique se re-gare à la tête de la réponse pour la même conversation au lieu de sauter au bas.

- **L'ouverture d'une conversation existante atterrit sur les derniers messages.** L'actualisation d'une URL de chat fait maintenant défiler jusqu'au bas comme le passage de la barre latérale, au lieu de commencer en haut du fil.

- **Les réponses rapides ne semblent plus bloquées sur "Traitement…".** Un tour terminé sans titre d'étape généré affiche l'étiquette du processus de réflexion au lieu du texte de shimmer de traitement en direct.

- **L'animation suit maintenant le paramètre système "réduire le mouvement".** Le dégradé à la dérive du fond de connexion, l'entrée du héros, le texte du message injecté respirant et le curseur de streaming continuaient de se déplacer quel que soit le paramètre du système d'exploitation. Ils restent maintenant immobiles, tandis que les indicateurs qui signalent le travail en cours restent visibles pour que l'interface ne semble jamais bloquée.

- **Le sélecteur de mode d'exécution reste fermé pendant qu'un tour s'exécute.** La puce était grisée mais ouvrait toujours son menu au clic, contrairement au sélecteur d'agent à côté ; les deux sont maintenant inertes jusqu'à la fin de l'exécution.

- **Les flux de raisonnement sont à nouveau en direct sur les modèles plus récents.** Le raisonnement en direct était câblé à partir d'une liste codée en dur de noms de modèles, donc tout ce qui a été publié après l'écriture de cette liste affichait un spinner puis vidait tout le bloc de raisonnement à la fois. Tout modèle qui émet du raisonnement le diffuse maintenant en continu.

- **La vue de chat suit la sortie rapide et atterrit sur la réponse.** Elle ne s'arrête plus au milieu du raisonnement et ne signale pas faussement le nouveau contenu ci-dessous, elle suit le contenu qui se développe sans nouveaux événements, et lorsqu'un tour se termine, elle se stabilise en haut du résultat au lieu du bas. S'éloigner en faisant défiler rend toujours le contrôle, et revenir au bas reprend le suivi.

- **Une réponse courte reste où vous la lisiez après la fin du tour.** Une fois qu'un tour s'était terminé, la transcription glissait vers le bas pour que la question et sa réponse se collent à la boîte d'entrée ; la réponse conserve maintenant sa position en haut de la vue jusqu'à ce que le message suivant soit envoyé.
...
- **L'indicateur d'occupation de l'onglet du navigateur ne disparaît plus en milieu de tour.** Pendant qu'un tour s'exécute, l'icône de l'onglet conserve la marque de l'application et la pulse entre la couleur complète et le gris, se maintenant pendant tout le tour et restaurant de manière fiable l'icône réelle lorsque le tour se termine, même après la navigation dans l'application. Avec la réduction du mouvement activée, elle maintient une marque grise statique au lieu de s'animer.

- **Les agents ne récupèrent plus un point de terminaison des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue de frapper le même point de terminaison avec des paramètres variables est averti puis arrêté brutalement.
...
- **Les événements diffusés divisés entre les chunks réseau ne sont plus perdus.** Un flux repris pourrait perdre un événement dont la trame est arrivée en deux parties ; les deux chemins de flux partagent maintenant un analyseur qui gère les limites de chunk arbitraires.
...
- **Un outil bloqué ne peut plus bloquer une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps réelle (180 s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de coincer le tour.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après compaction.** Le résumé qui condense les tours plus anciens était silencieusement supprimé lors de la prochaine exécution ; il est maintenant reporté, de sorte que l'agent conserve ce qui a été discuté avant.

- **Les résultats d'outils volumineux conservent leur fin ainsi que leur début.** Lorsqu'un long résultat est réduit pour s'adapter à la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande est généralement la plus importante), la troncature tient correctement compte du texte non-latin, et les résultats ultérieurs sont garantis une fenêtre minimale au lieu d'être complètement supprimés.
