...
- **Les réponses en flux s'affichent régulièrement sans scintillement.** Une réponse en cours de construction s'affiche bloc par bloc : les blocs terminés restent stables au lieu de se réafficher à chaque image, et les formatages partiellement arrivés (gras, liens, mathématiques) sont complétés ou maintenus jusqu'à la fin du flux.

- **Les réponses finales se diffusent en direct au fur et à mesure de leur rédaction.** L'agent signale maintenant quand il a terminé la collecte et rédige la réponse sous forme de tour diffusé en direct, de sorte que les réponses longues arrivent jeton par jeton avec tous les détails au lieu d'être rejouées après une attente. `REACT_FINISH_SIGNAL=false` restaure le comportement précédent.
...
- **Les chats simples proposent toujours des suggestions de suivi.** Les conversations sans agent lié reçoivent des questions suggérées après chaque réponse ; les chats liés à un agent nécessitent toujours le bouton bascule Suggestions de suivi par agent (désactivé par défaut). Les tours de fermeture comme les salutations ou les remerciements peuvent ne retourner aucune puce.

- **Un message nouvellement envoyé commence en haut de l'écran.** La transcription conserve juste assez d'espace sous le tour actuel pour que sa première ligne atteigne le haut, de sorte que la question reste en vue et la réponse se développe dans l'espace ci-dessous au lieu que le tour entier soit collé au bord inférieur. L'espace disparaît une fois que le tour dépasse un écran.

- **Les blocs de raisonnement restent repliés pendant l'exécution d'un tour.** Le raisonnement dans la sortie ReAct et DAG affiche maintenant un aperçu d'une ligne par défaut au lieu de se développer au fur et à mesure du flux, gardant le résultat en vue ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement composés pour le chinois.** L'export PDF intègre maintenant une véritable police CJK au lieu d'en référencer une, corrigeant l'espacement effondré dans le texte anglais et les puces qui s'affichaient comme des caractères chinois sans rapport, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est de sorte que le texte gras mixte chinois/anglais ne sort plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les en-têtes d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent sur le plan lors de tâches longues.** L'agent est maintenant rappelé à changer d'approche quand il répète le même outil sans succès, encouragé à rédiger un plan quand il travaille sans, et ne peut plus fournir une réponse finale alors que des éléments du plan sont toujours ouverts sans une passe de vérification explicite.

- **Les réponses en flux s'affichent progressivement au lieu d'apparaître brutalement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du flux, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en évidence la ligne sous le curseur, et les puces de liste se trouvent derrière le texte au lieu de rivaliser avec lui.
...
- **La barre latérale garde maintenant la conversation ensemble.** La liste des conversations se trouve directement sous Nouveau chat et Recherche, et la navigation des modules a été déplacée dans un dock compact teinté au-dessus de la zone utilisateur, donnant plus d'espace à la liste et une hiérarchie claire de haut en bas à la barre latérale.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de s'afficher en direct pendant sa génération, puis se replie quand l'étape se termine.

- **Les pages de liste affichent progressivement leurs cartes.** Les agents, connecteurs, bases de connaissances, compétences, flux de travail, outils, conversations, artefacts et le marché échelonnent leurs cartes au premier chargement au lieu d'apparaître tous à la fois. Le filtrage et la pagination restent instantanés, de sorte que taper dans une boîte de recherche ne rejamais l'entrée.
