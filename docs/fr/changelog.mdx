...
- **Les workflows déclenchés par webhook et cron comptent désormais par rapport au quota de jetons du propriétaire.** Une exécution de workflow sans surveillance — déclenchée via sa clé API publique ou selon un calendrier — exécutait auparavant les étapes LLM et agent sans vérifier ni enregistrer l'utilisation des jetons, un chemin LLM gratuit non mesuré qui contournait entièrement les quotas. Ces exécutions sont désormais soumises au quota du propriétaire du workflow en amont (un déclencheur public est refusé, et une exécution planifiée est ignorée, lorsque le propriétaire a déjà dépassé son quota) et leur utilisation de jetons est enregistrée par exécution et comptabilisée dans le même quota mensuel que le chat.
