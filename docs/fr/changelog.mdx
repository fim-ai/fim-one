...
- **Les agents peuvent poser des questions à choix multiples de clarification en cours d'exécution.** Quand une tâche pourrait prendre des directions matériellement différentes, l'agent fait une pause et présente jusqu'à quatre questions structurées dans le chat, chacune avec 2-4 options plus un champ texte libre « Autre » ; répondre reprend l'exécution sur place, et ignorer ou un délai d'attente lui permet de continuer selon son propre jugement.
...
- **Les modèles d'un fournisseur peuvent être supprimés en masse.** Chaque ligne de modèle dans Admin → Modèles porte une case à cocher, l'en-tête sélectionne la liste entière, et maintenir Maj enfoncé tout en cliquant étend la sélection sur une plage de lignes. Supprimer la sélection supprime chaque modèle choisi en une seule requête, après une confirmation nommant le nombre.

- **Les petits tableaux de comparaison dans les réponses du chat s'affichent désormais sous forme de mises en page de style carte.** Un tableau à 2-3 colonnes dont la première colonne contient des libellés d'attribut affiche chaque attribut à l'intérieur de sa cellule de valeur (correspondant au style de carte de comparaison popularisé par les interfaces de chat IA) ; les tableaux plus grands conservent la grille classique.

- **Les réponses du chat affichent désormais les diagrammes Mermaid et les figures SVG.** Un bloc de code `mermaid` devient un diagramme thématisé et un bloc `svg` devient une image avec un bouton bascule de source et un téléchargement ; les deux reviennent au code brut si la source est incomplète ou ne s'analyse pas, donc un diagramme à moitié diffusé ne clignote jamais une erreur.

- **Les réponses, les blocs de code et les tableaux peuvent être copiés.** Chaque réponse a une action copier-en-Markdown, les blocs de code portent un en-tête nommant leur langage, et les tableaux offrent copier-en-Markdown plus un téléchargement CSV (UTF-8, s'ouvre proprement dans Excel).

- **Les citations indiquent le document dont elles proviennent.** Un marqueur de citation s'affiche désormais sous forme de pastille portant le nom du document source, avec les marqueurs adjacents réduits en une pastille et un nombre ; le popover liste chaque source derrière.

- **`/clear` fonctionne dans l'entrée du chat.** Taper `/clear` démarre une conversation nouvelle, identique à cliquer sur Nouveau chat ; un tour en cours est d'abord arrêté.

- **Les diagrammes et les blocs de code peuvent être téléchargés, pas seulement copiés.** Les diagrammes Mermaid exportent le SVG rendu, et chaque bloc de code se télécharge avec un nom de fichier correspondant à son langage ; un bloc `csv` se télécharge en tant que fichier UTF-8 qui s'ouvre proprement dans Excel.
...
- **Un message nouvellement envoyé commence en haut de l'écran.** La transcription conserve juste assez d'espace sous le tour actuel pour que sa première ligne atteigne le haut, de sorte que la question reste visible et la réponse s'étend dans l'espace ci-dessous au lieu que le tour entier soit collé au bord inférieur. L'espace disparaît une fois que le tour dépasse un écran.

- **Les blocs de raisonnement restent repliés pendant l'exécution d'un tour.** Le raisonnement dans la sortie ReAct et DAG affiche désormais un aperçu d'une ligne par défaut au lieu de s'étendre au fur et à mesure du streaming, gardant le résultat visible ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement composés pour le chinois.** L'export PDF intègre désormais une véritable police CJK au lieu de la référencer, corrigeant l'espacement réduit dans le texte anglais et les puces qui s'affichaient comme des caractères chinois non liés, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est afin que le texte gras mixte chinois/anglais ne soit plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les en-têtes d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent sur le plan lors de tâches longues.** L'agent est maintenant rappelé de changer d'approche lorsqu'il répète le même outil sans succès, incité à écrire un plan lorsqu'il travaille sans plan, et ne peut plus fournir une réponse finale tant que les éléments du plan sont toujours ouverts sans une vérification explicite.

- **Les réponses en streaming s'atténuent au lieu d'apparaître soudainement.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du streaming, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en surbrillance la ligne sous le curseur, et les puces de liste se trouvent derrière le texte au lieu de rivaliser avec lui.

- **Les actions de bloc restent à l'écart jusqu'à ce que vous les utilisiez.** Les boutons Copier et Exporter sur les réponses, les blocs de code, les diagrammes et les tableaux sont révélés au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale garde maintenant la conversation ensemble.** La liste de conversation se trouve directement sous Nouveau chat et Recherche, et la navigation du module s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et une hiérarchie claire de haut en bas à la barre latérale.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de s'afficher en streaming pendant qu'il est généré, puis se replie lorsque l'étape se termine.
