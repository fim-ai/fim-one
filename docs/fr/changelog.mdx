...
- **Les agents conservent désormais un plan visible lors de tâches multi-étapes.** Les agents ReAct obtiennent un outil `update_plan` pour noter leur plan sous forme de liste de contrôle et le mettre à jour au fur et à mesure de leur progression ; si le plan devient obsolète après plusieurs appels d'outils, la liste de contrôle complète est réinjectée comme rappel — les tâches longues restent ainsi alignées sur l'objectif même après la compaction du contexte. Activé par défaut (`REACT_PLAN_TOOL_ENABLED`) ; les sous-étapes DAG et les agents sans outils l'ignorent.

- **Les agents peuvent désormais exécuter un workflow au cours d'une conversation.** Les workflows actifs sans approbation humaine que vous possédez ou auxquels vous êtes abonné sont proposés à l'agent comme un outil `run_workflow` — en chat normal et avec un agent sélectionné — afin qu'une tâche puisse confier un sous-processus déterministe et reproductible à un workflow et récupérer ses résultats. Chaque exécution est enregistrée dans l'historique des exécutions du workflow et son utilisation de tokens est comptabilisée dans le quota du propriétaire du workflow (l'exécution est refusée si le propriétaire a déjà dépassé son quota). Les workflows avec une étape d'intervention humaine restent accessibles uniquement par déclenchement (depuis la page Workflows ou leur clé API), car ils peuvent être bloquants pendant longtemps.

- **Deux nouveaux modèles de démarrage de workflow : Connector Action et MCP Tool Call.** La galerie « Créer à partir d'un modèle » présente désormais les nœuds connecteur et MCP — appelez un système externe ou un outil MCP, puis résumez le résultat avec un LLM. Choisissez le connecteur/serveur spécifique après la création.
