...
- **Le graphique Star History du README s'affiche à nouveau.** Les intégrations tierces en direct (`api.star-history.com`, starchart.cc, …) se sont cassées après que GitHub a restreint l'API des stargazers (juil. 2026). Le graphique est maintenant une paire SVG légère/sombre auto-hébergée dans le dépôt (suit le schéma de couleurs de GitHub), actualisée automatiquement deux fois par jour par GitHub Actions (`scripts/gen_star_history.py`).

- **La rotation des jetons de rafraîchissement invalide maintenant réellement le jeton précédent.** JWT `exp` n'a qu'une précision à la seconde, donc deux jetons de rafraîchissement émis pour le même utilisateur dans la même seconde avaient l'habitude d'être codés de manière identique — la rotation réécrivait alors le hash stocké avec la même valeur, et un jeton de rafraîchissement volé pouvait être renouvelé indéfiniment. Chaque jeton de rafraîchissement porte maintenant un `jti` unique, donc le jeton pivoté diffère et l'ancien cesse de fonctionner immédiatement.

- **Les portes d'approbation se maintiennent maintenant quand un agent est atteint via un autre agent ou un workflow.** Un appel de connecteur sensible qui demanderait une confirmation en chat s'exécutait auparavant sans vérification une fois délégué via `call_agent` ou enveloppé dans un nœud `AGENT` de workflow — ces chemins démarraient l'agent sans aucun hook. Les deux exécutent maintenant les portes propres de l'agent, en utilisant les paramètres de confirmation de cet agent, et refusent de l'exécuter si ses hooks ne peuvent pas être chargés.
...
- **Les bases de connaissances précédemment partagées avec une organisation sont rappelées lors de la mise à niveau.** L'exception v0.8.9 qui maintenait les KBs déjà publiées accessibles à leurs abonnés a disparu : chaque KB partagée revient à l'état personnel, ses abonnements sont supprimés, et l'action de dépublication restante ainsi que les badges KB visibles par les abonnés disparaissent. Une base de connaissances n'atteint désormais d'autres utilisateurs que par le biais d'un Agent partagé, qui délègue le contenu du propriétaire au moment de l'exécution.
