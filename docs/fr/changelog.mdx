...
- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en cours d'exécution.** Le favicon affiche désormais un glyphe d'activité constant pendant toute l'exécution et restaure de manière fiable l'icône réelle à la fin du tour, même après une navigation dans l'application.

- **Les agents ne récupèrent plus un endpoint des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète sauvegardée, et un agent qui continue à frapper le même endpoint avec des paramètres variables est averti puis arrêté définitivement.
...
- **La reconnexion à un flux ne rejoue plus la sortie que vous aviez déjà vue.** Le flux en direct porte maintenant le même curseur de position que l'endpoint de reprise, donc après une connexion interrompue seuls les événements manqués sont rejoués au lieu de tout le tour.
...
- **L'appel d'un autre agent retourne maintenant sa réponse réelle.** La réponse du sous-agent arrivait autrefois enfouie dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; elle reçoit maintenant juste la réponse.

- **Un outil suspendu ne peut plus bloquer une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps réel (180s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de bloquer le tour.

- **Un agent coincé à répéter le même appel d'outil est maintenant arrêté tôt.** Les appels identiques répétés sont refusés après quelques tentatives au lieu de consommer tout le budget d'itération.

- **Les longues conversations ne perdent plus leur contexte antérieur après compaction.** Le résumé qui condense les tours antérieurs était silencieusement supprimé à la prochaine exécution ; il est maintenant conservé, donc l'agent garde ce qui a été discuté avant.

- **Les longs résultats d'outils conservent maintenant leur fin ainsi que leur début.** Quand un long résultat est réduit pour tenir dans la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande importe généralement le plus), la réduction tient correctement compte du texte non-latin, et les résultats ultérieurs sont garantis une fenêtre minimale au lieu d'être complètement effacés.

- **Le même fichier généré n'apparaît plus comme deux téléchargements.** Une sortie HTML rendue sauvegardée à nouveau sous un vrai nom de fichier produisait autrefois deux cartes de livrable avec un contenu identique ; les doublons sont maintenant détectés par contenu et fusionnés dans le fichier spécifiquement nommé.

- **Les fichiers de déchargement interne ne peuvent plus apparaître comme des livrables téléchargeables.** Avec les outils partageant un système de fichiers, une grande sortie d'outil déchargée lors d'une exécution de code pouvait être récupérée par l'analyse d'artefacts et s'afficher comme une fausse carte de téléchargement. Les fichiers de comptabilité du runtime sont maintenant exclus de la détection d'artefacts.

- **Chaque outil dans une conversation fonctionne maintenant sur un système de fichiers partagé unique.** Les grandes sorties déchargées atterrissaient autrefois dans un répertoire que seuls les outils de lecture d'espace de travail pouvaient voir, tandis que `file_ops`, `shell_exec` et `python_exec` opéraient ailleurs — l'agent listait un fichier, puis on lui disait qu'il n'existe pas, et brûlait son budget d'itération à le chercher. Les fichiers déchargés vivent maintenant dans le sandbox de conversation que chaque outil (y compris les conteneurs exec) monte ; les fichiers des conversations antérieures restent lisibles.

- **La lecture d'un grand fichier déchargé ne génère plus une chaîne infinie de fichiers pointeurs.** La sortie d'une lecture d'espace de travail pouvait elle-même dépasser le seuil de déchargement et être sauvegardée comme un nouveau fichier pointant vers du contenu déjà sur disque, envoyant l'agent en boucles. Les lectures d'espace de travail sont maintenant plafonnées avec une ligne de reprise exacte au lieu d'être re-déchargées.
