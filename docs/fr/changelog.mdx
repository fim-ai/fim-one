...
- **Les DAG ne refont plus le travail terminé lors d'une replanification, et les exécutions interrompues reprennent où elles se sont arrêtées.** Quand un plan doit être révisé, les étapes déjà complétées sont conservées dans le nouveau plan avec leurs résultats — seul le travail restant est replanifié et exécuté. La progression des étapes est également sauvegardée au fur et à mesure, donc si une exécution est interrompue en cours (crash, déploiement, connexion perdue), réessayer la même requête reprend à partir des étapes complétées au lieu de recommencer.
- **Les outils lents peuvent désormais s'exécuter en arrière-plan sans bloquer l'agent.** L'exécution de code en sandbox (Python/Shell/Node) offre une option `run_in_background` : l'agent reçoit immédiatement un identifiant de tâche, continue à travailler sur d'autres étapes, et reçoit le résultat sous forme de notification quand l'outil se termine. Si l'agent essaie de finaliser pendant que des tâches s'exécutent encore, il les attend (avec limite) et intègre leurs résultats avant de répondre.
- **L'historique de conversation est capturé avant la compaction, et les résultats d'outils tronqués par le budget de tokens sont sauvegardés dans l'espace de travail.** Avant que la compaction de longue conversation remplace l'historique par un résumé, la transcription complète est écrite dans l'espace de travail de conversation sous forme de fichier JSONL ; de même, un résultat d'outil coupé par le budget de tokens global est sauvegardé en intégralité dans l'espace de travail avec un pointeur laissé en place — dans les deux cas, l'agent (et vous) peut relire l'original au lieu de le perdre.
- **Les réponses ne s'arrêtent plus au milieu d'une phrase quand le modèle atteint sa limite de sortie.** Quand une réponse est coupée par le plafond de tokens de sortie du fournisseur, l'agent demande automatiquement au modèle de reprendre où il s'est arrêté (jusqu'à 3 tours) et assemble les segments en une réponse fluide — à la fois dans la boucle de raisonnement et dans la réponse finale en streaming.
- **Les appels LLM réessaient maintenant en cas de surcharge du fournisseur (HTTP 529) et de timeout de passerelle (504), et respectent `Retry-After`.** Auparavant, un 529 d'Anthropic échouait immédiatement sauf si un modèle de secours était configuré ; maintenant il recule et réessaie comme 429/5xx, en attendant exactement aussi longtemps que le fournisseur le demande quand un en-tête `Retry-After` est présent.
- **Les agents conservent désormais un plan visible pendant les tâches multi-étapes.** Les agents ReAct obtiennent un outil `update_plan` pour noter leur plan sous forme de liste de contrôle et le mettre à jour au fur et à mesure de leur progression ; si le plan devient obsolète après plusieurs appels d'outils, la liste de contrôle complète est réinjectée comme rappel — ainsi les tâches longues restent sur la bonne voie même après la compaction du contexte. Activé par défaut (`REACT_PLAN_TOOL_ENABLED`) ; les sous-étapes DAG et les agents sans outils l'ignorent.

- **Les agents peuvent désormais exécuter un workflow en cours de conversation.** Les workflows actifs et sans approbation humaine que vous possédez ou auxquels vous êtes abonné sont proposés à l'agent comme un outil `run_workflow` — en chat normal et avec un agent sélectionné également — afin qu'une tâche puisse déléguer un sous-processus déterministe et reproductible à un workflow et récupérer ses résultats. Chaque exécution de ce type est enregistrée dans l'historique des exécutions du workflow et son utilisation de tokens compte dans le quota du propriétaire du workflow (l'exécution est refusée si le propriétaire a déjà dépassé le quota). Les workflows avec une étape d'intervention humaine restent déclenchables uniquement (depuis la page Workflows ou leur clé API), car ils peuvent bloquer longtemps.
- **Deux nouveaux modèles de démarrage de workflow : Connector Action et MCP Tool Call.** La galerie « Créer à partir d'un modèle » présente désormais les nœuds de connecteur et MCP — appelez un système externe ou un outil MCP, puis résumez le résultat avec un LLM. Choisissez le connecteur/serveur spécifique après la création.
