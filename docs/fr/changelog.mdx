...
- **Les événements en flux divisés sur plusieurs segments réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux parties ; les deux chemins de flux partagent maintenant un seul analyseur qui gère les limites de segments arbitraires.

- **L'appel d'un autre agent retourne maintenant sa réponse réelle.** La réponse du sous-agent arrivait autrefois enfouie dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; il reçoit maintenant uniquement la réponse.

- **Un outil bloqué ne peut plus immobiliser une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps réel (180 s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de bloquer le tour.

- **Un agent coincé à répéter le même appel d'outil est maintenant arrêté rapidement.** Les appels identiques répétés sont refusés après quelques tentatives au lieu de consommer tout le budget d'itération.
...
- **Les fichiers de déchargement interne ne peuvent plus apparaître comme des livrables téléchargeables.** Avec les outils partageant un système de fichiers, une sortie d'outil volumineux déchargée lors d'une exécution de code pouvait être captée par l'analyse d'artefacts et s'afficher comme une fausse carte de téléchargement. Les fichiers de gestion d'exécution sont maintenant exclus de la détection d'artefacts.

- **Chaque outil dans une conversation fonctionne maintenant sur un système de fichiers partagé unique.** Les sorties voluminuses déchargées atterrissaient autrefois dans un répertoire que seuls les outils de lecture d'espace de travail pouvaient voir, tandis que `file_ops`, `shell_exec` et `python_exec` opéraient ailleurs — l'agent listait un fichier, puis on lui disait qu'il n'existe pas, et il brûlait son budget d'itération à le chercher. Les fichiers déchargés vivent maintenant dans le bac à sable de conversation que chaque outil (y compris les conteneurs exec) monte ; les fichiers des conversations plus anciennes restent lisibles.
