...
- **Les agents ne récupèrent plus un endpoint des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à marteler le même endpoint avec des paramètres variables reçoit un avertissement puis est arrêté définitivement.

- **Une exécution plantée signale maintenant une erreur au lieu d'un « terminé » vert.** Quand l'agent s'arrêtait à cause d'une défaillance interne, le texte d'exception brut était livré comme réponse sous une coche de succès ; les défaillances passent maintenant par le canal d'erreur et s'affichent comme une notification d'erreur.

- **La reconnexion à un flux ne rejoue plus la sortie que vous aviez déjà vue.** Le flux en direct porte maintenant le même curseur de position que l'endpoint de reprise, donc après une connexion interrompue seuls les événements manqués sont rejoués au lieu du tour entier.

- **Les événements en flux divisés sur plusieurs chunks réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux morceaux ; les deux chemins de flux partagent maintenant un seul analyseur qui gère des limites de chunk arbitraires.

- **Appeler un autre agent retourne maintenant sa réponse réelle.** La réponse du sous-agent arrivait autrefois enfouie dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; elle reçoit maintenant juste la réponse.

- **Un outil bloqué ne peut plus paralyser une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps mur (180s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de bloquer le tour.
...
- **Les fichiers de déchargement interne ne peuvent plus apparaître comme des livrables téléchargeables.** Avec les outils partageant un système de fichiers, une grande sortie d'outil déchargée lors d'une exécution de code pouvait être repérée par l'analyse d'artefacts et s'afficher comme une fausse carte de téléchargement. Les fichiers de tenue de livres d'exécution sont maintenant exclus de la détection d'artefacts.

- **Chaque outil dans une conversation fonctionne maintenant sur un système de fichiers partagé.** Les grandes sorties déchargées atterrissaient autrefois dans un répertoire que seuls les outils de lecture d'espace de travail pouvaient voir, tandis que `file_ops`, `shell_exec` et `python_exec` opéraient ailleurs — l'agent listait un fichier, puis on lui disait qu'il n'existe pas, et brûlait son budget d'itération en le cherchant. Les fichiers déchargés vivent maintenant dans le bac à sable de conversation que chaque outil (y compris les conteneurs d'exécution) monte ; les fichiers des conversations antérieures restent lisibles.

- **La lecture d'un grand fichier déchargé ne génère plus une chaîne sans fin de fichiers pointeurs.** La sortie d'une lecture d'espace de travail pouvait elle-même dépasser le seuil de déchargement et être enregistrée comme un nouveau fichier pointant vers du contenu déjà sur disque, envoyant l'agent en boucle. Les lectures d'espace de travail sont maintenant limitées avec une ligne de reprise exacte au lieu d'être re-déchargées.
...
- **Un appel d'outil coupé par la limite de sortie du modèle ne passe plus comme travail terminé.** Quand une longue charge utile (écrire une page HTML complète, par exemple) épuisait le budget de sortie en plein appel, l'appel à moitié écrit était silencieusement supprimé et le texte d'introduction de l'agent — « maintenant je vais enregistrer le fichier » — devenait le résultat de l'étape, donc une étape qui ne produisait rien était signalée comme complétée. La troncature est maintenant détectée, et l'agent est invité à réessayer en divisant le travail en appels plus petits.
