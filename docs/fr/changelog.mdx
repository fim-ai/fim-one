...
- **Les documents d'architecture mappent l'ingénierie de graphes sur FIM One.** [Planning Landscape](/architecture/planning-landscape) explique l'ingénierie en boucle par rapport à celle en graphe, comment cela diffère des workflows visuels classiques, comment cela se mappe à Workflow / DAG / ReAct, et ce qu'il faut emprunter pour chaque moteur. Il indique également le fait actuel que chaque étape DAG est déjà un agent ReAct complet (et non un appel LLM unique). Les pages des moteurs ReAct et DAG y font référence.
...
- **Un message nouvellement envoyé commence en haut de l'écran.** La transcription conserve juste assez d'espace sous le tour actuel pour que sa première ligne atteigne le haut, de sorte que la question reste visible et la réponse se développe dans l'espace ci-dessous au lieu que le tour entier ne s'accroche au bord inférieur. L'espace disparaît une fois que le tour dépasse un écran.

- **Les blocs de raisonnement restent repliés pendant l'exécution d'un tour.** Le raisonnement dans la sortie ReAct et DAG affiche désormais un aperçu d'une ligne par défaut au lieu de se développer au fur et à mesure du streaming, gardant le résultat en vue ; cliquez sur n'importe quel bloc pour lire la trace complète.

- **Les fichiers PDF et Word exportés sont correctement composés pour le chinois.** L'export PDF intègre désormais une véritable police CJK au lieu d'en référencer une, corrigeant l'espacement effondré dans le texte anglais et les puces qui s'affichaient comme des caractères chinois sans rapport, et le gras fonctionne maintenant. Les exports Word déclarent une police d'Asie de l'Est afin que le texte gras mixte chinois/anglais ne sorte plus à moitié gras, et les deux formats utilisent une échelle de taille à cinq niveaux qui garde les en-têtes d'une réponse subordonnés à la structure propre de la transcription.

- **Les agents restent dans le plan pendant les tâches longues.** L'agent est désormais rappelé de changer d'approche lorsqu'il répète le même outil sans résultat, incité à rédiger un plan lorsqu'il travaille sans, et ne peut plus fournir une réponse finale tant que les éléments du plan sont toujours ouverts sans un passage de vérification explicite.

- **Les réponses en streaming s'atténuent au lieu de surgir.** Les blocs nouvellement arrivés s'estompent brièvement au fur et à mesure du streaming, en respectant la préférence de mouvement réduit du lecteur. Les longs tableaux mettent également en surbrillance la ligne sous le curseur, et les puces de liste se situent derrière le texte au lieu de rivaliser avec lui.

- **Les actions de bloc restent à l'écart jusqu'à ce que vous les atteigniez.** Les boutons Copier et Exporter sur les réponses, les blocs de code, les diagrammes et les tableaux sont révélés au survol et partagent un traitement cohérent ; ils restent accessibles au clavier.

- **La barre latérale garde désormais la conversation ensemble.** La liste de conversations se trouve directement sous Nouveau chat et Recherche, et la navigation des modules s'est déplacée dans un dock teinté compact au-dessus de la zone utilisateur, donnant plus d'espace à la liste et une hiérarchie claire de haut en bas à la barre latérale.

- **Le raisonnement est réduit par défaut.** Les sections de réflexion dans les exécutions en direct et les conversations passées se replient en un aperçu d'une ligne et se développent au clic ; le raisonnement continue de se diffuser en direct pendant qu'il est généré, puis se replie lorsque l'étape se termine.

- **Les pages de liste atténuent progressivement leurs cartes en place.** Les agents, connecteurs, bases de connaissances, compétences, workflows, outils, conversations, artefacts et le marché échelonnent leurs cartes au premier chargement au lieu d'apparaître tous à la fois. Le filtrage et la pagination restent instantanés, donc taper dans une zone de recherche ne rejoue jamais l'entrée.
