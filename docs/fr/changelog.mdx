...
- **Le sélecteur de mode d'exécution reste fermé pendant qu'un tour s'exécute.** Le chip était grisé mais ouvrait quand même son menu au clic, contrairement au sélecteur d'agent à côté ; les deux sont maintenant inactifs jusqu'à la fin de l'exécution.

- **Les flux de raisonnement fonctionnent à nouveau sur les modèles plus récents.** Le raisonnement en direct était câblé à partir d'une liste codée en dur de noms de modèles, donc tout ce qui a été publié après cette liste affichait un spinner puis déversait tout le bloc de raisonnement à la fois. Tout modèle qui émet du raisonnement le diffuse maintenant.
...
- **Les conversations longues sont moins susceptibles de rencontrer des erreurs de débordement de contexte dur.** La compaction se déclenche maintenant avec une marge de sécurité en dessous de la limite d'entrée du modèle au lieu de s'y déclencher exactement, et le démarrage enregistre un avertissement quand la fenêtre du modèle rapide est trop petite pour contenir le budget de contexte du modèle général.

- **Les figures SVG restent lisibles en mode sombre.** Le SVG créé par le modèle suppose une toile claire, donc les figures sont dessinées sur une surface claire fixe au lieu de devenir sombre sur sombre.

- **L'indicateur de tab occupé du navigateur ne disparaît plus en cours de tour.** Le favicon affiche maintenant un glyphe occupé constant pour toute l'exécution et restaure de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application.

- **Les agents ne récupèrent plus un endpoint des dizaines de fois après un résultat tronqué.** Les avis de troncature commencent maintenant par un pointeur impératif vers la copie complète enregistrée, et un agent qui continue à frapper le même endpoint avec des paramètres variables est averti puis arrêté brutalement.
...
- **Appeler un autre agent retourne sa réponse réelle.** La réponse du sous-agent arrivait autrefois enfouie dans une trace d'exécution brute, inondant le contexte de l'agent appelant ; elle reçoit maintenant juste la réponse.

- **Un outil bloqué ne peut plus immobiliser une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps mur (180s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de coincer le tour.
...
- **Les conversations longues ne perdent plus leur contexte antérieur après compaction.** Le résumé qui condense les tours antérieurs était silencieusement supprimé à la prochaine exécution ; il est maintenant reporté, donc l'agent garde ce qui a été discuté avant.

- **Les longs résultats d'outil conservent leur fin ainsi que leur début.** Quand un long résultat est coupé pour tenir dans la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande compte généralement le plus), la troncature tient compte correctement du texte non-latin, et les résultats ultérieurs se voient garantir une fenêtre minimale au lieu d'être complètement effacés.
