...
- **Les exécutions DAG reprises ne perdent plus la fin des longues sorties d'étapes.** Les preuves d'étapes stockées dans le point de contrôle de reprise après sinistre étaient coupées en tête uniquement à la limite de taille, supprimant silencieusement tout ce qui vivait à la fin de la sortie — c'est exactement là que les conclusions, les sommes de contrôle et les lignes finales ont tendance à être. Les preuves plafonnées conservent maintenant à la fois la tête et la queue avec un marqueur d'omission explicite entre les deux.
- **Les agents ne « fonctionnent » plus en imprimant leurs propres notes via Python.** Les conseils d'efficacité qui encourageaient à écrire un script complet par appel d'outil sont maintenant limités aux tâches qui ont réellement besoin d'exécution de code — les questions de connaissance, les comparaisons et l'analyse que le modèle peut produire directement sont répondues directement au lieu d'être acheminées via des scripts sandbox inutiles.
...
- **Les étapes du flux de travail n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un flux de travail avait le repli des identifiants du propriétaire désactivé, un non-propriétaire exécutant ce flux de travail revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue maintenant avec un message clair pour configurer vos propres identifiants, correspondant au chemin du chat.
...
- **Vous pouvez maintenant attacher un connecteur abonné, une base de connaissances ou un serveur MCP à un agent.** La liaison exigeait autrefois que vous *possédiez* la ressource, donc un connecteur auquel vous vous étiez légitimement abonné (partagé par l'organisation ou installé via le Marché) était rejeté comme « non possédé ». La liaison suit maintenant la même règle de visibilité que le reste de l'application utilise — tout ce que vous possédez ou auquel vous vous êtes abonné est bindable.
- **Les étapes du flux de travail ne peuvent appeler que les connecteurs auxquels vous avez réellement accès.** Une étape de connecteur du flux de travail s'exécutait autrefois n'importe quel connecteur dont elle référençait l'identifiant, tant que ce connecteur existait ; elle applique maintenant la visibilité du lanceur (possédé ou abonné), donc un flux de travail partagé ne peut pas atteindre un connecteur auquel vous n'avez pas accès en référençant simplement son identifiant.
- **La recherche de base de connaissances fonctionne maintenant sur les bases de connaissances auxquelles vous vous êtes abonné, pas seulement celles que vous possédez.** Les données d'une base de connaissances vivent sous le stockage de son propriétaire, mais une étape de base de connaissances du flux de travail (et une base de connaissances de chat sans agent) cherchait autrefois sous votre propre stockage — donc la recherche d'une base de connaissances partagée avec vous retournait silencieusement rien. Le propriétaire est maintenant résolu à partir de la base de connaissances elle-même et la recherche est vérifiée d'accès (propriétaire ou abonné), donc les bases de connaissances partagées retournent des résultats et les bases de connaissances que vous ne pouvez pas voir retournent rien au lieu d'un résultat vide confus.
- **Les travaux en arrière-plan ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées en tant que tâches fire-and-forget que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont maintenant conservées jusqu'à complétion (et leurs défaillances enregistrées) afin qu'elles se terminent de manière fiable.
