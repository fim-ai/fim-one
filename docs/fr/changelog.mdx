...
- **Chaque outil dans une conversation fonctionne désormais sur un système de fichiers partagé unique.** Les sorties volumineuses déportées se retrouvaient autrefois dans un répertoire que seuls les outils de lecture d'espace de travail pouvaient voir, tandis que `file_ops`, `shell_exec` et `python_exec` opéraient ailleurs — l'agent listait un fichier, puis on lui disait qu'il n'existe pas, et il brûlait son budget d'itération à le chercher. Les fichiers déportés résident désormais dans le sandbox de conversation que chaque outil (y compris les conteneurs d'exécution) monte ; les fichiers des conversations plus anciennes restent lisibles.

- **La lecture d'un fichier déporté volumineux ne génère plus une chaîne infinie de fichiers pointeurs.** La sortie d'une lecture d'espace de travail pouvait elle-même dépasser le seuil de déport et être enregistrée en tant que nouveau fichier pointant vers du contenu déjà sur disque, envoyant l'agent en boucles. Les lectures d'espace de travail sont désormais limitées avec une ligne de reprise exacte au lieu d'être re-déportées.

- **Le texte d'appel d'outil improvisé de DeepSeek ne fuit plus dans les réponses.** Quand un modèle DeepSeek écrivait une invocation d'outil en tant que texte brut `<｜｜DSML｜｜Tool call: ...` au lieu d'utiliser le canal d'appel de fonction, le balisage brut atteignait le chat verbatim. Il est désormais supprimé comme les autres pseudo-protocoles d'appel d'outil.

- **Un appel d'outil coupé par la limite de sortie du modèle ne passe plus pour un travail terminé.** Quand une charge utile longue (écrire une page HTML complète, par exemple) épuisait le budget de sortie au milieu d'un appel, l'appel à moitié écrit était silencieusement supprimé et le texte d'introduction de l'agent — « je vais maintenant enregistrer le fichier » — devenait le résultat de l'étape, donc une étape qui ne produisait rien était signalée comme complétée. La troncature est désormais détectée, et l'agent est invité à réessayer en divisant le travail en appels plus petits.

- **Un objectif que l'agent est sûr d'avoir manqué est désormais réessayé au lieu de terminer l'exécution.** La re-planification s'arrêtait chaque fois que l'analyseur était confiant, ce qui interprétait un « le livrable est manquant » confiant comme une raison d'abandonner — exactement le cas pour lequel la re-planification existe. Les exécutions s'arrêtent désormais tôt uniquement quand l'objectif est jugé véritablement inatteignable, et `DAG_MAX_REPLAN_ROUNDS` limite le reste.

- **L'auto-vérification de l'agent n'apparaît plus comme la réponse.** Après avoir utilisé plusieurs outils, l'agent exécute une vérification interne unique avant de répondre ; cette vérification était diffusée verbatim, donc les réponses arrivaient comme « Laissez-moi vérifier : 1. Entièrement traité ? ✅ … » au lieu de la réponse elle-même. La vérification est désormais interne, et la réponse que vous voyez est écrite comme sa propre étape.

- **La rotation des jetons d'actualisation invalide désormais réellement le jeton précédent.** JWT `exp` n'a qu'une précision à la seconde, donc deux jetons d'actualisation émis pour le même utilisateur à la même seconde codaient autrefois de manière identique — la rotation réécrivait alors le hash stocké avec la même valeur, et un jeton d'actualisation volé pouvait être renouvelé indéfiniment. Chaque jeton d'actualisation porte désormais un `jti` unique, donc le jeton pivoté diffère et l'ancien cesse de fonctionner immédiatement.

- **Les portes d'approbation se maintiennent désormais quand un agent est atteint via un autre agent ou un flux de travail.** Un appel de connecteur sensible qui demanderait une confirmation en chat s'exécutait autrefois sans vérification une fois délégué via `call_agent` ou enveloppé dans un nœud `AGENT` de flux de travail — ces chemins démarraient l'agent sans aucun crochet. Les deux exécutent désormais les portes propres de l'agent, en utilisant les paramètres de confirmation de cet agent, et refusent de l'exécuter si ses crochets ne peuvent pas être chargés.

- **Une faute de frappe dans `SMTP_SSL` n'envoie plus silencieusement votre mot de passe SMTP en clair.** Auparavant, toute valeur autre que `ssl` ou `tls` (par exemple `SMTP_SSL=true`) tombait dans une connexion non chiffrée sans avertissement, à la fois dans les e-mails système et les notifications par e-mail. Les valeurs non reconnues sont désormais rejetées catégoriquement, et la livraison non chiffrée doit être demandée explicitement comme `SMTP_SSL=none`, ce qui enregistre un avertissement à chaque envoi.
