...
- **Les puces de suivi ne font plus défiler la transcription au-delà de la réponse.** Après qu'un tour se soit arrêté sur le résultat, la croissance ultérieure de la mise en page (suggestions, titre) reste en place, et le repliage du tour en direct dans l'historique se réancre à la tête de la réponse pour la même conversation au lieu de sauter vers le bas.

- **L'ouverture d'une conversation existante affiche les derniers messages.** L'actualisation d'une URL de chat fait maintenant défiler vers le bas comme lors du changement depuis la barre latérale, au lieu de commencer en haut du fil.
...
- **L'animation suit maintenant le paramètre système « réduire les mouvements ».** Le dégradé flottant de l'arrière-plan de connexion, l'entrée du héros, le texte du message injecté respirant et le curseur de diffusion en continu continuaient de se déplacer quel que soit le paramètre du système d'exploitation. Ils restent maintenant immobiles, tandis que les indicateurs qui signalent le travail en cours restent visibles pour que l'interface ne semble jamais bloquée.

- **Le sélecteur de mode d'exécution reste fermé pendant qu'un tour s'exécute.** La puce était grisée mais ouvrait quand même son menu au clic, contrairement au sélecteur d'agent à côté ; les deux sont maintenant inactifs jusqu'à la fin de l'exécution.
...
- **La vue de chat suit la sortie rapide et se concentre sur la réponse.** Elle ne s'arrête plus au milieu du raisonnement et ne signale pas faussement du nouveau contenu en dessous, elle suit le contenu qui se développe sans nouveaux événements, et quand un tour se termine, elle se stabilise en haut du résultat au lieu du bas. S'éloigner en faisant défiler rend toujours le contrôle, et revenir au bas reprend le suivi.

- **Une réponse courte reste où vous la lisiez après la fin du tour.** Une fois qu'un tour était terminé, la transcription glissait vers le bas pour que la question et sa réponse épousent la boîte d'entrée ; la réponse conserve maintenant sa position en haut de la vue jusqu'à ce que le message suivant soit envoyé.

- **La connexion applique immédiatement la langue préférée du compte.** Le cookie de langue est maintenant synchronisé lors de la connexion et du rappel OAuth, et la page d'accueil est chargée à nouveau quand la langue du compte diffère de celle dans laquelle la page de connexion a été rendue, donc la destination n'apparaît plus dans la langue précédente jusqu'au prochain rechargement.
...
- **Les figures SVG restent lisibles en mode sombre.** Les SVG créés par le modèle supposent un canevas clair, donc les figures sont dessinées sur une surface claire fixe au lieu de devenir sombre sur sombre.

- **L'indicateur d'activité de l'onglet du navigateur ne disparaît plus en milieu de tour.** Pendant qu'un tour s'exécute, l'icône de l'onglet conserve la marque de l'application et la pulse entre la couleur complète et le gris, se maintenant pendant tout le tour et restaurant de manière fiable l'icône réelle quand le tour se termine, même après la navigation dans l'application. Avec la réduction des mouvements activée, elle maintient une marque grise statique au lieu de s'animer.
...
- **La reconnexion à un flux ne relit plus la sortie que vous avez déjà vue.** Le flux en direct porte maintenant le même curseur de position que le point de terminaison de reprise, donc après une connexion interrompue, seuls les événements manqués sont relus au lieu de tout le tour.

- **Les événements diffusés divisés entre les blocs réseau ne sont plus perdus.** Un flux repris pouvait perdre un événement dont la trame arrivait en deux parties ; les deux chemins de flux partagent maintenant un seul analyseur qui gère les limites de bloc arbitraires.
...
- **Un outil suspendu ne peut plus bloquer une exécution indéfiniment.** Chaque appel d'outil a maintenant une limite de temps d'horloge murale (180 s par défaut), donc un connecteur bloqué ou un serveur qui ne répond pas termine l'étape avec un message clair au lieu de coincer le tour.
...
- **Les longues conversations ne perdent plus leur contexte antérieur après la compaction.** Le résumé qui condense les tours plus anciens était silencieusement supprimé à la prochaine exécution ; il est maintenant reporté, donc l'agent garde ce qui a été discuté avant.

- **Les résultats d'outils volumineux conservent leur fin ainsi que leur début.** Quand un long résultat est coupé pour tenir dans la fenêtre de contexte, les deux extrémités sont maintenant conservées (la conclusion de la sortie d'une commande importe généralement le plus), la coupure tient correctement compte du texte non-latin, et les résultats ultérieurs sont garantis une fenêtre minimale au lieu d'être complètement effacés.
...
- **La rotation du jeton d'actualisation invalide maintenant réellement le jeton précédent.** JWT `exp` n'a qu'une précision à la seconde, donc deux jetons d'actualisation émis pour le même utilisateur dans la même seconde utilisaient autrefois le codage identique — la rotation réécrivait alors le hachage stocké avec la même valeur, et un jeton d'actualisation volé pouvait être renouvelé indéfiniment. Chaque jeton d'actualisation porte maintenant un `jti` unique, donc le jeton pivoté diffère et l'ancien cesse de fonctionner immédiatement.
...
- **Une faute de frappe dans `SMTP_SSL` n'envoie plus silencieusement votre mot de passe SMTP en clair.** Auparavant, toute valeur autre que `ssl` ou `tls` (par exemple `SMTP_SSL=true`) tombait dans une connexion non chiffrée sans avertissement, à la fois dans les e-mails système et les notifications par e-mail. Les valeurs non reconnues sont maintenant rejetées catégoriquement, et la livraison non chiffrée doit être demandée explicitement comme `SMTP_SSL=none`, ce qui enregistre un avertissement à chaque envoi.
