...
- **Les exécutions DAG reprises ne perdent plus la fin des longs résultats d'étapes.** Les preuves d'étapes stockées dans le point de contrôle de reprise après panne étaient tronquées au début uniquement à la limite de taille, supprimant silencieusement tout ce qui se trouvait à la fin du résultat — c'est exactement là que se trouvent les conclusions, les sommes de contrôle et les dernières lignes. Les preuves limitées conservent maintenant à la fois le début et la fin avec un marqueur d'omission explicite entre les deux.
- **Les agents ne « fonctionnent » plus en imprimant leurs propres notes via Python.** Les conseils d'efficacité qui encourageaient l'écriture d'un script complet par appel d'outil sont maintenant limités aux tâches qui ont réellement besoin d'une exécution de code — les questions de connaissance, les comparaisons et l'analyse que le modèle peut produire directement sont répondues directement au lieu d'être acheminées via des scripts sandbox inutiles.
- **Une étape d'agent à l'intérieur d'un workflow exécute maintenant l'agent complet, pas seulement son invite.** Le nœud Agent d'un workflow s'exécutait auparavant uniquement avec les instructions de l'agent sur un modèle générique et un ensemble d'outils par défaut — supprimant silencieusement les connecteurs, les bases de connaissances, les serveurs MCP et le modèle configuré de l'agent. Il exécute maintenant l'agent exactement comme il le ferait en chat, avec toutes ses ressources liées et son modèle, et uniquement si cet agent est visible pour le propriétaire du workflow.
- **Quitter (ou être retiré d') une organisation révoque maintenant son accès partagé.** Les ressources auxquelles vous ne pouviez accéder que via cette organisation sont désabonnées, et les identifiants de connecteur/MCP par utilisateur que vous aviez enregistrés pour eux sont supprimés — vous ne pouvez donc plus utiliser ces ressources ni revenir aux identifiants du propriétaire après votre départ. Les ressources auxquelles vous vous êtes abonné via la Place ne sont pas affectées.
- **La suppression d'un compte supprime maintenant tout, de manière cohérente.** Les chemins d'accès « supprimer l'utilisateur » administrateur et « supprimer mon compte » en libre-service divergeaient auparavant — chacun nettoyait un ensemble différent de fichiers sur disque (et les deux calculaient mal leurs chemins de fichiers, donc chacun en manquait silencieusement certains), et le chemin administrateur laissait derrière lui les clés API, l'historique de connexion, les données d'évaluation, les journaux d'appels et les abonnements. Les deux passent maintenant par une seule routine qui supprime chaque enregistrement et fichier, et la suppression n'échoue plus sur les bases de données qui appliquent les clés étrangères.
...
- **La suppression d'un agent ne casse plus les conversations passées.** Les conversations qui faisaient référence à l'agent sont conservées comme des enregistrements historiques et ont simplement le lien de l'agent effacé, plutôt que de bloquer la suppression.
- **Les actions de connecteur nécessitant une confirmation ne peuvent plus s'exécuter sans surveillance dans un workflow.** Une action de connecteur marquée comme nécessitant une confirmation s'exécutait auparavant à l'intérieur d'un workflow avec la porte d'approbation silencieusement contournée. L'étape échoue maintenant en mode fermé avec un message pour placer un nœud d'intervention humaine avant celle-ci (la porte d'approbation native du workflow) ou utiliser une action qui ne nécessite pas de confirmation.
- **Les étapes du workflow n'empruntent plus les identifiants MCP du propriétaire.** Quand le serveur MCP d'un workflow avait le repli sur les identifiants du propriétaire désactivé, un non-propriétaire exécutant ce workflow revenait silencieusement aux identifiants du serveur du propriétaire au lieu d'être arrêté. L'étape échoue maintenant avec un message clair pour configurer vos propres identifiants, correspondant au chemin du chat.
...
- **La recherche dans la base de connaissances fonctionne maintenant sur les bases auxquelles vous vous êtes abonné, pas seulement celles que vous possédez.** Les données d'une base de connaissances se trouvent sous le stockage de son propriétaire, mais une étape de base de connaissances du workflow (et le chat de base de connaissances sans agent) cherchait auparavant sous votre propre stockage — donc la recherche d'une base partagée avec vous retournait silencieusement rien. Le propriétaire est maintenant résolu à partir de la base de connaissances elle-même et la recherche est vérifiée pour l'accès (propriétaire ou abonné), donc les bases partagées retournent des résultats et les bases que vous ne pouvez pas voir retournent rien au lieu d'un résultat vide déroutant.
- **Les tâches de fond ne disparaissent plus au hasard.** L'ingestion de documents, la livraison de webhooks, les exécutions d'évaluation et les notifications post-réponse étaient lancées comme des tâches « fire-and-forget » que le runtime ne référençait que faiblement, donc sous charge elles pouvaient être garbage-collectées en vol et s'arrêter silencieusement. Elles sont maintenant conservées jusqu'à la fin (et leurs défaillances enregistrées) pour qu'elles se terminent de manière fiable.
- **Les téléchargements de fichiers volumineux ne bloquent plus les autres demandes.** L'extraction de texte à partir d'un PDF/DOCX téléchargé s'exécutait de manière synchrone sur la boucle d'événements principale, gélant brièvement toutes les demandes concurrentes pendant qu'un gros fichier était analysé. L'extraction est maintenant déportée vers un thread de travail.
- **Les réponses multi-étapes (DAG) ne déforment plus leurs sources.** Quand l'étape de recherche d'un plan récupérait une page et la résumait, seul ce résumé progressait — donc une source qui énumérait 11 éléments pouvait apparaître dans la réponse finale comme 6, ou la gravité d'un élément pouvait être mal étiquetée, et l'étape d'évaluation des objectifs ne pouvait pas le détecter car elle ne voyait jamais la source originale. Chaque étape conserve maintenant sa sortie d'outil brute comme preuve source faisant autorité, que la synthèse finale et l'évaluateur vérifient par rapport à la réponse, donc les totaux, les énumérations et les attributs correspondent à ce que les sources ont réellement dit (et la confiance diminue avec une divergence nommée quand ce n'est pas le cas).
- **Le nœud LLM du workflow n'affiche plus les contrôles de modèle qui ne signifiaient pas ce qu'ils impliquaient.** Il avait une liste déroulante de modèle codée en dur dont la sélection était ignorée au moment de l'exécution, plus un bouton bascule de niveau Rapide/Principal exposant un concept interne (auto-dégradant). Les deux sont supprimés — le nœud exécute maintenant toujours votre modèle configuré par le système (Fournisseur de modèle, revenant à `.env`), de la même manière que la zone de jeu le résout, avec une note d'une ligne pointant vers l'endroit où le modèle est configuré.
