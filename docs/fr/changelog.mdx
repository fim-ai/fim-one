...
- **Menu contextuel des nœuds de flux de travail** — Cliquez avec le bouton droit sur n'importe quel nœud pour accéder aux actions Configurer, Copier, Dupliquer et Supprimer. L'action Supprimer est masquée pour les nœuds singleton Début/Fin. Le menu se ferme automatiquement lors d'un clic extérieur ou d'un défilement.
- **Modèles de flux de travail Phase 2** — Deux nouveaux modèles intégrés (8 au total) : Pipeline de traitement de listes (Début → ListOperation → Transform → Fin) et Réponse aux questions améliorée (Début → QuestionUnderstanding → LLM → Fin).
- **Validation du flux de travail Phase 2** — Avertissements de validation de blueprint étendus pour ListOperation (entrée/expression manquante), Transform (entrée manquante/opérations vides), Iterator (variable de liste manquante), Loop (condition manquante), DocumentExtractor, QuestionUnderstanding (entrée manquante), ParameterExtractor (texte d'entrée/paramètres vides manquants). Correction : l'opération de tri ne nécessite plus incorrectement une expression.
- **Notes des nœuds de flux de travail** — Fonctionnalité d'annotation par nœud. Cliquez sur « Note » dans le panneau de configuration de n'importe quel nœud pour ajouter une note en texte libre affichée en italique sous le nœud sur le canevas. Textarea à expansion automatique avec stockage `data.note`.
...
- **Dépôt compatible avec le zoom du flux de travail** — Le glisser-déposer de la palette de nœuds utilise `screenToFlowPosition()` pour un placement correct à n'importe quel niveau de zoom.
...
- **Édition de description en ligne du flux de travail** — Cliquez sur la ligne de description dans la barre d'outils pour ouvrir un textarea de popover pour éditer les descriptions de flux de travail sans naviguer.
...
- **Étiquettes d'arête réactives** — Les étiquettes d'arête pour les branches condition/classificateur utilisent désormais `useNodesData()` pour les mises à jour en temps réel lorsque les conditions ou les classes sont modifiées dans le panneau de configuration.
...
- **Nœuds de flux de travail Phase 2 : DocumentExtractor et QuestionUnderstanding** — Deux autres types de nœuds :
  ...
  - **QuestionUnderstanding** : Prétraitement de question basé sur LLM avec 4 modes — réécrire (clarté), développer (contexte + sous-questions), classer (JSON intention/sujet/confiance), décomposer (tableau de sous-questions). Utilise le LLM rapide avec remplacement de système personnalisé optionnel.
...
- **Nœuds de flux de travail Phase 2 : ListOperation et Transform** — Deux types de nœuds de traitement de données :
  - **ListOperation** : 8 opérations de liste (filtre, map, tri, slice, aplatir, unique, inverser, longueur) avec expressions simpleeval pour filtre/map/tri. Prend en charge l'analyse automatique d'entrée de chaîne JSON.
...
- **Nœuds de flux de travail Phase 2 : Iterator et VariableAggregator** — Deux nouveaux types de nœuds avancés :
  ...
  - Intégration frontend complète : composants de nœud, entrées de palette, formulaires du panneau de configuration, couleurs de minimap, support du sélecteur de variables. Exécuteurs backend enregistrés dans le moteur.
...
- **Dialogue des variables ENV du flux de travail** — Gestion des variables d'environnement chiffrées par flux de travail. Éditeur clé-valeur avec affichage masqué, détection de clé dupliquée et validation en ligne. Connecté aux points de terminaison `GET/PUT /api/workflows/{id}/env`. Accessible via le menu déroulant de la barre d'outils.
...
- **Recherche et filtrage de la liste de flux de travail** — Recherche côté client par nom/description, boutons de filtre d'état (Tous/Brouillon/Actif), menu déroulant de tri (Récemment mis à jour/créé/Nom A-Z) et état vide pour aucun résultat.
- **MiniMap codée par couleur du flux de travail** — Les couleurs des nœuds MiniMap correspondent désormais à la couleur de catégorie de chaque type de nœud (vert=Début, rouge=Fin, bleu=LLM, orange=Condition, etc.).
...
- **Flux de publication du flux de travail** — Points de terminaison de publication/dépublication/renvoi pour les flux de travail, correspondant au modèle de publication d'agent existant. Nouveau drapeau `review_workflows` sur le modèle Organization (avec migration Alembic) pour activer la passerelle d'examen au niveau de l'organisation. Les actions de publication sont câblées dans l'interface utilisateur de la carte de flux de travail et de la barre d'outils.
- **Système de blueprint de flux de travail** — Éditeur de flux de travail visuel pour concevoir et exécuter des blueprints d'automatisation multi-étapes :
  ...
  - **Étiquettes d'arête** : `WorkflowEdgeDef` porte `source_handle` et `target_handle` pour les branches condition/classificateur, permettant les arêtes étiquetées qui indiquent visuellement quel chemin de branche chaque connexion représente.
  ...
  - **Feuille d'historique d'exécution** : Feuille latérale affichant les exécutions de flux de travail passées avec état, durée, données d'entrée/sortie et résultats par nœud. Paginé via `GET /api/workflows/{id}/runs`.
  - **Flux de travail en double** : Point de terminaison `POST /api/workflows/{id}/duplicate` et action d'interface utilisateur pour cloner un flux de travail existant (copie le blueprint, le schéma, la description ; réinitialise l'état au brouillon).
  - **Validation de blueprint** : Point de terminaison `POST /api/workflows/validate` — valide un blueprint sans l'enregistrer (vérifie la présence des nœuds Début/Fin, les ID dupliqués, les types de nœuds inconnus, les références d'arête, la détection de cycle). Retourne les comptages de nœud/arête en cas de succès ou les erreurs de validation spécifiques.
  ...
  - **Délai d'expiration par nœud** : `timeout_ms` configurable par nœud (30 s par défaut) appliqué via `asyncio.wait_for` ; le délai d'expiration produit un résultat `FAILED` structuré avec un message d'erreur descriptif.
  - **Panneau de configuration des nœuds** : Formulaires de configuration par type — les nœuds LLM ont une invite système, un sélecteur de modèle, un basculement de niveau de modèle (rapide/principal), un curseur de température, des tokens max ; ConditionBranch a des menus déroulants d'opérateur (==, !=, >, <, contient, etc.) avec mode expression/LLM ; HTTPRequest a un sélecteur de méthode, URL, éditeur d'en-têtes (ajouter/supprimer des paires clé-valeur), corps avec insertion de variable ; CodeExecution a un sélecteur de langue (Python/JavaScript) avec des indices appropriés à la syntaxe ; tous les nœuds de sortie incluent `OutputVariableField` et `InsertVariableBar`.
  ...
  - **Disposition automatique (ELK.js)** : Utilitaire `getAutoLayoutedNodes()` utilisant l'algorithme en couches ELK.js avec `elk.direction: RIGHT` pour l'arrangement horizontal des nœuds de gauche à droite. Espacement de nœud configurable (60 px entre les nœuds, 120 px entre les couches).
  - **Suite de tests complète** : 34 tests couvrant la validation de l'analyseur (présence Début/Fin, ID dupliqués, types inconnus, détection de cycle, analyse de stratégie d'erreur/délai d'expiration), tri topologique (branches linéaires et parallèles), `VariableStore` (obtenir/définir, interpolation, injection env, sécurité d'instantané, sorties de nœud), exécution du moteur (Début-à-Fin, passage d'entrée, annulation, stratégie STOP_WORKFLOW, CONTINUE) et exécuteurs de nœud (VariableAssign, CodeExecution avec variables, gestion des erreurs).
...
- **Suppression de nœud/arête de flux de travail** : Les raccourcis clavier Retour arrière/Supprimer suppriment les nœuds et arêtes sélectionnés. Les nœuds Début/Fin sont protégés de la suppression. Le panneau de configuration inclut un bouton « Supprimer le nœud » avec confirmation AlertDialog pour la suppression explicite.
- **Interface utilisateur d'importation/exportation/duplication de flux de travail** : Exporter les flux de travail sous forme de téléchargement de fichier JSON, importer avec détection automatique de format, dupliquer le flux de travail via le menu déroulant de la carte. Enveloppe de schéma `WorkflowExportFile` pour le partage de flux de travail portable.
- **Modèles de flux de travail** : 4 modèles de démarrage intégrés (Chaîne LLM simple, Routeur conditionnel, QA augmentée de connaissances, Pipeline API HTTP). Dialogue du sélecteur de modèle lors de la création de flux de travail avec points de terminaison `GET /api/workflows/templates` et `POST /api/workflows/from-template`.
...
- **Admin : Onglet du journal d'audit de révision** — La page d'audit admin a désormais un basculement segmenté « Journal système / Journal de révision ». Le panneau Journal de révision interroge le nouveau point de terminaison `GET /api/admin/review-log` (filtres : org, type de ressource, action ; pagination limite/décalage) et affiche un tableau avec heure, nom de l'organisation, type de ressource, nom de la ressource, badge d'action (codé par couleur : vert=approuvé, rouge=rejeté, ambre=soumis/renvoyé, muet=dépublié) et opérateur. i18n complet (EN + ZH).
...
- **Interface utilisateur d'examen de publication (Frontend)** : Interface utilisateur du système d'examen de publication au niveau de l'organisation — basculement d'examen dans la boîte de dialogue d'édition de l'organisation, ReviewsSheet avec filtres de type/état et flux de travail d'approbation/rejet, badges d'état de publication (en attente/approuvé/rejeté) sur les cartes d'agent avec infobulle pour les notes de rejet, avis d'examen dans la boîte de dialogue de publication lorsque l'organisation cible nécessite un examen, action de renvoi pour les ressources rejetées, menu déroulant « Gestion des révisions » pour l'administrateur/propriétaire de l'organisation. Tous les types de ressources (agents, connecteurs, KB, serveurs MCP) gagnent les champs de type `publish_status`, `reviewed_by`, `reviewed_at`, `review_note`. i18n complet (EN + ZH).
