...
FIM One est fourni avec le code source public sous la licence FIM One Source Available License (FIM-SAL) — source disponible, non approuvée par l'OSI — avec trois formes de déploiement et trois niveaux d'édition.
...
<CardGroup cols={3}>
  ...
  <Card title="Cloud" icon="cloud">
    Hébergé par Wuzhi sur cloud.fim.ai. Abonnement par utilisateur + par connecteur. L'entité de Singapour gère les contrats à l'étranger.
  </Card>
...
La base de code compte environ 170 000 lignes de Python réparties sur plus de 1 590 modules, avec environ 100 fichiers de test et un support intégré pour 6 langues d'interface utilisateur. Le code source est disponible publiquement sous FIM-SAL — les entreprises peuvent auditer la sécurité elles-mêmes. Le coût d'exécution dominant est celui des tokens LLM, non l'infrastructure ; l'agnosticité des fournisseurs signifie que vous bénéficiez de la baisse des prix à mesure que la frontière progresse, sans migration.
