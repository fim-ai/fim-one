...
- **Admin LLM-provider edits now take effect immediately, without needing a process restart.** Updating a provider's `base_url` or `api_key` via the admin panel previously bumped our internal registry version but left LiteLLM's own `AsyncOpenAI` client cache (600 s TTL) bound to the old credentials, so requests could continue hitting the stale endpoint until the cached client expired or the API container was restarted — surfacing as opaque `litellm.InternalServerError: OpenAIException - Connection error.` retries in production. The admin invalidation hook now flushes that cache, and the registry has a 60 s TTL safety net so any future code path that forgets to invalidate self-heals within at most a minute.
