...
- **ENV fallback can be synced from the admin panel.** `scripts/env_from_active_group.py` copies the active model group (general / fast / reasoning, optionally image generation) into `.env`, so the i18n hook, evals and other DB-less paths stop drifting onto a stale provider.

### Changed

- **The AI assistant panels use the same composer as the chat.** The knowledge base, agent and connector assistants now get the pill-shaped input with a multi-line textarea and round send button; Enter while an input method is composing no longer sends the message.

### Fixed

- **Image generation works against OpenAI's `gpt-image-*` models.** The request no longer sends `response_format`, and aspect ratios map to the sizes those models accept.
- **The i18n translation script accepts GPT-5.x and o-series models.** It omits `temperature` for reasoning models instead of failing with a 400.

## [v0.8.11] - 2026-09-03

### Changed

- **The chat composer floats over the message stream.** Attachments, the mode and agent selectors, and the send button now sit in one pill-shaped input that stays over the conversation while messages scroll beneath it; on an empty chat it is centered above the example cards.
...
- **Admin billing and access-model settings read in the interface language again.** The billing posture, access-model and token-quota strings under Admin → Settings, and the matching subscription errors and account usage panel, showed English text or a raw key in Chinese, Japanese, Korean, German and French.
...
- **The admin panel opens in a new tab.** Choosing Admin Panel from the user menu leaves the current workspace open behind it, matching the other external links in that menu.

- **The sidebar puts navigation first.** Module navigation sits above the conversation list as a compact continuous list (no Build / Resources / Operate section labels), with tighter spacing on nav items, New chat / Search, and conversation rows, instead of being tucked into a dock at the bottom of the sidebar.

- **Streamed replies render steadily instead of flickering.** A growing answer now renders block by block: finished blocks stay stable rather than re-rendering on every frame, and half-arrived formatting (bold, links, math) is completed or held until it finishes streaming in.

- **Final answers stream as they are written.** The agent now signals when it is done gathering and writes the answer as a live streamed turn, so long replies arrive token by token with full detail instead of replaying after a wait. `REACT_FINISH_SIGNAL=false` restores the previous behavior.

- **Architecture docs map graph engineering onto FIM One.** [Planning Landscape](/architecture/planning-landscape) explains loop vs graph engineering, how that differs from classic visual workflows, how it maps to Workflow / DAG / ReAct, and what to borrow for each engine. It also states the current fact that every DAG step is already a full ReAct agent (not a one-shot LLM call). ReAct and DAG engine pages link to it.

- **Plain chats always offer follow-up suggestions.** Conversations without a bound agent get suggested next questions after each answer; agent-bound chats still require the per-agent Follow-up Suggestions toggle (default off). Closure turns such as greetings or thanks may return no chips.

- **A newly sent message starts at the top of the screen.** The transcript keeps just enough room below the current turn for its first line to reach the top, so the question stays in view and the answer grows into the space below instead of the whole turn hugging the bottom edge. The room disappears once the turn outgrows one screen.

- **Reasoning blocks stay folded while a turn runs.** Reasoning in both ReAct and DAG output now shows a one-line preview by default instead of expanding as it streams, keeping the result in view; click any block to read the full trace.

- **Exported PDFs and Word files are properly typeset for Chinese.** PDF export now embeds a real CJK font instead of referencing one, fixing collapsed spacing in English text and bullets that rendered as unrelated Chinese characters, and bold now works. Word exports declare an East Asian font so mixed Chinese/English bold text no longer comes out half-bold, and both formats use a five-tier size scale that keeps headings inside a reply subordinate to the transcript's own structure.

- **Agents stay on plan through long tasks.** The agent is now reminded to change approach when it repeats the same tool fruitlessly, nudged to write a plan when working without one, and can no longer deliver a final answer while plan items are still open without an explicit verification pass.

- **Streamed answers ease in instead of popping.** Newly arrived blocks fade in briefly as they stream, honouring the reader's reduced-motion preference. Long tables also highlight the row under the cursor, and list bullets sit behind the text instead of competing with it.

- **Block actions stay out of the way until you reach for them.** Copy and download on answers, code blocks, diagrams and tables are icon-only with tooltips (download always rightmost), revealed on hover, and remain reachable by keyboard.

- **The sidebar now keeps chat together.** The conversation list sits directly under New chat and Search, and the module navigation moved into a compact tinted dock above the user area, giving the list more room and the sidebar a clear top-to-bottom hierarchy.

- **Reasoning is collapsed by default.** Thinking sections in both live runs and past conversations fold to a one-line preview and expand on click; reasoning still streams open while it is being generated, then folds when the step completes.

- **List pages ease their cards into place.** Agents, connectors, knowledge bases, skills, workflows, tools, conversations, artifacts and the market stagger their cards in on first load instead of appearing all at once. Filtering and paging stay instant, so typing in a search box never replays the entrance.

### Security

- **Raw HTML in rendered markdown is now sanitized.** Model output and uploaded markdown files are scrubbed before display, so injected scripts, event handlers, iframes, forms and style overrides can no longer reach the page. Math, syntax highlighting, tables, task lists and disclosure blocks are unaffected. Raw inline `<svg>` is no longer rendered; use an `svg` code block instead.

### Fixed

- **GPT-5.x agents run tools with reasoning enabled instead of erroring.** GPT-5.x models now go through the Responses API first, where function tools and reasoning work together; endpoints without a `/v1/responses` route fall back to chat completions automatically, where GPT-5.x tool calls explicitly disable reasoning as that API requires. Other model families on OpenAI-compatible endpoints keep their existing chat-completions path.

- **Ask-first requests complete in one round under Planner mode.** A goal that explicitly asks the agent to clarify requirements with multiple-choice questions before delivering ("问清楚我再出方案") now produces one clean questionnaire as the turn's result, instead of planning steps that wait for answers, looping through re-plans and ending with "goal not achieved" plus repeated question lists. Auto mode routes such requests to Standard mode.

- **Follow-up chips no longer pull the transcript past the answer.** After a turn parks on the result, later layout growth (suggestions, title) stays put, and folding the live turn into history re-parks at the answer head for the same conversation instead of jumping to the bottom.

- **Opening an existing conversation lands on the latest messages.** Refreshing a chat URL now scrolls to the bottom like switching from the sidebar, instead of starting at the top of the thread.

- **Quick answers no longer look stuck on "Processing…".** A finished turn without a generated step title shows the thought-process label instead of the live processing shimmer text.

- **Animation now follows the system "reduce motion" setting.** The login background's drifting gradient, the hero entrance, the breathing injected-message text and the streaming caret carried on moving whatever the operating system preference said. They now hold still, while the indicators that report work in progress stay visible so the interface never looks stalled.

- **The execution-mode selector stays shut while a turn is running.** The chip was greyed out but still opened its menu on click, unlike the agent selector next to it; both are now inert until the run finishes.

- **Reasoning streams live again on newer models.** Live reasoning was wired up from a hard-coded list of model names, so anything released after that list was written showed a spinner and then dumped the whole reasoning block at once. Any model that emits reasoning now streams it.

- **The chat view keeps up with fast output and lands on the answer.** It no longer stalls mid-reasoning and falsely reports new content below, it follows content that grows without new events, and when a turn finishes it settles at the top of the result instead of the bottom of it. Scrolling away still hands control back, and returning to the bottom resumes following.

- **A short answer stays where you were reading it after the turn ends.** Once a turn finished, the transcript slid down so the question and its reply hugged the input box; the reply now holds its position at the top of the view until the next message is sent.

- **Signing in applies the account's preferred language right away.** The locale cookie is now synced during login and OAuth callback, and the landing page is loaded fresh when the account's language differs from the one the login page was rendered in, so the destination no longer appears in the previous language until the next reload.

- **Cmd+K opens search even when the sidebar is collapsed.** The shortcut listener stayed active but its dialog was never mounted in the collapsed state, so the shortcut silently did nothing.

- **Citations name the document they came from again.** In simple retrieval mode every source was labelled "Knowledge Base" because the retrieval tool dropped the origin filename; it now reports the source, matching what the grounding pipeline already did.

- **Long conversations are less likely to hit hard context-overflow errors.** Compaction now triggers with a safety margin below the model's input limit instead of exactly on it, and startup logs a warning when the fast model's window is too small to hold the general model's context budget.

- **SVG figures stay readable in dark mode.** Model-authored SVG assumes a light canvas, so figures are drawn on a fixed light surface instead of turning dark-on-dark.

- **The browser-tab busy indicator no longer disappears mid-turn.** While a turn runs the tab icon keeps the app mark and pulses it between full colour and grey, holding for the whole run and reliably restoring the real icon when the turn ends, even after in-app navigation. With reduced motion enabled it holds a static grey mark instead of animating.

- **Agents no longer re-fetch an endpoint dozens of times after a truncated result.** Truncation notices now lead with an imperative pointer to the saved full copy, and an agent that keeps hammering the same endpoint with varying parameters is warned and then hard-stopped.

- **A crashed run now reports an error instead of a green "done".** When the agent stopped because of an internal failure, the raw exception text was delivered as the answer under a success checkmark; failures now go through the error channel and surface as an error notification.

- **Reconnecting to a stream no longer replays output you already saw.** The live stream now carries the same position cursor as the resume endpoint, so after a dropped connection only the missed events are replayed instead of the whole turn.

- **Streamed events split across network chunks are no longer lost.** A resumed stream could drop an event whose frame arrived in two pieces; both stream paths now share one parser that handles arbitrary chunk boundaries.

- **Calling another agent returns its actual answer.** The sub-agent's reply used to arrive buried inside a raw execution trace, flooding the calling agent's context; it now receives just the answer.

- **A hung tool can no longer stall a run indefinitely.** Every tool call now has a wall-clock time limit (default 180s), so a stalled connector or unresponsive server ends the step with a clear message instead of wedging the turn.

- **An agent stuck repeating the same tool call is now stopped early.** Identical repeated calls are refused after a couple of attempts instead of burning the whole iteration budget.

- **Long conversations no longer lose their earlier context after compaction.** The summary that condenses older turns was being silently dropped on the next run; it is now carried forward, so the agent keeps what was discussed before.

- **Large tool results keep their ending as well as their beginning.** When a long result is trimmed to fit the context window, both ends are now kept (the conclusion of a command's output usually matters most), trimming accounts correctly for non-Latin text, and later results are guaranteed a minimum window instead of being blanked entirely.

- **The same generated file no longer shows up as two downloads.** A rendered HTML output saved again under a real filename used to produce two deliverable cards with identical content; duplicates are now detected by content and collapsed into the specifically named file.

- **Internal offload files can no longer appear as downloadable deliverables.** With tools sharing one filesystem, a large tool output offloaded during a code-execution run could be picked up by the artifact scan and surface as a bogus download card. Runtime bookkeeping files are now excluded from artifact detection.

- **Every tool in a conversation now works on one shared filesystem.** Offloaded large outputs used to land in a directory only the workspace-reading tools could see, while `file_ops`, `shell_exec` and `python_exec` operated elsewhere — the agent would list a file, then be told it doesn't exist, and burn its iteration budget hunting for it. Offloaded files now live in the conversation sandbox that every tool (including exec containers) mounts; files from older conversations remain readable.

- **Reading a large offloaded file no longer spawns an endless chain of pointer files.** The output of a workspace read could itself exceed the offload threshold and get saved as a new file pointing back at content that was already on disk, sending the agent in circles. Workspace reads are now clamped with an exact resume line instead of being re-offloaded.

- **DeepSeek's improvised tool-call text no longer leaks into answers.** When a DeepSeek model wrote a tool invocation as plain `<｜｜DSML｜｜Tool call: ...` text instead of using the function-calling channel, the raw markup reached the chat verbatim. It is now stripped like other tool-call pseudo-protocols.

- **A tool call cut off by the model's output limit no longer passes as finished work.** When a long payload (writing a full HTML page, for example) exhausted the output budget mid-call, the half-written call was dropped silently and the agent's lead-in text — "now I'll save the file" — became the step result, so a step that produced nothing was reported as completed. The truncation is now detected, and the agent is told to retry with the work split into smaller calls.

- **A goal the agent is sure it missed now gets retried instead of ending the run.** Re-planning stopped whenever the analyzer was confident, which read a confident "the deliverable is missing" as a reason to give up — the exact case re-planning exists for. Runs now stop early only when the goal is judged genuinely unreachable, and `DAG_MAX_REPLAN_ROUNDS` bounds the rest.

- **The agent's self-check no longer shows up as the answer.** After using several tools, the agent runs a one-time internal verification before replying; that verification was being streamed to you verbatim, so answers arrived as "Let me verify: 1. Fully addressed? ✅ …" instead of the answer itself. The check is now internal, and the reply you see is written as its own step.

- **Refresh-token rotation now actually invalidates the previous token.** JWT `exp` is only second-precision, so two refresh tokens issued for the same user in the same second used to encode identically — rotation then rewrote the stored hash with the same value, and a stolen refresh token could be renewed forever. Each refresh token now carries a unique `jti`, so the rotated token differs and the old one stops working immediately.

- **Approval gates now hold when an agent is reached through another agent or a workflow.** A sensitive connector call that would ask for confirmation in chat previously ran unchecked once it was delegated via `call_agent` or wrapped in a workflow `AGENT` node — those paths started the agent with no hooks at all. Both now run the agent's own gates, using that agent's confirmation settings, and refuse to run it if its hooks can't be loaded.

- **A typo in `SMTP_SSL` no longer silently sends your SMTP password in cleartext.** Previously any value other than `ssl` or `tls` (for example `SMTP_SSL=true`) fell through to an unencrypted connection without warning, in both system emails and email notifications. Unrecognized values are now rejected outright, and unencrypted delivery must be requested explicitly as `SMTP_SSL=none`, which logs a warning on every send.

### Removed

- **README Star History chart and its refresh workflow.** After GitHub restricted the stargazers API (Jul 2026), keeping a self-owned chart required a long-lived admin PAT that kept expiring and failing the scheduled job. The chart, SVG assets, generator script, and `Star History` GitHub Actions workflow are gone.

- **Knowledge bases previously shared to an organization are recalled on upgrade.** The v0.8.9 carve-out that kept already-published KBs reachable to their subscribers is gone: every shared KB reverts to personal, its subscriptions are removed, and the leftover unpublish action and subscriber-facing KB badges disappear. A knowledge base now reaches other users only through a shared Agent, which delegates the owner's content at run time.

## [v0.8.9] - 2026-07-08

### Added

- **Skills and Workflows are now optional modules, off by default.** A new install boots with just the core (chat, agents, knowledge bases, connectors); an admin turns Skills and/or Workflows on from Admin → Settings → Modules. While a module is off it's hidden from navigation and its pages, APIs, Market entries, and triggers are unavailable — nothing is deleted, and everything returns when it's switched back on.
...
- **Knowledge bases are no longer shareable.** Publishing a knowledge base to an organization or the Market has been removed; a KB reaches other people only by being bound to a shared Agent, which delegates the owner's content at run time. Any knowledge base already published stays reachable to its current subscribers and can be reverted to personal from its menu.
...
- **API-key scopes are now carried per-request instead of being attached to the user object.** This closes a latent scope-confusion path where a cached or identity-map-shared user could inherit another auth context's scope set.
- **An auto-created `.env` holding the generated JWT secret is now written with owner-only (0600) permissions.** Pre-existing `.env` files keep their permissions.
- **Password hashing cost is now tunable via `FIM_BCRYPT_COST`** (default 12, clamped to bcrypt's 4–31 range) for operators balancing hardware and hardening needs.
...
- **Feishu approval cards now enforce who may approve.** A card click is only honored if the clicker's Feishu account is linked to a FIM One user who is actually allowed to approve under the agent's approver-scope setting (the initiator, the agent owner, or an org member) — the same rule the in-portal approval already applied. Previously anyone in the Feishu group could approve or reject a pending action, and the decision was recorded without an identity.
- **Feishu channels now require an encryption key, and callbacks without a valid signature are rejected.** Callback signature verification fails closed when no encrypt key is configured (previously it accepted unsigned callbacks, so anyone who learned a channel's callback URL could forge an approval), and the encrypted event envelope Feishu sends when encryption is enabled is now decrypted. New and edited Feishu channels must supply the encrypt key.
- **Approval requests are no longer routed to an unintended chat.** When an agent had no approval channel explicitly configured, a pending approval could be pushed to the agent's completion-notification channel or, failing that, to the first channel in the organization — surfacing a sensitive-action card to a group that was never chosen as an approver. Approvals now go only to the agent's explicitly configured approval channel, and otherwise fall back to in-portal approval.
- **Shared MCP servers and knowledge bases are re-checked for access at run time, not just when bound.** A workflow MCP step, an agent's bound MCP server, or an agent's bound knowledge base is now verified against the current subscription state each time it runs, so access revoked by leaving an organization takes effect immediately instead of lingering through a stale binding.

### Fixed

- **OAuth sign-in now fails fast with a clear "provider took too long" message when GitHub/Google/Discord/Feishu are slow**, instead of hanging on library-default timeouts and surfacing a generic failure.
- **Agent no longer generates a spurious image when asked to restyle an HTML deliverable, and never claims a file was created without actually writing it to the workspace.** Image generation is now explicitly scoped to raster pictures, and file deliverables must be written via a tool call — pasting content in chat no longer passes as "generating a file".
- **Generated HTML pages now default to a warm Claude-inspired palette instead of the generic purple-gradient look.** When you don't specify a style, the agent uses a warm minimal baseline; your own style always wins when specified.
- **Long tool-heavy conversations no longer break when history is compacted.** Context compaction could cut the kept window between a tool call and its results, producing a message sequence the LLM provider rejects outright (HTTP 400) — exactly on the long runs that need compaction most. The compactor now keeps each tool call together with its results on the kept side.
- **Background tool results now show up in the step stream and in DAG evidence.** A tool run in the background used to deliver its real output only inside the agent's internal notification message — the UI's step timeline and the DAG answer-verification evidence saw nothing but the dispatch placeholder. The drained result is now emitted as a normal tool completion event under the real tool name.
- **A rare planner output with duplicate step ids no longer fails the whole DAG turn.** Duplicate ids used to be misdiagnosed as a "circular dependency" error; they are now renamed deterministically and the plan proceeds.
- **Several ReAct loop correctness fixes.** A tool returning empty output kept its "completed with no output" note instead of a blank result when the workspace is enabled; DAG steps blocked by an unsatisfiable dependency now emit a proper terminal event instead of leaving the UI hanging; and for models without native tool calling, the fallback answer synthesis now sees the tool results, not just the tool calls.
- **The conversation workspace is now actually wired into chat — oversized tool results are recoverable instead of silently lost.** The agent layer's workspace recovery paths (large-output offload with a `workspace://` pointer, full-copy rescue of budget-truncated tool results, pre-compaction transcript snapshots) existed but the chat endpoints never created a workspace, so none of them ran: once the tool-result budget was exhausted the model received empty observations with no way to read the data back, and answered from guesswork. ReAct and DAG-step agents now get a per-conversation workspace, and deleting a conversation cleans up its workspace files and any leftover crash-resume checkpoint.
...
- **The agent editor no longer warns about unsaved changes when nothing was edited, and saving no longer fails with an approver-scope error.** Opening an agent and leaving could trigger a phantom "unsaved changes" dialog, and saving an agent with a legacy approver-scope value could be rejected outright — both traced to the same field being read inconsistently. The editor now normalizes those fields on load, so the dirty check is accurate and a stale stored value is corrected instead of echoed back.
