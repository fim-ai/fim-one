...
- **GPT-5.x agents run tools with reasoning enabled instead of erroring.** OpenAI models now go through the Responses API first, where function tools and reasoning work together; endpoints without a `/v1/responses` route fall back to chat completions automatically, where GPT-5.x tool calls explicitly disable reasoning as that API requires.
