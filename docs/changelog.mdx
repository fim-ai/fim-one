...
- **README Star History chart renders again.** The live `api.star-history.com` embed times out after GitHub restricted the stargazers API (Jul 2026); the chart is now a static SVG snapshot in-repo (refresh with `python3 scripts/gen_star_history.py`).
