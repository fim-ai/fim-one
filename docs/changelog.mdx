...
- **README Star History chart renders again.** Third-party live embeds (`api.star-history.com`, starchart.cc, …) break after GitHub restricted the stargazers API (Jul 2026). The chart is now a self-owned light/dark SVG pair in-repo (follows GitHub’s color scheme), auto-refreshed twice daily by GitHub Actions (`scripts/gen_star_history.py`).
