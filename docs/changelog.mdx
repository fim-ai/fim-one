...
### Added
- **Database Connectors (Phase 1-3)**: Agents can now introspect schema and execute read-only SQL queries on PostgreSQL, MySQL, Oracle, SQL Server, and Chinese legacy databases (DM/Dameng, KingbaseES, GBase, Highgo). Key capabilities:
  - **Fernet encryption** for database passwords at rest (derived from `JWT_SECRET_KEY`)
  - **SQL safety layer**: sqlparse classification + single-statement enforcement + dangerous pattern blocklist
  - **Database drivers**: `DatabaseDriver` ABC with PostgreSQL (`asyncpg`) and MySQL (`aiomysql`) implementations; connection pool manager (LRU eviction, max 50)
  - **`DatabaseToolAdapter`**: creates 3 tools per DB connector (`list_tables`, `describe_table`, `query`) auto-loaded in chat when bound to agents
  - **8 new API endpoints**: test-connection, introspect, schema CRUD, query execution, AI-powered schema annotation
  - **Schema Manager UI**: two-panel layout with schema discovery, AI annotate, inline editing, visibility toggles
  - **Query Playground**: SQL textarea with run/clear, results table, and truncation warning
  - **Polymorphic connector detail page**: API connectors show Settings+Actions; DB connectors show Settings+Schema+Query
  - **New Connector dropdown**: "New API Connector" / "New Database Connector" split
  - **ConnectorCard**: shows host:port/database and driver badge for DB-type connectors
  - **DB Connector Settings Form**: 8 database types with auto-port fill, SSL/read-only toggles, test connection
  - **59 unit tests** covering SQL safety, encryption, and adapter logic
  - New optional extra `[database]` with `sqlparse`, `cryptography`, `aiomysql` dependencies
...
### Changed (Organization & Visibility)
