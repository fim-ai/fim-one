...
Die **Connector Governance Layer** ergänzt MCP um Governance-Funktionen für Unternehmen, die MCP allein nicht bietet:

| Aspekt | MCP | Connector Governance Layer |
|---|---|---|
| Durchsetzung des Nur-Lese-Modus | Nein | `read_only`-Flag für Operationen; Schreibvorgänge sind standardmäßig blockiert |
| Audit-Protokollierung | Nein | Jeder Tool-Aufruf wird protokolliert (Zeitstempel, Benutzer, Tool, Parameter, Ergebnis) |
| Weitergabe der Authentifizierung | Nein | Proxy für die Authentifizierung des Host-Systems; der Agent handelt im Namen des angemeldeten Benutzers |
| Bestätigungs-Gate | Nein | Schreibvorgänge erfordern die Zustimmung eines Menschen (SSE `confirmation_required`) |
| Circuit Breaker | Nein | Ein Verbindungsfehler löst eine kontrollierte Einschränkung aus |
| Klassifizierung von Operationen | Nein | Operationen werden als Lese-, Schreib- oder Admin-Operationen gekennzeichnet, mit Richtlinien pro Ebene |
| Maskierung auf Spaltenebene | Nein | Als personenbezogene Daten markierte Datenbankspalten geben anstelle ihrer Werte eine Maske zurück |

### Maskierung von Datenbankspalten

Ein Datenbank-Connector wird häufig mit einem privilegierten Konto konfiguriert, sodass jeder Aufrufer auf dieselben Tabellen zugreift. Wenn eine Spalte im Schema-Manager des Connectors als **PII** markiert wird, schränkt dies ein, was der Agent daraus sehen kann:

- Die Spalte bleibt im Schema aufgeführt und gekennzeichnet, sodass das Modell weiß, dass sie existiert, und keinen Ersatz dafür erfindet.
- Jeder unter diesem Spaltennamen zurückgegebene Wert wird durch `***` ersetzt, bevor das Ergebnis das Modell erreicht – auch bei `SELECT *`.
- Das Aufrufprotokoll erfasst, welche Spalten maskiert wurden, sowie, ob sich der Connector im schreibgeschützten Modus befand.

Der Abgleich erfolgt über den Spaltennamen, da dies der einzige Bezeichner ist, den ein Ergebnissatz enthält. Daraus ergeben sich zwei Einschränkungen, die beide beabsichtigt sind: Die Datenbank verwendet die Spalte weiterhin, daher filtert `WHERE salary > 5000` normal, und eine Abfrage, die die Spalte umbenennt (`SELECT salary AS s`), gibt nicht maskierte Werte zurück. Wenn die Anforderung besteht, dass das Abfragekonto eine Spalte überhaupt nicht lesen kann, gewähren Sie diese Einschränkung in der Datenbank und verweisen Sie den Connector auf das eingeschränkte Konto.
