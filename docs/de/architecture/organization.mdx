...
Organisationen sind FIM Ones Einheit für Teamzusammenarbeit. Sie ermöglichen es Benutzergruppen, Ressourcen — Agents, Connectors, MCP Servers, Workflows und Skills — innerhalb eines vertrauenswürdigen Bereichs zu teilen. Knowledge Bases sind die eine Ausnahme: Sie sind niemals teilbar und erreichen andere Mitglieder nur als interne Abhängigkeit eines freigegebenen Agents (siehe [Sharing and credentials](/architecture/sharing-and-credentials)).

Jede Ressource in FIM One beginnt als **persönlich** (nur für ihren Ersteller sichtbar). Wenn Sie eine Ressource in einer Organisation veröffentlichen, wird sie **auffindbar** für andere Organisationsmitglieder über den Marktplatz im Organisationsbereich. Mitglieder durchsuchen die freigegebenen Ressourcen der Organisation und abonnieren die, die sie benötigen.
...
Organisationen und der globale Marktplatz nutzen das gleiche abonnementbasierte Zugriffsmodell. Der Hauptunterschied ist Vertrauen: Organisationen stellen ein Team oder Unternehmen dar, in dem sich Mitglieder kennen und vertrauen, daher ist eine Überprüfung optional und die Anmeldedatenverwaltung ist unkompliziert.
...
## Infrastructure-Ressourcen (organisationsweit, kein Abonnement)

Eine zweite Klasse von organisationsweit geltenden Ressourcen folgt **nicht** dem oben beschriebenen Lebenszyklus „persönlich → veröffentlichen → abonnieren". Diese werden **einmalig von einem Org-Eigentümer oder Admin konfiguriert** und sind dann transparent für jeden Agenten in der Organisation verfügbar:

| Ressource | Zweck | Konfiguriert von | Verwendet von |
|---|---|---|---|
| **Channels** | Ausgehende Verbindung zu einer IM-Plattform (derzeit Feishu; Slack / WeCom / Teams geplant) | Org-Eigentümer / Admin | [Hook-System](/architecture/hook-system) jedes Agenten und proaktive Benachrichtigungswerkzeuge |
...
Der Schlüsseunterschied: **veröffentlichbare Ressourcen** (Agenten, Connectoren, Skills, …) sind benutzerbasierte Artefakte, die ein explizites Mitgliederabonnement erfordern; **Infrastruktur-Ressourcen** (Channels, OAuth, API-Schlüssel) sind Plattform-Level-Verkabelung, die alle Agenten implizit gemeinsam nutzen. Channels sind speziell die IM-Hotline für die gesamte Organisation — ein gut gepflegter Feishu-Channel unterstützt jedes Approval Gate eines Agenten, jeden geplanten Bericht, jedes Eskalationsereignis.
