...
description: "Wie FIM One die Ressourcenfreigabe strukturiert – von Organisationen auf Teamebene bis zum globalen Markt."
---

## Übersicht

FIM One verwendet **Organisationen** als primäre Einheit für Zusammenarbeit und Ressourcenverteilung. Jede Ressource (Agent, Connector, Knowledge Base, MCP Server, Workflow, Skill) beginnt als **persönlich** und kann in einer Organisation veröffentlicht werden, um sie zu teilen.
...
Wenn ein Benutzer eine Ressource in seiner Organisation veröffentlicht, wird sie in der entsprechenden Ressourcenliste für alle Mitglieder angezeigt — Agenten werden in der Agentenliste angezeigt, Konnektoren in der Konnektorenliste und so weiter.
...
- **Fallback deaktiviert** (`allow_fallback=false`): Jedes Mitglied muss seine eigenen Anmeldedaten konfigurieren. Dies ist angemessen, wenn jeder Benutzer seinen eigenen API-Schlüssel benötigt (z. B. pro-Platz-SaaS-Lizenzen).
...
2. **Alle sind Teilnehmer.** Alle Benutzer können Marktplatz-Ressourcen durchsuchen und abonnieren. Der Marktplatz ist immer zugänglich – er ist der Standard-Erkennungskanal.
3. **Obligatorische Überprüfung.** Im Gegensatz zu regulären Organisationen erfordert der Marktplatz **immer** eine Überprüfung. Jede veröffentlichte Ressource muss von einem Systemadministrator genehmigt werden, bevor sie sichtbar wird. Diese Überprüfungsanforderung ist gesperrt und kann nicht geändert werden.
4. **Abonnieren zum Verwenden.** Benutzer müssen eine Marktplatz-Ressource explizit abonnieren, bevor sie in ihren Ressourcenlisten angezeigt wird. Dies unterscheidet sich vom organisationsinternen Sharing, bei dem Ressourcen automatisch für alle Mitglieder verfügbar sind.
...
Nachdem eine Ressource genehmigt und im Marktplatz aufgelistet wurde, kann jeder Benutzer:

1. **Den Marktplatz durchsuchen**, um verfügbare Ressourcen zu entdecken
...
Die Unterscheidung zwischen Organisation und Markt spiegelt eine grundlegende **Vertrauensgrenze** wider:
...
Mitglieder derselben Organisation teilen eine implizite **Vertrauensbeziehung**. Der Organisationseigentümer hat entschieden, diese Personen zusammenzubringen, daher:

- Veröffentlichte Ressourcen sind **sofort verfügbar** (es sei denn, die Überprüfung ist explizit aktiviert)
- Credential-Fallback bedeutet, dass Mitglieder die gemeinsamen API-Schlüssel des Eigentümers verwenden können
- Kein Abonnementschritt erforderlich — wenn Sie in der Organisation sind, sehen Sie alles, das freigegeben wurde

Dies spiegelt wider, wie Teams in der Praxis funktionieren: Sie vertrauen Ihren Teamkollegen mit gemeinsamer Infrastruktur.

### Across the Market

Der Market ist **global** — jeder kann veröffentlichen und jeder kann abonnieren. Es gibt keine vorbestehende Vertrauensbeziehung, daher:

- **Überprüfung ist obligatorisch**, um zu verhindern, dass minderwertige oder böswillige Ressourcen in das Ökosystem gelangen
- **Abonnement ist erforderlich**, damit Benutzer explizit Ressourcen auswählen (keine unerwarteten Ergänzungen in ihrem Workspace)
- **Credential-Handling** folgt demselben Fallback-Mechanismus, aber Benutzer sollten beachten, dass die Verwendung einer Market-Ressource mit Fallback bedeutet, dass ihre Anfragen durch die Anmeldedaten des Herausgebers fließen
...
| `org` + Market | Global | Jeder, der abonniert (falls vom Admin genehmigt) |

Die Sichtbarkeitsfiltierungslogik ist einheitlich — dieselbe Abfrage verarbeitet persönliche, organisatorische und abonnierte Ressourcen:

```
Sichtbar wenn:
  1. Du es besitzt (beliebige Sichtbarkeit), ODER
  2. Es ist in einer Organisation veröffentlicht, der du angehörst UND genehmigt, ODER
...
2. Alice veröffentlicht ihn in der Org ihres Teams (Überprüfung für Konnektoren deaktiviert)
...
5. Der Agent verweist auf einen Konnektor, der einen API-Schlüssel mit `allow_fallback=false` erfordert — Bob muss seinen eigenen Schlüssel konfigurieren, bevor er ihn verwendet
...
2. Wenn ein Mitarbeiter einen neuen Agenten veröffentlicht, erhält dieser den Status "pending_review"
...
5. Wenn der Herausgeber den genehmigten Agenten später bearbeitet, wird er automatisch in den Status "pending_review" zurückgesetzt, um erneut genehmigt zu werden
