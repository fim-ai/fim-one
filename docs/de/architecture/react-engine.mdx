...
Für die Funktionsweise dieser Schleife neben DAG-Planung, Workflow-Blueprints und industriellem **Graph Engineering** siehe [Planning Landscape](/architecture/planning-landscape).

## Die Architektur

Die ReAct-Engine implementiert ein zweiphasiges Ausführungsmodell. Die erste Phase ist eine iterative Werkzeugschleife: Der Agent fordert das LLM wiederholt zu einer Aktion auf, führt jedes angeforderte Werkzeug aus, fügt die Beobachtung hinzu und fährt fort, bis das LLM „fertig“ signalisiert. Die zweite Phase ist die Antwortsynthese: ein separater Streaming-LLM-Aufruf, der den vollständigen Ausführungs-Trace liest und die Antwort für den Benutzer erzeugt.

Diese Aufteilung ist beabsichtigt. Werkzeugiterationen sind auf Geschwindigkeit optimiert – jeder LLM-Aufruf innerhalb der Schleife verwendet das nicht-streamende `chat()`, da der Benutzer weder partielle JSON-Aktionen noch Zwischen-Tokens der Schlussfolgerung sehen muss. Die Antwortgenerierung ist auf eine gute UX optimiert – sie verwendet `stream_chat()`, sodass der Benutzer die Tokens in Echtzeit erscheinen sieht. Das Ergebnis vereint die Vorteile beider Ansätze: schnelle Werkzeugausführung und eine reaktionsschnelle Antwortübermittlung.
...
Die Werkzeugschleife erzeugt ein `AgentResult`, das den vollständigen Konversationsverlauf enthält – System-Prompt, Benutzeranfrage, jede Assistentennachricht und jedes Werkzeugergebnis. Die Methode `stream_answer()` verdichtet diesen Trace zu einer prägnanten, kohärenten Antwort. Werkzeugergebnisse werden im Synthesekontext jeweils auf 2.000 Zeichen gekürzt, sodass der Prompt auch nach komplexen Workflows mit mehreren Werkzeugen kompakt bleibt.

**Modellbindung.** Das LLM wird in `ReActAgent.__init__()` injiziert und als `self._llm` gespeichert. Jeder Aufruf innerhalb einer einzelnen `run()`-Ausführung – alle Iterationen der Werkzeugschleife sowie die abschließende Antwortsynthese – verwendet dieselbe Instanz. Das Modell ändert sich zwischen den Iterationen nicht. Um ein anderes Modell zu verwenden, muss ein neuer `ReActAgent` erstellt werden. Im DAG-Modus nutzt `DAGExecutor._resolve_agent()` dieses Muster: Direkt bevor die ReAct-Schleife des jeweiligen Schritts beginnt, erstellt die Methode einen neuen Agenten pro Schritt und wählt das Modell anhand von `step.model_hint` aus der `ModelRegistry` aus. Weitere Informationen finden Sie unter [DAG Engine – Überschreibung pro Schritt](/architecture/dag-engine#multi-llm-architecture).
...
Die Checkliste ist ein interner Austausch. Der Turn, der sie beantwortet, ist Verifizierung, nicht die Antwort — die wörtliche Annahme würde „Lassen Sie mich überprüfen: 1. ... ✓" in den Chat durchsickern lassen. Dieser Turn wird daher intern verbraucht und der Agent wird erneut aufgefordert, die benutzerfreundliche Antwort zu geben, die `stream_answer()` streamt. Wenn der Agent stattdessen entscheidet, dass etwas fehlt, setzt er die Tool-Call-Schleife fort und seine spätere Antwort wird unverändert übernommen.

Die Checkliste wird höchstens einmal pro Durchlauf ausgelöst, wird nicht für einfache Konversationsantworten ausgelöst (keine Tools verwendet) und wird übersprungen, wenn das verbleibende Iterations-Budget nicht sowohl den Verifizierungs-Turn als auch den Antwort-Turn abdecken kann.

Der Schwellenwert verhindert unnötige Latenz bei einfachen Aufgaben — ein einzelner Tool-Aufruf (z. B. eine schnelle Berechnung) rechtfertigt keine Verifizierung, während eine Multi-Tool-Untersuchung von einer abschließenden Plausibilitätsprüfung profitiert. Setzen Sie `REACT_COMPLETION_CHECK_MIN_TOOLS=1` für ständig aktivierte Verifizierung.
