...
ReAct wird nicht verschwinden – es versenkt sich im Modell. Betrachten Sie die Analogie: Sie schreiben TCP-Handshakes nicht von Hand, aber TCP ist nicht weg; es wurde vom Betriebssystem absorbiert. Wenn Modelle stark genug sind, wird die Think-Act-Observe-Schleife zu implizitem Verhalten im Modell, nicht zu explizitem Framework-Code. Das passiert bereits: Claude Code ist im Grunde ein ReAct-Agent, bei dem die Schleife vom Modell selbst angetrieben wird, nicht von einem externen Harness.

Der bleibende Wert von DAG-Planung ist nicht „dummen Modellen helfen, Aufgaben zu zerlegen" – es ist **gleichzeitige Planung**. Selbst mit unendlich leistungsstarken Modellen setzt die Physik Latenzen durch: eine serielle Kette von 10 LLM-Aufrufen dauert 10-mal länger als 3 parallele Wellen. DAG ist ein Engineering-Problem (wie man Dinge schnell und zuverlässig ausführt), nicht ein Intelligenz-Problem (wie man entscheidet, was man ausführt). Wiederholungslogik, Kostenkontrolle, Timeout-Verwaltung, Observability – diese verschwinden nicht, wenn Modelle intelligenter werden.

Das Endspiel: **Modelle besitzen das „Was" (Planungsintelligenz verinnerlicht sich im Modell), Frameworks besitzen das „Wie" (Nebenläufigkeit, Wiederholung, Überwachung, Kostengovernance)**. Der bleibende Wert eines Frameworks ist nicht Intelligenz – es ist Governance.

Das ist der Satz, von dem der Rest von FIM One abhängig ist. Sobald das Modell das „Was" besitzt, verlagert sich der dauerhafte Wert auf die Governance des „Wie" – und das höchstwertige „Wie" in jedem Unternehmen ist *Reichweite* in die Systeme, die sich weigern, miteinander zu sprechen. Der Connector-Hub ist einen Inferenzschritt von hier entfernt, und die Geschichte zeigt es: Das Projekt begann als DAG-Orchestrierungs-Framework (`fim-agent`) und wurde innerhalb einer Woche zu einer Connector-Plattform – nicht durch einen Rewrite, sondern weil diese Wette es bereits erforderte.

## Warum nicht Difys Workflow-Editor spiegeln

Eine berechtigte Frage: Wenn DAG den flexiblen Fall abdeckt, sollten wir dann nicht auch einen statischen Workflow-Editor für den deterministischen Fall bauen?
...
Die strategische Wette: **nicht bei der Workflow-Visualisierung konkurrieren; konkurrieren, indem man die Agent-Plattform ist, wo Systeme auf AI treffen**. Lass Dify „AI-Workflows visuell bauen" besitzen. FIM One besitzt „die All-in-One-Agent-Plattform für Global × China Enterprises."
