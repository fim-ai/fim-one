...
- **Der Ausführungsmodus-Selektor bleibt geschlossen, während ein Durchlauf läuft.** Der Chip war ausgegraut, öffnete sein Menü aber trotzdem bei Klick, anders als der daneben liegende Agent-Selektor; beide sind jetzt inaktiv, bis der Durchlauf beendet ist.

- **Reasoning-Streams funktionieren wieder bei neueren Modellen.** Live Reasoning war über eine hartcodierte Liste von Modellnamen verdrahtet, daher zeigte alles, das nach dieser Liste veröffentlicht wurde, einen Spinner und gab dann den ganzen Reasoning-Block auf einmal aus. Jedes Modell, das Reasoning ausgibt, streamt es jetzt.

- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie stellt sich nicht mehr mitten im Reasoning auf und meldet fälschlicherweise neue Inhalte darunter, sie folgt Inhalten, die ohne neue Events wachsen, und wenn ein Durchlauf beendet ist, bleibt sie oben im Ergebnis statt unten. Wenn Sie wegscollen, wird die Kontrolle zurückgegeben, und wenn Sie zur Unterseite zurückkehren, wird das Folgen fortgesetzt.

- **Anmelden wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Startseite wird neu geladen, wenn sich die Sprache des Kontos von der unterscheidet, in der die Anmeldeseite gerendert wurde, sodass das Ziel nicht mehr bis zur nächsten Aktualisierung in der vorherigen Sprache angezeigt wird.

- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener blieb aktiv, aber sein Dialog wurde im eingeklappten Zustand nie bereitgestellt, daher tat der Shortcut stillschweigend nichts.

- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abruftmodus wurde jede Quelle mit „Knowledge Base" gekennzeichnet, weil das Abruftool den Ursprungsdateinamen gelöscht hat; es meldet jetzt die Quelle, was dem entspricht, was die Grounding-Pipeline bereits tat.

- **Lange Unterhaltungen treffen weniger wahrscheinlich auf harte Kontextüberlauffehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unter dem Eingabelimit des Modells ausgelöst, statt genau darauf, und beim Start wird eine Warnung protokolliert, wenn das Fenster des schnellen Modells zu klein ist, um das Kontextbudget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVG geht von einer hellen Leinwand aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, statt dunkel-auf-dunkel zu werden.

- **Der Busy-Indikator des Browser-Tabs verschwindet nicht mehr mitten im Durchlauf.** Das Favicon zeigt jetzt ein stetiges Busy-Glyphe für den ganzen Durchlauf und stellt das echte Symbol zuverlässig wieder her, wenn der Durchlauf endet, auch nach In-App-Navigation.

- **Agenten rufen einen Endpunkt nach einem abgekürzten Ergebnis nicht mehr dutzende Male ab.** Abkürzungsmitteilungen führen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie an, und ein Agent, der denselben Endpunkt immer wieder mit unterschiedlichen Parametern bombardiert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Durchlauf meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshäkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und werden als Fehlerbenachrichtigung angezeigt.

- **Erneutes Verbinden mit einem Stream wiederholt nicht mehr Ausgaben, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, daher werden nach einer unterbrochenen Verbindung nur die verpassten Events statt des ganzen Durchlaufs wiedergegeben.
...
- **Das Aufrufen eines anderen Agenten gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher in einer rohen Ausführungsspur begraben an und überflutete den Kontext des aufrufenden Agenten; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Durchlauf nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Wanduhr-Zeitlimit (Standard 180s), daher endet ein blockierter Connector oder nicht reagierender Server den Schritt mit einer klaren Meldung, statt den Durchlauf zu blockieren.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen abgelehnt, statt das ganze Iterationsbudget zu verbrauchen.

- **Lange Unterhaltungen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Durchlauf stillschweigend gelöscht; sie wird jetzt weitergeleitet, daher behält der Agent, was vorher besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und späteren Ergebnissen ist ein Minimalfenster garantiert, statt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungsdurchlaufs offloaded wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien sind jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Unterhaltung funktioniert jetzt auf einem gemeinsamen Dateisystem.** Offgeladene große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann gesagt bekommen, dass sie nicht existiert, und sein Iterationsbudget beim Suchen danach verbrauchen. Offgeladene Dateien befinden sich jetzt in der Unterhaltungs-Sandbox, die jedes Tool (einschließlich Exec-Container) bereitstellt; Dateien aus älteren Unterhaltungen bleiben lesbar.

- **Das Lesen einer großen offgeladenen Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte verweist, die bereits auf der Festplatte waren, was den Agenten in Kreise schickt. Workspace-Lesevorgänge werden jetzt mit einer exakten Resume-Zeile begrenzt, statt erneut offgeloaded zu werden.

- **DeepSeeks improvisierten Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, statt den Function-Calling-Kanal zu verwenden, erreichte das rohe Markup den Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit weitergegeben.** Wenn eine lange Nutzlast (zum Beispiel das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halbgeschriebene Aufruf stillschweigend gelöscht und der einleitende Text des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, daher wurde ein Schritt, der nichts produzierte, als abgeschlossen gemeldet. Die Kürzung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.
...
- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, daher kamen Antworten als „Lassen Sie mich überprüfen: 1. Vollständig behoben? ✅ …" statt der Antwort selbst an. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als ihr eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` ist nur Sekunden-Präzision, daher codierten zwei Refresh-Tokens, die in derselben Sekunde für denselben Benutzer ausgegeben wurden, früher identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte auf ewig erneuert werden. Jedes Refresh-Token trägt jetzt eine eindeutige `jti`, daher unterscheidet sich das rotierte Token und das alte funktioniert sofort nicht mehr.

- **Approval Gates halten jetzt, wenn ein Agent durch einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensitiver Connector-Aufruf, der in Chat um Bestätigung fragen würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT` Node eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen jetzt die eigenen Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Früher fiel jeder Wert außer `ssl` oder `tls` (zum Beispiel `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung durch, sowohl in System-E-Mails als auch in E-Mail-Benachrichtigungen. Unerkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
