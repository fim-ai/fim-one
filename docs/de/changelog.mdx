...
- **Ein SSRF-Bypass über IPv4-gemappte IPv6-Adressen wurde blockiert.** Eine URL, deren Host ein IPv4-gemapptes IPv6-Literal war (z. B. `http://[::ffff:169.254.169.254]/`), umging die Blocklist für private IPs und konnte auf Dual-Stack-Hosts auf interne Dienste zugreifen — einschließlich Cloud-Instanz-Metadaten. Diese Adressen werden nun entpackt und gegen die IPv4-Blocklist überprüft, wodurch die Lücke über alle SSRF-geschützten Pfade geschlossen wird (Web Fetch, HTTP-Tool, Connectors, RAG-Importer).
- **MCP SSE / Streamable-HTTP Server-URLs werden nun SSRF-validiert.** Ein authentifizierter Benutzer konnte einen MCP-Server auf eine interne Adresse verweisen und eine Anfrage an ihn auslösen, wenn der Server getestet oder verbunden wurde. Server-URLs werden nun bei der Erstellung/Aktualisierung und erneut bei der Verbindung validiert.

### Behoben

- **Enforcement Hooks schlagen jetzt fehl, wenn sie abstürzen.** Ein `PRE_TOOL_USE` Hook (z. B. ein Approval Gate), der eine Exception auslöste, gab zuvor `allow=True` zurück, sodass der blockierte Tool Call trotzdem fortgesetzt wurde — ein abgestürztes Gate wurde stillschweigend zu einem Bypass. PRE Hooks lehnen jetzt standardmäßig bei Fehlern ab; nicht durchsetzende Hooks (Rate Limiter, Logger) behalten das alte Fail-Open-Verhalten über ein neues `fail_open` Flag.
