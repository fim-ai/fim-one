...
- **Das Versenden einer Nachricht in einem Chat wird jetzt sofort gestreamt statt zu stocken.** Jeder Versand blockierte bisher auf einem vollständigen Neuladen des Gesprächsverlaufs, bevor die Antwort beginnen konnte, sodass längere Chats für einen Moment eingefroren wirkten und das gesamte Transkript bei jedem Durchgang erneut abriefen. Ein abgeschlossener Durchgang wird jetzt im Hintergrund in den Verlauf integriert, sodass Versände sofort gestreamt werden und das redundante Neuladen übersprungen wird.
- **Extended Thinking funktioniert jetzt auf neueren Anthropic-Modellen (Opus 4.6+, Sonnet 4.6, Fable 5).** Diese Modelle haben den alten Parameter für festes Thinking-Budget entfernt — Opus 4.7/4.8 und Fable 5 lehnen ihn mit einem 400-Fehler ab — sodass die Konfiguration des Reasoning-Aufwands entweder zu Fehlern führte oder stillschweigend nichts bewirkte. Wir senden jetzt das Adaptive-Thinking-Protokoll für diese Modelle (und verwerfen die Sampling-Parameter, die sie nicht mehr akzeptieren), sodass der Reasoning-Aufwand korrekt wirksam wird. Ein Modell, das Adaptive Thinking nutzt, aber über einen generischen OpenAI-kompatiblen Proxy verbunden ist (wo Thinking nicht aktiviert werden kann), protokolliert jetzt eine klare Warnung statt stillschweigend zu fehlschlagen — konfigurieren Sie es als Anthropic-Provider, um Thinking zu aktivieren.
- **Force-Logout vergleicht Zeitstempel jetzt korrekt auf PostgreSQL.** Die Session-Invalidierungsprüfung nahm an, dass die gespeicherte Invalidierungszeit naiv UTC war, und erzwang sie als solche, was Vergleiche mit zeitzonenabhängigen Nicht-UTC-Werten, die PostgreSQL zurückgeben kann, stillschweigend beschädigte. Zeitstempel werden jetzt durch Konvertierung normalisiert, nicht durch Überschreibung, sodass widerrufene Token auf jedem Backend zuverlässig abgelehnt werden.
- **Enforcement Hooks schlagen jetzt geschlossen fehl, wenn sie abstürzen.** Ein `PRE_TOOL_USE` Hook (z. B. ein Approval Gate), der eine Ausnahme auslöste, gab bisher `allow=True` zurück, sodass der blockierte Tool-Aufruf trotzdem fortgesetzt wurde — ein abstürzender Gate wurde stillschweigend zu einem Bypass. PRE Hooks verweigern jetzt standardmäßig bei Fehler; Nicht-Enforcement Hooks (Rate Limiter, Logger) behalten das alte Fail-Open-Verhalten über ein neues `fail_open` Flag bei.
- **Agent-Antworten verlieren jetzt nicht mehr die Schlussfolgerungen des Reasoning.** Wenn ein Reasoning-Modell bereits eine endgültige Antwort produziert hatte, führte die Pipeline einen zweiten, verlustbehafteten Syntheseaufruf durch, der die eigenen Schlussfolgerungen des Modells verwerfen konnte — und Reasoning-Details gingen weiter verloren, wenn Gespräche komprimiert, gegen das Kontextlimit geschützt oder durch mehrstufige Pläne weitergeleitet wurden. Die Schleife streamt jetzt ihre echte endgültige Antwort wörtlich, und Reasoning wird durch Komprimierung, Kontextwiederaufbau und Sub-Agent-Schritte beibehalten, sodass die Antwort, die Sie sehen, die vollständige Gedankenkette widerspiegelt statt einer verwässerten Umformulierung.
