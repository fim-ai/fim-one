...
- **Jedes Tool in einer Konversation arbeitet jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent listete eine Datei auf, wurde dann mitgeteilt, dass sie nicht existiert, und verschwendete sein Iterations-Budget bei der Suche danach. Ausgelagerte Dateien befinden sich jetzt in der Konversations-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Konversationen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt keine endlose Kette von Zeiger-Dateien mehr.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst die Auslagerschwelle überschreiten und als neue Datei gespeichert werden, die auf bereits auf der Festplatte vorhandene Inhalte verwies, was den Agent in Kreise schickte. Workspace-Lesevorgänge werden jetzt mit einer exakten Wiederaufnahmzeile begrenzt, anstatt erneut ausgelagert zu werden.
...
- **Ein Tool-Call, der durch das Ausgabelimit des Modells unterbrochen wird, wird nicht mehr als abgeschlossene Arbeit akzeptiert.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabe-Budget mitten im Call erschöpfte, wurde der halb geschriebene Call stillschweigend verworfen und der Einleitungstext des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und dem Agent wird mitgeteilt, die Arbeit in kleinere Calls aufzuteilen.

- **Ein Ziel, das der Agent sicher verfehlt hat, wird jetzt erneut versucht, anstatt den Lauf zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Läufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Verifizierung durch, bevor er antwortet; diese Verifizierung wurde wörtlich zu Ihnen gestreamt, sodass Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" ankamen, anstatt der Antwort selbst. Die Prüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich den vorherigen Token.** JWT `exp` hat nur Sekunden-Genauigkeit, daher codierten zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgestellt wurden, früher identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Refresh-Token konnte auf ewig erneuert werden. Jeder Refresh-Token trägt jetzt eine eindeutige `jti`, sodass sich der rotierte Token unterscheidet und der alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt an, wenn ein Agent über einen anderen Agent oder einen Workflow erreicht wird.** Ein sensibler Connector-Call, der in Chat um Bestätigung fragen würde, lief früher ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingewickelt wurde — diese Pfade starteten den Agent ohne Hooks. Beide führen jetzt die eigenen Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Früher führte jeder andere Wert als `ssl` oder `tls` (z. B. `SMTP_SSL=true`) zu einer unverschlüsselten Verbindung ohne Warnung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt vollständig abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
