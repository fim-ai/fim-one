...
- **Nachfolge-Vorschlagschips bleiben in der Stimme des Benutzers.** Vorgeschlagene nächste Fragen werden als etwas formuliert, das der Benutzer senden würde (Anfragen oder Fragen an den Assistenten), nicht als umgekehrte Präferenzumfragen wie „welche bevorzugst du…?".

- **Exa-Dokumentation nennt Exa nicht mehr den Standard-Suchanbieter.** Integrations- und Umgebungsvariablenseiten beschreiben Exa jetzt als optionales Backend, das Sie mit `WEB_SEARCH_PROVIDER=exa` aktivieren; der Standard-Suchpfad bleibt Jina.

- **Sidebar-Logo ist wieder ein echter Link.** Mittleres Klicken und Cmd/Ctrl-Klick öffnen das Dashboard in einem neuen Tab; Linksklick navigiert weiterhin und behält die kurze Bounce-Animation bei.
