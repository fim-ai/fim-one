...
- **Konversationsverlauf wird vor der Komprimierung als Snapshot gespeichert, und durch das Budget gekürzte Toolergebnisse werden im Workspace gespeichert.** Bevor die Komprimierung langer Konversationen den Verlauf durch eine Zusammenfassung ersetzt, wird das vollständige Transkript als JSONL-Datei in den Konversations-Workspace geschrieben; ebenso wird ein durch das aggregierte Token-Budget gekürzte Toolergebnis vollständig im Workspace gespeichert mit einem Zeiger an Ort und Stelle — in beiden Fällen können der Agent (und Sie) das Original später lesen, anstatt es zu verlieren.
- **Antworten enden nicht mehr mitten im Satz, wenn das Modell sein Ausgabelimit erreicht.** Wenn eine Antwort durch das Output-Token-Limit des Anbieters unterbrochen wird, fordert der Agent das Modell automatisch auf, dort fortzufahren, wo es stoppte (bis zu 3 Runden), und fügt die Segmente zu einer nahtlosen Antwort zusammen — sowohl in der Reasoning-Schleife als auch in der gestreamten Endantwort.
- **LLM-Aufrufe werden jetzt bei Anbieterüberlastung (HTTP 529) und Gateway-Timeout (504) wiederholt, und `Retry-After` wird beachtet.** Zuvor führte ein 529 von Anthropic sofort zum Fehler der Anfrage, es sei denn, ein Fallback-Modell war konfiguriert; jetzt wird es mit Backoff wiederholt wie 429/5xx, und wartet genau so lange, wie der Anbieter es verlangt, wenn ein `Retry-After`-Header vorhanden ist.
- **Agenten behalten während mehrstufiger Aufgaben einen sichtbaren Plan bei.** ReAct-Agenten erhalten ein `update_plan`-Tool, um ihren Plan als Checkliste zu notieren und ihn während des Fortschritts zu aktualisieren; wenn der Plan nach mehreren Toolaufrufen veraltet wird, wird die vollständige Checkliste als Erinnerung erneut eingefügt — so bleiben lange Aufgaben auch nach Kontextkomprimierung auf Kurs. Standardmäßig aktiviert (`REACT_PLAN_TOOL_ENABLED`); DAG-Unterschritte und werkzeuglose Agenten überspringen dies.

- **Agenten können jetzt einen Workflow während einer Konversation ausführen.** Aktive, genehmigungsfreie Workflows, die Sie besitzen oder abonniert haben, werden dem Agenten als `run_workflow`-Tool angeboten — in normalem Chat und mit ausgewähltem Agenten gleichermaßen — sodass eine Aufgabe einen deterministischen, wiederholbaren Unterprozess an einen Workflow übergeben und dessen Ausgaben zurückerhalten kann. Jeder solche Durchlauf wird in der Workflow-Ausführungsverlauf aufgezeichnet und seine Token-Nutzung wird gegen das Kontingent des Workflow-Besitzers angerechnet (der Durchlauf wird abgelehnt, wenn der Besitzer bereits über dem Kontingent liegt). Workflows mit einem Human Intervention-Schritt bleiben nur auslösbar (von der Workflows-Seite oder ihrem API-Schlüssel), da sie lange blockieren können.
- **Zwei neue Workflow-Starter-Vorlagen: Connector Action und MCP Tool Call.** Der Katalog „Create from Template" zeigt jetzt die Connector- und MCP-Knoten — rufen Sie ein externes System oder ein MCP-Tool auf, fassen Sie das Ergebnis dann mit einem LLM zusammen. Wählen Sie den spezifischen Connector/Server nach der Erstellung aus.
