...
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Tasks gestartet, auf die die Runtime nur schwach verwiesen, sodass sie unter Last während des Flugs garbage-collected werden konnten und stillschweigend stoppten. Sie werden nun bis zur Fertigstellung beibehalten (und ihre Fehler protokolliert), sodass sie zuverlässig abgeschlossen werden.
- **Große Datei-Uploads blockieren andere Anfragen nicht mehr.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptevent-Schleife und fror kurzzeitig alle gleichzeitigen Anfragen ein, während eine große Datei geparst wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Multi-Step-(DAG-)Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Rechercheschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 erscheinen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine Raw-Tool-Ausgabe als autoritative Quellenbelege, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn dies nicht der Fall ist).
