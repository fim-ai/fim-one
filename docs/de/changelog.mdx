...
- **Agenten können während der Ausführung Verständnisfragen mit mehreren Antwortmöglichkeiten stellen.** Wenn eine Aufgabe in verschiedene Richtungen gehen könnte, pausiert der Agent und präsentiert bis zu vier strukturierte Fragen im Chat, jeweils mit 2–4 Optionen plus einem Freitextfeld „Sonstiges"; das Beantworten setzt die Ausführung fort, und das Überspringen oder ein Timeout lässt den Agent nach eigenem Ermessen weitermachen.

- **Agent-Schritte tragen Live-Titel in einer Zeile.** Während ein Agent arbeitet, erhält jede Iteration ein kurz generiertes Label („Preisdaten werden überprüft…"), das in einem einzelnen eingeklappten Header angezeigt wird, der zum neuesten Schritt wechselt; das Erweitern zeigt die vollständige Schritt-Chronologie, und die Labels bleiben in der Gesprächshistorie erhalten.

- **Die Modelle eines Anbieters können in Masse gelöscht werden.** Jede Modellzeile in Admin → Models trägt ein Kontrollkästchen, der Header wählt die gesamte Liste aus, und das Halten von Shift beim Klicken erweitert die Auswahl über einen Bereich von Zeilen. Das Löschen der Auswahl entfernt jedes ausgewählte Modell in einer Anfrage, nach einer Bestätigung mit der Anzahl.

- **Kleine Vergleichstabellen in Chat-Antworten werden nun als Kartenlayouts dargestellt.** Eine Tabelle mit 2–3 Spalten, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (entsprechend dem Vergleichskarten-Stil, der von KI-Chat-UIs populär gemacht wurde); größere Tabellen behalten das klassische Raster.

- **Chat-Antworten rendern nun Mermaid-Diagramme und SVG-Abbildungen.** Ein `mermaid`-Codeblock wird zu einem Design-Diagramm und ein `svg`-Block zu einem Bild mit einem Quellen-Toggle und Download; beide fallen auf einfachen Code zurück, wenn die Quelle unvollständig ist oder nicht analysiert wird, sodass ein halb gestreamtes Diagramm niemals einen Fehler anzeigt.

- **Antworten, Codeblöcke und Tabellen können kopiert werden.** Jede Antwort hat eine Aktion zum Kopieren als Markdown, Codeblöcke tragen einen Header mit ihrer Sprache, und Tabellen bieten Kopieren als Markdown sowie einen CSV-Download (UTF-8, öffnet sich sauber in Excel).

- **Zitate zeigen, aus welchem Dokument sie stammen.** Ein Zitiermarker wird nun als Pille mit dem Namen des Quelldokuments dargestellt, wobei benachbarte Marker in einer Pille zusammengefasst und gezählt werden; das Popover listet jede Quelle dahinter auf.

- **`/clear` funktioniert in der Chat-Eingabe.** Die Eingabe von `/clear` startet ein neues Gespräch, identisch mit dem Klick auf „Neuer Chat"; ein laufender Zug wird zuerst gestoppt.
...
- **Einfache Chats bieten immer Folgefragen an.** Unterhaltungen ohne einen gebundenen Agent erhalten nach jeder Antwort vorgeschlagene nächste Fragen; Agent-gebundene Chats erfordern weiterhin den Umschalter „Folgefragen" pro Agent (standardmäßig aus). Abschließende Turns wie Grüße oder Dankbarkeit können keine Chips zurückgeben.

- **Eine neu gesendete Nachricht beginnt oben auf dem Bildschirm.** Das Transkript behält gerade genug Platz unter dem aktuellen Turn, damit seine erste Zeile oben ankommt, sodass die Frage sichtbar bleibt und die Antwort in den Platz darunter wächst, anstatt dass der gesamte Turn an der unteren Kante klebt. Der Platz verschwindet, sobald der Turn einen Bildschirm übersteigt.

- **Reasoning-Blöcke bleiben während eines Turns zusammengeklappt.** Reasoning in ReAct- und DAG-Ausgaben zeigt jetzt standardmäßig eine einzeilige Vorschau an, anstatt sich beim Streaming zu erweitern, wodurch das Ergebnis sichtbar bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDF- und Word-Dateien sind ordnungsgemäß für Chinesisch gesetzt.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, anstatt eine zu referenzieren, was den zusammengefallenen Abstand im englischen Text und Aufzählungszeichen behebt, die als nicht verwandte chinesische Zeichen dargestellt wurden, und Fettdruck funktioniert jetzt. Word-Exporte deklarieren eine Ostasien-Schriftart, sodass gemischter chinesisch/englischer Fetttext nicht mehr halb-fett dargestellt wird, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften in einer Antwort der Struktur des Transkripts unterordnet.

- **Agents bleiben bei langen Aufgaben im Plan.** Der Agent wird jetzt daran erinnert, den Ansatz zu ändern, wenn er dasselbe Tool erfolglos wiederholt, wird angestoßen, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifizierungsdurchgang.

- **Gestreamte Antworten werden sanft eingeleitet, anstatt zu erscheinen.** Neu angekommene Blöcke verblassen kurz beim Streaming, wobei die Einstellung für reduzierte Bewegung des Lesers berücksichtigt wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenaufzählungszeichen sitzen hinter dem Text, anstatt damit zu konkurrieren.

- **Block-Aktionen bleiben aus dem Weg, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hover angezeigt und teilen sich eine konsistente Behandlung; sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste hält Chats jetzt zusammen.** Die Gesprächsliste sitzt direkt unter „Neuer Chat" und „Suchen", und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare Hierarchie von oben nach unten.

- **Reasoning ist standardmäßig zusammengeklappt.** Thinking-Abschnitte in Live-Läufen und vergangenen Unterhaltungen werden zu einer einzeiligen Vorschau zusammengeklappt und erweitern sich beim Klick; Reasoning wird weiterhin offen gestreamt, während es generiert wird, wird dann zusammengeklappt, wenn der Schritt abgeschlossen ist.

- **Listenseiten bringen ihre Karten sanft an ihren Platz.** Agents, Connectors, Knowledge Bases, Skills, Workflows, Tools, Conversations, Artifacts und der Markt staffeln ihre Karten beim ersten Laden gestaffelt ein, anstatt alle auf einmal zu erscheinen. Filterung und Paginierung bleiben sofort, sodass das Tippen in ein Suchfeld die Eingabe nie wiederholt.
