...
- **README Star History Chart wird wieder angezeigt.** Einbettungen von Drittanbietern (`api.star-history.com`, starchart.cc, …) funktionieren nicht mehr, nachdem GitHub die Stargazers-API eingeschränkt hat (Jul 2026). Das Diagramm ist jetzt ein selbstverwaltetes Light/Dark-SVG-Paar im Repository (folgt GitHubs Farbschema) und wird zweimal täglich von GitHub Actions automatisch aktualisiert (`scripts/gen_star_history.py`).

- **Refresh-Token-Rotation invalidiert jetzt tatsächlich den vorherigen Token.** JWT `exp` hat nur Sekundenpräzision, daher codierten zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Refresh-Token konnte unbegrenzt erneuert werden. Jeder Refresh-Token trägt jetzt eine eindeutige `jti`, sodass sich der rotierte Token unterscheidet und der alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt an, wenn ein Agent über einen anderen Agent oder einen Workflow erreicht wird.** Ein vertraulicher Connector-Aufruf, der normalerweise eine Bestätigung im Chat anfordern würde, lief zuvor ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingebunden wurde — diese Pfade starteten den Agent ohne Hooks. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Zuvor führte jeder andere Wert als `ssl` oder `tls` (z. B. `SMTP_SSL=true`) zu einer unverschlüsselten Verbindung ohne Warnung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.

### Entfernt

- **Wissensdatenbanken, die zuvor für eine Organisation freigegeben waren, werden beim Upgrade zurückgerufen.** Die Ausnahmeregelung aus v0.8.9, die bereits veröffentlichte Wissensdatenbanken für ihre Abonnenten erreichbar hielt, entfällt: Jede freigegebene Wissensdatenbank wird auf persönlich zurückgesetzt, ihre Abonnements werden entfernt, und die verbleibende Aktion zum Aufheben der Veröffentlichung sowie die Wissensdatenbank-Badges für Abonnenten verschwinden. Eine Wissensdatenbank erreicht andere Benutzer jetzt nur noch über einen freigegebenen Agent, der den Inhalt des Eigentümers zur Laufzeit delegiert.
