...
- **Agent-Schritte tragen Live-Einzeilentitel.** Während ein Agent arbeitet, erhält jede Iteration ein kurz generiertes Label („Preisdaten werden überprüft…"), das in einem einzelnen eingeklappten Header angezeigt wird, der zum neuesten Schritt wechselt; beim Erweitern wird die vollständige Schritt-Zeitleiste angezeigt, und die Labels bleiben in der Gesprächshistorie erhalten.

- **Modelle eines Anbieters können in Massen gelöscht werden.** Jede Modellzeile in Admin → Models trägt ein Kontrollkästchen, der Header wählt die gesamte Liste aus, und das Halten von Shift beim Klicken erweitert die Auswahl über mehrere Zeilen. Das Löschen der Auswahl entfernt jedes ausgewählte Modell in einer Anfrage, nach einer Bestätigung, die die Anzahl angibt.

- **Kleine Vergleichstabellen in Chat-Antworten werden nun als kartenähnliche Layouts dargestellt.** Eine Tabelle mit 2–3 Spalten, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (im Stil der Vergleichskarten, die von KI-Chat-UIs populär gemacht wurden); größere Tabellen behalten das klassische Gitter.

- **Chat-Antworten rendern nun Mermaid-Diagramme und SVG-Abbildungen.** Ein `mermaid`-Codeblock wird zu einem thematisierten Diagramm und ein `svg`-Block wird zu einem Bild mit einem Quellumschalter und Download; beide fallen auf einfachen Code zurück, wenn die Quelle unvollständig ist oder nicht analysiert wird, sodass ein halb gestreamtes Diagramm niemals einen Fehler anzeigt.
...
- **Zitate zeigen, aus welchem Dokument sie stammen.** Ein Zitiermarker wird nun als Pille dargestellt, die den Namen des Quelldokuments trägt, wobei benachbarte Marker in einer Pille zusammengefasst und gezählt werden; das Popover listet jede Quelle dahinter auf.

- **`/clear` funktioniert in der Chat-Eingabe.** Das Eingeben von `/clear` startet ein neues Gespräch, identisch mit dem Klicken auf „Neuer Chat"; ein laufender Zug wird zuerst gestoppt.
