...
- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als "Knowledge Base" gekennzeichnet, da das Abruftool den Ursprungsdateinamen verwarf; es meldet jetzt die Quelle und stimmt damit überein, was die Grounding-Pipeline bereits tat.

- **Lange Konversationen treffen weniger wahrscheinlich auf harte Kontextüberlauffehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unterhalb des Eingabelimits des Modells ausgelöst, anstatt genau darauf, und der Start protokolliert eine Warnung, wenn das Fenster des schnellen Modells zu klein ist, um das Kontextbudget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVGs gehen von einer hellen Leinwand aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, anstatt sich dunkel-auf-dunkel zu verwandeln.

- **Der Busy-Indikator des Browser-Tabs verschwindet nicht mehr während eines Durchlaufs.** Das Favicon zeigt jetzt ein durchgehendes Busy-Glyphe für den gesamten Durchlauf und stellt das echte Symbol zuverlässig wieder her, wenn der Durchlauf endet, auch nach In-App-Navigation.

- **Agenten rufen einen Endpunkt nach einem abgekürzten Ergebnis nicht mehr dutzende Male ab.** Abkürzungsmitteilungen beginnen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie, und ein Agent, der denselben Endpunkt wiederholt mit unterschiedlichen Parametern aufruft, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Durchlauf meldet jetzt einen Fehler statt eines grünen "fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshäkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und erscheinen als Fehlerbenachrichtigung.

- **Das Wiederherstellen einer Verbindung zu einem Stream wiederholt nicht mehr die Ausgabe, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse statt des gesamten Durchlaufs wiedergegeben werden.

- **Gestreamte Ereignisse, die über Netzwerk-Chunks verteilt sind, gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teile ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.
...
- **Ein hängender Tool kann einen Durchlauf nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Wanduhrzeitlimit (Standard 180s), sodass ein blockierter Connector oder nicht reagierender Server den Schritt mit einer klaren Meldung beendet, anstatt den Durchlauf zu blockieren.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen abgelehnt, anstatt das gesamte Iterationsbudget zu verbrauchen.

- **Lange Konversationen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Durchlauf stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und spätere Ergebnisse ist ein Mindestfenster garantiert, anstatt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Da Tools ein gemeinsames Dateisystem nutzen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungsdurchlaufs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Bookkeeping-Dateien sind jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Konversation funktioniert jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann wird ihm gesagt, dass sie nicht existiert, und verbraucht sein Iterationsbudget bei der Suche danach. Ausgelagerte Dateien befinden sich jetzt in der Konversations-Sandbox, die jedes Tool (einschließlich Exec-Container) bereitstellt; Dateien aus älteren Konversationen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte verweist, die bereits auf der Festplatte waren, was den Agenten in Kreise schickte. Workspace-Lesevorgänge werden jetzt mit einer genauen Resume-Zeile begrenzt, anstatt erneut ausgelagert zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, anstatt den Funktionsaufrufskanal zu verwenden, erreichte das rohe Markup die Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells unterbrochen wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der einleitende Text des Agenten — "jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.

- **Ein Ziel, das der Agent sicher verpasst hat, wird jetzt erneut versucht, anstatt den Durchlauf zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres "die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Durchläufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.
...
- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` ist nur Sekundenpräzision, daher zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgestellt wurden, verwendeten früher identische Codierung — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt jetzt einen eindeutigen `jti`, sodass sich das rotierte Token unterscheidet und das alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt, wenn ein Agent über einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat um Bestätigung bitten würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT` Node eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.
