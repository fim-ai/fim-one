...
- **Agents arbeiten nicht mehr, indem sie ihre eigenen Notizen über Python ausdrucken.** Die Effizienzanleitung, die das Schreiben eines umfassenden Skripts pro Tool-Aufruf empfahl, ist jetzt auf Aufgaben beschränkt, die echte Code-Ausführung benötigen — Wissensfragen, Vergleiche und Analysen, die das Modell direkt erzeugen kann, werden direkt beantwortet, statt durch unnötige Sandbox-Skripte geleitet zu werden.
- **Ein Agent-Schritt in einem Workflow führt jetzt den vollständigen Agent aus, nicht nur seinen Prompt.** Ein Workflow-Agent-Knoten wurde früher nur mit den Agent-Anweisungen auf einem generischen Modell und Standard-Toolset ausgeführt — wobei die Connectors, Knowledge Bases, MCP-Server und das konfigurierte Modell des Agents stillschweigend ignoriert wurden. Er führt jetzt den Agent genau wie im Chat aus, mit allen gebundenen Ressourcen und Modellen, und nur wenn dieser Agent für den Workflow-Besitzer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft jetzt deren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Connector-/MCP-Anmeldedaten pro Benutzer, die Sie dafür gespeichert haben, werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden oder auf die Anmeldedaten des Besitzers zurückgreifen können. Ressourcen, die Sie über den Market abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt jetzt konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" unterschieden sich früher — jeder bereinigt einen anderen Satz von Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad ließ API-Schlüssel, Anmeldungsverlauf, Eval-Daten, Call-Logs und Abonnements zurück. Beide durchlaufen jetzt eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird jetzt mit einer klaren Meldung abgelehnt, um zuerst den Besitz zu übertragen (oder die Organisation zu löschen), statt unerwartet zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agents bricht jetzt keine vergangenen Gespräche.** Gespräche, die auf den Agent verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agent-Link gelöscht, statt das Löschen zu blockieren.
- **Connector-Aktionen, die Bestätigung erfordern, können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Connector-Aktion, die als bestätigungspflichtig gekennzeichnet ist, wurde früher in einem Workflow mit der Genehmigungspforte stillschweigend umgangen. Der Schritt schlägt jetzt geschlossen fehl mit einer Meldung, einen Human Intervention-Knoten davor zu platzieren (die Workflow-native Genehmigungspforte) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Besitzers.** Wenn ein MCP-Server eines Workflows das Fallback auf Besitzer-Anmeldedaten deaktiviert hatte, fiel ein Nicht-Besitzer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Besitzers zurück, statt gestoppt zu werden. Der Schritt schlägt jetzt mit einer klaren Meldung fehl, Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Connector-Aufrufe aus Workflows werden jetzt im Audit-Log erfasst.** Ein Connector-Aktion, die in einem Workflow ausgeführt wurde, hinterließ keinen Eintrag im Connector-Call-Log (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben jetzt die gleiche Audit-Spur.
- **Sie können jetzt einen abonnierten Connector, eine Knowledge Base oder einen MCP-Server an einen Agent binden.** Das Binden erforderte früher, dass Sie die Ressource *besaßen*, sodass ein Connector, den Sie legitim abonniert hatten (Org-gemeinsam oder Market-installiert), als „nicht besessen" abgelehnt wurde. Das Binden folgt jetzt der gleichen Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Schritte können nur Connectors aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Connector-Schritt eines Workflows führte früher jeden Connector aus, dessen ID er referenzierte, solange dieser Connector existierte; er erzwingt jetzt die Sichtbarkeit des Ausführenden (besessen oder abonniert), sodass ein gemeinsamer Workflow keinen Connector erreichen kann, auf den Sie keinen Zugriff haben, nur durch Referenzierung seiner ID.
- **Die Knowledge-Base-Suche funktioniert jetzt gegen KBs, die Sie abonniert haben, nicht nur solche, die Sie besitzen.** Die Daten einer KB befinden sich unter dem Speicher ihres Besitzers, aber ein Workflow-Knowledge-Base-Schritt (und Knowledge-Base-Chat ohne Agent) suchte früher unter dem eigenen Speicher des Aufrufers — sodass die Suche in einer mit Ihnen geteilten KB stillschweigend nichts zurückgab. Der Besitzer wird jetzt aus der KB selbst aufgelöst und die Suche wird zugriffsgeprüft (Besitzer oder Abonnent), sodass gemeinsame KBs Ergebnisse zurückgeben und KBs, die Sie nicht sehen können, nichts zurückgeben, statt eines verwirrenden leeren Treffers.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Laufzeit nur schwach verwies, sodass sie unter Last während des Flugs Garbage-collected werden konnten und stillschweigend stoppten. Sie werden jetzt bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), sodass sie zuverlässig fertig werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife, wobei kurzzeitig alle gleichzeitigen Anfragen eingefroren wurden, während eine große Datei analysiert wurde. Die Extraktion wird jetzt an einen Worker-Thread ausgelagert.
- **Multi-Step-(DAG-)Antworten stellen ihre Quellen nicht mehr falsch dar.** Wenn ein Plan-Forschungsschritt eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet sein, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält jetzt seine rohe Tool-Ausgabe als autoritative Quellenbeweise, die die endgültige Synthese und der Evaluator gegen die Antwort prüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerungen an, die nicht bedeuteten, was sie implizierten.** Er hatte ein hardcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Toggle, der ein internes (automatisch abbaubares) Konzept offenlegte. Beide werden entfernt — der Knoten führt jetzt immer Ihr systemkonfiguriertes Modell aus (Model Provider, mit Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die darauf hinweist, wo das Modell konfiguriert ist.
