...
- **Architektur-Dokumentation ordnet Graph Engineering FIM One zu.** [Planning Landscape](/architecture/planning-landscape) erklärt Loop vs. Graph Engineering, wie sich das von klassischen visuellen Workflows unterscheidet, wie es sich auf Workflow / DAG / ReAct abbildet, und was man von jedem Engine übernehmen kann. Es besagt auch die aktuelle Tatsache, dass jeder DAG-Schritt bereits ein vollständiger ReAct-Agent ist (kein One-Shot-LLM-Aufruf). ReAct- und DAG-Engine-Seiten verlinken darauf.

- **Einfache Chats bieten immer Folgefragen an.** Unterhaltungen ohne gebundenen Agent erhalten nach jeder Antwort vorgeschlagene nächste Fragen; an Agents gebundene Chats erfordern weiterhin den Toggle „Follow-up Suggestions" pro Agent (Standard aus). Abschlusssätze wie Grüße oder Dankbarkeit können keine Chips zurückgeben.

- **Eine neu gesendete Nachricht beginnt oben auf dem Bildschirm.** Das Transkript behält gerade genug Platz unter dem aktuellen Zug, damit seine erste Zeile oben ankommt, sodass die Frage in Sicht bleibt und die Antwort in den Platz darunter wächst, anstatt dass der ganze Zug an der unteren Kante klebt. Der Platz verschwindet, sobald der Zug einen Bildschirm übersteigt.

- **Reasoning-Blöcke bleiben eingeklappt, während ein Zug läuft.** Reasoning in ReAct- und DAG-Ausgabe zeigt jetzt standardmäßig eine einzeilige Vorschau statt sich beim Streaming zu erweitern, wodurch das Ergebnis in Sicht bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDFs und Word-Dateien sind korrekt für Chinesisch gesetzt.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, anstatt auf eine zu verweisen, was den zusammengefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als nicht verwandte chinesische Zeichen gerendert wurden, und Fett funktioniert jetzt. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer fetter Text nicht mehr halb-fett herauskommt, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften in einer Antwort der eigenen Struktur des Transkripts unterordnet.

- **Agents bleiben bei langen Aufgaben im Plan.** Der Agent wird jetzt daran erinnert, den Ansatz zu ändern, wenn er das gleiche Tool fruchtlos wiederholt, wird angestoßen, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifizierungsdurchgang.

- **Gestreamte Antworten werden sanft eingeleitet statt zu erscheinen.** Neu angekommene Blöcke verblassen kurz beim Streaming, wobei die Voreinstellung für reduzierte Bewegung beachtet wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenaufzählungszeichen sitzen hinter dem Text statt mit ihm zu konkurrieren.
...
- **Die Seitenleiste hält Chat jetzt zusammen.** Die Gesprächsliste sitzt direkt unter „Neuer Chat" und „Suchen", und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare von oben nach unten Hierarchie.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Läufen und früheren Unterhaltungen werden zu einer einzeiligen Vorschau eingeklappt und beim Klick erweitert; Reasoning wird weiterhin offen gestreamt, während es generiert wird, wird dann eingeklappt, wenn der Schritt abgeschlossen ist.

- **List-Seiten schieben ihre Karten sanft an Ort und Stelle.** Agents, Connectors, Knowledge Bases, Skills, Workflows, Tools, Unterhaltungen, Artifacts und der Markt staffeln ihre Karten beim ersten Laden statt alle auf einmal zu erscheinen. Filterung und Paginierung bleiben sofort, sodass das Tippen in ein Suchfeld die Einfahrt nie wiederholt.
