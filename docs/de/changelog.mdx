...
- **Modelle eines Anbieters können in Massen gelöscht werden.** Jede Modellzeile in Admin → Models trägt ein Kontrollkästchen, der Header wählt die ganze Liste aus, und das Halten von Shift beim Klicken erweitert die Auswahl über einen Bereich von Zeilen. Das Löschen der Auswahl entfernt jedes gewählte Modell in einer Anfrage, nach einer Bestätigung, die die Anzahl nennt.

- **Kleine Vergleichstabellen in Chat-Antworten werden nun als Kartenlayouts dargestellt.** Eine 2–3-spaltige Tabelle, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (passend zum Vergleichskartenstil, der von KI-Chat-UIs populär gemacht wurde); größere Tabellen behalten das klassische Gitter.

- **Chat-Antworten rendern nun Mermaid-Diagramme und SVG-Abbildungen.** Ein `mermaid`-Codeblock wird zu einem thematisierten Diagramm und ein `svg`-Block wird zu einem Bild mit einem Quellumschalter und Download; beide fallen auf einfachen Code zurück, wenn die Quelle unvollständig ist oder nicht geparst wird, sodass ein halb gestreamtes Diagramm niemals einen Fehler anzeigt.

- **Antworten, Codeblöcke und Tabellen können kopiert werden.** Jede Antwort hat eine Aktion zum Kopieren als Markdown, Codeblöcke tragen einen Header mit ihrer Sprache, und Tabellen bieten Kopieren als Markdown plus einen CSV-Download (UTF-8, öffnet sich sauber in Excel).
...
- **`/clear` funktioniert in der Chat-Eingabe.** Die Eingabe von `/clear` startet eine neue Konversation, identisch mit dem Klick auf Neuer Chat; ein laufender Zug wird zuerst gestoppt.
...
- **Exportierte PDFs und Word-Dateien sind für Chinesisch korrekt formatiert.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, statt nur auf eine zu verweisen, was den Abstand in englischem Text behebt und Aufzählungszeichen, die als unzusammenhängende chinesische Zeichen dargestellt wurden, korrigiert. Fettdruck funktioniert jetzt. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer Fettdruck nicht mehr halb-fett wirkt. Beide Formate verwenden eine fünfstufige Größenskala, die Überschriften in einer Antwort der Struktur des Transkripts unterordnet.

- **Agenten bleiben bei langen Aufgaben im Plan.** Der Agent wird jetzt daran erinnert, seinen Ansatz zu ändern, wenn er dasselbe Tool erfolglos wiederholt, wird ermutigt, einen Plan zu schreiben, wenn er ohne arbeitet, und kann keine endgültige Antwort liefern, während noch Planpunkte offen sind, ohne einen expliziten Verifizierungsdurchlauf.

- **Gestreamte Antworten blenden sich ein statt zu erscheinen.** Neu angekommene Blöcke blenden sich kurz ein, während sie streamen, wobei die Einstellung für reduzierte Bewegung des Lesers berücksichtigt wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Aufzählungszeichen sitzen hinter dem Text statt mit ihm zu konkurrieren.

- **Block-Aktionen bleiben verborgen, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Überfahren mit der Maus angezeigt und verwenden eine konsistente Behandlung; sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste hält Chats jetzt zusammen.** Die Gesprächsliste sitzt direkt unter „Neuer Chat" und „Suchen", und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare von oben nach unten verlaufende Hierarchie.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Läufen und früheren Gesprächen werden zu einer einzeiligen Vorschau zusammengeklappt und erweitern sich beim Anklicken; Reasoning streamt während der Generierung offen, wird dann eingeklappt, wenn der Schritt abgeschlossen ist.
