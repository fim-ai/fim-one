...
- **Fortgesetzte DAG-Ausführungen verlieren nicht mehr das Ende langer Schrittausgaben.** Schrittbeweise, die im Crash-Resume-Checkpoint gespeichert waren, wurden nur am Anfang bei der Größenbeschränkung gekürzt und verwarfen stillschweigend alles am Ende der Ausgabe — genau dort, wo Schlussfolgerungen, Prüfsummen und letzte Zeilen zu finden sind. Begrenzte Beweise behalten nun sowohl den Anfang als auch das Ende mit einem expliziten Auslassungsmarker dazwischen.
- **Intelligente Agenten funktionieren nicht mehr, indem sie ihre eigenen Notizen über Python ausdrucken.** Die Effizienzanleitung, die das Schreiben eines umfassenden Skripts pro Toolaufruf empfahl, ist nun auf Aufgaben beschränkt, die echte Code-Ausführung benötigen — Wissensfragen, Vergleiche und Analysen, die das Modell direkt erzeugen kann, werden direkt beantwortet, anstatt sie durch unnötige Sandbox-Skripte zu leiten.
- **Ein Agentenschritt innerhalb eines Workflows führt nun den vollständigen Agenten aus, nicht nur seine Anleitung.** Ein Workflow-Agent-Knoten wurde früher nur mit den Anweisungen des Agenten auf einem generischen Modell und Standard-Toolset ausgeführt — wobei die Konnektoren, Wissensdatenbanken, MCP-Server und das konfigurierte Modell des Agenten stillschweigend verworfen wurden. Er führt nun den Agenten genau wie im Chat aus, mit allen gebundenen Ressourcen und dem Modell, und nur wenn dieser Agent für den Workflow-Besitzer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft nun deren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Konnektoren-/MCP-Anmeldedaten pro Benutzer, die Sie dafür gespeichert haben, werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden können oder nicht auf die Anmeldedaten des Besitzers zurückgreifen können, nachdem Sie die Organisation verlassen haben. Ressourcen, die Sie über den Markt abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt nun konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" unterschieden sich früher — jeder bereinigt eine andere Menge von Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad hinterließ API-Schlüssel, Anmeldungsverlauf, Evaluierungsdaten, Aufruflogs und Abonnements. Beide durchlaufen nun eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr auf Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Meldung abgelehnt, um zuerst das Eigentum zu übertragen (oder die Organisation zu löschen), anstatt unklar zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten unterbricht nicht mehr vergangene Gespräche.** Gespräche, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agenten-Link gelöscht, anstatt das Löschen zu blockieren.
- **Bestätigungserforderliche Konnektoraktionen können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Konnektoraktion, die als bestätigungserforderlich gekennzeichnet ist, wurde früher innerhalb eines Workflows mit dem Genehmigungstor stillschweigend umgangen. Der Schritt schlägt nun geschlossen fehl mit einer Meldung, einen Human-Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungstor) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Besitzers.** Wenn der MCP-Server eines Workflows das Fallback für Besitzer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Besitzer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Besitzers zurück, anstatt gestoppt zu werden. Der Schritt schlägt nun mit einer klaren Meldung fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Konnektoraufrufe aus Workflows werden nun im Audit-Log erfasst.** Ein Konnektoraktion, die innerhalb eines Workflows ausgeführt wurde, hinterließ keinen Eintrag im Konnektoraufrufs-Log (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun denselben Audit-Trail.
- **Sie können nun einen abonnierten Konnektor, eine Wissensdatenbank oder einen MCP-Server an einen Agenten anhängen.** Die Bindung erforderte früher, dass Sie die Ressource *besaßen*, sodass ein Konnektor, den Sie legitim abonniert hatten (Org-freigegeben oder Markt-installiert), als „nicht im Besitz" abgelehnt wurde. Die Bindung folgt nun der gleichen Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Schritte können nur Konnektoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Konnektorschritt eines Workflows konnte früher jeden Konnektor ausführen, dessen ID er referenzierte, solange dieser Konnektor existierte; er erzwingt nun die Sichtbarkeit des Ausführenden (im Besitz oder abonniert), sodass ein freigegebener Workflow einen Konnektor, auf den Sie keinen Zugriff haben, nicht erreichen kann, indem er einfach seine ID referenziert.
- **Die Wissensdatenbank-Suche funktioniert nun gegen KBs, die Sie abonniert haben, nicht nur gegen solche, die Sie besitzen.** Die Daten einer KB befinden sich unter dem Speicher ihres Besitzers, aber ein Workflow-Wissensdatenbank-Schritt (und Wissensdatenbank-Chat ohne Agenten) suchte früher unter dem eigenen Speicher des Aufrufers — sodass die Suche in einer mit Ihnen geteilten KB stillschweigend nichts zurückgab. Der Besitzer wird nun aus der KB selbst aufgelöst und die Suche wird zugriffsgeprüft (Besitzer oder Abonnent), sodass freigegebene KBs Ergebnisse zurückgeben und KBs, die Sie nicht sehen können, nichts zurückgeben, anstatt eines verwirrenden leeren Treffers.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Laufzeit nur schwach verweist, sodass sie unter Last während des Flugs Garbage-Collection unterliegen konnten und stillschweigend stoppten. Sie werden nun bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), sodass sie zuverlässig fertig werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife und fror kurzzeitig alle gleichzeitigen Anfragen ein, während eine große Datei analysiert wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Multi-Schritt-(DAG-)Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn ein Planungsforschungsschritt eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 erscheinen konnte, oder die Schweregrad eines Elements könnte falsch gekennzeichnet sein, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Toolausgabe als autoritative Quellenevidenz, die die endgültige Synthese und der Evaluator gegen die Antwort prüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hardcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Schalter, der ein internes (automatisch herabgestuftes) Konzept offenlegte. Beide werden entfernt — der Knoten führt nun immer Ihr systemkonfiguriertes Modell aus (Model Provider, Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf die Stelle verweist, wo das Modell konfiguriert ist.
