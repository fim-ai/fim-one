...
- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener war aktiv, aber sein Dialog wurde im eingeklappten Zustand nie eingebunden, sodass der Shortcut stillschweigend nichts tat.

- **Zitate nennen das Dokument, aus dem sie stammen, wieder.** Im einfachen Abrufmodus wurde jede Quelle als "Knowledge Base" gekennzeichnet, da das Abruftool den Ursprungsdateinamen verwarf; es meldet jetzt die Quelle, was der Grounding-Pipeline entspricht.
...
- **Der Browser-Tab-Busy-Indikator verschwindet nicht mehr während eines Durchlaufs.** Das Favicon zeigt jetzt während des gesamten Durchlaufs ein stabiles Busy-Glyphen und stellt das echte Symbol zuverlässig wieder her, wenn der Durchlauf endet, auch nach In-App-Navigation.
...
- **Das Wiederverbinden mit einem Stream wiederholt keine bereits angesehene Ausgabe mehr.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse wiedergegeben werden, statt des gesamten Durchlaufs.

- **Gestreamte Ereignisse, die über Netzwerk-Chunks verteilt sind, gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade nutzen jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Das Aufrufen eines anderen Agenten gibt jetzt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher in einer rohen Ausführungsverfolgung vergraben an, was den Kontext des aufrufenden Agenten überflutete; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Durchlauf nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Echtzeit-Limit (Standard 180s), sodass ein blockierter Connector oder nicht reagierender Server den Schritt mit einer klaren Meldung beendet, statt den Durchlauf zu blockieren.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen abgelehnt, statt das gesamte Iterationsbudget zu verbrauchen.

- **Lange Konversationen verlieren ihren früheren Kontext nach Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Durchlauf stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und spätere Ergebnisse erhalten ein Mindestfenster statt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Da Tools ein gemeinsames Dateisystem nutzen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungsdurchlaufs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien werden jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Konversation funktioniert jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann wird ihm gesagt, dass sie nicht existiert, und verbraucht sein Iterationsbudget bei der Suche danach. Ausgelagerte Dateien befinden sich jetzt in der Konversations-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Konversationen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs könnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte verweist, die bereits auf der Festplatte waren, was den Agenten in Kreise schickt. Workspace-Lesevorgänge werden jetzt mit einer exakten Resume-Zeile begrenzt, statt erneut ausgelagert zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, statt den Funktionsaufrufs-Kanal zu nutzen, erreichte das rohe Markup den Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der Einleitungstext des Agenten — "jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.

- **Ein Ziel, das der Agent sicher verfehlt hat, wird jetzt erneut versucht, statt den Durchlauf zu beenden.** Die Neuplanung stoppte, wenn der Analyzer sicher war, was ein sicheres "die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Durchläufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich an Sie gestreamt, sodass Antworten als "Lass mich überprüfen: 1. Vollständig adressiert? ✅ …" statt der Antwort selbst ankamen. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` hat nur Sekunden-Präzision, sodass zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgestellt wurden, früher identisch kodiert wurden — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt jetzt eine eindeutige `jti`, sodass sich das rotierte Token unterscheidet und das alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt, wenn ein Agent durch einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat um Bestätigung fragen würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Früher fiel jeder Wert außer `ssl` oder `tls` (zum Beispiel `SMTP_SSL=true`) auf eine unverschlüsselte Verbindung ohne Warnung zurück, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
