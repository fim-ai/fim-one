...
- **Instanzbasiertes Zugangsmodell für die Abrechnung.** Administratoren wählen eine von drei Positionen: keine Abonnements (Softwarestandard, monatliches Kontingent `0` = unbegrenzt), enthalten + bezahlt oder nur bezahlt. Der enthaltene Standardplan kann nicht gelöscht werden; bezahlte Vorlagen werden nicht mehr mit einer FIM Stripe Price ID ausgeliefert.

- **GPT-5.x-Agenten behalten ihr Reasoning über Toolaufrufe hinweg.** FIM One spricht jetzt direkt die OpenAI Responses API für GPT-5.x-Modelle an und überträgt das verschlüsselte Reasoning jeder Runde in die nächste, sodass ein mehrstufiger Agent auf dem aufbaut, was er bereits erarbeitet hat, anstatt es bei jeder Tool-Runde neu abzuleiten. Reasoning-Token-Zählungen erscheinen in der Nutzung, und das Thinking-Panel zeigt die eigene Zusammenfassung des Modells. Setzen Sie `FIM_GPT5_RESPONSES_MODE=bridge` (oder `off`), um zum vorherigen Verhalten zurückzukehren; Endpunkte ohne eine `/v1/responses`-Route fallen automatisch auf Chat Completions zurück.

- **Reine Transformationsschritte in DAG-Plänen laufen als einzelner direkter LLM-Aufruf.** Der Planer kennzeichnet Merge-/Umschreib-/Zusammenfassungsschritte, die nur Upstream-Ausgaben verbrauchen, als direkte Schritte und überspringt die Tool-Schleife, die sie nie verwenden würden, was Kosten und Latenz senkt; jedes Schrittergebnis verzeichnet nun auch, wie es ausgeführt wurde (direkter Aufruf oder Agent-Schleife, Iterationen, verwendete Tools).

- **Agenten können klärende Multiple-Choice-Fragen während der Ausführung stellen.** Wenn eine Aufgabe in wesentlich unterschiedliche Richtungen gehen könnte, pausiert der Agent und präsentiert bis zu vier strukturierte Fragen im Chat, jeweils mit 2–4 Optionen plus einem Freitextfeld „Sonstiges"; das Beantworten setzt die Ausführung an Ort und Stelle fort, und das Überspringen oder ein Timeout lässt ihn nach eigenem Ermessen fortfahren.

- **Agent-Schritte tragen Live-Einzeilentitel.** Während ein Agent arbeitet, erhält jede Iteration ein kurz generiertes Label („Preisdaten überprüfen…"), das in einer einzelnen eingeklappten Kopfzeile angezeigt wird, die zum neuesten Schritt wechselt; das Erweitern zeigt die vollständige Schritt-Zeitleiste, und die Labels bleiben in der Gesprächshistorie erhalten.

- **Die Modelle eines Anbieters können in Massen gelöscht werden.** Jede Modellzeile in Admin → Modelle trägt ein Kontrollkästchen, die Kopfzeile wählt die gesamte Liste aus, und das Halten von Umschalt beim Klicken erweitert die Auswahl über einen Bereich von Zeilen. Das Löschen der Auswahl entfernt jedes gewählte Modell in einer Anfrage, nach einer Bestätigung, die die Anzahl nennt.

- **Kleine Vergleichstabellen in Chat-Antworten werden nun als kartenähnliche Layouts gerendert.** Eine Tabelle mit 2–3 Spalten, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (entsprechend dem Vergleichskartenstil, der von KI-Chat-UIs popularisiert wurde); größere Tabellen behalten das klassische Gitter.

- **Chat-Antworten rendern nun Mermaid-Diagramme und SVG-Abbildungen.** Ein `mermaid`-Codeblock wird zu einem thematischen Diagramm und ein `svg`-Block zu einem Bild mit einem Quellumschalter und Download; beide fallen auf einfachen Code zurück, wenn die Quelle unvollständig ist oder nicht geparst wird, sodass ein halb gestreamtes Diagramm nie einen Fehler anzeigt.

- **Antworten, Codeblöcke und Tabellen können kopiert werden.** Jede Antwort hat eine Aktion zum Kopieren als Markdown, Codeblöcke tragen eine Kopfzeile mit ihrer Sprache, und Tabellen bieten Kopieren als Markdown plus einen CSV-Download (UTF-8, öffnet sich sauber in Excel).

- **Zitate zeigen, aus welchem Dokument sie stammen.** Ein Zitiermarker wird nun als Pille mit dem Namen des Quelldokuments gerendert, wobei benachbarte Marker in einer Pille zusammengefasst und gezählt werden; das Popover listet jede Quelle dahinter auf.

- **`/clear` funktioniert in der Chat-Eingabe.** Das Eingeben von `/clear` startet ein neues Gespräch, identisch mit dem Klicken auf Neuer Chat; eine laufende Runde wird zuerst gestoppt.
...
- **Der Composer warnt, bevor ein Bild an ein reines Textmodell geht.** Das Anhängen eines Bildes, während das aktive Modell keine Bilder lesen kann, zeigt eine Notiz an, dass das Modell nur den Dateinamen erhält, aufgelöst vom Modell, das diese Runde tatsächlich verwenden würde.

- **Ungesendete Eingabe wird beibehalten.** Text, eingefügte Clips und fertiggestellte Anhänge, die nicht gesendet wurden, werden pro Gespräch und separat für einen neuen Chat gespeichert, sodass sie eine Aktualisierung, einen Wechsel zu einem anderen Gespräch und eine abgelaufene Sitzung überstehen. Ein Entwurf wird gelöscht, sobald die Nachricht gesendet oder ihr Gespräch gelöscht wird.
...
- **Die Callback-URL eines Feishu-Kanals kann nun die Feishu-Verifizierung bestehen.** Feishu lässt seine Signatur-Header beim einmaligen URL-Verifizierungs-Handshake weg, und der Callback-Endpunkt lehnte alles Unsignierte ab, sodass das Speichern der Anfrage-Adresse stets mit „Challenge-Code nicht zurückgegeben" fehlschlug und die Karten-Buttons des Kanals nie funktionierten. Eine unsignierte Anfrage wird nun anhand der anderen gemeinsamen Geheimnisse authentifiziert: Ihr verschlüsselter Umschlag muss sich mit dem Encrypt Key des Kanals öffnen lassen, und ihr Token muss mit dem Verification Token übereinstimmen, sofern einer konfiguriert ist. Ein unsignierter, unverschlüsselter Body wird weiterhin abgelehnt.

- **Ein Social-Login kann nicht mehr von einem Konto mit unbestätigter E-Mail-Adresse übernommen werden.** Das Verknüpfen eines Social-Logins mit einem bestehenden Konto erfordert nun, dass die Adresse auf beiden Seiten verifiziert wurde – nicht nur durch den eingehenden Anbieter. Das Anmelden mit einem Anbieter, der die Adresse nie bestätigt hat, erstellt weiterhin ein Konto, aber dieses Konto kann nicht mehr mit einer späteren verifizierten Anmeldung abgeglichen werden.

- **Eine durch das Ausgabelimit abgeschnittene Antwort führt nicht mehr die Hälfte ihrer Tool-Aufrufe aus.** Wenn das Modell das Token-Limit mitten in einer Nachricht erreicht, werden alle Tool-Aufrufe in dieser Antwort verworfen und das Modell wird aufgefordert, sie in kleineren Einheiten erneut zu senden. Zuvor wurden Aufrufe, deren Argumente zufällig vollständig waren, ausgeführt, sodass ein Batch, den das Modell als Einheit geplant hatte, nur teilweise ausgeführt werden konnte, während das Transkript so aussah, als wäre alles ausgeführt worden.

- **Das Senden während ein Anhang noch hochgeladen wird, lässt ihn nicht mehr fallen.** Das Senden wartet nun, bis der Upload abgeschlossen ist, und schließt die Datei ein; der Senden-Button zeigt dabei einen Spinner. Zuvor wurde die Nachricht ohne das Bild oder Dokument und ohne Warnung gesendet.
...
- **Vorschläge für Folgefragen bleiben in der Stimme des Nutzers.** Vorgeschlagene nächste Fragen sind so formuliert, als würde der Nutzer sie senden (Anfragen oder Fragen an den Assistenten), nicht als umgekehrte Präferenzumfragen wie „Was bevorzugen Sie…?".
...
- **Das Sidebar-Logo ist wieder ein echter Link.** Mittlere Maustaste und cmd/ctrl-Klick öffnen das Dashboard in einem neuen Tab; ein normaler Linksklick navigiert weiterhin und behält die kurze Sprung-Animation.
...
- **Gestreamte Antworten werden gleichmäßig gerendert, ohne zu flackern.** Eine wachsende Antwort wird nun Block für Block gerendert: Fertige Blöcke bleiben stabil, anstatt bei jedem Frame neu gerendert zu werden, und halb angekommene Formatierungen (Fett, Links, Mathematik) werden vervollständigt oder zurückgehalten, bis das Streaming abgeschlossen ist.

- **Endantworten werden während des Schreibens gestreamt.** Der Agent signalisiert nun, wenn er mit dem Sammeln fertig ist, und schreibt die Antwort als live gestreamten Zug – sodass lange Antworten Token für Token mit vollem Detail eintreffen, anstatt nach einer Wartezeit wiedergegeben zu werden. `REACT_FINISH_SIGNAL=false` stellt das vorherige Verhalten wieder her.

- **Architekturdokumentation bildet Graph-Engineering auf FIM One ab.** [Planning Landscape](/architecture/planning-landscape) erläutert Loop- vs. Graph-Engineering, wie sich das von klassischen visuellen Workflows unterscheidet, wie es auf Workflow / DAG / ReAct abgebildet wird und was für jede Engine zu übernehmen ist. Außerdem wird der aktuelle Stand festgehalten, dass jeder DAG-Schritt bereits ein vollständiger ReAct-Agent ist (kein One-Shot-LLM-Aufruf). Die ReAct- und DAG-Engine-Seiten verlinken darauf.

- **Einfache Chats bieten immer Folgefragen an.** Gespräche ohne gebundenen Agent erhalten nach jeder Antwort vorgeschlagene Folgefragen; agentengebundene Chats erfordern weiterhin den agentenbezogenen Schalter „Folgefragen" (standardmäßig deaktiviert). Abschlusszüge wie Begrüßungen oder Danksagungen können keine Chips zurückgeben.

- **Eine neu gesendete Nachricht beginnt oben auf dem Bildschirm.** Das Transkript lässt gerade genug Platz unterhalb des aktuellen Zuges, damit seine erste Zeile den oberen Rand erreicht – sodass die Frage sichtbar bleibt und die Antwort in den darunter liegenden Raum wächst, anstatt dass der gesamte Zug am unteren Rand klebt. Der Platz verschwindet, sobald der Zug einen Bildschirm überschreitet.

- **Reasoning-Blöcke bleiben während eines Zuges eingeklappt.** Reasoning in ReAct- und DAG-Ausgaben zeigt nun standardmäßig eine einzeilige Vorschau, anstatt sich beim Streamen zu erweitern – so bleibt das Ergebnis sichtbar; klicken Sie auf einen Block, um den vollständigen Trace zu lesen.

- **Exportierte PDFs und Word-Dateien sind für Chinesisch korrekt gesetzt.** Der PDF-Export bettet nun eine echte CJK-Schriftart ein, anstatt auf eine zu verweisen – das behebt zusammengefallene Abstände in englischem Text und Aufzählungszeichen, die als unzusammenhängende chinesische Zeichen dargestellt wurden, und Fettschrift funktioniert nun. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer Fetttext nicht mehr halb fett erscheint; beide Formate verwenden eine fünfstufige Größenskala, die Überschriften innerhalb einer Antwort der eigenen Struktur des Transkripts unterordnet.

- **Agenten bleiben bei langen Aufgaben auf Kurs.** Der Agent wird nun daran erinnert, den Ansatz zu wechseln, wenn er dasselbe Tool wiederholt erfolglos einsetzt, wird aufgefordert, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine Endantwort liefern, solange Planpunkte noch offen sind, ohne einen expliziten Verifikationsdurchlauf.

- **Gestreamte Antworten blenden sich ein, anstatt abrupt zu erscheinen.** Neu eingetroffene Blöcke blenden sich beim Streamen kurz ein und berücksichtigen dabei die Einstellung des Nutzers zur reduzierten Bewegung. Lange Tabellen heben außerdem die Zeile unter dem Cursor hervor, und Aufzählungspunkte liegen hinter dem Text, anstatt mit ihm zu konkurrieren.

- **Block-Aktionen bleiben unauffällig, bis Sie sie benötigen.** Kopieren und Herunterladen bei Antworten, Codeblöcken, Diagrammen und Tabellen sind nur als Symbole mit Tooltips sichtbar (Herunterladen immer ganz rechts), werden beim Hover angezeigt und sind per Tastatur erreichbar.

- **Die Seitenleiste hält den Chat zusammen.** Die Gesprächsliste befindet sich direkt unter „Neuer Chat" und „Suche", und die Modulnavigation wurde in ein kompaktes, getöntes Dock oberhalb des Benutzerbereichs verschoben – so erhält die Liste mehr Platz und die Seitenleiste eine klare Hierarchie von oben nach unten.

- **Reasoning ist standardmäßig eingeklappt.** Denkabschnitte in Live-Läufen und vergangenen Gesprächen werden auf eine einzeilige Vorschau reduziert und lassen sich per Klick erweitern; Reasoning wird beim Generieren weiterhin offen gestreamt und klappt sich zusammen, wenn der Schritt abgeschlossen ist.

- **Listenseiten blenden ihre Karten sanft ein.** Agenten, Connectors, Wissensdatenbanken, Skills, Workflows, Tools, Gespräche, Artefakte und der Marktplatz staffeln ihre Karten beim ersten Laden ein, anstatt alle auf einmal zu erscheinen. Filtern und Blättern bleiben sofort, sodass das Tippen in ein Suchfeld die Einblendung nie wiederholt.
