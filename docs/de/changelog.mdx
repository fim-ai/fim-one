...
- **Gestreamte Antworten werden sanft eingeblendet statt plötzlich zu erscheinen.** Neu angekommene Blöcke werden beim Streamen kurz eingeblendet und berücksichtigen dabei die Einstellung für reduzierte Bewegung des Lesers. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenpunkte befinden sich hinter dem Text statt mit ihm zu konkurrieren.

- **Block-Aktionen bleiben verborgen, bis du sie brauchst.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hovern angezeigt und folgen einer einheitlichen Gestaltung; sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste hält Chats jetzt zusammen.** Die Gesprächsliste befindet sich direkt unter „Neuer Chat" und „Suche", und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare Hierarchie von oben nach unten verleiht.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Läufen und früheren Gesprächen werden zu einer einzeiligen Vorschau zusammengeklappt und erweitern sich beim Anklicken; Reasoning wird weiterhin offen gestreamt, während es generiert wird, und wird dann eingeklappt, wenn der Schritt abgeschlossen ist.
...
- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als "Knowledge Base" gekennzeichnet, da das Abruftool den Ursprungsdateinamen verwarf; es meldet jetzt die Quelle und entspricht damit dem, was die Grounding-Pipeline bereits tat.

- **SVG-Abbildungen bleiben im Dark Mode lesbar.** Von Modellen erstellte SVGs gehen von einer hellen Leinwand aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, statt dunkel-auf-dunkel zu werden.

- **Der Browser-Tab-Beschäftigungsindikator verschwindet nicht mehr mitten im Durchlauf.** Das Favicon zeigt jetzt während des gesamten Durchlaufs ein gleichmäßiges Beschäftigungssymbol und stellt das echte Symbol zuverlässig wieder her, wenn der Durchlauf endet, auch nach In-App-Navigation.

- **Intelligente Agenten rufen einen Endpunkt nicht mehr dutzende Male nach einem abgekürzten Ergebnis ab.** Abkürzungsmitteilungen führen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie an, und ein Agent, der denselben Endpunkt immer wieder mit unterschiedlichen Parametern aufruft, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Durchlauf meldet jetzt einen Fehler statt eines grünen "Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgs-Häkchen geliefert; Fehler durchlaufen jetzt den Fehlerkanal und werden als Fehlerbenachrichtigung angezeigt.

- **Das erneute Verbinden mit einem Stream wiederholt keine bereits gesehene Ausgabe mehr.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse statt des gesamten Durchlaufs wiedergegeben werden.

- **Über Netzwerk-Chunks verteilte gestreamte Ereignisse gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade nutzen jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Der Aufruf eines anderen Agenten gibt jetzt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher in einer rohen Ausführungsverfolgung vergraben an und überflutete den Kontext des aufrufenden Agenten; er erhält jetzt nur die Antwort.
...
- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen abgelehnt, statt das gesamte Iterations-Budget zu verbrauchen.

- **Lange Konversationen verlieren ihren früheren Kontext nach Verdichtung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Durchlauf stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende der Ausgabe eines Befehls ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und spätere Ergebnisse erhalten ein garantiertes Mindestfenster statt vollständig gelöscht zu werden.
...
- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, statt den Function-Calling-Kanal zu nutzen, erreichte das rohe Markup die Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Output-Limit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Output-Budget mitten im Aufruf erschöpfte, wurde der halbgeschriebene Aufruf stillschweigend verworfen und der einleitende Text des Agenten — "jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.

- **Ein Ziel, das der Agent sicher verpasst hat, wird jetzt erneut versucht, statt den Durchlauf zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres "das Liefergut fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Durchläufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, sodass Antworten als "Lassen Sie mich überprüfen: 1. Vollständig beantwortet? ✅ …" statt der Antwort selbst ankamen. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` hat nur Sekundenpräzision, sodass zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, früher identisch kodiert wurden — Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt jetzt einen eindeutigen `jti`, sodass sich das rotierte Token unterscheidet und das alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt, wenn ein Agent über einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat eine Bestätigung anfordern würde, lief früher ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT` Node eingebunden wurde — diese Pfade starteten den Agenten ohne Hooks. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Früher fiel jeder Wert außer `ssl` oder `tls` (z. B. `SMTP_SSL=true`) auf eine unverschlüsselte Verbindung ohne Warnung durch, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
