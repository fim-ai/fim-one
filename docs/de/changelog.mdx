...
- **Die Seitenleiste stellt Navigation an erste Stelle.** Die Modulnavigation sitzt jetzt über der Gesprächsliste mit beschrifteten Abschnitten Build / Resources / Operate, statt in einem Dock am unteren Ende der Seitenleiste versteckt zu sein.

- **Gestreamte Antworten werden stabil gerendert statt zu flimmern.** Eine wachsende Antwort wird jetzt Block für Block gerendert: fertige Blöcke bleiben stabil, statt bei jedem Frame neu zu rendern, und unvollständige Formatierung (Fett, Links, Mathematik) wird abgeschlossen oder gehalten, bis sie vollständig gestreamt ist.

- **Endgültige Antworten werden gestreamt, während sie geschrieben werden.** Der Agent signalisiert jetzt, wenn er fertig ist mit dem Sammeln, und schreibt die Antwort als live gestreamten Turn, sodass lange Antworten Token für Token mit vollständigen Details ankommen, statt nach einer Wartezeit abgespielt zu werden. `REACT_FINISH_SIGNAL=false` stellt das vorherige Verhalten wieder her.

- **Architektur-Docs ordnen Graph-Engineering FIM One zu.** [Planning Landscape](/architecture/planning-landscape) erklärt Loop vs. Graph Engineering, wie sich das von klassischen visuellen Workflows unterscheidet, wie es sich auf Workflow / DAG / ReAct abbildet, und was man für jede Engine übernehmen sollte. Es stellt auch die aktuelle Tatsache fest, dass jeder DAG-Schritt bereits ein vollständiger ReAct-Agent ist (kein One-Shot-LLM-Aufruf). ReAct- und DAG-Engine-Seiten verlinken darauf.

- **Einfache Chats bieten immer Folgefragen an.** Gespräche ohne gebundenen Agent erhalten nach jeder Antwort vorgeschlagene nächste Fragen; an Agents gebundene Chats erfordern weiterhin den Pro-Agent-Schalter Follow-up Suggestions (Standard aus). Abschlusstürne wie Grüße oder Dank können keine Chips zurückgeben.

- **Eine neu gesendete Nachricht startet oben auf dem Bildschirm.** Das Transkript behält gerade genug Platz unter dem aktuellen Turn, damit seine erste Zeile oben ankommt, sodass die Frage in Sicht bleibt und die Antwort in den Platz darunter wächst, statt dass der ganze Turn an der unteren Kante klebt. Der Platz verschwindet, sobald der Turn einen Bildschirm übersteigt.

- **Reasoning-Blöcke bleiben gefaltet, während ein Turn läuft.** Reasoning in sowohl ReAct- als auch DAG-Ausgabe zeigt jetzt standardmäßig eine einzeilige Vorschau statt sich beim Streamen zu erweitern, wodurch das Ergebnis in Sicht bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Spur zu lesen.

- **Exportierte PDF- und Word-Dateien sind ordnungsgemäß für Chinesisch gesetzt.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, statt auf eine zu verweisen, was den zusammengefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als nicht verwandte chinesische Zeichen gerendert wurden, und Fett funktioniert jetzt. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer fetter Text nicht mehr halb-fett herauskommt, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften in einer Antwort der Struktur des Transkripts untergeordnet hält.

- **Agents bleiben bei langen Aufgaben im Plan.** Der Agent wird jetzt daran erinnert, seinen Ansatz zu ändern, wenn er dasselbe Tool fruchtlos wiederholt, wird angestoßen, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifizierungsdurchgang.

- **Gestreamte Antworten werden sanft eingeleitet statt zu erscheinen.** Neu angekommene Blöcke werden kurz eingefadet, während sie streamen, wobei die Einstellung für reduzierte Bewegung des Lesers beachtet wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenaufzählungszeichen sitzen hinter dem Text statt mit ihm zu konkurrieren.

- **Block-Aktionen bleiben aus dem Weg, bis Sie danach greifen.** Copy- und Export-Schaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hover angezeigt und teilen sich eine konsistente Behandlung; sie bleiben per Tastatur erreichbar.

- **Die Seitenleiste hält jetzt Chats zusammen.** Die Gesprächsliste sitzt direkt unter Neuer Chat und Suche, und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare Top-to-Bottom-Hierarchie verleiht.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in sowohl Live-Läufen als auch vergangenen Gesprächen falten sich zu einer einzeiligen Vorschau und erweitern sich beim Klicken; Reasoning streamt weiterhin offen, während es generiert wird, faltet sich dann zusammen, wenn der Schritt abgeschlossen ist.

- **List-Seiten leiten ihre Karten sanft ein.** Agents, Connectors, Knowledge Bases, Skills, Workflows, Tools, Conversations, Artifacts und der Market staffeln ihre Karten beim ersten Laden ein, statt alle auf einmal zu erscheinen. Filtern und Paging bleiben sofort, sodass das Tippen in ein Suchfeld die Einleitung nie abspielt.
