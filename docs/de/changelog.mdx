...
- **Lange werkzeugintensive Gespräche brechen nicht mehr ab, wenn der Verlauf komprimiert wird.** Die Kontextkomprimierung konnte das beibehaltene Fenster zwischen einem Werkzeugaufruf und seinen Ergebnissen unterbrechen und eine Nachrichtensequenz erzeugen, die der LLM-Anbieter ablehnt (HTTP 400) — genau bei den langen Läufen, die Komprimierung am meisten brauchen. Der Kompressor behält nun jeden Werkzeugaufruf zusammen mit seinen Ergebnissen auf der beibehaltenen Seite.
- **Hintergrund-Werkzeugergebnisse erscheinen nun im Step-Stream und in DAG-Belegen.** Ein Werkzeug, das im Hintergrund ausgeführt wurde, lieferte seine echte Ausgabe nur in der internen Benachrichtigungsnachricht des Agenten — die Schritt-Zeitleiste der Benutzeroberfläche und der DAG-Antwort-Verifikationsbeweis sahen nur den Dispatch-Platzhalter. Das geleerte Ergebnis wird nun als normales Werkzeugvervollständigungsereignis unter dem echten Werkzeugnamen ausgegeben.
- **Ein seltener Planer-Output mit doppelten Schritt-IDs bricht nicht mehr den gesamten DAG-Turn ab.** Doppelte IDs wurden früher als „zirkuläre Abhängigkeit"-Fehler fehldiagnostiziert; sie werden nun deterministisch umbenannt und der Plan wird fortgesetzt.
- **Mehrere ReAct-Loop-Korrekturfixes.** Ein Werkzeug, das leere Ausgabe zurückgibt, behielt seine „mit leerer Ausgabe abgeschlossen"-Notiz statt eines leeren Ergebnisses, wenn der Workspace aktiviert ist; DAG-Schritte, die durch eine nicht erfüllbare Abhängigkeit blockiert sind, geben nun ein ordnungsgemäßes Terminalereignis aus, statt die Benutzeroberfläche hängen zu lassen; und für Modelle ohne natives Werkzeugaufrufen zeigt die Fallback-Antwort-Synthese nun die Werkzeugergebnisse, nicht nur die Werkzeugaufrufe.
- **Der Conversation-Workspace ist nun tatsächlich in den Chat verdrahtet — übergroße Werkzeugergebnisse sind wiederherstellbar statt stillschweigend verloren.** Die Workspace-Wiederherstellungspfade der Agent-Schicht (Großausgabe-Offload mit einem `workspace://`-Zeiger, vollständige Rettung von Budget-gekürzten Werkzeugergebnissen, Pre-Komprimierungs-Transkript-Snapshots) existierten, aber die Chat-Endpunkte erstellten nie einen Workspace, also liefen sie nicht: Sobald das Werkzeugergebnis-Budget erschöpft war, erhielt das Modell leere Beobachtungen ohne Möglichkeit, die Daten zurückzulesen, und antwortete aus Vermutungen. ReAct- und DAG-Step-Agenten erhalten nun einen Pro-Konversations-Workspace, und das Löschen einer Konversation bereinigt ihre Workspace-Dateien und alle verbleibenden Crash-Resume-Checkpoints.
- **Wiederaufgenommene DAG-Läufe verlieren nicht mehr das Ende langer Step-Ausgaben.** Step-Belege, die im Crash-Resume-Checkpoint gespeichert waren, wurden nur am Anfang bei der Größenbegrenzung gekürzt und verwarfen stillschweigend alles, was am Ende der Ausgabe lebte — genau dort, wo Schlussfolgerungen, Checksummen und letzte Zeilen tendenziell sind. Begrenzte Belege behalten nun sowohl den Anfang als auch das Ende mit einem expliziten Auslassungsmarker dazwischen.
- **Agenten „funktionieren" nicht mehr, indem sie ihre eigenen Notizen durch Python drucken.** Die Effizienzanleitung, die das Schreiben eines umfassenden Skripts pro Werkzeugaufruf ermutigte, ist nun auf Aufgaben beschränkt, die echte Code-Ausführung benötigen — Wissensfragen, Vergleiche und Analysen, die das Modell direkt produzieren kann, werden direkt beantwortet, statt durch nutzlose Sandbox-Skripte geleitet zu werden.
- **Ein Agent-Schritt innerhalb eines Workflows führt nun den vollständigen Agent aus, nicht nur seinen Prompt.** Der Agent-Knoten eines Workflows wurde früher nur mit den Anweisungen des Agenten auf einem generischen Modell und Standard-Toolset ausgeführt — stillschweigend die Konnektoren, Knowledge Bases, MCP-Server und konfigurierten Modelle des Agenten verworfen. Er führt nun den Agent genau so aus, wie er im Chat wäre, mit allen gebundenen Ressourcen und Modellen, und nur wenn dieser Agent für den Workflow-Besitzer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft nun ihren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Pro-Benutzer-Konnektoren/MCP-Anmeldedaten, die Sie dafür gespeichert haben, werden gelöscht — damit Sie diese Ressourcen nicht mehr verwenden oder auf die Anmeldedaten des Besitzers zurückgreifen können. Ressourcen, die Sie über den Market abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt nun alles konsistent.** Die Admin-Pfade „Benutzer löschen" und Self-Service-„Mein Konto löschen" unterschieden sich früher — jeder bereinigt einen anderen Satz von Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, also verpasste jeder stillschweigend einige), und der Admin-Pfad ließ API-Schlüssel, Anmeldungsverlauf, Eval-Daten, Anrufprotokolle und Abonnements zurück. Beide laufen nun durch eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Nachricht abgelehnt, um zuerst den Besitz zu übertragen (oder die Organisation zu löschen), statt dunkel zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten bricht nicht mehr vergangene Konversationen ab.** Konversationen, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agent-Link gelöscht, statt das Löschen zu blockieren.
- **Bestätigungserforderliche Konnektoren-Aktionen können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Konnektoren-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher innerhalb eines Workflows mit dem Genehmigungstor stillschweigend umgangen. Der Schritt schlägt nun geschlossen fehl mit einer Nachricht, einen Human-Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungstor) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Besitzers.** Wenn der MCP-Server eines Workflows das Fallback für Besitzer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Besitzer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Besitzers zurück, statt gestoppt zu werden. Der Schritt schlägt nun mit einer klaren Nachricht fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Konnektoren-Aufrufe aus Workflows werden nun audit-protokolliert.** Ein Konnektoren-Aktion, die innerhalb eines Workflows ausgeführt wurde, hinterließ keinen Eintrag im Konnektoren-Anrufprotokoll (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun die gleiche Audit-Spur.
- **Sie können nun einen abonnierten Konnektoren, eine Knowledge Base oder einen MCP-Server an einen Agenten anhängen.** Die Bindung erforderte früher, dass Sie die Ressource *besaßen*, also wurde ein Konnektoren, den Sie legitim abonniert hatten (Org-gemeinsam oder Market-installiert), als „nicht im Besitz" abgelehnt. Die Bindung folgt nun der gleichen Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Schritte können nur Konnektoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Konnektoren-Schritt eines Workflows führte früher jeden Konnektoren aus, dessen ID er referenzierte, solange dieser Konnektoren existierte; er erzwingt nun die Sichtbarkeit des Läufers (im Besitz oder abonniert), also kann ein gemeinsamer Workflow keinen Konnektoren erreichen, auf den Sie keinen Zugriff haben, nur indem er seine ID referenziert.
- **Die Knowledge-Base-Suche funktioniert nun gegen KBs, die Sie abonniert haben, nicht nur gegen die, die Sie besitzen.** Die Daten einer KB leben unter dem Speicher ihres Besitzers, aber ein Workflow-Knowledge-Base-Schritt (und Knowledge-Base-Chat ohne Agenten) suchte früher unter dem eigenen Speicher des Aufrufers — also gab die Suche einer KB, die mit Ihnen geteilt wurde, stillschweigend nichts zurück. Der Besitzer wird nun aus der KB selbst aufgelöst und die Suche wird zugegriffen (Besitzer oder Abonnent), also geben gemeinsame KBs Ergebnisse zurück und KBs, die Sie nicht sehen können, geben nichts statt einer verwirrenden leeren Treffer zurück.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokument-Erfassung, Webhook-Zustellung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Laufzeit nur schwach verweist, also konnten sie unter Last während des Fluges Garbage-collected werden und stillschweigend stoppen. Sie werden nun bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), damit sie zuverlässig fertig werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife, was alle gleichzeitigen Anfragen kurzzeitig einfror, während eine große Datei analysiert wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Forschungsschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — also könnte eine Quelle, die 11 Elemente auflistet, in der endgültigen Antwort als 6 auftauchen, oder die Schweregrad eines Elements könnte falsch gekennzeichnet sein, und der Ziel-Evaluierungsschritt konnte es nicht fangen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Werkzeugausgabe als autoritative Quellenbeweis, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, also stimmen Summen, Aufzählungen und Attribute mit dem überein, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht stimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hardcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Toggle, der ein internes (Auto-Degrading)-Konzept offenlegte. Beide werden entfernt — der Knoten führt nun immer Ihr system-konfiguriertes Modell aus (Model Provider, mit Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf die Stelle verweist, wo das Modell konfiguriert ist.
