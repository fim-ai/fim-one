...
- **Workflow-Knoten-Kontextmenü** — Rechtsklick auf einen beliebigen Knoten, um auf Konfigurieren, Kopieren, Duplizieren und Löschen zuzugreifen. Die Löschaktion ist für Start-/End-Singleton-Knoten ausgeblendet. Das Menü schließt sich automatisch bei Außenklick oder Scrollen.
...
- **Workflow Phase 2 Validierung** — Erweiterte Blueprint-Validierungswarnungen für ListOperation (fehlende Eingabe/Ausdruck), Transform (fehlende Eingabe/leere Operationen), Iterator (fehlende Listvariable), Loop (fehlende Bedingung), DocumentExtractor, QuestionUnderstanding (fehlende Eingabe), ParameterExtractor (fehlender Eingabetext/leere Parameter). Behoben: Die Sortieoperation erfordert nicht mehr fälschlicherweise einen Ausdruck.
- **Workflow-Knotennotizen** — Pro-Knoten-Anmerkungsfunktion. Klicken Sie auf „Notiz" im Konfigurationsbereich eines beliebigen Knotens, um eine freie Notiz hinzuzufügen, die als kursiver Text unter dem Knoten auf der Canvas angezeigt wird. Automatisch erweiterbare Textarea mit `data.note`-Speicherung.
...
- **Workflow Zoom-Aware Drop** — Das Knoten-Palette-Drag-and-Drop verwendet `screenToFlowPosition()` für korrekte Platzierung auf jeder Zoomstufe.
- **Workflow SSE Multi-Line Data** — Der SSE-Parser verarbeitet nun mehrzeilige `data:`-Felder und ignoriert Keepalive-Kommentarzeilen.
- **Workflow-Tastaturkürzel** — Kopieren (Cmd+C), Einfügen (Cmd+V), Duplizieren (Cmd+D), Alles auswählen (Cmd+A), Escape zum Abwählen. Kopieren/Einfügen generiert neue IDs und versetzt Positionen; Start-/End-Knoten sind vom Kopieren ausgeschlossen, um die Singleton-Einschränkung zu wahren.
...
- **Workflow ConditionBranch LLM-Modus** — Ordnungsgemäße LLM-basierte Bedingungsevaluierung, die den naiven „Match First"-Stub ersetzt. Erstellt eine Klassifizierungsaufforderung aus Bedingungsetiketten, ruft das schnelle LLM auf und verwendet Groß-/Kleinschreibung-unabhängiges + Fuzzy-Matching.
- **Reaktive Edge-Labels** — Edge-Labels für Bedingung/Klassifizierer-Branches verwenden nun `useNodesData()` für Echtzeit-Updates, wenn Bedingungen oder Klassen im Konfigurationsbereich bearbeitet werden.
- **Workflow-Statistik-Panel** — Sheet-basierte Statistikanzeige mit Gesamtläufen, abgeschlossenen/fehlgeschlagenen/abgebrochenen Zählungen, Erfolgsrate mit visueller Fortschrittsleiste, durchschnittlicher Dauer und letzter Laufzeit mit relativer Formatierung. Ausgelöst durch ein BarChart3-Symbol in der Symbolleiste.
- **Phase 2 Workflow-Knoten: DocumentExtractor & QuestionUnderstanding** — Zwei weitere Knotentypen:
  ...
  - **QuestionUnderstanding**: LLM-basierte Fragenvorbereitung mit 4 Modi — Umschreiben (Klarheit), Erweitern (Kontext + Unterfragen), Klassifizieren (Intent/Topic/Confidence JSON), Zerlegen (Unterfrage-Array). Verwendet schnelles LLM mit optionalem benutzerdefinierten System-Prompt-Override.
...
- **Phase 2 Workflow-Knoten: Iterator & VariableAggregator** — Zwei neue erweiterte Knotentypen:
  ...
  - **VariableAggregator**: Zusammenführen von Ausgaben aus mehreren vorgelagerten Branches über vier Modi — in Liste sammeln, Strings verketten (benutzerdefinierter Trennzeichen), Dicts zusammenführen oder ersten Nicht-Leer-Wert nehmen.
...
- **Workflow Auto-Save** — 5-Sekunden-Debounced Auto-Save-Timer mit sofortigem Cmd+S-Override. Save-Status-Anzeige zeigt „Speichern...", „Ungespeicherte Änderungen" oder „Vor Xs gespeichert" mit 30s Auto-Refresh.
- **Workflow-Listensuche & Filter** — Client-seitige Suche nach Name/Beschreibung, Status-Filter-Schaltflächen (Alle/Entwurf/Aktiv), Sortierungs-Dropdown (Kürzlich aktualisiert/erstellt/Name A-Z) und leerer Status für keine Ergebnisse.
- **Workflow Color-Coded MiniMap** — MiniMap-Knotenfarben entsprechen nun der Kategorie-Farbe jedes Knotentyps (grün=Start, rot=End, blau=LLM, orange=Bedingung, etc.).
- **Agent-Fähigkeitensystem** — On-Demand-Fähigkeitenladen für Agent-Anweisungen: `Skill`-Modell (Name, Beschreibung, Inhalt, Skript) mit vollständigem CRUD, Veröffentlichungsüberprüfung und Sichtbarkeits-Lebenszyklus; `read_skill(name)`-Built-in-Tool für progressive Offenlegung (~80% Token-Reduktion); Fähigkeits-Stubs in System-Prompt injiziert (~15 Token/Fähigkeit); `compact_instructions`-Pro-Agent-Feld für benutzerdefinierte ContextGuard-Kompaktierungsstrategie.
- **Skills-UI** — Fähigkeitenlisten-Seite, Vollseiten-Editor (Inhalt/Skript/Einstellungen-Tabs), SkillCard-Komponente, Sidebar-Navigation, Agent-Editor-Fähigkeitswähler mit kompakten Anweisungen-Textarea.
- **Workflow-Veröffentlichungsfluss** — Veröffentlichungs-/Unveröffentlichungs-/Erneut-Einreichungs-Endpunkte für Workflows, entsprechend dem bestehenden Agent-Veröffentlichungsmuster. Neues `review_workflows`-Flag auf Organization-Modell (mit Alembic-Migration) zur Aktivierung von Org-Level-Workflow-Überprüfungs-Gating. Veröffentlichungsaktionen in Workflow-Karte und Symbolleisten-UI verdrahtet.
- **Workflow-Blueprint-System** — Visueller Workflow-Editor zum Entwerfen und Ausführen mehrstufiger Automatisierungs-Blueprints:
  ...
  - **Ausführungs-Engine**: `WorkflowEngine` mit topologischer Sortierung für Knotenausführungsreihenfolge, `asyncio.Semaphore`-basierter Parallelitätskontrolle und Bedingung/Klassifizierer-Branching, das Downstream-Kanten basierend auf `active_handles` aktiviert/deaktiviert.
  ...
  - **Variable Store**: Thread-sicherer `VariableStore` mit `{{node_id.output}}`-Interpolation, Flat-Name-Fallback-Auflösung, `env.*`-Namespace für verschlüsselte Umgebungsvariablen, `snapshot_safe()` (schließt Geheimnisse aus) und `list_available_variables()` für Frontend-Picker.
  - **Edge Labels**: `WorkflowEdgeDef` trägt `source_handle` und `target_handle` für Bedingung/Klassifizierer-Branches, was beschriftete Kanten ermöglicht, die visuell anzeigen, welcher Branch-Pfad jede Verbindung darstellt.
  ...
  - **Run History Sheet**: Side Sheet mit vergangenen Workflow-Ausführungsläufen mit Status, Dauer, Ein-/Ausgabedaten und Pro-Knoten-Ergebnissen. Paginiert über `GET /api/workflows/{id}/runs`.
  - **Workflow Duplizieren**: `POST /api/workflows/{id}/duplicate`-Endpunkt und UI-Aktion zum Klonen eines vorhandenen Workflows (kopiert Blueprint, Schema, Beschreibung; setzt Status auf Entwurf zurück).
  ...
  - **Fehlerstrategien**: Pro-Knoten-`error_strategy`-Feld mit drei Modi — `STOP_WORKFLOW` (Standard: halte alle verbleibenden Knoten an), `CONTINUE` (markiere fehlgeschlagen, aber lasse Downstream fortfahren), `FAIL_BRANCH` (überspringe alle Knoten, die nur durch den fehlgeschlagenen Knoten erreichbar sind).
  - **Pro-Knoten-Timeout**: Konfigurierbarer `timeout_ms` pro Knoten (Standard 30s) durchgesetzt über `asyncio.wait_for`; Timeout erzeugt ein strukturiertes `FAILED`-Ergebnis mit einer beschreibenden Fehlermeldung.
  - **Knoten-Konfigurationspanel**: Pro-Typ-Konfigurationsformulare — LLM-Knoten haben System-Prompt, Modellwähler, Modell-Tier (schnell/Haupt) Toggle, Temperatur-Schieberegler, max Tokens; ConditionBranch hat Operator-Dropdowns (==, !=, >, <, enthält, etc.) mit Ausdruck/LLM-Modus; HTTPRequest hat Methodenwähler, URL, Headers-Editor (Schlüssel-Wert-Paare hinzufügen/entfernen), Body mit Variableneinfügung; CodeExecution hat Sprachenwähler (Python/JavaScript) mit syntaxgerechten Hinweisen; alle Ausgabe-Knoten enthalten `OutputVariableField` und `InsertVariableBar`.
  - **Add-Node-on-Edge**: Benutzerdefinierte `AddNodeEdge`-Komponente mit „+"-Mittelpunkt-Schaltfläche — Klicken öffnet ein Knotentyp-Wähler-Popup, das einen neuen Knoten inline einfügt und die vorhandene Kante in zwei Verbindungen aufteilt.
  - **Auto-Layout (ELK.js)**: `getAutoLayoutedNodes()`-Utility mit ELK.js-Schichtalgorithmus mit `elk.direction: RIGHT` für horizontale Links-nach-Rechts-Knotenordnung. Konfigurierbare Knotenabstände (60px zwischen Knoten, 120px zwischen Schichten).
  - **Umfassende Test-Suite**: 34 Tests, die Parser-Validierung (Start-/End-Präsenz, doppelte IDs, unbekannte Typen, Zyklenerkennung, Fehler-Strategie/Timeout-Parsing), topologische Sortierung (lineare und parallele Branches), `VariableStore` (Get/Set, Interpolation, Env-Injektion, Snapshot-Sicherheit, Knotenausgaben), Engine-Ausführung (Start-to-End, Input-Passthrough, Abbruch, STOP_WORKFLOW, CONTINUE-Strategie) und Knoten-Executors (VariableAssign, CodeExecution mit Variablen, Fehlerbehandlung) abdecken.
  ...
  - **Sicherheit**: Code-Ausführungs-Knoten verwendet `subprocess` statt bloßem `exec()` für OS-Level-Isolation; Umgebungsvariablen gefiltert aus Eval-/Template-Ausführungs-Kontexten; `ExecutionContext`-Verdrahtung über alle Knoten-Executors.
  - **Frontend**: Visueller Editor mit React Flow v12 — Drag-and-Drop-Knoten-Palette mit Kategorie-Gruppierung (kollabierbare Nur-Symbole-Modus), 12 benutzerdefinierte Knotenkomponenten, Run-Panel mit SSE-getriebenem Live-Ausführungs-Status, Workflow-Listenseite mit Karten-Raster. Vollständige i18n-Unterstützung (6 Sprachen).
...
- **Workflow-Knoten-/Edge-Löschung**: Backspace-/Delete-Tastaturkürzel entfernen ausgewählte Knoten und Kanten. Start-/End-Knoten sind vor Löschung geschützt. Das Konfigurationspanel enthält eine „Knoten löschen"-Schaltfläche mit AlertDialog-Bestätigung für explizites Entfernen.
...
- **Workflow-Templates**: 4 integrierte Starter-Templates (Simple LLM Chain, Conditional Router, Knowledge-Augmented QA, HTTP API Pipeline). Template-Wähler-Dialog bei Workflow-Erstellung mit `GET /api/workflows/templates` und `POST /api/workflows/from-template`-Endpunkten.
- **Workflow-Statistik-Endpunkt**: `GET /api/workflows/{id}/stats` gibt Ausführungsstatistiken zurück (Gesamtläufe, Erfolgsrate, durchschnittliche Dauer, Status-Aufschlüsselung, letzter Lauf-Zeitstempel).
- **Workflow-Fehlerstrategien-Konfigurations-UI**: Pro-Knoten-Erweiterte-Konfigurationsabschnitt im Konfigurationspanel mit Fehlerstrategiewähler (Workflow stoppen / Fortfahren / Branch fehlschlagen) und benutzerdefinierter Timeout-Eingabe.
...
- **Ressourcen-Abonnements**: Benutzer können sich nun von gemeinsamen Ressourcen (Agenten, Konnektoren, Wissensdatenbanken, MCP-Server) aus Org-Märkten abmelden/anmelden. Neue `resource_subscriptions`-Datenbanktabelle verfolgt Abonnement-Beziehungen.
- **Market API**: Neue Endpunkte zum Durchsuchen und Verwalten von Abonnements:
  - `GET /api/market` — durchsuchen Sie verfügbare Ressourcen in der aktuellen Org
  - `POST /api/market/subscribe` — abonnieren Sie eine Ressource
  - `DELETE /api/market/unsubscribe` — kündigen Sie ein Ressourcen-Abonnement
  - `GET /api/market/subscriptions` — listen Sie aktuelle Abonnements des Benutzers auf
...
- **Publish Review UI (Frontend)**: Org-Level-Veröffentlichungsüberprüfungs-System-Frontend — Überprüfungs-Toggle im Org-Bearbeitungs-Dialog, ReviewsSheet mit Typ-/Status-Filtern und Genehmigungen-/Ablehnungs-Workflow, Veröffentlichungs-Status-Badges (ausstehend/genehmigt/abgelehnt) auf Agent-Karten mit Tooltip für Ablehnungsnotizen, Überprüfungsmitteilung im Veröffentlichungs-Dialog, wenn Ziel-Org Überprüfung erfordert, Erneut-Einreichungs-Aktion für abgelehnte Ressourcen, „Überprüfungsverwaltung"-Dropdown für Org-Admin/Besitzer. Alle Ressourcentypen (Agenten, Konnektoren, KBs, MCP-Server) erhalten `publish_status`, `reviewed_by`, `reviewed_at`, `review_note`-Typ-Felder. Vollständige i18n (EN + ZH).
