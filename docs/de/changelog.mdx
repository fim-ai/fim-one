...
- **Datenbankverbindungen können nicht mehr freigegeben werden, und ihr Raw-SQL-Tool ist nur für den Eigentümer zugänglich.** Das Veröffentlichen eines Datenbankverbinders für eine Organisation oder global wird jetzt abgelehnt, und das Tool für beliebige SQL-Datenbanken wird nur dem Eigentümer des Verbinders angeboten — niemals einem Abonnenten, unabhängig von der Einstellung für das Fallback mit Eigentümeranmeldedaten. Zuvor setzte ein freigegebener Datenbankverbinder uneingeschränktes SQL über das (typischerweise hochprivilegierte) Datenbankkonto des Eigentümers für jeden, der sich abonnierte, ohne Scoping pro Aufrufer frei. Der Zugriff auf Datenbanken mit Benutzerbereich wird durch eine zukünftige deklarative Aktionsebene zurückkehren.
- **Webhook- und Cron-ausgelöste Workflows werden jetzt auf das Token-Kontingent des Eigentümers angerechnet.** Ein unbeaufsichtigter Workflow-Lauf — ausgelöst durch seinen öffentlichen API-Schlüssel oder nach einem Zeitplan — führte früher LLM- und Agent-Schritte aus, ohne die Token-Nutzung zu überprüfen oder zu erfassen, ein nicht gemessener kostenloser LLM-Pfad, der Kontingente vollständig umging. Solche Läufe werden jetzt im Voraus durch das Kontingent des Workflow-Eigentümers gated (ein öffentlicher Trigger wird abgelehnt und ein geplanter Lauf wird übersprungen, wenn der Eigentümer bereits über dem Kontingent liegt) und ihre Token-Nutzung wird pro Lauf erfasst und auf das gleiche monatliche Kontingent wie Chat angerechnet.

### Behoben

- **Der Agent generiert kein falsches Bild mehr, wenn er aufgefordert wird, ein HTML-Ergebnis umzugestalten, und behauptet nie, eine Datei erstellt zu haben, ohne sie tatsächlich in den Arbeitsbereich zu schreiben.** Die Bildgenerierung ist nun explizit auf Rasterbilder beschränkt, und Datei-Ergebnisse müssen über einen Tool-Aufruf geschrieben werden — das Einfügen von Inhalten im Chat zählt nicht mehr als „Generieren einer Datei".
- **Generierte HTML-Seiten verwenden nun standardmäßig eine warme, von Claude inspirierte Farbpalette statt des generischen Purple-Gradient-Looks.** Wenn Sie keinen Stil angeben, verwendet der Agent eine warme minimale Baseline; Ihr eigener Stil gewinnt immer, wenn er angegeben ist.
- **Lange Tool-intensive Konversationen brechen nicht mehr ab, wenn der Verlauf komprimiert wird.** Die Kontext-Komprimierung konnte das beibehaltene Fenster zwischen einem Tool-Aufruf und seinen Ergebnissen unterbrechen und eine Nachrichtensequenz erzeugen, die der LLM-Provider ablehnt (HTTP 400) — genau bei den langen Läufen, die Komprimierung am meisten brauchen. Der Kompressor behält nun jeden Tool-Aufruf zusammen mit seinen Ergebnissen auf der beibehaltenen Seite.
- **Hintergrund-Tool-Ergebnisse werden nun im Step-Stream und in DAG-Evidenz angezeigt.** Ein Tool-Lauf im Hintergrund lieferte seine echte Ausgabe nur in der internen Benachrichtigungsmitteilung des Agenten — die UI-Step-Timeline und die DAG-Antwort-Verifizierungsevidenz sahen nur den Dispatch-Platzhalter. Das geleerte Ergebnis wird nun als normales Tool-Completion-Event unter dem echten Tool-Namen ausgegeben.
- **Ein seltener Planner-Output mit doppelten Step-IDs bricht den ganzen DAG-Turn nicht mehr ab.** Doppelte IDs wurden früher als „Zirkelabhängigkeit"-Fehler fehldiagnostiziert; sie werden nun deterministisch umbenannt und der Plan wird fortgesetzt.
- **Mehrere ReAct-Loop-Korrekturen.** Ein Tool, das leere Ausgabe zurückgibt, behielt seine „mit leerer Ausgabe abgeschlossen"-Notiz statt eines leeren Ergebnisses, wenn der Arbeitsbereich aktiviert ist; DAG-Steps, die durch eine nicht erfüllbare Abhängigkeit blockiert sind, geben nun ein ordnungsgemäßes Terminal-Event aus, statt die UI hängen zu lassen; und für Modelle ohne natives Tool-Calling sieht die Fallback-Antwort-Synthese nun die Tool-Ergebnisse, nicht nur die Tool-Aufrufe.
- **Der Konversations-Arbeitsbereich ist nun tatsächlich in den Chat integriert — übergroße Tool-Ergebnisse sind wiederherstellbar statt stillschweigend verloren.** Die Workspace-Recovery-Pfade der Agent-Schicht (großer Output-Offload mit einem `workspace://`-Pointer, vollständige Rettung von Budget-gekürzten Tool-Ergebnissen, Pre-Komprimierungs-Transkript-Snapshots) existierten, aber die Chat-Endpoints erstellten nie einen Arbeitsbereich, daher liefen sie nie: Sobald das Tool-Ergebnis-Budget erschöpft war, erhielt das Modell leere Beobachtungen ohne Möglichkeit, die Daten zurückzulesen, und antwortete aus Vermutungen. ReAct- und DAG-Step-Agenten erhalten nun einen Pro-Konversations-Arbeitsbereich, und das Löschen einer Konversation bereinigt ihre Arbeitsbereich-Dateien und alle verbleibenden Crash-Resume-Checkpoints.
- **Fortgesetzte DAG-Läufe verlieren nicht mehr das Ende langer Step-Ausgaben.** Die in dem Crash-Resume-Checkpoint gespeicherte Step-Evidenz wurde nur am Anfang bei der Größenbeschränkung gekürzt und ließ stillschweigend alles am Ende der Ausgabe fallen — genau dort, wo Schlussfolgerungen, Checksummen und letzte Zeilen zu finden sind. Begrenzte Evidenz behält nun sowohl den Anfang als auch das Ende mit einem expliziten Auslassungsmarker dazwischen.
- **Agenten „funktionieren" nicht mehr, indem sie ihre eigenen Notizen über Python ausdrucken.** Die Effizienzanleitung, die das Schreiben eines umfassenden Skripts pro Tool-Aufruf förderte, ist nun auf Aufgaben beschränkt, die echte Code-Ausführung benötigen — Wissensfragen, Vergleiche und Analysen, die das Modell direkt produzieren kann, werden direkt beantwortet, statt durch nutzlose Sandbox-Skripte geleitet zu werden.
- **Ein Agent-Step in einem Workflow führt nun den vollständigen Agent aus, nicht nur seinen Prompt.** Der Agent-Node eines Workflows wurde früher nur mit den Anweisungen des Agenten auf einem generischen Modell und Standard-Toolset ausgeführt — der Agent-Konnektoren, Wissensdatenbanken, MCP-Server und konfiguriertes Modell wurden stillschweigend gelöscht. Er führt nun den Agent genau so aus, wie er im Chat wäre, mit allen gebundenen Ressourcen und Modellen, und nur wenn dieser Agent für den Workflow-Besitzer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft nun ihren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Pro-Benutzer-Konnektoren/MCP-Anmeldedaten, die Sie dafür gespeichert haben, werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden oder auf die Anmeldedaten des Besitzers zurückgreifen können. Ressourcen, die Sie über den Market abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt nun konsistent alles.** Die Admin-Pfade „Benutzer löschen" und Self-Service „Mein Konto löschen" unterschieden sich früher — jeder reinigte einen anderen Satz von On-Disk-Dateien (und beide berechneten ihre Dateipfade falsch, daher verfehlte jeder stillschweigend einige), und der Admin-Pfad hinterließ API-Schlüssel, Login-Verlauf, Eval-Daten, Call-Logs und Abonnements. Beide laufen nun durch eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr auf Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Meldung abgelehnt, um zuerst den Besitz zu übertragen (oder die Organisation zu löschen), statt unklar zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten bricht nicht mehr frühere Konversationen ab.** Konversationen, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agent-Link gelöscht, statt das Löschen zu blockieren.
- **Bestätigungserforderliche Konnektoren-Aktionen können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Konnektoren-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher in einem Workflow mit dem Approval-Gate stillschweigend umgangen ausgeführt. Der Step schlägt nun geschlossen fehl mit einer Meldung, einen Human Intervention-Node davor zu platzieren (das Workflow-native Approval-Gate) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Steps leihen sich nicht mehr die MCP-Anmeldedaten des Besitzers.** Wenn der MCP-Server eines Workflows das Fallback-Anmeldedaten des Besitzers deaktiviert hatte, griff ein Nicht-Besitzer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Besitzers zurück, statt gestoppt zu werden. Der Step schlägt nun mit einer klaren Meldung fehl, Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Konnektoren-Aufrufe aus Workflows werden nun audit-protokolliert.** Ein Konnektoren-Action-Lauf in einem Workflow hinterließ keinen Eintrag im Konnektoren-Call-Log (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun die gleiche Audit-Spur.
- **Sie können nun einen abonnierten Konnektoren, eine Wissensdatenbank oder einen MCP-Server an einen Agenten anhängen.** Das Binden erforderte früher, dass Sie die Ressource *besaßen*, daher wurde ein Konnektoren, den Sie legitim abonniert hatten (Org-gemeinsam oder Market-installiert), als „nicht besessen" abgelehnt. Das Binden folgt nun der gleichen Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Steps können nur Konnektoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Konnektoren-Step eines Workflows konnte jeden Konnektoren aufrufen, dessen ID er referenzierte, solange dieser Konnektoren existierte; er erzwingt nun die Sichtbarkeit des Läufers (besessen oder abonniert), daher kann ein gemeinsamer Workflow keinen Konnektoren erreichen, auf den Sie keinen Zugriff haben, nur indem Sie seine ID referenzieren.
- **Die Wissensdatenbank-Suche funktioniert nun gegen KBs, die Sie abonniert haben, nicht nur gegen solche, die Sie besitzen.** Die Daten einer KB befinden sich unter dem Speicher ihres Besitzers, aber ein Workflow-Wissensdatenbank-Step (und Wissensdatenbank-Chat ohne Agent) suchte früher unter dem eigenen Speicher des Anrufers — daher gab die Suche einer mit Ihnen geteilten KB stillschweigend nichts zurück. Der Besitzer wird nun aus der KB selbst aufgelöst und die Suche wird zugriffsgeprüft (Besitzer oder Abonnent), daher geben gemeinsame KBs Ergebnisse zurück und KBs, die Sie nicht sehen können, geben nichts statt einer verwirrenden leeren Treffer zurück.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokument-Aufnahme, Webhook-Zustellung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Tasks gestartet, auf die die Runtime nur schwach verweist, daher konnten sie unter Last während des Flugs Garbage-Collected werden und stillschweigend stoppen. Sie werden nun bis zum Abschluss gehalten (und ihre Fehler protokolliert), daher beenden sie sich zuverlässig.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Haupt-Event-Loop, was alle gleichzeitigen Anfragen kurzzeitig einfriert, während eine große Datei geparst wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn ein Forschungs-Step eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — daher konnte eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen, oder die Schweregrad eines Elements konnte falsch gekennzeichnet sein, und der Goal-Evaluation-Step konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Step behält nun seine rohe Tool-Ausgabe als autoritative Quellen-Evidenz, die die endgültige Synthese und der Evaluator gegen die Antwort prüfen, daher stimmen Summen, Aufzählungen und Attribute mit dem überein, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht stimmen).
- **Der Workflow-LLM-Node zeigt nicht mehr Modell-Steuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hardcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Toggle, der ein internes (Auto-Degrading) Konzept offenlegte. Beide werden entfernt — der Node führt nun immer Ihr System-konfiguriertes Modell aus (Model Provider, mit Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf den Ort verweist, wo das Modell konfiguriert ist.
