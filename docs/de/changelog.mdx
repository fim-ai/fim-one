...
- Hochgeladene Dateiinhalte werden nicht mehr in exportierte Gespräche durchsickern; nur der Nachrichtentext des Benutzers ist enthalten.
...
- Einstellungen → Kanäle spiegelt nun die aktuelle Organisationsrolle des Benutzers wider: Mitglieder (Nicht-Admin/Eigentümer) sehen eine deaktivierte Schaltfläche "Neuer Kanal", versteckte Aktionen zum Bearbeiten / Aktivieren-Deaktivieren / Löschen, ein schreibgeschütztes Banner und einen berechtigungsabhängigen leeren Zustand — anstatt eines aktivierten CTA, das bei der Übermittlung mit "Organisationsadministrator-Zugriff erforderlich" fehlgeschlagen ist.
- Die Umleitung bei Sitzungsablauf bewahrt nun die Abfragezeichenfolge, sodass Benutzer nach der erneuten Authentifizierung auf die genaue Registerkarte / den Filter zurückkehren, den sie angesehen haben, anstatt auf den bloßen Pfad.
- Das Feishu-Kanalformular zeigt nicht mehr eine falsche Aufforderung "Ungespeicherte Änderungen verwerfen" an, wenn mit der Chat-Auswahl interagiert wird, die über dem Dialog überlagert ist.
- Feishu-Kanal-Setup-Hinweise duplizieren nicht mehr chinesische Kennzeichen, wenn die Benutzeroberfläche selbst bereits auf Chinesisch ist (z. B. zuvor "事件与回调 (事件与回调)" angezeigt).
- Feishu-Genehmigungskarten werden nach der ersten Entscheidung schreibgeschützt: Der `/callback` Webhook gibt eine Ersatzkarte zurück, bei der die Schaltflächen "Genehmigen/Ablehnen" entfernt sind und die Kopfzeile grün (genehmigt) oder rot (abgelehnt) gefärbt ist, was wiederholte Klicks verhindert. Doppelte Klicks, die noch von veralteten Feishu-Clients ankommen, erhalten einen Toast "Diese Anfrage wurde bereits genehmigt/abgelehnt." und eine frische Kopie der entschiedenen Karte, damit die veraltete Ansicht aufgeholt wird.
- Die einfache Aktion "Testnachricht senden" auf Kanalzeilen und dem Detailblatt wurde wiederhergestellt. Der Approval Playground übt die vollständige Hook-Rundreise aus, aber ein reiner Benachrichtigungskanal (kein Genehmigungshook verdrahtet) benötigt immer noch eine schnelle Anmeldedaten-/Konnektivitätsprüfung, die der einfache Test-Send abdeckt.
...
- "Testnachricht senden" liefert nun eine einfache Textbenachrichtigung (keine Schaltflächen "Genehmigen/Ablehnen") und befindet sich nur im Detailblatt des Kanals — nicht im Zeilendropdown. Benutzer, die keine Genehmigungshooks verwenden möchten, werden nicht durch interaktive Schaltflächen auf einer "Test"-Nachricht verwirrt. Das Approval-Rundreise-Testen bleibt über die Schaltfläche "Approval Playground" verfügbar.
- Detailblatt des Kanals straffer: "Wie Setup beendet wird" ist nun ein standardmäßig eingeklappter Abschnitt (sodass er das Blatt für bereits konfigurierte Kanäle nicht dominiert), und der äußere Abstand wurde reduziert, sodass der Inhalt näher an der Blattkante sitzt.
