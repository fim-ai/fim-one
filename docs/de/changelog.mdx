...
- **Eine neu gesendete Nachricht erscheint oben auf dem Bildschirm.** Das Transkript behält gerade genug Platz unterhalb des aktuellen Durchgangs, damit die erste Zeile oben ankommt, sodass die Frage sichtbar bleibt und die Antwort in den Platz darunter wächst, anstatt dass der gesamte Durchgang an der unteren Kante klebt. Der Platz verschwindet, sobald der Durchgang einen Bildschirm übersteigt.

- **Reasoning-Blöcke bleiben während eines Durchgangs eingeklappt.** Reasoning in ReAct- und DAG-Ausgaben zeigt jetzt standardmäßig eine einzeilige Vorschau statt sich beim Streamen zu erweitern, wodurch das Ergebnis sichtbar bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDF- und Word-Dateien sind für Chinesisch ordnungsgemäß formatiert.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, anstatt auf eine zu verweisen, was den zusammengefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als nicht verwandte chinesische Zeichen dargestellt wurden, und Fettdruck funktioniert jetzt. Word-Exporte deklarieren eine Ostasien-Schriftart, sodass gemischter chinesisch/englischer Fettdruck nicht mehr halb fett aussieht, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften innerhalb einer Antwort der eigenen Struktur des Transkripts unterordnet.
...
- **Gestreamte Antworten fahren sanft ein statt zu erscheinen.** Neu angekommene Blöcke verblassen kurz beim Streamen, wobei die Einstellung für reduzierte Bewegung des Lesers berücksichtigt wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenaufzählungszeichen sitzen hinter dem Text statt mit ihm zu konkurrieren.
...
- **Die Seitenleiste hält Chats jetzt zusammen.** Die Gesprächsliste sitzt direkt unter Neuer Chat und Suche, und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare Hierarchie von oben nach unten.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Durchläufen und früheren Gesprächen werden zu einer einzeiligen Vorschau eingeklappt und beim Klicken erweitert; Reasoning wird weiterhin offen gestreamt, während es generiert wird, wird dann eingeklappt, wenn der Schritt abgeschlossen ist.

- **Listenseiten fahren ihre Karten sanft ein.** Agenten, Konnektoren, Wissensdatenbanken, Skills, Workflows, Tools, Gespräche, Artefakte und der Markt staffeln ihre Karten beim ersten Laden gestaffelt ein, anstatt alle auf einmal zu erscheinen. Filterung und Paginierung bleiben sofort, sodass die Eingabe in ein Suchfeld die Eingabe nie wiederholt.
...
- **Animation folgt jetzt der Systemeinstellung „Bewegung reduzieren".** Der driftende Farbverlauf des Login-Hintergrunds, der Hero-Eingang, der atmende injizierte Nachrichtentext und das Streaming-Caret bewegten sich weiterhin, unabhängig von der Betriebssystem-Einstellung. Sie bleiben jetzt stehen, während die Indikatoren, die laufende Arbeiten anzeigen, sichtbar bleiben, sodass die Benutzeroberfläche nie stecken zu bleiben scheint.

- **Der Ausführungsmodus-Selector bleibt geschlossen, während ein Turn läuft.** Der Chip war ausgegraut, öffnete aber sein Menü trotzdem bei Klick, anders als der Agent-Selector daneben; beide sind jetzt inaktiv, bis der Run beendet ist.
...
- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie steckt nicht mehr mitten im Reasoning fest und meldet falscherweise neue Inhalte darunter, sie folgt Inhalten, die ohne neue Events wachsen, und wenn ein Turn beendet ist, bleibt sie oben im Ergebnis statt unten. Wenn Sie wegscollen, geben Sie die Kontrolle zurück, und wenn Sie zur Unterseite zurückkehren, wird das Folgen fortgesetzt.

- **Eine kurze Antwort bleibt dort, wo Sie sie nach Ende des Turns gelesen haben.** Sobald ein Turn beendet war, rutschte das Transkript nach unten, sodass die Frage und ihre Antwort die Eingabebox umarmten; die Antwort behält jetzt ihre Position oben in der Ansicht, bis die nächste Nachricht gesendet wird.

- **Die Anmeldung wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landingpage wird neu geladen, wenn die Sprache des Kontos sich von der unterscheidet, in der die Anmeldeseite gerendert wurde, sodass das Ziel nicht mehr in der vorherigen Sprache angezeigt wird, bis zur nächsten Neuladung.
...
- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als „Knowledge Base" gekennzeichnet, weil das Abruftool den Ursprungsdateinamen verwarf; es meldet jetzt die Quelle, was der Grounding-Pipeline bereits tat.

- **Lange Unterhaltungen treffen weniger wahrscheinlich auf harte Kontextüberfluss-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unterhalb des Eingabelimits des Modells ausgelöst, anstatt genau darauf, und der Start protokolliert eine Warnung, wenn das Fenster des schnellen Modells zu klein ist, um das Kontextbudget des allgemeinen Modells zu halten.

- **SVG-Figuren bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVG geht von einer hellen Leinwand aus, sodass Figuren auf einer festen hellen Oberfläche gezeichnet werden, anstatt dunkel-auf-dunkel zu werden.

- **Der Browser-Tab-Beschäftigungsindikator verschwindet nicht mehr mitten im Turn.** Während ein Turn läuft, behält das Tab-Symbol die App-Markierung und pulsiert sie zwischen voller Farbe und Grau, hält für den gesamten Run und stellt das echte Symbol zuverlässig wieder her, wenn der Turn endet, auch nach In-App-Navigation. Mit reduzierter Bewegung hält es eine statische graue Markierung statt Animation.

- **Agents rufen einen Endpoint nicht mehr Dutzende Male nach einem abgeschnittenen Ergebnis ab.** Abschneidungsmitteilungen beginnen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie, und ein Agent, der denselben Endpoint immer wieder mit unterschiedlichen Parametern hämmert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Run meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshäkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und erscheinen als Fehlerbenachrichtigung.

- **Das erneute Verbinden mit einem Stream gibt keine Ausgabe wieder, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpoint, sodass nach einer unterbrochenen Verbindung nur die verpassten Events wiedergegeben werden, anstatt des gesamten Turns.
...
- **Das Aufrufen eines anderen Agents gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agents kam früher begraben in einer rohen Ausführungsverfolgung an, die den Kontext des aufrufenden Agents überflutete; sie erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Run nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Wanduhrzeitlimit (Standard 180s), sodass ein steckengebliebener Connector oder unresponsiver Server den Schritt mit einer klaren Meldung beendet, anstatt den Turn zu blockieren.
...
- **Lange Unterhaltungen verlieren ihren früheren Kontext nach Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Turns verdichtet, wurde beim nächsten Run stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), das Kürzen berücksichtigt korrekt nicht-lateinischen Text, und spätere Ergebnisse erhalten ein garantiertes Mindestfenster, anstatt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Execution-Runs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien sind jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Unterhaltung funktioniert jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann wird ihm gesagt, dass sie nicht existiert, und würde sein Iterations-Budget verbrauchen, um sie zu suchen. Ausgelagerte Dateien befinden sich jetzt in der Unterhaltungs-Sandbox, die jedes Tool (einschließlich Exec-Container) mountet; Dateien aus älteren Unterhaltungen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs könnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte zeigt, die bereits auf der Festplatte waren, was den Agent in Kreise schickt. Workspace-Lesevorgänge werden jetzt mit einer genauen Resume-Zeile begrenzt, anstatt erneut ausgelagert zu werden.
...
- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit weitergegeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der einleitende Text des Agents — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Das Abschneiden wird jetzt erkannt, und dem Agent wird gesagt, die Arbeit in kleinere Aufrufe aufzuteilen.

- **Ein Ziel, das der Agent sicher verpasst hat, wird jetzt wiederholt, anstatt den Run zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Runs stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agents wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, sodass Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" ankamen, anstatt der Antwort selbst. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich den vorherigen Token.** JWT `exp` hat nur Sekundenpräzision, sodass zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, früher identisch kodiert wurden — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Refresh-Token konnte für immer erneuert werden. Jeder Refresh-Token trägt jetzt einen eindeutigen `jti`, sodass sich der rotierte Token unterscheidet und der alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt, wenn ein Agent durch einen anderen Agent oder einen Workflow erreicht wird.** Ein empfindlicher Connector-Aufruf, der im Chat eine Bestätigung anfordern würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT`-Knoten eingewickelt wurde — diese Pfade starteten den Agent ohne Hooks. Beide führen jetzt die Gates des Agents aus, verwenden die Bestätigungseinstellungen dieses Agents, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Früher fiel jeder Wert außer `ssl` oder `tls` (z. B. `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Unbekannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
