...
- **Agent-Einstellungen: gebundene Ressourcen flackern nicht mehr als "(已删除)" beim Eintrag.** Die KB- / Connector- / MCP-Server-Picker leiteten das Waisenkennzeichen synchron von `selected − available` ab, sodass jeder Agent, dessen Inventarliste langsamer abgerufen wurde als der Agent selbst, kurzzeitig jede verknüpfte Ressource als gelöscht markierte, bevor der korrekte Status wiederhergestellt wurde — besonders auffällig bei Org-Agenten mit mehreren Connectoren. Das Waisenkennzeichen wartet nun darauf, dass jeder Inventarabruf abgeschlossen ist, bevor es gerendert wird.
- **Toasts folgen nun dem Light/Dark-Theme.** Der Root-Toaster war unabhängig von der Benutzereinstellung auf Dark codiert.
- **Nutzungsaufschlüsselung nach Agent dupliziert keine "Direct Chat"-Zeilen mehr.** Konversationen, die an gelöschte Agenten gebunden waren, erzeugten eine Zeile pro verwaisten `agent_id`; sie werden nun in einer einzelnen Zeile zusammengefasst.
