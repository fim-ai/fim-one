...
- **Das Löschen eines Kontos entfernt jetzt konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" führten früher zu unterschiedlichen Ergebnissen — jeder bereinigt eine andere Menge von Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad hinterließ API-Schlüssel, Anmeldungsverlauf, Evaluierungsdaten, Aufruflogs und Abonnements. Beide durchlaufen jetzt eine einzige Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der immer noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird jetzt mit einer klaren Nachricht abgelehnt, um zuerst das Eigentum zu übertragen (oder die Organisation zu löschen), anstatt unerwartet fehlzuschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten bricht jetzt nicht mehr frühere Konversationen.** Konversationen, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agenten-Link gelöscht, anstatt das Löschen zu blockieren.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Runtime nur schwach verweist, sodass sie unter Last während des Flugs Garbage-Collected werden und stillschweigend stoppen konnten. Sie werden jetzt bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), sodass sie zuverlässig fertig werden.
- **Große Datei-Uploads blockieren jetzt nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife und fror kurzzeitig alle gleichzeitigen Anfragen ein, während eine große Datei analysiert wurde. Die Extraktion wird jetzt an einen Worker-Thread ausgelagert.
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Forschungsschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält jetzt seine rohe Werkzeugausgabe als autoritative Quellenbelege, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen mit einer benannten Diskrepanz sinkt, wenn sie nicht übereinstimmen).
