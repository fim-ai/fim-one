...
- **Endgültige Antworten werden beim Schreiben gestreamt.** Der Agent signalisiert nun, wenn er fertig ist mit dem Sammeln von Informationen, und schreibt die Antwort als live gestreamten Turn, sodass lange Antworten Token für Token mit vollständigen Details ankommen, anstatt nach einer Wartezeit wiedergegeben zu werden. `REACT_FINISH_SIGNAL=false` stellt das vorherige Verhalten wieder her.

- **Architektur-Dokumentation ordnet Graph Engineering FIM One zu.** [Planning Landscape](/architecture/planning-landscape) erklärt Loop vs. Graph Engineering, wie sich das von klassischen visuellen Workflows unterscheidet, wie es sich auf Workflow / DAG / ReAct abbildet, und was man für jede Engine übernehmen sollte. Es besagt auch die aktuelle Tatsache, dass jeder DAG-Schritt bereits ein vollständiger ReAct Agent ist (nicht ein One-Shot LLM Call). ReAct- und DAG-Engine-Seiten verlinken darauf.

- **Einfache Chats bieten immer Folgefragen an.** Gespräche ohne einen gebundenen Agent erhalten nach jeder Antwort vorgeschlagene nächste Fragen; Agent-gebundene Chats erfordern weiterhin den Pro-Agent-Toggle für Folgefragen (Standard aus). Abschluss-Turns wie Grüße oder Dankesagungen können keine Chips zurückgeben.

- **Eine neu gesendete Nachricht startet oben auf dem Bildschirm.** Das Transkript behält gerade genug Platz unter dem aktuellen Turn, damit seine erste Zeile oben ankommt, sodass die Frage in Sicht bleibt und die Antwort in den Platz darunter wächst, anstatt dass der ganze Turn an der unteren Kante klebt. Der Platz verschwindet, sobald der Turn einen Bildschirm übersteigt.

- **Reasoning-Blöcke bleiben während eines Turns eingeklappt.** Reasoning in sowohl ReAct- als auch DAG-Ausgabe zeigt nun standardmäßig eine einzeilige Vorschau anstatt sich beim Streamen zu erweitern, wodurch das Ergebnis in Sicht bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDF- und Word-Dateien sind ordnungsgemäß für Chinesisch gesetzt.** PDF-Export bettet nun eine echte CJK-Schriftart ein, anstatt eine zu referenzieren, was kollabierte Abstände in englischem Text und Aufzählungszeichen behebt, die als unzusammenhängende chinesische Zeichen gerendert wurden, und Fett funktioniert nun. Word-Exporte deklarieren eine Ostasien-Schriftart, sodass gemischter chinesisch/englischer Fetttext nicht mehr halb-fett herauskommt, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften in einer Antwort der Struktur des Transkripts selbst untergeordnet hält.

- **Agents bleiben bei langen Aufgaben im Plan.** Der Agent wird nun daran erinnert, seinen Ansatz zu ändern, wenn er das gleiche Tool fruchtlos wiederholt, wird angestoßen, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifikationsdurchgang.

- **Gestreamte Antworten werden sanft eingeleitet, anstatt zu erscheinen.** Neu angekommene Blöcke verblassen kurz beim Streamen, wobei die Vorliebe des Lesers für reduzierte Bewegung berücksichtigt wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenaufzählungszeichen sitzen hinter dem Text, anstatt mit ihm zu konkurrieren.

- **Block-Aktionen bleiben aus dem Weg, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hover angezeigt und teilen eine konsistente Behandlung; sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste hält nun Chats zusammen.** Die Gesprächsliste sitzt direkt unter Neuer Chat und Suche, und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare von oben nach unten Hierarchie.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in sowohl Live-Läufen als auch vergangenen Gesprächen falten sich zu einer einzeiligen Vorschau zusammen und erweitern sich beim Klick; Reasoning wird weiterhin offen gestreamt, während es generiert wird, faltet sich dann zusammen, wenn der Schritt abgeschlossen ist.

- **Listenseiten lassen ihre Karten sanft an Ort und Stelle gleiten.** Agents, Connectors, Knowledge Bases, Skills, Workflows, Tools, Conversations, Artifacts und der Markt staffeln ihre Karten beim ersten Laden gestaffelt ein, anstatt alle auf einmal zu erscheinen. Filtern und Paging bleiben sofort, sodass das Tippen in ein Suchfeld die Eingabe nie wiederholt.
...
- **Ask-first-Anfragen werden im Planner-Modus in einer Runde abgeschlossen.** Ein Ziel, das den Agent explizit auffordert, Anforderungen mit Multiple-Choice-Fragen zu klären, bevor er liefert („问清楚我再出方案"), erzeugt jetzt einen sauberen Fragebogen als Ergebnis der Runde, anstatt Planungsschritte zu erstellen, die auf Antworten warten, durch Neuplanungen zu schleifen und mit „Ziel nicht erreicht" plus wiederholten Fragenlisten zu enden. Der Auto-Modus leitet solche Anfragen an den Standard-Modus weiter.

- **Follow-up-Chips ziehen das Transkript nicht mehr über die Antwort hinaus.** Nachdem eine Runde beim Ergebnis geparkt wird, bleibt das spätere Layout-Wachstum (Vorschläge, Titel) an Ort und Stelle, und das Einklappen der Live-Runde in die Historie parkt wieder am Anfang der Antwort für dasselbe Gespräch, anstatt zum Ende zu springen.

- **Das Öffnen eines bestehenden Gesprächs führt zu den neuesten Nachrichten.** Das Aktualisieren einer Chat-URL scrollt jetzt nach unten wie beim Wechsel aus der Seitenleiste, anstatt oben im Thread zu beginnen.

- **Schnelle Antworten sehen nicht mehr fest auf „Verarbeitung…" aus.** Eine abgeschlossene Runde ohne generierten Schrittitel zeigt stattdessen das Gedankenprozess-Label anstelle des Live-Verarbeitungs-Shimmer-Textes.

- **Animation folgt jetzt der Systemeinstellung „Bewegung reduzieren".** Der driftende Gradient des Login-Hintergrunds, der Hero-Eingang, der atmende eingespritzte Nachrichtentext und das Streaming-Caret bewegten sich weiter, unabhängig von der Betriebssystem-Einstellung. Sie bleiben jetzt still, während die Indikatoren, die laufende Arbeiten melden, sichtbar bleiben, damit die Oberfläche nie steckengeblieben aussieht.

- **Der Ausführungsmodus-Selector bleibt geschlossen, während eine Runde läuft.** Der Chip war ausgegraut, aber öffnete sein Menü trotzdem beim Klick, anders als der Agent-Selector daneben; beide sind jetzt inaktiv, bis die Ausführung beendet ist.

- **Reasoning-Streams funktionieren wieder bei neueren Modellen.** Live-Reasoning war von einer hartcodierten Liste von Modellnamen verdrahtet, daher zeigte alles, was nach dieser Liste veröffentlicht wurde, einen Spinner und dann den gesamten Reasoning-Block auf einmal. Jedes Modell, das Reasoning ausgibt, streamt es jetzt.

- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie steckt nicht mehr mitten im Reasoning fest und meldet fälschlicherweise neuen Inhalt darunter, sie folgt Inhalten, die ohne neue Events wachsen, und wenn eine Runde endet, bleibt sie oben beim Ergebnis stehen, anstatt unten. Das Wegscollen gibt die Kontrolle immer noch ab, und die Rückkehr nach unten setzt das Folgen fort.

- **Eine kurze Antwort bleibt dort, wo Sie sie nach Ende der Runde gelesen haben.** Nachdem eine Runde beendet war, rutschte das Transkript nach unten, sodass die Frage und ihre Antwort die Eingabebox umarmten; die Antwort behält jetzt ihre Position oben in der Ansicht, bis die nächste Nachricht gesendet wird.

- **Die Anmeldung wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landing Page wird neu geladen, wenn die Sprache des Kontos sich von der unterscheidet, in der die Anmeldeseite gerendert wurde, sodass das Ziel nicht mehr in der vorherigen Sprache angezeigt wird, bis zum nächsten Neuladen.

- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener blieb aktiv, aber sein Dialog wurde im eingeklappten Zustand nie eingebunden, daher machte der Shortcut stillschweigend nichts.

- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als „Knowledge Base" gekennzeichnet, weil das Abruftool den Ursprungsdateinamen fallen ließ; es meldet jetzt die Quelle, was der Grounding-Pipeline bereits entsprach.

- **Lange Gespräche treffen weniger wahrscheinlich auf harte Kontextüberfluss-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unter dem Eingabelimit des Modells ausgelöst, anstatt genau darauf, und der Start protokolliert eine Warnung, wenn das Fenster des schnellen Modells zu klein ist, um das Kontextbudget des allgemeinen Modells zu halten.

- **SVG-Figuren bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVG geht von einer hellen Leinwand aus, daher werden Figuren auf einer festen hellen Oberfläche gezeichnet, anstatt dunkel-auf-dunkel zu werden.

- **Der Browser-Tab-Busy-Indikator verschwindet nicht mehr mitten in einer Runde.** Während eine Runde läuft, behält das Tab-Symbol die App-Markierung und pulsiert sie zwischen voller Farbe und Grau, hält für die gesamte Ausführung und stellt das echte Symbol zuverlässig wieder her, wenn die Runde endet, auch nach In-App-Navigation. Mit reduzierter Bewegung hält es eine statische graue Markierung anstelle von Animation.

- **Agents rufen einen Endpoint nicht mehr Dutzende Male nach einem abgeschnittenen Ergebnis ab.** Abschneidungshinweise führen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie an, und ein Agent, der denselben Endpoint immer wieder mit unterschiedlichen Parametern hämmert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Lauf meldet jetzt einen Fehler anstelle eines grünen „fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgs-Häkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und erscheinen als Fehlerbenachrichtigung.

- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr Ausgaben, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpoint, daher werden nach einer unterbrochenen Verbindung nur die verpassten Events wiedergegeben, anstatt die gesamte Runde.

- **Gestreamte Events, die über Netzwerk-Chunks aufgeteilt sind, gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Event verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen handhabt.

- **Das Aufrufen eines anderen Agents gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agents kam früher in einer rohen Ausführungsverfolgung begraben an, die den Kontext des aufrufenden Agents überflutete; sie empfängt jetzt nur die Antwort.

- **Ein hängender Tool kann eine Ausführung nicht mehr unbegrenzt steckenbleiben lassen.** Jeder Tool-Aufruf hat jetzt ein Wanduhrzeitlimit (Standard 180s), daher endet ein steckengebliebener Connector oder nicht reagierender Server den Schritt mit einer klaren Nachricht, anstatt die Runde zu verkeilen.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen abgelehnt, anstatt das gesamte Iterations-Budget zu verbrauchen.

- **Lange Gespräche verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Runden verdichtet, wurde beim nächsten Lauf stillschweigend gelöscht; sie wird jetzt weitergeleitet, daher behält der Agent, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (der Abschluss einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und spätere Ergebnisse erhalten ein garantiertes Mindestfenster, anstatt vollständig gelöscht zu werden.

- **Die gleiche generierte Datei wird nicht mehr als zwei Downloads angezeigt.** Eine gerenderte HTML-Ausgabe, die erneut unter einem echten Dateinamen gespeichert wurde, erzeugte früher zwei Lieferkarten mit identischem Inhalt; Duplikate werden jetzt nach Inhalt erkannt und in die speziell benannte Datei eingeklappt.

- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungslaufs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien werden jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einem Gespräch funktioniert jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann gesagt bekommen, dass sie nicht existiert, und sein Iterations-Budget verbrauchen, um sie zu suchen. Ausgelagerte Dateien leben jetzt in der Gesprächs-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Gesprächen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs könnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte zeigt, die bereits auf der Festplatte waren, und den Agent in Kreise schicken. Workspace-Lesevorgänge werden jetzt mit einer exakten Resume-Zeile begrenzt, anstatt erneut ausgelagert zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, anstatt den Function-Calling-Kanal zu verwenden, erreichte das rohe Markup den Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit weitergeleitet.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halbgeschriebene Aufruf stillschweigend gelöscht und der einleitende Text des Agents — „jetzt speichere ich die Datei" — wurde zum Schrittergebnis, daher wurde ein Schritt, der nichts erzeugte, als abgeschlossen gemeldet. Die Abschneidung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.

- **Ein Ziel, bei dem der Agent sicher ist, dass er es verpasst hat, wird jetzt erneut versucht, anstatt die Ausführung zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Läufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agents wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich an Sie gestreamt, daher kamen Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" anstelle der Antwort selbst an. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` ist nur Sekunden-Präzision, daher codierten zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt jetzt einen eindeutigen `jti`, daher unterscheidet sich das rotierte Token und das alte funktioniert sofort nicht mehr.

- **Approval Gates halten jetzt, wenn ein Agent über einen anderen Agent oder einen Workflow erreicht wird.** Ein sensitiver Connector-Aufruf, der in Chat um Bestätigung fragen würde, lief zuvor ungeprüft ab, sobald er über `call_agent` delegiert oder in einem Workflow `AGENT`-Knoten eingewickelt wurde — diese Pfade starteten den Agent ohne Hooks überhaupt. Beide führen jetzt die Gates des Agents aus, verwenden die Bestätigungseinstellungen dieses Agents, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext
