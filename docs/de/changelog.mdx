...
- **Webhook- und Cron-ausgelöste Workflows werden nun auf das Token-Kontingent des Eigentümers angerechnet.** Ein unbeaufsichtigter Workflow-Durchlauf – ausgelöst über seinen öffentlichen API-Schlüssel oder nach einem Zeitplan – führte früher LLM- und Agent-Schritte aus, ohne die Token-Nutzung zu überprüfen oder zu erfassen, ein ungezählter kostenloser LLM-Pfad, der Kontingente vollständig umging. Solche Durchläufe werden nun vorab am Kontingent des Workflow-Eigentümers geprüft (ein öffentlicher Trigger wird abgelehnt und ein geplanter Durchlauf wird übersprungen, wenn der Eigentümer bereits das Kontingent überschritten hat) und ihre Token-Nutzung wird pro Durchlauf erfasst und auf dasselbe monatliche Kontingent wie Chat angerechnet.
