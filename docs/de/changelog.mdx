...
- **Extended Thinking funktioniert jetzt auf neueren Anthropic-Modellen (Opus 4.6+, Sonnet 4.6, Fable 5).** Diese Modelle haben den alten Fixed-Budget-Thinking-Parameter entfernt — Opus 4.7/4.8 und Fable 5 lehnen ihn mit einem 400-Fehler ab — daher führte die Konfiguration des Reasoning Effort entweder zu Fehlern oder tat stillschweigend nichts. Wir senden jetzt das Adaptive-Thinking-Protokoll für diese Modelle (und verwerfen die Sampling-Parameter, die sie nicht mehr akzeptieren), sodass Reasoning Effort korrekt wirksam wird. Ein Modell, das Adaptive Thinking verwendet, aber über einen generischen OpenAI-kompatiblen Proxy verbunden ist (wo Thinking nicht aktiviert werden kann), protokolliert jetzt eine klare Warnung, anstatt stillschweigend zu fehlschlagen — konfigurieren Sie es als Anthropic-Provider, um Thinking zu aktivieren.
...
- **Enforcement Hooks schlagen jetzt geschlossen fehl, wenn sie abstürzen.** Ein `PRE_TOOL_USE` Hook (z. B. ein Approval Gate), der eine Exception auslöste, gab zuvor `allow=True` zurück, sodass der blockierte Tool Call trotzdem fortgesetzt wurde — ein abgestürztes Gate wurde stillschweigend zu einem Bypass. PRE Hooks verweigern jetzt standardmäßig bei Fehler; Non-Enforcement Hooks (Rate Limiter, Logger) behalten das alte Fail-Open-Verhalten über ein neues `fail_open` Flag bei.
