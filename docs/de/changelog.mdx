...
- **Antworten enden nicht mehr mitten im Satz, wenn das Modell sein Ausgabelimit erreicht.** Wenn eine Antwort durch das Ausgabe-Token-Limit des Anbieters unterbrochen wird, fordert der Agent das Modell automatisch auf, dort fortzufahren, wo es stoppte (bis zu 3 Runden), und fügt die Segmente zu einer nahtlosen Antwort zusammen — sowohl in der Reasoning-Schleife als auch in der gestreamten Endantwort.
- **LLM-Aufrufe wiederholen sich jetzt bei Anbieterüberlastung (HTTP 529) und Gateway-Timeout (504) und beachten `Retry-After`.** Zuvor führte ein 529 von Anthropic sofort zum Fehlschlag der Anfrage, es sei denn, ein Fallback-Modell war konfiguriert; jetzt wird es wie bei 429/5xx zurückgefahren und wiederholt, wobei genau so lange gewartet wird, wie der Anbieter mit einem `Retry-After`-Header anfordert.
- **Agents behalten jetzt einen sichtbaren Plan während mehrstufiger Aufgaben.** ReAct-Agents erhalten ein `update_plan`-Tool, um ihren Plan als Todo-Checkliste aufzuschreiben und ihn während des Fortschritts zu aktualisieren; wenn der Plan nach mehreren Tool-Aufrufen veraltet wird, wird die vollständige Checkliste als Erinnerung erneut eingefügt — so bleiben lange Aufgaben auch nach Kontext-Komprimierung auf Kurs. Standardmäßig aktiviert (`REACT_PLAN_TOOL_ENABLED`); DAG-Unterschritte und werkzeuglose Agents überspringen dies.

- **Agents können jetzt einen Workflow mitten in einem Gespräch ausführen.** Aktive, genehmigungsfreie Workflows, die Sie besitzen oder abonniert haben, werden dem Agent als `run_workflow`-Tool angeboten — im normalen Chat und mit ausgewähltem Agent gleichermaßen — sodass eine Aufgabe einen deterministischen, wiederholbaren Unterprozess an einen Workflow übergeben und dessen Ausgaben zurückerhalten kann. Jeder solche Lauf wird im Workflow-Verlauf aufgezeichnet und seine Token-Nutzung wird gegen das Kontingent des Workflow-Besitzers angerechnet (der Lauf wird abgelehnt, wenn der Besitzer bereits über dem Kontingent liegt). Workflows mit einem Human-Intervention-Schritt bleiben nur auslösbar (von der Workflows-Seite oder ihrem API-Schlüssel), da sie lange blockieren können.
- **Zwei neue Workflow-Starter-Vorlagen: Connector Action und MCP Tool Call.** Der Katalog „Create from Template" präsentiert jetzt die Connector- und MCP-Knoten — rufen Sie ein externes System oder ein MCP-Tool auf und fassen Sie das Ergebnis dann mit einem LLM zusammen. Wählen Sie den spezifischen Connector/Server nach der Erstellung aus.
