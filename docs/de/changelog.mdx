...
- **Die Aktualisierung von Refresh-Token führt jetzt tatsächlich zur Ungültigmachung des vorherigen Tokens.** JWT `exp` hat nur Sekundenpräzision, daher codierten zwei Refresh-Token, die in derselben Sekunde für denselben Benutzer ausgestellt wurden, früher identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Refresh-Token konnte auf unbestimmte Zeit erneuert werden. Jeder Refresh-Token trägt jetzt eine eindeutige `jti`, sodass sich der rotierte Token unterscheidet und der alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt an, wenn ein Agent über einen anderen Agent oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in einem Chat um Bestätigung bitten würde, lief zuvor ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingebunden wurde — diese Pfade starteten den Agent ohne Hooks. Beide führen jetzt die eigenen Gates des Agents aus, verwenden die Bestätigungseinstellungen dieses Agents und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Zuvor führte jeder Wert, der nicht `ssl` oder `tls` war (zum Beispiel `SMTP_SSL=true`), zu einer unverschlüsselten Verbindung ohne Warnung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
