...
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft nun deren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die pro Benutzer gespeicherten Connector-/MCP-Anmeldedaten werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden oder nach dem Verlassen auf die Anmeldedaten des Besitzers zurückgreifen können. Ressourcen, die Sie über den Market abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt nun konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" unterschieden sich früher — jeder bereinigt eine andere Menge an Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad hinterließ API-Schlüssel, Anmeldungsverlauf, Evaluierungsdaten, Aufruflogs und Abonnements. Beide durchlaufen nun eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Meldung abgelehnt, um zuerst den Besitz zu übertragen (oder die Organisation zu löschen), anstatt unklar zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten unterbricht nun keine vergangenen Gespräche mehr.** Gespräche, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agenten-Link gelöscht, anstatt das Löschen zu blockieren.
- **Connector-Aktionen, die eine Bestätigung erfordern, können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Connector-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher innerhalb eines Workflows mit dem Genehmigungsgate stillschweigend umgangen ausgeführt. Der Schritt schlägt nun geschlossen fehl mit einer Meldung, um einen Human Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungsgate) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
...
- **Connector-Aufrufe aus Workflows werden nun im Audit-Log protokolliert.** Ein Connector-Aktion, die innerhalb eines Workflows ausgeführt wurde, hinterließ keinen Eintrag im Connector-Aufruflogs (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun die gleiche Audit-Spur.
- **Sie können nun einen abonnierten Connector, eine Knowledge Base oder einen MCP-Server an einen Agenten anhängen.** Das Binden erforderte früher, dass Sie die Ressource *besaßen*, sodass ein Connector, den Sie legitim abonniert hatten (organisationsweit gemeinsam oder über den Market installiert), als „nicht im Besitz" abgelehnt wurde. Das Binden folgt nun der gleichen Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Schritte können nur Connectoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Connector-Schritt eines Workflows konnte früher jeden Connector aufrufen, dessen ID er referenzierte, solange dieser Connector existierte; er erzwingt nun die Sichtbarkeit des Ausführenden (im Besitz oder abonniert), sodass ein gemeinsamer Workflow keinen Connector erreichen kann, auf den Sie keinen Zugriff haben, nur durch Referenzierung seiner ID.
- **Die Knowledge-Base-Suche funktioniert nun gegen KBs, die Sie abonniert haben, nicht nur gegen solche, die Sie besitzen.** Die Daten einer KB befinden sich im Speicher ihres Besitzers, aber ein Workflow-Knowledge-Base-Schritt (und Knowledge-Base-Chat ohne einen Agenten) suchte früher im eigenen Speicher des Aufrufers — sodass die Suche in einer mit Ihnen geteilten KB stillschweigend nichts zurückgab. Der Besitzer wird nun aus der KB selbst aufgelöst und die Suche wird zugriffsgeprüft (Besitzer oder Abonnent), sodass gemeinsame KBs Ergebnisse zurückgeben und KBs, die Sie nicht sehen können, nichts zurückgeben, anstatt eines verwirrenden leeren Treffers.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Laufzeit nur schwach verweist, sodass sie unter Last während des Flugs Garbage-collected werden konnten und stillschweigend stoppten. Sie werden nun bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), sodass sie zuverlässig fertig werden.
...
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Forschungsschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 erscheinen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Tool-Ausgabe als autoritative Quellenbeweise, die die endgültige Synthese und der Evaluator gegen die Antwort prüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hartcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Umschalter, der ein internes (automatisch herabgestuftes) Konzept offenlegte. Beide werden entfernt — der Knoten führt nun immer Ihr systemkonfiguriertes Modell aus (Model Provider, Fallback zu `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf die Stelle verweist, wo das Modell konfiguriert ist.
