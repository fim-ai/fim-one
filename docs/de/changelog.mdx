...
- **Der Gesprächsarbeitsbereich ist jetzt tatsächlich in den Chat integriert — übergroße Werkzeugergebnisse sind wiederherstellbar statt stillschweigend verloren.** Die Wiederherstellungspfade des Agent-Layers (großer Output-Offload mit einem `workspace://`-Zeiger, vollständige Rettung von budgetgekürzten Werkzeugergebnissen, Pre-Compaction-Transkript-Snapshots) existierten, aber die Chat-Endpunkte erstellten nie einen Arbeitsbereich, daher liefen sie nie: Sobald das Werkzeugergebnis-Budget erschöpft war, erhielt das Modell leere Beobachtungen ohne Möglichkeit, die Daten zurückzulesen, und antwortete aus Vermutungen. ReAct- und DAG-Step-Agenten erhalten jetzt einen Pro-Gespräch-Arbeitsbereich, und das Löschen eines Gesprächs bereinigt seine Arbeitsbereichsdateien und alle verbleibenden Crash-Resume-Checkpoints.
- **Wiederaufgenommene DAG-Läufe verlieren nicht mehr das Ende langer Step-Outputs.** Step-Evidenz, die im Crash-Resume-Checkpoint gespeichert war, wurde am Größenlimit nur am Anfang gekürzt und ließ stillschweigend alles am Ende des Outputs fallen — genau dort, wo Schlussfolgerungen, Checksummen und letzte Zeilen zu finden sind. Begrenzte Evidenz behält jetzt Anfang und Ende mit einem expliziten Auslassungsmarker dazwischen.
- **Agenten „funktionieren" nicht mehr, indem sie ihre eigenen Notizen über Python ausdrucken.** Die Effizienzanleitung, die das Schreiben eines umfassenden Skripts pro Werkzeugaufruf förderte, ist jetzt auf Aufgaben beschränkt, die echte Code-Ausführung benötigen — Wissensfragen, Vergleiche und Analysen, die das Modell direkt produzieren kann, werden direkt beantwortet statt durch nutzlose Sandbox-Skripte geleitet.
- **Ein Agent-Step in einem Workflow führt jetzt den vollständigen Agent aus, nicht nur seinen Prompt.** Der Agent-Knoten eines Workflows wurde früher nur mit den Agent-Anweisungen auf einem generischen Modell und Standard-Toolset ausgeführt — stillschweigend wurden die Konnektoren, Wissensdatenbanken, MCP-Server und das konfigurierte Modell des Agenten fallen gelassen. Er führt jetzt den Agent genau wie im Chat aus, mit allen gebundenen Ressourcen und Modell, und nur wenn dieser Agent für den Workflow-Besitzer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft jetzt deren gemeinsamen Zugriff.** Ressourcen, die Sie nur über diese Organisation erreichen konnten, werden abgemeldet, und die Pro-Benutzer-Konnektoren/MCP-Anmeldedaten, die Sie dafür gespeichert haben, werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden oder auf die Anmeldedaten des Besitzers zurückgreifen können. Ressourcen, die Sie über den Markt abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt jetzt konsistent alles.** Die Admin-Pfade „Benutzer löschen" und Self-Service „Mein Konto löschen" divergierten früher — jeder bereinigt einen anderen Satz von On-Disk-Dateien (und beide berechneten ihre Dateipfade falsch, daher verfehlte jeder stillschweigend einige), und der Admin-Pfad ließ API-Schlüssel, Anmeldungsverlauf, Eval-Daten, Aufruflogs und Abos zurück. Beide laufen jetzt durch eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird jetzt mit einer klaren Nachricht abgelehnt, um zuerst die Eigentümerschaft zu übertragen (oder die Organisation zu löschen), statt obscur zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten bricht jetzt keine vergangenen Gespräche.** Gespräche, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agent-Link gelöscht, statt das Löschen zu blockieren.
- **Bestätigungserforderliche Konnektoren-Aktionen können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Konnektoren-Aktion, die als bestätigungserforderlich gekennzeichnet ist, wurde früher in einem Workflow mit dem Genehmigungsgate stillschweigend umgangen ausgeführt. Der Step schlägt jetzt geschlossen fehl mit einer Nachricht, einen Human Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungsgate) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Steps leihen sich nicht mehr die MCP-Anmeldedaten des Besitzers.** Wenn der MCP-Server eines Workflows die Fallback-Anmeldedaten des Besitzers deaktiviert hatte, fiel ein Nicht-Besitzer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Besitzers zurück, statt gestoppt zu werden. Der Step schlägt jetzt mit einer klaren Nachricht fehl, Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Konnektoren-Aufrufe aus Workflows werden jetzt audit-protokolliert.** Ein Konnektoren-Aufruf, der in einem Workflow ausgeführt wurde, hinterließ keinen Eintrag im Konnektoren-Aufrufsprotokoll (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben jetzt die gleiche Audit-Spur.
- **Sie können jetzt einen abonnierten Konnektor, eine Wissensdatenbank oder einen MCP-Server an einen Agenten anhängen.** Die Bindung erforderte früher, dass Sie die Ressource *besaßen*, daher wurde ein Konnektor, den Sie legitim abonniert hatten (Org-geteilt oder Markt-installiert), als „nicht im Besitz" abgelehnt. Die Bindung folgt jetzt der gleichen Sichtbarkeitregel, die der Rest der App verwendet — alles, was Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Steps können nur Konnektoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Konnektoren-Step eines Workflows führte früher jeden Konnektor aus, dessen ID er referenzierte, solange dieser Konnektor existierte; er erzwingt jetzt die Sichtbarkeit des Ausführenden (im Besitz oder abonniert), daher kann ein gemeinsamer Workflow keinen Konnektor erreichen, auf den Sie keinen Zugriff haben, nur indem er seine ID referenziert.
- **Die Wissensdatenbank-Suche funktioniert jetzt gegen KBs, die Sie abonniert haben, nicht nur solche, die Sie besitzen.** Die Daten einer KB befinden sich unter dem Speicher ihres Besitzers, aber ein Workflow-Wissensdatenbank-Step (und Wissensdatenbank-Chat ohne Agent) suchte früher unter dem eigenen Speicher des Aufrufers — daher gab die Suche einer mit Ihnen geteilten KB stillschweigend nichts zurück. Der Besitzer wird jetzt von der KB selbst aufgelöst und die Suche wird zugegriffen (Besitzer oder Abonnent), daher geben gemeinsame KBs Ergebnisse zurück und KBs, die Sie nicht sehen können, geben nichts statt einer verwirrenden leeren Trefferliste zurück.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Runtime nur schwach verwiesen, daher konnten sie unter Last während des Flugs garbage-collected werden und stillschweigend stoppen. Sie werden jetzt bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), daher beenden sie zuverlässig.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife, wobei kurzzeitig alle gleichzeitigen Anfragen eingefroren wurden, während eine große Datei analysiert wurde. Die Extraktion wird jetzt auf einen Worker-Thread ausgelagert.
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Forschungs-Step eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — daher konnte eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen, oder die Schweregrad eines Elements konnte falsch gekennzeichnet sein, und der Zielauswertungs-Step konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Step behält jetzt seinen rohen Werkzeugoutput als autoritative Quellevidenz, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, daher stimmen Summen, Aufzählungen und Attribute mit dem überein, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hartcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Toggle, der ein internes (Auto-Degrading) Konzept offenlegte. Beide werden entfernt — der Knoten führt jetzt immer Ihr System-konfiguriertes Modell aus (Model Provider, Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf den Ort verweist, wo das Modell konfiguriert ist.
