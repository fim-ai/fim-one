...
- **Animation folgt jetzt der Systemeinstellung „Bewegung reduzieren".** Der driftende Gradient des Login-Hintergrunds, der Hero-Eintritt, der atmende injizierte Nachrichtentext und das Streaming-Caret bewegten sich weiterhin, unabhängig von der Betriebssystem-Einstellung. Sie bleiben jetzt still, während die Indikatoren, die laufende Arbeiten anzeigen, sichtbar bleiben, sodass die Benutzeroberfläche nie stecken zu bleiben scheint.

- **Der Ausführungsmodus-Selector bleibt geschlossen, während ein Turn läuft.** Der Chip war ausgegraut, öffnete aber sein Menü trotzdem bei Klick, anders als der daneben liegende Agent-Selector; beide sind jetzt inaktiv, bis der Run beendet ist.

- **Reasoning-Streams funktionieren wieder bei neueren Modellen.** Live Reasoning war an eine hartcodierte Liste von Modellnamen gebunden, daher zeigte alles, das nach dieser Liste veröffentlicht wurde, einen Spinner und gab dann den gesamten Reasoning-Block auf einmal aus. Jedes Modell, das Reasoning ausgibt, streamt es jetzt.

- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie steckt nicht mehr mitten im Reasoning fest und meldet fälschlicherweise neue Inhalte darunter, folgt Inhalten, die ohne neue Events wachsen, und wenn ein Turn beendet ist, wird er oben im Ergebnis statt unten angezeigt. Wenn Sie wegscollen, wird die Kontrolle zurückgegeben, und wenn Sie zur Unterseite zurückkehren, wird das Folgen fortgesetzt.

- **Die Anmeldung wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landing Page wird neu geladen, wenn sich die Sprache des Kontos von der unterscheidet, in der die Login-Seite gerendert wurde, sodass das Ziel nicht mehr in der vorherigen Sprache angezeigt wird, bis zur nächsten Aktualisierung.

- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener blieb aktiv, aber sein Dialog wurde im eingeklappten Zustand nie bereitgestellt, daher machte der Shortcut stillschweigend nichts.

- **Zitate nennen das Dokument, aus dem sie stammen, wieder.** Im einfachen Abruf-Modus wurde jede Quelle als „Knowledge Base" gekennzeichnet, da das Abruf-Tool den Ursprungsdateinamen gelöscht hat; es meldet jetzt die Quelle, was dem entspricht, was die Grounding-Pipeline bereits tat.

- **Lange Unterhaltungen treffen weniger wahrscheinlich auf harte Context-Overflow-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unter dem Eingabelimit des Modells ausgelöst, anstatt genau darauf, und der Startup protokolliert eine Warnung, wenn das Fenster des schnellen Modells zu klein ist, um das Context-Budget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVG geht von einer hellen Leinwand aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, anstatt sich dunkel-auf-dunkel zu verwandeln.

- **Der Browser-Tab-Busy-Indikator verschwindet nicht mehr mitten in einem Turn.** Während ein Turn läuft, behält das Tab-Symbol die App-Markierung und pulsiert sie zwischen voller Farbe und Grau, hält für den gesamten Run und stellt das echte Symbol zuverlässig wieder her, wenn der Turn endet, auch nach In-App-Navigation. Mit deaktivierter Bewegung wird eine statische graue Markierung statt Animation beibehalten.

- **Agents rufen einen Endpoint nicht mehr Dutzende Male nach einem abgeschnittenen Ergebnis ab.** Abschnitt-Hinweise führen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie an, und ein Agent, der denselben Endpoint immer wieder mit unterschiedlichen Parametern bombardiert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Run meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgs-Häkchen geliefert; Fehler gehen jetzt durch den Fehler-Kanal und werden als Fehlerbenachrichtigung angezeigt.

- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr die Ausgabe, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positions-Cursor wie der Resume-Endpoint, daher werden nach einer unterbrochenen Verbindung nur die verpassten Events wiedergegeben, anstatt des gesamten Turns.

- **Gestreamte Events, die über Netzwerk-Chunks aufgeteilt sind, gehen nicht mehr verloren.** Ein fortgesetzter Stream könnte ein Event verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Das Aufrufen eines anderen Agents gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agents kam früher in einer rohen Ausführungs-Trace begraben an, was den Context des aufrufenden Agents überflutete; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Run nicht mehr unbegrenzt stecken lassen.** Jeder Tool-Aufruf hat jetzt ein Wall-Clock-Zeitlimit (Standard 180s), daher endet ein steckender Connector oder nicht reagierender Server den Schritt mit einer klaren Meldung, anstatt den Turn zu blockieren.
...
- **Lange Unterhaltungen verlieren ihren früheren Context nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Turns verdichtet, wurde beim nächsten Run stillschweigend gelöscht; sie wird jetzt weitergeleitet, daher behält der Agent, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Context-Fenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und später Ergebnisse erhalten ein garantiertes Mindestfenster, anstatt vollständig gelöscht zu werden.

- **Die gleiche generierte Datei wird nicht mehr als zwei Downloads angezeigt.** Eine gerenderte HTML-Ausgabe, die erneut unter einem echten Dateinamen gespeichert wurde, erzeugte früher zwei Deliverable-Karten mit identischem Inhalt; Duplikate werden jetzt nach Inhalt erkannt und in die speziell benannte Datei zusammengefasst.

- **Interne Offload-Dateien können nicht mehr als herunterladbare Deliverables angezeigt werden.** Da Tools ein gemeinsames Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Execution-Runs offloaded wurde, vom Artifact-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Runtime-Bookkeeping-Dateien werden jetzt von der Artifact-Erkennung ausgeschlossen.

- **Jedes Tool in einer Unterhaltung funktioniert jetzt auf einem gemeinsamen Dateisystem.** Offgeladene große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Reading-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann gesagt bekommen, dass sie nicht existiert, und sein Iterations-Budget bei der Suche danach verbrauchen. Offgeladene Dateien befinden sich jetzt in der Conversation-Sandbox, die jedes Tool (einschließlich Exec-Container) bereitstellt; Dateien aus älteren Unterhaltungen bleiben lesbar.

- **Das Lesen einer großen offgeladenen Datei erzeugt nicht mehr eine endlose Kette von Zeiger-Dateien.** Die Ausgabe eines Workspace-Reads könnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte zeigt, die bereits auf der Festplatte waren, was den Agent in Kreise schickt. Workspace-Reads werden jetzt mit einer genauen Resume-Zeile begrenzt, anstatt erneut offgeloaded zu werden.
...
- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabe-Budget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend gelöscht und der einleitende Text des Agents — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, daher wurde ein Schritt, der nichts produzierte, als abgeschlossen gemeldet. Die Kürzung wird jetzt erkannt, und dem Agent wird gesagt, die Arbeit in kleinere Aufrufe aufzuteilen.

- **Ein Ziel, das der Agent sicher verpasst hat, wird jetzt erneut versucht, anstatt den Run zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „das Deliverable fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Runs stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agents wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, daher kamen Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" an, anstatt die Antwort selbst. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` ist nur Sekunden-Genauigkeit, daher zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgestellt wurden, kodiert identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt jetzt eine eindeutige `jti`, daher unterscheidet sich das rotierte Token und das alte stoppt sofort zu funktionieren.

- **Approval Gates halten jetzt, wenn ein Agent durch einen anderen Agent oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat um Bestätigung bitten würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einem Workflow `AGENT`-Knoten umhüllt wurde — diese Pfade starteten den Agent ohne Hooks überhaupt. Beide führen jetzt die Gates des Agents aus, verwenden die Bestätigungseinstellungen dieses Agents, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Früher fiel jeder andere Wert als `ssl` oder `tls` (zum Beispiel `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung durch, sowohl in System-E-Mails als auch in E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
