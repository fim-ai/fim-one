...
- **Reasoning-Blöcke bleiben während einer Ausführung eingeklappt.** Reasoning in ReAct- und DAG-Ausgaben zeigt nun standardmäßig eine einzeilige Vorschau statt sich beim Streaming zu erweitern, wodurch das Ergebnis sichtbar bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDFs und Word-Dateien sind für Chinesisch korrekt formatiert.** Der PDF-Export bettet nun eine echte CJK-Schriftart ein, statt auf eine zu verweisen, was den zusammengefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als unzusammenhängende chinesische Zeichen dargestellt wurden, und Fettdruck funktioniert nun. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer Fettdruck nicht mehr halb-fett aussieht, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften innerhalb einer Antwort der Struktur des Transkripts unterordnet.

- **Agenten bleiben bei langen Aufgaben im Plan.** Der Agent wird nun daran erinnert, seinen Ansatz zu ändern, wenn er dasselbe Tool fruchtlos wiederholt, wird ermutigt, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifizierungsdurchlauf.

- **Gestreamte Antworten werden sanft eingeblendet statt zu erscheinen.** Neu angekommene Blöcke werden beim Streaming kurz eingeblendet, wobei die Einstellung für reduzierte Bewegung des Lesers berücksichtigt wird. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Aufzählungszeichen sitzen hinter dem Text statt mit ihm zu konkurrieren.

- **Block-Aktionen bleiben verborgen, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hover angezeigt und haben eine konsistente Behandlung; sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste hält Chats nun zusammen.** Die Gesprächsliste sitzt direkt unter Neuer Chat und Suche, und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare Hierarchie von oben nach unten.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Ausführungen und früheren Gesprächen werden zu einer einzeiligen Vorschau eingeklappt und beim Klick erweitert; Reasoning wird beim Streaming weiterhin offen gestreamt, während es generiert wird, und wird dann eingeklappt, wenn der Schritt abgeschlossen ist.
