...
- **Endgültige Antworten werden gestreamt, während sie geschrieben werden.** Der Agent signalisiert nun, wenn er mit dem Sammeln fertig ist, und schreibt die Antwort als Live-gestreamte Runde, sodass lange Antworten Token für Token mit vollständigen Details ankommen, anstatt nach einer Wartezeit erneut abgespielt zu werden. `REACT_FINISH_SIGNAL=false` stellt das vorherige Verhalten wieder her.

- **Architektur-Dokumentation ordnet Graph Engineering FIM One zu.** [Planning Landscape](/architecture/planning-landscape) erklärt Loop- vs. Graph Engineering, wie sich das von klassischen visuellen Workflows unterscheidet, wie es auf Workflow / DAG / ReAct abgebildet wird, und was für jede Engine zu übernehmen ist. Es wird auch die aktuelle Tatsache dargelegt, dass jeder DAG-Schritt bereits ein vollständiger ReAct Agent ist (kein One-Shot-LLM-Aufruf). ReAct- und DAG-Engine-Seiten verlinken darauf.

- **Einfache Chats bieten immer Folgefragen an.** Gespräche ohne einen gebundenen Agent erhalten nach jeder Antwort vorgeschlagene nächste Fragen; Agent-gebundene Chats erfordern weiterhin den Pro-Agent-Toggle für Folgefragen (standardmäßig aus). Abschlussmeldungen wie Grüße oder Dankbarkeit können keine Chips zurückgeben.

- **Eine neu gesendete Nachricht startet oben auf dem Bildschirm.** Das Transkript behält gerade genug Platz unter der aktuellen Runde, damit ihre erste Zeile oben ankommt, sodass die Frage in Sicht bleibt und die Antwort in den Platz darunter wächst, anstatt dass die ganze Runde an der unteren Kante klebt. Der Platz verschwindet, sobald die Runde einen Bildschirm übersteigt.

- **Reasoning-Blöcke bleiben während einer Runde zusammengeklappt.** Reasoning in sowohl ReAct- als auch DAG-Ausgabe zeigt nun standardmäßig eine einzeilige Vorschau, anstatt während des Streamings zu erweitern, wodurch das Ergebnis in Sicht bleibt; klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDF- und Word-Dateien sind ordnungsgemäß für Chinesisch gesetzt.** Der PDF-Export bettet nun eine echte CJK-Schriftart ein, anstatt auf eine zu verweisen, was den zusammengefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als unabhängige chinesische Zeichen gerendert wurden, und Fett funktioniert nun. Word-Exporte deklarieren eine Ostasien-Schriftart, sodass gemischter chinesisch/englischer fetter Text nicht mehr halb-fett aussieht, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften in einer Antwort der Struktur des Transkripts selbst unterordnet.

- **Agents bleiben bei langen Aufgaben auf Kurs.** Der Agent wird nun daran erinnert, seinen Ansatz zu ändern, wenn er das gleiche Tool fruchtlos wiederholt, wird angestoßen, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifizierungsdurchgang.

- **Gestreamte Antworten werden sanft eingeleitet, anstatt zu erscheinen.** Neu angekommene Blöcke verblassen kurz, während sie streamen, und respektieren die Voreinstellung für reduzierte Bewegung des Lesers. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Listenaufzählungszeichen sitzen hinter dem Text, anstatt damit zu konkurrieren.

- **Block-Aktionen bleiben aus dem Weg, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hovern angezeigt und teilen sich eine konsistente Behandlung; sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste hält Chats nun zusammen.** Die Gesprächsliste sitzt direkt unter Neuer Chat und Suche, und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare von oben nach unten verlaufende Hierarchie.

- **Reasoning ist standardmäßig zusammengeklappt.** Thinking-Abschnitte in sowohl Live-Läufen als auch vergangenen Gesprächen falten sich zu einer einzeiligen Vorschau zusammen und erweitern sich beim Klicken; Reasoning streamt weiterhin offen, während es generiert wird, faltet sich dann zusammen, wenn der Schritt abgeschlossen ist.

- **Listenseiten bringen ihre Karten sanft an ihren Platz.** Agents, Connectors, Knowledge Bases, Skills, Workflows, Tools, Conversations, Artifacts und der Marketplace staffeln ihre Karten beim ersten Laden gestaffelt ein, anstatt alle auf einmal zu erscheinen. Filtern und Paging bleiben sofort, sodass das Tippen in ein Suchfeld die Eingabe nie erneut abspielt.
