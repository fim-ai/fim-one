...
- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Verifizierung durch, bevor er antwortet; diese Verifizierung wurde bisher wörtlich an Sie gestreamt, sodass Antworten als "Let me verify: 1. Fully addressed? ✅ …" ankamen, statt als die eigentliche Antwort. Die Prüfung ist nun intern, und die Antwort, die Sie sehen, wird als eigener Schritt formuliert.

- **Die Rotation von Refresh-Tokens invalidiert nun tatsächlich das vorherige Token.** JWT `exp` hat nur Sekundenpräzision, daher codierten zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, früher identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte auf unbestimmte Zeit erneuert werden. Jedes Refresh-Token trägt nun eine eindeutige `jti`, sodass das rotierte Token sich unterscheidet und das alte sofort nicht mehr funktioniert.

- **Approval Gates halten nun an, wenn ein Agent über einen anderen Agent oder einen Workflow erreicht wird.** Ein sensitiver Connector-Aufruf, der in einem Chat um Bestätigung fragen würde, lief bisher ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingebunden wurde — diese Pfade starteten den Agent ohne Hooks. Beide führen nun die Hooks des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Bisher führte jeder Wert außer `ssl` oder `tls` (zum Beispiel `SMTP_SSL=true`) zu einer unverschlüsselten Verbindung ohne Warnung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden nun abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
