...
- **Kleine Vergleichstabellen in Chat-Antworten werden jetzt als Card-Layouts dargestellt.** Eine Tabelle mit 2–3 Spalten, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (entspricht dem Vergleichskarten-Stil, der von KI-Chat-UIs popularisiert wurde); größere Tabellen behalten das klassische Gitter.

### Behoben

- **Der Beschäftigungsindikator im Browser-Tab verschwindet nicht mehr während eines Durchlaufs.** Das Favicon zeigt nun während des gesamten Durchlaufs ein stabiles Beschäftigungssymbol an und stellt das echte Symbol zuverlässig wieder her, wenn der Durchlauf endet, auch nach der Navigation innerhalb der App.

- **Agenten rufen einen Endpunkt nicht mehr dutzende Male nach einem abgekürzten Ergebnis ab.** Abkürzungsmitteilungen beginnen nun mit einem imperativen Hinweis auf die gespeicherte vollständige Kopie, und ein Agent, der denselben Endpunkt wiederholt mit unterschiedlichen Parametern aufruft, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Durchlauf meldet nun einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshäkchen geliefert; Fehler durchlaufen nun den Fehlerkanal und werden als Fehlerbenachrichtigung angezeigt.

- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr bereits angesehene Ausgaben.** Der Live-Stream trägt nun denselben Positionscursor wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse statt des gesamten Durchlaufs wiedergegeben werden.

- **Über Netzwerk-Chunks verteilte gestreamte Ereignisse gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade nutzen nun einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Der Aufruf eines anderen Agenten gibt nun seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher in einer rohen Ausführungsverfolgung begraben an und überflutete den Kontext des aufrufenden Agenten; er erhält nun nur die Antwort.

- **Ein hängender Tool kann einen Durchlauf nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat nun ein Wanduhrzeitlimit (Standard 180s), sodass ein blockierter Connector oder ein nicht reagierender Server den Schritt mit einer klaren Meldung beendet, statt den Durchlauf zu blockieren.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird nun früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen abgelehnt, statt das gesamte Iterationsbudget zu verbrauchen.

- **Lange Konversationen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Durchlauf stillschweigend verworfen; sie wird nun weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten sowohl ihr Ende als auch ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden nun beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und spätere Ergebnisse erhalten ein garantiertes Mindestfenster statt vollständig gelöscht zu werden.

- **Die gleiche generierte Datei wird nicht mehr als zwei Downloads angezeigt.** Eine gerenderte HTML-Ausgabe, die erneut unter einem echten Dateinamen gespeichert wurde, erzeugte früher zwei Lieferkarten mit identischem Inhalt; Duplikate werden nun nach Inhalt erkannt und in die spezifisch benannte Datei zusammengefasst.

- **Interne Offload-Dateien können nicht mehr als herunterladbare Liefergegenstände angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungsdurchlaufs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als falsche Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien werden nun von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Konversation arbeitet nun auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann wird ihm gesagt, dass sie nicht existiert, und verbraucht sein Iterationsbudget bei der Suche danach. Ausgelagerte Dateien befinden sich nun in der Konversations-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Konversationen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte verweist, die bereits auf der Festplatte waren, was den Agenten in Kreise schickt. Workspace-Lesevorgänge werden nun mit einer exakten Resume-Zeile begrenzt, statt erneut ausgelagert zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, statt den Funktionsaufrufskanal zu nutzen, erreichte das rohe Markup die Chat wörtlich. Es wird nun wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halbgeschriebene Aufruf stillschweigend verworfen und der einleitende Text des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird nun erkannt, und dem Agenten wird mitgeteilt, dass er es mit kleineren Aufrufen erneut versuchen soll.

- **Ein Ziel, das der Agent sicher verpasst hat, wird nun erneut versucht, statt den Durchlauf zu beenden.** Die Neuplanung stoppte, wenn der Analyzer sicher war, was ein sicheres „das Liefergut fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Durchläufe stoppen nun früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Verifizierung durch, bevor er antwortet; diese Verifizierung wurde wörtlich zu dir gestreamt, sodass Antworten als „Lassen Sie mich überprüfen: 1. Vollständig beantwortet? ✅ …" statt der Antwort selbst ankamen. Die Prüfung ist nun intern, und die Antwort, die du siehst, wird als eigener Schritt geschrieben.

- **Die Aktualisierung von Refresh-Tokens invalidiert nun tatsächlich das vorherige Token.** JWT `exp` hat nur Sekundenpräzision, sodass zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, früher identisch codiert wurden — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt nun eine eindeutige `jti`, sodass sich das rotierte Token unterscheidet und das alte sofort nicht mehr funktioniert.

- **Genehmigungsgates halten nun, wenn ein Agent über einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat um Bestätigung bitten würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen nun die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet dein SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Früher fiel jeder Wert außer `ssl` oder `tls` (z. B. `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung zurück, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden nun kategorisch abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
