...
- **Ein abgestürzter Lauf meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgs-Häkchen geliefert; Fehler durchlaufen jetzt den Fehlerkanal und werden als Fehlerbenachrichtigung angezeigt.

- **Das erneute Verbinden mit einem Stream wiederholt keine bereits angesehene Ausgabe mehr.** Der Live-Stream trägt jetzt denselben Positionszeiger wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse statt des gesamten Durchlaufs wiedergegeben werden.
...
- **Das Aufrufen eines anderen Agenten gibt jetzt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher in einer rohen Ausführungsverfolgung begraben an und überflutete den Kontext des aufrufenden Agenten; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Lauf nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Zeitlimit (Standard 180s), sodass ein blockierter Connector oder ein nicht reagierender Server den Schritt mit einer klaren Meldung beendet, statt den Durchlauf zu blockieren.
...
- **Lange Unterhaltungen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Lauf stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent weiß, was vorher besprochen wurde.

- **Große Tool-Ergebnisse behalten jetzt ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und spätere Ergebnisse erhalten ein Mindestfenster statt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Bei Tools, die ein gemeinsames Dateisystem nutzen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungslaufs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien werden jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Unterhaltung arbeitet jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo arbeiteten — der Agent würde eine Datei auflisten, dann wird ihm gesagt, dass sie nicht existiert, und verbraucht sein Iterationsbudget bei der Suche danach. Ausgelagerte Dateien befinden sich jetzt in der Unterhaltungs-Sandbox, die jedes Tool (einschließlich Exec-Container) bereitstellt; Dateien aus älteren Unterhaltungen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte verweist, die bereits auf der Festplatte waren, was den Agenten in Kreise schickte. Workspace-Lesevorgänge werden jetzt mit einer genauen Resume-Zeile begrenzt, statt erneut ausgelagert zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, statt den Function-Calling-Kanal zu nutzen, erreichte das rohe Markup den Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells unterbrochen wird, wird nicht mehr als abgeschlossene Arbeit akzeptiert.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf aufbrauchte, wurde der halbgeschriebene Aufruf stillschweigend verworfen und der Einleitungstext des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und dem Agenten wird gesagt, den Vorgang mit kleineren Aufrufen zu wiederholen.
