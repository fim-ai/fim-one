...
- **Kleine Vergleichstabellen in Chat-Antworten werden jetzt als Card-Layouts dargestellt.** Eine Tabelle mit 2–3 Spalten, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (im Vergleich-Card-Stil, der von KI-Chat-UIs popularisiert wurde); größere Tabellen behalten das klassische Raster.

- **Chat-Antworten rendern jetzt Mermaid-Diagramme und SVG-Abbildungen.** Ein `mermaid`-Codeblock wird zu einem themenbezogenen Diagramm und ein `svg`-Block zu einem Bild mit Quellumschalter und Download; beide fallen auf reinen Code zurück, wenn die Quelle unvollständig ist oder nicht geparst wird, sodass ein halb gestreamtes Diagramm niemals einen Fehler anzeigt.
...
- **Zitate zeigen, aus welchem Dokument sie stammen.** Ein Zitiermarker wird jetzt als Pill dargestellt, die den Dokumentnamen der Quelle trägt, wobei benachbarte Marker in einer Pill zusammengefasst und gezählt werden; das Popover listet jede Quelle dahinter auf.

- **`/clear` funktioniert in der Chat-Eingabe.** Die Eingabe von `/clear` startet ein neues Gespräch, identisch mit dem Klick auf „Neuer Chat"; ein laufender Turn wird zuerst gestoppt.
...
- **Exportierte PDFs und Word-Dateien sind korrekt für Chinesisch formatiert.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, anstatt auf eine zu verweisen, was den eingefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als unzusammenhängende chinesische Zeichen dargestellt wurden, und Fettdruck funktioniert jetzt. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer Fetttext nicht mehr halb-fett dargestellt wird, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften einer Antwort der Struktur des Transkripts unterordnet.

- **Agenten bleiben bei langen Aufgaben im Plan.** Der Agent wird jetzt daran erinnert, seinen Ansatz zu ändern, wenn er das gleiche Werkzeug erfolglos wiederholt, wird angestoßen, einen Plan zu schreiben, wenn er ohne einen arbeitet, und kann keine endgültige Antwort liefern, während noch Planelemente offen sind, ohne einen expliziten Verifizierungsdurchgang.

- **Gestreamte Antworten werden sanft eingeblendet statt plötzlich zu erscheinen.** Neu angekommene Blöcke werden beim Streamen kurz eingeblendet und berücksichtigen die Voreinstellung für reduzierte Bewegung des Lesers. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Aufzählungszeichen sitzen hinter dem Text, anstatt damit zu konkurrieren.

- **Blockaktionen bleiben verborgen, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Codeblöcken, Diagrammen und Tabellen werden beim Hovern angezeigt und verwenden eine konsistente Behandlung; sie bleiben über die Tastatur erreichbar.
...
- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Läufen und früheren Gesprächen werden zu einer einzeiligen Vorschau eingeklappt und beim Klicken erweitert; Reasoning wird beim Generieren weiterhin offen gestreamt, wird dann eingeklappt, wenn der Schritt abgeschlossen ist.
...
- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als „Knowledge Base" gekennzeichnet, weil das Abruftool den ursprünglichen Dateinamen verwarf; es meldet jetzt die Quelle, was der Grounding-Pipeline entspricht.

- **Lange Unterhaltungen treffen weniger wahrscheinlich auf harte Kontextüberlauffehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unterhalb des Eingabelimits des Modells ausgelöst, anstatt genau darauf, und beim Start wird eine Warnung protokolliert, wenn das Fenster des schnellen Modells zu klein ist, um das Kontextbudget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVGs gehen von einer hellen Leinwand aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, anstatt dunkel auf dunkel zu werden.

- **Der Browser-Tab-Aktivitätsindikator verschwindet nicht mehr während eines Durchlaufs.** Das Favicon zeigt jetzt ein gleichmäßiges Aktivitätssymbol für den gesamten Durchlauf und stellt das echte Symbol zuverlässig wieder her, wenn der Durchlauf endet, auch nach In-App-Navigation.
...
- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr bereits gesehene Ausgaben.** Der Live-Stream trägt jetzt denselben Positionszeiger wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse wiedergegeben werden, anstatt des gesamten Durchlaufs.

- **Gestreamte Ereignisse, die über Netzwerk-Chunks verteilt sind, gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.
...
- **Lange Unterhaltungen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Durchlauf stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und später Ergebnisse erhalten ein garantiertes Mindestfenster, anstatt vollständig gelöscht zu werden.

- **Die gleiche generierte Datei wird nicht mehr als zwei Downloads angezeigt.** Eine gerenderte HTML-Ausgabe, die erneut unter einem echten Dateinamen gespeichert wurde, erzeugte früher zwei identische Lieferkarten; Duplikate werden jetzt nach Inhalt erkannt und in die speziell benannte Datei zusammengefasst.

- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Ausführungsdurchlaufs ausgelagert wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Buchführungsdateien sind jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einer Unterhaltung funktioniert jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lesewerkzeuge sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann würde ihm gesagt, dass sie nicht existiert, und würde sein Iterationsbudget bei der Suche danach verbrauchen. Ausgelagerte Dateien befinden sich jetzt in der Unterhaltungs-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Unterhaltungen bleiben lesbar.
...
- **DeepSeeks improvisierten Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, anstatt den Function-Calling-Kanal zu verwenden, erreichte das rohe Markup das Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der Einleitungstext des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Abkürzung wird jetzt erkannt, und dem Agenten wird mitgeteilt, die Arbeit in kleinere Aufrufe aufzuteilen.
