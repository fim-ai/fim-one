...
- **Das Löschen eines Kontos entfernt nun konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" unterschieden sich früher – jeder bereinigt eine andere Menge von Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad ließ API-Schlüssel, Anmeldungsverlauf, Evaluierungsdaten, Aufruflogs und Abos zurück. Beide durchlaufen nun eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Nachricht abgelehnt, um zuerst den Besitz zu übertragen (oder die Organisation zu löschen), anstatt unerwartet fehlzuschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten unterbricht nun keine vergangenen Konversationen mehr.** Konversationen, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agenten-Link gelöscht, anstatt das Löschen zu blockieren.
- **Connector-Aktionen, die eine Bestätigung erfordern, können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Connector-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher innerhalb eines Workflows mit dem Genehmigungsgate stillschweigend umgangen ausgeführt. Der Schritt schlägt nun geschlossen fehl mit einer Nachricht, um einen Human Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungsgate) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Eigentümers.** Wenn der MCP-Server eines Workflows das Fallback für Eigentümer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Eigentümer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Eigentümers zurück, anstatt gestoppt zu werden. Der Schritt schlägt nun mit einer klaren Nachricht fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Connector-Aufrufe aus Workflows werden nun im Audit-Log protokolliert.** Eine Connector-Aktion, die innerhalb eines Workflows ausgeführt wurde, hinterließ keinen Eintrag im Connector-Aufrufsprotokoll (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun die gleiche Audit-Spur.
- **Sie können nun einen abonnierten Connector, eine Knowledge Base oder einen MCP-Server an einen Agenten anhängen.** Das Binden erforderte früher, dass Sie die Ressource *besaßen*, sodass ein Connector, den Sie legitim abonniert hatten (Org-freigegeben oder Market-installiert), als „nicht im Besitz" abgelehnt wurde. Das Binden folgt nun der gleichen Sichtbarkeitsregel, die der Rest der App verwendet – alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Schritte können nur Connectoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Connector-Schritt eines Workflows konnte früher jeden Connector aufrufen, dessen ID er referenzierte, solange dieser Connector existierte; er erzwingt nun die Sichtbarkeit des Ausführenden (im Besitz oder abonniert), sodass ein freigegebener Workflow keinen Connector erreichen kann, auf den Sie keinen Zugriff haben, nur durch Referenzierung seiner ID.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Laufzeit nur schwach verweist, sodass sie unter Last während des Flugs vom Garbage Collector entfernt und stillschweigend gestoppt werden konnten. Sie werden nun bis zur Fertigstellung beibehalten (und ihre Fehler protokolliert), sodass sie zuverlässig abgeschlossen werden.
...
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Forschungsschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter – sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements könnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Werkzeugausgabe als autoritative Quellenbeweise, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn dies nicht der Fall ist).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hardcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Schalter, der ein internes (automatisch herabgestuftes) Konzept offenlegte. Beide werden entfernt – der Knoten führt nun immer Ihr systemkonfiguriertes Modell aus (Model Provider, mit Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf den Ort verweist, an dem das Modell konfiguriert ist.
