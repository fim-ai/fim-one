...
- **Das Löschen eines Kontos entfernt nun konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" unterschieden sich früher – jeder bereinigt eine andere Menge an Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad hinterließ API-Schlüssel, Anmeldungsverlauf, Evaluierungsdaten, Aufruflogs und Abos. Beide durchlaufen nun eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Meldung abgelehnt, um zunächst den Besitz zu übertragen (oder die Organisation zu löschen), anstatt unklar fehlzuschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten unterbricht nun nicht mehr frühere Konversationen.** Konversationen, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agenten-Link gelöscht, anstatt das Löschen zu blockieren.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Eigentümers.** Wenn ein MCP-Server eines Workflows das Fallback auf Eigentümer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Eigentümer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Eigentümers zurück, anstatt gestoppt zu werden. Der Schritt schlägt nun mit einer klaren Meldung fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Connector-Aufrufe aus Workflows werden nun protokolliert.** Ein Connector-Aktion, die in einem Workflow ausgeführt wurde, hinterließ keinen Eintrag im Connector-Aufrufsprotokoll (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun die gleiche Audit-Spur.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Runtime nur schwach verweist, sodass sie unter Last während des Flugs Garbage-collected werden konnten und stillschweigend stoppten. Sie werden nun bis zur Fertigstellung beibehalten (und ihre Fehler protokolliert), sodass sie zuverlässig abgeschlossen werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron in der Hauptereignisschleife ab und fror kurzzeitig alle gleichzeitigen Anfragen ein, während eine große Datei analysiert wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Multi-Step-(DAG-)Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn ein Planungsschritt eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter – sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Tool-Ausgabe als autoritative Quellenbelege, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
