...
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft nun deren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Pro-Benutzer-Connector-/MCP-Anmeldedaten, die Sie dafür gespeichert haben, werden gelöscht – sodass Sie diese Ressourcen nach dem Verlassen nicht mehr nutzen oder auf die Anmeldedaten des Eigentümers zurückgreifen können. Ressourcen, die Sie über den Marketplace abonniert haben, sind nicht betroffen.
...
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird nun mit einer klaren Nachricht abgelehnt, um zuerst das Eigentum zu übertragen (oder die Organisation zu löschen), anstatt undurchsichtig fehlzuschlagen oder die Organisation verwaist zu hinterlassen.
...
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Runtime nur schwach verwies, sodass sie unter Last während des Flugs von der Garbage Collection erfasst werden konnten und stillschweigend stoppten. Sie werden nun bis zum Abschluss beibehalten (und ihre Fehler protokolliert), sodass sie zuverlässig beendet werden.
- **Große Datei-Uploads blockieren nun nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife und fror kurzzeitig alle gleichzeitigen Anfragen ein, während eine große Datei analysiert wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Multi-Schritt-(DAG-)Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Rechercheschritt eines Plans eine Seite abgerufen und zusammengefasst hat, floss nur diese Zusammenfassung weiter – sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erfassen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Toolausgabe als autoritative Quellenbelege, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen bei einer benannten Diskrepanz sinkt, wenn dies nicht der Fall ist).
