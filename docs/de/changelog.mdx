...
- **Reine Transformationsschritte in DAG-Plänen werden als einzelner direkter LLM-Aufruf ausgeführt.** Der Planner kennzeichnet Merge-/Rewrite-/Zusammenfassungsschritte, die nur Upstream-Ausgaben verbrauchen, als direkte Schritte und überspringt die Tool-Schleife, die sie nie verwenden würden, was Kosten und Latenz senkt; jedes Schrittergebnis erfasst nun auch, wie es ausgeführt wurde (direkter Aufruf oder Agent-Schleife, Iterationen, verwendete Tools).

- **Agenten können während der Ausführung klärende Multiple-Choice-Fragen stellen.** Wenn eine Aufgabe in wesentlich unterschiedliche Richtungen gehen könnte, pausiert der Agent und präsentiert bis zu vier strukturierte Fragen im Chat, jeweils mit 2-4 Optionen plus einem Freitextfeld „Sonstiges"; die Beantwortung setzt die Ausführung an Ort und Stelle fort, und das Überspringen oder ein Timeout lässt ihn nach eigenem Ermessen fortfahren.

- **Agent-Schritte haben Live-Einzeilentitel.** Während ein Agent arbeitet, erhält jede Iteration ein kurz generiertes Label („Preisdaten werden überprüft…"), das in einer einzelnen eingeklappten Kopfzeile angezeigt wird, die zum neuesten Schritt wechselt; das Erweitern zeigt die vollständige Schritt-Zeitleiste, und die Labels bleiben in der Gesprächshistorie erhalten.

- **Die Modelle eines Providers können in Masse gelöscht werden.** Jede Modellzeile in Admin → Models trägt ein Kontrollkästchen, die Kopfzeile wählt die ganze Liste aus, und das Halten von Shift beim Klicken erweitert die Auswahl über einen Bereich von Zeilen. Das Löschen der Auswahl entfernt jedes ausgewählte Modell in einer Anfrage, nach einer Bestätigung, die die Anzahl nennt.

- **Kleine Vergleichstabellen in Chat-Antworten werden nun als kartenähnliche Layouts dargestellt.** Eine Tabelle mit 2-3 Spalten, deren erste Spalte Attributbezeichnungen enthält, zeigt jedes Attribut in seiner Wertzelle an (entsprechend dem Vergleichskartenstil, der von KI-Chat-UIs popularisiert wurde); größere Tabellen behalten das klassische Raster.

- **Chat-Antworten rendern nun Mermaid-Diagramme und SVG-Abbildungen.** Ein `mermaid`-Codeblock wird zu einem thematisierten Diagramm und ein `svg`-Block wird zu einem Bild mit einem Quellumschalter und Download; beide fallen auf einfachen Code zurück, wenn die Quelle unvollständig ist oder nicht analysiert wird, sodass ein halb gestreamtes Diagramm nie einen Fehler anzeigt.
...
- **Zitate zeigen, aus welchem Dokument sie stammen.** Ein Zitiermarker wird nun als Pille mit dem Namen des Quelldokuments dargestellt, wobei benachbarte Marker in einer Pille und einer Anzahl zusammengefasst werden; das Popover listet jede Quelle dahinter auf.
