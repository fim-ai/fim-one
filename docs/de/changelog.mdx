...
- **README Star History Chart wird wieder angezeigt.** Das Live-Embed von `api.star-history.com` läuft nach GitHub-Beschränkungen der Stargazers-API (Jul 2026) ab; das Diagramm ist jetzt ein statischer SVG-Snapshot im Repository (aktualisieren mit `python3 scripts/gen_star_history.py`).

- **Refresh-Token-Rotation invalidiert jetzt tatsächlich den vorherigen Token.** JWT `exp` hat nur Sekundengenauigkeit, daher codierten zwei für denselben Benutzer in derselben Sekunde ausgegebene Refresh-Tokens früher identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Refresh-Token konnte auf unbestimmte Zeit erneuert werden. Jeder Refresh-Token trägt jetzt eine eindeutige `jti`, daher unterscheidet sich der rotierte Token und der alte funktioniert sofort nicht mehr.

- **Approval Gates halten jetzt an, wenn ein Agent über einen anderen Agent oder einen Workflow erreicht wird.** Ein sensitiver Connector-Aufruf, der in Chat um Bestätigung fragen würde, lief früher ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingebunden wurde — diese Pfade starteten den Agent ohne Hooks. Beide führen jetzt die eigenen Gates des Agents aus, verwenden die Bestätigungseinstellungen dieses Agents und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Früher führte jeder andere Wert als `ssl` oder `tls` (zum Beispiel `SMTP_SSL=true`) zu einer unverschlüsselten Verbindung ohne Warnung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
