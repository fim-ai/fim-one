...
- **Ein Benutzer, der noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird jetzt mit einer klaren Nachricht abgelehnt, um zuerst den Eigentümer zu übertragen (oder die Organisation zu löschen), anstatt unklar zu fehlschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agenten unterbricht jetzt nicht mehr frühere Gespräche.** Gespräche, die auf den Agenten verwiesen haben, werden als historische Datensätze beibehalten und haben einfach den Agenten-Link gelöscht, anstatt das Löschen zu blockieren.
- **Connector-Aktionen, die eine Bestätigung erfordern, können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Connector-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher innerhalb eines Workflows mit dem Genehmigungstor stillschweigend umgangen ausgeführt. Der Schritt schlägt jetzt geschlossen fehl mit einer Nachricht, um einen Human Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungstor) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Eigentümers.** Wenn ein MCP-Server eines Workflows das Fallback auf Eigentümer-Anmeldedaten deaktiviert hatte, fiel ein Nicht-Eigentümer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Eigentümers zurück, anstatt gestoppt zu werden. Der Schritt schlägt jetzt mit einer klaren Nachricht fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Connector-Aufrufe aus Workflows werden jetzt im Audit-Log protokolliert.** Ein Connector-Aktion, die innerhalb eines Workflows ausgeführt wurde, hinterließ keinen Eintrag im Connector-Aufruflogs (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben jetzt die gleiche Audit-Spur.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Runtime nur schwach verweist, sodass sie unter Last während des Flugs Garbage-collected werden konnten und stillschweigend stoppten. Sie werden jetzt bis zur Fertigstellung beibehalten (und ihre Fehler protokolliert), sodass sie zuverlässig fertig werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife, wodurch kurzzeitig alle gleichzeitigen Anfragen eingefroren wurden, während eine große Datei analysiert wurde. Die Extraktion wird jetzt auf einen Worker-Thread ausgelagert.
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn ein Planungsschritt eine Seite abgerufen und zusammengefasst hat, floss nur diese Zusammenfassung weiter – sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements könnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält jetzt seine rohe Werkzeugausgabe als autoritative Quellenbeweise, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
