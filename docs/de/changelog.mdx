...
- **API-Schlüssel-Bereiche werden nun pro Anfrage übertragen, anstatt an das Benutzerobjekt angehängt zu werden.** Dies schließt einen latenten Scope-Verwirrungs-Pfad, bei dem ein gecachter oder Identity-Map-gemeinsamer Benutzer den Scope-Satz eines anderen Auth-Kontexts erben könnte.
...
- **Die Kosten für das Passwort-Hashing sind nun über `FIM_BCRYPT_COST`** (Standard 12, begrenzt auf bcrypts Bereich 4–31) abstimmbar, damit Betreiber zwischen Hardware- und Härtungsanforderungen abwägen können.
- **Datenbankverbinder können nicht mehr gemeinsam genutzt werden, und ihr Raw-SQL-Tool ist nur für den Eigentümer.** Das Veröffentlichen eines Datenbankverbinders für eine Organisation oder global wird nun abgelehnt, und das Arbitrary-SQL-Datenbankwerkzeug wird nur dem Eigentümer des Verbinders angeboten — niemals einem Abonnenten, unabhängig von der Einstellung für das Eigentümer-Anmeldedaten-Fallback. Zuvor exponierte ein gemeinsam genutzter Datenbankverbinder uneingeschränktes SQL über das (typischerweise hochprivilegierte) Datenbankkonto des Eigentümers für jeden, der sich abonnierte, ohne Pro-Aufrufer-Scoping. Der Zugriff auf Datenbanken mit Pro-Benutzer-Scoping wird durch eine zukünftige deklarative Aktionsebene zurückkehren.
- **Webhook- und Cron-ausgelöste Workflows werden nun gegen das Token-Kontingent des Eigentümers angerechnet.** Ein unbeaufsichtigter Workflow-Durchlauf — ausgelöst durch seinen öffentlichen API-Schlüssel oder nach einem Zeitplan — wurde früher LLM- und Agent-Schritte ausführen, ohne die Token-Nutzung zu prüfen oder zu erfassen, ein ungezählter kostenloser-LLM-Pfad, der Kontingente vollständig umging. Solche Durchläufe werden nun vorab gegen das Kontingent des Workflow-Eigentümers gated (ein öffentlicher Trigger wird abgelehnt, und ein geplanter Durchlauf wird übersprungen, wenn der Eigentümer bereits über dem Kontingent ist) und ihre Token-Nutzung wird pro Durchlauf erfasst und gegen das gleiche monatliche Kontingent wie Chat angerechnet.
...
- **Agent generiert kein überflüssiges Bild mehr, wenn er aufgefordert wird, ein HTML-Deliverable neu zu gestalten, und behauptet niemals, dass eine Datei erstellt wurde, ohne sie tatsächlich in den Workspace zu schreiben.** Die Bildgenerierung ist jetzt explizit auf Rasterbilder beschränkt, und Datei-Deliverables müssen über einen Tool-Aufruf geschrieben werden — das Einfügen von Inhalten im Chat zählt nicht mehr als "Generieren einer Datei".
- **Generierte HTML-Seiten verwenden jetzt standardmäßig eine warme Claude-inspirierte Palette anstelle des generischen Purple-Gradient-Looks.** Wenn Sie keinen Stil angeben, verwendet der Agent eine warme minimale Baseline; Ihr eigener Stil gewinnt immer, wenn er angegeben ist.
...
- **Hintergrund-Tool-Ergebnisse werden jetzt im Step-Stream und in DAG-Evidenz angezeigt.** Ein Tool, das im Hintergrund ausgeführt wurde, lieferte seine echte Ausgabe nur in der internen Benachrichtigungsmeldung des Agenten — die UI-Step-Timeline und die DAG-Antwort-Verifizierungsevidenz sahen nur den Dispatch-Platzhalter. Das geleerte Ergebnis wird jetzt als normales Tool-Completion-Event unter dem echten Tool-Namen ausgegeben.
- **Eine seltene Planner-Ausgabe mit doppelten Step-IDs bricht nicht mehr den gesamten DAG-Turn ab.** Doppelte IDs wurden früher als "zirkuläre Abhängigkeit"-Fehler fehldiagnostiziert; sie werden jetzt deterministisch umbenannt und der Plan wird fortgesetzt.
- **Mehrere ReAct-Loop-Korrekturfixes.** Ein Tool, das leere Ausgabe zurückgibt, behielt seine "abgeschlossen ohne Ausgabe"-Notiz anstelle eines leeren Ergebnisses, wenn der Workspace aktiviert ist; DAG-Steps, die durch eine nicht erfüllbare Abhängigkeit blockiert sind, geben jetzt ein ordnungsgemäßes Terminal-Event aus, anstatt die UI hängen zu lassen; und für Modelle ohne natives Tool-Calling gibt die Fallback-Antwort-Synthese jetzt die Tool-Ergebnisse, nicht nur die Tool-Aufrufe.
- **Der Konversations-Workspace ist jetzt tatsächlich in den Chat integriert — übergroße Tool-Ergebnisse sind wiederherstellbar anstatt stillschweigend verloren.** Die Workspace-Recovery-Pfade der Agent-Schicht (großer Output-Offload mit einem `workspace://`-Zeiger, vollständige Rettung von Budget-gekürzte Tool-Ergebnissen, Pre-Komprimierungs-Transkript-Snapshots) existierten, aber die Chat-Endpoints erstellten niemals einen Workspace, daher liefen sie nicht: Sobald das Tool-Result-Budget erschöpft war, erhielt das Modell leere Beobachtungen ohne Möglichkeit, die Daten zurückzulesen, und antwortete aus Vermutungen. ReAct- und DAG-Step-Agenten erhalten jetzt einen Pro-Konversations-Workspace, und das Löschen einer Konversation bereinigt ihre Workspace-Dateien und alle verbleibenden Crash-Resume-Checkpoints.
- **Wiederaufgenommene DAG-Läufe verlieren nicht mehr das Ende langer Step-Ausgaben.** Step-Evidenz, die im Crash-Resume-Checkpoint gespeichert ist, wurde nur am Anfang bei der Größenbeschränkung gekürzt, wodurch stillschweigend alles am Ende der Ausgabe gelöscht wurde — genau dort, wo Schlussfolgerungen, Checksummen und letzte Zeilen tendenziell sind. Begrenzte Evidenz behält jetzt sowohl den Anfang als auch das Ende mit einem expliziten Auslassungsmarker dazwischen.
- **Agenten "funktionieren" nicht mehr, indem sie ihre eigenen Notizen über Python ausdrucken.** Die Effizienzanleitung, die das Schreiben eines umfassenden Scripts pro Tool-Aufruf förderte, ist jetzt auf Aufgaben beschränkt, die echte Code-Ausführung benötigen — Wissensfragen, Vergleiche und Analysen, die das Modell direkt erzeugen kann, werden direkt beantwortet, anstatt durch sinnlose Sandbox-Scripts geleitet zu werden.
- **Ein Agent-Step in einem Workflow führt jetzt den vollständigen Agent aus, nicht nur seine Anleitung.** Der Agent-Node eines Workflows wurde früher nur mit den Anweisungen des Agenten auf einem generischen Modell und Standard-Toolset ausgeführt — wodurch die Konnektoren, Wissensdatenbanken, MCP-Server und das konfigurierte Modell des Agenten stillschweigend gelöscht wurden. Er führt jetzt den Agent genau so aus, wie er im Chat wäre, mit allen gebundenen Ressourcen und Modellen, und nur wenn dieser Agent für den Workflow-Eigentümer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft jetzt ihren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Pro-Benutzer-Konnektoren/MCP-Anmeldedaten, die Sie dafür gespeichert haben, werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden oder auf die Anmeldedaten des Eigentümers zurückgreifen können. Ressourcen, die Sie über den Market abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt jetzt alles konsistent.** Die Admin-Pfade "Benutzer löschen" und Self-Service "Mein Konto löschen" unterschieden sich früher — jeder bereinigt einen anderen Satz von On-Disk-Dateien (und beide berechneten ihre Dateipfade falsch, daher verpasste jeder stillschweigend einige), und der Admin-Pfad ließ API-Schlüssel, Anmeldungsverlauf, Eval-Daten, Call-Logs und Abonnements zurück. Beide laufen jetzt durch eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr auf Datenbanken fehl, die Fremdschlüssel erzwingen.
...
- **Bestätigungspflichtige Konnektoren-Aktionen können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Konnektoren-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher in einem Workflow mit dem Approval-Gate stillschweigend umgangen ausgeführt. Der Step schlägt jetzt geschlossen fehl mit einer Meldung, um einen Human Intervention-Node davor zu platzieren (das Workflow-native Approval-Gate) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Steps leihen sich nicht mehr die MCP-Anmeldedaten des Eigentümers.** Wenn der MCP-Server eines Workflows das Fallback für Eigentümer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Eigentümer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Eigentümers zurück, anstatt gestoppt zu werden. Der Step schlägt jetzt mit einer klaren Meldung fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Konnektoren-Aufrufe aus Workflows werden jetzt audit-protokolliert.** Ein Konnektoren-Aufruf, der in einem Workflow ausgeführt wurde, hinterließ keinen Eintrag im Konnektoren-Call-Log (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben jetzt denselben Audit-Trail.
- **Sie können jetzt einen abonnierten Konnektoren, eine Wissensdatenbank oder einen MCP-Server an einen Agenten anhängen.** Die Bindung erforderte früher, dass Sie die Ressource *besitzen*, daher wurde ein Konnektoren, den Sie legitim abonniert hatten (Org-gemeinsam oder Market-installiert), als "nicht besessen" abgelehnt. Die Bindung folgt jetzt derselben Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Steps können nur Konnektoren aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Konnektoren-Step eines Workflows führte früher jeden Konnektoren aus, dessen ID er referenzierte, solange dieser Konnektoren existierte; er erzwingt jetzt die Sichtbarkeit des Ausführenden (besessen oder abonniert), daher kann ein gemeinsamer Workflow keinen Konnektoren erreichen, auf den Sie keinen Zugriff haben, nur indem er seine ID referenziert.
- **Wissensdatenbank-Suche funktioniert jetzt gegen KBs, die Sie abonniert haben, nicht nur solche, die Sie besitzen.** Die Daten einer KB befinden sich unter dem Speicher ihres Eigentümers, aber ein Wissensdatenbank-Step eines Workflows (und Wissensdatenbank-Chat ohne einen Agenten) suchte früher unter dem eigenen Speicher des Anrufers — daher gab die Suche einer mit Ihnen geteilten KB stillschweigend nichts zurück. Der Eigentümer wird jetzt aus der KB selbst aufgelöst und die Suche wird zugegriffen (Eigentümer oder Abonnent), daher geben gemeinsame KBs Ergebnisse zurück und KBs, die Sie nicht sehen können, geben nichts anstelle einer verwirrenden leeren Treffer zurück.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokument-Aufnahme, Webhook-Lieferung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Tasks gestartet, auf die die Runtime nur schwach verweist, daher konnten sie unter Last während des Fluges Garbage-Collection unterliegen und stillschweigend stoppen. Sie werden jetzt bis zum Abschluss gehalten (und ihre Fehler protokolliert), daher beenden sie sich zuverlässig.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einem hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife, wodurch alle gleichzeitigen Anfragen kurzzeitig eingefroren wurden, während eine große Datei analysiert wurde. Die Extraktion wird jetzt auf einen Worker-Thread ausgelagert.
- **Multi-Step (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn ein Planungs-Step eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — daher konnte eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen, oder die Schwere eines Elements konnte falsch gekennzeichnet werden, und der Ziel-Evaluierungs-Step konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Step behält jetzt seine rohe Tool-Ausgabe als autoritative Quellen-Evidenz, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, daher stimmen Summen, Aufzählungen und Attribute mit dem überein, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Node zeigt nicht mehr Modell-Steuerelemente an, die nicht bedeuteten, was sie impliziert.** Er hatte ein hartcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Toggle, der ein internes (Auto-Degrading) Konzept offenlegte. Beide werden entfernt — der Node führt jetzt immer Ihr System-konfiguriertes Modell aus (Model Provider, Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf den Ort verweist, an dem das Modell konfiguriert ist.
