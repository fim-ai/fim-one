...
- **Das Löschen eines Agenten unterbricht nun nicht mehr frühere Konversationen.** Konversationen, die auf den Agenten verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agent-Link gelöscht, anstatt das Löschen zu blockieren.
- **Connector-Aktionen, die eine Bestätigung erfordern, können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Connector-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher innerhalb eines Workflows mit dem Genehmigungstor stillschweigend umgangen ausgeführt. Der Schritt schlägt nun geschlossen fehl mit einer Meldung, um einen Human Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungstor) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Eigentümers.** Wenn der MCP-Server eines Workflows das Fallback auf Eigentümer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Eigentümer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Eigentümers zurück, anstatt gestoppt zu werden. Der Schritt schlägt nun mit einer klaren Meldung fehl, um Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Connector-Aufrufe aus Workflows werden nun im Audit-Log erfasst.** Ein Connector-Aktion, die innerhalb eines Workflows ausgeführt wurde, hinterließ keinen Eintrag im Connector-Aufrufsprotokoll (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben nun die gleiche Audit-Spur.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Evaluierungsläufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Laufzeit nur schwach verwies, sodass sie unter Last während des Flugs Garbage-Collected werden konnten und stillschweigend stoppten. Sie werden nun bis zum Abschluss beibehalten (und ihre Fehler protokolliert), sodass sie zuverlässig beendet werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einer hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife, was alle gleichzeitigen Anfragen kurzzeitig einfriert, während eine große Datei analysiert wurde. Die Extraktion wird nun auf einen Worker-Thread ausgelagert.
- **Mehrstufige (DAG) Antworten misrepräsentieren ihre Quellen nicht mehr.** Wenn der Forschungsschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter – sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält nun seine rohe Tool-Ausgabe als autoritative Quellenbeweise, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hartcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Umschalter, der ein internes (automatisch herabgestuftes) Konzept offenlegte. Beide werden entfernt – der Knoten führt nun immer Ihr systemkonfiguriertes Modell aus (Model Provider, Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die auf den Ort verweist, an dem das Modell konfiguriert ist.
