...
- Der Gesprächsexport zeigt nun das korrekte Moduskennzeichen ("Planner" / "规划") für automatisch geroutete DAG-Gespräche an, statt immer "Standard" anzuzeigen.
- Export-Zeitstempel berücksichtigen nun die konfigurierte Zeitzone des Benutzers statt rohe UTC anzuzeigen.
- Hochgeladene Dateiinhalte lecken nicht mehr in exportierte Gespräche; nur der Nachrichtentext des Benutzers ist enthalten.
...
- Einstellungen → Kanäle spiegelt nun die aktuelle Organisationsrolle des Benutzers wider: Mitglieder (nicht-Admin/Besitzer) sehen eine deaktivierte Schaltfläche "Neuer Kanal", versteckte Aktionen zum Bearbeiten / Aktivieren-Deaktivieren / Löschen, ein schreibgeschütztes Banner und einen berechtigungsabhängigen leeren Zustand — statt eines aktivierten CTA, das bei der Übermittlung mit "Organisationsadministrator-Zugriff erforderlich" fehlgeschlagen ist.
- Die Sitzungsablauf-Umleitung bewahrt nun die Query-Zeichenkette, sodass Benutzer nach der erneuten Authentifizierung auf die genaue Registerkarte / den Filter zurückkehren, den sie angesehen haben, statt auf den bloßen Pfad.
...
- Feishu-Kanal-Setup-Hinweise duplizieren keine chinesischen Kennzeichen mehr, wenn die Benutzeroberfläche selbst bereits auf Chinesisch ist (z. B. zuvor "事件与回调 (事件与回调)" angezeigt).
...
- Ausstehende Genehmigungsanfragen verfallen nun automatisch nach `CHANNEL_CONFIRMATION_TTL_MINUTES` (Standard 24h) über einen Hintergrund-Sweeper. Verhindert, dass ein veralteter Klick Tage später einen Agent-Status flippt, der bereits abgebaut wurde; der nächste Klick auf eine abgelaufene Karte erhält eine graue "Abgelaufen"-Entscheidungskarte und einen "nicht mehr aktiv"-Toast.
