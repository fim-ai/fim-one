...
- **GPT-5.x-Agenten führen Tools mit aktiviertem Reasoning aus, statt Fehler zu werfen.** OpenAI-Modelle durchlaufen jetzt zuerst die Responses API, wo Funktionstools und Reasoning zusammenarbeiten; Endpunkte ohne `/v1/responses`-Route fallen automatisch auf Chat Completions zurück, wobei GPT-5.x-Tool-Aufrufe Reasoning explizit deaktivieren, da diese API dies erfordert.

- **Ask-first-Anfragen werden im Planner-Modus in einer Runde abgeschlossen.** Ein Ziel, das den Agenten explizit auffordert, Anforderungen mit Multiple-Choice-Fragen zu klären, bevor er eine Lösung liefert, erzeugt jetzt einen sauberen Fragebogen als Ergebnis der Runde, statt Planungsschritte, die auf Antworten warten, durch Neuplanungen zu loopen und mit „Ziel nicht erreicht" plus wiederholten Fragenlisten zu enden. Der Auto-Modus leitet solche Anfragen an den Standard-Modus weiter.

- **Follow-up-Chips ziehen das Transkript nicht mehr über die Antwort hinaus.** Nachdem eine Runde beim Ergebnis parkt, bleibt späteres Layout-Wachstum (Vorschläge, Titel) an Ort und Stelle, und das Falten der Live-Runde in die Historie parkt wieder am Anfang der Antwort für dasselbe Gespräch, statt zum Ende zu springen.

- **Das Öffnen eines bestehenden Gesprächs landet bei den neuesten Nachrichten.** Das Aktualisieren einer Chat-URL scrollt jetzt nach unten wie beim Wechsel aus der Seitenleiste, statt oben im Thread zu beginnen.

- **Schnelle Antworten sehen nicht mehr fest auf „Wird verarbeitet…" aus.** Eine abgeschlossene Runde ohne generierten Schritt-Titel zeigt stattdessen das Gedankenprozess-Label an, nicht den Live-Verarbeitungs-Shimmer-Text.

- **Animation folgt jetzt der Systemeinstellung „Bewegung reduzieren".** Der driftende Gradient des Login-Hintergrunds, der Hero-Eingang, der atmende eingefügte Nachrichtentext und das Streaming-Caret bewegten sich weiter, unabhängig von der Betriebssystem-Einstellung. Sie bleiben jetzt still, während die Indikatoren, die laufende Arbeiten anzeigen, sichtbar bleiben, damit die Oberfläche nie stecken bleibt.

- **Der Ausführungsmodus-Selector bleibt geschlossen, während eine Runde läuft.** Der Chip war ausgegraut, öffnete aber sein Menü trotzdem bei Klick, anders als der Agenten-Selector daneben; beide sind jetzt inert, bis der Lauf endet.

- **Reasoning-Streams funktionieren wieder bei neueren Modellen.** Live-Reasoning war aus einer hartcodierten Liste von Modellnamen verdrahtet, also zeigte alles, was nach dieser Liste veröffentlicht wurde, einen Spinner und dann den ganzen Reasoning-Block auf einmal. Jedes Modell, das Reasoning ausgibt, streamt es jetzt.

- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie steckt nicht mehr mitten im Reasoning fest und meldet fälschlicherweise neuen Inhalt darunter, folgt Inhalten, die ohne neue Events wachsen, und wenn eine Runde endet, bleibt sie oben beim Ergebnis statt unten. Wegscollen gibt die Kontrolle immer noch ab, und die Rückkehr nach unten setzt das Folgen fort.

- **Eine kurze Antwort bleibt dort, wo Sie sie gelesen haben, nachdem die Runde endet.** Sobald eine Runde endete, rutschte das Transkript nach unten, damit Frage und Antwort die Eingabebox umarmten; die Antwort behält jetzt ihre Position oben in der Ansicht, bis die nächste Nachricht gesendet wird.

- **Die Anmeldung wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landingpage wird neu geladen, wenn sich die Sprache des Kontos von der unterscheidet, in der die Anmeldeseite gerendert wurde, damit das Ziel nicht mehr in der vorherigen Sprache erscheint, bis zur nächsten Aktualisierung.

- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener blieb aktiv, aber sein Dialog wurde im eingeklappten Zustand nie eingebunden, also machte der Shortcut stillschweigend nichts.

- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle mit „Knowledge Base" gekennzeichnet, weil das Abruf-Tool den Ursprungsdateinamen fallen ließ; es meldet jetzt die Quelle, was der Grounding-Pipeline bereits tat.

- **Lange Gespräche treffen weniger wahrscheinlich auf harte Kontext-Overflow-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unter dem Eingabelimit des Modells ausgelöst, statt genau darauf, und der Start protokolliert eine Warnung, wenn das Fenster des schnellen Modells zu klein ist, um das Kontext-Budget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im Dark Mode lesbar.** Von Modellen erstellte SVGs gehen von einer hellen Leinwand aus, also werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, statt dunkel-auf-dunkel zu werden.

- **Der Busy-Indikator des Browser-Tabs verschwindet nicht mehr mitten in einer Runde.** Während eine Runde läuft, behält das Tab-Symbol die App-Markierung und pulsiert sie zwischen voller Farbe und Grau, hält für den ganzen Lauf und stellt das echte Symbol zuverlässig wieder her, wenn die Runde endet, auch nach In-App-Navigation. Mit aktivierter Bewegungsreduktion hält es eine statische graue Markierung statt zu animieren.

- **Agenten rufen einen Endpunkt nicht mehr Dutzende Male nach einem abgeschnittenen Ergebnis ab.** Abschneidungshinweise führen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie an, und ein Agent, der denselben Endpunkt immer wieder mit unterschiedlichen Parametern bombardiert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Lauf meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshaken geliefert; Fehler gehen jetzt durch den Fehlerkanal und erscheinen als Fehlerbenachrichtigung.

- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr Ausgaben, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, also werden nach einer unterbrochenen Verbindung nur die verpassten Events wiedergegeben, statt der ganzen Runde.

- **Gestreamte Events, die über Netzwerk-Chunks verteilt sind, gehen nicht mehr verloren.** Ein fortgesetzter Stream konnte ein Event verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Das Aufrufen eines anderen Agenten gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher begraben in einer rohen Ausführungsverfolgung an, was den Kontext des aufrufenden Agenten überflutete; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Lauf nicht mehr unbegrenzt stecken lassen.** Jeder Tool-Aufruf hat jetzt ein Wall-Clock-Zeitlimit (Standard 180s), also endet ein steckengebliebener Connector oder nicht reagierender Server den Schritt mit einer klaren Nachricht, statt die Runde zu verkeilen.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen verweigert, statt das ganze Iterations-Budget zu verbrennen.

- **Lange Gespräche verlieren ihren früheren Kontext nach Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Runden verdichtet, wurde beim nächsten Lauf stillschweigend gelöscht; sie wird jetzt weitergeleitet, also behält der Agent, was vorher besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontext-Fenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und spätere Ergebnisse erhalten ein Mindestfenster, statt völlig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen erscheinen.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Execution-Laufs ausgelagert wurde, vom Artifact-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Runtime-Bookkeeping-Dateien werden jetzt von der Artifact-Erkennung ausgeschlossen.

- **Jedes Tool in einem Gespräch funktioniert jetzt auf einem gemeinsamen Dateisystem.** Ausgelagerte große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann gesagt bekommen, dass sie nicht existiert, und sein Iterations-Budget beim Suchen verbrennen. Ausgelagerte Dateien leben jetzt in der Gesprächs-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Gesprächen bleiben lesbar.

- **Das Lesen einer großen ausgelagerten Datei erzeugt nicht mehr eine endlose Kette von Zeiger-Dateien.** Die Ausgabe eines Workspace-Lesevorgangs könnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte zeigt, die bereits auf der Festplatte waren, was den Agenten in Kreise schickt. Workspace-Lesevorgänge werden jetzt mit einer exakten Resume-Zeile begrenzt, statt erneut ausgelagert zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als einfachen `<｜｜DSML｜｜Tool call: ...` Text schrieb, statt den Function-Calling-Kanal zu nutzen, erreichte das rohe Markup das Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabe-Budget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend gelöscht und der Einleitungstext des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, also wurde ein Schritt, der nichts erzeugte, als abgeschlossen gemeldet. Die Abschneidung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.

- **Ein Ziel, das der Agent sicher verfehlt hat, wird jetzt erneut versucht, statt den Lauf zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den Neuplanung existiert. Läufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten erscheint nicht mehr als Antwort.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, also kamen Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" statt der Antwort selbst an. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` hat nur Sekundenpräzision, also zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgestellt wurden, kodierten identisch — Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte für immer erneuert werden. Jedes Refresh-Token trägt jetzt eine eindeutige `jti`, also unterscheidet sich das rotierte Token und das alte hört sofort auf zu funktionieren.

- **Approval Gates halten jetzt, wenn ein Agent durch einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat um Bestätigung bitten würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT`-Knoten eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen
