...
- **Ein Agent-Schritt in einem Workflow führt jetzt den vollständigen Agent aus, nicht nur seinen Prompt.** Ein Workflow-Agent-Knoten wurde früher nur mit den Anweisungen des Agents auf einem generischen Modell und dem Standard-Toolset ausgeführt — wodurch die Connectors, Knowledge Bases, MCP-Server und das konfigurierte Modell des Agents stillschweigend ignoriert wurden. Er wird jetzt genau wie im Chat ausgeführt, mit allen gebundenen Ressourcen und dem Modell, und nur wenn dieser Agent für den Workflow-Besitzer sichtbar ist.
- **Das Verlassen (oder Entfernen aus) einer Organisation widerruft jetzt deren gemeinsamen Zugriff.** Ressourcen, auf die Sie nur über diese Organisation zugreifen konnten, werden abgemeldet, und die Connector-/MCP-Anmeldedaten pro Benutzer, die Sie dafür gespeichert haben, werden gelöscht — sodass Sie diese Ressourcen nicht mehr verwenden oder auf die Anmeldedaten des Besitzers zurückgreifen können. Ressourcen, die Sie über den Marketplace abonniert haben, sind nicht betroffen.
- **Das Löschen eines Kontos entfernt jetzt konsistent alles.** Die Admin-Funktion „Benutzer löschen" und der Self-Service-Pfad „Mein Konto löschen" unterschieden sich früher — jeder bereinigt einen anderen Satz von Dateien auf der Festplatte (und beide berechneten ihre Dateipfade falsch, sodass jeder stillschweigend einige verpasste), und der Admin-Pfad hinterließ API-Schlüssel, Anmeldungsverlauf, Eval-Daten, Aufruflogs und Abonnements. Beide durchlaufen jetzt eine Routine, die jeden Datensatz und jede Datei entfernt, und das Löschen schlägt nicht mehr bei Datenbanken fehl, die Fremdschlüssel erzwingen.
- **Ein Benutzer, der immer noch eine Organisation besitzt, kann nicht mehr gelöscht werden.** Die Anfrage wird jetzt mit einer klaren Meldung abgelehnt, um zuerst den Besitz zu übertragen (oder die Organisation zu löschen), anstatt unerwartet fehlzuschlagen oder die Organisation verwaist zu hinterlassen.
- **Das Löschen eines Agents bricht jetzt frühere Konversationen nicht mehr.** Konversationen, die auf den Agent verwiesen, werden als historische Datensätze beibehalten und haben einfach den Agent-Link gelöscht, anstatt das Löschen zu blockieren.
- **Connector-Aktionen, die eine Bestätigung erfordern, können nicht mehr unbeaufsichtigt in einem Workflow ausgeführt werden.** Eine Connector-Aktion, die als Bestätigung erforderlich gekennzeichnet ist, wurde früher in einem Workflow mit dem Genehmigungstor stillschweigend umgangen ausgeführt. Der Schritt schlägt jetzt geschlossen fehl mit einer Meldung, einen Human Intervention-Knoten davor zu platzieren (das Workflow-native Genehmigungstor) oder eine Aktion zu verwenden, die keine Bestätigung erfordert.
- **Workflow-Schritte leihen sich nicht mehr die MCP-Anmeldedaten des Besitzers.** Wenn ein MCP-Server eines Workflows das Fallback für Besitzer-Anmeldedaten deaktiviert hatte, griff ein Nicht-Besitzer, der diesen Workflow ausführte, stillschweigend auf die Server-Anmeldedaten des Besitzers zurück, anstatt gestoppt zu werden. Der Schritt schlägt jetzt mit einer klaren Meldung fehl, Ihre eigenen Anmeldedaten zu konfigurieren, was dem Chat-Pfad entspricht.
- **Connector-Aufrufe aus Workflows werden jetzt im Audit-Log erfasst.** Ein Connector-Aktion, die in einem Workflow ausgeführt wurde, hinterließ keinen Eintrag im Connector-Aufrufslog (nur Chat-initiierte Aufrufe wurden aufgezeichnet); Workflow-Aufrufe schreiben jetzt denselben Audit-Trail.
- **Sie können jetzt einen abonnierten Connector, eine Knowledge Base oder einen MCP-Server an einen Agent binden.** Das Binden erforderte früher, dass Sie die Ressource *besaßen*, sodass ein Connector, den Sie legitim abonniert hatten (org-geteilt oder Marketplace-installiert), als „nicht im Besitz" abgelehnt wurde. Das Binden folgt jetzt derselben Sichtbarkeitsregel, die der Rest der App verwendet — alles, das Sie besitzen oder abonniert haben, ist bindbar.
- **Workflow-Schritte können nur Connectors aufrufen, auf die Sie tatsächlich Zugriff haben.** Ein Connector-Schritt eines Workflows konnte früher jeden Connector ausführen, dessen ID er referenzierte, solange dieser Connector existierte; er erzwingt jetzt die Sichtbarkeit des Ausführenden (im Besitz oder abonniert), sodass ein gemeinsamer Workflow keinen Connector erreichen kann, auf den Sie keinen Zugriff haben, nur durch Referenzierung seiner ID.
- **Die Knowledge-Base-Suche funktioniert jetzt gegen KBs, die Sie abonniert haben, nicht nur gegen solche, die Sie besitzen.** Die Daten einer KB befinden sich unter dem Speicher ihres Besitzers, aber ein Workflow-Knowledge-Base-Schritt (und Knowledge-Base-Chat ohne einen Agent) suchte früher unter dem eigenen Speicher des Aufrufers — sodass die Suche in einer mit Ihnen geteilten KB stillschweigend nichts zurückgab. Der Besitzer wird jetzt aus der KB selbst aufgelöst und die Suche wird zugriffsgeprüft (Besitzer oder Abonnent), sodass gemeinsame KBs Ergebnisse zurückgeben und KBs, die Sie nicht sehen können, nichts zurückgeben, anstatt eines verwirrenden leeren Treffers.
- **Hintergrund-Jobs verschwinden nicht mehr zufällig.** Dokumentaufnahme, Webhook-Zustellung, Eval-Läufe und Post-Response-Benachrichtigungen wurden als Fire-and-Forget-Aufgaben gestartet, auf die die Runtime nur schwach verwiesen, sodass sie unter Last während des Flugs Garbage-Collected werden konnten und stillschweigend stoppten. Sie werden jetzt bis zur Fertigstellung gehalten (und ihre Fehler protokolliert), sodass sie zuverlässig fertig werden.
- **Große Datei-Uploads blockieren nicht mehr andere Anfragen.** Die Textextraktion aus einem hochgeladenen PDF/DOCX lief synchron auf der Hauptereignisschleife und fror kurzzeitig alle gleichzeitigen Anfragen ein, während eine große Datei geparst wurde. Die Extraktion wird jetzt auf einen Worker-Thread ausgelagert.
- **Multi-Schritt-Antworten (DAG) stellen ihre Quellen nicht mehr falsch dar.** Wenn der Rechercheschritt eines Plans eine Seite abrief und zusammenfasste, floss nur diese Zusammenfassung weiter — sodass eine Quelle, die 11 Elemente auflistete, in der endgültigen Antwort als 6 auftauchen konnte, oder die Schweregrad eines Elements konnte falsch gekennzeichnet werden, und der Zielauswertungsschritt konnte es nicht erkennen, weil er die ursprüngliche Quelle nie sah. Jeder Schritt behält jetzt seine rohe Tool-Ausgabe als autoritative Quellenbeweise, die die endgültige Synthese und der Evaluator gegen die Antwort überprüfen, sodass Summen, Aufzählungen und Attribute dem entsprechen, was die Quellen tatsächlich sagten (und das Vertrauen sinkt mit einer benannten Diskrepanz, wenn sie nicht übereinstimmen).
- **Der Workflow-LLM-Knoten zeigt nicht mehr Modellsteuerelemente an, die nicht das bedeuteten, was sie implizierten.** Er hatte ein hardcodiertes Modell-Dropdown, dessen Auswahl zur Laufzeit ignoriert wurde, plus einen Fast/Main-Tier-Umschalter, der ein internes (automatisch herabgestuftes) Konzept offenlegte. Beide werden entfernt — der Knoten führt jetzt immer Ihr systemkonfiguriertes Modell aus (Model Provider, Fallback auf `.env`), auf die gleiche Weise wie der Playground es auflöst, mit einer einzeiligen Notiz, die darauf hinweist, wo das Modell konfiguriert ist.
