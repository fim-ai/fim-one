...
- **Langsame Tools können jetzt im Hintergrund ausgeführt werden, ohne den Agent zu blockieren.** Die Sandbox-Code-Ausführung (Python/Shell/Node) bietet eine `run_in_background`-Option: Der Agent erhält sofort eine Task-ID, arbeitet an anderen Schritten weiter und erhält das Ergebnis als Benachrichtigung, wenn das Tool fertig ist. Wenn der Agent versucht, zu finalisieren, während noch Tasks laufen, wartet er auf diese (begrenzt) und integriert ihre Ergebnisse vor der Antwort.
- **Konversationsverlauf wird vor der Komprimierung gespeichert, und durch Budget gekürzte Tool-Ergebnisse werden im Workspace gespeichert.** Bevor die Komprimierung langer Konversationen den Verlauf durch eine Zusammenfassung ersetzt, wird das vollständige Transkript als JSONL-Datei in den Konversations-Workspace geschrieben; ebenso wird ein durch das aggregierte Token-Budget gekürzte Tool-Ergebnis vollständig im Workspace gespeichert mit einem Zeiger an Ort und Stelle — in beiden Fällen können der Agent (und Sie) das Original später lesen, anstatt es zu verlieren.
- **Antworten enden nicht mehr mitten im Satz, wenn das Modell sein Ausgabelimit erreicht.** Wenn eine Antwort durch das Output-Token-Limit des Providers unterbrochen wird, fragt der Agent das Modell automatisch auf, wo es aufgehört hat, fortzufahren (bis zu 3 Runden) und fügt die Segmente zu einer nahtlosen Antwort zusammen — sowohl in der Reasoning-Schleife als auch in der gestreamten finalen Antwort.
- **LLM-Aufrufe werden jetzt bei Provider-Überlastung (HTTP 529) und Gateway-Timeout (504) erneut versucht und beachten `Retry-After`.** Zuvor führte ein 529 von Anthropic sofort zum Fehler der Anfrage, es sei denn, ein Fallback-Modell war konfiguriert; jetzt wird es mit Backoff erneut versucht wie 429/5xx, und es wird genau so lange gewartet, wie der Provider es verlangt, wenn ein `Retry-After`-Header vorhanden ist.
- **Agents behalten jetzt einen sichtbaren Plan während mehrstufiger Aufgaben.** ReAct-Agents erhalten ein `update_plan`-Tool, um ihren Plan als Todo-Checkliste zu notieren und ihn während des Fortschritts zu aktualisieren; wenn der Plan über mehrere Tool-Aufrufe hinweg veraltet wird, wird die vollständige Checkliste als Erinnerung erneut eingefügt — so bleiben lange Aufgaben auch nach Kontext-Komprimierung auf Kurs. Standardmäßig aktiviert (`REACT_PLAN_TOOL_ENABLED`); DAG-Unterschritte und Tool-lose Agents überspringen dies.

- **Agents können jetzt einen Workflow während einer Konversation ausführen.** Aktive, genehmigungsfreie Workflows, die Sie besitzen oder abonniert haben, werden dem Agent als `run_workflow`-Tool angeboten — im normalen Chat und mit ausgewähltem Agent gleichermaßen — so dass eine Aufgabe einen deterministischen, wiederholbaren Unterprozess an einen Workflow übergeben und seine Ausgaben zurückerhalten kann. Jede solche Ausführung wird im Workflow-Verlauf aufgezeichnet und ihre Token-Nutzung wird gegen das Kontingent des Workflow-Besitzers angerechnet (die Ausführung wird abgelehnt, wenn der Besitzer bereits über dem Kontingent liegt). Workflows mit einem Human Intervention-Schritt bleiben nur auslösbar (von der Workflows-Seite oder ihrem API-Schlüssel), da sie lange blockieren können.
- **Zwei neue Workflow-Starter-Templates: Connector Action und MCP Tool Call.** Die Galerie "Create from Template" zeigt jetzt die Connector- und MCP-Knoten — rufen Sie ein externes System oder ein MCP-Tool auf, fassen Sie dann das Ergebnis mit einem LLM zusammen. Wählen Sie den spezifischen Connector/Server nach der Erstellung aus.
