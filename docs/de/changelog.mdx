...
- **Konnektoren und MCP-Server teilen die Anmeldedaten des Besitzers standardmäßig nicht mehr (Breaking Change).** "Fallback auf die Anmeldedaten des Besitzers zulassen" ist jetzt opt-in: neue Konnektoren/MCP-Server werden mit dieser Einstellung deaktiviert erstellt, und alle vorhandenen wurden ebenfalls deaktiviert. Ein Abonnent, der sich auf das Token des Besitzers verlassen hat, um einen gemeinsamen Konnektor oder MCP-Server zu nutzen, muss jetzt seine eigenen Anmeldedaten hinzufügen oder der Besitzer muss das Fallback für diese Ressource erneut aktivieren. Besitzer können immer ihre eigenen Anmeldedaten verwenden, unabhängig von der Einstellung. Dadurch ist ein Konnektor, dessen Fallback deaktiviert ist und für den Sie keine Anmeldedaten haben, jetzt in der Toolbox des Agenten ausgeblendet (wie MCP-Server bereits verhalten), anstatt angeboten zu werden und dann beim Aufrufen fehlzuschlagen.
- **Neue Agenten verwenden standardmäßig ReAct-Ausführung statt Auto.** Der Auto-Modus führte pro Anfrage einen zusätzlichen Fast-Model-Klassifizierungsaufruf durch, um zwischen ReAct und DAG zu entscheiden; neue Agenten (und die integrierten/Marketplace-Vorlagen) verwenden jetzt standardmäßig ReAct, wodurch diese Pro-Nachricht-Latenz und Kosten entfallen. Auto und DAG bleiben pro Agent wählbar; vorhandene Agenten behalten ihren aktuellen Modus.
- **Die Erstellung von KI-Agenten erholt sich von einem schwachen Fast Model.** Das Erstellen oder Verfeinern eines Agenten aus einer natürlichsprachigen Anweisung läuft auf dem Fast Model; wenn es keine gültige Konfiguration erzeugen kann, wird die Anfrage jetzt einmal auf dem stärkeren allgemeinen Modell wiederholt, bevor ein Fehler auftritt, sodass ein zu schwaches Fast Model nicht mehr als Fehler für den Benutzer sichtbar wird.
