...
- **DAG plant nicht länger abgeschlossene Arbeiten neu, und unterbrochene Ausführungen werden dort fortgesetzt, wo sie unterbrochen wurden.** Wenn ein Plan überarbeitet werden muss, werden Schritte, die bereits abgeschlossen sind, mit ihren Ergebnissen in den neuen Plan übernommen — nur die verbleibende Arbeit wird neu geplant und ausgeführt. Der Fortschritt der Schritte wird auch während der Ausführung gespeichert, sodass bei einer unterbrochenen Ausführung (Absturz, Bereitstellung, unterbrochene Verbindung) das erneute Ausführen derselben Anfrage bei den abgeschlossenen Schritten fortgesetzt wird, anstatt von vorne zu beginnen.
- **Langsame Tools können jetzt im Hintergrund ausgeführt werden, ohne den Agent zu blockieren.** Die Sandbox-Codeausführung (Python/Shell/Node) bietet eine `run_in_background`-Option: Der Agent erhält sofort eine Task-ID, arbeitet an anderen Schritten weiter und erhält das Ergebnis als Benachrichtigung, wenn das Tool fertig ist. Wenn der Agent versucht, die Ausführung abzuschließen, während noch Tasks laufen, wartet er auf diese (begrenzt) und berücksichtigt ihre Ergebnisse, bevor er antwortet.
- **Konversationsverlauf wird vor der Komprimierung gespeichert, und durch das Budget gekürzte Tool-Ergebnisse werden im Workspace gespeichert.** Bevor die Komprimierung langer Konversationen den Verlauf durch eine Zusammenfassung ersetzt, wird das vollständige Transkript als JSONL-Datei in den Konversations-Workspace geschrieben; ebenso wird ein Tool-Ergebnis, das durch das aggregierte Token-Budget gekürzt wurde, vollständig im Workspace gespeichert, mit einem Zeiger an seiner Stelle — in beiden Fällen kann der Agent (und Sie) das Original später lesen, anstatt es zu verlieren.
- **Antworten enden nicht länger mitten im Satz, wenn das Modell sein Ausgabelimit erreicht.** Wenn eine Antwort durch das Output-Token-Limit des Anbieters unterbrochen wird, fordert der Agent das Modell automatisch auf, dort fortzufahren, wo es stoppte (bis zu 3 Runden), und verbindet die Segmente zu einer nahtlosen Antwort — sowohl in der Reasoning-Schleife als auch in der gestreamten endgültigen Antwort.
- **LLM-Aufrufe werden jetzt bei Anbieterüberlastung (HTTP 529) und Gateway-Timeout (504) wiederholt, und beachten `Retry-After`.** Zuvor führte eine 529 von Anthropic zum sofortigen Fehlschlag der Anfrage, es sei denn, ein Fallback-Modell war konfiguriert; jetzt wird es mit Backoff und Wiederholung wie bei 429/5xx versucht, und es wird genau so lange gewartet, wie der Anbieter anfordert, wenn ein `Retry-After`-Header vorhanden ist.
- **Agents behalten jetzt einen sichtbaren Plan während mehrstufiger Aufgaben bei.** ReAct-Agents erhalten ein `update_plan`-Tool, um ihren Plan als Todo-Checkliste aufzuschreiben und ihn während des Fortschritts zu aktualisieren; wenn der Plan nach mehreren Tool-Aufrufen veraltet wird, wird die vollständige Checkliste als Erinnerung erneut eingefügt — sodass lange Aufgaben auch nach Kontext-Komprimierung zielgerichtet bleiben. Standardmäßig aktiviert (`REACT_PLAN_TOOL_ENABLED`); DAG-Unterschritte und Tool-lose Agents überspringen dies.

- **Agents können jetzt einen Workflow während einer Konversation ausführen.** Aktive, genehmigungsfreie Workflows, die Sie besitzen oder abonniert haben, werden dem Agent als `run_workflow`-Tool angeboten — sowohl im normalen Chat als auch mit ausgewähltem Agent — sodass eine Aufgabe einen deterministischen, wiederholbaren Unterprozess an einen Workflow übergeben und dessen Ausgaben zurückerhalten kann. Jede solche Ausführung wird im Ausführungsverlauf des Workflows aufgezeichnet, und die Token-Nutzung wird auf das Kontingent des Workflow-Besitzers angerechnet (die Ausführung wird abgelehnt, wenn der Besitzer bereits über dem Kontingent liegt). Workflows mit einem Human Intervention-Schritt bleiben nur auslösbar (von der Workflows-Seite oder ihrem API-Schlüssel), da sie lange blockieren können.
- **Zwei neue Workflow-Starter-Vorlagen: Connector Action und MCP Tool Call.** Der Katalog „Aus Vorlage erstellen" präsentiert jetzt die Connector- und MCP-Knoten — rufen Sie ein externes System oder ein MCP-Tool auf, fassen Sie dann das Ergebnis mit einem LLM zusammen. Wählen Sie den spezifischen Connector/Server nach der Erstellung aus.
