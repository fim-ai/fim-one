...
- **Ein durch das Ausgabelimit des Modells unterbrochener Toolaufruf wird nicht mehr als abgeschlossene Arbeit akzeptiert.** Wenn eine große Payload (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabebudget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der einleitende Text des Agenten — „jetzt speichere ich die Datei" — wurde zum Schrittergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird nun erkannt, und dem Agenten wird mitgeteilt, die Arbeit in kleinere Aufrufe aufzuteilen und erneut zu versuchen.

- **Ein Ziel, das der Agent sicher verfehlt hat, wird nun erneut versucht, anstatt den Durchlauf zu beenden.** Die Neuplanung stoppte, wenn der Analyzer sicher war, was ein sicheres „die Lieferung fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Durchläufe stoppen nun nur früh, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Verifizierung durch, bevor er antwortet; diese Verifizierung wurde wörtlich an Sie gestreamt, sodass Antworten als „Lassen Sie mich überprüfen: 1. Vollständig beantwortet? ✅ …" ankamen, anstatt der Antwort selbst. Die Prüfung ist nun intern, und die Antwort, die Sie sehen, wird als eigener Schritt formuliert.

- **Die Aktualisierungstokenrotation invalidiert nun tatsächlich den vorherigen Token.** JWT `exp` hat nur Sekundengenauigkeit, daher verwendeten zwei Aktualisierungstoken, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, identische Kodierungen — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Aktualisierungstoken konnte auf unbestimmte Zeit erneuert werden. Jeder Aktualisierungstoken trägt nun eine eindeutige `jti`, sodass sich der rotierte Token unterscheidet und der alte sofort nicht mehr funktioniert.

- **Genehmigungsgates halten nun, wenn ein Agent über einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in einem Chat um Bestätigung fragen würde, lief zuvor ungeprüft ab, sobald er über `call_agent` delegiert oder in einen Workflow-`AGENT`-Knoten eingebunden wurde — diese Pfade starteten den Agenten ohne Hooks. Beide führen nun die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Zuvor führte jeder andere Wert als `ssl` oder `tls` (z. B. `SMTP_SSL=true`) zu einer unverschlüsselten Verbindung ohne Warnung, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden nun kategorisch abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
