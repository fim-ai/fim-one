...
- **Nachfolge-Chips ziehen das Transkript nicht mehr über die Antwort hinaus.** Nachdem eine Runde beim Ergebnis geparkt wird, bleibt das spätere Layout-Wachstum (Vorschläge, Titel) an Ort und Stelle, und das Einklappen der Live-Runde in den Verlauf parkt wieder am Anfang der Antwort für dasselbe Gespräch, anstatt zum Ende zu springen.

- **Das Öffnen eines bestehenden Gesprächs landet bei den neuesten Nachrichten.** Das Aktualisieren einer Chat-URL scrollt jetzt nach unten wie beim Wechsel aus der Seitenleiste, anstatt oben im Thread zu beginnen.

- **Schnelle Antworten sehen nicht mehr fest auf „Wird verarbeitet…" aus.** Eine abgeschlossene Runde ohne generierten Schritt-Titel zeigt das Gedankenprozess-Label anstelle des Live-Verarbeitungs-Shimmer-Textes.

- **Animation folgt jetzt der Systemeinstellung „Bewegung reduzieren".** Der driftende Gradient des Login-Hintergrunds, der Hero-Eingang, der atmende eingespritzte Nachrichtentext und die Streaming-Einfügemarke bewegten sich weiterhin, unabhängig von der Betriebssystem-Einstellung. Sie bleiben jetzt still, während die Indikatoren, die laufende Arbeiten anzeigen, sichtbar bleiben, damit die Benutzeroberfläche nie stecken bleibt.

- **Der Ausführungsmodus-Selektor bleibt geschlossen, während eine Runde läuft.** Der Chip war ausgegraut, öffnete sein Menü aber trotzdem beim Klick, anders als der Agenten-Selektor daneben; beide sind jetzt inaktiv, bis die Ausführung beendet ist.

- **Reasoning-Streams funktionieren wieder bei neueren Modellen.** Live-Reasoning war über eine hartcodierte Liste von Modellnamen verdrahtet, sodass alles, was nach dieser Liste veröffentlicht wurde, einen Spinner zeigte und dann den gesamten Reasoning-Block auf einmal ausgab. Jedes Modell, das Reasoning ausgibt, streamt es jetzt.

- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie stellt sich nicht mehr während des Reasoning hin und meldet fälschlicherweise neuen Inhalt darunter, sie folgt Inhalten, die ohne neue Ereignisse wachsen, und wenn eine Runde endet, bleibt sie oben am Ergebnis stehen, anstatt unten. Wenn Sie wegscollen, wird die Kontrolle zurückgegeben, und wenn Sie zum Ende zurückkehren, wird das Folgen fortgesetzt.

- **Eine kurze Antwort bleibt dort, wo Sie sie nach dem Ende der Runde gelesen haben.** Nachdem eine Runde beendet war, rutschte das Transkript nach unten, sodass die Frage und ihre Antwort die Eingabebox umarmten; die Antwort behält jetzt ihre Position oben in der Ansicht, bis die nächste Nachricht gesendet wird.

- **Die Anmeldung wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landingpage wird neu geladen, wenn sich die Sprache des Kontos von der unterscheidet, in der die Anmeldeseite gerendert wurde, sodass das Ziel nicht mehr in der vorherigen Sprache angezeigt wird, bis zur nächsten Neuladung.

- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener blieb aktiv, aber sein Dialog wurde im eingeklappten Zustand nie gemountet, sodass der Shortcut stillschweigend nichts tat.

- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als „Knowledge Base" gekennzeichnet, weil das Abruftool den Ursprungsdateinamen fallen ließ; es meldet jetzt die Quelle, was der Grounding-Pipeline bereits tat.

- **Lange Gespräche treffen weniger wahrscheinlich auf harte Kontext-Overflow-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unter dem Eingabelimit des Modells ausgelöst, anstatt genau darauf, und beim Start wird eine Warnung protokolliert, wenn das Fenster des schnellen Modells zu klein ist, um das Kontext-Budget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im dunklen Modus lesbar.** Von Modellen erstellte SVG geht von einer hellen Leinwand aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, anstatt dunkel-auf-dunkel zu werden.

- **Der Browser-Tab-Beschäftigungsindikator verschwindet nicht mehr mitten in einer Runde.** Während eine Runde läuft, behält das Tab-Symbol die App-Markierung und pulsiert sie zwischen voller Farbe und Grau, hält für die gesamte Runde und stellt das echte Symbol zuverlässig wieder her, wenn die Runde endet, auch nach In-App-Navigation. Mit reduzierter Bewegung hält es eine statische graue Markierung anstatt zu animieren.

- **Agenten rufen einen Endpunkt nicht mehr Dutzende Male nach einem abgeschnittenen Ergebnis ab.** Abschneidungsmitteilungen beginnen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie, und ein Agent, der denselben Endpunkt immer wieder mit unterschiedlichen Parametern hämmert, wird gewarnt und dann hart gestoppt.

- **Eine abgestürzte Ausführung meldet jetzt einen Fehler anstelle eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgs-Häkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und erscheinen als Fehlerbenachrichtigung.

- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr Ausgaben, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die verpassten Ereignisse wiedergegeben werden, anstatt der gesamten Runde.

- **Gestreamte Ereignisse, die über Netzwerk-Chunks aufgeteilt sind, gehen nicht mehr verloren.** Ein fortgesetzter Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Der Aufruf eines anderen Agenten gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher begraben in einer rohen Ausführungsverfolgung an und überflutete den Kontext des aufrufenden Agenten; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann eine Ausführung nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Echtzeit-Limit (Standard 180s), sodass ein blockierter Connector oder nicht reagierender Server den Schritt mit einer klaren Nachricht beendet, anstatt die Runde zu verkeilen.

- **Ein Agent, der denselben Tool-Aufruf wiederholt, wird jetzt früh gestoppt.** Identische wiederholte Aufrufe werden nach ein paar Versuchen verweigert, anstatt das gesamte Iterations-Budget zu verbrauchen.

- **Lange Gespräche verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Runden verdichtet, wurde beim nächsten Durchlauf stillschweigend gelöscht; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontext-Fenster zu passen, werden jetzt beide Enden beibehalten (das Ende der Ausgabe eines Befehls ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und spätere Ergebnisse erhalten ein garantiertes Mindestfenster, anstatt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Lieferungen erscheinen.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während einer Code-Ausführung offloaded wurde, vom Artefakt-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Laufzeit-Bookkeeping-Dateien sind jetzt von der Artefakt-Erkennung ausgeschlossen.

- **Jedes Tool in einem Gespräch funktioniert jetzt auf einem gemeinsamen Dateisystem.** Offgeladene große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-lesenden Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann wird ihm gesagt, dass sie nicht existiert, und verbraucht sein Iterations-Budget, um sie zu suchen. Offgeladene Dateien befinden sich jetzt in der Gesprächs-Sandbox, die jedes Tool (einschließlich Exec-Container) mountet; Dateien aus älteren Gesprächen bleiben lesbar.

- **Das Lesen einer großen offgeladenen Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte zeigt, die bereits auf der Festplatte waren, was den Agenten in Kreise schickte. Workspace-Lesevorgänge werden jetzt mit einer exakten Resume-Zeile begrenzt, anstatt erneut offgeloaded zu werden.
...
- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells unterbrochen wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabe-Budget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend gelöscht und der Einleitungstext des Agenten — „jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und dem Agenten wird mitgeteilt, die Arbeit in kleinere Aufrufe aufzuteilen.

- **Ein Ziel, das der Agent sicher verpasst hat, wird jetzt erneut versucht, anstatt die Ausführung zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „das Lieferbare fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Ausführungen stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten erscheint nicht mehr als Antwort.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, sodass Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" ankamen, anstelle der Antwort selbst. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich den vorherigen Token.** JWT `exp` ist nur Sekunden-Genauigkeit, sodass zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgestellt wurden, früher identisch kodiert wurden — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlener Refresh-Token konnte für immer erneuert werden. Jeder Refresh-Token trägt jetzt eine eindeutige `jti`, sodass sich der rotierte Token unterscheidet und der alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt, wenn ein Agent über einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensibler Connector-Aufruf, der in Chat um Bestätigung fragen würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT` Node eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend unverschlüsselt.** Früher fiel jeder Wert außer `ssl` oder `tls` (z. B. `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung durch, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt direkt abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
