...
- **Eine neu gesendete Nachricht wird oben auf dem Bildschirm angezeigt.** Das Transkript behält gerade genug Platz unter dem aktuellen Turn, damit seine erste Zeile oben ankommt, sodass die Frage sichtbar bleibt und die Antwort in den Platz darunter wächst, anstatt dass der gesamte Turn an der unteren Kante klebt. Der Platz verschwindet, sobald der Turn größer als ein Bildschirm wird.

- **Reasoning-Blöcke bleiben während eines Turns eingeklappt.** Reasoning in ReAct- und DAG-Ausgaben zeigt jetzt standardmäßig eine einzeilige Vorschau an, anstatt sich beim Streaming zu erweitern, wodurch das Ergebnis sichtbar bleibt. Klicken Sie auf einen beliebigen Block, um die vollständige Trace zu lesen.

- **Exportierte PDF- und Word-Dateien sind korrekt für Chinesisch formatiert.** Der PDF-Export bettet jetzt eine echte CJK-Schriftart ein, anstatt eine zu referenzieren, was den zusammengefallenen Abstand in englischem Text und Aufzählungszeichen behebt, die als nicht verwandte chinesische Zeichen dargestellt wurden, und Fettdruck funktioniert jetzt. Word-Exporte deklarieren eine ostasiatische Schriftart, sodass gemischter chinesisch/englischer Fetttext nicht mehr halb-fett aussieht, und beide Formate verwenden eine fünfstufige Größenskala, die Überschriften innerhalb einer Antwort der Struktur des Transkripts unterordnet.
...
- **Gestreamte Antworten werden sanft eingeblendet, anstatt zu erscheinen.** Neu angekommene Blöcke werden beim Streaming kurz eingeblendet und respektieren die Voreinstellung für reduzierte Bewegung. Lange Tabellen heben auch die Zeile unter dem Cursor hervor, und Aufzählungszeichen sitzen hinter dem Text, anstatt damit zu konkurrieren.

- **Block-Aktionen bleiben aus dem Weg, bis Sie danach greifen.** Kopier- und Exportschaltflächen auf Antworten, Code-Blöcken, Diagrammen und Tabellen werden beim Hover angezeigt und haben eine konsistente Behandlung. Sie bleiben über die Tastatur erreichbar.

- **Die Seitenleiste behält den Chat zusammen.** Die Gesprächsliste sitzt direkt unter Neuer Chat und Suche, und die Modulnavigation wurde in ein kompaktes getöntes Dock über dem Benutzerbereich verschoben, was der Liste mehr Platz gibt und der Seitenleiste eine klare Hierarchie von oben nach unten.

- **Reasoning ist standardmäßig eingeklappt.** Thinking-Abschnitte in Live-Läufen und früheren Gesprächen werden zu einer einzeiligen Vorschau eingeklappt und werden beim Klick erweitert. Reasoning wird beim Generieren weiterhin offen gestreamt und wird dann eingeklappt, wenn der Schritt abgeschlossen ist.
...
- **Der Ausführungsmodus-Selector bleibt geschlossen, während ein Turn läuft.** Der Chip war ausgegraut, öffnete sein Menü aber trotzdem beim Klick, anders als der daneben liegende Agent-Selector; beide sind jetzt inert, bis der Run beendet ist.
...
- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie stellt sich nicht mehr mitten im Reasoning fest und meldet fälschlicherweise neue Inhalte darunter, folgt Inhalten, die ohne neue Events wachsen, und wenn ein Turn beendet ist, bleibt sie oben im Ergebnis statt unten. Wenn man wegscrollt, wird die Kontrolle zurückgegeben, und wenn man zur Unterseite zurückkehrt, wird das Folgen fortgesetzt.

- **Das Anmelden wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landing Page wird neu geladen, wenn sich die Sprache des Kontos von der unterscheidet, in der die Anmeldeseite gerendert wurde, sodass das Ziel nicht mehr in der vorherigen Sprache angezeigt wird, bis zur nächsten Aktualisierung.

- **Cmd+K öffnet die Suche auch wenn die Seitenleiste eingeklappt ist.** Der Shortcut-Listener blieb aktiv, aber sein Dialog wurde im eingeklappten Zustand nie eingebunden, sodass der Shortcut stillschweigend nichts tat.

- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als "Knowledge Base" bezeichnet, weil das Abruftool den Ursprungsdateinamen verwarf; es meldet jetzt die Quelle, was der Grounding-Pipeline entspricht.

- **Lange Unterhaltungen treffen weniger wahrscheinlich auf harte Context-Overflow-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unterhalb des Eingabelimits des Modells ausgelöst, anstatt genau darauf, und beim Start wird eine Warnung protokolliert, wenn das Fenster des schnellen Modells zu klein ist, um das Context-Budget des allgemeinen Modells zu halten.

- **SVG-Figuren bleiben im Dark Mode lesbar.** Von Modellen erstellte SVG geht von einer hellen Canvas aus, sodass Figuren auf einer festen hellen Oberfläche gezeichnet werden, anstatt sich dunkel-auf-dunkel zu verwandeln.

- **Der Browser-Tab-Busy-Indicator verschwindet nicht mehr mitten im Turn.** Während ein Turn läuft, behält das Tab-Icon die App-Markierung und pulsiert sie zwischen voller Farbe und Grau, hält für den gesamten Run und stellt das echte Icon zuverlässig wieder her, wenn der Turn endet, auch nach In-App-Navigation. Mit reduzierter Bewegung wird eine statische graue Markierung gehalten, anstatt zu animieren.

- **Agenten rufen einen Endpoint nicht mehr dutzende Male nach einem abgekürzten Ergebnis ab.** Abkürzungshinweise werden jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie eingeleitet, und ein Agent, der denselben Endpoint immer wieder mit unterschiedlichen Parametern bombardiert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Run meldet jetzt einen Fehler statt eines grünen "done".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgs-Häkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und werden als Fehlerbenachrichtigung angezeigt.

- **Das Wiederverbinden mit einem Stream wiederholt nicht mehr die Ausgabe, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpoint, sodass nach einer unterbrochenen Verbindung nur die verpassten Events wiedergegeben werden, anstatt des gesamten Turns.

- **Gestreamte Events, die über Netzwerk-Chunks verteilt sind, gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Event verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade teilen sich jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Das Aufrufen eines anderen Agenten gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher begraben in einer rohen Ausführungsverfolgung an und überflutete den Context des aufrufenden Agenten; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Run nicht mehr auf unbestimmte Zeit blockieren.** Jeder Tool-Aufruf hat jetzt ein Wall-Clock-Zeitlimit (Standard 180s), sodass ein steckengebliebener Connector oder ein nicht reagierender Server den Schritt mit einer klaren Meldung beendet, anstatt den Turn zu blockieren.
...
- **Lange Unterhaltungen verlieren ihren früheren Context nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Turns verdichtet, wurde beim nächsten Run stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was vorher besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Context-Fenster zu passen, werden jetzt beide Enden beibehalten (der Abschluss der Ausgabe eines Befehls ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinischen Text, und später Ergebnisse erhalten ein Mindestfenster, anstatt ganz gelöscht zu werden.

- **Dieselbe generierte Datei wird nicht mehr als zwei Downloads angezeigt.** Eine gerenderte HTML-Ausgabe, die erneut unter einem echten Dateinamen gespeichert wurde, erzeugte früher zwei Deliverable-Karten mit identischem Inhalt; Duplikate werden jetzt nach Inhalt erkannt und in die speziell benannte Datei eingeklappt.

- **Interne Offload-Dateien können nicht mehr als herunterladbare Deliverables angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Execution-Runs offgeladen wurde, vom Artifact-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Runtime-Bookkeeping-Dateien sind jetzt von der Artifact-Erkennung ausgeschlossen.

- **Jedes Tool in einer Unterhaltung funktioniert jetzt auf einem gemeinsamen Dateisystem.** Offgeladene große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Reading-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann gesagt bekommen, dass sie nicht existiert, und sein Iterations-Budget aufbrauchen, um sie zu suchen. Offgeladene Dateien befinden sich jetzt in der Conversation-Sandbox, die jedes Tool (einschließlich Exec-Container) einbindet; Dateien aus älteren Unterhaltungen bleiben lesbar.

- **Das Lesen einer großen offgeladenen Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs konnte selbst das Offload-Schwellenwert überschreiten und als neue Datei gespeichert werden, die auf Inhalte zeigt, die bereits auf der Festplatte waren, was den Agenten in Kreise schickte. Workspace-Lesevorgänge werden jetzt mit einer exakten Resume-Zeile begrenzt, anstatt erneut offgeladen zu werden.
...
- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Payload (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabe-Budget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der Einleitungstext des Agenten — "jetzt speichere ich die Datei" — wurde zum Schritt-Ergebnis, sodass ein Schritt, der nichts produzierte, als abgeschlossen gemeldet wurde. Die Kürzung wird jetzt erkannt, und der Agent wird aufgefordert, es erneut zu versuchen, wobei die Arbeit in kleinere Aufrufe aufgeteilt wird.

- **Ein Ziel, das der Agent sicher verfehlt hat, wird jetzt erneut versucht, anstatt den Run zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres "das Deliverable fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Runs stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.
...
- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` hat nur Sekunden-Präzision, sodass zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, früher identisch codiert wurden — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte auf ewig erneuert werden. Jedes Refresh-Token trägt jetzt eine eindeutige `jti`, sodass sich das rotierte Token unterscheidet und das alte sofort nicht mehr funktioniert.

- **Approval Gates halten jetzt, wenn ein Agent über einen anderen Agenten oder einen Workflow erreicht wird.** Ein empfindlicher Connector-Aufruf, der in Chat um Bestätigung bitten würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT` Node eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Früher fiel jeder andere Wert als `ssl` oder `tls` (zum Beispiel `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung durch, sowohl bei System-E-Mails als auch bei E-Mail-Benachrichtigungen. Nicht erkannte Werte werden jetzt kategorisch abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
