...
- **Agenten können jetzt einen Workflow während eines Gesprächs ausführen.** Aktive, genehmigungsfreie Workflows, die Sie besitzen oder abonniert haben, werden dem Agenten als `run_workflow`-Tool angeboten — sowohl in normalen Chats als auch mit ausgewähltem Agenten — damit eine Aufgabe einen deterministischen, wiederholbaren Unterprozess an einen Workflow übergeben und dessen Ausgaben zurückerhalten kann. Jede solche Ausführung wird im Ausführungsverlauf des Workflows aufgezeichnet und ihre Token-Nutzung wird gegen das Kontingent des Workflow-Eigentümers angerechnet (die Ausführung wird abgelehnt, wenn der Eigentümer bereits über dem Kontingent liegt). Workflows mit einem Human Intervention-Schritt bleiben nur auslösbar (von der Workflows-Seite oder ihrem API-Schlüssel), da sie lange blockieren können.
