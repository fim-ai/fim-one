...
- **Agents rufen einen Endpunkt nach einem abgekürzten Ergebnis nicht mehr dutzende Male ab.** Abkürzungsmitteilungen beginnen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie, und ein Agent, der denselben Endpunkt wiederholt mit unterschiedlichen Parametern aufruft, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Lauf meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshäkchen geliefert; Fehler durchlaufen jetzt den Fehlerkanal und werden als Fehlerbenachrichtigung angezeigt.

- **Das Wiederverbinden mit einem Stream gibt keine bereits angesehene Ausgabe mehr wieder.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpunkt, sodass nach einer unterbrochenen Verbindung nur die fehlenden Ereignisse statt des gesamten Durchlaufs wiedergegeben werden.

- **Über Netzwerk-Chunks verteilte Stream-Ereignisse gehen nicht mehr verloren.** Ein wiederaufgenommener Stream konnte ein Ereignis verlieren, dessen Frame in zwei Teilen ankam; beide Stream-Pfade nutzen jetzt einen Parser, der beliebige Chunk-Grenzen verarbeitet.

- **Das Aufrufen eines anderen Agents gibt jetzt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agents kam früher in einer rohen Ausführungsverfolgung begraben an und überflutete den Kontext des aufrufenden Agents; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Lauf nicht mehr unbegrenzt blockieren.** Jeder Tool-Aufruf hat jetzt ein Echtzeit-Limit (Standard 180s), sodass ein blockierter Connector oder ein nicht reagierender Server den Schritt mit einer klaren Meldung beendet, statt den Durchlauf zu blockieren.
...
- **Lange Konversationen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Durchläufe verdichtet, wurde beim nächsten Lauf stillschweigend verworfen; sie wird jetzt weitergeleitet, sodass der Agent behält, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (das Ende einer Befehlsausgabe ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und später Ergebnisse erhalten ein garantiertes Minimalfenster statt vollständig gelöscht zu werden.
