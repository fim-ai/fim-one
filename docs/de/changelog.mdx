...
- **Reasoning-Streams funktionieren wieder auf neueren Modellen.** Live Reasoning war an eine hartcodierte Liste von Modellnamen gebunden, daher zeigte alles, was nach dieser Liste veröffentlicht wurde, einen Spinner an und gab dann den gesamten Reasoning-Block auf einmal aus. Jedes Modell, das Reasoning ausgibt, streamt es jetzt.

- **Die Chat-Ansicht hält mit schneller Ausgabe Schritt und landet bei der Antwort.** Sie stellt sich nicht mehr mitten im Reasoning auf und meldet fälschlicherweise neuen Inhalt darunter, folgt Inhalten, die ohne neue Events wachsen, und wenn ein Turn endet, bleibt sie oben im Ergebnis statt unten. Das Wegscollen gibt die Kontrolle noch immer ab, und die Rückkehr zum unteren Ende setzt das Folgen fort.

- **Das Anmelden wendet die bevorzugte Sprache des Kontos sofort an.** Das Locale-Cookie wird jetzt während der Anmeldung und des OAuth-Callbacks synchronisiert, und die Landing Page wird neu geladen, wenn sich die Sprache des Kontos von der unterscheidet, in der die Anmeldeseite gerendert wurde, sodass das Ziel nicht mehr in der vorherigen Sprache angezeigt wird, bis zum nächsten Neuladen.
...
- **Zitate nennen wieder das Dokument, aus dem sie stammen.** Im einfachen Abrufmodus wurde jede Quelle als „Knowledge Base" gekennzeichnet, weil das Abruftool den Ursprungsdateinamen verwarf; es meldet jetzt die Quelle, was dem entspricht, was die Grounding-Pipeline bereits tat.

- **Lange Konversationen treffen weniger wahrscheinlich auf harte Context-Overflow-Fehler.** Die Komprimierung wird jetzt mit einem Sicherheitsabstand unterhalb des Eingabelimits des Modells ausgelöst, anstatt genau darauf, und beim Start wird eine Warnung protokolliert, wenn das Fenster des schnellen Modells zu klein ist, um das Context-Budget des allgemeinen Modells zu halten.

- **SVG-Abbildungen bleiben im Dark Mode lesbar.** Von Modellen erstellte SVGs gehen von einer hellen Canvas aus, daher werden Abbildungen auf einer festen hellen Oberfläche gezeichnet, anstatt dunkel-auf-dunkel zu werden.

- **Der Browser-Tab-Busy-Indikator verschwindet nicht mehr mitten im Turn.** Das Favicon zeigt jetzt ein stetiges Busy-Glyphe für den gesamten Lauf und stellt das echte Symbol zuverlässig wieder her, wenn der Turn endet, auch nach In-App-Navigation.

- **Agenten rufen einen Endpoint nicht mehr Dutzende Male nach einem abgekürzten Ergebnis ab.** Abkürzungsmitteilungen beginnen jetzt mit einem imperativen Zeiger auf die gespeicherte vollständige Kopie, und ein Agent, der denselben Endpoint immer wieder mit unterschiedlichen Parametern bombardiert, wird gewarnt und dann hart gestoppt.

- **Ein abgestürzter Lauf meldet jetzt einen Fehler statt eines grünen „Fertig".** Wenn der Agent wegen eines internen Fehlers stoppte, wurde der rohe Ausnahmtext als Antwort unter einem Erfolgshäkchen geliefert; Fehler gehen jetzt durch den Fehlerkanal und erscheinen als Fehlerbenachrichtigung.

- **Die Wiederverbindung mit einem Stream wiederholt nicht mehr die Ausgabe, die Sie bereits gesehen haben.** Der Live-Stream trägt jetzt denselben Positionscursor wie der Resume-Endpoint, daher werden nach einer unterbrochenen Verbindung nur die fehlenden Events statt des gesamten Turns wiedergegeben.
...
- **Das Aufrufen eines anderen Agenten gibt seine tatsächliche Antwort zurück.** Die Antwort des Sub-Agenten kam früher begraben in einer rohen Ausführungsverfolgung an und überflutete den Kontext des aufrufenden Agenten; er erhält jetzt nur die Antwort.

- **Ein hängender Tool kann einen Lauf nicht mehr unbegrenzt stallen.** Jeder Tool-Aufruf hat jetzt ein Wanduhrzeitlimit (Standard 180s), daher endet ein stalled Connector oder unresponsiver Server den Schritt mit einer klaren Meldung, anstatt den Turn zu blockieren.
...
- **Lange Konversationen verlieren ihren früheren Kontext nach der Komprimierung nicht mehr.** Die Zusammenfassung, die ältere Turns verdichtet, wurde beim nächsten Lauf stillschweigend verworfen; sie wird jetzt weitergeleitet, daher behält der Agent, was zuvor besprochen wurde.

- **Große Tool-Ergebnisse behalten ihr Ende sowie ihren Anfang.** Wenn ein langes Ergebnis gekürzt wird, um in das Kontextfenster zu passen, werden jetzt beide Enden beibehalten (der Abschluss der Ausgabe eines Befehls ist normalerweise am wichtigsten), die Kürzung berücksichtigt korrekt nicht-lateinische Texte, und späteren Ergebnissen ist ein Mindestfenster garantiert, anstatt vollständig gelöscht zu werden.
...
- **Interne Offload-Dateien können nicht mehr als herunterladbare Deliverables angezeigt werden.** Mit Tools, die ein Dateisystem teilen, konnte eine große Tool-Ausgabe, die während eines Code-Execution-Laufs offloaded wurde, vom Artifact-Scan aufgegriffen und als gefälschte Download-Karte angezeigt werden. Runtime-Bookkeeping-Dateien sind jetzt von der Artifact-Erkennung ausgeschlossen.

- **Jedes Tool in einer Konversation funktioniert jetzt auf einem gemeinsamen Dateisystem.** Offloaded große Ausgaben landeten früher in einem Verzeichnis, das nur die Workspace-Lese-Tools sehen konnten, während `file_ops`, `shell_exec` und `python_exec` anderswo operierten — der Agent würde eine Datei auflisten, dann würde ihm gesagt, dass sie nicht existiert, und würde sein Iterations-Budget verbrauchen, um sie zu suchen. Offloaded-Dateien befinden sich jetzt in der Konversations-Sandbox, die jedes Tool (einschließlich Exec-Container) bereitstellt; Dateien aus älteren Konversationen bleiben lesbar.

- **Das Lesen einer großen offloaded Datei erzeugt nicht mehr eine endlose Kette von Zeigerdateien.** Die Ausgabe eines Workspace-Lesevorgangs könnte selbst die Offload-Schwelle überschreiten und als neue Datei gespeichert werden, die auf Inhalte verweist, die bereits auf der Festplatte waren, was den Agenten in Kreise schickt. Workspace-Lesevorgänge werden jetzt mit einer genauen Resume-Zeile begrenzt, anstatt erneut offloaded zu werden.

- **DeepSeeks improvisierter Tool-Call-Text leckt nicht mehr in Antworten.** Wenn ein DeepSeek-Modell einen Tool-Aufruf als reinen `<｜｜DSML｜｜Tool call: ...` Text schrieb, anstatt den Function-Calling-Kanal zu verwenden, erreichte das rohe Markup den Chat wörtlich. Es wird jetzt wie andere Tool-Call-Pseudo-Protokolle entfernt.

- **Ein Tool-Aufruf, der durch das Ausgabelimit des Modells abgeschnitten wird, wird nicht mehr als abgeschlossene Arbeit übergeben.** Wenn eine lange Nutzlast (z. B. das Schreiben einer vollständigen HTML-Seite) das Ausgabe-Budget mitten im Aufruf erschöpfte, wurde der halb geschriebene Aufruf stillschweigend verworfen und der Einleitungstext des Agenten — „jetzt speichere ich die Datei" — wurde zum Step-Ergebnis, daher wurde ein Step, der nichts produzierte, als abgeschlossen gemeldet. Die Kürzung wird jetzt erkannt, und dem Agenten wird mitgeteilt, die Arbeit in kleinere Aufrufe aufzuteilen.

- **Ein Ziel, das der Agent sicher verpasst hat, wird jetzt erneut versucht, anstatt den Lauf zu beenden.** Die Neuplanung stoppte, wann immer der Analyzer sicher war, was ein sicheres „das Deliverable fehlt" als Grund zum Aufgeben las — genau der Fall, für den die Neuplanung existiert. Läufe stoppen jetzt früh nur, wenn das Ziel als wirklich unerreichbar beurteilt wird, und `DAG_MAX_REPLAN_ROUNDS` begrenzt den Rest.

- **Die Selbstprüfung des Agenten wird nicht mehr als Antwort angezeigt.** Nach der Verwendung mehrerer Tools führt der Agent eine einmalige interne Überprüfung durch, bevor er antwortet; diese Überprüfung wurde wörtlich zu Ihnen gestreamt, daher kamen Antworten als „Lassen Sie mich überprüfen: 1. Vollständig adressiert? ✅ …" an, anstatt der Antwort selbst. Die Überprüfung ist jetzt intern, und die Antwort, die Sie sehen, wird als eigener Schritt geschrieben.

- **Die Refresh-Token-Rotation invalidiert jetzt tatsächlich das vorherige Token.** JWT `exp` ist nur Sekunden-Präzision, daher zwei Refresh-Tokens, die für denselben Benutzer in derselben Sekunde ausgegeben wurden, codierten identisch — die Rotation schrieb dann den gespeicherten Hash mit demselben Wert um, und ein gestohlenes Refresh-Token konnte auf ewig erneuert werden. Jedes Refresh-Token trägt jetzt eine eindeutige `jti`, daher unterscheidet sich das rotierte Token und das alte stoppt sofort zu funktionieren.

- **Approval Gates halten jetzt, wenn ein Agent durch einen anderen Agenten oder einen Workflow erreicht wird.** Ein sensitiver Connector-Aufruf, der in Chat um Bestätigung bitten würde, lief früher ungeprüft, sobald er über `call_agent` delegiert oder in einen Workflow `AGENT` Node eingewickelt wurde — diese Pfade starteten den Agenten ohne Hooks überhaupt. Beide führen jetzt die Gates des Agenten aus, verwenden die Bestätigungseinstellungen dieses Agenten, und weigern sich, ihn auszuführen, wenn seine Hooks nicht geladen werden können.

- **Ein Tippfehler in `SMTP_SSL` sendet Ihr SMTP-Passwort nicht mehr stillschweigend im Klartext.** Früher fiel jeder andere Wert als `ssl` oder `tls` (z. B. `SMTP_SSL=true`) ohne Warnung auf eine unverschlüsselte Verbindung durch, sowohl in System-E-Mails als auch in E-Mail-Benachrichtigungen. Unbekannte Werte werden jetzt rundweg abgelehnt, und unverschlüsselte Zustellung muss explizit als `SMTP_SSL=none` angefordert werden, was bei jedem Versand eine Warnung protokolliert.
