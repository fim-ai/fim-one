...
| `REACT_MAX_TURN_TOKENS` | Nein | `0` | Notfall-Schutzschalter: max. kumulative Tokens (Prompt + Completion über alle Iterationen) pro einzelnem ReAct-Durchgang. Standard `0` = unbegrenzt. **Dies ist NICHT für tägliche Token-Kontrolle** — verwenden Sie dafür `token_quota` pro Benutzer. Dies ist ein letzter Ausweg für extreme Szenarien wie ein Agent in einer Endlosschleife von Werkzeugaufrufen. Das Erreichen dieses Limits bricht die Aufgabe mitten in der Ausführung ab, verschwendet alle verbrauchten Tokens und gibt ein unvollständiges Ergebnis zurück. Behalten Sie `0` bei, es sei denn, Sie haben ein spezifisches Problem mit durchgehenden Agenten zu beheben |
| `REACT_TOOL_SELECTION_THRESHOLD` | Nein | `12` | Wenn die Gesamtzahl der registrierten Werkzeuge diesen Schwellenwert überschreitet, wählt ein leichtgewichtiger LLM-Aufruf die relevanteste Teilmenge vor jeder Anfrage aus |
...
| `REACT_SELF_REFLECTION_INTERVAL` | Nein | `6` | Injizieren Sie alle N Werkzeugaufrufe einen Selbstreflexions-Prompt, um dem Agent zu helfen, den Kurs zu korrigieren und Schleifen zu vermeiden |
| `REACT_TOOL_OBS_TRUNCATION` | Nein | `8000` | Max. Zeichen pro Werkzeugbeobachtung bei der Synthese der endgültigen Antwort. Höhere Werte bewahren mehr strukturierte Daten (JSON, Tabellen) auf Kosten von mehr Tokens |
| `REACT_TOOL_RESULT_BUDGET` | Nein | `40000` | Gesamtes Token-Budget für alle Werkzeugergebnisse in einer einzelnen Sitzung. Wenn die Gesamttoken der Werkzeugergebnisse diese Obergrenze überschreiten, werden neue Ergebnisse mit einer Benachrichtigung gekürzt. Verhindert Kontext-Überfluss durch große API-Antworten (z. B. 5 Connector-Aufrufe, die jeweils 8K zurückgeben). Setzen Sie auf `0`, um die Obergrenze zu deaktivieren |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | Nein | `800` | Überspringen Sie den LLM-Aufruf zur Vollständigkeitsprüfung nach der Antwort, wenn die endgültige Antwort des Agenten diese viele Zeichen überschreitet. Lange detaillierte Antworten benötigen keine Verifizierungs-Hin- und Rückfahrt „Habe ich etwas übersehen?". Setzen Sie niedriger, um aggressiver zu überspringen; setzen Sie auf einen sehr großen Wert, um die Prüfung immer auszuführen |
| `REACT_CYCLE_DETECTION_THRESHOLD` | Nein | `2` | Wenn dasselbe Werkzeug mit identischen Argumenten diese viele Male hintereinander aufgerufen wird, wird eine deterministische Warnung injiziert, die dem Agent mitteilt, einen anderen Ansatz zu versuchen. Im Gegensatz zur Selbstreflexion (die darauf angewiesen ist, dass das LLM die Schleife bemerkt) ist dies eine Hash-basierte Prüfung, die nicht umgangen werden kann. Gilt auch für DAG-Schritte |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | Nein | `3` | Minimale Anzahl von Werkzeugaufrufen, bevor die Vollständigkeitsprüfung ausgelöst wird. Einfache Aufgaben (1-2 Werkzeugaufrufe) überspringen die Verifizierung, um unnötige Latenz zu vermeiden. Setzen Sie auf `1` für immer aktivierte Verifizierung. Gilt auch für DAG-Schritte |
| `REACT_TURN_PROFILE_ENABLED` | Nein | `true` | Geben Sie Pro-Durchgang-Phasen-Timing-Logs aus (`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`). Eine strukturierte Log-Zeile pro Durchgang. Setzen Sie auf `false`, um die Profilerstellung vollständig zu deaktivieren (null Overhead) |
| `REACT_PLAN_TOOL_ENABLED` | Nein | `true` | Registrieren Sie das `update_plan` Todo-Werkzeug, damit der Agent während mehrstufiger Aufgaben eine Planprüfliste schreibt und verwaltet. Wird automatisch für DAG-Schritt-Agenten und Agenten ohne Werkzeuge übersprungen |
| `REACT_PLAN_REMINDER_INTERVAL` | Nein | `3` | Werkzeugrunden ohne `update_plan`-Aufruf, bevor eine Erinnerung für veraltete Pläne (mit vollständiger Prüfliste) erneut in das Gespräch injiziert wird, damit der Plan die Kontext-Komprimierung überlebt |
| `REACT_PLAN_REPEAT_THRESHOLD` | Nein | `4` | Aufeinanderfolgende Runden, in denen dasselbe Werkzeug aufgerufen wird (mit unterschiedlichen Argumenten), bevor eine Erinnerung dem Agent mitteilt, einen anderen Ansatz zu versuchen, anstatt fruchtlose Aufrufe zu wiederholen. Exakte doppelte Aufrufe werden separat durch Zyklenerkennung behandelt |
| `REACT_PLAN_NUDGE_AFTER` | Nein | `5` | Werkzeugrunden ohne aufgezeichneten Plan, bevor ein einmaliger Anstoß vorschlägt, den Plan aufzuschreiben. Gilt nur, wenn das Plan-Werkzeug aktiviert ist |
| `REACT_MAX_CONTINUATIONS` | Nein | `3` | Maximale Fortsetzungsrunden, wenn die Antwort des Modells durch das Output-Token-Limit des Anbieters gekürzt wird (`finish_reason=length`). Gekürzte Segmente werden sowohl in der Agent-Schleife als auch in der gestreamten Synthese nahtlos zusammengefügt |
...
| `DAG_CHECKPOINT_EVIDENCE_CHARS` | Nein | `4000` | Pro-Schritt-Evidenz-Obergrenze in der DAG-Crash-Resume-Checkpoint-Datei (`data/dag_checkpoints/`). Schrittzusammenfassungen werden vollständig gespeichert |
| `DAG_CHECKPOINT_MAX_AGE_HOURS` | Nein | `24` | DAG-Checkpoints älter als dies werden beim Laden ignoriert, sodass veraltete Crash-Reste nie in einen frischen Durchgang fortgesetzt werden |
| `LLM_RATE_LIMIT_PER_USER` | Nein | `true` | Verwenden Sie pro-Benutzer-Schlüssel-Rate-Limit-Buckets anstelle eines einzelnen prozessglobalen Buckets. Verhindert, dass ein lauter Benutzer alle anderen auf demselben Worker verhungert. Die zugrunde liegende Rate ist auf 60 Anfragen/Min und 100K Tokens/Min pro Bucket festgelegt — diese Einstellung steuert nur, ob der Bucket gemeinsam genutzt wird (global) oder partitioniert wird (pro Benutzer). Setzen Sie auf `false`, um zum Legacy-Global-Bucket zurückzukehren (nicht empfohlen) |

### DAG Planner

| Variable | Required | Default | Description |
|---|---|---|---|
| `MAX_CONCURRENCY` | No | `5` | Maximale parallele Schritte im DAG-Executor |
| `DAG_STEP_MAX_ITERATIONS` | No | `15` | Maximale Werkzeugaufrufe pro DAG-Schritt |
| `DAG_STEP_TIMEOUT` | No | `600` | Timeout für Schrittausführung in Sekunden. Schritte, die diesen Wert überschreiten, werden als fehlgeschlagen markiert und ihre abhängigen Schritte werden übersprungen |
| `DAG_MAX_REPLAN_ROUNDS` | No | `3` | Maximale autonome Neuplanungsversuche, wenn das Ziel nicht erreicht wird. Benutzerunterbrechungen (Injektionen) sind unbegrenzt und zählen nicht gegen dieses Budget |
| `DAG_REPLAN_STOP_CONFIDENCE` | No | `0.8` | Gilt nur, wenn der Agent das Ziel als **unerreichbar** einstuft (unmögliche Anfrage, fehlende Fähigkeit, nicht erreichbare Ressource): Wiederholungen bei dieser oder höherer Sicherheit stoppen. Ein fehlendes oder unvollständiges Ergebnis wird immer neu geplant, unabhängig von der Sicherheit — nur `DAG_MAX_REPLAN_ROUNDS` begrenzt diese Wiederholungen |
| `DAG_VERIFY_TRUNCATION` | No | `2000` | Maximale Zeichen der Schrittausgabe, die an das Schritt-Verifikations-LLM zur Qualitätsprüfung gesendet werden |
| `DAG_ANALYZER_TRUNCATION` | No | `10000` | Maximale Zeichen pro Schrittergebnis beim Formatieren für den Post-Execution-Analyzer |
| `DAG_STEP_EVIDENCE_CHARS` | No | `16000` | Maximale Zeichen der rohen Werkzeugausgabe (Web-Abrufe, Suchergebnisse, Dateileser) pro Schritt als autoritative "Quellevidence". Dies wird dem Analyzer und der abschließenden Synthese zusammen mit der Schrittzusammenfassung zugeführt, sodass die faktischen Ansprüche der Antwort (Summen, Aufzählungen, Schweregrade) gegen die Quelle überprüft werden können, anstatt einer Zusammenfassung zu vertrauen, die möglicherweise Elemente stillschweigend gelöscht oder falsch gekennzeichnet hat. Auf `0` setzen, um die Evidence-Erfassung zu deaktivieren |
| `DAG_REPLAN_RECENT_TRUNCATION` | No | `500` | Maximale Zeichen pro Schrittergebnis aus der letzten Runde beim Aufbau des Neuplanungskontexts |
| `DAG_REPLAN_OLDER_TRUNCATION` | No | `200` | Maximale Zeichen pro Schrittergebnis aus älteren Runden beim Aufbau des Neuplanungskontexts. Ältere Runden werden aggressiver gekürzt, um Kontext zu sparen |
| `DAG_TOOL_CACHE` | No | `true` | Identische Werkzeugaufrufe innerhalb einer einzelnen DAG-Ausführung zwischenspeichern. Nur Werkzeuge, die explizit als `cacheable` markiert sind (schreibgeschützte Werkzeuge wie Suche, Wissensbeschaffung), werden zwischengespeichert. Auf `false` setzen, um das Zwischenspeichern vollständig zu deaktivieren |
| `DAG_STEP_VERIFICATION` | No | `false` | Generische LLM-basierte Qualitätsprüfung nach jedem DAG-Schritt. Bei Fehler wird der Schritt einmal mit Feedback wiederholt. **Standardmäßig deaktiviert** — fügt Latenz bei jedem Schritt hinzu und ist selten erforderlich; die meisten Schrittergebnisse sind ohne Neuprüfung akzeptabel. Verwenden Sie dies nur, wenn Sie häufig Schrittergebnisse mit niedriger Qualität beobachten |
| `DAG_CITATION_VERIFICATION` | No | `true` | Zitiergenauigkeitsprüfung für Schritte in Spezialdomänen. **Voraussetzung**: Die Anfrage muss zunächst vom LLM-Domänenklassifizierer als Spezialdomäne klassifiziert werden (siehe `ESCALATION_DOMAINS`). Wenn die Domäne erkannt wird UND dieses Flag `true` ist, wird jeder abgeschlossene Schritt auf rechtliche/medizinische/finanzielle Zitate gescannt und auf Genauigkeit überprüft — um halluzinierte Artikelnummern, erfundene Fallreferenzen und falsche behördliche Zitate zu erkennen. Wenn die Domänenklassifizierung `null` zurückgibt (allgemeine Anfrage), wird die Zitierprüfung unabhängig von dieser Einstellung nicht ausgeführt |
| `DAG_CITATION_VERIFY_TRUNCATION` | No | `6000` | Maximale Zeichen des Schrittergebnisses, die an die Zitierverifikationseingabeaufforderung gesendet werden |
...
| Variable | Required | Default | Description |
|---|---|---|---|
| ~~`SYSTEM_PROMPT_RESERVE`~~ | — | — | **Entfernt.** Subtrahierte zuvor einen festen 4K-Reserve vom Kontext-Budget für System-Prompts. Dies führte zu Doppelzählungen, da ContextGuard bereits den System-Prompt bei der Schätzung der Token der Nachrichtenliste berücksichtigt. Die Budget-Formel lautet nun `(context_size - max_output_tokens) × 0.92` (die Marge absorbiert Token-Schätzungsfehler), und die tatsächliche Größe des System-Prompts wird dynamisch berücksichtigt |
...
| Variable | Erforderlich | Standard | Beschreibung |
|---|---|---|---|
| `DATABASE_URL` | Nein | `sqlite+aiosqlite:///./data/fim_one.db` | Datenbankverbindungszeichenfolge. **SQLite** (keine Konfiguration): `sqlite+aiosqlite:///./data/fim_one.db`. **PostgreSQL** (Produktion): `postgresql+asyncpg://user:pass@localhost:5432/fim_one`. Docker Compose konfiguriert PostgreSQL automatisch. |
...
| `FIM_BCRYPT_COST` | Nein | `12` | bcrypt-Arbeitsfaktor für Passwort-Hashing, begrenzt auf 4-31. Der Standard 12 kostet etwa 200ms pro Hash auf modernen CPUs; senken Sie ihn auf schwacher Hardware, erhöhen Sie ihn für sicherheitsverstärkte Bereitstellungen. |
| `CORS_ORIGINS` | Nein | — | Kommagetrennte Liste zusätzlich zulässiger CORS-Ursprünge über die Standard-Localhost-Einträge hinaus. Erforderlich, wenn das Frontend auf einer Nicht-Localhost-Domain läuft (z. B. `https://app.example.com`). |
| `UPLOADS_DIR` | Nein | `./uploads` | Verzeichnis für hochgeladene Dateien |
| `EXPORT_FONT_DIR` | Nein | *(automatisch erkannt)* | Verzeichnis mit `NotoSansSC-Regular.ttf` / `NotoSansSC-Bold.ttf` für PDF-Export. Abrufen mit `python scripts/fetch_export_fonts.py`; Docker-Images enthalten es bereits. Ohne einbettbare TrueType-CJK-Schriftart fällt der PDF-Export auf eine nicht eingebettete CID-Schriftart mit verschlechtertem Abstand und ohne Fettdruck zurück. |
| `MAX_UPLOAD_SIZE_MB` | Nein | `50` | Maximale Dateigröße beim Upload in Megabyte (Backend-Durchsetzung) |
| `NEXT_PUBLIC_MAX_UPLOAD_SIZE_MB` | Nein | `50` | Maximale Dateigröße beim Upload in der Frontend-UI angezeigt. **Build-Zeit-Variable** — muss mit `MAX_UPLOAD_SIZE_MB` übereinstimmen. |
| `MCP_SERVERS` | Nein | — | JSON-Array von MCP-Server-Konfigurationen (erfordert `uv sync --extra mcp`) |
...
| `REDIS_URL` | Nein | — | Redis-Verbindungs-URL für Cross-Worker-Interrupt-Relais. **Erforderlich, wenn `WORKERS>1`** — ohne sie können Mid-Stream-Interrupt-/Inject-Anfragen einen anderen Worker treffen und stillschweigend fehlschlagen. Wird von Docker Compose automatisch konfiguriert. |
| `WORKERS` | Nein | `1` | Uvicorn-Worker-Prozesse. `1` ist sicher und benötigt keine externen Services. Für Multi-Worker-Produktion verwenden Sie PostgreSQL (SQLite ist Single-Writer). SQLite funktioniert für lokale Entwicklung unter leichter Last. Authentifizierung, OAuth und Dateivorgänge sind vollständig Multi-Worker-sicher (JWT-basiert). Docker Compose konfiguriert automatisch sowohl PostgreSQL als auch Redis. |
