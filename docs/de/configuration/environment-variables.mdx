...
| `REACT_MAX_TURN_TOKENS` | No | `0` | Notfall-Schutzschalter: maximale kumulative Tokens (Prompt + Completion über alle Iterationen) pro einzelnem ReAct-Turn. Standard `0` = unbegrenzt. **Dies ist NICHT für die tägliche Token-Kontrolle** — verwenden Sie dafür `token_quota` pro Benutzer. Dies ist ein letztes Sicherheitsventil für extreme Szenarien wie ein Agent, der in einer Endlosschleife von Tool-Aufrufen steckt. Das Erreichen dieses Limits bricht die Aufgabe während der Ausführung ab, verschwendet alle verbrauchten Tokens und gibt ein unvollständiges Ergebnis zurück. Behalten Sie `0` bei, es sei denn, Sie haben ein spezifisches Runaway-Agent-Problem zu kontrollieren |
| `REACT_TOOL_SELECTION_THRESHOLD` | No | `12` | Wenn die Gesamtzahl der registrierten Tools diesen Schwellenwert überschreitet, wählt ein leichtgewichtiger LLM-Aufruf vor jeder Anfrage die relevanteste Teilmenge aus |
| `REACT_TOOL_SELECTION_MAX` | No | `6` | Max Tools nach intelligenter Auswahl (nur wirksam, wenn die Tool-Anzahl `REACT_TOOL_SELECTION_THRESHOLD` überschreitet) |
...
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | Max Zeichen pro Tool-Observation bei der Synthese der endgültigen Antwort. Höhere Werte bewahren mehr strukturierte Daten (JSON, Tabellen) auf Kosten von mehr Tokens |
| `REACT_TOOL_RESULT_BUDGET` | No | `40000` | Aggregiertes Token-Budget für alle Tool-Ergebnisse in einer einzelnen Sitzung. Wenn die gesamten Tool-Ergebnis-Tokens diese Obergrenze überschreiten, werden neue Ergebnisse mit einer Benachrichtigung gekürzt. Verhindert Context-Bloat durch große API-Antworten (z. B. 5 Connector-Aufrufe mit je 8K). Setzen Sie auf `0`, um die Obergrenze zu deaktivieren |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | Überspringen Sie den Post-Answer-Completion-Check-LLM-Aufruf, wenn die endgültige Antwort des Agenten diese viele Zeichen überschreitet. Lange detaillierte Antworten benötigen keine „Habe ich etwas übersehen?"-Verifikationsrunde. Setzen Sie niedriger, um aggressiver zu überspringen; setzen Sie auf einen sehr großen Wert, um die Überprüfung immer auszuführen |
| `REACT_CYCLE_DETECTION_THRESHOLD` | No | `2` | Wenn das gleiche Tool mit identischen Argumenten diese viele Male hintereinander aufgerufen wird, wird eine deterministische Warnung injiziert, die dem Agent mitteilt, einen anderen Ansatz zu versuchen. Im Gegensatz zu Self-Reflection (das darauf angewiesen ist, dass das LLM die Schleife bemerkt), ist dies eine Hash-basierte Überprüfung, die nicht umgangen werden kann. Gilt auch für DAG-Schritte |
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | Minimale Anzahl von Tool-Aufrufen, bevor die Completion-Checkliste aktiviert wird. Einfache Aufgaben (1-2 Tool-Aufrufe) überspringen die Verifizierung, um unnötige Latenz zu vermeiden. Setzen Sie auf `1` für immer aktivierte Verifizierung. Gilt auch für DAG-Schritte |
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | Emittieren Sie Pro-Turn-Phasen-Level-Timing-Logs (`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`). Eine strukturierte Log-Zeile pro Turn. Setzen Sie auf `false`, um Profiling vollständig zu deaktivieren (null Overhead) |
...
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | Tool-Runden ohne `update_plan`-Aufruf, bevor eine Erinnerung für veraltete Pläne (mit vollständiger Checkliste) erneut in das Gespräch eingefügt wird, damit der Plan die Context-Komprimierung überlebt |
| `REACT_MAX_CONTINUATIONS` | No | `3` | Maximale Fortsetzungsrunden, wenn die Antwort des Modells durch das Output-Token-Limit des Providers (`finish_reason=length`) unterbrochen wird. Gekürzte Segmente werden zu einer nahtlosen Antwort zusammengefügt, sowohl in der Agent-Schleife als auch in der gestreamten Synthese |
...
| `REACT_BG_WAIT_TIMEOUT` | No | `300` | Maximale Sekunden zum Warten auf noch laufende Background-Tools, wenn der Agent seine Antwort finalisieren möchte. Aufgaben, die nicht innerhalb des Fensters abgeschlossen sind, werden mit einer expliziten Timeout-Benachrichtigung storniert |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | Verwenden Sie pro Benutzer verschlüsselte Rate-Limit-Buckets anstelle eines einzelnen prozessweiten Buckets. Verhindert, dass ein lauter Benutzer alle anderen auf dem gleichen Worker verhungert. Die zugrunde liegende Rate ist auf 60 Anfragen/min und 100K Tokens/min pro Bucket festgelegt — diese Einstellung steuert nur, ob der Bucket gemeinsam genutzt wird (global) oder partitioniert (pro Benutzer). Setzen Sie auf `false`, um zum Legacy-Global-Bucket zurückzukehren (nicht empfohlen) |
