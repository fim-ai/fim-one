...
| `REACT_MAX_TURN_TOKENS` | No | `0` | Notfall-Schutzschalter: maximale kumulative Tokens (Prompt + Completion über alle Iterationen) pro einzelner ReAct-Runde. Standard `0` = unbegrenzt. **Dies ist NICHT für die tägliche Token-Kontrolle** — verwenden Sie dafür `token_quota` pro Benutzer. Dies ist ein letztes Sicherheitsventil für extreme Szenarien wie einen Agent, der in einer Endlosschleife von Tool-Aufrufen steckt. Das Erreichen dieses Limits bricht die Aufgabe während der Ausführung ab, verschwendet alle verbrauchten Tokens und gibt ein unvollständiges Ergebnis zurück. Behalten Sie `0` bei, es sei denn, Sie haben ein spezifisches Runaway-Agent-Problem zu beheben |
| `REACT_TOOL_SELECTION_THRESHOLD` | No | `12` | Wenn die Gesamtzahl der registrierten Tools diesen Schwellenwert überschreitet, wählt ein leichtgewichtiger LLM-Aufruf die relevanteste Teilmenge vor jeder Anfrage aus |
...
| `REACT_SELF_REFLECTION_INTERVAL` | No | `6` | Injizieren Sie alle N Tool-Aufrufe einen Self-Reflection-Prompt, um dem Agent zu helfen, seinen Kurs zu korrigieren und Schleifen zu vermeiden |
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | Max Zeichen pro Tool-Beobachtung bei der Synthese der endgültigen Antwort. Höhere Werte bewahren mehr strukturierte Daten (JSON, Tabellen) auf Kosten von mehr Tokens |
| `REACT_TOOL_RESULT_BUDGET` | No | `40000` | Aggregiertes Token-Budget für alle Tool-Ergebnisse in einer einzelnen Sitzung. Wenn die gesamten Tool-Ergebnis-Tokens diese Obergrenze überschreiten, werden neue Ergebnisse mit einer Benachrichtigung gekürzt. Verhindert Context Bloat durch große API-Antworten (z. B. 5 Connector-Aufrufe, die jeweils 8K zurückgeben). Setzen Sie auf `0`, um die Obergrenze zu deaktivieren |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | Überspringen Sie den Post-Answer-Completion-Check-LLM-Aufruf, wenn die endgültige Antwort des Agenten diese viele Zeichen überschreitet. Lange detaillierte Antworten benötigen keine Verifikations-Hin- und Rückfahrt „Habe ich etwas übersehen?". Setzen Sie niedriger, um aggressiver zu überspringen; setzen Sie auf einen sehr großen Wert, um die Überprüfung immer auszuführen |
...
| `REACT_COMPLETION_CHECK_MIN_TOOLS` | No | `3` | Minimale Anzahl von Tool-Aufrufen, bevor die Completion-Checkliste aktiviert wird. Einfache Aufgaben (1-2 Tool-Aufrufe) überspringen die Verifikation, um unnötige Latenz zu vermeiden. Setzen Sie auf `1` für immer aktivierte Verifikation. Gilt auch für DAG-Schritte |
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | Geben Sie Pro-Runde-Phasen-Timing-Logs aus (`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`). Eine strukturierte Log-Zeile pro Runde. Setzen Sie auf `false`, um Profiling vollständig zu deaktivieren (null Overhead) |
...
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | Tool-Runden ohne `update_plan`-Aufruf, bevor eine veraltete Plan-Erinnerung (mit vollständiger Checkliste) wieder in das Gespräch injiziert wird, damit der Plan die Context-Komprimierung übersteht |
| `REACT_MAX_CONTINUATIONS` | No | `3` | Maximale Fortsetzungsrunden, wenn die Antwort des Modells durch das Output-Token-Limit des Providers begrenzt wird (`finish_reason=length`). Abgeschnittene Segmente werden in einer nahtlosen Antwort zusammengefügt, sowohl in der Agent-Schleife als auch in der gestreamten Synthese |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | Verwenden Sie pro-Benutzer-Schlüssel-Rate-Limit-Buckets anstelle eines einzelnen prozessweiten Buckets. Verhindert, dass ein lauter Benutzer alle anderen auf dem gleichen Worker verhungert. Die zugrunde liegende Rate ist auf 60 Anfragen/min und 100K Tokens/min pro Bucket hartcodiert — diese Einstellung steuert nur, ob der Bucket gemeinsam genutzt wird (global) oder partitioniert wird (pro Benutzer). Setzen Sie auf `false`, um zum Legacy-Global-Bucket zurückzukehren (nicht empfohlen) |
