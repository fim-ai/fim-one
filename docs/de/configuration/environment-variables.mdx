...
### Den ENV-Fallback mit dem Admin-Panel synchron halten

Sobald eine Modellgruppe unter Admin → Modelle aktiv ist, bezieht die Laufzeit ihre Modelle aus dieser Gruppe, und die oben genannten Werte `LLM_*` / `FAST_LLM_*` / `REASONING_LLM_*` dienen nur als Fallback. Nebenpfade, die die Datenbank nie öffnen, lesen diese Werte weiterhin direkt aus: `scripts/translate.py` (der i18n-Pre-Commit-Hook), `evals/`, der Jina/OpenAI-Reranker und die Bildgenerierung. Damit diese nicht auf einen veralteten Provider ausweichen, kopieren Sie die aktive Gruppe in die Umgebungsdatei:
...
Das Skript liest die Provider-Schlüssel über das ORM aus und schreibt sie direkt in die Datei; es wird nichts ausgegeben. Starten Sie die App anschließend neu.
