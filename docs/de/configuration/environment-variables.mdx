...
| `REACT_MAX_TURN_TOKENS` | No | `0` | Notfall-Sicherheitsschalter: maximale kumulative Token (Prompt + Completion über alle Iterationen) pro einzelnem ReAct-Turn. Standard `0` = unbegrenzt. **Dies ist NICHT für die tägliche Token-Kontrolle** — verwenden Sie dafür `token_quota` pro Benutzer. Dies ist ein letzter Ausweg für extreme Szenarien wie ein Agent, der in einer Endlosschleife von Tool-Aufrufen steckt. Das Erreichen dieses Limits bricht die Aufgabe mitten in der Ausführung ab, verschwendet alle verbrauchten Token und gibt ein unvollständiges Ergebnis zurück. Behalten Sie `0` bei, es sei denn, Sie haben ein spezifisches Runaway-Agent-Problem zu beheben |
...
| `REACT_TOOL_OBS_TRUNCATION` | No | `8000` | Max Zeichen pro Tool-Observation bei der Synthese der endgültigen Antwort. Höhere Werte bewahren mehr strukturierte Daten (JSON, Tabellen) auf Kosten von mehr Token |
| `REACT_TOOL_RESULT_BUDGET` | No | `40000` | Aggregiertes Token-Budget für alle Tool-Ergebnisse in einer einzelnen Sitzung. Wenn die gesamten Tool-Ergebnis-Token diese Obergrenze überschreiten, werden neue Ergebnisse mit einer Benachrichtigung gekürzt. Verhindert Context-Bloat durch große API-Antworten (z. B. 5 Connector-Aufrufe, die jeweils 8K zurückgeben). Setzen Sie auf `0`, um die Obergrenze zu deaktivieren |
| `REACT_COMPLETION_CHECK_SKIP_CHARS` | No | `800` | Überspringen Sie den Post-Answer-Completion-Check-LLM-Aufruf, wenn die endgültige Antwort des Agenten diese Anzahl von Zeichen überschreitet. Lange detaillierte Antworten benötigen keine Verifikations-Hin- und Rückfahrt "Habe ich etwas übersehen?". Setzen Sie niedriger, um aggressiver zu überspringen; setzen Sie auf einen sehr großen Wert, um die Prüfung immer auszuführen |
| `REACT_CYCLE_DETECTION_THRESHOLD` | No | `2` | Wenn das gleiche Tool mit identischen Argumenten diese Anzahl von Malen hintereinander aufgerufen wird, wird eine deterministische Warnung injiziert, die dem Agent sagt, einen anderen Ansatz zu versuchen. Im Gegensatz zur Self-Reflection (die darauf angewiesen ist, dass das LLM die Schleife bemerkt), ist dies eine Hash-basierte Prüfung, die nicht umgangen werden kann. Gilt auch für DAG-Schritte |
...
| `REACT_TURN_PROFILE_ENABLED` | No | `true` | Emit pro-Turn Phase-Level Timing-Logs (`memory_load`, `compact`, `tool_schema_build`, `llm_first_token`, `llm_total`, `tool_exec`). Eine strukturierte Log-Zeile pro Turn. Setzen Sie auf `false`, um Profiling komplett zu deaktivieren (null Overhead) |
...
| `REACT_PLAN_REMINDER_INTERVAL` | No | `3` | Tool-Runden ohne `update_plan`-Aufruf, bevor eine Erinnerung für veraltete Pläne (mit vollständiger Checkliste) erneut in das Gespräch injiziert wird, damit der Plan Context-Komprimierung übersteht |
| `LLM_RATE_LIMIT_PER_USER` | No | `true` | Verwenden Sie pro-Benutzer-Schlüssel-Rate-Limit-Buckets statt eines einzelnen prozessglobalen Buckets. Verhindert, dass ein lauter Benutzer alle anderen auf dem gleichen Worker verhungert. Die zugrunde liegende Rate ist auf 60 Anfragen/Min und 100K Token/Min pro Bucket hartcodiert — diese Einstellung steuert nur, ob der Bucket gemeinsam genutzt wird (global) oder partitioniert wird (pro-Benutzer). Setzen Sie auf `false`, um zum Legacy-Global-Bucket zurückzukehren (nicht empfohlen) |
