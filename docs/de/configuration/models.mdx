...
<Tip>
**Vision** gibt an, ob das Modell Bildeingaben akzeptiert. Dies ist erforderlich für [Intelligent Document Processing (IDP)](/features/idp) — wenn Ihr Modell Vision nicht unterstützt, greift IDP auf reine Textextraktion zurück. Anbieter, die mit ⚠️ gekennzeichnet sind, haben Vision auf einigen Modellen, aber nicht auf anderen; überprüfen Sie das spezifische Modell, das Sie verwenden.

Chat-Anhänge folgen demselben Flag: Bei einem reinen Textmodell wird ein angehängtes Bild nicht gesendet, und das Modell erhält nur seinen Dateinamen. Der Chat-Composer teilt dies mit, bevor Sie senden, basierend auf dem Modell, das der Turn tatsächlich verwenden würde (Agent-Einstellung, dann die aktive Modellgruppe, dann der Systemstandard). Dokumenttext ist nicht betroffen — eine PDF oder DOCX hat ihren Inhalt weiterhin extrahiert und eingefügt.
</Tip>

Diese Tabelle listet die Kombinationen auf, die wir empfehlen, nicht die vollständige Menge der Anbieter, die FIM One unterstützt. xAI (Grok), ByteDance Doubao, Mistral und jedes OpenAI-kompatible Relay funktionieren; siehe die [Provider Capability Matrix](/architecture/llm-provider-guide#provider-capability-matrix) für die vollständige Liste und wie jeder weitergeleitet wird.
