...
|---|---|---|---|---|---|
| **OpenAI** | `gpt-5.4` | `gpt-5.4-mini` / `gpt-5.4-nano` | ✅ `reasoning_effort` | ✅ Alle | Bestes natives Tool-Calling; GPT-5.4 ist das neueste Spitzenmodell (März 2026) |
| **Anthropic** | `claude-sonnet-4-6` | `claude-haiku-4-5` | ✅ über LiteLLM | ✅ Alle | Native API-Routenführung; vollständige Unterstützung für `reasoning_content`; 1M-Kontext allgemein verfügbar |
| **Google Gemini** | `gemini-2.5-pro` / `gemini-3.1-pro-preview` | `gemini-2.5-flash` / `gemini-3-flash-preview` | ✅ `reasoning_effort` | ✅ Alle | 2.5 ist stabil allgemein verfügbar; 3.x ist eine Preview; `gemini-3-pro-preview` wurde am 9. März abgeschaltet |
| **DeepSeek** | `deepseek-chat` (V3.2) | `deepseek-chat` | ✅ `deepseek-reasoner` | ❌ | Nur Text; V4 (April 2026) wird Vision-Unterstützung hinzufügen |
| **Qwen (Alibaba)** | `qwen3.5-plus` / `qwen3-max` | `qwen3.5-flash` / `qwen-turbo` | ✅ `enable_thinking` bei `qwen3-max` | ⚠️ nur qwen3.5 | Stärkste Unterstützung für die chinesische Sprache; qwq/Reasoning nur für Text |
| **ChatGLM (Zhipu)** | `glm-4.7` | `glm-4.7-flash` | `glm-5` | ⚠️ GLM-4.6V | Erzwungenes FC wird nicht unterstützt; Vision erfordert ein separates VLM-Modell |
| **MiniMax** | `MiniMax-M2.7` | `MiniMax-M2.5` | ❌ | ❌ | Nur Text; M2.7 ist das neueste Modell (März 2026); 80,2 % SWE-Bench |
| **Kimi (Moonshot)** | `kimi-k2.5` | `kimi-k2` | ✅ `kimi-k2-thinking` | ⚠️ nur K2.5 | K2-thinking unterstützt nur Text; erzwungenes FC wird bei Verwendung von Thinking nicht unterstützt |
| **Ollama (lokal)** | `qwen3.5` / `llama4` | `qwen3.5:9b` | ❌ | Variiert | Vollständig offline, kein API-Schlüssel erforderlich; Llama 4 unterstützt Vision |

<Tip>
**Vision** gibt an, ob das Modell Bildeingaben akzeptiert. Dies ist für [Intelligent Document Processing (IDP)](/concepts/document-processing) erforderlich — wenn dein Modell Vision nicht unterstützt, fällt IDP auf eine reine Textextraktion zurück. Bei mit ⚠️ gekennzeichneten Anbietern wird Vision von einigen Modellen unterstützt, von anderen jedoch nicht. Prüfe daher das von dir verwendete spezifische Modell.

Chat-Anhänge folgen derselben Kennzeichnung: Bei einem reinen Textmodell wird ein angehängtes Bild nicht gesendet, und das Modell erhält nur dessen Dateinamen. Der Chat-Composer weist dich vor dem Senden darauf hin, basierend auf dem Modell, das für den Durchlauf tatsächlich verwendet würde (Agent-Einstellung, dann die aktive Modellgruppe, dann der Systemstandard). Dokumenttext ist davon nicht betroffen — der Inhalt einer PDF- oder DOCX-Datei wird weiterhin extrahiert und eingefügt.
</Tip>

Diese Tabelle führt die von uns empfohlenen Kombinationen auf, nicht die vollständige Liste der von FIM One unterstützten Anbieter. xAI (Grok), ByteDance Doubao, Mistral und jedes OpenAI-kompatible Relay funktionieren ebenfalls. Eine vollständige Liste sowie Informationen zur jeweiligen Routenführung findest du in der [Anbieter-Fähigkeitsmatrix](/architecture/llm-provider-guide#provider-capability-matrix).

---

## Strukturierte Ausgabekompatibilität

FIM Ones DAG-Planer benötigt, dass das Modell gültiges strukturiertes JSON zurückgibt. Intern versucht es drei Extraktionsebenen in dieser Reihenfolge:
...
3. **Plain Text Extraction** — analysiert JSON aus Freitext als letzten Ausweg.

Modelle, die Level 1 (natives FC mit erzwungenem `tool_choice`) unterstützen, bieten die beste Planungszuverlässigkeit. Wenn ein Modell nur Level 2 erreicht, hängt seine Ausgabequalität davon ab, wie gut es den Anweisungen folgt — schwächere Modelle können gültiges JSON produzieren, das nicht der erwarteten Struktur entspricht.

| Anbieter | Erzwungenes Function Calling | JSON Mode | Planungszuverlässigkeit |
|---|---|---|---|
...
| **xAI** (Grok 4.1) | ✅ Vollständige Unterstützung | ✅ | ⭐⭐ Gut |
| **DeepSeek** (V3.2) | ✅ auf `deepseek-chat`; ❌ auf `deepseek-reasoner`, dessen Thinking-Modus lehnt es ab | ✅ | ⭐⭐ Gut |
| **Qwen** (3.x) | ✅ Unterstützt | ✅ | ⭐⭐ Gut |
| **ByteDance** (Doubao Seed) | ✅ Vollständige Unterstützung | ✅ | ⭐⭐ Gut |
| **Kimi** (K2.5) | ⚠️ Mit Thinking nur `auto` unterstützt; erzwungene Tool-Wahl benötigt Thinking aus (`kimi-k2`) | ✅ | ⭐ Befriedigend — kann fehlerhafte Pläne produzieren |
| **ChatGLM** (GLM-4.7/5) | ❌ Nicht unterstützt (`auto` nur) | ✅ | ⭐ Befriedigend |
| **MiniMax** (M2.5/M2.7) | ✅ Vollständige Unterstützung, Thinking zum Trotz | ✅ | ⭐⭐ Gut |
| **Lokal (Ollama)** | Variiert je nach Modell | Variiert | ⭐ Befriedigend — 32B+ empfohlen |

Diese Tabelle ist eine Entscheidungshilfe. Die autoritative, codegebundene Version, einschließlich welche `tool_choice`-Zustände jeder Anbieter akzeptiert und was FIM One tut, wenn einer abgelehnt wird, ist die [Provider Capability Matrix](/architecture/llm-provider-guide#provider-capability-matrix).

<Tip>
Wenn Sie den Fehler „failed to generate a valid task plan" sehen, ist die strukturierte Ausgabefähigkeit des Modells für DAG-Planung unzureichend. Wechseln Sie Ihr **Main LLM** zu einem Modell mit einer Bewertung von ⭐⭐⭐ oder ⭐⭐ oder höher, oder deaktivieren Sie den DAG-Modus und verwenden Sie stattdessen den einfacheren ReAct-Agenten.
</Tip>
...
| Anbieter | Aktivierung | Deaktivierbar? | Thinking-Ausgabe | Erzwungener FC-Konflikt? |
|---|---|---|---|---|
| **OpenAI** (GPT-5.x) | `reasoning_effort` param | ✅ Opt-in | Intern (nicht für Benutzer sichtbar) | ✅ Tools + Reasoning laufen zusammen über die Responses API (automatisch; chat-completions Fallback erzwingt `reasoning_effort: "none"`) |
...
| **Google Gemini** (2.5/3.x) | `reasoning_effort` param | ✅ Opt-in | Intern | ✅ Kein Konflikt |
| **DeepSeek** | Modellvariante (`deepseek-reasoner`) | Auf Modellebene | API `reasoning_content` Feld → Reasoning-Panel | ❌ Bei `deepseek-reasoner` abgelehnt, bei `deepseek-chat` akzeptiert |
| **Qwen** (3.x) | `enable_thinking` param, anbieterseitig gesetzt | ✅ Opt-in | `<think>` Tags im Inhalt | ✅ Kein Konflikt |
| **MiniMax** (M2.7) | Immer aktiviert | ❌ | `<think>` Tags im Inhalt | ✅ Kein Konflikt |
| **ChatGLM** (GLM-5) | Modellvariante | Auf Modellebene | Nicht externalisiert | ❌ Erzwungener FC nicht unterstützt, mit oder ohne Thinking |
| **Kimi** (K2-thinking) | Modellvariante | Auf Modellebene | API-Feld | ❌ Mit aktiviertem Thinking nur `auto` unterstützt |

Die anbieterorientierte Detailinformation hinter dieser Tabelle, einschließlich der Übersetzung jeder `effort`-Stufe und ob Reasoning in späteren Durchläufen wiedergegeben wird, befindet sich in [Tabelle C der Provider-Capability-Matrix](/architecture/llm-provider-guide#provider-capability-matrix).
...
<Note>
Wenn Sie ein Modell verwenden, das immer denkt (MiniMax M2.7, `deepseek-reasoner`) als Ihr Main LLM, wird die Thinking-Ausgabe in jedem Reasoning-Panel der Agent-Iteration angezeigt. Dies ist normal — es beeinträchtigt die Funktionalität nicht, und Sie können den Reasoning-Prozess des Modells sehen.
</Note>
...
**Empfohlene Modelle:**
- Main: `gpt-5.4` (neuestes Flaggschiff, März 2026 — 1M+ Kontext, Computer Use) oder `o3` (beste Reasoning-Genauigkeit)
- Fast: `gpt-5.4-mini` ($0,75/$4,50 pro MTok) oder `gpt-5.4-nano` (günstigste mit $0,20/$1,25 pro MTok)
- Budget Fast: `gpt-5-mini` ($0,25/$2,00) und `gpt-5-nano` ($0,05/$0,40) bleiben zu niedrigeren Preisen verfügbar
- Legacy: `gpt-4.1` (noch in der API, 1M Kontext, gut für Coding)
...
Claude zeichnet sich durch differenzierte Argumentation und komplexe mehrstufige Aufgaben aus. FIM One verbindet sich über [LiteLLM](https://github.com/BerriAI/litellm), das Anthropic-Modelle automatisch über deren native API weiterleitet. Die aktuelle Generation ist Claude 4.6 (Februar 2026).
...
- **Anthropic nativer Endpunkt** bietet dir ordnungsgemäße `reasoning_content`-Unterstützung (erweitertes Denken sichtbar in der UI), korrektes Tool-Calling-Format und `x-api-key`-Authentifizierung — Funktionen, die bei Verwendung der OpenAI-kompatiblen Übersetzung verloren gehen.
- **Google nativer Endpunkt** bietet dir native Gemini-Parameter und `x-goog-api-key`-Authentifizierung.
- **OpenAI kompatibel** ist der universelle Fallback und funktioniert mit jedem Relay, aber anbieterspezifische Funktionen (wie die Ausgabe des erweiterten Denkens) sind möglicherweise nicht verfügbar.
...
Relays schlagen auch auf Weise fehl, die ein direkter Anbieter nicht tut, und die meisten dieser Fehler sind stumm: ein gelöschter Parameter, ein entfernter Cache-Breakpoint, ein gepufferter Stream, der nie einen Fehler verursacht. Die Symptom-für-Symptom-Liste findest du unter [Relay/Proxy-Fallstricke](/architecture/llm-provider-guide#relayproxy-gotchas).
