...
|---|---|---|
| **Anbieter** | Ein Satz gemeinsamer Anmeldedaten (API-Schlüssel + Basis-URL). Ein Anbieter kann viele Modelle hosten. | "Mein OpenAI-Konto", "Company Bedrock Relay" |
...
Jedes Modell hat erweiterte Einstellungen, die steuern, wie FIM One mit der API des Anbieters für die Extraktion strukturierter Ausgaben interagiert. Diese Einstellungen finden Sie unter dem Schalter **Advanced** im Dialog zum Erstellen/Bearbeiten von Modellen.
...
Steuert, ob FIM One erzwungenes `tool_choice` für die Extraktion strukturierter Ausgaben verwendet. Dies ist Level 1 in der Degradationskette für strukturierte Ausgaben -- die zuverlässigste Methode, wenn das Modell dies unterstützt.

**Wann deaktivieren:**
- Ihr Modell gibt Fehler wie `"tool_choice 'specified' is incompatible with thinking enabled"` zurück -- häufig bei Modellen mit ständig aktiviertem Denken (DeepSeek R1, Kimi K2.5)
- Anfragen für strukturierte Ausgaben sind konsistent langsam mit einer Strafe von etwa 10 Sekunden pro Aufruf, gefolgt von einem Fallback zu JSON Mode

**Auswirkung bei Deaktivierung:** FIM One überspringt Level 1 (Native Function Calling) und startet bei Level 2 (JSON Mode) für strukturierte Ausgaben. Der ReAct-Agent's Tool-Aufrufe sind völlig unbeeinträchtigt -- er verwendet `tool_choice="auto"`, das mit allen Modellen unabhängig von dieser Einstellung funktioniert.

<Note>
Diese Einstellung betrifft nur **erzwungene** Tool-Auswahl, die für die Extraktion strukturierter Ausgaben verwendet wird (DAG-Planung, Schema-Annotation). Sie betrifft **nicht** den ReAct-Agent, der frei entscheidet, wann Tools mit `tool_choice="auto"` aufgerufen werden.
</Note>

Technische Details finden Sie unter [LLM-Provider-Kompatibilität -- tool_choice_enabled](/architecture/llm-provider-guide#the-fix-tool_choice_enabled).
...
**Standard:** 0.7 (geerbt von der globalen Einstellung, wenn nicht gesetzt)
...
Die maximale Anzahl von Tokens, die das Modell in einer einzelnen Antwort generieren kann. Lassen Sie dieses Feld leer, um den Systemstandard (64.000) zu verwenden. Für lokale Modelle mit begrenztem VRAM setzen Sie diesen Wert explizit auf einen niedrigeren Wert (z. B. 8192).

### Kontextgröße

Die Größe des Kontextfensters des Modells in Token. Lassen Sie dieses Feld leer, um den Systemstandard (128.000) zu verwenden. Stellen Sie dies auf die tatsächliche Kapazität Ihres Modells ein – bei lokalen Modellen liegt dies oft zwischen 4K und 32K, je nach Modell und verfügbarem Speicher.
...
| `deepseek-reasoner` | Reasoning | 128K | 64K | **AUS** | AN | Thinking immer aktiviert; erzwungene tool_choice abgelehnt; 64K enthält CoT |
...
| `qwq-plus` | Reasoning | 131K | 16K | AN | AN | Reasoning + Funktionsaufrufe; Denken umschaltbar über enable_thinking |
...
| Modell | Rolle | Kontext | Max. Ausgabe | Native FC | JSON-Modus | Hinweise |
|---|---|---|---|---|---|---|
| `kimi-k2.5` | Allgemein | 262K | 65K | **AUS** | AN | FC funktioniert, aber erzwungene tool_choice abgelehnt, wenn Denken aktiviert ist (Standard) |
...
<Tip>
**Diagnose:** Überprüfen Sie Ihre Anwendungsprotokolle auf Warnungen `structured_llm_call: native_fc call raised`. Wenn Sie diese Warnungen gefolgt von erfolgreicher JSON-Modus-Extraktion sehen, profitiert das Modell nicht von nativen Funktionsaufrufen. Deaktivieren Sie **Native Funktionsaufrufe** für dieses Modell, um den verschwendeten API-Aufruf und die Latenzstrafe von ~10 Sekunden pro strukturierter Ausgabeanfrage zu vermeiden.
</Tip>
...
<Steps>
  ...
  <Step title="Weisen Sie Modelle Rollen zu">
    Wählen Sie für jede Rolle (General, Fast, Reasoning) ein Modell aus dem Dropdown-Menü aus. Das Dropdown-Menü zeigt alle aktiven Modelle von aktiven Anbietern, gruppiert nach Anbieter-Name. Sie können eine Rolle unzugewiesen lassen – sie wird dann auf das General-Modell zurückgreifen (oder auf ENV-konfigurierte Modelle, wenn General auch unzugewiesen ist).
  </Step>
  <Step title="Speichern">
...
Um eine Modellgruppe zu aktivieren, verwenden Sie das Dropdown-Menü oder die Aktivierungskontrolle auf der Seite „Modelle". Es kann jeweils nur eine Gruppe aktiv sein. Die Aktivierung einer Gruppe wendet ihre Modellzuweisungen sofort auf alle neuen Konversationen an.

Um die aktuelle Gruppe zu deaktivieren (Rückgriff auf ENV-konfigurierte Modelle), wählen Sie die Deaktivierungsoption.

<Warning>
Das Wechseln der aktiven Modellgruppe beeinflusst **alle neuen Konversationen** systemweit. Bestehende laufende Konversationen verwenden weiterhin das Modell, das aktiv war, als sie gestartet wurden.
</Warning>
...
|---|---|
| General | `LLM_MODEL` |
| Fast | `FAST_LLM_MODEL` (greift auf `LLM_MODEL` zurück) |
| Reasoning | `REASONING_LLM_MODEL` (greift auf `LLM_MODEL` zurück) |

Vom Administrator konfigurierte Modelle haben immer Vorrang vor ENV-Variablen. Die Systemzustandsprüfung berücksichtigt beide Quellen – solange entweder eine aktive Modellgruppe oder gültige ENV-Variablen konfiguriert sind, meldet das LLM-Subsystem einen fehlerfreien Status.

Die vollständige ENV-Referenz finden Sie unter [Environment Variables](/configuration/environment-variables#llm-required).

## Export und Import

Die Seite „Models" unterstützt den Export Ihrer gesamten Anbieter- und Modellkonfiguration (Anbieter, Modelle und Gruppen) als JSON-Datei und den Import auf einer anderen Instanz. Dies ist nützlich für:
