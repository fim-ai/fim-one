...
- [ ] **Webhook-Callbacks** — Webhook-URLs pro API-Schlüssel registrieren; POST-Benachrichtigungen für Gesprächsvervollständigung, Agenten-Fehler und asynchrone Task-Ergebnisse erhalten
...
- [ ] **Batch-/asynchrone API** — `POST /api/batch` akzeptiert bis zu 100 Abfragen; gibt `batch_id` für Polling-Ergebnisse zurück; nützlich für Bulk-KB-Abfragen oder Multi-Agenten-Orchestrierung
- [ ] **Circuit Breaker pro externe Abhängigkeit** — Verhindert kaskadierende Ausfälle, wenn nachgelagerte LLM-Provider oder Konnektoren nicht verfügbar sind; automatisches Fallback und Wiederherstellung

- [ ] **Agent Trace Layer (Observability++)**: Anwendungsebenen-Run/Trace/Thread-Hierarchie für Agenten-Debugging — jedes Gespräch → `Trace`, jeder LLM-Aufruf/Tool-Aufruf/DAG-Schritt → `Span` mit Input/Output/Tokens/Timing. Frontend-Trace-Viewer mit Timeline und erweiterbaren LLM-Aufruf-Payloads. Dies geht über OTel (Infrastrukturebene) hinaus, um umsetzbares Agenten-Loop-Debugging für Entwickler und Enterprise-Kunden bereitzustellen. OpenTelemetry-Export als Daten-Sink-Option. Modelliert nach LangSmith's Run/Trace/Thread-Konzepten — das branchenbewährte Muster für Agenten-Observability.
- [ ] **Metrics-Dashboard**: Latenz, Erfolgsquote, Token-Nutzung, Konnektoren-Call-Analytik — pro-Agent, pro-Benutzer, pro-Org-Aufschlüsselungen
...
- [ ] **Intelligente Dokumentenverarbeitung (Vision-Aware)**: Adaptive Dokumentenbehandlung basierend auf Modell-Fähigkeiten — wenn das Ziel-LLM Vision unterstützt (Claude, GPT-4o, Gemini), senden Sie PDF/DOCX-Seiten als gerenderte Bilder über Document/Image-Content-Blöcke (Claude-Stil natives PDF-Handling) für volle visuelle Treue (Layout, Diagramme, Abbildungen, Fotos). Wenn das Modell nur Text ist, fallen Sie auf aktuelle pdfplumber/markitdown-Textextraktion zurück. Für DOCX/PDF mit eingebetteten Bildern: extrahieren Sie Bilder separat und leiten Sie sie durch Vision-Content-Blöcke statt Base64 in Text zu dumpen. Drei Stufen: (1) **Vision-first** — Seiten als Bilder rendern, als Vision-Content senden (höchste Treue, höhere Token-Kosten); (2) **Hybrid** — Textextraktion + separate Bildextraktion für eingebettete Bilder (ausgewogen); (3) **Text-only** — aktuelles pdfplumber/markitdown-Verhalten (niedrigste Kosten, nur-Text-Modelle). Modell-Fähigkeitserkennung über Provider-Registry (`supports_vision`-Flag). Benutzerorientierte Modusauswahl: „Precise" (Vision) vs „Fast" (Textextraktion) mit Auto-Detect-Standard
- [ ] **Cross-Session-Dateiverwaltung**: Dateibrowser-UI, Speicherquotas, Auto-Expiration-Cleanup
- [ ] **Session-Level-Datei-Zuordnungen**: Nachverfolgung, welche Dateien in welchen Gesprächen verwendet wurden
- [ ] **Cross-Session-Gesprächsabruf**: Agent-Tools zum Auflisten, Durchsuchen und Lesen von Gesprächsverlauf — `list_conversations`, `search_conversations` (Schlüsselwort/Regex über Verlauf), `read_conversation` (vollständigen Thread abrufen). Ermöglicht „was haben wir letztes Mal besprochen" und „finde die API-Änderung, die ich letzte Woche gefragt habe"-Workflows. Gepaart mit Cross-Session-Dateiverwaltung, um eine vollständige langfristige Agent-Memory-Schicht zu bilden
...
- [ ] **Geplante Jobs + Event-getriggerte Agenten (Loop)**: Cron-ähnliche Background-Task-Trigger; `scheduled_jobs` + `job_runs` DB-Tabellen; APScheduler-Integration; Job-CRUD-API + Job-Verlauf-UI; Ergebnis-Benachrichtigung über Message-Push-Konnektoren. Umfang deckt sowohl zeitgesteuerte (Cron) als auch ereignisgesteuerte (Webhook-Inbound)-Muster ab — ein Agent, der asynchron im Hintergrund läuft, IST der Async-Sub-Agent-Use-Case für Hub-Modus.
- [ ] **Vorgefertigte Solution-Vorlagen (Market Seed Content)**: 8-10 einsatzbereite vertikale Solutions, die beim ersten Start auf Market veröffentlicht werden — Finance Audit, Contract Review, Data Reporting, IT Helpdesk, HR Onboarding, Sales Assistant, Content Writer, Meeting Summary. Jede ist ein vollständiges Agent + Skill + Konnektoren-Bundle; Benutzer abonnieren und forken zum Anpassen. Niedriger Engineering-Aufwand (Konfiguration + Skill-Authoring), hohe Benutzer-Akquisitions-Auswirkung
...
- [ ] **Resource Fork (Package Phase 1 — Voraussetzung für v1.0 Package System)**: Pro-Resource Clone/Fork-Endpunkte als atomare Bausteine für Package-Fork. Jeder `POST /api/{type}/{id}/fork` erstellt eine benutzergesteuerte tiefe Kopie der Ressourcen-Konfiguration, entkoppelt vom Original (kein Update-Link). **Test-Matrix**: jede Ressource muss sowohl für Market-Abonnement (Installation) als auch für Org-Level-Veröffentlichung getestet werden (zwei unterschiedliche Code-Pfade). Implementierungsreihenfolge nach Komplexität:
  1. **MCP Server Fork** — am einfachsten; Konfiguration kopieren (Befehl, Args, Env-Vorlage). MCP-Anmeldedaten pro Benutzer Override (`mcp_server_credentials`) bietet bereits atomare Anmeldedaten-Isolation — erweitern Sie dieses Muster
  ...
  5. **Workflow Fork** — Duplizierung existiert bereits; überprüfen Sie, ob es Node-Referenzen
