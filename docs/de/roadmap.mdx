...
- [x] PII-Spaltenmarkierung – markierte Spalten bleiben im Schema erhalten, Werte werden in jedem Abfrageergebnis maskiert
- [x] Schemasichtbarkeit – Allow-Deny-Regeln für Tabellen und Spalten sowie Verb-Blockierung (Erzwingung des Nur-Lese-Modus)
- [x] Auditierbarkeit der Schranken – `scope_rules_applied` in `ConnectorCallLog` erfasst die Schranken, unter denen jeder Aufruf ausgeführt wurde
- [ ] Konfigurationsweitergabe pro Hook (`{"name", "config"}`-Schema) – der Träger für ScopeGuard-Regeln {/* dev: dev/hook-system.md */}
