...
- [x] ~~PreToolUse-Erzwingungshooks schließen bei Fehler fehl — ein abstürzender Genehmigungsgateway blockiert den Aufruf nicht mehr stillschweigend; Nicht-Erzwingungshooks behalten Fail-Open über `fail_open`~~ *(in v0.8.8 ausgeliefert)*
- [x] ~~Skeleton + FeishuGateHook + Approval Playground + ReAct/DAG-Laufzeit~~ *(in v0.8.5 ausgeliefert)*
...
- [ ] Nutzungsstatistik-Schreibpufferung für High-QPS-Schlüssel (prozesslokal Flush-on-Shutdown, exakte Zählungen, keine neue Abhängigkeit) — aufgeschoben bis zu einem gemessenen Hotspot {/* dev: dev/public-api-phase2.md */}
...
#### Prompt-Cache + Reasoning-Folgeabfragen {/* dev: dev/prompt-cache-followups.md */}

- [x] ~~Reasoning-Detail end-to-end bewahrt — Loop streamt seine echte finale Antwort (keine verlustbehaftete Resynthese); Reasoning übersteht Komprimierung, Kontext-Neuaufbau und Sub-Agent-Schritte~~ *(ausgeliefert in v0.8.8)*
...
- [ ] **Geplante Jobs + Event-gesteuerte Agents** — `scheduled_jobs` + `job_runs` + APScheduler; cron + webhook-inbound. Async Sub-Agent-Anwendungsfall für Hub-Modus
