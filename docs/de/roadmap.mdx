...
- Extended Thinking / Reasoning-Unterstützung (`LLM_REASONING_EFFORT`, `LLM_REASONING_BUDGET_TOKENS`) für OpenAI o-Serie, Gemini 2.5+, Claude
...
- SSE v2 Frontend: Streaming-Punkt-Puls-Cursor, DAG-Neuplanung-Runden-Snapshots als einklappbare Karten, DAG-Layout entkoppelt von Schrittzuständen
- Konzeptdokumentationsseite für AI Builder mit Connector- und Agent-Builder-Leitfäden
- Organisationssystem: vollständiges CRUD mit rollenbasierter Mitgliedschaft (Owner/Admin/Member), Admin-Verwaltungs-UI
- Drei-Ebenen-Ressourcensichtbarkeit (persönlich/org/global) für Agents, Connectors, Knowledge Bases, MCP-Server
- Veröffentlichungs-/Unveröffentlichungs-API für alle Ressourcentypen; Owner-Delegation für veröffentlichte Agents
...
- Drei Modellrollen (General/Fast/Reasoning) mit Pro-Tier-Umgebungskonfiguration-Isolation; Fast-Modell erbt nicht mehr Hauptmodell-Einstellungen
...
- Auto-Routing: Fast-LLM klassifiziert Abfragen als ReAct oder DAG; `/api/auto`-Endpoint; Frontend 3-Wege-Modus-Toggle (`AUTO_ROUTING`)
- [x] ~~**Shadow Market Organization + Resource Subscriptions**~~: Integrierte Market-Org (Shadow, kein Auto-Join) ersetzt Platform-Org; Ressourcen über Marketplace-Browsing entdeckt und explizit abonniert (Pull-Modell); Market-API zum Abonnieren gemeinsamer Ressourcen; Veröffentlichung auf Market erfordert immer Überprüfung; Resource-Subscriptions-Tabelle; Org-basierte Ressourcenfreigabe ersetzt globale Sichtbarkeit
...
- [x] ~~**MCP Server Credentials + Per-User Override**~~: `mcp_server_credentials`-Tabelle; `PUT /api/mcp-servers/{id}/my-credentials`-Endpoint; `allow_fallback`-Flag für Credential-Fallback-Verhalten
...
- [x] ~~**Standalone KB Conversations**~~: `kb_ids`-Feld auf Conversations für direkten KB-Chat ohne Agent-Bindung
...
- [ ] Nutzungsstatistik-Schreibpufferung für High-QPS-Schlüssel (prozesslokal Flush-on-Shutdown, exakte Zählungen, keine neuen Abhängigkeiten) — aufgeschoben bis zu einem gemessenen Hotspot {/* dev: dev/public-api-phase2.md */}
