...
- [x] Endgültige Antworten werden nativ gestreamt — die Schleife übergibt über ein `finish`-Signal und die Antwort wird als live token-gestreamt Turn geschrieben
- [x] Antworten rendern Mermaid-Diagramme, SVG-Figuren und kartenförmige Vergleichstabellen mit Kopier-/Exportfunktion auf Antworten, Code-Blöcken und Tabellen
- [x] Gerendertes Markdown wird bereinigt, wodurch Raw-HTML-Injection aus Modellausgabe und hochgeladenen Dateien geschlossen wird
- [x] Diagramme und Code-Blöcke können als Dateien heruntergeladen werden; Reasoning wird in Live- und früheren Gesprächen standardmäßig zu einzeiligen Vorschauen eingeklappt
