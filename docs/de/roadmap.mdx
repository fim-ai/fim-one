...
Neu geplant am 2026-09-06. Der Kern ist fertiggestellt: ReAct-Engine, Konnektoren,
Zugangsdaten, Freigabegate, Audit und mandantenfähige Organisationen. Von hier an geht es
darum, das, was bereits in der Codebasis vorhanden ist, fertigzustellen und korrekt zu halten,
nicht darum, neue Funktionsbereiche zu eröffnen. Alles, was unten nicht aufgeführt ist, ist nicht
geplant; siehe den abschließenden Hinweis.

### v0.9 — Sicher zum Teilen

**Ziel**: Ein Datenbank-Connector kann sicher an einen Kollegen übergeben werden, das Zurücknehmen nimmt ihn tatsächlich zurück, und die halbfertigen Pfade, die bereits im Tree vorhanden sind, werden abgeschlossen.

#### DB-Connector-Schranken {/* dev: dev/connector-rbac/00-overview.md */}

- [x] Markierung von PII-Spalten — markierte Spalten bleiben im Schema, Werte werden in jedem Abfrageergebnis maskiert
...
Wenn eine Funktionalität oben nicht aufgeführt ist, ist sie nicht geplant. Das ist eine bewusste Entscheidung und kein Versehen: Das Projekt befindet sich im Wartungsmodus, und die Arbeit wird danach priorisiert, was tatsächlich benötigt wird, statt danach, was die Funktionsmatrix abrunden würde.

Wenn dir etwas fehlt, eröffne ein Issue und beschreibe das Problem, an dem du festhängst. Ein konkreter blockierter Anwendungsfall ist der Grund, aus dem ein Punkt auf diese Seite gelangt.
