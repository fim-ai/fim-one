...
- [x] Shared LLM HTTP pool self-heals after a LiteLLM client-cache eviction closes it — calls recreate the pool transparently instead of failing indefinitely until restart
...
- [ ] DAG `tools_used` Genauigkeit (echte Toolnamen aus Per-Step ReAct über Step-Completion-Callback)
...
- [x] Reasoning-Detail vollständig erhalten — Loop streamt seine echte finale Antwort (keine verlustbehaftete Neusynthese); Reasoning übersteht Komprimierung, Kontext-Neuerstellungen und Sub-Agent-Schritte
