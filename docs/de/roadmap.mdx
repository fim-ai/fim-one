...
- [x] ~~Plan board: `update_plan` todo tool + stale-plan reminder re-injection keeps long ReAct runs on-goal and survives compaction~~ *(shipped in v0.8.9)*
- [x] ~~LLM-call resilience: output-truncation continuation + 529/504 retry with Retry-After (overload model fallback already shipped as `FallbackLLM`)~~ *(shipped in v0.8.9)*
- [x] ~~Budget-truncated tool results persist to workspace before cutting; full transcript snapshot before lossy compaction~~ *(shipped in v0.8.9)*
- [x] ~~Background tool execution in ReAct — whitelisted slow tools return a task id and notify on completion~~ *(shipped in v0.8.9)*
- [x] ~~Incremental DAG replan: completed steps carry across replan rounds; step-level checkpoint resumes interrupted runs on retry~~ *(shipped in v0.8.9)* {/* dev: dev/incremental-dag.md */}
- [x] ~~Compaction preserves tool_use/tool_result pairing — long tool-heavy runs no longer die on provider 400 after compact; background tool results reach the step stream + DAG evidence~~ *(shipped in v0.8.9)*
