...
This is the per-model operator view: which toggles to set when you add a model. The provider-level rules it follows from, with the code anchors that implement them, are in the [Provider Capability Matrix](/architecture/llm-provider-guide#provider-capability-matrix). If the two ever disagree, the matrix is authoritative and this table needs updating.
