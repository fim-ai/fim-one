...
When `LLM_REASONING_EFFORT` is set, FIM Agent enables the model's extended thinking capability so the internal chain-of-thought is surfaced in the UI "thinking" step. FIM Agent uses [LiteLLM](https://github.com/BerriAI/litellm) to translate the reasoning effort parameter into each provider's native format automatically.
...
| `DOCKER_HOST_DATA_DIR` | No | *(not set)* | Host-side absolute path of the `./data` volume mount. Required for Docker-in-Docker deployments; `docker-compose.yml` sets this automatically. |
...
| Variable | Required | Default | Description |
|---|---|---|---|
| `DATABASE_URL` | No | `sqlite+aiosqlite:///./data/fim_agent.db` | Database connection string. **SQLite** (zero-config): `sqlite+aiosqlite:///./data/fim_agent.db`. **PostgreSQL** (production): `postgresql+asyncpg://user:pass@localhost:5432/fim_agent`. Docker Compose auto-sets PostgreSQL. |
