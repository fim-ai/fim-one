...
### Keeping the ENV fallback in step with the admin panel

Once a model group is active in Admin → Models, the runtime takes its models from that group and the `LLM_*` / `FAST_LLM_*` / `REASONING_LLM_*` values above are only the fallback. Side paths that never open the database still read them directly: `scripts/translate.py` (the i18n pre-commit hook), `evals/`, the Jina/OpenAI reranker, and image generation. To stop those from drifting onto a stale provider, copy the active group into the env file:
...
The script reads the provider keys through the ORM and writes them straight to the file; nothing is echoed. Restart the app afterwards.

### MarkItDown OCR Resolution

The `convert_to_markdown` built-in tool and the RAG ingestion pipeline both use Microsoft's [MarkItDown](https://github.com/microsoft/markitdown) + the official [`markitdown-ocr`](https://github.com/microsoft/markitdown/tree/main/packages/markitdown-ocr) plugin to extract text from documents — including OCR on embedded images and scanned PDF pages when a vision-capable LLM is available.
...
**Reasoning models are never preferred for OCR.** Reasoning tiers (`o1`, `o3-mini`, `DeepSeek-R1`) historically lack vision support and are the wrong tool for OCR anyway — OCR is a perception task, not deliberation. If a workspace has only a reasoning model with `supports_vision=True` it will still be picked up via the primary-LLM path, but the resolver does not actively rank it above fast/general.
...
Which providers accept thinking, how each one is switched on, which `effort` values it takes, and where the reasoning text ends up are recorded once, with code anchors, in [Table C of the Provider Capability Matrix](/architecture/llm-provider-guide#provider-capability-matrix). That table is the authoritative list; this page only documents the variables.

FIM One resolves the provider from `LLM_BASE_URL` (plus an explicit provider field when one is configured) and maps the request to the correct API format. Unknown URLs are treated as OpenAI-compatible.
...
Some providers restrict `temperature` when reasoning is active. **All of these are enforced automatically; leave `LLM_TEMPERATURE` at whatever suits your workload.**

- **Anthropic**: requires `temperature=1` while extended thinking is enabled. The request builder pins the value on Anthropic routes, so your configured temperature is overridden for those calls rather than rejected.
- **Anthropic, strict models** (Opus 4.7 and 4.8, Fable 5, Mythos 5): reject `temperature`, `top_p` and `top_k` outright, thinking or not. FIM One removes `temperature` from the request for these models.
- **OpenAI GPT-5.x**: only supports `temperature=1`. LiteLLM's `drop_params` filtering removes unsupported values.

Setting `LLM_TEMPERATURE=1` by hand to satisfy Anthropic is unnecessary and costs you the ability to run a lower temperature on every non-thinking call.

#### How `LLM_REASONING_BUDGET_TOKENS` works

This variable is **meaningful only on the legacy Anthropic thinking path** (Claude 4.5 and older, routed as `anthropic/`). There it overrides the auto-calculated budget and is sent as `budget_tokens` inside the `thinking` parameter. Adaptive-thinking models (Opus 4.6 and newer, Sonnet 4.6, Fable 5, Mythos 5) take an effort level instead of a budget and ignore this variable entirely. When it is not set, the budget is derived from `LLM_MAX_OUTPUT_TOKENS` x effort ratio:
...
Comma-separated names of guardrails that inspect the *content* of input or output. Independent of the tool-permission gate (`core/hooks/*`) and the security layer (`core/security/*`). See [Content Guardrails](/configuration/guardrails) for the full picture.
...
> **Quick start tip**: Setting just `JINA_API_KEY` enables the default web search stack, plus web fetch, embedding, and reranking — one key, four services. Switch search to Tavily, Brave, or Exa with `WEB_SEARCH_PROVIDER` and the matching API key.
...
| `SMTP_SSL` | No | `ssl` | TLS mode: `ssl` (port 465) / `tls` (STARTTLS, port 587) / `none` or `""` (plain, credentials sent in cleartext). Any other value is rejected rather than silently falling back to plain. |
...
| `FIM_BCRYPT_COST` | No | `12` | bcrypt work factor for password hashing, clamped to 4-31. Default 12 costs roughly 200ms per hash on modern CPUs; lower it on weak hardware, raise it for security-hardened deployments. |
...
| `EXPORT_FONT_DIR` | No | *(auto-detected)* | Directory holding `NotoSansSC-Regular.ttf` / `NotoSansSC-Bold.ttf` for PDF export. Fetch with `python scripts/fetch_export_fonts.py`; Docker images already bundle it. Without an embeddable TrueType CJK font, PDF export falls back to a non-embedded CID font with degraded spacing and no bold. |
