...
| Bedrock via OpenAI-compatible endpoint | `_run_native` | native_fc | Full support — no prefill issue; see tip above |
...
2. **GPT-5.x with tools**: FIM One first tries the OpenAI Responses API (via LiteLLM's chat-completions bridge), where tools and reasoning are allowed together and reasoning state carries across tool-call rounds. If the endpoint has no usable `/v1/responses` route (some OpenAI-compatible proxies), FIM One silently falls back to `/v1/chat/completions` and remembers the verdict per endpoint; on that fallback path, GPT-5.x with `tools` sends an explicit `reasoning_effort: "none"`, because the endpoint rejects the tools + reasoning combination and omitting the field is not equivalent (the server default is not `none`).
